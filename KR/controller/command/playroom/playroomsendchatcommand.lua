slot0 = class("PlayRoomSendChatCommand", pm.SimpleCommand)
...
slot0.execute = function(slot0, slot1)
	...
	if slot3 == PlayRoomChatConst.CHANNEL_PLAYROOM then
		for slot14 = #getProxy(PlayRoomProxy):GetChatMsgs(), 1, -1 do
			if slot10[slot14].playerId == slot7.id then
...
slot0.Send = function(slot0, slot1, slot2, slot3)
	...
	if slot1 == PlayRoomChatConst.CHANNEL_PLAYROOM then
		slot6 = pg.ConnectionMgr.GetInstance()

		slot6:Send(23023, {
			type = slot2,
			content = slot5
		}, 23024, function (slot0)
			if slot0.result ~= 0 then
				pg.TipsMgr.GetInstance():ShowTips(slot0.tip)
			end
		end)
	elseif slot1 == PlayRoomChatConst.CHANNEL_WORLD then
		slot0:sendNotification(GAME.SEND_MSG, slot5)
	elseif slot1 == PlayRoomChatConst.CHANNEL_GUILD then
