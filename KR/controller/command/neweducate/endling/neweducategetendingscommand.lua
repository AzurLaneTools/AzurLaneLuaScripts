...
slot0.execute = function(slot0, slot1)
	...
	pg.ConnectionMgr.GetInstance():Send(29003, {
		...
		if slot0.result == 0 then
			...
			slot2:SetSystemNo(NewEducateFSM.SYSTEM.ENDING)
			slot2:SetState(NewEducateFSM.SYSTEM.ENDING, NewEducateEndingState.New({
				select = 0,
				ends = slot0.endings
			}))
			existCall(uv0)
			...
			pg.TipsMgr.GetInstance():ShowTips("NewEducate_GetEndings: " .. slot0.result)
		end
	end)
end
