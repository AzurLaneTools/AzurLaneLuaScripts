	...
	pg.ConnectionMgr.GetInstance():Send(29030, {
		...
		if slot0.result == 0 then
			...
			uv1:sendNotification(GAME.NEW_EDUCATE_NEXT_NODE, {
				...
				drop = NewEducateDropHelper.HandleDrops(slot0.drop),
				noNextCb = function ()
					if uv0.next_node == 0 then
						if uv1:GetSystemNo() == NewEducateFSM.SYSTEM.PLAN then
							if uv1:GetState(NewEducateFSM.SYSTEM.PLAN):IsFinish() then
		...
		pg.TipsMgr.GetInstance():ShowTips("NewEducate_TriggerNode: " .. slot0.result)
	end)
end
