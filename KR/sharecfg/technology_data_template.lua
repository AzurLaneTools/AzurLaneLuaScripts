pg = pg or {}
pg.technology_data_template = {
	{
		id = 1,
		rarity = 1,
		time = 21600,
		type = 1,
		name = "기초 연구",
		lv_limit = 30,
		desc = "성정 큐브에 관한 기초 이론은 큐브 구조에 대한 인식으로부터 시작된다. 큐브 기초 구조에 대한 이론 구조 작업은 여전히 전력을 다해 진행 중이다.",
		condition = 0,
		bg = "1",
		blueprint_version = 1,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "item_1",
		label_color = "tag_c",
		consume = {},
		drop_client = {
			{
				2,
				52001,
				1
			},
			{
				2,
				52002,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	{
		id = 2,
		rarity = 1,
		time = 21600,
		type = 1,
		name = "기초 연구",
		lv_limit = 30,
		desc = "성정 큐브에 관한 기초 이론은 큐브 구조에 대한 인식으로부터 시작된다. 큐브 기초 구조에 대한 이론 구조 작업은 여전히 전력을 다해 진행 중이다.",
		condition = 0,
		bg = "1",
		blueprint_version = 1,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "item_1",
		label_color = "tag_c",
		consume = {},
		drop_client = {
			{
				2,
				52001,
				1
			},
			{
				2,
				52002,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	{
		id = 3,
		rarity = 2,
		time = 28800,
		type = 1,
		name = "기초 연구",
		lv_limit = 30,
		desc = "성정 큐브에 관한 기초 이론은 큐브 구조에 대한 인식으로부터 시작된다. 큐브 기초 구조에 대한 이론 구조 작업은 여전히 전력을 다해 진행 중이다.",
		condition = 0,
		bg = "2",
		blueprint_version = 1,
		sub_name = "대형 항목",
		label = "tag_red",
		bg_icon = "item_2",
		label_color = "tag_c",
		consume = {},
		drop_client = {
			{
				2,
				52001,
				1
			},
			{
				2,
				52002,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	{
		id = 4,
		rarity = 2,
		time = 28800,
		type = 1,
		name = "기초 연구",
		lv_limit = 30,
		desc = "성정 큐브에 관한 기초 이론은 큐브 구조에 대한 인식으로부터 시작된다. 큐브 기초 구조에 대한 이론 구조 작업은 여전히 전력을 다해 진행 중이다.",
		condition = 0,
		bg = "2",
		blueprint_version = 1,
		sub_name = "대형 항목",
		label = "tag_red",
		bg_icon = "item_2",
		label_color = "tag_c",
		consume = {},
		drop_client = {
			{
				2,
				52001,
				1
			},
			{
				2,
				52002,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[11] = {
		id = 11,
		rarity = 1,
		time = 14400,
		type = 1,
		name = "데이터 수집",
		lv_limit = 30,
		desc = "성정 큐브 구조에 관한 여러가지 가설에 대해 실증실험을 진행하는 연구 프로젝트. 실전을 통해 데이터를 수집하여, 이론의 과학적 증명에 기여하자.",
		condition = 52001,
		bg = "1",
		blueprint_version = 1,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "battle_1",
		label_color = "tag_b",
		consume = {},
		drop_client = {
			{
				2,
				52001,
				1
			},
			{
				2,
				52002,
				1
			},
			{
				2,
				52004,
				1
			}
		}
	},
	[12] = {
		id = 12,
		rarity = 1,
		time = 14400,
		type = 1,
		name = "데이터 수집",
		lv_limit = 30,
		desc = "성정 큐브 구조에 관한 여러가지 가설에 대해 실증실험을 진행하는 연구 프로젝트. 실전을 통해 데이터를 수집하여, 이론의 과학적 증명에 기여하자.",
		condition = 52002,
		bg = "1",
		blueprint_version = 1,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "battle_1",
		label_color = "tag_b",
		consume = {},
		drop_client = {
			{
				2,
				52001,
				1
			},
			{
				2,
				52002,
				1
			},
			{
				2,
				52004,
				1
			}
		}
	},
	[13] = {
		id = 13,
		rarity = 1,
		time = 14400,
		type = 1,
		name = "데이터 수집",
		lv_limit = 30,
		desc = "성정 큐브 구조에 관한 여러가지 가설에 대해 실증실험을 진행하는 연구 프로젝트. 실전을 통해 데이터를 수집하여, 이론의 과학적 증명에 기여하자.",
		condition = 52003,
		bg = "1",
		blueprint_version = 1,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "battle_1",
		label_color = "tag_b",
		consume = {},
		drop_client = {
			{
				2,
				52001,
				1
			},
			{
				2,
				52002,
				1
			},
			{
				2,
				52004,
				1
			}
		}
	},
	[14] = {
		id = 14,
		rarity = 1,
		time = 14400,
		type = 1,
		name = "데이터 수집",
		lv_limit = 40,
		desc = "성정 큐브 구조에 관한 여러가지 가설에 대해 실증실험을 진행하는 연구 프로젝트. 실전을 통해 데이터를 수집하여, 이론의 과학적 증명에 기여하자.",
		condition = 52004,
		bg = "1",
		blueprint_version = 1,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "battle_1",
		label_color = "tag_b",
		consume = {},
		drop_client = {
			{
				2,
				52001,
				1
			},
			{
				2,
				52002,
				1
			},
			{
				2,
				52004,
				1
			}
		}
	},
	[15] = {
		id = 15,
		rarity = 2,
		time = 14400,
		type = 1,
		name = "데이터 수집",
		lv_limit = 50,
		desc = "성정 큐브 구조에 관한 여러가지 가설에 대해 실증실험을 진행하는 연구 프로젝트. 실전을 통해 데이터를 수집하여, 이론의 과학적 증명에 기여하자.",
		condition = 52005,
		bg = "2",
		blueprint_version = 1,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "battle_2",
		label_color = "tag_b",
		consume = {},
		drop_client = {
			{
				2,
				52001,
				1
			},
			{
				2,
				52002,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[16] = {
		id = 16,
		rarity = 2,
		time = 14400,
		type = 1,
		name = "데이터 수집",
		lv_limit = 60,
		desc = "성정 큐브 구조에 관한 여러가지 가설에 대해 실증실험을 진행하는 연구 프로젝트. 실전을 통해 데이터를 수집하여, 이론의 과학적 증명에 기여하자.",
		condition = 52006,
		bg = "2",
		blueprint_version = 1,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "battle_2",
		label_color = "tag_b",
		consume = {},
		drop_client = {
			{
				2,
				52001,
				1
			},
			{
				2,
				52002,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[17] = {
		id = 17,
		rarity = 2,
		time = 14400,
		type = 1,
		name = "데이터 수집",
		lv_limit = 65,
		desc = "성정 큐브 구조에 관한 여러가지 가설에 대해 실증실험을 진행하는 연구 프로젝트. 실전을 통해 데이터를 수집하여, 이론의 과학적 증명에 기여하자.",
		condition = 52007,
		bg = "2",
		blueprint_version = 1,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "battle_2",
		label_color = "tag_b",
		consume = {},
		drop_client = {
			{
				2,
				52001,
				1
			},
			{
				2,
				52002,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[18] = {
		id = 18,
		rarity = 2,
		time = 14400,
		type = 1,
		name = "데이터 수집",
		lv_limit = 70,
		desc = "성정 큐브 구조에 관한 여러가지 가설에 대해 실증실험을 진행하는 연구 프로젝트. 실전을 통해 데이터를 수집하여, 이론의 과학적 증명에 기여하자.",
		condition = 52008,
		bg = "2",
		blueprint_version = 1,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "battle_2",
		label_color = "tag_b",
		consume = {},
		drop_client = {
			{
				2,
				52001,
				1
			},
			{
				2,
				52002,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[21] = {
		id = 21,
		rarity = 1,
		time = 10800,
		type = 1,
		name = "연구 의뢰",
		lv_limit = 30,
		desc = "성정 큐브에 관한 연구는 [건조] 방면뿐만 아니라, 일상생활 이용 등 다양한 방면으로 진행되고 있다. 성정 큐브의 다방면 활용 가능성을 찾는 프로젝트에는 군의 조력이 필수불가결이다.",
		condition = 52021,
		bg = "1",
		blueprint_version = 1,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "item_1",
		label_color = "tag_t",
		consume = {},
		drop_client = {
			{
				2,
				52002,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[22] = {
		id = 22,
		rarity = 1,
		time = 14400,
		type = 1,
		name = "연구 의뢰",
		lv_limit = 30,
		desc = "성정 큐브에 관한 연구는 [건조] 방면뿐만 아니라, 일상생활 이용 등 다양한 방면으로 진행되고 있다. 성정 큐브의 다방면 활용 가능성을 찾는 프로젝트에는 군의 조력이 필수불가결이다.",
		condition = 52022,
		bg = "1",
		blueprint_version = 1,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "item_1",
		label_color = "tag_t",
		consume = {},
		drop_client = {
			{
				2,
				52002,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[23] = {
		id = 23,
		rarity = 2,
		time = 21600,
		type = 1,
		name = "연구 의뢰",
		lv_limit = 30,
		desc = "성정 큐브에 관한 연구는 [건조] 방면뿐만 아니라, 일상생활 이용 등 다양한 방면으로 진행되고 있다. 성정 큐브의 다방면 활용 가능성을 찾는 프로젝트에는 군의 조력이 필수불가결이다.",
		condition = 52023,
		bg = "2",
		blueprint_version = 1,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "item_2",
		label_color = "tag_t",
		consume = {},
		drop_client = {
			{
				2,
				52002,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[31] = {
		id = 31,
		rarity = 1,
		time = 7200,
		type = 1,
		name = "샘플 조달",
		lv_limit = 30,
		desc = "성정 큐브와 [장비]의 관련성을 조사하는 다수의 시도는, 방대한 실험 데이터에 기반하여 진행되고 있다. 불필요한 장비를 제공하여, 실험 샘플을 충당하자.",
		condition = 52031,
		bg = "1",
		blueprint_version = 1,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "item_1",
		label_color = "tag_e",
		consume = {},
		drop_client = {
			{
				2,
				52002,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[32] = {
		id = 32,
		rarity = 2,
		time = 7200,
		type = 1,
		name = "샘플 조달",
		lv_limit = 30,
		desc = "성정 큐브와 [장비]의 관련성을 조사하는 다수의 시도는, 방대한 실험 데이터에 기반하여 진행되고 있다. 불필요한 장비를 제공하여, 실험 샘플을 충당하자.",
		condition = 52032,
		bg = "2",
		blueprint_version = 1,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "item_2",
		label_color = "tag_e",
		consume = {},
		drop_client = {
			{
				2,
				52002,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[33] = {
		id = 33,
		rarity = 2,
		time = 21600,
		type = 1,
		name = "새 장비 시안 작성 ",
		lv_limit = 50,
		desc = "현재 비치 중인 장비를 분해함으로써 기초 설계를 재검토하여, 추후 개발이 예정된 새 장비의 기초 연구를 진행한다. ",
		condition = 0,
		bg = "2",
		blueprint_version = 1,
		sub_name = "중형 항목",
		label = "tag_blue",
		bg_icon = "item_2",
		label_color = "tag_e",
		consume = {
			{
				2,
				30023,
				15
			}
		},
		drop_client = {
			{
				2,
				13001,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[34] = {
		id = 34,
		rarity = 2,
		time = 21600,
		type = 1,
		name = "새 장비 시안 작성 ",
		lv_limit = 50,
		desc = "현재 비치 중인 장비를 분해함으로써 기초 설계를 재검토하여, 추후 개발이 예정된 새 장비의 기초 연구를 진행한다. ",
		condition = 0,
		bg = "2",
		blueprint_version = 1,
		sub_name = "중형 항목",
		label = "tag_blue",
		bg_icon = "item_2",
		label_color = "tag_e",
		consume = {
			{
				2,
				30033,
				15
			}
		},
		drop_client = {
			{
				2,
				13003,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[35] = {
		id = 35,
		rarity = 2,
		time = 21600,
		type = 1,
		name = "새 장비 시안 작성 ",
		lv_limit = 50,
		desc = "현재 비치 중인 장비를 분해함으로써 기초 설계를 재검토하여, 추후 개발이 예정된 새 장비의 기초 연구를 진행한다. ",
		condition = 0,
		bg = "2",
		blueprint_version = 1,
		sub_name = "중형 항목",
		label = "tag_blue",
		bg_icon = "item_2",
		label_color = "tag_e",
		consume = {
			{
				2,
				30043,
				15
			}
		},
		drop_client = {
			{
				2,
				13004,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[36] = {
		id = 36,
		rarity = 2,
		time = 21600,
		type = 1,
		name = "새 장비 시안 작성 ",
		lv_limit = 50,
		desc = "현재 비치 중인 장비를 분해함으로써 기초 설계를 재검토하여, 추후 개발이 예정된 새 장비의 기초 연구를 진행한다. ",
		condition = 0,
		bg = "2",
		blueprint_version = 1,
		sub_name = "중형 항목",
		label = "tag_blue",
		bg_icon = "item_2",
		label_color = "tag_e",
		consume = {
			{
				2,
				30013,
				15
			}
		},
		drop_client = {
			{
				2,
				13005,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[37] = {
		id = 37,
		rarity = 2,
		time = 21600,
		type = 1,
		name = "새 장비 시안 작성 ",
		lv_limit = 50,
		desc = "현재 비치 중인 장비를 분해함으로써 기초 설계를 재검토하여, 추후 개발이 예정된 새 장비의 기초 연구를 진행한다. ",
		condition = 0,
		bg = "2",
		blueprint_version = 1,
		sub_name = "중형 항목",
		label = "tag_blue",
		bg_icon = "item_2",
		label_color = "tag_e",
		consume = {
			{
				2,
				30023,
				15
			}
		},
		drop_client = {
			{
				2,
				13006,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[38] = {
		id = 38,
		rarity = 2,
		time = 21600,
		type = 1,
		name = "새 장비 시안 작성 ",
		lv_limit = 50,
		desc = "현재 비치 중인 장비를 분해함으로써 기초 설계를 재검토하여, 추후 개발이 예정된 새 장비의 기초 연구를 진행한다. ",
		condition = 0,
		bg = "2",
		blueprint_version = 1,
		sub_name = "중형 항목",
		label = "tag_blue",
		bg_icon = "item_2",
		label_color = "tag_e",
		consume = {
			{
				2,
				30033,
				15
			}
		},
		drop_client = {
			{
				2,
				13008,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[41] = {
		id = 41,
		rarity = 1,
		time = 5400,
		type = 1,
		name = "비용 기부",
		lv_limit = 30,
		desc = "뛰어난 과학의 발전을 뒷받침하기 위해선 영민한 두뇌뿐만 아니라, 지원자로부터 제공되는 연구 자금도 필요하다. 남아도는 자금을 기부하여 기술의 연구를 서포트하자.",
		condition = 0,
		bg = "1",
		blueprint_version = 1,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "gold_1",
		label_color = "tag_g",
		consume = {
			{
				1,
				1,
				1500
			}
		},
		drop_client = {
			{
				2,
				52001,
				1
			},
			{
				2,
				52002,
				1
			},
			{
				2,
				52004,
				1
			}
		}
	},
	[42] = {
		id = 42,
		rarity = 1,
		time = 5400,
		type = 1,
		name = "비용 기부",
		lv_limit = 30,
		desc = "뛰어난 과학의 발전을 뒷받침하기 위해선 영민한 두뇌뿐만 아니라, 지원자로부터 제공되는 연구 자금도 필요하다. 남아도는 자금을 기부하여 기술의 연구를 서포트하자.",
		condition = 0,
		bg = "1",
		blueprint_version = 1,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "gold_1",
		label_color = "tag_g",
		consume = {
			{
				1,
				1,
				1500
			}
		},
		drop_client = {
			{
				2,
				52001,
				1
			},
			{
				2,
				52002,
				1
			},
			{
				2,
				52004,
				1
			}
		}
	},
	[43] = {
		id = 43,
		rarity = 1,
		time = 9000,
		type = 1,
		name = "비용 기부",
		lv_limit = 30,
		desc = "뛰어난 과학의 발전을 뒷받침하기 위해선 영민한 두뇌뿐만 아니라, 지원자로부터 제공되는 연구 자금도 필요하다. 남아도는 자금을 기부하여 기술의 연구를 서포트하자.",
		condition = 0,
		bg = "1",
		blueprint_version = 1,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "gold_1",
		label_color = "tag_g",
		consume = {
			{
				1,
				1,
				3000
			}
		},
		drop_client = {
			{
				2,
				52001,
				1
			},
			{
				2,
				52002,
				1
			},
			{
				2,
				52004,
				1
			}
		}
	},
	[44] = {
		id = 44,
		rarity = 1,
		time = 9000,
		type = 1,
		name = "비용 기부",
		lv_limit = 30,
		desc = "뛰어난 과학의 발전을 뒷받침하기 위해선 영민한 두뇌뿐만 아니라, 지원자로부터 제공되는 연구 자금도 필요하다. 남아도는 자금을 기부하여 기술의 연구를 서포트하자.",
		condition = 0,
		bg = "1",
		blueprint_version = 1,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "gold_1",
		label_color = "tag_g",
		consume = {
			{
				1,
				1,
				3000
			}
		},
		drop_client = {
			{
				2,
				52001,
				1
			},
			{
				2,
				52002,
				1
			},
			{
				2,
				52004,
				1
			}
		}
	},
	[45] = {
		id = 45,
		rarity = 2,
		time = 14400,
		type = 1,
		name = "비용 기부",
		lv_limit = 30,
		desc = "뛰어난 과학의 발전을 뒷받침하기 위해선 영민한 두뇌뿐만 아니라, 지원자로부터 제공되는 연구 자금도 필요하다. 남아도는 자금을 기부하여 기술의 연구를 서포트하자.",
		condition = 0,
		bg = "2",
		blueprint_version = 1,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "gold_2",
		label_color = "tag_g",
		consume = {
			{
				1,
				1,
				6000
			}
		},
		drop_client = {
			{
				2,
				52001,
				1
			},
			{
				2,
				52002,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[46] = {
		id = 46,
		rarity = 2,
		time = 14400,
		type = 1,
		name = "비용 기부",
		lv_limit = 30,
		desc = "뛰어난 과학의 발전을 뒷받침하기 위해선 영민한 두뇌뿐만 아니라, 지원자로부터 제공되는 연구 자금도 필요하다. 남아도는 자금을 기부하여 기술의 연구를 서포트하자.",
		condition = 0,
		bg = "2",
		blueprint_version = 1,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "gold_2",
		label_color = "tag_g",
		consume = {
			{
				1,
				1,
				6000
			}
		},
		drop_client = {
			{
				2,
				52001,
				1
			},
			{
				2,
				52002,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[51] = {
		id = 51,
		rarity = 1,
		time = 9000,
		type = 1,
		name = "인식 방향성 연구",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수의 사람들의 사물에 대한 인식의 집합체를 구현화하는 것이라면, 그것의 '방향성' 을 정하는 것에 따라 결과는 과연 어떻게 바뀔 것인가.",
		condition = 0,
		bg = "1",
		blueprint_version = 1,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "chara_haiwangxing",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				3000
			}
		},
		drop_client = {
			{
				2,
				42001,
				1
			},
			{
				2,
				52002,
				1
			},
			{
				2,
				52004,
				1
			}
		}
	},
	[52] = {
		id = 52,
		rarity = 1,
		time = 9000,
		type = 1,
		name = "인식 방향성 연구",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수의 사람들의 사물에 대한 인식의 집합체를 구현화하는 것이라면, 그것의 '방향성' 을 정하는 것에 따라 결과는 과연 어떻게 바뀔 것인가.",
		condition = 0,
		bg = "1",
		blueprint_version = 1,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "chara_junzhu",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				3000
			}
		},
		drop_client = {
			{
				2,
				42002,
				1
			},
			{
				2,
				52002,
				1
			},
			{
				2,
				52004,
				1
			}
		}
	},
	[53] = {
		id = 53,
		rarity = 1,
		time = 9000,
		type = 1,
		name = "인식 방향성 연구",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수의 사람들의 사물에 대한 인식의 집합체를 구현화하는 것이라면, 그것의 '방향성' 을 정하는 것에 따라 결과는 과연 어떻게 바뀔 것인가.",
		condition = 0,
		bg = "1",
		blueprint_version = 1,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "chara_yichui",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				3000
			}
		},
		drop_client = {
			{
				2,
				42003,
				1
			},
			{
				2,
				52002,
				1
			},
			{
				2,
				52004,
				1
			}
		}
	},
	[54] = {
		id = 54,
		rarity = 1,
		time = 9000,
		type = 1,
		name = "인식 방향성 연구",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수의 사람들의 사물에 대한 인식의 집합체를 구현화하는 것이라면, 그것의 '방향성' 을 정하는 것에 따라 결과는 과연 어떻게 바뀔 것인가.",
		condition = 0,
		bg = "1",
		blueprint_version = 1,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "chara_chuyun",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				3000
			}
		},
		drop_client = {
			{
				2,
				42004,
				1
			},
			{
				2,
				52002,
				1
			},
			{
				2,
				52004,
				1
			}
		}
	},
	[55] = {
		id = 55,
		rarity = 1,
		time = 9000,
		type = 1,
		name = "인식 방향성 연구",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수의 사람들의 사물에 대한 인식의 집합체를 구현화하는 것이라면, 그것의 '방향성' 을 정하는 것에 따라 결과는 과연 어떻게 바뀔 것인가.",
		condition = 0,
		bg = "1",
		blueprint_version = 1,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "chara_luoen",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				3000
			}
		},
		drop_client = {
			{
				2,
				42005,
				1
			},
			{
				2,
				52002,
				1
			},
			{
				2,
				52004,
				1
			}
		}
	},
	[56] = {
		id = 56,
		rarity = 1,
		time = 9000,
		type = 1,
		name = "인식 방향성 연구",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수의 사람들의 사물에 대한 인식의 집합체를 구현화하는 것이라면, 그것의 '방향성' 을 정하는 것에 따라 결과는 과연 어떻게 바뀔 것인가.",
		condition = 0,
		bg = "1",
		blueprint_version = 1,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "chara_luyijiushi",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				3000
			}
		},
		drop_client = {
			{
				2,
				42006,
				1
			},
			{
				2,
				52002,
				1
			},
			{
				2,
				52004,
				1
			}
		}
	},
	[61] = {
		id = 61,
		rarity = 2,
		time = 18000,
		type = 1,
		name = "인식 방향성 연구",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수의 사람들의 사물에 대한 인식의 집합체를 구현화하는 것이라면, 그것의 '방향성' 을 정하는 것에 따라 결과는 과연 어떻게 바뀔 것인가.",
		condition = 0,
		bg = "2",
		blueprint_version = 1,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "chara_haiwangxing",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				5000
			}
		},
		drop_client = {
			{
				2,
				42001,
				1
			},
			{
				2,
				52002,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[62] = {
		id = 62,
		rarity = 2,
		time = 18000,
		type = 1,
		name = "인식 방향성 연구",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수의 사람들의 사물에 대한 인식의 집합체를 구현화하는 것이라면, 그것의 '방향성' 을 정하는 것에 따라 결과는 과연 어떻게 바뀔 것인가.",
		condition = 0,
		bg = "2",
		blueprint_version = 1,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "chara_junzhu",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				5000
			}
		},
		drop_client = {
			{
				2,
				42002,
				1
			},
			{
				2,
				52002,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[63] = {
		id = 63,
		rarity = 2,
		time = 18000,
		type = 1,
		name = "인식 방향성 연구",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수의 사람들의 사물에 대한 인식의 집합체를 구현화하는 것이라면, 그것의 '방향성' 을 정하는 것에 따라 결과는 과연 어떻게 바뀔 것인가.",
		condition = 0,
		bg = "2",
		blueprint_version = 1,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "chara_yichui",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				5000
			}
		},
		drop_client = {
			{
				2,
				42003,
				1
			},
			{
				2,
				52002,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[64] = {
		id = 64,
		rarity = 2,
		time = 18000,
		type = 1,
		name = "인식 방향성 연구",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수의 사람들의 사물에 대한 인식의 집합체를 구현화하는 것이라면, 그것의 '방향성' 을 정하는 것에 따라 결과는 과연 어떻게 바뀔 것인가.",
		condition = 0,
		bg = "2",
		blueprint_version = 1,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "chara_chuyun",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				5000
			}
		},
		drop_client = {
			{
				2,
				42004,
				1
			},
			{
				2,
				52002,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[65] = {
		id = 65,
		rarity = 2,
		time = 18000,
		type = 1,
		name = "인식 방향성 연구",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수의 사람들의 사물에 대한 인식의 집합체를 구현화하는 것이라면, 그것의 '방향성' 을 정하는 것에 따라 결과는 과연 어떻게 바뀔 것인가.",
		condition = 0,
		bg = "2",
		blueprint_version = 1,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "chara_luoen",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				5000
			}
		},
		drop_client = {
			{
				2,
				42005,
				1
			},
			{
				2,
				52002,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[66] = {
		id = 66,
		rarity = 2,
		time = 18000,
		type = 1,
		name = "인식 방향성 연구",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수의 사람들의 사물에 대한 인식의 집합체를 구현화하는 것이라면, 그것의 '방향성' 을 정하는 것에 따라 결과는 과연 어떻게 바뀔 것인가.",
		condition = 0,
		bg = "2",
		blueprint_version = 1,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "chara_luyijiushi",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				5000
			}
		},
		drop_client = {
			{
				2,
				42006,
				1
			},
			{
				2,
				52002,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[71] = {
		id = 71,
		rarity = 1,
		time = 3600,
		type = 1,
		name = "장비 해석",
		lv_limit = 30,
		desc = "장비와 성정 큐브의 공명 현상을 제어할 수 있도록, 갖은 수단을 동원해 해당 현상을 재현하고, 최대한 효과적으로 이용할 수 있도록 한다.",
		condition = 0,
		bg = "1",
		blueprint_version = 1,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_1",
		label_color = "tag_q",
		consume = {
			{
				2,
				17001,
				20
			}
		},
		drop_client = {
			{
				2,
				52002,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[72] = {
		id = 72,
		rarity = 1,
		time = 3600,
		type = 1,
		name = "장비 해석",
		lv_limit = 30,
		desc = "장비와 성정 큐브의 공명 현상을 제어할 수 있도록, 갖은 수단을 동원해 해당 현상을 재현하고, 최대한 효과적으로 이용할 수 있도록 한다.",
		condition = 0,
		bg = "1",
		blueprint_version = 1,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_1",
		label_color = "tag_q",
		consume = {
			{
				2,
				17011,
				20
			}
		},
		drop_client = {
			{
				2,
				52002,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[73] = {
		id = 73,
		rarity = 1,
		time = 3600,
		type = 1,
		name = "장비 해석",
		lv_limit = 30,
		desc = "장비와 성정 큐브의 공명 현상을 제어할 수 있도록, 갖은 수단을 동원해 해당 현상을 재현하고, 최대한 효과적으로 이용할 수 있도록 한다.",
		condition = 0,
		bg = "1",
		blueprint_version = 1,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_1",
		label_color = "tag_q",
		consume = {
			{
				2,
				17021,
				20
			}
		},
		drop_client = {
			{
				2,
				52002,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[74] = {
		id = 74,
		rarity = 1,
		time = 3600,
		type = 1,
		name = "장비 해석",
		lv_limit = 30,
		desc = "장비와 성정 큐브의 공명 현상을 제어할 수 있도록, 갖은 수단을 동원해 해당 현상을 재현하고, 최대한 효과적으로 이용할 수 있도록 한다.",
		condition = 0,
		bg = "1",
		blueprint_version = 1,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_1",
		label_color = "tag_q",
		consume = {
			{
				2,
				17031,
				20
			}
		},
		drop_client = {
			{
				2,
				52002,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[75] = {
		id = 75,
		rarity = 1,
		time = 3600,
		type = 1,
		name = "장비 해석",
		lv_limit = 30,
		desc = "장비와 성정 큐브의 공명 현상을 제어할 수 있도록, 갖은 수단을 동원해 해당 현상을 재현하고, 최대한 효과적으로 이용할 수 있도록 한다.",
		condition = 0,
		bg = "1",
		blueprint_version = 1,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_1",
		label_color = "tag_q",
		consume = {
			{
				2,
				17041,
				20
			}
		},
		drop_client = {
			{
				2,
				52002,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[76] = {
		id = 76,
		rarity = 1,
		time = 7200,
		type = 1,
		name = "장비 해석",
		lv_limit = 30,
		desc = "장비와 성정 큐브의 공명 현상을 제어할 수 있도록, 갖은 수단을 동원해 해당 현상을 재현하고, 최대한 효과적으로 이용할 수 있도록 한다.",
		condition = 0,
		bg = "1",
		blueprint_version = 1,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_1",
		label_color = "tag_q",
		consume = {
			{
				2,
				17002,
				20
			}
		},
		drop_client = {
			{
				2,
				52002,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[77] = {
		id = 77,
		rarity = 1,
		time = 7200,
		type = 1,
		name = "장비 해석",
		lv_limit = 30,
		desc = "장비와 성정 큐브의 공명 현상을 제어할 수 있도록, 갖은 수단을 동원해 해당 현상을 재현하고, 최대한 효과적으로 이용할 수 있도록 한다.",
		condition = 0,
		bg = "1",
		blueprint_version = 1,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_1",
		label_color = "tag_q",
		consume = {
			{
				2,
				17012,
				20
			}
		},
		drop_client = {
			{
				2,
				52002,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[78] = {
		id = 78,
		rarity = 1,
		time = 7200,
		type = 1,
		name = "장비 해석",
		lv_limit = 30,
		desc = "장비와 성정 큐브의 공명 현상을 제어할 수 있도록, 갖은 수단을 동원해 해당 현상을 재현하고, 최대한 효과적으로 이용할 수 있도록 한다.",
		condition = 0,
		bg = "1",
		blueprint_version = 1,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_1",
		label_color = "tag_q",
		consume = {
			{
				2,
				17022,
				20
			}
		},
		drop_client = {
			{
				2,
				52002,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[79] = {
		id = 79,
		rarity = 1,
		time = 7200,
		type = 1,
		name = "장비 해석",
		lv_limit = 30,
		desc = "장비와 성정 큐브의 공명 현상을 제어할 수 있도록, 갖은 수단을 동원해 해당 현상을 재현하고, 최대한 효과적으로 이용할 수 있도록 한다.",
		condition = 0,
		bg = "1",
		blueprint_version = 1,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_1",
		label_color = "tag_q",
		consume = {
			{
				2,
				17032,
				20
			}
		},
		drop_client = {
			{
				2,
				52002,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[80] = {
		id = 80,
		rarity = 1,
		time = 7200,
		type = 1,
		name = "장비 해석",
		lv_limit = 30,
		desc = "장비와 성정 큐브의 공명 현상을 제어할 수 있도록, 갖은 수단을 동원해 해당 현상을 재현하고, 최대한 효과적으로 이용할 수 있도록 한다.",
		condition = 0,
		bg = "1",
		blueprint_version = 1,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_1",
		label_color = "tag_q",
		consume = {
			{
				2,
				17042,
				20
			}
		},
		drop_client = {
			{
				2,
				52002,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[81] = {
		id = 81,
		rarity = 2,
		time = 14400,
		type = 1,
		name = "장비 해석",
		lv_limit = 30,
		desc = "장비와 성정 큐브의 공명 현상을 제어할 수 있도록, 갖은 수단을 동원해 해당 현상을 재현하고, 최대한 효과적으로 이용할 수 있도록 한다.",
		condition = 0,
		bg = "2",
		blueprint_version = 1,
		sub_name = "중형 항목",
		label = "tag_blue",
		bg_icon = "gold_2",
		label_color = "tag_q",
		consume = {
			{
				2,
				17003,
				20
			}
		},
		drop_client = {
			{
				2,
				52002,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[82] = {
		id = 82,
		rarity = 2,
		time = 14400,
		type = 1,
		name = "장비 해석",
		lv_limit = 30,
		desc = "장비와 성정 큐브의 공명 현상을 제어할 수 있도록, 갖은 수단을 동원해 해당 현상을 재현하고, 최대한 효과적으로 이용할 수 있도록 한다.",
		condition = 0,
		bg = "2",
		blueprint_version = 1,
		sub_name = "중형 항목",
		label = "tag_blue",
		bg_icon = "gold_2",
		label_color = "tag_q",
		consume = {
			{
				2,
				17013,
				20
			}
		},
		drop_client = {
			{
				2,
				52002,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[83] = {
		id = 83,
		rarity = 2,
		time = 14400,
		type = 1,
		name = "장비 해석",
		lv_limit = 30,
		desc = "장비와 성정 큐브의 공명 현상을 제어할 수 있도록, 갖은 수단을 동원해 해당 현상을 재현하고, 최대한 효과적으로 이용할 수 있도록 한다.",
		condition = 0,
		bg = "2",
		blueprint_version = 1,
		sub_name = "중형 항목",
		label = "tag_blue",
		bg_icon = "gold_2",
		label_color = "tag_q",
		consume = {
			{
				2,
				17023,
				20
			}
		},
		drop_client = {
			{
				2,
				52002,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[84] = {
		id = 84,
		rarity = 2,
		time = 14400,
		type = 1,
		name = "장비 해석",
		lv_limit = 30,
		desc = "장비와 성정 큐브의 공명 현상을 제어할 수 있도록, 갖은 수단을 동원해 해당 현상을 재현하고, 최대한 효과적으로 이용할 수 있도록 한다.",
		condition = 0,
		bg = "2",
		blueprint_version = 1,
		sub_name = "중형 항목",
		label = "tag_blue",
		bg_icon = "gold_2",
		label_color = "tag_q",
		consume = {
			{
				2,
				17033,
				20
			}
		},
		drop_client = {
			{
				2,
				52002,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[85] = {
		id = 85,
		rarity = 2,
		time = 14400,
		type = 1,
		name = "장비 해석",
		lv_limit = 30,
		desc = "장비와 성정 큐브의 공명 현상을 제어할 수 있도록, 갖은 수단을 동원해 해당 현상을 재현하고, 최대한 효과적으로 이용할 수 있도록 한다.",
		condition = 0,
		bg = "2",
		blueprint_version = 1,
		sub_name = "중형 항목",
		label = "tag_blue",
		bg_icon = "gold_2",
		label_color = "tag_q",
		consume = {
			{
				2,
				17043,
				20
			}
		},
		drop_client = {
			{
				2,
				52002,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[91] = {
		id = 91,
		rarity = 1,
		time = 3600,
		type = 1,
		name = "큐브 구조 해석",
		lv_limit = 30,
		desc = "성정 큐브를 문자 그대로 물리적으로 '분해'하여, 구조 분석을 진행한다. 성공 사례가 없을수록, 실험의 가치는 그만큼 크다고 볼 수 있다.",
		condition = 0,
		bg = "1",
		blueprint_version = 1,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "gold_1",
		label_color = "tag_h",
		consume = {
			{
				2,
				20001,
				3
			}
		},
		drop_client = {
			{
				2,
				52001,
				1
			},
			{
				2,
				54049,
				1
			},
			{
				2,
				15008,
				1
			}
		}
	},
	[92] = {
		id = 92,
		rarity = 1,
		time = 3600,
		type = 1,
		name = "큐브 구조 해석",
		lv_limit = 30,
		desc = "성정 큐브를 문자 그대로 물리적으로 '분해'하여, 구조 분석을 진행한다. 성공 사례가 없을수록, 실험의 가치는 그만큼 크다고 볼 수 있다.",
		condition = 0,
		bg = "1",
		blueprint_version = 1,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "gold_1",
		label_color = "tag_h",
		consume = {
			{
				2,
				20001,
				3
			}
		},
		drop_client = {
			{
				2,
				52001,
				1
			},
			{
				2,
				54049,
				1
			},
			{
				2,
				15008,
				1
			}
		}
	},
	[93] = {
		id = 93,
		rarity = 2,
		time = 7200,
		type = 1,
		name = "큐브 구조 해석",
		lv_limit = 30,
		desc = "성정 큐브를 문자 그대로 물리적으로 '분해'하여, 구조 분석을 진행한다. 성공 사례가 없을수록, 실험의 가치는 그만큼 크다고 볼 수 있다.",
		condition = 0,
		bg = "2",
		blueprint_version = 1,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "gold_2",
		label_color = "tag_h",
		consume = {
			{
				2,
				20001,
				6
			}
		},
		drop_client = {
			{
				2,
				52001,
				1
			},
			{
				2,
				54050,
				1
			},
			{
				2,
				15008,
				1
			}
		}
	},
	[94] = {
		id = 94,
		rarity = 2,
		time = 7200,
		type = 1,
		name = "큐브 구조 해석",
		lv_limit = 30,
		desc = "성정 큐브를 문자 그대로 물리적으로 '분해'하여, 구조 분석을 진행한다. 성공 사례가 없을수록, 실험의 가치는 그만큼 크다고 볼 수 있다.",
		condition = 0,
		bg = "2",
		blueprint_version = 1,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "gold_2",
		label_color = "tag_h",
		consume = {
			{
				2,
				20001,
				6
			}
		},
		drop_client = {
			{
				2,
				52001,
				1
			},
			{
				2,
				54050,
				1
			},
			{
				2,
				15008,
				1
			}
		}
	},
	[101] = {
		id = 101,
		rarity = 1,
		time = 21600,
		type = 1,
		name = "기초 연구",
		lv_limit = 30,
		desc = "성정 큐브에 관한 기초 이론은 큐브 구조에 대한 인식으로부터 시작된다. 큐브 기초 구조에 대한 이론 구조 작업은 여전히 전력을 다해 진행 중이다.",
		condition = 0,
		bg = "1",
		blueprint_version = 2,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "item_1",
		label_color = "tag_c",
		consume = {},
		drop_client = {
			{
				2,
				52012,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[102] = {
		id = 102,
		rarity = 1,
		time = 21600,
		type = 1,
		name = "기초 연구",
		lv_limit = 30,
		desc = "성정 큐브에 관한 기초 이론은 큐브 구조에 대한 인식으로부터 시작된다. 큐브 기초 구조에 대한 이론 구조 작업은 여전히 전력을 다해 진행 중이다.",
		condition = 0,
		bg = "1",
		blueprint_version = 2,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "item_1",
		label_color = "tag_c",
		consume = {},
		drop_client = {
			{
				2,
				52012,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[103] = {
		id = 103,
		rarity = 2,
		time = 28800,
		type = 1,
		name = "기초 연구",
		lv_limit = 30,
		desc = "성정 큐브에 관한 기초 이론은 큐브 구조에 대한 인식으로부터 시작된다. 큐브 기초 구조에 대한 이론 구조 작업은 여전히 전력을 다해 진행 중이다.",
		condition = 0,
		bg = "2",
		blueprint_version = 2,
		sub_name = "대형 항목",
		label = "tag_red",
		bg_icon = "item_2",
		label_color = "tag_c",
		consume = {},
		drop_client = {
			{
				2,
				52011,
				1
			},
			{
				2,
				52012,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[104] = {
		id = 104,
		rarity = 2,
		time = 28800,
		type = 1,
		name = "기초 연구",
		lv_limit = 30,
		desc = "성정 큐브에 관한 기초 이론은 큐브 구조에 대한 인식으로부터 시작된다. 큐브 기초 구조에 대한 이론 구조 작업은 여전히 전력을 다해 진행 중이다.",
		condition = 0,
		bg = "2",
		blueprint_version = 2,
		sub_name = "대형 항목",
		label = "tag_red",
		bg_icon = "item_2",
		label_color = "tag_c",
		consume = {},
		drop_client = {
			{
				2,
				52011,
				1
			},
			{
				2,
				52012,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[111] = {
		id = 111,
		rarity = 1,
		time = 14400,
		type = 1,
		name = "데이터 수집",
		lv_limit = 30,
		desc = "성정 큐브 구조에 관한 여러가지 가설에 대해 실증실험을 진행하는 연구 프로젝트. 실전을 통해 데이터를 수집하여, 이론의 과학적 증명에 기여하자.",
		condition = 52001,
		bg = "1",
		blueprint_version = 2,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "battle_1",
		label_color = "tag_b",
		consume = {},
		drop_client = {
			{
				2,
				52011,
				1
			},
			{
				2,
				52012,
				1
			},
			{
				2,
				52004,
				1
			}
		}
	},
	[112] = {
		id = 112,
		rarity = 1,
		time = 14400,
		type = 1,
		name = "데이터 수집",
		lv_limit = 30,
		desc = "성정 큐브 구조에 관한 여러가지 가설에 대해 실증실험을 진행하는 연구 프로젝트. 실전을 통해 데이터를 수집하여, 이론의 과학적 증명에 기여하자.",
		condition = 52002,
		bg = "1",
		blueprint_version = 2,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "battle_1",
		label_color = "tag_b",
		consume = {},
		drop_client = {
			{
				2,
				52011,
				1
			},
			{
				2,
				52012,
				1
			},
			{
				2,
				52004,
				1
			}
		}
	},
	[113] = {
		id = 113,
		rarity = 1,
		time = 14400,
		type = 1,
		name = "데이터 수집",
		lv_limit = 30,
		desc = "성정 큐브 구조에 관한 여러가지 가설에 대해 실증실험을 진행하는 연구 프로젝트. 실전을 통해 데이터를 수집하여, 이론의 과학적 증명에 기여하자.",
		condition = 52003,
		bg = "1",
		blueprint_version = 2,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "battle_1",
		label_color = "tag_b",
		consume = {},
		drop_client = {
			{
				2,
				52011,
				1
			},
			{
				2,
				52012,
				1
			},
			{
				2,
				52004,
				1
			}
		}
	},
	[114] = {
		id = 114,
		rarity = 1,
		time = 14400,
		type = 1,
		name = "데이터 수집",
		lv_limit = 40,
		desc = "성정 큐브 구조에 관한 여러가지 가설에 대해 실증실험을 진행하는 연구 프로젝트. 실전을 통해 데이터를 수집하여, 이론의 과학적 증명에 기여하자.",
		condition = 52004,
		bg = "1",
		blueprint_version = 2,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "battle_1",
		label_color = "tag_b",
		consume = {},
		drop_client = {
			{
				2,
				52011,
				1
			},
			{
				2,
				52012,
				1
			},
			{
				2,
				52004,
				1
			}
		}
	},
	[115] = {
		id = 115,
		rarity = 2,
		time = 14400,
		type = 1,
		name = "데이터 수집",
		lv_limit = 50,
		desc = "성정 큐브 구조에 관한 여러가지 가설에 대해 실증실험을 진행하는 연구 프로젝트. 실전을 통해 데이터를 수집하여, 이론의 과학적 증명에 기여하자.",
		condition = 52005,
		bg = "2",
		blueprint_version = 2,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "battle_2",
		label_color = "tag_b",
		consume = {},
		drop_client = {
			{
				2,
				52011,
				1
			},
			{
				2,
				52012,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[116] = {
		id = 116,
		rarity = 2,
		time = 14400,
		type = 1,
		name = "데이터 수집",
		lv_limit = 60,
		desc = "성정 큐브 구조에 관한 여러가지 가설에 대해 실증실험을 진행하는 연구 프로젝트. 실전을 통해 데이터를 수집하여, 이론의 과학적 증명에 기여하자.",
		condition = 52006,
		bg = "2",
		blueprint_version = 2,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "battle_2",
		label_color = "tag_b",
		consume = {},
		drop_client = {
			{
				2,
				52011,
				1
			},
			{
				2,
				52012,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[117] = {
		id = 117,
		rarity = 2,
		time = 14400,
		type = 1,
		name = "데이터 수집",
		lv_limit = 65,
		desc = "성정 큐브 구조에 관한 여러가지 가설에 대해 실증실험을 진행하는 연구 프로젝트. 실전을 통해 데이터를 수집하여, 이론의 과학적 증명에 기여하자.",
		condition = 52007,
		bg = "2",
		blueprint_version = 2,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "battle_2",
		label_color = "tag_b",
		consume = {},
		drop_client = {
			{
				2,
				52011,
				1
			},
			{
				2,
				52012,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[118] = {
		id = 118,
		rarity = 2,
		time = 14400,
		type = 1,
		name = "데이터 수집",
		lv_limit = 70,
		desc = "성정 큐브 구조에 관한 여러가지 가설에 대해 실증실험을 진행하는 연구 프로젝트. 실전을 통해 데이터를 수집하여, 이론의 과학적 증명에 기여하자.",
		condition = 52008,
		bg = "2",
		blueprint_version = 2,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "battle_2",
		label_color = "tag_b",
		consume = {},
		drop_client = {
			{
				2,
				52011,
				1
			},
			{
				2,
				52012,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[121] = {
		id = 121,
		rarity = 1,
		time = 10800,
		type = 1,
		name = "연구 의뢰",
		lv_limit = 30,
		desc = "성정 큐브에 관한 연구는 [건조] 방면뿐만 아니라, 일상생활 이용 등 다양한 방면으로 진행되고 있다. 성정 큐브의 다방면 활용 가능성을 찾는 프로젝트에는 군의 조력이 필수불가결이다.",
		condition = 52021,
		bg = "1",
		blueprint_version = 2,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "item_1",
		label_color = "tag_t",
		consume = {},
		drop_client = {
			{
				2,
				52012,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[122] = {
		id = 122,
		rarity = 1,
		time = 14400,
		type = 1,
		name = "연구 의뢰",
		lv_limit = 30,
		desc = "성정 큐브에 관한 연구는 [건조] 방면뿐만 아니라, 일상생활 이용 등 다양한 방면으로 진행되고 있다. 성정 큐브의 다방면 활용 가능성을 찾는 프로젝트에는 군의 조력이 필수불가결이다.",
		condition = 52022,
		bg = "1",
		blueprint_version = 2,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "item_1",
		label_color = "tag_t",
		consume = {},
		drop_client = {
			{
				2,
				52012,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[123] = {
		id = 123,
		rarity = 2,
		time = 21600,
		type = 1,
		name = "연구 의뢰",
		lv_limit = 30,
		desc = "성정 큐브에 관한 연구는 [건조] 방면뿐만 아니라, 일상생활 이용 등 다양한 방면으로 진행되고 있다. 성정 큐브의 다방면 활용 가능성을 찾는 프로젝트에는 군의 조력이 필수불가결이다.",
		condition = 52023,
		bg = "2",
		blueprint_version = 2,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "item_2",
		label_color = "tag_t",
		consume = {},
		drop_client = {
			{
				2,
				52012,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[131] = {
		id = 131,
		rarity = 1,
		time = 7200,
		type = 1,
		name = "샘플 조달",
		lv_limit = 30,
		desc = "성정 큐브와 [장비]의 관련성을 조사하는 다수의 시도는, 방대한 실험 데이터에 기반하여 진행되고 있다. 불필요한 장비를 제공하여, 실험 샘플을 충당하자.",
		condition = 52031,
		bg = "1",
		blueprint_version = 2,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "item_1",
		label_color = "tag_e",
		consume = {},
		drop_client = {
			{
				2,
				52012,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[132] = {
		id = 132,
		rarity = 2,
		time = 7200,
		type = 1,
		name = "샘플 조달",
		lv_limit = 30,
		desc = "성정 큐브와 [장비]의 관련성을 조사하는 다수의 시도는, 방대한 실험 데이터에 기반하여 진행되고 있다. 불필요한 장비를 제공하여, 실험 샘플을 충당하자.",
		condition = 52032,
		bg = "2",
		blueprint_version = 2,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "item_2",
		label_color = "tag_e",
		consume = {},
		drop_client = {
			{
				2,
				52012,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[133] = {
		id = 133,
		rarity = 2,
		time = 21600,
		type = 1,
		name = "E-179-MI",
		lv_limit = 50,
		desc = "현재 비치 중인 장비를 분해함으로써 기초 설계를 재검토하여, 추후 개발이 예정된 새 장비의 기초 연구를 진행한다. ",
		condition = 0,
		bg = "2",
		blueprint_version = 2,
		sub_name = "중형 항목",
		label = "tag_blue",
		bg_icon = "item_2",
		label_color = "tag_e",
		consume = {
			{
				2,
				30043,
				15
			}
		},
		drop_client = {
			{
				2,
				13013,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[134] = {
		id = 134,
		rarity = 2,
		time = 21600,
		type = 1,
		name = "E-105-MI",
		lv_limit = 50,
		desc = "현재 비치 중인 장비를 분해함으로써 기초 설계를 재검토하여, 추후 개발이 예정된 새 장비의 기초 연구를 진행한다. ",
		condition = 0,
		bg = "2",
		blueprint_version = 2,
		sub_name = "중형 항목",
		label = "tag_blue",
		bg_icon = "item_2",
		label_color = "tag_e",
		consume = {
			{
				2,
				30013,
				15
			}
		},
		drop_client = {
			{
				2,
				13014,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[135] = {
		id = 135,
		rarity = 2,
		time = 21600,
		type = 1,
		name = "E-146-MI",
		lv_limit = 50,
		desc = "현재 비치 중인 장비를 분해함으로써 기초 설계를 재검토하여, 추후 개발이 예정된 새 장비의 기초 연구를 진행한다. ",
		condition = 0,
		bg = "2",
		blueprint_version = 2,
		sub_name = "중형 항목",
		label = "tag_blue",
		bg_icon = "item_2",
		label_color = "tag_e",
		consume = {
			{
				2,
				30033,
				15
			}
		},
		drop_client = {
			{
				2,
				13015,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[136] = {
		id = 136,
		rarity = 2,
		time = 21600,
		type = 1,
		name = "E-157-MI",
		lv_limit = 50,
		desc = "현재 비치 중인 장비를 분해함으로써 기초 설계를 재검토하여, 추후 개발이 예정된 새 장비의 기초 연구를 진행한다. ",
		condition = 0,
		bg = "2",
		blueprint_version = 2,
		sub_name = "중형 항목",
		label = "tag_blue",
		bg_icon = "item_2",
		label_color = "tag_e",
		consume = {
			{
				2,
				30033,
				15
			}
		},
		drop_client = {
			{
				2,
				13016,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[137] = {
		id = 137,
		rarity = 2,
		time = 21600,
		type = 1,
		name = "E-194-MI",
		lv_limit = 50,
		desc = "현재 비치 중인 장비를 분해함으로써 기초 설계를 재검토하여, 추후 개발이 예정된 새 장비의 기초 연구를 진행한다. ",
		condition = 0,
		bg = "2",
		blueprint_version = 2,
		sub_name = "중형 항목",
		label = "tag_blue",
		bg_icon = "item_2",
		label_color = "tag_e",
		consume = {
			{
				2,
				30013,
				15
			}
		},
		drop_client = {
			{
				2,
				13017,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[141] = {
		id = 141,
		rarity = 1,
		time = 5400,
		type = 1,
		name = "비용 기부",
		lv_limit = 30,
		desc = "뛰어난 과학의 발전을 뒷받침하기 위해선 영민한 두뇌뿐만 아니라, 지원자로부터 제공되는 연구 자금도 필요하다. 남아도는 자금을 기부하여 기술의 연구를 서포트하자.",
		condition = 0,
		bg = "1",
		blueprint_version = 2,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "gold_1",
		label_color = "tag_g",
		consume = {
			{
				1,
				1,
				1500
			}
		},
		drop_client = {
			{
				2,
				52011,
				1
			},
			{
				2,
				52012,
				1
			},
			{
				2,
				52004,
				1
			}
		}
	},
	[142] = {
		id = 142,
		rarity = 1,
		time = 5400,
		type = 1,
		name = "비용 기부",
		lv_limit = 30,
		desc = "뛰어난 과학의 발전을 뒷받침하기 위해선 영민한 두뇌뿐만 아니라, 지원자로부터 제공되는 연구 자금도 필요하다. 남아도는 자금을 기부하여 기술의 연구를 서포트하자.",
		condition = 0,
		bg = "1",
		blueprint_version = 2,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "gold_1",
		label_color = "tag_g",
		consume = {
			{
				1,
				1,
				1500
			}
		},
		drop_client = {
			{
				2,
				52011,
				1
			},
			{
				2,
				52012,
				1
			},
			{
				2,
				52004,
				1
			}
		}
	},
	[143] = {
		id = 143,
		rarity = 1,
		time = 9000,
		type = 1,
		name = "비용 기부",
		lv_limit = 30,
		desc = "뛰어난 과학의 발전을 뒷받침하기 위해선 영민한 두뇌뿐만 아니라, 지원자로부터 제공되는 연구 자금도 필요하다. 남아도는 자금을 기부하여 기술의 연구를 서포트하자.",
		condition = 0,
		bg = "1",
		blueprint_version = 2,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "gold_1",
		label_color = "tag_g",
		consume = {
			{
				1,
				1,
				3000
			}
		},
		drop_client = {
			{
				2,
				52011,
				1
			},
			{
				2,
				52012,
				1
			},
			{
				2,
				52004,
				1
			}
		}
	},
	[144] = {
		id = 144,
		rarity = 1,
		time = 9000,
		type = 1,
		name = "비용 기부",
		lv_limit = 30,
		desc = "뛰어난 과학의 발전을 뒷받침하기 위해선 영민한 두뇌뿐만 아니라, 지원자로부터 제공되는 연구 자금도 필요하다. 남아도는 자금을 기부하여 기술의 연구를 서포트하자.",
		condition = 0,
		bg = "1",
		blueprint_version = 2,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "gold_1",
		label_color = "tag_g",
		consume = {
			{
				1,
				1,
				3000
			}
		},
		drop_client = {
			{
				2,
				52011,
				1
			},
			{
				2,
				52012,
				1
			},
			{
				2,
				52004,
				1
			}
		}
	},
	[145] = {
		id = 145,
		rarity = 2,
		time = 14400,
		type = 1,
		name = "비용 기부",
		lv_limit = 30,
		desc = "뛰어난 과학의 발전을 뒷받침하기 위해선 영민한 두뇌뿐만 아니라, 지원자로부터 제공되는 연구 자금도 필요하다. 남아도는 자금을 기부하여 기술의 연구를 서포트하자.",
		condition = 0,
		bg = "2",
		blueprint_version = 2,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "gold_2",
		label_color = "tag_g",
		consume = {
			{
				1,
				1,
				6000
			}
		},
		drop_client = {
			{
				2,
				52011,
				1
			},
			{
				2,
				52012,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[146] = {
		id = 146,
		rarity = 2,
		time = 14400,
		type = 1,
		name = "비용 기부",
		lv_limit = 30,
		desc = "뛰어난 과학의 발전을 뒷받침하기 위해선 영민한 두뇌뿐만 아니라, 지원자로부터 제공되는 연구 자금도 필요하다. 남아도는 자금을 기부하여 기술의 연구를 서포트하자.",
		condition = 0,
		bg = "2",
		blueprint_version = 2,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "gold_2",
		label_color = "tag_g",
		consume = {
			{
				1,
				1,
				6000
			}
		},
		drop_client = {
			{
				2,
				52011,
				1
			},
			{
				2,
				52012,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[151] = {
		id = 151,
		rarity = 1,
		time = 9000,
		type = 1,
		name = "인식 방향성 연구",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수의 사람들의 사물에 대한 인식의 집합체를 구현화하는 것이라면, 그것의 '방향성' 을 정하는 것에 따라 결과는 과연 어떻게 바뀔 것인가.",
		condition = 0,
		bg = "1",
		blueprint_version = 2,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "chara_xiyatu",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				3000
			}
		},
		drop_client = {
			{
				2,
				42011,
				1
			},
			{
				2,
				52012,
				1
			},
			{
				2,
				52004,
				1
			}
		}
	},
	[152] = {
		id = 152,
		rarity = 1,
		time = 9000,
		type = 1,
		name = "인식 방향성 연구",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수의 사람들의 사물에 대한 인식의 집합체를 구현화하는 것이라면, 그것의 '방향성' 을 정하는 것에 따라 결과는 과연 어떻게 바뀔 것인가.",
		condition = 0,
		bg = "1",
		blueprint_version = 2,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "chara_zuozhiya",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				3000
			}
		},
		drop_client = {
			{
				2,
				42012,
				1
			},
			{
				2,
				52012,
				1
			},
			{
				2,
				52004,
				1
			}
		}
	},
	[153] = {
		id = 153,
		rarity = 1,
		time = 9000,
		type = 1,
		name = "인식 방향성 연구",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수의 사람들의 사물에 대한 인식의 집합체를 구현화하는 것이라면, 그것의 '방향성' 을 정하는 것에 따라 결과는 과연 어떻게 바뀔 것인가.",
		condition = 0,
		bg = "1",
		blueprint_version = 2,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "chara_beifeng",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				3000
			}
		},
		drop_client = {
			{
				2,
				42013,
				1
			},
			{
				2,
				52012,
				1
			},
			{
				2,
				52004,
				1
			}
		}
	},
	[154] = {
		id = 154,
		rarity = 1,
		time = 9000,
		type = 1,
		name = "인식 방향성 연구",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수의 사람들의 사물에 대한 인식의 집합체를 구현화하는 것이라면, 그것의 '방향성' 을 정하는 것에 따라 결과는 과연 어떻게 바뀔 것인가.",
		condition = 0,
		bg = "1",
		blueprint_version = 2,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "chara_wuqi",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				3000
			}
		},
		drop_client = {
			{
				2,
				42014,
				1
			},
			{
				2,
				52012,
				1
			},
			{
				2,
				52004,
				1
			}
		}
	},
	[155] = {
		id = 155,
		rarity = 1,
		time = 9000,
		type = 1,
		name = "인식 방향성 연구",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수의 사람들의 사물에 대한 인식의 집합체를 구현화하는 것이라면, 그것의 '방향성' 을 정하는 것에 따라 결과는 과연 어떻게 바뀔 것인가.",
		condition = 0,
		bg = "1",
		blueprint_version = 2,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "chara_feiteliedadi",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				3000
			}
		},
		drop_client = {
			{
				2,
				42015,
				1
			},
			{
				2,
				52012,
				1
			},
			{
				2,
				52004,
				1
			}
		}
	},
	[156] = {
		id = 156,
		rarity = 1,
		time = 9000,
		type = 1,
		name = "인식 방향성 연구",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수의 사람들의 사물에 대한 인식의 집합체를 구현화하는 것이라면, 그것의 '방향성' 을 정하는 것에 따라 결과는 과연 어떻게 바뀔 것인가.",
		condition = 0,
		bg = "1",
		blueprint_version = 2,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "chara_jiasikenie",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				3000
			}
		},
		drop_client = {
			{
				2,
				42016,
				1
			},
			{
				2,
				52012,
				1
			},
			{
				2,
				52004,
				1
			}
		}
	},
	[161] = {
		id = 161,
		rarity = 2,
		time = 18000,
		type = 1,
		name = "인식 방향성 연구",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수의 사람들의 사물에 대한 인식의 집합체를 구현화하는 것이라면, 그것의 '방향성' 을 정하는 것에 따라 결과는 과연 어떻게 바뀔 것인가.",
		condition = 0,
		bg = "2",
		blueprint_version = 2,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "chara_xiyatu",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				5000
			}
		},
		drop_client = {
			{
				2,
				42011,
				1
			},
			{
				2,
				52012,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[162] = {
		id = 162,
		rarity = 2,
		time = 18000,
		type = 1,
		name = "인식 방향성 연구",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수의 사람들의 사물에 대한 인식의 집합체를 구현화하는 것이라면, 그것의 '방향성' 을 정하는 것에 따라 결과는 과연 어떻게 바뀔 것인가.",
		condition = 0,
		bg = "2",
		blueprint_version = 2,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "chara_zuozhiya",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				5000
			}
		},
		drop_client = {
			{
				2,
				42012,
				1
			},
			{
				2,
				52012,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[163] = {
		id = 163,
		rarity = 2,
		time = 18000,
		type = 1,
		name = "인식 방향성 연구",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수의 사람들의 사물에 대한 인식의 집합체를 구현화하는 것이라면, 그것의 '방향성' 을 정하는 것에 따라 결과는 과연 어떻게 바뀔 것인가.",
		condition = 0,
		bg = "2",
		blueprint_version = 2,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "chara_beifeng",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				5000
			}
		},
		drop_client = {
			{
				2,
				42013,
				1
			},
			{
				2,
				52012,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[164] = {
		id = 164,
		rarity = 2,
		time = 18000,
		type = 1,
		name = "인식 방향성 연구",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수의 사람들의 사물에 대한 인식의 집합체를 구현화하는 것이라면, 그것의 '방향성' 을 정하는 것에 따라 결과는 과연 어떻게 바뀔 것인가.",
		condition = 0,
		bg = "2",
		blueprint_version = 2,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "chara_wuqi",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				5000
			}
		},
		drop_client = {
			{
				2,
				42014,
				1
			},
			{
				2,
				52012,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[165] = {
		id = 165,
		rarity = 2,
		time = 18000,
		type = 1,
		name = "인식 방향성 연구",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수의 사람들의 사물에 대한 인식의 집합체를 구현화하는 것이라면, 그것의 '방향성' 을 정하는 것에 따라 결과는 과연 어떻게 바뀔 것인가.",
		condition = 0,
		bg = "2",
		blueprint_version = 2,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "chara_feiteliedadi",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				5000
			}
		},
		drop_client = {
			{
				2,
				42015,
				1
			},
			{
				2,
				52012,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[166] = {
		id = 166,
		rarity = 2,
		time = 18000,
		type = 1,
		name = "인식 방향성 연구",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수의 사람들의 사물에 대한 인식의 집합체를 구현화하는 것이라면, 그것의 '방향성' 을 정하는 것에 따라 결과는 과연 어떻게 바뀔 것인가.",
		condition = 0,
		bg = "2",
		blueprint_version = 2,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "chara_jiasikenie",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				5000
			}
		},
		drop_client = {
			{
				2,
				42016,
				1
			},
			{
				2,
				52012,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[171] = {
		id = 171,
		rarity = 1,
		time = 3600,
		type = 1,
		name = "장비 해석",
		lv_limit = 30,
		desc = "장비와 성정 큐브의 공명 현상을 제어할 수 있도록, 갖은 수단을 동원해 해당 현상을 재현하고, 최대한 효과적으로 이용할 수 있도록 한다.",
		condition = 0,
		bg = "1",
		blueprint_version = 2,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_1",
		label_color = "tag_q",
		consume = {
			{
				2,
				17001,
				20
			}
		},
		drop_client = {
			{
				2,
				52012,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[172] = {
		id = 172,
		rarity = 1,
		time = 3600,
		type = 1,
		name = "장비 해석",
		lv_limit = 30,
		desc = "장비와 성정 큐브의 공명 현상을 제어할 수 있도록, 갖은 수단을 동원해 해당 현상을 재현하고, 최대한 효과적으로 이용할 수 있도록 한다.",
		condition = 0,
		bg = "1",
		blueprint_version = 2,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_1",
		label_color = "tag_q",
		consume = {
			{
				2,
				17011,
				20
			}
		},
		drop_client = {
			{
				2,
				52012,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[173] = {
		id = 173,
		rarity = 1,
		time = 3600,
		type = 1,
		name = "장비 해석",
		lv_limit = 30,
		desc = "장비와 성정 큐브의 공명 현상을 제어할 수 있도록, 갖은 수단을 동원해 해당 현상을 재현하고, 최대한 효과적으로 이용할 수 있도록 한다.",
		condition = 0,
		bg = "1",
		blueprint_version = 2,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_1",
		label_color = "tag_q",
		consume = {
			{
				2,
				17021,
				20
			}
		},
		drop_client = {
			{
				2,
				52012,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[174] = {
		id = 174,
		rarity = 1,
		time = 3600,
		type = 1,
		name = "장비 해석",
		lv_limit = 30,
		desc = "장비와 성정 큐브의 공명 현상을 제어할 수 있도록, 갖은 수단을 동원해 해당 현상을 재현하고, 최대한 효과적으로 이용할 수 있도록 한다.",
		condition = 0,
		bg = "1",
		blueprint_version = 2,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_1",
		label_color = "tag_q",
		consume = {
			{
				2,
				17031,
				20
			}
		},
		drop_client = {
			{
				2,
				52012,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[175] = {
		id = 175,
		rarity = 1,
		time = 3600,
		type = 1,
		name = "장비 해석",
		lv_limit = 30,
		desc = "장비와 성정 큐브의 공명 현상을 제어할 수 있도록, 갖은 수단을 동원해 해당 현상을 재현하고, 최대한 효과적으로 이용할 수 있도록 한다.",
		condition = 0,
		bg = "1",
		blueprint_version = 2,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_1",
		label_color = "tag_q",
		consume = {
			{
				2,
				17041,
				20
			}
		},
		drop_client = {
			{
				2,
				52012,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[176] = {
		id = 176,
		rarity = 1,
		time = 7200,
		type = 1,
		name = "장비 해석",
		lv_limit = 30,
		desc = "장비와 성정 큐브의 공명 현상을 제어할 수 있도록, 갖은 수단을 동원해 해당 현상을 재현하고, 최대한 효과적으로 이용할 수 있도록 한다.",
		condition = 0,
		bg = "1",
		blueprint_version = 2,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_1",
		label_color = "tag_q",
		consume = {
			{
				2,
				17002,
				20
			}
		},
		drop_client = {
			{
				2,
				52012,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[177] = {
		id = 177,
		rarity = 1,
		time = 7200,
		type = 1,
		name = "장비 해석",
		lv_limit = 30,
		desc = "장비와 성정 큐브의 공명 현상을 제어할 수 있도록, 갖은 수단을 동원해 해당 현상을 재현하고, 최대한 효과적으로 이용할 수 있도록 한다.",
		condition = 0,
		bg = "1",
		blueprint_version = 2,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_1",
		label_color = "tag_q",
		consume = {
			{
				2,
				17012,
				20
			}
		},
		drop_client = {
			{
				2,
				52012,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[178] = {
		id = 178,
		rarity = 1,
		time = 7200,
		type = 1,
		name = "장비 해석",
		lv_limit = 30,
		desc = "장비와 성정 큐브의 공명 현상을 제어할 수 있도록, 갖은 수단을 동원해 해당 현상을 재현하고, 최대한 효과적으로 이용할 수 있도록 한다.",
		condition = 0,
		bg = "1",
		blueprint_version = 2,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_1",
		label_color = "tag_q",
		consume = {
			{
				2,
				17022,
				20
			}
		},
		drop_client = {
			{
				2,
				52012,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[179] = {
		id = 179,
		rarity = 1,
		time = 7200,
		type = 1,
		name = "장비 해석",
		lv_limit = 30,
		desc = "장비와 성정 큐브의 공명 현상을 제어할 수 있도록, 갖은 수단을 동원해 해당 현상을 재현하고, 최대한 효과적으로 이용할 수 있도록 한다.",
		condition = 0,
		bg = "1",
		blueprint_version = 2,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_1",
		label_color = "tag_q",
		consume = {
			{
				2,
				17032,
				20
			}
		},
		drop_client = {
			{
				2,
				52012,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[180] = {
		id = 180,
		rarity = 1,
		time = 7200,
		type = 1,
		name = "장비 해석",
		lv_limit = 30,
		desc = "장비와 성정 큐브의 공명 현상을 제어할 수 있도록, 갖은 수단을 동원해 해당 현상을 재현하고, 최대한 효과적으로 이용할 수 있도록 한다.",
		condition = 0,
		bg = "1",
		blueprint_version = 2,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_1",
		label_color = "tag_q",
		consume = {
			{
				2,
				17042,
				20
			}
		},
		drop_client = {
			{
				2,
				52012,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[181] = {
		id = 181,
		rarity = 2,
		time = 14400,
		type = 1,
		name = "장비 해석",
		lv_limit = 30,
		desc = "장비와 성정 큐브의 공명 현상을 제어할 수 있도록, 갖은 수단을 동원해 해당 현상을 재현하고, 최대한 효과적으로 이용할 수 있도록 한다.",
		condition = 0,
		bg = "2",
		blueprint_version = 2,
		sub_name = "중형 항목",
		label = "tag_blue",
		bg_icon = "gold_2",
		label_color = "tag_q",
		consume = {
			{
				2,
				17003,
				20
			}
		},
		drop_client = {
			{
				2,
				52012,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[182] = {
		id = 182,
		rarity = 2,
		time = 14400,
		type = 1,
		name = "장비 해석",
		lv_limit = 30,
		desc = "장비와 성정 큐브의 공명 현상을 제어할 수 있도록, 갖은 수단을 동원해 해당 현상을 재현하고, 최대한 효과적으로 이용할 수 있도록 한다.",
		condition = 0,
		bg = "2",
		blueprint_version = 2,
		sub_name = "중형 항목",
		label = "tag_blue",
		bg_icon = "gold_2",
		label_color = "tag_q",
		consume = {
			{
				2,
				17013,
				20
			}
		},
		drop_client = {
			{
				2,
				52012,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[183] = {
		id = 183,
		rarity = 2,
		time = 14400,
		type = 1,
		name = "장비 해석",
		lv_limit = 30,
		desc = "장비와 성정 큐브의 공명 현상을 제어할 수 있도록, 갖은 수단을 동원해 해당 현상을 재현하고, 최대한 효과적으로 이용할 수 있도록 한다.",
		condition = 0,
		bg = "2",
		blueprint_version = 2,
		sub_name = "중형 항목",
		label = "tag_blue",
		bg_icon = "gold_2",
		label_color = "tag_q",
		consume = {
			{
				2,
				17023,
				20
			}
		},
		drop_client = {
			{
				2,
				52012,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[184] = {
		id = 184,
		rarity = 2,
		time = 14400,
		type = 1,
		name = "장비 해석",
		lv_limit = 30,
		desc = "장비와 성정 큐브의 공명 현상을 제어할 수 있도록, 갖은 수단을 동원해 해당 현상을 재현하고, 최대한 효과적으로 이용할 수 있도록 한다.",
		condition = 0,
		bg = "2",
		blueprint_version = 2,
		sub_name = "중형 항목",
		label = "tag_blue",
		bg_icon = "gold_2",
		label_color = "tag_q",
		consume = {
			{
				2,
				17033,
				20
			}
		},
		drop_client = {
			{
				2,
				52012,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[185] = {
		id = 185,
		rarity = 2,
		time = 14400,
		type = 1,
		name = "장비 해석",
		lv_limit = 30,
		desc = "장비와 성정 큐브의 공명 현상을 제어할 수 있도록, 갖은 수단을 동원해 해당 현상을 재현하고, 최대한 효과적으로 이용할 수 있도록 한다.",
		condition = 0,
		bg = "2",
		blueprint_version = 2,
		sub_name = "중형 항목",
		label = "tag_blue",
		bg_icon = "gold_2",
		label_color = "tag_q",
		consume = {
			{
				2,
				17043,
				20
			}
		},
		drop_client = {
			{
				2,
				52012,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[191] = {
		id = 191,
		rarity = 1,
		time = 3600,
		type = 1,
		name = "큐브 구조 해석",
		lv_limit = 30,
		desc = "성정 큐브를 문자 그대로 물리적으로 '분해'하여, 구조 분석을 진행한다. 성공 사례가 없을수록, 실험의 가치는 그만큼 크다고 볼 수 있다.",
		condition = 0,
		bg = "1",
		blueprint_version = 2,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "gold_1",
		label_color = "tag_h",
		consume = {
			{
				2,
				20001,
				3
			}
		},
		drop_client = {
			{
				2,
				52011,
				1
			},
			{
				2,
				54049,
				1
			},
			{
				2,
				15008,
				1
			}
		}
	},
	[192] = {
		id = 192,
		rarity = 1,
		time = 3600,
		type = 1,
		name = "큐브 구조 해석",
		lv_limit = 30,
		desc = "성정 큐브를 문자 그대로 물리적으로 '분해'하여, 구조 분석을 진행한다. 성공 사례가 없을수록, 실험의 가치는 그만큼 크다고 볼 수 있다.",
		condition = 0,
		bg = "1",
		blueprint_version = 2,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "gold_1",
		label_color = "tag_h",
		consume = {
			{
				2,
				20001,
				3
			}
		},
		drop_client = {
			{
				2,
				52011,
				1
			},
			{
				2,
				54049,
				1
			},
			{
				2,
				15008,
				1
			}
		}
	},
	[193] = {
		id = 193,
		rarity = 2,
		time = 7200,
		type = 1,
		name = "큐브 구조 해석",
		lv_limit = 30,
		desc = "성정 큐브를 문자 그대로 물리적으로 '분해'하여, 구조 분석을 진행한다. 성공 사례가 없을수록, 실험의 가치는 그만큼 크다고 볼 수 있다.",
		condition = 0,
		bg = "2",
		blueprint_version = 2,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "gold_2",
		label_color = "tag_h",
		consume = {
			{
				2,
				20001,
				6
			}
		},
		drop_client = {
			{
				2,
				52011,
				1
			},
			{
				2,
				54050,
				1
			},
			{
				2,
				15008,
				1
			}
		}
	},
	[194] = {
		id = 194,
		rarity = 2,
		time = 7200,
		type = 1,
		name = "큐브 구조 해석",
		lv_limit = 30,
		desc = "성정 큐브를 문자 그대로 물리적으로 '분해'하여, 구조 분석을 진행한다. 성공 사례가 없을수록, 실험의 가치는 그만큼 크다고 볼 수 있다.",
		condition = 0,
		bg = "2",
		blueprint_version = 2,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "gold_2",
		label_color = "tag_h",
		consume = {
			{
				2,
				20001,
				6
			}
		},
		drop_client = {
			{
				2,
				52011,
				1
			},
			{
				2,
				54050,
				1
			},
			{
				2,
				15008,
				1
			}
		}
	},
	[201] = {
		id = 201,
		rarity = 1,
		time = 21600,
		type = 1,
		name = "C-153-MI",
		lv_limit = 30,
		desc = "성정 큐브에 관한 기초 이론은 큐브 구조에 대한 인식으로부터 시작된다. 큐브 기초 구조에 대한 이론 구조 작업은 여전히 전력을 다해 진행 중이다.",
		condition = 0,
		bg = "1",
		blueprint_version = 3,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "item_1",
		label_color = "tag_c",
		consume = {},
		drop_client = {
			{
				2,
				52022,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[202] = {
		id = 202,
		rarity = 1,
		time = 21600,
		type = 1,
		name = "C-153-MI",
		lv_limit = 30,
		desc = "성정 큐브에 관한 기초 이론은 큐브 구조에 대한 인식으로부터 시작된다. 큐브 기초 구조에 대한 이론 구조 작업은 여전히 전력을 다해 진행 중이다.",
		condition = 0,
		bg = "1",
		blueprint_version = 3,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "item_1",
		label_color = "tag_c",
		consume = {},
		drop_client = {
			{
				2,
				52022,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[203] = {
		id = 203,
		rarity = 2,
		time = 28800,
		type = 1,
		name = "C-185-MI",
		lv_limit = 30,
		desc = "성정 큐브에 관한 기초 이론은 큐브 구조에 대한 인식으로부터 시작된다. 큐브 기초 구조에 대한 이론 구조 작업은 여전히 전력을 다해 진행 중이다.",
		condition = 0,
		bg = "2",
		blueprint_version = 3,
		sub_name = "대형 항목",
		label = "tag_red",
		bg_icon = "item_2",
		label_color = "tag_c",
		consume = {},
		drop_client = {
			{
				2,
				52021,
				1
			},
			{
				2,
				52022,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[204] = {
		id = 204,
		rarity = 2,
		time = 28800,
		type = 1,
		name = "C-185-MI",
		lv_limit = 30,
		desc = "성정 큐브에 관한 기초 이론은 큐브 구조에 대한 인식으로부터 시작된다. 큐브 기초 구조에 대한 이론 구조 작업은 여전히 전력을 다해 진행 중이다.",
		condition = 0,
		bg = "2",
		blueprint_version = 3,
		sub_name = "대형 항목",
		label = "tag_red",
		bg_icon = "item_2",
		label_color = "tag_c",
		consume = {},
		drop_client = {
			{
				2,
				52021,
				1
			},
			{
				2,
				52022,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[211] = {
		id = 211,
		rarity = 1,
		time = 14400,
		type = 1,
		name = "B-622-MI",
		lv_limit = 30,
		desc = "성정 큐브 구조에 관한 여러가지 가설에 대해 실증실험을 진행하는 연구 프로젝트. 실전을 통해 데이터를 수집하여, 이론의 과학적 증명에 기여하자.",
		condition = 52001,
		bg = "1",
		blueprint_version = 3,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "battle_1",
		label_color = "tag_b",
		consume = {},
		drop_client = {
			{
				2,
				52021,
				1
			},
			{
				2,
				52022,
				1
			},
			{
				2,
				52004,
				1
			}
		}
	},
	[212] = {
		id = 212,
		rarity = 1,
		time = 14400,
		type = 1,
		name = "B-636-MI",
		lv_limit = 30,
		desc = "성정 큐브 구조에 관한 여러가지 가설에 대해 실증실험을 진행하는 연구 프로젝트. 실전을 통해 데이터를 수집하여, 이론의 과학적 증명에 기여하자.",
		condition = 52002,
		bg = "1",
		blueprint_version = 3,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "battle_1",
		label_color = "tag_b",
		consume = {},
		drop_client = {
			{
				2,
				52021,
				1
			},
			{
				2,
				52022,
				1
			},
			{
				2,
				52004,
				1
			}
		}
	},
	[213] = {
		id = 213,
		rarity = 1,
		time = 14400,
		type = 1,
		name = "B-654-MI",
		lv_limit = 30,
		desc = "성정 큐브 구조에 관한 여러가지 가설에 대해 실증실험을 진행하는 연구 프로젝트. 실전을 통해 데이터를 수집하여, 이론의 과학적 증명에 기여하자.",
		condition = 52003,
		bg = "1",
		blueprint_version = 3,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "battle_1",
		label_color = "tag_b",
		consume = {},
		drop_client = {
			{
				2,
				52021,
				1
			},
			{
				2,
				52022,
				1
			},
			{
				2,
				52004,
				1
			}
		}
	},
	[214] = {
		id = 214,
		rarity = 1,
		time = 14400,
		type = 1,
		name = "B-682-MI",
		lv_limit = 40,
		desc = "성정 큐브 구조에 관한 여러가지 가설에 대해 실증실험을 진행하는 연구 프로젝트. 실전을 통해 데이터를 수집하여, 이론의 과학적 증명에 기여하자.",
		condition = 52004,
		bg = "1",
		blueprint_version = 3,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "battle_1",
		label_color = "tag_b",
		consume = {},
		drop_client = {
			{
				2,
				52021,
				1
			},
			{
				2,
				52022,
				1
			},
			{
				2,
				52004,
				1
			}
		}
	},
	[215] = {
		id = 215,
		rarity = 2,
		time = 14400,
		type = 1,
		name = "B-235-MI",
		lv_limit = 50,
		desc = "성정 큐브 구조에 관한 여러가지 가설에 대해 실증실험을 진행하는 연구 프로젝트. 실전을 통해 데이터를 수집하여, 이론의 과학적 증명에 기여하자.",
		condition = 52005,
		bg = "2",
		blueprint_version = 3,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "battle_2",
		label_color = "tag_b",
		consume = {},
		drop_client = {
			{
				2,
				52021,
				1
			},
			{
				2,
				52022,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[216] = {
		id = 216,
		rarity = 2,
		time = 14400,
		type = 1,
		name = "B-268-MI",
		lv_limit = 60,
		desc = "성정 큐브 구조에 관한 여러가지 가설에 대해 실증실험을 진행하는 연구 프로젝트. 실전을 통해 데이터를 수집하여, 이론의 과학적 증명에 기여하자.",
		condition = 52006,
		bg = "2",
		blueprint_version = 3,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "battle_2",
		label_color = "tag_b",
		consume = {},
		drop_client = {
			{
				2,
				52021,
				1
			},
			{
				2,
				52022,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[217] = {
		id = 217,
		rarity = 2,
		time = 14400,
		type = 1,
		name = "B-128-MI",
		lv_limit = 65,
		desc = "성정 큐브 구조에 관한 여러가지 가설에 대해 실증실험을 진행하는 연구 프로젝트. 실전을 통해 데이터를 수집하여, 이론의 과학적 증명에 기여하자.",
		condition = 52007,
		bg = "2",
		blueprint_version = 3,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "battle_2",
		label_color = "tag_b",
		consume = {},
		drop_client = {
			{
				2,
				52021,
				1
			},
			{
				2,
				52022,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[218] = {
		id = 218,
		rarity = 2,
		time = 14400,
		type = 1,
		name = "B-164-MI",
		lv_limit = 70,
		desc = "성정 큐브 구조에 관한 여러가지 가설에 대해 실증실험을 진행하는 연구 프로젝트. 실전을 통해 데이터를 수집하여, 이론의 과학적 증명에 기여하자.",
		condition = 52008,
		bg = "2",
		blueprint_version = 3,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "battle_2",
		label_color = "tag_b",
		consume = {},
		drop_client = {
			{
				2,
				52021,
				1
			},
			{
				2,
				52022,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[221] = {
		id = 221,
		rarity = 1,
		time = 10800,
		type = 1,
		name = "T-018-MI",
		lv_limit = 30,
		desc = "성정 큐브에 관한 연구는 [건조] 방면뿐만 아니라, 일상생활 이용 등 다양한 방면으로 진행되고 있다. 성정 큐브의 다방면 활용 가능성을 찾는 프로젝트에는 군의 조력이 필수불가결이다.",
		condition = 52021,
		bg = "1",
		blueprint_version = 3,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "item_1",
		label_color = "tag_t",
		consume = {},
		drop_client = {
			{
				2,
				52022,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[222] = {
		id = 222,
		rarity = 1,
		time = 14400,
		type = 1,
		name = "T-384-MI",
		lv_limit = 30,
		desc = "성정 큐브에 관한 연구는 [건조] 방면뿐만 아니라, 일상생활 이용 등 다양한 방면으로 진행되고 있다. 성정 큐브의 다방면 활용 가능성을 찾는 프로젝트에는 군의 조력이 필수불가결이다.",
		condition = 52022,
		bg = "1",
		blueprint_version = 3,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "item_1",
		label_color = "tag_t",
		consume = {},
		drop_client = {
			{
				2,
				52022,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[223] = {
		id = 223,
		rarity = 2,
		time = 21600,
		type = 1,
		name = "T-249-MI",
		lv_limit = 30,
		desc = "성정 큐브에 관한 연구는 [건조] 방면뿐만 아니라, 일상생활 이용 등 다양한 방면으로 진행되고 있다. 성정 큐브의 다방면 활용 가능성을 찾는 프로젝트에는 군의 조력이 필수불가결이다.",
		condition = 52023,
		bg = "2",
		blueprint_version = 3,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "item_2",
		label_color = "tag_t",
		consume = {},
		drop_client = {
			{
				2,
				52022,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[231] = {
		id = 231,
		rarity = 1,
		time = 7200,
		type = 1,
		name = "E-031-MI",
		lv_limit = 30,
		desc = "성정 큐브와 [장비]의 관련성을 조사하는 다수의 시도는, 방대한 실험 데이터에 기반하여 진행되고 있다. 불필요한 장비를 제공하여, 실험 샘플을 충당하자.",
		condition = 52031,
		bg = "1",
		blueprint_version = 3,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "item_1",
		label_color = "tag_e",
		consume = {},
		drop_client = {
			{
				2,
				52022,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[232] = {
		id = 232,
		rarity = 2,
		time = 7200,
		type = 1,
		name = "E-315-MI",
		lv_limit = 30,
		desc = "성정 큐브와 [장비]의 관련성을 조사하는 다수의 시도는, 방대한 실험 데이터에 기반하여 진행되고 있다. 불필요한 장비를 제공하여, 실험 샘플을 충당하자.",
		condition = 52032,
		bg = "2",
		blueprint_version = 3,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "item_2",
		label_color = "tag_e",
		consume = {},
		drop_client = {
			{
				2,
				52022,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[233] = {
		id = 233,
		rarity = 2,
		time = 21600,
		type = 1,
		name = "E-121-MI",
		lv_limit = 50,
		desc = "현재 비치 중인 장비를 분해함으로써 기초 설계를 재검토하여, 추후 개발이 예정된 새 장비의 기초 연구를 진행한다.",
		condition = 0,
		bg = "2",
		blueprint_version = 3,
		sub_name = "중형 항목",
		label = "tag_blue",
		bg_icon = "item_2",
		label_color = "tag_e",
		consume = {
			{
				2,
				30023,
				15
			}
		},
		drop_client = {
			{
				2,
				13018,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[234] = {
		id = 234,
		rarity = 2,
		time = 21600,
		type = 1,
		name = "E-123-MI",
		lv_limit = 50,
		desc = "현재 비치 중인 장비를 분해함으로써 기초 설계를 재검토하여, 추후 개발이 예정된 새 장비의 기초 연구를 진행한다.",
		condition = 0,
		bg = "2",
		blueprint_version = 3,
		sub_name = "중형 항목",
		label = "tag_blue",
		bg_icon = "item_2",
		label_color = "tag_e",
		consume = {
			{
				2,
				30023,
				15
			}
		},
		drop_client = {
			{
				2,
				13020,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[235] = {
		id = 235,
		rarity = 2,
		time = 21600,
		type = 1,
		name = "E-124-MI",
		lv_limit = 50,
		desc = "현재 비치 중인 장비를 분해함으로써 기초 설계를 재검토하여, 추후 개발이 예정된 새 장비의 기초 연구를 진행한다.",
		condition = 0,
		bg = "2",
		blueprint_version = 3,
		sub_name = "중형 항목",
		label = "tag_blue",
		bg_icon = "item_2",
		label_color = "tag_e",
		consume = {
			{
				2,
				30023,
				15
			}
		},
		drop_client = {
			{
				2,
				13021,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[236] = {
		id = 236,
		rarity = 2,
		time = 21600,
		type = 1,
		name = "E-177-MI",
		lv_limit = 50,
		desc = "현재 비치 중인 장비를 분해함으로써 기초 설계를 재검토하여, 추후 개발이 예정된 새 장비의 기초 연구를 진행한다.",
		condition = 0,
		bg = "2",
		blueprint_version = 3,
		sub_name = "중형 항목",
		label = "tag_blue",
		bg_icon = "item_2",
		label_color = "tag_e",
		consume = {
			{
				2,
				30043,
				15
			}
		},
		drop_client = {
			{
				2,
				13022,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[237] = {
		id = 237,
		rarity = 2,
		time = 21600,
		type = 1,
		name = "E-195-MI",
		lv_limit = 50,
		desc = "현재 비치 중인 장비를 분해함으로써 기초 설계를 재검토하여, 추후 개발이 예정된 새 장비의 기초 연구를 진행한다.",
		condition = 0,
		bg = "2",
		blueprint_version = 3,
		sub_name = "중형 항목",
		label = "tag_blue",
		bg_icon = "item_2",
		label_color = "tag_e",
		consume = {
			{
				2,
				30013,
				15
			}
		},
		drop_client = {
			{
				2,
				13023,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[241] = {
		id = 241,
		rarity = 1,
		time = 5400,
		type = 1,
		name = "G-412-MI",
		lv_limit = 30,
		desc = "뛰어난 과학의 발전을 뒷받침하기 위해선 영민한 두뇌뿐만 아니라, 지원자로부터 제공되는 연구 자금도 필요하다. 남아도는 자금을 기부하여 기술의 연구를 서포트하자.",
		condition = 0,
		bg = "1",
		blueprint_version = 3,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "gold_1",
		label_color = "tag_g",
		consume = {
			{
				1,
				1,
				1500
			}
		},
		drop_client = {
			{
				2,
				52021,
				1
			},
			{
				2,
				52022,
				1
			},
			{
				2,
				52004,
				1
			}
		}
	},
	[242] = {
		id = 242,
		rarity = 1,
		time = 5400,
		type = 1,
		name = "G-412-MI",
		lv_limit = 30,
		desc = "뛰어난 과학의 발전을 뒷받침하기 위해선 영민한 두뇌뿐만 아니라, 지원자로부터 제공되는 연구 자금도 필요하다. 남아도는 자금을 기부하여 기술의 연구를 서포트하자.",
		condition = 0,
		bg = "1",
		blueprint_version = 3,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "gold_1",
		label_color = "tag_g",
		consume = {
			{
				1,
				1,
				1500
			}
		},
		drop_client = {
			{
				2,
				52021,
				1
			},
			{
				2,
				52022,
				1
			},
			{
				2,
				52004,
				1
			}
		}
	},
	[243] = {
		id = 243,
		rarity = 1,
		time = 9000,
		type = 1,
		name = "G-236-MI",
		lv_limit = 30,
		desc = "뛰어난 과학의 발전을 뒷받침하기 위해선 영민한 두뇌뿐만 아니라, 지원자로부터 제공되는 연구 자금도 필요하다. 남아도는 자금을 기부하여 기술의 연구를 서포트하자.",
		condition = 0,
		bg = "1",
		blueprint_version = 3,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "gold_1",
		label_color = "tag_g",
		consume = {
			{
				1,
				1,
				3000
			}
		},
		drop_client = {
			{
				2,
				52021,
				1
			},
			{
				2,
				52022,
				1
			},
			{
				2,
				52004,
				1
			}
		}
	},
	[244] = {
		id = 244,
		rarity = 1,
		time = 9000,
		type = 1,
		name = "G-236-MI",
		lv_limit = 30,
		desc = "뛰어난 과학의 발전을 뒷받침하기 위해선 영민한 두뇌뿐만 아니라, 지원자로부터 제공되는 연구 자금도 필요하다. 남아도는 자금을 기부하여 기술의 연구를 서포트하자.",
		condition = 0,
		bg = "1",
		blueprint_version = 3,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "gold_1",
		label_color = "tag_g",
		consume = {
			{
				1,
				1,
				3000
			}
		},
		drop_client = {
			{
				2,
				52021,
				1
			},
			{
				2,
				52022,
				1
			},
			{
				2,
				52004,
				1
			}
		}
	},
	[245] = {
		id = 245,
		rarity = 2,
		time = 14400,
		type = 1,
		name = "G-531-MI",
		lv_limit = 30,
		desc = "뛰어난 과학의 발전을 뒷받침하기 위해선 영민한 두뇌뿐만 아니라, 지원자로부터 제공되는 연구 자금도 필요하다. 남아도는 자금을 기부하여 기술의 연구를 서포트하자.",
		condition = 0,
		bg = "2",
		blueprint_version = 3,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "gold_2",
		label_color = "tag_g",
		consume = {
			{
				1,
				1,
				6000
			}
		},
		drop_client = {
			{
				2,
				52021,
				1
			},
			{
				2,
				52022,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[246] = {
		id = 246,
		rarity = 2,
		time = 14400,
		type = 1,
		name = "G-531-MI",
		lv_limit = 30,
		desc = "뛰어난 과학의 발전을 뒷받침하기 위해선 영민한 두뇌뿐만 아니라, 지원자로부터 제공되는 연구 자금도 필요하다. 남아도는 자금을 기부하여 기술의 연구를 서포트하자.",
		condition = 0,
		bg = "2",
		blueprint_version = 3,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "gold_2",
		label_color = "tag_g",
		consume = {
			{
				1,
				1,
				6000
			}
		},
		drop_client = {
			{
				2,
				52021,
				1
			},
			{
				2,
				52022,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[251] = {
		id = 251,
		rarity = 1,
		time = 9000,
		type = 1,
		name = "D-721-MI",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수의 사람들의 사물에 대한 인식의 집합체를 구현화하는 것이라면, 그것의 '방향성' 을 정하는 것에 따라 결과는 과연 어떻게 바뀔 것인가.？",
		condition = 0,
		bg = "1",
		blueprint_version = 3,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "chara_chaijun",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				3000
			}
		},
		drop_client = {
			{
				2,
				42021,
				1
			},
			{
				2,
				52022,
				1
			},
			{
				2,
				52004,
				1
			}
		}
	},
	[252] = {
		id = 252,
		rarity = 1,
		time = 9000,
		type = 1,
		name = "D-722-MI",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수의 사람들의 사물에 대한 인식의 집합체를 구현화하는 것이라면, 그것의 '방향성' 을 정하는 것에 따라 결과는 과연 어떻게 바뀔 것인가.？",
		condition = 0,
		bg = "1",
		blueprint_version = 3,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "chara_deleike",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				3000
			}
		},
		drop_client = {
			{
				2,
				42022,
				1
			},
			{
				2,
				52022,
				1
			},
			{
				2,
				52004,
				1
			}
		}
	},
	[253] = {
		id = 253,
		rarity = 1,
		time = 9000,
		type = 1,
		name = "D-772-MI",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수의 사람들의 사물에 대한 인식의 집합체를 구현화하는 것이라면, 그것의 '방향성' 을 정하는 것에 따라 결과는 과연 어떻게 바뀔 것인가.？",
		condition = 0,
		bg = "1",
		blueprint_version = 3,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "chara_meiyinci",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				3000
			}
		},
		drop_client = {
			{
				2,
				42023,
				1
			},
			{
				2,
				52022,
				1
			},
			{
				2,
				52004,
				1
			}
		}
	},
	[254] = {
		id = 254,
		rarity = 1,
		time = 9000,
		type = 1,
		name = "D-777-MI",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수의 사람들의 사물에 대한 인식의 집합체를 구현화하는 것이라면, 그것의 '방향성' 을 정하는 것에 따라 결과는 과연 어떻게 바뀔 것인가.？",
		condition = 0,
		bg = "1",
		blueprint_version = 3,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "chara_aoding",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				3000
			}
		},
		drop_client = {
			{
				2,
				42024,
				1
			},
			{
				2,
				52022,
				1
			},
			{
				2,
				52004,
				1
			}
		}
	},
	[255] = {
		id = 255,
		rarity = 1,
		time = 9000,
		type = 1,
		name = "D-795-MI",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수의 사람들의 사물에 대한 인식의 집합체를 구현화하는 것이라면, 그것의 '방향성' 을 정하는 것에 따라 결과는 과연 어떻게 바뀔 것인가.？",
		condition = 0,
		bg = "1",
		blueprint_version = 3,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "chara_xiangbin",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				3000
			}
		},
		drop_client = {
			{
				2,
				42025,
				1
			},
			{
				2,
				52022,
				1
			},
			{
				2,
				52004,
				1
			}
		}
	},
	[261] = {
		id = 261,
		rarity = 2,
		time = 18000,
		type = 1,
		name = "D-321-MI",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수의 사람들의 사물에 대한 인식의 집합체를 구현화하는 것이라면, 그것의 '방향성' 을 정하는 것에 따라 결과는 과연 어떻게 바뀔 것인가.？",
		condition = 0,
		bg = "2",
		blueprint_version = 3,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "chara_chaijun",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				5000
			}
		},
		drop_client = {
			{
				2,
				42021,
				1
			},
			{
				2,
				52022,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[262] = {
		id = 262,
		rarity = 2,
		time = 18000,
		type = 1,
		name = "D-322-MI",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수의 사람들의 사물에 대한 인식의 집합체를 구현화하는 것이라면, 그것의 '방향성' 을 정하는 것에 따라 결과는 과연 어떻게 바뀔 것인가.？",
		condition = 0,
		bg = "2",
		blueprint_version = 3,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "chara_deleike",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				5000
			}
		},
		drop_client = {
			{
				2,
				42022,
				1
			},
			{
				2,
				52022,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[263] = {
		id = 263,
		rarity = 2,
		time = 18000,
		type = 1,
		name = "D-372-MI",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수의 사람들의 사물에 대한 인식의 집합체를 구현화하는 것이라면, 그것의 '방향성' 을 정하는 것에 따라 결과는 과연 어떻게 바뀔 것인가.？",
		condition = 0,
		bg = "2",
		blueprint_version = 3,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "chara_meiyinci",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				5000
			}
		},
		drop_client = {
			{
				2,
				42023,
				1
			},
			{
				2,
				52022,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[264] = {
		id = 264,
		rarity = 2,
		time = 18000,
		type = 1,
		name = "D-377-MI",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수의 사람들의 사물에 대한 인식의 집합체를 구현화하는 것이라면, 그것의 '방향성' 을 정하는 것에 따라 결과는 과연 어떻게 바뀔 것인가.？",
		condition = 0,
		bg = "2",
		blueprint_version = 3,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "chara_aoding",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				5000
			}
		},
		drop_client = {
			{
				2,
				42024,
				1
			},
			{
				2,
				52022,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[265] = {
		id = 265,
		rarity = 2,
		time = 18000,
		type = 1,
		name = "D-395-MI",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수의 사람들의 사물에 대한 인식의 집합체를 구현화하는 것이라면, 그것의 '방향성' 을 정하는 것에 따라 결과는 과연 어떻게 바뀔 것인가.？",
		condition = 0,
		bg = "2",
		blueprint_version = 3,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "chara_xiangbin",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				5000
			}
		},
		drop_client = {
			{
				2,
				42025,
				1
			},
			{
				2,
				52022,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[271] = {
		id = 271,
		rarity = 1,
		time = 3600,
		type = 1,
		name = "Q-302-MI",
		lv_limit = 30,
		desc = "장비와 성정 큐브의 공명 현상을 제어할 수 있도록, 갖은 수단을 동원해 해당 현상을 재현하고, 최대한 효과적으로 이용할 수 있도록 한다.",
		condition = 0,
		bg = "1",
		blueprint_version = 3,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_1",
		label_color = "tag_q",
		consume = {
			{
				2,
				17001,
				20
			}
		},
		drop_client = {
			{
				2,
				52022,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[272] = {
		id = 272,
		rarity = 1,
		time = 3600,
		type = 1,
		name = "Q-310-MI",
		lv_limit = 30,
		desc = "장비와 성정 큐브의 공명 현상을 제어할 수 있도록, 갖은 수단을 동원해 해당 현상을 재현하고, 최대한 효과적으로 이용할 수 있도록 한다.",
		condition = 0,
		bg = "1",
		blueprint_version = 3,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_1",
		label_color = "tag_q",
		consume = {
			{
				2,
				17011,
				20
			}
		},
		drop_client = {
			{
				2,
				52022,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[273] = {
		id = 273,
		rarity = 1,
		time = 3600,
		type = 1,
		name = "Q-351-MI",
		lv_limit = 30,
		desc = "장비와 성정 큐브의 공명 현상을 제어할 수 있도록, 갖은 수단을 동원해 해당 현상을 재현하고, 최대한 효과적으로 이용할 수 있도록 한다.",
		condition = 0,
		bg = "1",
		blueprint_version = 3,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_1",
		label_color = "tag_q",
		consume = {
			{
				2,
				17021,
				20
			}
		},
		drop_client = {
			{
				2,
				52022,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[274] = {
		id = 274,
		rarity = 1,
		time = 3600,
		type = 1,
		name = "Q-368-MI",
		lv_limit = 30,
		desc = "장비와 성정 큐브의 공명 현상을 제어할 수 있도록, 갖은 수단을 동원해 해당 현상을 재현하고, 최대한 효과적으로 이용할 수 있도록 한다.",
		condition = 0,
		bg = "1",
		blueprint_version = 3,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_1",
		label_color = "tag_q",
		consume = {
			{
				2,
				17031,
				20
			}
		},
		drop_client = {
			{
				2,
				52022,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[275] = {
		id = 275,
		rarity = 1,
		time = 3600,
		type = 1,
		name = "Q-389-MI",
		lv_limit = 30,
		desc = "장비와 성정 큐브의 공명 현상을 제어할 수 있도록, 갖은 수단을 동원해 해당 현상을 재현하고, 최대한 효과적으로 이용할 수 있도록 한다.",
		condition = 0,
		bg = "1",
		blueprint_version = 3,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_1",
		label_color = "tag_q",
		consume = {
			{
				2,
				17041,
				20
			}
		},
		drop_client = {
			{
				2,
				52022,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[276] = {
		id = 276,
		rarity = 1,
		time = 7200,
		type = 1,
		name = "Q-202-MI",
		lv_limit = 30,
		desc = "장비와 성정 큐브의 공명 현상을 제어할 수 있도록, 갖은 수단을 동원해 해당 현상을 재현하고, 최대한 효과적으로 이용할 수 있도록 한다.",
		condition = 0,
		bg = "1",
		blueprint_version = 3,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_1",
		label_color = "tag_q",
		consume = {
			{
				2,
				17002,
				20
			}
		},
		drop_client = {
			{
				2,
				52022,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[277] = {
		id = 277,
		rarity = 1,
		time = 7200,
		type = 1,
		name = "Q-210-MI",
		lv_limit = 30,
		desc = "장비와 성정 큐브의 공명 현상을 제어할 수 있도록, 갖은 수단을 동원해 해당 현상을 재현하고, 최대한 효과적으로 이용할 수 있도록 한다.",
		condition = 0,
		bg = "1",
		blueprint_version = 3,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_1",
		label_color = "tag_q",
		consume = {
			{
				2,
				17012,
				20
			}
		},
		drop_client = {
			{
				2,
				52022,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[278] = {
		id = 278,
		rarity = 1,
		time = 7200,
		type = 1,
		name = "Q-251-MI",
		lv_limit = 30,
		desc = "장비와 성정 큐브의 공명 현상을 제어할 수 있도록, 갖은 수단을 동원해 해당 현상을 재현하고, 최대한 효과적으로 이용할 수 있도록 한다.",
		condition = 0,
		bg = "1",
		blueprint_version = 3,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_1",
		label_color = "tag_q",
		consume = {
			{
				2,
				17022,
				20
			}
		},
		drop_client = {
			{
				2,
				52022,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[279] = {
		id = 279,
		rarity = 1,
		time = 7200,
		type = 1,
		name = "Q-268-MI",
		lv_limit = 30,
		desc = "장비와 성정 큐브의 공명 현상을 제어할 수 있도록, 갖은 수단을 동원해 해당 현상을 재현하고, 최대한 효과적으로 이용할 수 있도록 한다.",
		condition = 0,
		bg = "1",
		blueprint_version = 3,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_1",
		label_color = "tag_q",
		consume = {
			{
				2,
				17032,
				20
			}
		},
		drop_client = {
			{
				2,
				52022,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[280] = {
		id = 280,
		rarity = 1,
		time = 7200,
		type = 1,
		name = "Q-289-MI",
		lv_limit = 30,
		desc = "장비와 성정 큐브의 공명 현상을 제어할 수 있도록, 갖은 수단을 동원해 해당 현상을 재현하고, 최대한 효과적으로 이용할 수 있도록 한다.",
		condition = 0,
		bg = "1",
		blueprint_version = 3,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_1",
		label_color = "tag_q",
		consume = {
			{
				2,
				17042,
				20
			}
		},
		drop_client = {
			{
				2,
				52022,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[281] = {
		id = 281,
		rarity = 2,
		time = 14400,
		type = 1,
		name = "Q-002-MI",
		lv_limit = 30,
		desc = "장비와 성정 큐브의 공명 현상을 제어할 수 있도록, 갖은 수단을 동원해 해당 현상을 재현하고, 최대한 효과적으로 이용할 수 있도록 한다.",
		condition = 0,
		bg = "2",
		blueprint_version = 3,
		sub_name = "중형 항목",
		label = "tag_blue",
		bg_icon = "gold_2",
		label_color = "tag_q",
		consume = {
			{
				2,
				17003,
				20
			}
		},
		drop_client = {
			{
				2,
				52022,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[282] = {
		id = 282,
		rarity = 2,
		time = 14400,
		type = 1,
		name = "Q-010-MI",
		lv_limit = 30,
		desc = "장비와 성정 큐브의 공명 현상을 제어할 수 있도록, 갖은 수단을 동원해 해당 현상을 재현하고, 최대한 효과적으로 이용할 수 있도록 한다.",
		condition = 0,
		bg = "2",
		blueprint_version = 3,
		sub_name = "중형 항목",
		label = "tag_blue",
		bg_icon = "gold_2",
		label_color = "tag_q",
		consume = {
			{
				2,
				17013,
				20
			}
		},
		drop_client = {
			{
				2,
				52022,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[283] = {
		id = 283,
		rarity = 2,
		time = 14400,
		type = 1,
		name = "Q-051-MI",
		lv_limit = 30,
		desc = "장비와 성정 큐브의 공명 현상을 제어할 수 있도록, 갖은 수단을 동원해 해당 현상을 재현하고, 최대한 효과적으로 이용할 수 있도록 한다.",
		condition = 0,
		bg = "2",
		blueprint_version = 3,
		sub_name = "중형 항목",
		label = "tag_blue",
		bg_icon = "gold_2",
		label_color = "tag_q",
		consume = {
			{
				2,
				17023,
				20
			}
		},
		drop_client = {
			{
				2,
				52022,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[284] = {
		id = 284,
		rarity = 2,
		time = 14400,
		type = 1,
		name = "Q-068-MI",
		lv_limit = 30,
		desc = "장비와 성정 큐브의 공명 현상을 제어할 수 있도록, 갖은 수단을 동원해 해당 현상을 재현하고, 최대한 효과적으로 이용할 수 있도록 한다.",
		condition = 0,
		bg = "2",
		blueprint_version = 3,
		sub_name = "중형 항목",
		label = "tag_blue",
		bg_icon = "gold_2",
		label_color = "tag_q",
		consume = {
			{
				2,
				17033,
				20
			}
		},
		drop_client = {
			{
				2,
				52022,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[285] = {
		id = 285,
		rarity = 2,
		time = 14400,
		type = 1,
		name = "Q-089-MI",
		lv_limit = 30,
		desc = "장비와 성정 큐브의 공명 현상을 제어할 수 있도록, 갖은 수단을 동원해 해당 현상을 재현하고, 최대한 효과적으로 이용할 수 있도록 한다.",
		condition = 0,
		bg = "2",
		blueprint_version = 3,
		sub_name = "중형 항목",
		label = "tag_blue",
		bg_icon = "gold_2",
		label_color = "tag_q",
		consume = {
			{
				2,
				17043,
				20
			}
		},
		drop_client = {
			{
				2,
				52022,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[291] = {
		id = 291,
		rarity = 1,
		time = 3600,
		type = 1,
		name = "H-387-MI",
		lv_limit = 30,
		desc = "성정 큐브를 문자 그대로 물리적으로 '분해'하여, 구조 분석을 진행한다. 성공 사례가 없을수록, 실험의 가치는 그만큼 크다고 볼 수 있다.",
		condition = 0,
		bg = "1",
		blueprint_version = 3,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "gold_1",
		label_color = "tag_h",
		consume = {
			{
				2,
				20001,
				3
			}
		},
		drop_client = {
			{
				2,
				52021,
				1
			},
			{
				2,
				54049,
				1
			},
			{
				2,
				15008,
				1
			}
		}
	},
	[292] = {
		id = 292,
		rarity = 1,
		time = 3600,
		type = 1,
		name = "H-387-MI",
		lv_limit = 30,
		desc = "성정 큐브를 문자 그대로 물리적으로 '분해'하여, 구조 분석을 진행한다. 성공 사례가 없을수록, 실험의 가치는 그만큼 크다고 볼 수 있다.",
		condition = 0,
		bg = "1",
		blueprint_version = 3,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "gold_1",
		label_color = "tag_h",
		consume = {
			{
				2,
				20001,
				3
			}
		},
		drop_client = {
			{
				2,
				52021,
				1
			},
			{
				2,
				54049,
				1
			},
			{
				2,
				15008,
				1
			}
		}
	},
	[293] = {
		id = 293,
		rarity = 2,
		time = 7200,
		type = 1,
		name = "H-339-MI",
		lv_limit = 30,
		desc = "성정 큐브를 문자 그대로 물리적으로 '분해'하여, 구조 분석을 진행한다. 성공 사례가 없을수록, 실험의 가치는 그만큼 크다고 볼 수 있다.",
		condition = 0,
		bg = "2",
		blueprint_version = 3,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "gold_2",
		label_color = "tag_h",
		consume = {
			{
				2,
				20001,
				6
			}
		},
		drop_client = {
			{
				2,
				52021,
				1
			},
			{
				2,
				54050,
				1
			},
			{
				2,
				15008,
				1
			}
		}
	},
	[294] = {
		id = 294,
		rarity = 2,
		time = 7200,
		type = 1,
		name = "H-339-MI",
		lv_limit = 30,
		desc = "성정 큐브를 문자 그대로 물리적으로 '분해'하여, 구조 분석을 진행한다. 성공 사례가 없을수록, 실험의 가치는 그만큼 크다고 볼 수 있다.",
		condition = 0,
		bg = "2",
		blueprint_version = 3,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "gold_2",
		label_color = "tag_h",
		consume = {
			{
				2,
				20001,
				6
			}
		},
		drop_client = {
			{
				2,
				52021,
				1
			},
			{
				2,
				54050,
				1
			},
			{
				2,
				15008,
				1
			}
		}
	},
	[301] = {
		id = 301,
		rarity = 1,
		time = 21600,
		type = 1,
		name = "C-153-MI",
		lv_limit = 30,
		desc = "성정 큐브에 관한 기초 이론은 큐브 구조에 대한 인식으로부터 시작된다. 큐브 기초 구조에 대한 이론 구조 작업은 여전히 전력을 다해 진행 중이다.",
		condition = 0,
		bg = "1",
		blueprint_version = 4,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "item_1",
		label_color = "tag_c",
		consume = {},
		drop_client = {
			{
				2,
				52032,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[302] = {
		id = 302,
		rarity = 1,
		time = 21600,
		type = 1,
		name = "C-153-MI",
		lv_limit = 30,
		desc = "성정 큐브에 관한 기초 이론은 큐브 구조에 대한 인식으로부터 시작된다. 큐브 기초 구조에 대한 이론 구조 작업은 여전히 전력을 다해 진행 중이다.",
		condition = 0,
		bg = "1",
		blueprint_version = 4,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "item_1",
		label_color = "tag_c",
		consume = {},
		drop_client = {
			{
				2,
				52032,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[303] = {
		id = 303,
		rarity = 2,
		time = 28800,
		type = 1,
		name = "C-185-MI",
		lv_limit = 30,
		desc = "성정 큐브에 관한 기초 이론은 큐브 구조에 대한 인식으로부터 시작된다. 큐브 기초 구조에 대한 이론 구조 작업은 여전히 전력을 다해 진행 중이다.",
		condition = 0,
		bg = "2",
		blueprint_version = 4,
		sub_name = "대형 항목",
		label = "tag_red",
		bg_icon = "item_2",
		label_color = "tag_c",
		consume = {},
		drop_client = {
			{
				2,
				52031,
				1
			},
			{
				2,
				52032,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[304] = {
		id = 304,
		rarity = 2,
		time = 28800,
		type = 1,
		name = "C-185-MI",
		lv_limit = 30,
		desc = "성정 큐브에 관한 기초 이론은 큐브 구조에 대한 인식으로부터 시작된다. 큐브 기초 구조에 대한 이론 구조 작업은 여전히 전력을 다해 진행 중이다.",
		condition = 0,
		bg = "2",
		blueprint_version = 4,
		sub_name = "대형 항목",
		label = "tag_red",
		bg_icon = "item_2",
		label_color = "tag_c",
		consume = {},
		drop_client = {
			{
				2,
				52031,
				1
			},
			{
				2,
				52032,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[311] = {
		id = 311,
		rarity = 1,
		time = 14400,
		type = 1,
		name = "B-622-MI",
		lv_limit = 30,
		desc = "성정 큐브 구조에 관한 여러가지 가설에 대해 실증실험을 진행하는 연구 프로젝트. 실전을 통해 데이터를 수집하여, 이론의 과학적 증명에 기여하자.",
		condition = 52001,
		bg = "1",
		blueprint_version = 4,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "battle_1",
		label_color = "tag_b",
		consume = {},
		drop_client = {
			{
				2,
				52031,
				1
			},
			{
				2,
				52032,
				1
			},
			{
				2,
				52004,
				1
			}
		}
	},
	[312] = {
		id = 312,
		rarity = 1,
		time = 14400,
		type = 1,
		name = "B-636-MI",
		lv_limit = 30,
		desc = "성정 큐브 구조에 관한 여러가지 가설에 대해 실증실험을 진행하는 연구 프로젝트. 실전을 통해 데이터를 수집하여, 이론의 과학적 증명에 기여하자.",
		condition = 52002,
		bg = "1",
		blueprint_version = 4,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "battle_1",
		label_color = "tag_b",
		consume = {},
		drop_client = {
			{
				2,
				52031,
				1
			},
			{
				2,
				52032,
				1
			},
			{
				2,
				52004,
				1
			}
		}
	},
	[313] = {
		id = 313,
		rarity = 1,
		time = 14400,
		type = 1,
		name = "B-654-MI",
		lv_limit = 30,
		desc = "성정 큐브 구조에 관한 여러가지 가설에 대해 실증실험을 진행하는 연구 프로젝트. 실전을 통해 데이터를 수집하여, 이론의 과학적 증명에 기여하자.",
		condition = 52003,
		bg = "1",
		blueprint_version = 4,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "battle_1",
		label_color = "tag_b",
		consume = {},
		drop_client = {
			{
				2,
				52031,
				1
			},
			{
				2,
				52032,
				1
			},
			{
				2,
				52004,
				1
			}
		}
	},
	[314] = {
		id = 314,
		rarity = 1,
		time = 14400,
		type = 1,
		name = "B-682-MI",
		lv_limit = 40,
		desc = "성정 큐브 구조에 관한 여러가지 가설에 대해 실증실험을 진행하는 연구 프로젝트. 실전을 통해 데이터를 수집하여, 이론의 과학적 증명에 기여하자.",
		condition = 52004,
		bg = "1",
		blueprint_version = 4,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "battle_1",
		label_color = "tag_b",
		consume = {},
		drop_client = {
			{
				2,
				52031,
				1
			},
			{
				2,
				52032,
				1
			},
			{
				2,
				52004,
				1
			}
		}
	},
	[315] = {
		id = 315,
		rarity = 2,
		time = 14400,
		type = 1,
		name = "B-235-MI",
		lv_limit = 50,
		desc = "성정 큐브 구조에 관한 여러가지 가설에 대해 실증실험을 진행하는 연구 프로젝트. 실전을 통해 데이터를 수집하여, 이론의 과학적 증명에 기여하자.",
		condition = 52005,
		bg = "2",
		blueprint_version = 4,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "battle_2",
		label_color = "tag_b",
		consume = {},
		drop_client = {
			{
				2,
				52031,
				1
			},
			{
				2,
				52032,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[316] = {
		id = 316,
		rarity = 2,
		time = 14400,
		type = 1,
		name = "B-268-MI",
		lv_limit = 60,
		desc = "성정 큐브 구조에 관한 여러가지 가설에 대해 실증실험을 진행하는 연구 프로젝트. 실전을 통해 데이터를 수집하여, 이론의 과학적 증명에 기여하자.",
		condition = 52006,
		bg = "2",
		blueprint_version = 4,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "battle_2",
		label_color = "tag_b",
		consume = {},
		drop_client = {
			{
				2,
				52031,
				1
			},
			{
				2,
				52032,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[317] = {
		id = 317,
		rarity = 2,
		time = 14400,
		type = 1,
		name = "B-128-MI",
		lv_limit = 65,
		desc = "성정 큐브 구조에 관한 여러가지 가설에 대해 실증실험을 진행하는 연구 프로젝트. 실전을 통해 데이터를 수집하여, 이론의 과학적 증명에 기여하자.",
		condition = 52007,
		bg = "2",
		blueprint_version = 4,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "battle_2",
		label_color = "tag_b",
		consume = {},
		drop_client = {
			{
				2,
				52031,
				1
			},
			{
				2,
				52032,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[318] = {
		id = 318,
		rarity = 2,
		time = 14400,
		type = 1,
		name = "B-164-MI",
		lv_limit = 70,
		desc = "성정 큐브 구조에 관한 여러가지 가설에 대해 실증실험을 진행하는 연구 프로젝트. 실전을 통해 데이터를 수집하여, 이론의 과학적 증명에 기여하자.",
		condition = 52008,
		bg = "2",
		blueprint_version = 4,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "battle_2",
		label_color = "tag_b",
		consume = {},
		drop_client = {
			{
				2,
				52031,
				1
			},
			{
				2,
				52032,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[321] = {
		id = 321,
		rarity = 1,
		time = 10800,
		type = 1,
		name = "T-018-MI",
		lv_limit = 30,
		desc = "성정 큐브에 관한 연구는 [건조] 방면뿐만 아니라, 일상생활 이용 등 다양한 방면으로 진행되고 있다. 성정 큐브의 다방면 활용 가능성을 찾는 프로젝트에는 군의 조력이 필수불가결이다.",
		condition = 52021,
		bg = "1",
		blueprint_version = 4,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "item_1",
		label_color = "tag_t",
		consume = {},
		drop_client = {
			{
				2,
				52032,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[322] = {
		id = 322,
		rarity = 1,
		time = 14400,
		type = 1,
		name = "T-384-MI",
		lv_limit = 30,
		desc = "성정 큐브에 관한 연구는 [건조] 방면뿐만 아니라, 일상생활 이용 등 다양한 방면으로 진행되고 있다. 성정 큐브의 다방면 활용 가능성을 찾는 프로젝트에는 군의 조력이 필수불가결이다.",
		condition = 52022,
		bg = "1",
		blueprint_version = 4,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "item_1",
		label_color = "tag_t",
		consume = {},
		drop_client = {
			{
				2,
				52032,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[323] = {
		id = 323,
		rarity = 2,
		time = 21600,
		type = 1,
		name = "T-249-MI",
		lv_limit = 30,
		desc = "성정 큐브에 관한 연구는 [건조] 방면뿐만 아니라, 일상생활 이용 등 다양한 방면으로 진행되고 있다. 성정 큐브의 다방면 활용 가능성을 찾는 프로젝트에는 군의 조력이 필수불가결이다.",
		condition = 52023,
		bg = "2",
		blueprint_version = 4,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "item_2",
		label_color = "tag_t",
		consume = {},
		drop_client = {
			{
				2,
				52032,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[331] = {
		id = 331,
		rarity = 1,
		time = 7200,
		type = 1,
		name = "E-031-MI",
		lv_limit = 30,
		desc = "성정 큐브와 [장비]의 관련성을 조사하는 다수의 시도는, 방대한 실험 데이터에 기반하여 진행되고 있다. 불필요한 장비를 제공하여, 실험 샘플을 충당하자.",
		condition = 52031,
		bg = "1",
		blueprint_version = 4,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "item_1",
		label_color = "tag_e",
		consume = {},
		drop_client = {
			{
				2,
				52032,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[332] = {
		id = 332,
		rarity = 2,
		time = 7200,
		type = 1,
		name = "E-315-MI",
		lv_limit = 30,
		desc = "성정 큐브와 [장비]의 관련성을 조사하는 다수의 시도는, 방대한 실험 데이터에 기반하여 진행되고 있다. 불필요한 장비를 제공하여, 실험 샘플을 충당하자.",
		condition = 52032,
		bg = "2",
		blueprint_version = 4,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "item_2",
		label_color = "tag_e",
		consume = {},
		drop_client = {
			{
				2,
				52032,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[333] = {
		id = 333,
		rarity = 2,
		time = 21600,
		type = 1,
		name = "E-175-MI",
		lv_limit = 50,
		desc = "현재 비치 중인 장비를 분해함으로써 기초 설계를 재검토하여, 추후 개발이 예정된 새 장비의 기초 연구를 진행한다.",
		condition = 0,
		bg = "2",
		blueprint_version = 4,
		sub_name = "중형 항목",
		label = "tag_blue",
		bg_icon = "item_2",
		label_color = "tag_e",
		consume = {
			{
				2,
				30043,
				15
			}
		},
		drop_client = {
			{
				2,
				13025,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[334] = {
		id = 334,
		rarity = 2,
		time = 21600,
		type = 1,
		name = "E-182-MI",
		lv_limit = 50,
		desc = "현재 비치 중인 장비를 분해함으로써 기초 설계를 재검토하여, 추후 개발이 예정된 새 장비의 기초 연구를 진행한다.",
		condition = 0,
		bg = "2",
		blueprint_version = 4,
		sub_name = "중형 항목",
		label = "tag_blue",
		bg_icon = "item_2",
		label_color = "tag_e",
		consume = {
			{
				2,
				30043,
				15
			}
		},
		drop_client = {
			{
				2,
				13026,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[335] = {
		id = 335,
		rarity = 2,
		time = 21600,
		type = 1,
		name = "E-168-MI",
		lv_limit = 50,
		desc = "현재 비치 중인 장비를 분해함으로써 기초 설계를 재검토하여, 추후 개발이 예정된 새 장비의 기초 연구를 진행한다.",
		condition = 0,
		bg = "2",
		blueprint_version = 4,
		sub_name = "중형 항목",
		label = "tag_blue",
		bg_icon = "item_2",
		label_color = "tag_e",
		consume = {
			{
				2,
				30043,
				15
			}
		},
		drop_client = {
			{
				2,
				13027,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[336] = {
		id = 336,
		rarity = 2,
		time = 21600,
		type = 1,
		name = "E-108-MI",
		lv_limit = 50,
		desc = "현재 비치 중인 장비를 분해함으로써 기초 설계를 재검토하여, 추후 개발이 예정된 새 장비의 기초 연구를 진행한다.",
		condition = 0,
		bg = "2",
		blueprint_version = 4,
		sub_name = "중형 항목",
		label = "tag_blue",
		bg_icon = "item_2",
		label_color = "tag_e",
		consume = {
			{
				2,
				30043,
				15
			}
		},
		drop_client = {
			{
				2,
				13028,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[337] = {
		id = 337,
		rarity = 2,
		time = 21600,
		type = 1,
		name = "E-163-MI",
		lv_limit = 50,
		desc = "현재 비치 중인 장비를 분해함으로써 기초 설계를 재검토하여, 추후 개발이 예정된 새 장비의 기초 연구를 진행한다.",
		condition = 0,
		bg = "2",
		blueprint_version = 4,
		sub_name = "중형 항목",
		label = "tag_blue",
		bg_icon = "item_2",
		label_color = "tag_e",
		consume = {
			{
				2,
				30033,
				15
			}
		},
		drop_client = {
			{
				2,
				13029,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[341] = {
		id = 341,
		rarity = 1,
		time = 5400,
		type = 1,
		name = "G-412-MI",
		lv_limit = 30,
		desc = "뛰어난 과학의 발전을 뒷받침하기 위해선 영민한 두뇌뿐만 아니라, 지원자로부터 제공되는 연구 자금도 필요하다. 남아도는 자금을 기부하여 기술의 연구를 서포트하자.",
		condition = 0,
		bg = "1",
		blueprint_version = 4,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "gold_1",
		label_color = "tag_g",
		consume = {
			{
				1,
				1,
				1500
			}
		},
		drop_client = {
			{
				2,
				52031,
				1
			},
			{
				2,
				52032,
				1
			},
			{
				2,
				52004,
				1
			}
		}
	},
	[342] = {
		id = 342,
		rarity = 1,
		time = 5400,
		type = 1,
		name = "G-412-MI",
		lv_limit = 30,
		desc = "뛰어난 과학의 발전을 뒷받침하기 위해선 영민한 두뇌뿐만 아니라, 지원자로부터 제공되는 연구 자금도 필요하다. 남아도는 자금을 기부하여 기술의 연구를 서포트하자.",
		condition = 0,
		bg = "1",
		blueprint_version = 4,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "gold_1",
		label_color = "tag_g",
		consume = {
			{
				1,
				1,
				1500
			}
		},
		drop_client = {
			{
				2,
				52031,
				1
			},
			{
				2,
				52032,
				1
			},
			{
				2,
				52004,
				1
			}
		}
	},
	[343] = {
		id = 343,
		rarity = 1,
		time = 9000,
		type = 1,
		name = "G-236-MI",
		lv_limit = 30,
		desc = "뛰어난 과학의 발전을 뒷받침하기 위해선 영민한 두뇌뿐만 아니라, 지원자로부터 제공되는 연구 자금도 필요하다. 남아도는 자금을 기부하여 기술의 연구를 서포트하자.",
		condition = 0,
		bg = "1",
		blueprint_version = 4,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "gold_1",
		label_color = "tag_g",
		consume = {
			{
				1,
				1,
				3000
			}
		},
		drop_client = {
			{
				2,
				52031,
				1
			},
			{
				2,
				52032,
				1
			},
			{
				2,
				52004,
				1
			}
		}
	},
	[344] = {
		id = 344,
		rarity = 1,
		time = 9000,
		type = 1,
		name = "G-236-MI",
		lv_limit = 30,
		desc = "뛰어난 과학의 발전을 뒷받침하기 위해선 영민한 두뇌뿐만 아니라, 지원자로부터 제공되는 연구 자금도 필요하다. 남아도는 자금을 기부하여 기술의 연구를 서포트하자.",
		condition = 0,
		bg = "1",
		blueprint_version = 4,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "gold_1",
		label_color = "tag_g",
		consume = {
			{
				1,
				1,
				3000
			}
		},
		drop_client = {
			{
				2,
				52031,
				1
			},
			{
				2,
				52032,
				1
			},
			{
				2,
				52004,
				1
			}
		}
	},
	[345] = {
		id = 345,
		rarity = 2,
		time = 14400,
		type = 1,
		name = "G-531-MI",
		lv_limit = 30,
		desc = "뛰어난 과학의 발전을 뒷받침하기 위해선 영민한 두뇌뿐만 아니라, 지원자로부터 제공되는 연구 자금도 필요하다. 남아도는 자금을 기부하여 기술의 연구를 서포트하자.",
		condition = 0,
		bg = "2",
		blueprint_version = 4,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "gold_2",
		label_color = "tag_g",
		consume = {
			{
				1,
				1,
				6000
			}
		},
		drop_client = {
			{
				2,
				52031,
				1
			},
			{
				2,
				52032,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[346] = {
		id = 346,
		rarity = 2,
		time = 14400,
		type = 1,
		name = "G-531-MI",
		lv_limit = 30,
		desc = "뛰어난 과학의 발전을 뒷받침하기 위해선 영민한 두뇌뿐만 아니라, 지원자로부터 제공되는 연구 자금도 필요하다. 남아도는 자금을 기부하여 기술의 연구를 서포트하자.",
		condition = 0,
		bg = "2",
		blueprint_version = 4,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "gold_2",
		label_color = "tag_g",
		consume = {
			{
				1,
				1,
				6000
			}
		},
		drop_client = {
			{
				2,
				52031,
				1
			},
			{
				2,
				52032,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[351] = {
		id = 351,
		rarity = 1,
		time = 9000,
		type = 1,
		name = "D-708-MI",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수의 사람들의 사물에 대한 인식의 집합체를 구현화하는 것이라면, 그것의 '방향성' 을 정하는 것에 따라 결과는 과연 어떻게 바뀔 것인가.",
		condition = 0,
		bg = "1",
		blueprint_version = 4,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "chara_ankeleiqi",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				3000
			}
		},
		drop_client = {
			{
				2,
				42031,
				1
			},
			{
				2,
				52032,
				1
			},
			{
				2,
				52004,
				1
			}
		}
	},
	[352] = {
		id = 352,
		rarity = 1,
		time = 9000,
		type = 1,
		name = "D-763-MI",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수의 사람들의 사물에 대한 인식의 집합체를 구현화하는 것이라면, 그것의 '방향성' 을 정하는 것에 따라 결과는 과연 어떻게 바뀔 것인가.",
		condition = 0,
		bg = "1",
		blueprint_version = 4,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "chara_bailong",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				3000
			}
		},
		drop_client = {
			{
				2,
				42032,
				1
			},
			{
				2,
				52032,
				1
			},
			{
				2,
				52004,
				1
			}
		}
	},
	[353] = {
		id = 353,
		rarity = 1,
		time = 9000,
		type = 1,
		name = "D-775-MI",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수의 사람들의 사물에 대한 인식의 집합체를 구현화하는 것이라면, 그것의 '방향성' 을 정하는 것에 따라 결과는 과연 어떻게 바뀔 것인가.",
		condition = 0,
		bg = "1",
		blueprint_version = 4,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "chara_aijier",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				3000
			}
		},
		drop_client = {
			{
				2,
				42033,
				1
			},
			{
				2,
				52032,
				1
			},
			{
				2,
				52004,
				1
			}
		}
	},
	[354] = {
		id = 354,
		rarity = 1,
		time = 9000,
		type = 1,
		name = "D-782-MI",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수의 사람들의 사물에 대한 인식의 집합체를 구현화하는 것이라면, 그것의 '방향성' 을 정하는 것에 따라 결과는 과연 어떻게 바뀔 것인가.",
		condition = 0,
		bg = "1",
		blueprint_version = 4,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "chara_aogusite",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				3000
			}
		},
		drop_client = {
			{
				2,
				42034,
				1
			},
			{
				2,
				52032,
				1
			},
			{
				2,
				52004,
				1
			}
		}
	},
	[355] = {
		id = 355,
		rarity = 1,
		time = 9000,
		type = 1,
		name = "D-768-MI",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수의 사람들의 사물에 대한 인식의 집합체를 구현화하는 것이라면, 그것의 '방향성' 을 정하는 것에 따라 결과는 과연 어떻게 바뀔 것인가.",
		condition = 0,
		bg = "1",
		blueprint_version = 4,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "chara_makeboluo",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				3000
			}
		},
		drop_client = {
			{
				2,
				42035,
				1
			},
			{
				2,
				52032,
				1
			},
			{
				2,
				52004,
				1
			}
		}
	},
	[361] = {
		id = 361,
		rarity = 2,
		time = 18000,
		type = 1,
		name = "D-308-MI",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수의 사람들의 사물에 대한 인식의 집합체를 구현화하는 것이라면, 그것의 '방향성' 을 정하는 것에 따라 결과는 과연 어떻게 바뀔 것인가.",
		condition = 0,
		bg = "2",
		blueprint_version = 4,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "chara_ankeleiqi",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				5000
			}
		},
		drop_client = {
			{
				2,
				42031,
				1
			},
			{
				2,
				52032,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[362] = {
		id = 362,
		rarity = 2,
		time = 18000,
		type = 1,
		name = "D-363-MI",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수의 사람들의 사물에 대한 인식의 집합체를 구현화하는 것이라면, 그것의 '방향성' 을 정하는 것에 따라 결과는 과연 어떻게 바뀔 것인가.",
		condition = 0,
		bg = "2",
		blueprint_version = 4,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "chara_bailong",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				5000
			}
		},
		drop_client = {
			{
				2,
				42032,
				1
			},
			{
				2,
				52032,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[363] = {
		id = 363,
		rarity = 2,
		time = 18000,
		type = 1,
		name = "D-375-MI",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수의 사람들의 사물에 대한 인식의 집합체를 구현화하는 것이라면, 그것의 '방향성' 을 정하는 것에 따라 결과는 과연 어떻게 바뀔 것인가.",
		condition = 0,
		bg = "2",
		blueprint_version = 4,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "chara_aijier",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				5000
			}
		},
		drop_client = {
			{
				2,
				42033,
				1
			},
			{
				2,
				52032,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[364] = {
		id = 364,
		rarity = 2,
		time = 18000,
		type = 1,
		name = "D-382-MI",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수의 사람들의 사물에 대한 인식의 집합체를 구현화하는 것이라면, 그것의 '방향성' 을 정하는 것에 따라 결과는 과연 어떻게 바뀔 것인가.",
		condition = 0,
		bg = "2",
		blueprint_version = 4,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "chara_aogusite",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				5000
			}
		},
		drop_client = {
			{
				2,
				42034,
				1
			},
			{
				2,
				52032,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[365] = {
		id = 365,
		rarity = 2,
		time = 18000,
		type = 1,
		name = "D-368-MI",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수의 사람들의 사물에 대한 인식의 집합체를 구현화하는 것이라면, 그것의 '방향성' 을 정하는 것에 따라 결과는 과연 어떻게 바뀔 것인가.",
		condition = 0,
		bg = "2",
		blueprint_version = 4,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "chara_makeboluo",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				5000
			}
		},
		drop_client = {
			{
				2,
				42035,
				1
			},
			{
				2,
				52032,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[371] = {
		id = 371,
		rarity = 1,
		time = 3600,
		type = 1,
		name = "Q-302-MI",
		lv_limit = 30,
		desc = "장비와 성정 큐브의 공명 현상을 제어할 수 있도록, 갖은 수단을 동원해 해당 현상을 재현하고, 최대한 효과적으로 이용할 수 있도록 한다.",
		condition = 0,
		bg = "1",
		blueprint_version = 4,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_1",
		label_color = "tag_q",
		consume = {
			{
				2,
				17001,
				20
			}
		},
		drop_client = {
			{
				2,
				52032,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[372] = {
		id = 372,
		rarity = 1,
		time = 3600,
		type = 1,
		name = "Q-310-MI",
		lv_limit = 30,
		desc = "장비와 성정 큐브의 공명 현상을 제어할 수 있도록, 갖은 수단을 동원해 해당 현상을 재현하고, 최대한 효과적으로 이용할 수 있도록 한다.",
		condition = 0,
		bg = "1",
		blueprint_version = 4,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_1",
		label_color = "tag_q",
		consume = {
			{
				2,
				17011,
				20
			}
		},
		drop_client = {
			{
				2,
				52032,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[373] = {
		id = 373,
		rarity = 1,
		time = 3600,
		type = 1,
		name = "Q-351-MI",
		lv_limit = 30,
		desc = "장비와 성정 큐브의 공명 현상을 제어할 수 있도록, 갖은 수단을 동원해 해당 현상을 재현하고, 최대한 효과적으로 이용할 수 있도록 한다.",
		condition = 0,
		bg = "1",
		blueprint_version = 4,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_1",
		label_color = "tag_q",
		consume = {
			{
				2,
				17021,
				20
			}
		},
		drop_client = {
			{
				2,
				52032,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[374] = {
		id = 374,
		rarity = 1,
		time = 3600,
		type = 1,
		name = "Q-368-MI",
		lv_limit = 30,
		desc = "장비와 성정 큐브의 공명 현상을 제어할 수 있도록, 갖은 수단을 동원해 해당 현상을 재현하고, 최대한 효과적으로 이용할 수 있도록 한다.",
		condition = 0,
		bg = "1",
		blueprint_version = 4,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_1",
		label_color = "tag_q",
		consume = {
			{
				2,
				17031,
				20
			}
		},
		drop_client = {
			{
				2,
				52032,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[375] = {
		id = 375,
		rarity = 1,
		time = 3600,
		type = 1,
		name = "Q-389-MI",
		lv_limit = 30,
		desc = "장비와 성정 큐브의 공명 현상을 제어할 수 있도록, 갖은 수단을 동원해 해당 현상을 재현하고, 최대한 효과적으로 이용할 수 있도록 한다.",
		condition = 0,
		bg = "1",
		blueprint_version = 4,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_1",
		label_color = "tag_q",
		consume = {
			{
				2,
				17041,
				20
			}
		},
		drop_client = {
			{
				2,
				52032,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[376] = {
		id = 376,
		rarity = 1,
		time = 7200,
		type = 1,
		name = "Q-202-MI",
		lv_limit = 30,
		desc = "장비와 성정 큐브의 공명 현상을 제어할 수 있도록, 갖은 수단을 동원해 해당 현상을 재현하고, 최대한 효과적으로 이용할 수 있도록 한다.",
		condition = 0,
		bg = "1",
		blueprint_version = 4,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_1",
		label_color = "tag_q",
		consume = {
			{
				2,
				17002,
				20
			}
		},
		drop_client = {
			{
				2,
				52032,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[377] = {
		id = 377,
		rarity = 1,
		time = 7200,
		type = 1,
		name = "Q-210-MI",
		lv_limit = 30,
		desc = "장비와 성정 큐브의 공명 현상을 제어할 수 있도록, 갖은 수단을 동원해 해당 현상을 재현하고, 최대한 효과적으로 이용할 수 있도록 한다.",
		condition = 0,
		bg = "1",
		blueprint_version = 4,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_1",
		label_color = "tag_q",
		consume = {
			{
				2,
				17012,
				20
			}
		},
		drop_client = {
			{
				2,
				52032,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[378] = {
		id = 378,
		rarity = 1,
		time = 7200,
		type = 1,
		name = "Q-251-MI",
		lv_limit = 30,
		desc = "장비와 성정 큐브의 공명 현상을 제어할 수 있도록, 갖은 수단을 동원해 해당 현상을 재현하고, 최대한 효과적으로 이용할 수 있도록 한다.",
		condition = 0,
		bg = "1",
		blueprint_version = 4,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_1",
		label_color = "tag_q",
		consume = {
			{
				2,
				17022,
				20
			}
		},
		drop_client = {
			{
				2,
				52032,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[379] = {
		id = 379,
		rarity = 1,
		time = 7200,
		type = 1,
		name = "Q-268-MI",
		lv_limit = 30,
		desc = "장비와 성정 큐브의 공명 현상을 제어할 수 있도록, 갖은 수단을 동원해 해당 현상을 재현하고, 최대한 효과적으로 이용할 수 있도록 한다.",
		condition = 0,
		bg = "1",
		blueprint_version = 4,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_1",
		label_color = "tag_q",
		consume = {
			{
				2,
				17032,
				20
			}
		},
		drop_client = {
			{
				2,
				52032,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[380] = {
		id = 380,
		rarity = 1,
		time = 7200,
		type = 1,
		name = "Q-289-MI",
		lv_limit = 30,
		desc = "장비와 성정 큐브의 공명 현상을 제어할 수 있도록, 갖은 수단을 동원해 해당 현상을 재현하고, 최대한 효과적으로 이용할 수 있도록 한다.",
		condition = 0,
		bg = "1",
		blueprint_version = 4,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_1",
		label_color = "tag_q",
		consume = {
			{
				2,
				17042,
				20
			}
		},
		drop_client = {
			{
				2,
				52032,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[381] = {
		id = 381,
		rarity = 2,
		time = 14400,
		type = 1,
		name = "Q-002-MI",
		lv_limit = 30,
		desc = "장비와 성정 큐브의 공명 현상을 제어할 수 있도록, 갖은 수단을 동원해 해당 현상을 재현하고, 최대한 효과적으로 이용할 수 있도록 한다.",
		condition = 0,
		bg = "2",
		blueprint_version = 4,
		sub_name = "중형 항목",
		label = "tag_blue",
		bg_icon = "gold_2",
		label_color = "tag_q",
		consume = {
			{
				2,
				17003,
				20
			}
		},
		drop_client = {
			{
				2,
				52032,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[382] = {
		id = 382,
		rarity = 2,
		time = 14400,
		type = 1,
		name = "Q-010-MI",
		lv_limit = 30,
		desc = "장비와 성정 큐브의 공명 현상을 제어할 수 있도록, 갖은 수단을 동원해 해당 현상을 재현하고, 최대한 효과적으로 이용할 수 있도록 한다.",
		condition = 0,
		bg = "2",
		blueprint_version = 4,
		sub_name = "중형 항목",
		label = "tag_blue",
		bg_icon = "gold_2",
		label_color = "tag_q",
		consume = {
			{
				2,
				17013,
				20
			}
		},
		drop_client = {
			{
				2,
				52032,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[383] = {
		id = 383,
		rarity = 2,
		time = 14400,
		type = 1,
		name = "Q-051-MI",
		lv_limit = 30,
		desc = "장비와 성정 큐브의 공명 현상을 제어할 수 있도록, 갖은 수단을 동원해 해당 현상을 재현하고, 최대한 효과적으로 이용할 수 있도록 한다.",
		condition = 0,
		bg = "2",
		blueprint_version = 4,
		sub_name = "중형 항목",
		label = "tag_blue",
		bg_icon = "gold_2",
		label_color = "tag_q",
		consume = {
			{
				2,
				17023,
				20
			}
		},
		drop_client = {
			{
				2,
				52032,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[384] = {
		id = 384,
		rarity = 2,
		time = 14400,
		type = 1,
		name = "Q-068-MI",
		lv_limit = 30,
		desc = "장비와 성정 큐브의 공명 현상을 제어할 수 있도록, 갖은 수단을 동원해 해당 현상을 재현하고, 최대한 효과적으로 이용할 수 있도록 한다.",
		condition = 0,
		bg = "2",
		blueprint_version = 4,
		sub_name = "중형 항목",
		label = "tag_blue",
		bg_icon = "gold_2",
		label_color = "tag_q",
		consume = {
			{
				2,
				17033,
				20
			}
		},
		drop_client = {
			{
				2,
				52032,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[385] = {
		id = 385,
		rarity = 2,
		time = 14400,
		type = 1,
		name = "Q-089-MI",
		lv_limit = 30,
		desc = "장비와 성정 큐브의 공명 현상을 제어할 수 있도록, 갖은 수단을 동원해 해당 현상을 재현하고, 최대한 효과적으로 이용할 수 있도록 한다.",
		condition = 0,
		bg = "2",
		blueprint_version = 4,
		sub_name = "중형 항목",
		label = "tag_blue",
		bg_icon = "gold_2",
		label_color = "tag_q",
		consume = {
			{
				2,
				17043,
				20
			}
		},
		drop_client = {
			{
				2,
				52032,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[391] = {
		id = 391,
		rarity = 1,
		time = 3600,
		type = 1,
		name = "H-387-MI",
		lv_limit = 30,
		desc = "성정 큐브를 문자 그대로 물리적으로 '분해'하여, 구조 분석을 진행한다. 성공 사례가 없을수록, 실험의 가치는 그만큼 크다고 볼 수 있다.",
		condition = 0,
		bg = "1",
		blueprint_version = 4,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "gold_1",
		label_color = "tag_h",
		consume = {
			{
				2,
				20001,
				3
			}
		},
		drop_client = {
			{
				2,
				52031,
				1
			},
			{
				2,
				54049,
				1
			},
			{
				2,
				15008,
				1
			}
		}
	},
	[392] = {
		id = 392,
		rarity = 1,
		time = 3600,
		type = 1,
		name = "H-387-MI",
		lv_limit = 30,
		desc = "성정 큐브를 문자 그대로 물리적으로 '분해'하여, 구조 분석을 진행한다. 성공 사례가 없을수록, 실험의 가치는 그만큼 크다고 볼 수 있다.",
		condition = 0,
		bg = "1",
		blueprint_version = 4,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "gold_1",
		label_color = "tag_h",
		consume = {
			{
				2,
				20001,
				3
			}
		},
		drop_client = {
			{
				2,
				52031,
				1
			},
			{
				2,
				54049,
				1
			},
			{
				2,
				15008,
				1
			}
		}
	},
	[393] = {
		id = 393,
		rarity = 2,
		time = 7200,
		type = 1,
		name = "H-339-MI",
		lv_limit = 30,
		desc = "성정 큐브를 문자 그대로 물리적으로 '분해'하여, 구조 분석을 진행한다. 성공 사례가 없을수록, 실험의 가치는 그만큼 크다고 볼 수 있다.",
		condition = 0,
		bg = "2",
		blueprint_version = 4,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "gold_2",
		label_color = "tag_h",
		consume = {
			{
				2,
				20001,
				6
			}
		},
		drop_client = {
			{
				2,
				52031,
				1
			},
			{
				2,
				54050,
				1
			},
			{
				2,
				15008,
				1
			}
		}
	},
	[394] = {
		id = 394,
		rarity = 2,
		time = 7200,
		type = 1,
		name = "H-339-MI",
		lv_limit = 30,
		desc = "성정 큐브를 문자 그대로 물리적으로 '분해'하여, 구조 분석을 진행한다. 성공 사례가 없을수록, 실험의 가치는 그만큼 크다고 볼 수 있다.",
		condition = 0,
		bg = "2",
		blueprint_version = 4,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "gold_2",
		label_color = "tag_h",
		consume = {
			{
				2,
				20001,
				6
			}
		},
		drop_client = {
			{
				2,
				52031,
				1
			},
			{
				2,
				54050,
				1
			},
			{
				2,
				15008,
				1
			}
		}
	},
	[401] = {
		id = 401,
		rarity = 1,
		time = 21600,
		type = 1,
		name = "C-153-MI",
		lv_limit = 30,
		desc = "성정 큐브에 관한 기초 이론은 큐브 구조에 대한 인식으로부터 시작된다. 큐브 기초 구조에 대한 이론 구조 작업은 여전히 전력을 다해 진행 중이다.",
		condition = 0,
		bg = "1",
		blueprint_version = 5,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "item_1",
		label_color = "tag_c",
		consume = {},
		drop_client = {
			{
				2,
				52062,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[402] = {
		id = 402,
		rarity = 1,
		time = 21600,
		type = 1,
		name = "C-153-MI",
		lv_limit = 30,
		desc = "성정 큐브에 관한 기초 이론은 큐브 구조에 대한 인식으로부터 시작된다. 큐브 기초 구조에 대한 이론 구조 작업은 여전히 전력을 다해 진행 중이다.",
		condition = 0,
		bg = "1",
		blueprint_version = 5,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "item_1",
		label_color = "tag_c",
		consume = {},
		drop_client = {
			{
				2,
				52062,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[403] = {
		id = 403,
		rarity = 2,
		time = 28800,
		type = 1,
		name = "C-185-MI",
		lv_limit = 30,
		desc = "성정 큐브에 관한 기초 이론은 큐브 구조에 대한 인식으로부터 시작된다. 큐브 기초 구조에 대한 이론 구조 작업은 여전히 전력을 다해 진행 중이다.",
		condition = 0,
		bg = "2",
		blueprint_version = 5,
		sub_name = "대형 항목",
		label = "tag_red",
		bg_icon = "item_2",
		label_color = "tag_c",
		consume = {},
		drop_client = {
			{
				2,
				52061,
				1
			},
			{
				2,
				52062,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[404] = {
		id = 404,
		rarity = 2,
		time = 28800,
		type = 1,
		name = "C-185-MI",
		lv_limit = 30,
		desc = "성정 큐브에 관한 기초 이론은 큐브 구조에 대한 인식으로부터 시작된다. 큐브 기초 구조에 대한 이론 구조 작업은 여전히 전력을 다해 진행 중이다.",
		condition = 0,
		bg = "2",
		blueprint_version = 5,
		sub_name = "대형 항목",
		label = "tag_red",
		bg_icon = "item_2",
		label_color = "tag_c",
		consume = {},
		drop_client = {
			{
				2,
				52061,
				1
			},
			{
				2,
				52062,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[411] = {
		id = 411,
		rarity = 1,
		time = 14400,
		type = 1,
		name = "B-622-MI",
		lv_limit = 30,
		desc = "성정 큐브 구조에 관한 여러가지 가설에 대해 실증실험을 진행하는 연구 프로젝트. 실전을 통해 데이터를 수집하여, 이론의 과학적 증명에 기여하자.",
		condition = 52001,
		bg = "1",
		blueprint_version = 5,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "battle_1",
		label_color = "tag_b",
		consume = {},
		drop_client = {
			{
				2,
				52061,
				1
			},
			{
				2,
				52062,
				1
			},
			{
				2,
				52004,
				1
			}
		}
	},
	[412] = {
		id = 412,
		rarity = 1,
		time = 14400,
		type = 1,
		name = "B-636-MI",
		lv_limit = 30,
		desc = "성정 큐브 구조에 관한 여러가지 가설에 대해 실증실험을 진행하는 연구 프로젝트. 실전을 통해 데이터를 수집하여, 이론의 과학적 증명에 기여하자.",
		condition = 52002,
		bg = "1",
		blueprint_version = 5,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "battle_1",
		label_color = "tag_b",
		consume = {},
		drop_client = {
			{
				2,
				52061,
				1
			},
			{
				2,
				52062,
				1
			},
			{
				2,
				52004,
				1
			}
		}
	},
	[413] = {
		id = 413,
		rarity = 1,
		time = 14400,
		type = 1,
		name = "B-654-MI",
		lv_limit = 30,
		desc = "성정 큐브 구조에 관한 여러가지 가설에 대해 실증실험을 진행하는 연구 프로젝트. 실전을 통해 데이터를 수집하여, 이론의 과학적 증명에 기여하자.",
		condition = 52003,
		bg = "1",
		blueprint_version = 5,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "battle_1",
		label_color = "tag_b",
		consume = {},
		drop_client = {
			{
				2,
				52061,
				1
			},
			{
				2,
				52062,
				1
			},
			{
				2,
				52004,
				1
			}
		}
	},
	[414] = {
		id = 414,
		rarity = 1,
		time = 14400,
		type = 1,
		name = "B-682-MI",
		lv_limit = 40,
		desc = "성정 큐브 구조에 관한 여러가지 가설에 대해 실증실험을 진행하는 연구 프로젝트. 실전을 통해 데이터를 수집하여, 이론의 과학적 증명에 기여하자.",
		condition = 52004,
		bg = "1",
		blueprint_version = 5,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "battle_1",
		label_color = "tag_b",
		consume = {},
		drop_client = {
			{
				2,
				52061,
				1
			},
			{
				2,
				52062,
				1
			},
			{
				2,
				52004,
				1
			}
		}
	},
	[415] = {
		id = 415,
		rarity = 2,
		time = 14400,
		type = 1,
		name = "B-235-MI",
		lv_limit = 50,
		desc = "성정 큐브 구조에 관한 여러가지 가설에 대해 실증실험을 진행하는 연구 프로젝트. 실전을 통해 데이터를 수집하여, 이론의 과학적 증명에 기여하자.",
		condition = 52005,
		bg = "2",
		blueprint_version = 5,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "battle_2",
		label_color = "tag_b",
		consume = {},
		drop_client = {
			{
				2,
				52061,
				1
			},
			{
				2,
				52062,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[416] = {
		id = 416,
		rarity = 2,
		time = 14400,
		type = 1,
		name = "B-268-MI",
		lv_limit = 60,
		desc = "성정 큐브 구조에 관한 여러가지 가설에 대해 실증실험을 진행하는 연구 프로젝트. 실전을 통해 데이터를 수집하여, 이론의 과학적 증명에 기여하자.",
		condition = 52006,
		bg = "2",
		blueprint_version = 5,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "battle_2",
		label_color = "tag_b",
		consume = {},
		drop_client = {
			{
				2,
				52061,
				1
			},
			{
				2,
				52062,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[417] = {
		id = 417,
		rarity = 2,
		time = 14400,
		type = 1,
		name = "B-128-MI",
		lv_limit = 65,
		desc = "성정 큐브 구조에 관한 여러가지 가설에 대해 실증실험을 진행하는 연구 프로젝트. 실전을 통해 데이터를 수집하여, 이론의 과학적 증명에 기여하자.",
		condition = 52007,
		bg = "2",
		blueprint_version = 5,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "battle_2",
		label_color = "tag_b",
		consume = {},
		drop_client = {
			{
				2,
				52061,
				1
			},
			{
				2,
				52062,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[418] = {
		id = 418,
		rarity = 2,
		time = 14400,
		type = 1,
		name = "B-164-MI",
		lv_limit = 70,
		desc = "성정 큐브 구조에 관한 여러가지 가설에 대해 실증실험을 진행하는 연구 프로젝트. 실전을 통해 데이터를 수집하여, 이론의 과학적 증명에 기여하자.",
		condition = 52008,
		bg = "2",
		blueprint_version = 5,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "battle_2",
		label_color = "tag_b",
		consume = {},
		drop_client = {
			{
				2,
				52061,
				1
			},
			{
				2,
				52062,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[421] = {
		id = 421,
		rarity = 1,
		time = 10800,
		type = 1,
		name = "T-018-MI",
		lv_limit = 30,
		desc = "성정 큐브에 관한 연구는 [건조] 방면뿐만 아니라, 일상생활 이용 등 다양한 방면으로 진행되고 있다. 성정 큐브의 다방면 활용 가능성을 찾는 프로젝트에는 군의 조력이 필수불가결이다.",
		condition = 52021,
		bg = "1",
		blueprint_version = 5,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "item_1",
		label_color = "tag_t",
		consume = {},
		drop_client = {
			{
				2,
				52062,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[422] = {
		id = 422,
		rarity = 1,
		time = 14400,
		type = 1,
		name = "T-384-MI",
		lv_limit = 30,
		desc = "성정 큐브에 관한 연구는 [건조] 방면뿐만 아니라, 일상생활 이용 등 다양한 방면으로 진행되고 있다. 성정 큐브의 다방면 활용 가능성을 찾는 프로젝트에는 군의 조력이 필수불가결이다.",
		condition = 52022,
		bg = "1",
		blueprint_version = 5,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "item_1",
		label_color = "tag_t",
		consume = {},
		drop_client = {
			{
				2,
				52062,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[423] = {
		id = 423,
		rarity = 2,
		time = 21600,
		type = 1,
		name = "T-249-MI",
		lv_limit = 30,
		desc = "성정 큐브에 관한 연구는 [건조] 방면뿐만 아니라, 일상생활 이용 등 다양한 방면으로 진행되고 있다. 성정 큐브의 다방면 활용 가능성을 찾는 프로젝트에는 군의 조력이 필수불가결이다.",
		condition = 52023,
		bg = "2",
		blueprint_version = 5,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "item_2",
		label_color = "tag_t",
		consume = {},
		drop_client = {
			{
				2,
				52062,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[431] = {
		id = 431,
		rarity = 1,
		time = 7200,
		type = 1,
		name = "E-031-MI",
		lv_limit = 30,
		desc = "성정 큐브와 [장비]의 관련성을 조사하는 다수의 시도는, 방대한 실험 데이터에 기반하여 진행되고 있다. 불필요한 장비를 제공하여, 실험 샘플을 충당하자.",
		condition = 52031,
		bg = "1",
		blueprint_version = 5,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "item_1",
		label_color = "tag_e",
		consume = {},
		drop_client = {
			{
				2,
				52062,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[432] = {
		id = 432,
		rarity = 2,
		time = 7200,
		type = 1,
		name = "E-315-MI",
		lv_limit = 30,
		desc = "성정 큐브와 [장비]의 관련성을 조사하는 다수의 시도는, 방대한 실험 데이터에 기반하여 진행되고 있다. 불필요한 장비를 제공하여, 실험 샘플을 충당하자.",
		condition = 52032,
		bg = "2",
		blueprint_version = 5,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "item_2",
		label_color = "tag_e",
		consume = {},
		drop_client = {
			{
				2,
				52062,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[433] = {
		id = 433,
		rarity = 2,
		time = 21600,
		type = 1,
		name = "E-193-MI",
		lv_limit = 50,
		desc = "현재 비치 중인 장비를 분해함으로써 기초 설계를 재검토하여, 추후 개발이 예정된 새 장비의 기초 연구를 진행한다.",
		condition = 0,
		bg = "2",
		blueprint_version = 5,
		sub_name = "중형 항목",
		label = "tag_blue",
		bg_icon = "item_2",
		label_color = "tag_e",
		consume = {
			{
				2,
				30013,
				15
			}
		},
		drop_client = {
			{
				2,
				13032,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[434] = {
		id = 434,
		rarity = 2,
		time = 21600,
		type = 1,
		name = "E-178-MI",
		lv_limit = 50,
		desc = "현재 비치 중인 장비를 분해함으로써 기초 설계를 재검토하여, 추후 개발이 예정된 새 장비의 기초 연구를 진행한다.",
		condition = 0,
		bg = "2",
		blueprint_version = 5,
		sub_name = "중형 항목",
		label = "tag_blue",
		bg_icon = "item_2",
		label_color = "tag_e",
		consume = {
			{
				2,
				30043,
				15
			}
		},
		drop_client = {
			{
				2,
				13035,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[435] = {
		id = 435,
		rarity = 2,
		time = 21600,
		type = 1,
		name = "E-186-MI",
		lv_limit = 50,
		desc = "현재 비치 중인 장비를 분해함으로써 기초 설계를 재검토하여, 추후 개발이 예정된 새 장비의 기초 연구를 진행한다.",
		condition = 0,
		bg = "2",
		blueprint_version = 5,
		sub_name = "중형 항목",
		label = "tag_blue",
		bg_icon = "item_2",
		label_color = "tag_e",
		consume = {
			{
				2,
				30043,
				15
			}
		},
		drop_client = {
			{
				2,
				13036,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[436] = {
		id = 436,
		rarity = 2,
		time = 21600,
		type = 1,
		name = "E-188-MI",
		lv_limit = 50,
		desc = "현재 비치 중인 장비를 분해함으로써 기초 설계를 재검토하여, 추후 개발이 예정된 새 장비의 기초 연구를 진행한다.",
		condition = 0,
		bg = "2",
		blueprint_version = 5,
		sub_name = "중형 항목",
		label = "tag_blue",
		bg_icon = "item_2",
		label_color = "tag_e",
		consume = {
			{
				2,
				30013,
				15
			}
		},
		drop_client = {
			{
				2,
				13033,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[437] = {
		id = 437,
		rarity = 2,
		time = 21600,
		type = 1,
		name = "E-189-MI",
		lv_limit = 50,
		desc = "현재 비치 중인 장비를 분해함으로써 기초 설계를 재검토하여, 추후 개발이 예정된 새 장비의 기초 연구를 진행한다.",
		condition = 0,
		bg = "2",
		blueprint_version = 5,
		sub_name = "중형 항목",
		label = "tag_blue",
		bg_icon = "item_2",
		label_color = "tag_e",
		consume = {
			{
				2,
				30013,
				15
			}
		},
		drop_client = {
			{
				2,
				13034,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[441] = {
		id = 441,
		rarity = 1,
		time = 5400,
		type = 1,
		name = "G-412-MI",
		lv_limit = 30,
		desc = "뛰어난 과학의 발전을 뒷받침하기 위해선 영민한 두뇌뿐만 아니라, 지원자로부터 제공되는 연구 자금도 필요하다. 남아도는 자금을 기부하여 기술의 연구를 서포트하자.",
		condition = 0,
		bg = "1",
		blueprint_version = 5,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "gold_1",
		label_color = "tag_g",
		consume = {
			{
				1,
				1,
				1500
			}
		},
		drop_client = {
			{
				2,
				52061,
				1
			},
			{
				2,
				52062,
				1
			},
			{
				2,
				52004,
				1
			}
		}
	},
	[442] = {
		id = 442,
		rarity = 1,
		time = 5400,
		type = 1,
		name = "G-412-MI",
		lv_limit = 30,
		desc = "뛰어난 과학의 발전을 뒷받침하기 위해선 영민한 두뇌뿐만 아니라, 지원자로부터 제공되는 연구 자금도 필요하다. 남아도는 자금을 기부하여 기술의 연구를 서포트하자.",
		condition = 0,
		bg = "1",
		blueprint_version = 5,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "gold_1",
		label_color = "tag_g",
		consume = {
			{
				1,
				1,
				1500
			}
		},
		drop_client = {
			{
				2,
				52061,
				1
			},
			{
				2,
				52062,
				1
			},
			{
				2,
				52004,
				1
			}
		}
	},
	[443] = {
		id = 443,
		rarity = 1,
		time = 9000,
		type = 1,
		name = "G-236-MI",
		lv_limit = 30,
		desc = "뛰어난 과학의 발전을 뒷받침하기 위해선 영민한 두뇌뿐만 아니라, 지원자로부터 제공되는 연구 자금도 필요하다. 남아도는 자금을 기부하여 기술의 연구를 서포트하자.",
		condition = 0,
		bg = "1",
		blueprint_version = 5,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "gold_1",
		label_color = "tag_g",
		consume = {
			{
				1,
				1,
				3000
			}
		},
		drop_client = {
			{
				2,
				52061,
				1
			},
			{
				2,
				52062,
				1
			},
			{
				2,
				52004,
				1
			}
		}
	},
	[444] = {
		id = 444,
		rarity = 1,
		time = 9000,
		type = 1,
		name = "G-236-MI",
		lv_limit = 30,
		desc = "뛰어난 과학의 발전을 뒷받침하기 위해선 영민한 두뇌뿐만 아니라, 지원자로부터 제공되는 연구 자금도 필요하다. 남아도는 자금을 기부하여 기술의 연구를 서포트하자.",
		condition = 0,
		bg = "1",
		blueprint_version = 5,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "gold_1",
		label_color = "tag_g",
		consume = {
			{
				1,
				1,
				3000
			}
		},
		drop_client = {
			{
				2,
				52061,
				1
			},
			{
				2,
				52062,
				1
			},
			{
				2,
				52004,
				1
			}
		}
	},
	[445] = {
		id = 445,
		rarity = 2,
		time = 14400,
		type = 1,
		name = "G-531-MI",
		lv_limit = 30,
		desc = "뛰어난 과학의 발전을 뒷받침하기 위해선 영민한 두뇌뿐만 아니라, 지원자로부터 제공되는 연구 자금도 필요하다. 남아도는 자금을 기부하여 기술의 연구를 서포트하자.",
		condition = 0,
		bg = "2",
		blueprint_version = 5,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "gold_2",
		label_color = "tag_g",
		consume = {
			{
				1,
				1,
				6000
			}
		},
		drop_client = {
			{
				2,
				52061,
				1
			},
			{
				2,
				52062,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[446] = {
		id = 446,
		rarity = 2,
		time = 14400,
		type = 1,
		name = "G-531-MI",
		lv_limit = 30,
		desc = "뛰어난 과학의 발전을 뒷받침하기 위해선 영민한 두뇌뿐만 아니라, 지원자로부터 제공되는 연구 자금도 필요하다. 남아도는 자금을 기부하여 기술의 연구를 서포트하자.",
		condition = 0,
		bg = "2",
		blueprint_version = 5,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "gold_2",
		label_color = "tag_g",
		consume = {
			{
				1,
				1,
				6000
			}
		},
		drop_client = {
			{
				2,
				52061,
				1
			},
			{
				2,
				52062,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[451] = {
		id = 451,
		rarity = 1,
		time = 9000,
		type = 1,
		name = "D-719-MI",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수의 사람들의 사물에 대한 인식의 집합체를 구현화하는 것이라면, 그것의 '방향성' 을 정하는 것에 따라 결과는 과연 어떻게 바뀔 것인가.",
		condition = 0,
		bg = "1",
		blueprint_version = 5,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "chara_pulimaosi",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				3000
			}
		},
		drop_client = {
			{
				2,
				42041,
				1
			},
			{
				2,
				52062,
				1
			},
			{
				2,
				52004,
				1
			}
		}
	},
	[452] = {
		id = 452,
		rarity = 1,
		time = 9000,
		type = 1,
		name = "D-778-MI",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수의 사람들의 사물에 대한 인식의 집합체를 구현화하는 것이라면, 그것의 '방향성' 을 정하는 것에 따라 결과는 과연 어떻게 바뀔 것인가.",
		condition = 0,
		bg = "1",
		blueprint_version = 5,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "chara_lupuleixiteqinwang",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				3000
			}
		},
		drop_client = {
			{
				2,
				42042,
				1
			},
			{
				2,
				52062,
				1
			},
			{
				2,
				52004,
				1
			}
		}
	},
	[453] = {
		id = 453,
		rarity = 1,
		time = 9000,
		type = 1,
		name = "D-786-MI",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수의 사람들의 사물에 대한 인식의 집합체를 구현화하는 것이라면, 그것의 '방향성' 을 정하는 것에 따라 결과는 과연 어떻게 바뀔 것인가.",
		condition = 0,
		bg = "1",
		blueprint_version = 5,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "chara_haerbin",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				3000
			}
		},
		drop_client = {
			{
				2,
				42043,
				1
			},
			{
				2,
				52062,
				1
			},
			{
				2,
				52004,
				1
			}
		}
	},
	[454] = {
		id = 454,
		rarity = 1,
		time = 9000,
		type = 1,
		name = "D-788-MI",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수의 사람들의 사물에 대한 인식의 집합체를 구현화하는 것이라면, 그것의 '방향성' 을 정하는 것에 따라 결과는 과연 어떻게 바뀔 것인가.",
		condition = 0,
		bg = "1",
		blueprint_version = 5,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "chara_qikaluofu",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				3000
			}
		},
		drop_client = {
			{
				2,
				42044,
				1
			},
			{
				2,
				52062,
				1
			},
			{
				2,
				52004,
				1
			}
		}
	},
	[455] = {
		id = 455,
		rarity = 1,
		time = 9000,
		type = 1,
		name = "D-793-MI",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수의 사람들의 사물에 대한 인식의 집합체를 구현화하는 것이라면, 그것의 '방향성' 을 정하는 것에 따라 결과는 과연 어떻게 바뀔 것인가.",
		condition = 0,
		bg = "1",
		blueprint_version = 5,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "chara_buleisite",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				3000
			}
		},
		drop_client = {
			{
				2,
				42045,
				1
			},
			{
				2,
				52062,
				1
			},
			{
				2,
				52004,
				1
			}
		}
	},
	[461] = {
		id = 461,
		rarity = 2,
		time = 18000,
		type = 1,
		name = "D-319-MI",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수의 사람들의 사물에 대한 인식의 집합체를 구현화하는 것이라면, 그것의 '방향성' 을 정하는 것에 따라 결과는 과연 어떻게 바뀔 것인가.",
		condition = 0,
		bg = "2",
		blueprint_version = 5,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "chara_pulimaosi",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				5000
			}
		},
		drop_client = {
			{
				2,
				42041,
				1
			},
			{
				2,
				52062,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[462] = {
		id = 462,
		rarity = 2,
		time = 18000,
		type = 1,
		name = "D-378-MI",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수의 사람들의 사물에 대한 인식의 집합체를 구현화하는 것이라면, 그것의 '방향성' 을 정하는 것에 따라 결과는 과연 어떻게 바뀔 것인가.",
		condition = 0,
		bg = "2",
		blueprint_version = 5,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "chara_lupuleixiteqinwang",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				5000
			}
		},
		drop_client = {
			{
				2,
				42042,
				1
			},
			{
				2,
				52062,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[463] = {
		id = 463,
		rarity = 2,
		time = 18000,
		type = 1,
		name = "D-386-MI",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수의 사람들의 사물에 대한 인식의 집합체를 구현화하는 것이라면, 그것의 '방향성' 을 정하는 것에 따라 결과는 과연 어떻게 바뀔 것인가.",
		condition = 0,
		bg = "2",
		blueprint_version = 5,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "chara_haerbin",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				5000
			}
		},
		drop_client = {
			{
				2,
				42043,
				1
			},
			{
				2,
				52062,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[464] = {
		id = 464,
		rarity = 2,
		time = 18000,
		type = 1,
		name = "D-388-MI",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수의 사람들의 사물에 대한 인식의 집합체를 구현화하는 것이라면, 그것의 '방향성' 을 정하는 것에 따라 결과는 과연 어떻게 바뀔 것인가.",
		condition = 0,
		bg = "2",
		blueprint_version = 5,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "chara_qikaluofu",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				5000
			}
		},
		drop_client = {
			{
				2,
				42044,
				1
			},
			{
				2,
				52062,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[465] = {
		id = 465,
		rarity = 2,
		time = 18000,
		type = 1,
		name = "D-393-MI",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수의 사람들의 사물에 대한 인식의 집합체를 구현화하는 것이라면, 그것의 '방향성' 을 정하는 것에 따라 결과는 과연 어떻게 바뀔 것인가.",
		condition = 0,
		bg = "2",
		blueprint_version = 5,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "chara_buleisite",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				5000
			}
		},
		drop_client = {
			{
				2,
				42045,
				1
			},
			{
				2,
				52062,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[471] = {
		id = 471,
		rarity = 1,
		time = 3600,
		type = 1,
		name = "Q-302-MI",
		lv_limit = 30,
		desc = "장비와 성정 큐브의 공명 현상을 제어할 수 있도록, 갖은 수단을 동원해 해당 현상을 재현하고, 최대한 효과적으로 이용할 수 있도록 한다.",
		condition = 0,
		bg = "1",
		blueprint_version = 5,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_1",
		label_color = "tag_q",
		consume = {
			{
				2,
				17001,
				20
			}
		},
		drop_client = {
			{
				2,
				52062,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[472] = {
		id = 472,
		rarity = 1,
		time = 3600,
		type = 1,
		name = "Q-310-MI",
		lv_limit = 30,
		desc = "장비와 성정 큐브의 공명 현상을 제어할 수 있도록, 갖은 수단을 동원해 해당 현상을 재현하고, 최대한 효과적으로 이용할 수 있도록 한다.",
		condition = 0,
		bg = "1",
		blueprint_version = 5,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_1",
		label_color = "tag_q",
		consume = {
			{
				2,
				17011,
				20
			}
		},
		drop_client = {
			{
				2,
				52062,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[473] = {
		id = 473,
		rarity = 1,
		time = 3600,
		type = 1,
		name = "Q-351-MI",
		lv_limit = 30,
		desc = "장비와 성정 큐브의 공명 현상을 제어할 수 있도록, 갖은 수단을 동원해 해당 현상을 재현하고, 최대한 효과적으로 이용할 수 있도록 한다.",
		condition = 0,
		bg = "1",
		blueprint_version = 5,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_1",
		label_color = "tag_q",
		consume = {
			{
				2,
				17021,
				20
			}
		},
		drop_client = {
			{
				2,
				52062,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[474] = {
		id = 474,
		rarity = 1,
		time = 3600,
		type = 1,
		name = "Q-368-MI",
		lv_limit = 30,
		desc = "장비와 성정 큐브의 공명 현상을 제어할 수 있도록, 갖은 수단을 동원해 해당 현상을 재현하고, 최대한 효과적으로 이용할 수 있도록 한다.",
		condition = 0,
		bg = "1",
		blueprint_version = 5,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_1",
		label_color = "tag_q",
		consume = {
			{
				2,
				17031,
				20
			}
		},
		drop_client = {
			{
				2,
				52062,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[475] = {
		id = 475,
		rarity = 1,
		time = 3600,
		type = 1,
		name = "Q-389-MI",
		lv_limit = 30,
		desc = "장비와 성정 큐브의 공명 현상을 제어할 수 있도록, 갖은 수단을 동원해 해당 현상을 재현하고, 최대한 효과적으로 이용할 수 있도록 한다.",
		condition = 0,
		bg = "1",
		blueprint_version = 5,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_1",
		label_color = "tag_q",
		consume = {
			{
				2,
				17041,
				20
			}
		},
		drop_client = {
			{
				2,
				52062,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[476] = {
		id = 476,
		rarity = 1,
		time = 7200,
		type = 1,
		name = "Q-202-MI",
		lv_limit = 30,
		desc = "장비와 성정 큐브의 공명 현상을 제어할 수 있도록, 갖은 수단을 동원해 해당 현상을 재현하고, 최대한 효과적으로 이용할 수 있도록 한다.",
		condition = 0,
		bg = "1",
		blueprint_version = 5,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_1",
		label_color = "tag_q",
		consume = {
			{
				2,
				17002,
				20
			}
		},
		drop_client = {
			{
				2,
				52062,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[477] = {
		id = 477,
		rarity = 1,
		time = 7200,
		type = 1,
		name = "Q-210-MI",
		lv_limit = 30,
		desc = "장비와 성정 큐브의 공명 현상을 제어할 수 있도록, 갖은 수단을 동원해 해당 현상을 재현하고, 최대한 효과적으로 이용할 수 있도록 한다.",
		condition = 0,
		bg = "1",
		blueprint_version = 5,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_1",
		label_color = "tag_q",
		consume = {
			{
				2,
				17012,
				20
			}
		},
		drop_client = {
			{
				2,
				52062,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[478] = {
		id = 478,
		rarity = 1,
		time = 7200,
		type = 1,
		name = "Q-251-MI",
		lv_limit = 30,
		desc = "장비와 성정 큐브의 공명 현상을 제어할 수 있도록, 갖은 수단을 동원해 해당 현상을 재현하고, 최대한 효과적으로 이용할 수 있도록 한다.",
		condition = 0,
		bg = "1",
		blueprint_version = 5,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_1",
		label_color = "tag_q",
		consume = {
			{
				2,
				17022,
				20
			}
		},
		drop_client = {
			{
				2,
				52062,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[479] = {
		id = 479,
		rarity = 1,
		time = 7200,
		type = 1,
		name = "Q-268-MI",
		lv_limit = 30,
		desc = "장비와 성정 큐브의 공명 현상을 제어할 수 있도록, 갖은 수단을 동원해 해당 현상을 재현하고, 최대한 효과적으로 이용할 수 있도록 한다.",
		condition = 0,
		bg = "1",
		blueprint_version = 5,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_1",
		label_color = "tag_q",
		consume = {
			{
				2,
				17032,
				20
			}
		},
		drop_client = {
			{
				2,
				52062,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[480] = {
		id = 480,
		rarity = 1,
		time = 7200,
		type = 1,
		name = "Q-289-MI",
		lv_limit = 30,
		desc = "장비와 성정 큐브의 공명 현상을 제어할 수 있도록, 갖은 수단을 동원해 해당 현상을 재현하고, 최대한 효과적으로 이용할 수 있도록 한다.",
		condition = 0,
		bg = "1",
		blueprint_version = 5,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_1",
		label_color = "tag_q",
		consume = {
			{
				2,
				17042,
				20
			}
		},
		drop_client = {
			{
				2,
				52062,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[481] = {
		id = 481,
		rarity = 2,
		time = 14400,
		type = 1,
		name = "Q-002-MI",
		lv_limit = 30,
		desc = "장비와 성정 큐브의 공명 현상을 제어할 수 있도록, 갖은 수단을 동원해 해당 현상을 재현하고, 최대한 효과적으로 이용할 수 있도록 한다.",
		condition = 0,
		bg = "2",
		blueprint_version = 5,
		sub_name = "중형 항목",
		label = "tag_blue",
		bg_icon = "gold_2",
		label_color = "tag_q",
		consume = {
			{
				2,
				17003,
				20
			}
		},
		drop_client = {
			{
				2,
				52062,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[482] = {
		id = 482,
		rarity = 2,
		time = 14400,
		type = 1,
		name = "Q-010-MI",
		lv_limit = 30,
		desc = "장비와 성정 큐브의 공명 현상을 제어할 수 있도록, 갖은 수단을 동원해 해당 현상을 재현하고, 최대한 효과적으로 이용할 수 있도록 한다.",
		condition = 0,
		bg = "2",
		blueprint_version = 5,
		sub_name = "중형 항목",
		label = "tag_blue",
		bg_icon = "gold_2",
		label_color = "tag_q",
		consume = {
			{
				2,
				17013,
				20
			}
		},
		drop_client = {
			{
				2,
				52062,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[483] = {
		id = 483,
		rarity = 2,
		time = 14400,
		type = 1,
		name = "Q-051-MI",
		lv_limit = 30,
		desc = "장비와 성정 큐브의 공명 현상을 제어할 수 있도록, 갖은 수단을 동원해 해당 현상을 재현하고, 최대한 효과적으로 이용할 수 있도록 한다.",
		condition = 0,
		bg = "2",
		blueprint_version = 5,
		sub_name = "중형 항목",
		label = "tag_blue",
		bg_icon = "gold_2",
		label_color = "tag_q",
		consume = {
			{
				2,
				17023,
				20
			}
		},
		drop_client = {
			{
				2,
				52062,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[484] = {
		id = 484,
		rarity = 2,
		time = 14400,
		type = 1,
		name = "Q-068-MI",
		lv_limit = 30,
		desc = "장비와 성정 큐브의 공명 현상을 제어할 수 있도록, 갖은 수단을 동원해 해당 현상을 재현하고, 최대한 효과적으로 이용할 수 있도록 한다.",
		condition = 0,
		bg = "2",
		blueprint_version = 5,
		sub_name = "중형 항목",
		label = "tag_blue",
		bg_icon = "gold_2",
		label_color = "tag_q",
		consume = {
			{
				2,
				17033,
				20
			}
		},
		drop_client = {
			{
				2,
				52062,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[485] = {
		id = 485,
		rarity = 2,
		time = 14400,
		type = 1,
		name = "Q-089-MI",
		lv_limit = 30,
		desc = "장비와 성정 큐브의 공명 현상을 제어할 수 있도록, 갖은 수단을 동원해 해당 현상을 재현하고, 최대한 효과적으로 이용할 수 있도록 한다.",
		condition = 0,
		bg = "2",
		blueprint_version = 5,
		sub_name = "중형 항목",
		label = "tag_blue",
		bg_icon = "gold_2",
		label_color = "tag_q",
		consume = {
			{
				2,
				17043,
				20
			}
		},
		drop_client = {
			{
				2,
				52062,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[491] = {
		id = 491,
		rarity = 1,
		time = 3600,
		type = 1,
		name = "H-387-MI",
		lv_limit = 30,
		desc = "성정 큐브를 문자 그대로 물리적으로 '분해'하여, 구조 분석을 진행한다. 성공 사례가 없을수록, 실험의 가치는 그만큼 크다고 볼 수 있다.",
		condition = 0,
		bg = "1",
		blueprint_version = 5,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "gold_1",
		label_color = "tag_h",
		consume = {
			{
				2,
				20001,
				3
			}
		},
		drop_client = {
			{
				2,
				52061,
				1
			},
			{
				2,
				54049,
				1
			},
			{
				2,
				15008,
				1
			}
		}
	},
	[492] = {
		id = 492,
		rarity = 1,
		time = 3600,
		type = 1,
		name = "H-387-MI",
		lv_limit = 30,
		desc = "성정 큐브를 문자 그대로 물리적으로 '분해'하여, 구조 분석을 진행한다. 성공 사례가 없을수록, 실험의 가치는 그만큼 크다고 볼 수 있다.",
		condition = 0,
		bg = "1",
		blueprint_version = 5,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "gold_1",
		label_color = "tag_h",
		consume = {
			{
				2,
				20001,
				3
			}
		},
		drop_client = {
			{
				2,
				52061,
				1
			},
			{
				2,
				54049,
				1
			},
			{
				2,
				15008,
				1
			}
		}
	},
	[493] = {
		id = 493,
		rarity = 2,
		time = 7200,
		type = 1,
		name = "H-339-MI",
		lv_limit = 30,
		desc = "성정 큐브를 문자 그대로 물리적으로 '분해'하여, 구조 분석을 진행한다. 성공 사례가 없을수록, 실험의 가치는 그만큼 크다고 볼 수 있다.",
		condition = 0,
		bg = "2",
		blueprint_version = 5,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "gold_2",
		label_color = "tag_h",
		consume = {
			{
				2,
				20001,
				6
			}
		},
		drop_client = {
			{
				2,
				52061,
				1
			},
			{
				2,
				54050,
				1
			},
			{
				2,
				15008,
				1
			}
		}
	},
	[494] = {
		id = 494,
		rarity = 2,
		time = 7200,
		type = 1,
		name = "H-339-MI",
		lv_limit = 30,
		desc = "성정 큐브를 문자 그대로 물리적으로 '분해'하여, 구조 분석을 진행한다. 성공 사례가 없을수록, 실험의 가치는 그만큼 크다고 볼 수 있다.",
		condition = 0,
		bg = "2",
		blueprint_version = 5,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "gold_2",
		label_color = "tag_h",
		consume = {
			{
				2,
				20001,
				6
			}
		},
		drop_client = {
			{
				2,
				52061,
				1
			},
			{
				2,
				54050,
				1
			},
			{
				2,
				15008,
				1
			}
		}
	},
	[501] = {
		id = 501,
		rarity = 1,
		time = 21600,
		type = 1,
		name = "C-153-MI",
		lv_limit = 30,
		desc = "성정 큐브에 관한 기초 이론은 큐브 구조에 대한 인식으로부터 시작된다. 큐브 기초 구조에 대한 이론 구조 작업은 여전히 전력을 다해 진행 중이다.",
		condition = 0,
		bg = "1",
		blueprint_version = 6,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "item_1",
		label_color = "tag_c",
		consume = {},
		drop_client = {
			{
				2,
				52072,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[502] = {
		id = 502,
		rarity = 1,
		time = 21600,
		type = 1,
		name = "C-153-MI",
		lv_limit = 30,
		desc = "성정 큐브에 관한 기초 이론은 큐브 구조에 대한 인식으로부터 시작된다. 큐브 기초 구조에 대한 이론 구조 작업은 여전히 전력을 다해 진행 중이다.",
		condition = 0,
		bg = "1",
		blueprint_version = 6,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "item_1",
		label_color = "tag_c",
		consume = {},
		drop_client = {
			{
				2,
				52072,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[503] = {
		id = 503,
		rarity = 2,
		time = 28800,
		type = 1,
		name = "C-185-MI",
		lv_limit = 30,
		desc = "성정 큐브에 관한 기초 이론은 큐브 구조에 대한 인식으로부터 시작된다. 큐브 기초 구조에 대한 이론 구조 작업은 여전히 전력을 다해 진행 중이다.",
		condition = 0,
		bg = "2",
		blueprint_version = 6,
		sub_name = "대형 항목",
		label = "tag_red",
		bg_icon = "item_2",
		label_color = "tag_c",
		consume = {},
		drop_client = {
			{
				2,
				52071,
				1
			},
			{
				2,
				52072,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[504] = {
		id = 504,
		rarity = 2,
		time = 28800,
		type = 1,
		name = "C-185-MI",
		lv_limit = 30,
		desc = "성정 큐브에 관한 기초 이론은 큐브 구조에 대한 인식으로부터 시작된다. 큐브 기초 구조에 대한 이론 구조 작업은 여전히 전력을 다해 진행 중이다.",
		condition = 0,
		bg = "2",
		blueprint_version = 6,
		sub_name = "대형 항목",
		label = "tag_red",
		bg_icon = "item_2",
		label_color = "tag_c",
		consume = {},
		drop_client = {
			{
				2,
				52071,
				1
			},
			{
				2,
				52072,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[511] = {
		id = 511,
		rarity = 1,
		time = 14400,
		type = 1,
		name = "B-622-MI",
		lv_limit = 30,
		desc = "성정 큐브 구조에 관한 여러가지 가설에 대해 실증실험을 진행하는 연구 프로젝트. 실전을 통해 데이터를 수집하여, 이론의 과학적 증명에 기여하자.",
		condition = 52001,
		bg = "1",
		blueprint_version = 6,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "battle_1",
		label_color = "tag_b",
		consume = {},
		drop_client = {
			{
				2,
				52071,
				1
			},
			{
				2,
				52072,
				1
			},
			{
				2,
				52004,
				1
			}
		}
	},
	[512] = {
		id = 512,
		rarity = 1,
		time = 14400,
		type = 1,
		name = "B-636-MI",
		lv_limit = 30,
		desc = "성정 큐브 구조에 관한 여러가지 가설에 대해 실증실험을 진행하는 연구 프로젝트. 실전을 통해 데이터를 수집하여, 이론의 과학적 증명에 기여하자.",
		condition = 52002,
		bg = "1",
		blueprint_version = 6,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "battle_1",
		label_color = "tag_b",
		consume = {},
		drop_client = {
			{
				2,
				52071,
				1
			},
			{
				2,
				52072,
				1
			},
			{
				2,
				52004,
				1
			}
		}
	},
	[513] = {
		id = 513,
		rarity = 1,
		time = 14400,
		type = 1,
		name = "B-654-MI",
		lv_limit = 30,
		desc = "성정 큐브 구조에 관한 여러가지 가설에 대해 실증실험을 진행하는 연구 프로젝트. 실전을 통해 데이터를 수집하여, 이론의 과학적 증명에 기여하자.",
		condition = 52003,
		bg = "1",
		blueprint_version = 6,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "battle_1",
		label_color = "tag_b",
		consume = {},
		drop_client = {
			{
				2,
				52071,
				1
			},
			{
				2,
				52072,
				1
			},
			{
				2,
				52004,
				1
			}
		}
	},
	[514] = {
		id = 514,
		rarity = 1,
		time = 14400,
		type = 1,
		name = "B-682-MI",
		lv_limit = 40,
		desc = "성정 큐브 구조에 관한 여러가지 가설에 대해 실증실험을 진행하는 연구 프로젝트. 실전을 통해 데이터를 수집하여, 이론의 과학적 증명에 기여하자.",
		condition = 52004,
		bg = "1",
		blueprint_version = 6,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "battle_1",
		label_color = "tag_b",
		consume = {},
		drop_client = {
			{
				2,
				52071,
				1
			},
			{
				2,
				52072,
				1
			},
			{
				2,
				52004,
				1
			}
		}
	},
	[515] = {
		id = 515,
		rarity = 2,
		time = 14400,
		type = 1,
		name = "B-235-MI",
		lv_limit = 50,
		desc = "성정 큐브 구조에 관한 여러가지 가설에 대해 실증실험을 진행하는 연구 프로젝트. 실전을 통해 데이터를 수집하여, 이론의 과학적 증명에 기여하자.",
		condition = 52005,
		bg = "2",
		blueprint_version = 6,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "battle_2",
		label_color = "tag_b",
		consume = {},
		drop_client = {
			{
				2,
				52071,
				1
			},
			{
				2,
				52072,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[516] = {
		id = 516,
		rarity = 2,
		time = 14400,
		type = 1,
		name = "B-268-MI",
		lv_limit = 60,
		desc = "성정 큐브 구조에 관한 여러가지 가설에 대해 실증실험을 진행하는 연구 프로젝트. 실전을 통해 데이터를 수집하여, 이론의 과학적 증명에 기여하자.",
		condition = 52006,
		bg = "2",
		blueprint_version = 6,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "battle_2",
		label_color = "tag_b",
		consume = {},
		drop_client = {
			{
				2,
				52071,
				1
			},
			{
				2,
				52072,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[517] = {
		id = 517,
		rarity = 2,
		time = 14400,
		type = 1,
		name = "B-128-MI",
		lv_limit = 65,
		desc = "성정 큐브 구조에 관한 여러가지 가설에 대해 실증실험을 진행하는 연구 프로젝트. 실전을 통해 데이터를 수집하여, 이론의 과학적 증명에 기여하자.",
		condition = 52007,
		bg = "2",
		blueprint_version = 6,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "battle_2",
		label_color = "tag_b",
		consume = {},
		drop_client = {
			{
				2,
				52071,
				1
			},
			{
				2,
				52072,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[518] = {
		id = 518,
		rarity = 2,
		time = 14400,
		type = 1,
		name = "B-164-MI",
		lv_limit = 70,
		desc = "성정 큐브 구조에 관한 여러가지 가설에 대해 실증실험을 진행하는 연구 프로젝트. 실전을 통해 데이터를 수집하여, 이론의 과학적 증명에 기여하자.",
		condition = 52008,
		bg = "2",
		blueprint_version = 6,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "battle_2",
		label_color = "tag_b",
		consume = {},
		drop_client = {
			{
				2,
				52071,
				1
			},
			{
				2,
				52072,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[521] = {
		id = 521,
		rarity = 1,
		time = 10800,
		type = 1,
		name = "T-018-MI",
		lv_limit = 30,
		desc = "성정 큐브에 관한 연구는 [건조] 방면뿐만 아니라, 일상생활 이용 등 다양한 방면으로 진행되고 있다. 성정 큐브의 다방면 활용 가능성을 찾는 프로젝트에는 군의 조력이 필수불가결이다.",
		condition = 52021,
		bg = "1",
		blueprint_version = 6,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "item_1",
		label_color = "tag_t",
		consume = {},
		drop_client = {
			{
				2,
				52072,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[522] = {
		id = 522,
		rarity = 1,
		time = 14400,
		type = 1,
		name = "T-384-MI",
		lv_limit = 30,
		desc = "성정 큐브에 관한 연구는 [건조] 방면뿐만 아니라, 일상생활 이용 등 다양한 방면으로 진행되고 있다. 성정 큐브의 다방면 활용 가능성을 찾는 프로젝트에는 군의 조력이 필수불가결이다.",
		condition = 52022,
		bg = "1",
		blueprint_version = 6,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "item_1",
		label_color = "tag_t",
		consume = {},
		drop_client = {
			{
				2,
				52072,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[523] = {
		id = 523,
		rarity = 2,
		time = 21600,
		type = 1,
		name = "T-249-MI",
		lv_limit = 30,
		desc = "성정 큐브에 관한 연구는 [건조] 방면뿐만 아니라, 일상생활 이용 등 다양한 방면으로 진행되고 있다. 성정 큐브의 다방면 활용 가능성을 찾는 프로젝트에는 군의 조력이 필수불가결이다.",
		condition = 52023,
		bg = "2",
		blueprint_version = 6,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "item_2",
		label_color = "tag_t",
		consume = {},
		drop_client = {
			{
				2,
				52072,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[531] = {
		id = 531,
		rarity = 1,
		time = 7200,
		type = 1,
		name = "E-031-MI",
		lv_limit = 30,
		desc = "성정 큐브와 [장비]의 관련성을 조사하는 다수의 시도는, 방대한 실험 데이터에 기반하여 진행되고 있다. 불필요한 장비를 제공하여, 실험 샘플을 충당하자.",
		condition = 52031,
		bg = "1",
		blueprint_version = 6,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "item_1",
		label_color = "tag_e",
		consume = {},
		drop_client = {
			{
				2,
				52072,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[532] = {
		id = 532,
		rarity = 2,
		time = 7200,
		type = 1,
		name = "E-315-MI",
		lv_limit = 30,
		desc = "성정 큐브와 [장비]의 관련성을 조사하는 다수의 시도는, 방대한 실험 데이터에 기반하여 진행되고 있다. 불필요한 장비를 제공하여, 실험 샘플을 충당하자.",
		condition = 52032,
		bg = "2",
		blueprint_version = 6,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "item_2",
		label_color = "tag_e",
		consume = {},
		drop_client = {
			{
				2,
				52072,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[533] = {
		id = 533,
		rarity = 2,
		time = 21600,
		type = 1,
		name = "E-113-MI",
		lv_limit = 50,
		desc = "현재 배치 중인 장비를 분해하여 기초 설계를 재검토하고, 장래에 개발이 예정된 신규 장비의 기초 연구를 실시한다. ",
		condition = 0,
		bg = "2",
		blueprint_version = 6,
		sub_name = "중형 항목",
		label = "tag_blue",
		bg_icon = "item_2",
		label_color = "tag_e",
		consume = {
			{
				2,
				30013,
				15
			}
		},
		drop_client = {
			{
				2,
				13037,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[534] = {
		id = 534,
		rarity = 2,
		time = 21600,
		type = 1,
		name = "E-139-MI",
		lv_limit = 50,
		desc = "현재 배치 중인 장비를 분해하여 기초 설계를 재검토하고, 장래에 개발이 예정된 신규 장비의 기초 연구를 실시한다. ",
		condition = 0,
		bg = "2",
		blueprint_version = 6,
		sub_name = "중형 항목",
		label = "tag_blue",
		bg_icon = "item_2",
		label_color = "tag_e",
		consume = {
			{
				2,
				30033,
				15
			}
		},
		drop_client = {
			{
				2,
				13039,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[535] = {
		id = 535,
		rarity = 2,
		time = 21600,
		type = 1,
		name = "E-171-MI",
		lv_limit = 50,
		desc = "현재 배치 중인 장비를 분해하여 기초 설계를 재검토하고, 장래에 개발이 예정된 신규 장비의 기초 연구를 실시한다. ",
		condition = 0,
		bg = "2",
		blueprint_version = 6,
		sub_name = "중형 항목",
		label = "tag_blue",
		bg_icon = "item_2",
		label_color = "tag_e",
		consume = {
			{
				2,
				30043,
				15
			}
		},
		drop_client = {
			{
				2,
				13040,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[536] = {
		id = 536,
		rarity = 2,
		time = 21600,
		type = 1,
		name = "E-196-MI",
		lv_limit = 50,
		desc = "현재 배치 중인 장비를 분해하여 기초 설계를 재검토하고, 장래에 개발이 예정된 신규 장비의 기초 연구를 실시한다. ",
		condition = 0,
		bg = "2",
		blueprint_version = 6,
		sub_name = "중형 항목",
		label = "tag_blue",
		bg_icon = "item_2",
		label_color = "tag_e",
		consume = {
			{
				2,
				30043,
				15
			}
		},
		drop_client = {
			{
				2,
				13041,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[537] = {
		id = 537,
		rarity = 2,
		time = 21600,
		type = 1,
		name = "E-184-MI",
		lv_limit = 50,
		desc = "현재 배치 중인 장비를 분해하여 기초 설계를 재검토하고, 장래에 개발이 예정된 신규 장비의 기초 연구를 실시한다. ",
		condition = 0,
		bg = "2",
		blueprint_version = 6,
		sub_name = "중형 항목",
		label = "tag_blue",
		bg_icon = "item_2",
		label_color = "tag_e",
		consume = {
			{
				2,
				30043,
				15
			}
		},
		drop_client = {
			{
				2,
				13042,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[541] = {
		id = 541,
		rarity = 1,
		time = 5400,
		type = 1,
		name = "G-412-MI",
		lv_limit = 30,
		desc = "뛰어난 과학의 발전을 뒷받침하기 위해선 영민한 두뇌뿐만 아니라, 지원자로부터 제공되는 연구 자금도 필요하다. 남아도는 자금을 기부하여 기술의 연구를 서포트하자.",
		condition = 0,
		bg = "1",
		blueprint_version = 6,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "gold_1",
		label_color = "tag_g",
		consume = {
			{
				1,
				1,
				1500
			}
		},
		drop_client = {
			{
				2,
				52071,
				1
			},
			{
				2,
				52072,
				1
			},
			{
				2,
				52004,
				1
			}
		}
	},
	[542] = {
		id = 542,
		rarity = 1,
		time = 5400,
		type = 1,
		name = "G-412-MI",
		lv_limit = 30,
		desc = "뛰어난 과학의 발전을 뒷받침하기 위해선 영민한 두뇌뿐만 아니라, 지원자로부터 제공되는 연구 자금도 필요하다. 남아도는 자금을 기부하여 기술의 연구를 서포트하자.",
		condition = 0,
		bg = "1",
		blueprint_version = 6,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "gold_1",
		label_color = "tag_g",
		consume = {
			{
				1,
				1,
				1500
			}
		},
		drop_client = {
			{
				2,
				52071,
				1
			},
			{
				2,
				52072,
				1
			},
			{
				2,
				52004,
				1
			}
		}
	},
	[543] = {
		id = 543,
		rarity = 1,
		time = 9000,
		type = 1,
		name = "G-236-MI",
		lv_limit = 30,
		desc = "뛰어난 과학의 발전을 뒷받침하기 위해선 영민한 두뇌뿐만 아니라, 지원자로부터 제공되는 연구 자금도 필요하다. 남아도는 자금을 기부하여 기술의 연구를 서포트하자.",
		condition = 0,
		bg = "1",
		blueprint_version = 6,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "gold_1",
		label_color = "tag_g",
		consume = {
			{
				1,
				1,
				3000
			}
		},
		drop_client = {
			{
				2,
				52071,
				1
			},
			{
				2,
				52072,
				1
			},
			{
				2,
				52004,
				1
			}
		}
	},
	[544] = {
		id = 544,
		rarity = 1,
		time = 9000,
		type = 1,
		name = "G-236-MI",
		lv_limit = 30,
		desc = "뛰어난 과학의 발전을 뒷받침하기 위해선 영민한 두뇌뿐만 아니라, 지원자로부터 제공되는 연구 자금도 필요하다. 남아도는 자금을 기부하여 기술의 연구를 서포트하자.",
		condition = 0,
		bg = "1",
		blueprint_version = 6,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "gold_1",
		label_color = "tag_g",
		consume = {
			{
				1,
				1,
				3000
			}
		},
		drop_client = {
			{
				2,
				52071,
				1
			},
			{
				2,
				52072,
				1
			},
			{
				2,
				52004,
				1
			}
		}
	},
	[545] = {
		id = 545,
		rarity = 2,
		time = 14400,
		type = 1,
		name = "G-531-MI",
		lv_limit = 30,
		desc = "뛰어난 과학의 발전을 뒷받침하기 위해선 영민한 두뇌뿐만 아니라, 지원자로부터 제공되는 연구 자금도 필요하다. 남아도는 자금을 기부하여 기술의 연구를 서포트하자.",
		condition = 0,
		bg = "2",
		blueprint_version = 6,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "gold_2",
		label_color = "tag_g",
		consume = {
			{
				1,
				1,
				6000
			}
		},
		drop_client = {
			{
				2,
				52071,
				1
			},
			{
				2,
				52072,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[546] = {
		id = 546,
		rarity = 2,
		time = 14400,
		type = 1,
		name = "G-531-MI",
		lv_limit = 30,
		desc = "뛰어난 과학의 발전을 뒷받침하기 위해선 영민한 두뇌뿐만 아니라, 지원자로부터 제공되는 연구 자금도 필요하다. 남아도는 자금을 기부하여 기술의 연구를 서포트하자.",
		condition = 0,
		bg = "2",
		blueprint_version = 6,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "gold_2",
		label_color = "tag_g",
		consume = {
			{
				1,
				1,
				6000
			}
		},
		drop_client = {
			{
				2,
				52071,
				1
			},
			{
				2,
				52072,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[551] = {
		id = 551,
		rarity = 1,
		time = 9000,
		type = 1,
		name = "D-783-MI",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수의 사람들의 사물에 대한 인식의 집합체를 구현화하는 것이라면, 그것의 '방향성' 을 정하는 것에 따라 결과는 과연 어떻게 바뀔 것인가.",
		condition = 0,
		bg = "1",
		blueprint_version = 6,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "chara_xingdengbao",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				3000
			}
		},
		drop_client = {
			{
				2,
				42054,
				1
			},
			{
				2,
				52072,
				1
			},
			{
				2,
				52004,
				1
			}
		}
	},
	[552] = {
		id = 552,
		rarity = 1,
		time = 9000,
		type = 1,
		name = "D-713-MI",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수의 사람들의 사물에 대한 인식의 집합체를 구현화하는 것이라면, 그것의 '방향성' 을 정하는 것에 따라 결과는 과연 어떻게 바뀔 것인가.",
		condition = 0,
		bg = "1",
		blueprint_version = 6,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "chara_qiershazhi",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				3000
			}
		},
		drop_client = {
			{
				2,
				42051,
				1
			},
			{
				2,
				52072,
				1
			},
			{
				2,
				52004,
				1
			}
		}
	},
	[553] = {
		id = 553,
		rarity = 1,
		time = 9000,
		type = 1,
		name = "D-739-MI",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수의 사람들의 사물에 대한 인식의 집합체를 구현화하는 것이라면, 그것의 '방향성' 을 정하는 것에 따라 결과는 과연 어떻게 바뀔 것인가.",
		condition = 0,
		bg = "1",
		blueprint_version = 6,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "chara_siwanshi",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				3000
			}
		},
		drop_client = {
			{
				2,
				42052,
				1
			},
			{
				2,
				52072,
				1
			},
			{
				2,
				52004,
				1
			}
		}
	},
	[554] = {
		id = 554,
		rarity = 1,
		time = 9000,
		type = 1,
		name = "D-771-MI",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수의 사람들의 사물에 대한 인식의 집합체를 구현화하는 것이라면, 그것의 '방향성' 을 정하는 것에 따라 결과는 과연 어떻게 바뀔 것인가.",
		condition = 0,
		bg = "1",
		blueprint_version = 6,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "chara_feilikesishuerci",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				3000
			}
		},
		drop_client = {
			{
				2,
				42053,
				1
			},
			{
				2,
				52072,
				1
			},
			{
				2,
				52004,
				1
			}
		}
	},
	[555] = {
		id = 555,
		rarity = 1,
		time = 9000,
		type = 1,
		name = "D-796-MI",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수의 사람들의 사물에 대한 인식의 집합체를 구현화하는 것이라면, 그것의 '방향성' 을 정하는 것에 따라 결과는 과연 어떻게 바뀔 것인가.",
		condition = 0,
		bg = "1",
		blueprint_version = 6,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "chara_fulandeer",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				3000
			}
		},
		drop_client = {
			{
				2,
				42055,
				1
			},
			{
				2,
				52072,
				1
			},
			{
				2,
				52004,
				1
			}
		}
	},
	[561] = {
		id = 561,
		rarity = 2,
		time = 18000,
		type = 1,
		name = "D-383-MI",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수의 사람들의 사물에 대한 인식의 집합체를 구현화하는 것이라면, 그것의 '방향성' 을 정하는 것에 따라 결과는 과연 어떻게 바뀔 것인가.",
		condition = 0,
		bg = "2",
		blueprint_version = 6,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "chara_xingdengbao",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				5000
			}
		},
		drop_client = {
			{
				2,
				42054,
				1
			},
			{
				2,
				52072,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[562] = {
		id = 562,
		rarity = 2,
		time = 18000,
		type = 1,
		name = "D-313-MI",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수의 사람들의 사물에 대한 인식의 집합체를 구현화하는 것이라면, 그것의 '방향성' 을 정하는 것에 따라 결과는 과연 어떻게 바뀔 것인가.",
		condition = 0,
		bg = "2",
		blueprint_version = 6,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "chara_qiershazhi",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				5000
			}
		},
		drop_client = {
			{
				2,
				42051,
				1
			},
			{
				2,
				52072,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[563] = {
		id = 563,
		rarity = 2,
		time = 18000,
		type = 1,
		name = "D-339-MI",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수의 사람들의 사물에 대한 인식의 집합체를 구현화하는 것이라면, 그것의 '방향성' 을 정하는 것에 따라 결과는 과연 어떻게 바뀔 것인가.",
		condition = 0,
		bg = "2",
		blueprint_version = 6,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "chara_siwanshi",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				5000
			}
		},
		drop_client = {
			{
				2,
				42052,
				1
			},
			{
				2,
				52072,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[564] = {
		id = 564,
		rarity = 2,
		time = 18000,
		type = 1,
		name = "D-371-MI",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수의 사람들의 사물에 대한 인식의 집합체를 구현화하는 것이라면, 그것의 '방향성' 을 정하는 것에 따라 결과는 과연 어떻게 바뀔 것인가.",
		condition = 0,
		bg = "2",
		blueprint_version = 6,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "chara_feilikesishuerci",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				5000
			}
		},
		drop_client = {
			{
				2,
				42053,
				1
			},
			{
				2,
				52072,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[565] = {
		id = 565,
		rarity = 2,
		time = 18000,
		type = 1,
		name = "D-396-MI",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수의 사람들의 사물에 대한 인식의 집합체를 구현화하는 것이라면, 그것의 '방향성' 을 정하는 것에 따라 결과는 과연 어떻게 바뀔 것인가.",
		condition = 0,
		bg = "2",
		blueprint_version = 6,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "chara_fulandeer",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				5000
			}
		},
		drop_client = {
			{
				2,
				42055,
				1
			},
			{
				2,
				52072,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[571] = {
		id = 571,
		rarity = 1,
		time = 3600,
		type = 1,
		name = "Q-302-MI",
		lv_limit = 30,
		desc = "장비와 성정 큐브의 공명 현상을 제어할 수 있도록, 갖은 수단을 동원해 해당 현상을 재현하고, 최대한 효과적으로 이용할 수 있도록 한다.",
		condition = 0,
		bg = "1",
		blueprint_version = 6,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_1",
		label_color = "tag_q",
		consume = {
			{
				2,
				17001,
				20
			}
		},
		drop_client = {
			{
				2,
				52072,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[572] = {
		id = 572,
		rarity = 1,
		time = 3600,
		type = 1,
		name = "Q-310-MI",
		lv_limit = 30,
		desc = "장비와 성정 큐브의 공명 현상을 제어할 수 있도록, 갖은 수단을 동원해 해당 현상을 재현하고, 최대한 효과적으로 이용할 수 있도록 한다.",
		condition = 0,
		bg = "1",
		blueprint_version = 6,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_1",
		label_color = "tag_q",
		consume = {
			{
				2,
				17011,
				20
			}
		},
		drop_client = {
			{
				2,
				52072,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[573] = {
		id = 573,
		rarity = 1,
		time = 3600,
		type = 1,
		name = "Q-351-MI",
		lv_limit = 30,
		desc = "장비와 성정 큐브의 공명 현상을 제어할 수 있도록, 갖은 수단을 동원해 해당 현상을 재현하고, 최대한 효과적으로 이용할 수 있도록 한다.",
		condition = 0,
		bg = "1",
		blueprint_version = 6,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_1",
		label_color = "tag_q",
		consume = {
			{
				2,
				17021,
				20
			}
		},
		drop_client = {
			{
				2,
				52072,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[574] = {
		id = 574,
		rarity = 1,
		time = 3600,
		type = 1,
		name = "Q-368-MI",
		lv_limit = 30,
		desc = "장비와 성정 큐브의 공명 현상을 제어할 수 있도록, 갖은 수단을 동원해 해당 현상을 재현하고, 최대한 효과적으로 이용할 수 있도록 한다.",
		condition = 0,
		bg = "1",
		blueprint_version = 6,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_1",
		label_color = "tag_q",
		consume = {
			{
				2,
				17031,
				20
			}
		},
		drop_client = {
			{
				2,
				52072,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[575] = {
		id = 575,
		rarity = 1,
		time = 3600,
		type = 1,
		name = "Q-389-MI",
		lv_limit = 30,
		desc = "장비와 성정 큐브의 공명 현상을 제어할 수 있도록, 갖은 수단을 동원해 해당 현상을 재현하고, 최대한 효과적으로 이용할 수 있도록 한다.",
		condition = 0,
		bg = "1",
		blueprint_version = 6,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_1",
		label_color = "tag_q",
		consume = {
			{
				2,
				17041,
				20
			}
		},
		drop_client = {
			{
				2,
				52072,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[576] = {
		id = 576,
		rarity = 1,
		time = 7200,
		type = 1,
		name = "Q-202-MI",
		lv_limit = 30,
		desc = "장비와 성정 큐브의 공명 현상을 제어할 수 있도록, 갖은 수단을 동원해 해당 현상을 재현하고, 최대한 효과적으로 이용할 수 있도록 한다.",
		condition = 0,
		bg = "1",
		blueprint_version = 6,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_1",
		label_color = "tag_q",
		consume = {
			{
				2,
				17002,
				20
			}
		},
		drop_client = {
			{
				2,
				52072,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[577] = {
		id = 577,
		rarity = 1,
		time = 7200,
		type = 1,
		name = "Q-210-MI",
		lv_limit = 30,
		desc = "장비와 성정 큐브의 공명 현상을 제어할 수 있도록, 갖은 수단을 동원해 해당 현상을 재현하고, 최대한 효과적으로 이용할 수 있도록 한다.",
		condition = 0,
		bg = "1",
		blueprint_version = 6,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_1",
		label_color = "tag_q",
		consume = {
			{
				2,
				17012,
				20
			}
		},
		drop_client = {
			{
				2,
				52072,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[578] = {
		id = 578,
		rarity = 1,
		time = 7200,
		type = 1,
		name = "Q-251-MI",
		lv_limit = 30,
		desc = "장비와 성정 큐브의 공명 현상을 제어할 수 있도록, 갖은 수단을 동원해 해당 현상을 재현하고, 최대한 효과적으로 이용할 수 있도록 한다.",
		condition = 0,
		bg = "1",
		blueprint_version = 6,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_1",
		label_color = "tag_q",
		consume = {
			{
				2,
				17022,
				20
			}
		},
		drop_client = {
			{
				2,
				52072,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[579] = {
		id = 579,
		rarity = 1,
		time = 7200,
		type = 1,
		name = "Q-268-MI",
		lv_limit = 30,
		desc = "장비와 성정 큐브의 공명 현상을 제어할 수 있도록, 갖은 수단을 동원해 해당 현상을 재현하고, 최대한 효과적으로 이용할 수 있도록 한다.",
		condition = 0,
		bg = "1",
		blueprint_version = 6,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_1",
		label_color = "tag_q",
		consume = {
			{
				2,
				17032,
				20
			}
		},
		drop_client = {
			{
				2,
				52072,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[580] = {
		id = 580,
		rarity = 1,
		time = 7200,
		type = 1,
		name = "Q-289-MI",
		lv_limit = 30,
		desc = "장비와 성정 큐브의 공명 현상을 제어할 수 있도록, 갖은 수단을 동원해 해당 현상을 재현하고, 최대한 효과적으로 이용할 수 있도록 한다.",
		condition = 0,
		bg = "1",
		blueprint_version = 6,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_1",
		label_color = "tag_q",
		consume = {
			{
				2,
				17042,
				20
			}
		},
		drop_client = {
			{
				2,
				52072,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[581] = {
		id = 581,
		rarity = 2,
		time = 14400,
		type = 1,
		name = "Q-002-MI",
		lv_limit = 30,
		desc = "장비와 성정 큐브의 공명 현상을 제어할 수 있도록, 갖은 수단을 동원해 해당 현상을 재현하고, 최대한 효과적으로 이용할 수 있도록 한다.",
		condition = 0,
		bg = "2",
		blueprint_version = 6,
		sub_name = "중형 항목",
		label = "tag_blue",
		bg_icon = "gold_2",
		label_color = "tag_q",
		consume = {
			{
				2,
				17003,
				20
			}
		},
		drop_client = {
			{
				2,
				52072,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[582] = {
		id = 582,
		rarity = 2,
		time = 14400,
		type = 1,
		name = "Q-010-MI",
		lv_limit = 30,
		desc = "장비와 성정 큐브의 공명 현상을 제어할 수 있도록, 갖은 수단을 동원해 해당 현상을 재현하고, 최대한 효과적으로 이용할 수 있도록 한다.",
		condition = 0,
		bg = "2",
		blueprint_version = 6,
		sub_name = "중형 항목",
		label = "tag_blue",
		bg_icon = "gold_2",
		label_color = "tag_q",
		consume = {
			{
				2,
				17013,
				20
			}
		},
		drop_client = {
			{
				2,
				52072,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[583] = {
		id = 583,
		rarity = 2,
		time = 14400,
		type = 1,
		name = "Q-051-MI",
		lv_limit = 30,
		desc = "장비와 성정 큐브의 공명 현상을 제어할 수 있도록, 갖은 수단을 동원해 해당 현상을 재현하고, 최대한 효과적으로 이용할 수 있도록 한다.",
		condition = 0,
		bg = "2",
		blueprint_version = 6,
		sub_name = "중형 항목",
		label = "tag_blue",
		bg_icon = "gold_2",
		label_color = "tag_q",
		consume = {
			{
				2,
				17023,
				20
			}
		},
		drop_client = {
			{
				2,
				52072,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[584] = {
		id = 584,
		rarity = 2,
		time = 14400,
		type = 1,
		name = "Q-068-MI",
		lv_limit = 30,
		desc = "장비와 성정 큐브의 공명 현상을 제어할 수 있도록, 갖은 수단을 동원해 해당 현상을 재현하고, 최대한 효과적으로 이용할 수 있도록 한다.",
		condition = 0,
		bg = "2",
		blueprint_version = 6,
		sub_name = "중형 항목",
		label = "tag_blue",
		bg_icon = "gold_2",
		label_color = "tag_q",
		consume = {
			{
				2,
				17033,
				20
			}
		},
		drop_client = {
			{
				2,
				52072,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[585] = {
		id = 585,
		rarity = 2,
		time = 14400,
		type = 1,
		name = "Q-089-MI",
		lv_limit = 30,
		desc = "장비와 성정 큐브의 공명 현상을 제어할 수 있도록, 갖은 수단을 동원해 해당 현상을 재현하고, 최대한 효과적으로 이용할 수 있도록 한다.",
		condition = 0,
		bg = "2",
		blueprint_version = 6,
		sub_name = "중형 항목",
		label = "tag_blue",
		bg_icon = "gold_2",
		label_color = "tag_q",
		consume = {
			{
				2,
				17043,
				20
			}
		},
		drop_client = {
			{
				2,
				52072,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[591] = {
		id = 591,
		rarity = 1,
		time = 3600,
		type = 1,
		name = "H-387-MI",
		lv_limit = 30,
		desc = "성정 큐브를 문자 그대로 물리적으로 '분해'하여, 구조 분석을 진행한다. 성공 사례가 없을수록, 실험의 가치는 그만큼 크다고 볼 수 있다.",
		condition = 0,
		bg = "1",
		blueprint_version = 6,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "gold_1",
		label_color = "tag_h",
		consume = {
			{
				2,
				20001,
				3
			}
		},
		drop_client = {
			{
				2,
				52071,
				1
			},
			{
				2,
				54049,
				1
			},
			{
				2,
				15008,
				1
			}
		}
	},
	[592] = {
		id = 592,
		rarity = 1,
		time = 3600,
		type = 1,
		name = "H-387-MI",
		lv_limit = 30,
		desc = "성정 큐브를 문자 그대로 물리적으로 '분해'하여, 구조 분석을 진행한다. 성공 사례가 없을수록, 실험의 가치는 그만큼 크다고 볼 수 있다.",
		condition = 0,
		bg = "1",
		blueprint_version = 6,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "gold_1",
		label_color = "tag_h",
		consume = {
			{
				2,
				20001,
				3
			}
		},
		drop_client = {
			{
				2,
				52071,
				1
			},
			{
				2,
				54049,
				1
			},
			{
				2,
				15008,
				1
			}
		}
	},
	[593] = {
		id = 593,
		rarity = 2,
		time = 7200,
		type = 1,
		name = "H-339-MI",
		lv_limit = 30,
		desc = "성정 큐브를 문자 그대로 물리적으로 '분해'하여, 구조 분석을 진행한다. 성공 사례가 없을수록, 실험의 가치는 그만큼 크다고 볼 수 있다.",
		condition = 0,
		bg = "2",
		blueprint_version = 6,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "gold_2",
		label_color = "tag_h",
		consume = {
			{
				2,
				20001,
				6
			}
		},
		drop_client = {
			{
				2,
				52071,
				1
			},
			{
				2,
				54050,
				1
			},
			{
				2,
				15008,
				1
			}
		}
	},
	[594] = {
		id = 594,
		rarity = 2,
		time = 7200,
		type = 1,
		name = "H-339-MI",
		lv_limit = 30,
		desc = "성정 큐브를 문자 그대로 물리적으로 '분해'하여, 구조 분석을 진행한다. 성공 사례가 없을수록, 실험의 가치는 그만큼 크다고 볼 수 있다.",
		condition = 0,
		bg = "2",
		blueprint_version = 6,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "gold_2",
		label_color = "tag_h",
		consume = {
			{
				2,
				20001,
				6
			}
		},
		drop_client = {
			{
				2,
				52071,
				1
			},
			{
				2,
				54050,
				1
			},
			{
				2,
				15008,
				1
			}
		}
	},
	[601] = {
		id = 601,
		rarity = 1,
		time = 21600,
		type = 1,
		name = "C-153-MI",
		lv_limit = 30,
		desc = "성정 큐브에 관한 기초 이론은 큐브의 구조에 대한 인식에서부터 시작한다. 큐브의 기초 구조에 대한 이론 구축 작업은 지금도 여전히 활발하게 이루어지고 있다.",
		condition = 0,
		bg = "1",
		blueprint_version = 7,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "item_1",
		label_color = "tag_c",
		consume = {},
		drop_client = {
			{
				2,
				52082,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[602] = {
		id = 602,
		rarity = 1,
		time = 21600,
		type = 1,
		name = "C-153-MI",
		lv_limit = 30,
		desc = "성정 큐브에 관한 기초 이론은 큐브 구조에 대한 인식으로부터 시작된다. 큐브 기초 구조에 대한 이론 구조 작업은 여전히 전력을 다해 진행 중이다.",
		condition = 0,
		bg = "1",
		blueprint_version = 7,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "item_1",
		label_color = "tag_c",
		consume = {},
		drop_client = {
			{
				2,
				52082,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[603] = {
		id = 603,
		rarity = 2,
		time = 28800,
		type = 1,
		name = "C-185-MI",
		lv_limit = 30,
		desc = "성정 큐브에 관한 기초 이론은 큐브 구조에 대한 인식으로부터 시작된다. 큐브 기초 구조에 대한 이론 구조 작업은 여전히 전력을 다해 진행 중이다.",
		condition = 0,
		bg = "2",
		blueprint_version = 7,
		sub_name = "대형 항목",
		label = "tag_red",
		bg_icon = "item_2",
		label_color = "tag_c",
		consume = {},
		drop_client = {
			{
				2,
				52081,
				1
			},
			{
				2,
				52082,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[604] = {
		id = 604,
		rarity = 2,
		time = 28800,
		type = 1,
		name = "C-185-MI",
		lv_limit = 30,
		desc = "성정 큐브에 관한 기초 이론은 큐브 구조에 대한 인식으로부터 시작된다. 큐브 기초 구조에 대한 이론 구조 작업은 여전히 전력을 다해 진행 중이다.",
		condition = 0,
		bg = "2",
		blueprint_version = 7,
		sub_name = "대형 항목",
		label = "tag_red",
		bg_icon = "item_2",
		label_color = "tag_c",
		consume = {},
		drop_client = {
			{
				2,
				52081,
				1
			},
			{
				2,
				52082,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[611] = {
		id = 611,
		rarity = 1,
		time = 14400,
		type = 1,
		name = "B-622-MI",
		lv_limit = 30,
		desc = "성정 큐브의 구조에 관한 다양한 가설에 대해 실증 실험을 실시하는 연구 프로젝트. 실전을 통해 데이터를 수집하여 아이디어의 과학적 증명에 기여하자.",
		condition = 52001,
		bg = "1",
		blueprint_version = 7,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "battle_1",
		label_color = "tag_b",
		consume = {},
		drop_client = {
			{
				2,
				52081,
				1
			},
			{
				2,
				52082,
				1
			},
			{
				2,
				52004,
				1
			}
		}
	},
	[612] = {
		id = 612,
		rarity = 1,
		time = 14400,
		type = 1,
		name = "B-636-MI",
		lv_limit = 30,
		desc = "성정 큐브 구조에 관한 여러가지 가설에 대해 실증실험을 진행하는 연구 프로젝트. 실전을 통해 데이터를 수집하여, 이론의 과학적 증명에 기여하자.",
		condition = 52002,
		bg = "1",
		blueprint_version = 7,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "battle_1",
		label_color = "tag_b",
		consume = {},
		drop_client = {
			{
				2,
				52081,
				1
			},
			{
				2,
				52082,
				1
			},
			{
				2,
				52004,
				1
			}
		}
	},
	[613] = {
		id = 613,
		rarity = 1,
		time = 14400,
		type = 1,
		name = "B-654-MI",
		lv_limit = 30,
		desc = "성정 큐브 구조에 관한 여러가지 가설에 대해 실증실험을 진행하는 연구 프로젝트. 실전을 통해 데이터를 수집하여, 이론의 과학적 증명에 기여하자.",
		condition = 52003,
		bg = "1",
		blueprint_version = 7,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "battle_1",
		label_color = "tag_b",
		consume = {},
		drop_client = {
			{
				2,
				52081,
				1
			},
			{
				2,
				52082,
				1
			},
			{
				2,
				52004,
				1
			}
		}
	},
	[614] = {
		id = 614,
		rarity = 1,
		time = 14400,
		type = 1,
		name = "B-682-MI",
		lv_limit = 40,
		desc = "성정 큐브 구조에 관한 여러가지 가설에 대해 실증실험을 진행하는 연구 프로젝트. 실전을 통해 데이터를 수집하여, 이론의 과학적 증명에 기여하자.",
		condition = 52004,
		bg = "1",
		blueprint_version = 7,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "battle_1",
		label_color = "tag_b",
		consume = {},
		drop_client = {
			{
				2,
				52081,
				1
			},
			{
				2,
				52082,
				1
			},
			{
				2,
				52004,
				1
			}
		}
	},
	[615] = {
		id = 615,
		rarity = 2,
		time = 14400,
		type = 1,
		name = "B-235-MI",
		lv_limit = 50,
		desc = "성정 큐브 구조에 관한 여러가지 가설에 대해 실증실험을 진행하는 연구 프로젝트. 실전을 통해 데이터를 수집하여, 이론의 과학적 증명에 기여하자.",
		condition = 52005,
		bg = "2",
		blueprint_version = 7,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "battle_2",
		label_color = "tag_b",
		consume = {},
		drop_client = {
			{
				2,
				52081,
				1
			},
			{
				2,
				52082,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[616] = {
		id = 616,
		rarity = 2,
		time = 14400,
		type = 1,
		name = "B-268-MI",
		lv_limit = 60,
		desc = "성정 큐브 구조에 관한 여러가지 가설에 대해 실증실험을 진행하는 연구 프로젝트. 실전을 통해 데이터를 수집하여, 이론의 과학적 증명에 기여하자.",
		condition = 52006,
		bg = "2",
		blueprint_version = 7,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "battle_2",
		label_color = "tag_b",
		consume = {},
		drop_client = {
			{
				2,
				52081,
				1
			},
			{
				2,
				52082,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[617] = {
		id = 617,
		rarity = 2,
		time = 14400,
		type = 1,
		name = "B-128-MI",
		lv_limit = 65,
		desc = "성정 큐브 구조에 관한 여러가지 가설에 대해 실증실험을 진행하는 연구 프로젝트. 실전을 통해 데이터를 수집하여, 이론의 과학적 증명에 기여하자.",
		condition = 52007,
		bg = "2",
		blueprint_version = 7,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "battle_2",
		label_color = "tag_b",
		consume = {},
		drop_client = {
			{
				2,
				52081,
				1
			},
			{
				2,
				52082,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[618] = {
		id = 618,
		rarity = 2,
		time = 14400,
		type = 1,
		name = "B-164-MI",
		lv_limit = 70,
		desc = "성정 큐브 구조에 관한 여러가지 가설에 대해 실증실험을 진행하는 연구 프로젝트. 실전을 통해 데이터를 수집하여, 이론의 과학적 증명에 기여하자.",
		condition = 52008,
		bg = "2",
		blueprint_version = 7,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "battle_2",
		label_color = "tag_b",
		consume = {},
		drop_client = {
			{
				2,
				52081,
				1
			},
			{
				2,
				52082,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[621] = {
		id = 621,
		rarity = 1,
		time = 10800,
		type = 1,
		name = "T-018-MI",
		lv_limit = 30,
		desc = "「건조」 이외에 성정 큐브의 민간 이용에 관한 연구도 제안되었다. 기술 유용의 가능성을 모색하는 프로젝트에는 군의 협력도 필요하다.",
		condition = 52021,
		bg = "1",
		blueprint_version = 7,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "item_1",
		label_color = "tag_t",
		consume = {},
		drop_client = {
			{
				2,
				52082,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[622] = {
		id = 622,
		rarity = 1,
		time = 14400,
		type = 1,
		name = "T-384-MI",
		lv_limit = 30,
		desc = "「건조」 이외에 성정 큐브의 민간 이용에 관한 연구도 제안되었다. 기술 유용의 가능성을 모색하는 프로젝트에는 군의 협력도 필요하다.",
		condition = 52022,
		bg = "1",
		blueprint_version = 7,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "item_1",
		label_color = "tag_t",
		consume = {},
		drop_client = {
			{
				2,
				52082,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[623] = {
		id = 623,
		rarity = 2,
		time = 21600,
		type = 1,
		name = "T-249-MI",
		lv_limit = 30,
		desc = "「건조」 이외에 성정 큐브의 민간 이용에 관한 연구도 제안되었다. 기술 유용의 가능성을 모색하는 프로젝트에는 군의 협력도 필요하다.",
		condition = 52023,
		bg = "2",
		blueprint_version = 7,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "item_2",
		label_color = "tag_t",
		consume = {},
		drop_client = {
			{
				2,
				52082,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[631] = {
		id = 631,
		rarity = 1,
		time = 7200,
		type = 1,
		name = "E-031-MI",
		lv_limit = 30,
		desc = "성정 큐브와 「의장」의 관련성을 조사하는 수많은 시도는 방대한 실험 데이터를 기반으로 이루어지고 있다. 불필요한 의장을 실험 샘플로 사용할 수 있도록 제공하자.",
		condition = 52031,
		bg = "1",
		blueprint_version = 7,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "item_1",
		label_color = "tag_e",
		consume = {},
		drop_client = {
			{
				2,
				52082,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[632] = {
		id = 632,
		rarity = 2,
		time = 7200,
		type = 1,
		name = "E-315-MI",
		lv_limit = 30,
		desc = "성정 큐브와 「의장」의 관련성을 조사하는 수많은 시도는 방대한 실험 데이터를 기반으로 이루어지고 있다. 불필요한 의장을 실험 샘플로 사용할 수 있도록 제공하자.",
		condition = 52032,
		bg = "2",
		blueprint_version = 7,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "item_2",
		label_color = "tag_e",
		consume = {},
		drop_client = {
			{
				2,
				52082,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[633] = {
		id = 633,
		rarity = 2,
		time = 21600,
		type = 1,
		name = "E-191-MI",
		lv_limit = 50,
		desc = "현재 배치 중인 장비를 분해하여 기초 설계를 수정하고, 미래의 개발이 예정된 신장비의 기초 연구를 실시한다. ",
		condition = 0,
		bg = "2",
		blueprint_version = 7,
		sub_name = "중형 항목",
		label = "tag_blue",
		bg_icon = "item_2",
		label_color = "tag_e",
		consume = {
			{
				2,
				30013,
				15
			}
		},
		drop_client = {
			{
				2,
				13044,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[634] = {
		id = 634,
		rarity = 2,
		time = 21600,
		type = 1,
		name = "E-166-MI",
		lv_limit = 50,
		desc = "현재 배치 중인 장비를 분해하여 기초 설계를 수정하고, 미래의 개발이 예정된 신장비의 기초 연구를 실시한다. ",
		condition = 0,
		bg = "2",
		blueprint_version = 7,
		sub_name = "중형 항목",
		label = "tag_blue",
		bg_icon = "item_2",
		label_color = "tag_e",
		consume = {
			{
				2,
				30043,
				15
			}
		},
		drop_client = {
			{
				2,
				13045,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[635] = {
		id = 635,
		rarity = 2,
		time = 21600,
		type = 1,
		name = "E-167-MI",
		lv_limit = 50,
		desc = "현재 배치 중인 장비를 분해하여 기초 설계를 수정하고, 미래의 개발이 예정된 신장비의 기초 연구를 실시한다. ",
		condition = 0,
		bg = "2",
		blueprint_version = 7,
		sub_name = "중형 항목",
		label = "tag_blue",
		bg_icon = "item_2",
		label_color = "tag_e",
		consume = {
			{
				2,
				30043,
				15
			}
		},
		drop_client = {
			{
				2,
				13046,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[636] = {
		id = 636,
		rarity = 2,
		time = 21600,
		type = 1,
		name = "E-13-MI",
		lv_limit = 50,
		desc = "현재 배치 중인 장비를 분해하여 기초 설계를 수정하고, 미래의 개발이 예정된 신장비의 기초 연구를 실시한다. ",
		condition = 0,
		bg = "2",
		blueprint_version = 7,
		sub_name = "중형 항목",
		label = "tag_blue",
		bg_icon = "item_2",
		label_color = "tag_e",
		consume = {
			{
				2,
				30013,
				15
			}
		},
		drop_client = {
			{
				2,
				13047,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[637] = {
		id = 637,
		rarity = 2,
		time = 21600,
		type = 1,
		name = "E-158-MI",
		lv_limit = 50,
		desc = "현재 배치 중인 장비를 분해하여 기초 설계를 수정하고, 미래의 개발이 예정된 신장비의 기초 연구를 실시한다. ",
		condition = 0,
		bg = "2",
		blueprint_version = 7,
		sub_name = "중형 항목",
		label = "tag_blue",
		bg_icon = "item_2",
		label_color = "tag_e",
		consume = {
			{
				2,
				30033,
				15
			}
		},
		drop_client = {
			{
				2,
				13048,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[641] = {
		id = 641,
		rarity = 1,
		time = 5400,
		type = 1,
		name = "G-412-MI",
		lv_limit = 30,
		desc = "눈부신 과학 발전을 이룩하기 위해서는 뛰어난 두뇌뿐만 아니라, 스폰서로부터 제공 받는 연구 자금도 빼놓을 수 없다. 남아도는 자금을 기부하여 기술 연구를 지원하자.",
		condition = 0,
		bg = "1",
		blueprint_version = 7,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "gold_1",
		label_color = "tag_g",
		consume = {
			{
				1,
				1,
				1500
			}
		},
		drop_client = {
			{
				2,
				52081,
				1
			},
			{
				2,
				52082,
				1
			},
			{
				2,
				52004,
				1
			}
		}
	},
	[642] = {
		id = 642,
		rarity = 1,
		time = 5400,
		type = 1,
		name = "G-412-MI",
		lv_limit = 30,
		desc = "눈부신 과학 발전을 이룩하기 위해서는 뛰어난 두뇌뿐만 아니라, 스폰서로부터 제공 받는 연구 자금도 빼놓을 수 없다. 남아도는 자금을 기부하여 기술 연구를 지원하자.",
		condition = 0,
		bg = "1",
		blueprint_version = 7,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "gold_1",
		label_color = "tag_g",
		consume = {
			{
				1,
				1,
				1500
			}
		},
		drop_client = {
			{
				2,
				52081,
				1
			},
			{
				2,
				52082,
				1
			},
			{
				2,
				52004,
				1
			}
		}
	},
	[643] = {
		id = 643,
		rarity = 1,
		time = 9000,
		type = 1,
		name = "G-236-MI",
		lv_limit = 30,
		desc = "눈부신 과학 발전을 이룩하기 위해서는 뛰어난 두뇌뿐만 아니라, 스폰서로부터 제공 받는 연구 자금도 빼놓을 수 없다. 남아도는 자금을 기부하여 기술 연구를 지원하자.",
		condition = 0,
		bg = "1",
		blueprint_version = 7,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "gold_1",
		label_color = "tag_g",
		consume = {
			{
				1,
				1,
				3000
			}
		},
		drop_client = {
			{
				2,
				52081,
				1
			},
			{
				2,
				52082,
				1
			},
			{
				2,
				52004,
				1
			}
		}
	},
	[644] = {
		id = 644,
		rarity = 1,
		time = 9000,
		type = 1,
		name = "G-236-MI",
		lv_limit = 30,
		desc = "눈부신 과학 발전을 이룩하기 위해서는 뛰어난 두뇌뿐만 아니라, 스폰서로부터 제공 받는 연구 자금도 빼놓을 수 없다. 남아도는 자금을 기부하여 기술 연구를 지원하자.",
		condition = 0,
		bg = "1",
		blueprint_version = 7,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "gold_1",
		label_color = "tag_g",
		consume = {
			{
				1,
				1,
				3000
			}
		},
		drop_client = {
			{
				2,
				52081,
				1
			},
			{
				2,
				52082,
				1
			},
			{
				2,
				52004,
				1
			}
		}
	},
	[645] = {
		id = 645,
		rarity = 2,
		time = 14400,
		type = 1,
		name = "G-531-MI",
		lv_limit = 30,
		desc = "눈부신 과학 발전을 이룩하기 위해서는 뛰어난 두뇌뿐만 아니라, 스폰서로부터 제공 받는 연구 자금도 빼놓을 수 없다. 남아도는 자금을 기부하여 기술 연구를 지원하자.",
		condition = 0,
		bg = "2",
		blueprint_version = 7,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "gold_2",
		label_color = "tag_g",
		consume = {
			{
				1,
				1,
				6000
			}
		},
		drop_client = {
			{
				2,
				52081,
				1
			},
			{
				2,
				52082,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[646] = {
		id = 646,
		rarity = 2,
		time = 14400,
		type = 1,
		name = "G-531-MI",
		lv_limit = 30,
		desc = "눈부신 과학 발전을 이룩하기 위해서는 뛰어난 두뇌뿐만 아니라, 스폰서로부터 제공 받는 연구 자금도 빼놓을 수 없다. 남아도는 자금을 기부하여 기술 연구를 지원하자.",
		condition = 0,
		bg = "2",
		blueprint_version = 7,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "gold_2",
		label_color = "tag_g",
		consume = {
			{
				1,
				1,
				6000
			}
		},
		drop_client = {
			{
				2,
				52081,
				1
			},
			{
				2,
				52082,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[651] = {
		id = 651,
		rarity = 1,
		time = 9000,
		type = 1,
		name = "D-703-MI",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수 인간의 집합 인식―― 이미지를 구현화한다면, 그것을 원하는 방향으로 「지향」시켜 어떠한 결과를 도출하는 것은 가능할까?",
		condition = 0,
		bg = "1",
		blueprint_version = 7,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "chara_haerfude",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				3000
			}
		},
		drop_client = {
			{
				2,
				42061,
				1
			},
			{
				2,
				52082,
				1
			},
			{
				2,
				52004,
				1
			}
		}
	},
	[652] = {
		id = 652,
		rarity = 1,
		time = 9000,
		type = 1,
		name = "D-758-MI",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수의 사람들의 사물에 대한 인식의 집합체를 구현화하는 것이라면, 그것의 '방향성' 을 정하는 것에 따라 결과는 과연 어떻게 바뀔 것인가.",
		condition = 0,
		bg = "1",
		blueprint_version = 7,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "chara_dashan",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				3000
			}
		},
		drop_client = {
			{
				2,
				42062,
				1
			},
			{
				2,
				52082,
				1
			},
			{
				2,
				52004,
				1
			}
		}
	},
	[653] = {
		id = 653,
		rarity = 1,
		time = 9000,
		type = 1,
		name = "D-766-MI",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수의 사람들의 사물에 대한 인식의 집합체를 구현화하는 것이라면, 그것의 '방향성' 을 정하는 것에 따라 결과는 과연 어떻게 바뀔 것인가.",
		condition = 0,
		bg = "1",
		blueprint_version = 7,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "chara_nabulesi",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				3000
			}
		},
		drop_client = {
			{
				2,
				42063,
				1
			},
			{
				2,
				52082,
				1
			},
			{
				2,
				52004,
				1
			}
		}
	},
	[654] = {
		id = 654,
		rarity = 1,
		time = 9000,
		type = 1,
		name = "D-790-MI",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수의 사람들의 사물에 대한 인식의 집합체를 구현화하는 것이라면, 그것의 '방향성' 을 정하는 것에 따라 결과는 과연 어떻게 바뀔 것인가.",
		condition = 0,
		bg = "1",
		blueprint_version = 7,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "chara_naximofuhaijunshangjiang",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				3000
			}
		},
		drop_client = {
			{
				2,
				42064,
				1
			},
			{
				2,
				52082,
				1
			},
			{
				2,
				52004,
				1
			}
		}
	},
	[655] = {
		id = 655,
		rarity = 1,
		time = 9000,
		type = 1,
		name = "D-797-MI",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수의 사람들의 사물에 대한 인식의 집합체를 구현화하는 것이라면, 그것의 '방향성' 을 정하는 것에 따라 결과는 과연 어떻게 바뀔 것인가.",
		condition = 0,
		bg = "1",
		blueprint_version = 7,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "chara_beiyade",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				3000
			}
		},
		drop_client = {
			{
				2,
				42065,
				1
			},
			{
				2,
				52082,
				1
			},
			{
				2,
				52004,
				1
			}
		}
	},
	[661] = {
		id = 661,
		rarity = 2,
		time = 18000,
		type = 1,
		name = "D-303-MI",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수의 사람들의 사물에 대한 인식의 집합체를 구현화하는 것이라면, 그것의 '방향성' 을 정하는 것에 따라 결과는 과연 어떻게 바뀔 것인가.",
		condition = 0,
		bg = "2",
		blueprint_version = 7,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "chara_haerfude",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				5000
			}
		},
		drop_client = {
			{
				2,
				42061,
				1
			},
			{
				2,
				52082,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[662] = {
		id = 662,
		rarity = 2,
		time = 18000,
		type = 1,
		name = "D-358-MI",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수의 사람들의 사물에 대한 인식의 집합체를 구현화하는 것이라면, 그것의 '방향성' 을 정하는 것에 따라 결과는 과연 어떻게 바뀔 것인가.",
		condition = 0,
		bg = "2",
		blueprint_version = 7,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "chara_dashan",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				5000
			}
		},
		drop_client = {
			{
				2,
				42062,
				1
			},
			{
				2,
				52082,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[663] = {
		id = 663,
		rarity = 2,
		time = 18000,
		type = 1,
		name = "D-366-MI",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수의 사람들의 사물에 대한 인식의 집합체를 구현화하는 것이라면, 그것의 '방향성' 을 정하는 것에 따라 결과는 과연 어떻게 바뀔 것인가.",
		condition = 0,
		bg = "2",
		blueprint_version = 7,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "chara_nabulesi",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				5000
			}
		},
		drop_client = {
			{
				2,
				42063,
				1
			},
			{
				2,
				52082,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[664] = {
		id = 664,
		rarity = 2,
		time = 18000,
		type = 1,
		name = "D-390-MI",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수의 사람들의 사물에 대한 인식의 집합체를 구현화하는 것이라면, 그것의 '방향성' 을 정하는 것에 따라 결과는 과연 어떻게 바뀔 것인가.",
		condition = 0,
		bg = "2",
		blueprint_version = 7,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "chara_naximofuhaijunshangjiang",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				5000
			}
		},
		drop_client = {
			{
				2,
				42064,
				1
			},
			{
				2,
				52082,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[665] = {
		id = 665,
		rarity = 2,
		time = 18000,
		type = 1,
		name = "D-397-MI",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수의 사람들의 사물에 대한 인식의 집합체를 구현화하는 것이라면, 그것의 '방향성' 을 정하는 것에 따라 결과는 과연 어떻게 바뀔 것인가.",
		condition = 0,
		bg = "2",
		blueprint_version = 7,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "chara_beiyade",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				5000
			}
		},
		drop_client = {
			{
				2,
				42065,
				1
			},
			{
				2,
				52082,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[671] = {
		id = 671,
		rarity = 1,
		time = 3600,
		type = 1,
		name = "Q-302-MI",
		lv_limit = 30,
		desc = "의장과 성정 큐브의 공명 현상을 제어할 수 있도록, 갖은 수단을 동원해 해당 현상이 발생하는 상황을 재현하여, 그 현상을 어떻게 해야 가장 효과적으로 이용할 수 있는지 검증한다.",
		condition = 0,
		bg = "1",
		blueprint_version = 7,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_1",
		label_color = "tag_q",
		consume = {
			{
				2,
				17001,
				20
			}
		},
		drop_client = {
			{
				2,
				52082,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[672] = {
		id = 672,
		rarity = 1,
		time = 3600,
		type = 1,
		name = "Q-310-MI",
		lv_limit = 30,
		desc = "의장과 성정 큐브의 공명 현상을 제어할 수 있도록, 갖은 수단을 동원해 해당 현상이 발생하는 상황을 재현하여, 그 현상을 어떻게 해야 가장 효과적으로 이용할 수 있는지 검증한다.",
		condition = 0,
		bg = "1",
		blueprint_version = 7,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_1",
		label_color = "tag_q",
		consume = {
			{
				2,
				17011,
				20
			}
		},
		drop_client = {
			{
				2,
				52082,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[673] = {
		id = 673,
		rarity = 1,
		time = 3600,
		type = 1,
		name = "Q-351-MI",
		lv_limit = 30,
		desc = "의장과 성정 큐브의 공명 현상을 제어할 수 있도록, 갖은 수단을 동원해 해당 현상이 발생하는 상황을 재현하여, 그 현상을 어떻게 해야 가장 효과적으로 이용할 수 있는지 검증한다.",
		condition = 0,
		bg = "1",
		blueprint_version = 7,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_1",
		label_color = "tag_q",
		consume = {
			{
				2,
				17021,
				20
			}
		},
		drop_client = {
			{
				2,
				52082,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[674] = {
		id = 674,
		rarity = 1,
		time = 3600,
		type = 1,
		name = "Q-368-MI",
		lv_limit = 30,
		desc = "의장과 성정 큐브의 공명 현상을 제어할 수 있도록, 갖은 수단을 동원해 해당 현상이 발생하는 상황을 재현하여, 그 현상을 어떻게 해야 가장 효과적으로 이용할 수 있는지 검증한다.",
		condition = 0,
		bg = "1",
		blueprint_version = 7,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_1",
		label_color = "tag_q",
		consume = {
			{
				2,
				17031,
				20
			}
		},
		drop_client = {
			{
				2,
				52082,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[675] = {
		id = 675,
		rarity = 1,
		time = 3600,
		type = 1,
		name = "Q-389-MI",
		lv_limit = 30,
		desc = "의장과 성정 큐브의 공명 현상을 제어할 수 있도록, 갖은 수단을 동원해 해당 현상이 발생하는 상황을 재현하여, 그 현상을 어떻게 해야 가장 효과적으로 이용할 수 있는지 검증한다.",
		condition = 0,
		bg = "1",
		blueprint_version = 7,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_1",
		label_color = "tag_q",
		consume = {
			{
				2,
				17041,
				20
			}
		},
		drop_client = {
			{
				2,
				52082,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[676] = {
		id = 676,
		rarity = 1,
		time = 7200,
		type = 1,
		name = "Q-202-MI",
		lv_limit = 30,
		desc = "의장과 성정 큐브의 공명 현상을 제어할 수 있도록, 갖은 수단을 동원해 해당 현상이 발생하는 상황을 재현하여, 그 현상을 어떻게 해야 가장 효과적으로 이용할 수 있는지 검증한다.",
		condition = 0,
		bg = "1",
		blueprint_version = 7,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_1",
		label_color = "tag_q",
		consume = {
			{
				2,
				17002,
				20
			}
		},
		drop_client = {
			{
				2,
				52082,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[677] = {
		id = 677,
		rarity = 1,
		time = 7200,
		type = 1,
		name = "Q-210-MI",
		lv_limit = 30,
		desc = "의장과 성정 큐브의 공명 현상을 제어할 수 있도록, 갖은 수단을 동원해 해당 현상이 발생하는 상황을 재현하여, 그 현상을 어떻게 해야 가장 효과적으로 이용할 수 있는지 검증한다.",
		condition = 0,
		bg = "1",
		blueprint_version = 7,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_1",
		label_color = "tag_q",
		consume = {
			{
				2,
				17012,
				20
			}
		},
		drop_client = {
			{
				2,
				52082,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[678] = {
		id = 678,
		rarity = 1,
		time = 7200,
		type = 1,
		name = "Q-251-MI",
		lv_limit = 30,
		desc = "의장과 성정 큐브의 공명 현상을 제어할 수 있도록, 갖은 수단을 동원해 해당 현상이 발생하는 상황을 재현하여, 그 현상을 어떻게 해야 가장 효과적으로 이용할 수 있는지 검증한다.",
		condition = 0,
		bg = "1",
		blueprint_version = 7,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_1",
		label_color = "tag_q",
		consume = {
			{
				2,
				17022,
				20
			}
		},
		drop_client = {
			{
				2,
				52082,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[679] = {
		id = 679,
		rarity = 1,
		time = 7200,
		type = 1,
		name = "Q-268-MI",
		lv_limit = 30,
		desc = "의장과 성정 큐브의 공명 현상을 제어할 수 있도록, 갖은 수단을 동원해 해당 현상이 발생하는 상황을 재현하여, 그 현상을 어떻게 해야 가장 효과적으로 이용할 수 있는지 검증한다.",
		condition = 0,
		bg = "1",
		blueprint_version = 7,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_1",
		label_color = "tag_q",
		consume = {
			{
				2,
				17032,
				20
			}
		},
		drop_client = {
			{
				2,
				52082,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[680] = {
		id = 680,
		rarity = 1,
		time = 7200,
		type = 1,
		name = "Q-289-MI",
		lv_limit = 30,
		desc = "의장과 성정 큐브의 공명 현상을 제어할 수 있도록, 갖은 수단을 동원해 해당 현상이 발생하는 상황을 재현하여, 그 현상을 어떻게 해야 가장 효과적으로 이용할 수 있는지 검증한다.",
		condition = 0,
		bg = "1",
		blueprint_version = 7,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_1",
		label_color = "tag_q",
		consume = {
			{
				2,
				17042,
				20
			}
		},
		drop_client = {
			{
				2,
				52082,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[681] = {
		id = 681,
		rarity = 2,
		time = 14400,
		type = 1,
		name = "Q-002-MI",
		lv_limit = 30,
		desc = "의장과 성정 큐브의 공명 현상을 제어할 수 있도록, 갖은 수단을 동원해 해당 현상이 발생하는 상황을 재현하여, 그 현상을 어떻게 해야 가장 효과적으로 이용할 수 있는지 검증한다.",
		condition = 0,
		bg = "2",
		blueprint_version = 7,
		sub_name = "중형 항목",
		label = "tag_blue",
		bg_icon = "gold_2",
		label_color = "tag_q",
		consume = {
			{
				2,
				17003,
				20
			}
		},
		drop_client = {
			{
				2,
				52082,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[682] = {
		id = 682,
		rarity = 2,
		time = 14400,
		type = 1,
		name = "Q-010-MI",
		lv_limit = 30,
		desc = "의장과 성정 큐브의 공명 현상을 제어할 수 있도록, 갖은 수단을 동원해 해당 현상이 발생하는 상황을 재현하여, 그 현상을 어떻게 해야 가장 효과적으로 이용할 수 있는지 검증한다.",
		condition = 0,
		bg = "2",
		blueprint_version = 7,
		sub_name = "중형 항목",
		label = "tag_blue",
		bg_icon = "gold_2",
		label_color = "tag_q",
		consume = {
			{
				2,
				17013,
				20
			}
		},
		drop_client = {
			{
				2,
				52082,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[683] = {
		id = 683,
		rarity = 2,
		time = 14400,
		type = 1,
		name = "Q-051-MI",
		lv_limit = 30,
		desc = "의장과 성정 큐브의 공명 현상을 제어할 수 있도록, 갖은 수단을 동원해 해당 현상이 발생하는 상황을 재현하여, 그 현상을 어떻게 해야 가장 효과적으로 이용할 수 있는지 검증한다.",
		condition = 0,
		bg = "2",
		blueprint_version = 7,
		sub_name = "중형 항목",
		label = "tag_blue",
		bg_icon = "gold_2",
		label_color = "tag_q",
		consume = {
			{
				2,
				17023,
				20
			}
		},
		drop_client = {
			{
				2,
				52082,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[684] = {
		id = 684,
		rarity = 2,
		time = 14400,
		type = 1,
		name = "Q-068-MI",
		lv_limit = 30,
		desc = "의장과 성정 큐브의 공명 현상을 제어할 수 있도록, 갖은 수단을 동원해 해당 현상이 발생하는 상황을 재현하여, 그 현상을 어떻게 해야 가장 효과적으로 이용할 수 있는지 검증한다.",
		condition = 0,
		bg = "2",
		blueprint_version = 7,
		sub_name = "중형 항목",
		label = "tag_blue",
		bg_icon = "gold_2",
		label_color = "tag_q",
		consume = {
			{
				2,
				17033,
				20
			}
		},
		drop_client = {
			{
				2,
				52082,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[685] = {
		id = 685,
		rarity = 2,
		time = 14400,
		type = 1,
		name = "Q-089-MI",
		lv_limit = 30,
		desc = "의장과 성정 큐브의 공명 현상을 제어할 수 있도록, 갖은 수단을 동원해 해당 현상이 발생하는 상황을 재현하여, 그 현상을 어떻게 해야 가장 효과적으로 이용할 수 있는지 검증한다.",
		condition = 0,
		bg = "2",
		blueprint_version = 7,
		sub_name = "중형 항목",
		label = "tag_blue",
		bg_icon = "gold_2",
		label_color = "tag_q",
		consume = {
			{
				2,
				17043,
				20
			}
		},
		drop_client = {
			{
				2,
				52082,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[691] = {
		id = 691,
		rarity = 1,
		time = 3600,
		type = 1,
		name = "H-387-MI",
		lv_limit = 30,
		desc = "성정 큐브를 말 그대로 물리적으로 「분해」하여 구조 분석을 실시한다. 아직 성공 사례는 없지만, 실험해 볼 가치는 충분히 있을 것으로 보인다.",
		condition = 0,
		bg = "1",
		blueprint_version = 7,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "gold_1",
		label_color = "tag_h",
		consume = {
			{
				2,
				20001,
				3
			}
		},
		drop_client = {
			{
				2,
				52081,
				1
			},
			{
				2,
				54049,
				1
			},
			{
				2,
				15008,
				1
			}
		}
	},
	[692] = {
		id = 692,
		rarity = 1,
		time = 3600,
		type = 1,
		name = "H-387-MI",
		lv_limit = 30,
		desc = "성정 큐브를 문자 그대로 물리적으로 '분해'하여, 구조 분석을 진행한다. 성공 사례가 없을수록, 실험의 가치는 그만큼 크다고 볼 수 있다.",
		condition = 0,
		bg = "1",
		blueprint_version = 7,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "gold_1",
		label_color = "tag_h",
		consume = {
			{
				2,
				20001,
				3
			}
		},
		drop_client = {
			{
				2,
				52081,
				1
			},
			{
				2,
				54049,
				1
			},
			{
				2,
				15008,
				1
			}
		}
	},
	[693] = {
		id = 693,
		rarity = 2,
		time = 7200,
		type = 1,
		name = "H-339-MI",
		lv_limit = 30,
		desc = "성정 큐브를 문자 그대로 물리적으로 '분해'하여, 구조 분석을 진행한다. 성공 사례가 없을수록, 실험의 가치는 그만큼 크다고 볼 수 있다.",
		condition = 0,
		bg = "2",
		blueprint_version = 7,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "gold_2",
		label_color = "tag_h",
		consume = {
			{
				2,
				20001,
				6
			}
		},
		drop_client = {
			{
				2,
				52081,
				1
			},
			{
				2,
				54050,
				1
			},
			{
				2,
				15008,
				1
			}
		}
	},
	[694] = {
		id = 694,
		rarity = 2,
		time = 7200,
		type = 1,
		name = "H-339-MI",
		lv_limit = 30,
		desc = "성정 큐브를 문자 그대로 물리적으로 '분해'하여, 구조 분석을 진행한다. 성공 사례가 없을수록, 실험의 가치는 그만큼 크다고 볼 수 있다.",
		condition = 0,
		bg = "2",
		blueprint_version = 7,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "gold_2",
		label_color = "tag_h",
		consume = {
			{
				2,
				20001,
				6
			}
		},
		drop_client = {
			{
				2,
				52081,
				1
			},
			{
				2,
				54050,
				1
			},
			{
				2,
				15008,
				1
			}
		}
	},
	[701] = {
		id = 701,
		rarity = 1,
		time = 21600,
		type = 1,
		name = "C-153-MI",
		lv_limit = 30,
		desc = "성정 큐브에 관한 기초 이론은 큐브의 구조에 대한 인식에서 시작된다. 큐브의 기초 구조에 대한 이론 구축 작업은 여전히 전력으로 이루어지고 있다. ",
		condition = 0,
		bg = "1",
		blueprint_version = 8,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "item_1",
		label_color = "tag_c",
		consume = {},
		drop_client = {
			{
				2,
				52092,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[702] = {
		id = 702,
		rarity = 1,
		time = 21600,
		type = 1,
		name = "C-153-MI",
		lv_limit = 30,
		desc = "성정 큐브에 관한 기초 이론은 큐브의 구조에 대한 인식에서 시작된다. 큐브의 기초 구조에 대한 이론 구축 작업은 여전히 전력으로 이루어지고 있다. ",
		condition = 0,
		bg = "1",
		blueprint_version = 8,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "item_1",
		label_color = "tag_c",
		consume = {},
		drop_client = {
			{
				2,
				52092,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[703] = {
		id = 703,
		rarity = 2,
		time = 28800,
		type = 1,
		name = "C-185-MI",
		lv_limit = 30,
		desc = "성정 큐브에 관한 기초 이론은 큐브의 구조에 대한 인식에서 시작된다. 큐브의 기초 구조에 대한 이론 구축 작업은 여전히 전력으로 이루어지고 있다. ",
		condition = 0,
		bg = "2",
		blueprint_version = 8,
		sub_name = "대형 항목",
		label = "tag_red",
		bg_icon = "item_2",
		label_color = "tag_c",
		consume = {},
		drop_client = {
			{
				2,
				52091,
				1
			},
			{
				2,
				52092,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[704] = {
		id = 704,
		rarity = 2,
		time = 28800,
		type = 1,
		name = "C-185-MI",
		lv_limit = 30,
		desc = "성정 큐브에 관한 기초 이론은 큐브의 구조에 대한 인식에서 시작된다. 큐브의 기초 구조에 대한 이론 구축 작업은 여전히 전력으로 이루어지고 있다. ",
		condition = 0,
		bg = "2",
		blueprint_version = 8,
		sub_name = "대형 항목",
		label = "tag_red",
		bg_icon = "item_2",
		label_color = "tag_c",
		consume = {},
		drop_client = {
			{
				2,
				52091,
				1
			},
			{
				2,
				52092,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[711] = {
		id = 711,
		rarity = 1,
		time = 14400,
		type = 1,
		name = "B-622-MI",
		lv_limit = 30,
		desc = "성정 큐브의 구조에 관한 다양한 가설에 대하여 실증 실험을 실시하는 연구 프로젝트. 실전에서 데이터를 수집하여 아이디어의 과학적 증명에 기여하자. ",
		condition = 52001,
		bg = "1",
		blueprint_version = 8,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "battle_1",
		label_color = "tag_b",
		consume = {},
		drop_client = {
			{
				2,
				52091,
				1
			},
			{
				2,
				52092,
				1
			},
			{
				2,
				52004,
				1
			}
		}
	},
	[712] = {
		id = 712,
		rarity = 1,
		time = 14400,
		type = 1,
		name = "B-636-MI",
		lv_limit = 30,
		desc = "성정 큐브의 구조에 관한 다양한 가설에 대하여 실증 실험을 실시하는 연구 프로젝트. 실전에서 데이터를 수집하여 아이디어의 과학적 증명에 기여하자. ",
		condition = 52002,
		bg = "1",
		blueprint_version = 8,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "battle_1",
		label_color = "tag_b",
		consume = {},
		drop_client = {
			{
				2,
				52091,
				1
			},
			{
				2,
				52092,
				1
			},
			{
				2,
				52004,
				1
			}
		}
	},
	[713] = {
		id = 713,
		rarity = 1,
		time = 14400,
		type = 1,
		name = "B-654-MI",
		lv_limit = 30,
		desc = "성정 큐브의 구조에 관한 다양한 가설에 대하여 실증 실험을 실시하는 연구 프로젝트. 실전에서 데이터를 수집하여 아이디어의 과학적 증명에 기여하자. ",
		condition = 52003,
		bg = "1",
		blueprint_version = 8,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "battle_1",
		label_color = "tag_b",
		consume = {},
		drop_client = {
			{
				2,
				52091,
				1
			},
			{
				2,
				52092,
				1
			},
			{
				2,
				52004,
				1
			}
		}
	},
	[714] = {
		id = 714,
		rarity = 1,
		time = 14400,
		type = 1,
		name = "B-682-MI",
		lv_limit = 40,
		desc = "성정 큐브의 구조에 관한 다양한 가설에 대하여 실증 실험을 실시하는 연구 프로젝트. 실전에서 데이터를 수집하여 아이디어의 과학적 증명에 기여하자. ",
		condition = 52004,
		bg = "1",
		blueprint_version = 8,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "battle_1",
		label_color = "tag_b",
		consume = {},
		drop_client = {
			{
				2,
				52091,
				1
			},
			{
				2,
				52092,
				1
			},
			{
				2,
				52004,
				1
			}
		}
	},
	[715] = {
		id = 715,
		rarity = 2,
		time = 14400,
		type = 1,
		name = "B-235-MI",
		lv_limit = 50,
		desc = "성정 큐브의 구조에 관한 다양한 가설에 대하여 실증 실험을 실시하는 연구 프로젝트. 실전에서 데이터를 수집하여 아이디어의 과학적 증명에 기여하자. ",
		condition = 52005,
		bg = "2",
		blueprint_version = 8,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "battle_2",
		label_color = "tag_b",
		consume = {},
		drop_client = {
			{
				2,
				52091,
				1
			},
			{
				2,
				52092,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[716] = {
		id = 716,
		rarity = 2,
		time = 14400,
		type = 1,
		name = "B-268-MI",
		lv_limit = 60,
		desc = "성정 큐브의 구조에 관한 다양한 가설에 대하여 실증 실험을 실시하는 연구 프로젝트. 실전에서 데이터를 수집하여 아이디어의 과학적 증명에 기여하자. ",
		condition = 52006,
		bg = "2",
		blueprint_version = 8,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "battle_2",
		label_color = "tag_b",
		consume = {},
		drop_client = {
			{
				2,
				52091,
				1
			},
			{
				2,
				52092,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[717] = {
		id = 717,
		rarity = 2,
		time = 14400,
		type = 1,
		name = "B-128-MI",
		lv_limit = 65,
		desc = "성정 큐브의 구조에 관한 다양한 가설에 대하여 실증 실험을 실시하는 연구 프로젝트. 실전에서 데이터를 수집하여 아이디어의 과학적 증명에 기여하자. ",
		condition = 52007,
		bg = "2",
		blueprint_version = 8,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "battle_2",
		label_color = "tag_b",
		consume = {},
		drop_client = {
			{
				2,
				52091,
				1
			},
			{
				2,
				52092,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[718] = {
		id = 718,
		rarity = 2,
		time = 14400,
		type = 1,
		name = "B-164-MI",
		lv_limit = 70,
		desc = "성정 큐브의 구조에 관한 다양한 가설에 대하여 실증 실험을 실시하는 연구 프로젝트. 실전에서 데이터를 수집하여 아이디어의 과학적 증명에 기여하자. ",
		condition = 52008,
		bg = "2",
		blueprint_version = 8,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "battle_2",
		label_color = "tag_b",
		consume = {},
		drop_client = {
			{
				2,
				52091,
				1
			},
			{
				2,
				52092,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[721] = {
		id = 721,
		rarity = 1,
		time = 10800,
		type = 1,
		name = "T-018-MI",
		lv_limit = 30,
		desc = "「건조」 이외에, 성정 큐브의 민간 이용에 관한 연구의 제안도 들어왔다. 기술 전용의 가능성을 모색하는 프로젝트를 위해서는 군의 조력도 필요하다. ",
		condition = 52021,
		bg = "1",
		blueprint_version = 8,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "item_1",
		label_color = "tag_t",
		consume = {},
		drop_client = {
			{
				2,
				52092,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[722] = {
		id = 722,
		rarity = 1,
		time = 14400,
		type = 1,
		name = "T-384-MI",
		lv_limit = 30,
		desc = "「건조」 이외에, 성정 큐브의 민간 이용에 관한 연구의 제안도 들어왔다. 기술 전용의 가능성을 모색하는 프로젝트를 위해서는 군의 조력도 필요하다. ",
		condition = 52022,
		bg = "1",
		blueprint_version = 8,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "item_1",
		label_color = "tag_t",
		consume = {},
		drop_client = {
			{
				2,
				52092,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[723] = {
		id = 723,
		rarity = 2,
		time = 21600,
		type = 1,
		name = "T-249-MI",
		lv_limit = 30,
		desc = "「건조」 이외에, 성정 큐브의 민간 이용에 관한 연구의 제안도 들어왔다. 기술 전용의 가능성을 모색하는 프로젝트를 위해서는 군의 조력도 필요하다. ",
		condition = 52023,
		bg = "2",
		blueprint_version = 8,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "item_2",
		label_color = "tag_t",
		consume = {},
		drop_client = {
			{
				2,
				52092,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[731] = {
		id = 731,
		rarity = 1,
		time = 7200,
		type = 1,
		name = "E-031-MI",
		lv_limit = 30,
		desc = "성정 큐브와 「의장」의 관련성을 조사하는 수많은 시도는 막대한 실험 데이터를 토대로 실시되고 있어. 불필요한 의장을 제공해서 실험 샘플로 돌리자. ",
		condition = 52031,
		bg = "1",
		blueprint_version = 8,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "item_1",
		label_color = "tag_e",
		consume = {},
		drop_client = {
			{
				2,
				52092,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[732] = {
		id = 732,
		rarity = 2,
		time = 7200,
		type = 1,
		name = "E-315-MI",
		lv_limit = 30,
		desc = "성정 큐브와 「의장」의 관련성을 조사하는 수많은 시도는 막대한 실험 데이터를 토대로 실시되고 있어. 불필요한 의장을 제공해서 실험 샘플로 돌리자. ",
		condition = 52032,
		bg = "2",
		blueprint_version = 8,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "item_2",
		label_color = "tag_e",
		consume = {},
		drop_client = {
			{
				2,
				52092,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[741] = {
		id = 741,
		rarity = 1,
		time = 5400,
		type = 1,
		name = "G-412-MI",
		lv_limit = 30,
		desc = "뛰어난 과학의 발전을 지탱하는 것은 우수한 두뇌, 그리고... 지원자로부터 제공되는 연구자금이다. 남은 자금을 기부하여 기술 연구를 응원하자. ",
		condition = 0,
		bg = "1",
		blueprint_version = 8,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "gold_1",
		label_color = "tag_g",
		consume = {
			{
				1,
				1,
				1500
			}
		},
		drop_client = {
			{
				2,
				52091,
				1
			},
			{
				2,
				52092,
				1
			},
			{
				2,
				52004,
				1
			}
		}
	},
	[742] = {
		id = 742,
		rarity = 1,
		time = 5400,
		type = 1,
		name = "G-412-MI",
		lv_limit = 30,
		desc = "뛰어난 과학의 발전을 지탱하는 것은 우수한 두뇌, 그리고... 지원자로부터 제공되는 연구자금이다. 남은 자금을 기부하여 기술 연구를 응원하자. ",
		condition = 0,
		bg = "1",
		blueprint_version = 8,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "gold_1",
		label_color = "tag_g",
		consume = {
			{
				1,
				1,
				1500
			}
		},
		drop_client = {
			{
				2,
				52091,
				1
			},
			{
				2,
				52092,
				1
			},
			{
				2,
				52004,
				1
			}
		}
	},
	[743] = {
		id = 743,
		rarity = 1,
		time = 9000,
		type = 1,
		name = "G-236-MI",
		lv_limit = 30,
		desc = "뛰어난 과학의 발전을 지탱하는 것은 우수한 두뇌, 그리고... 지원자로부터 제공되는 연구자금이다. 남은 자금을 기부하여 기술 연구를 응원하자. ",
		condition = 0,
		bg = "1",
		blueprint_version = 8,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "gold_1",
		label_color = "tag_g",
		consume = {
			{
				1,
				1,
				3000
			}
		},
		drop_client = {
			{
				2,
				52091,
				1
			},
			{
				2,
				52092,
				1
			},
			{
				2,
				52004,
				1
			}
		}
	},
	[744] = {
		id = 744,
		rarity = 1,
		time = 9000,
		type = 1,
		name = "G-236-MI",
		lv_limit = 30,
		desc = "뛰어난 과학의 발전을 지탱하는 것은 우수한 두뇌, 그리고... 지원자로부터 제공되는 연구자금이다. 남은 자금을 기부하여 기술 연구를 응원하자. ",
		condition = 0,
		bg = "1",
		blueprint_version = 8,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "gold_1",
		label_color = "tag_g",
		consume = {
			{
				1,
				1,
				3000
			}
		},
		drop_client = {
			{
				2,
				52091,
				1
			},
			{
				2,
				52092,
				1
			},
			{
				2,
				52004,
				1
			}
		}
	},
	[745] = {
		id = 745,
		rarity = 2,
		time = 14400,
		type = 1,
		name = "G-531-MI",
		lv_limit = 30,
		desc = "뛰어난 과학의 발전을 지탱하는 것은 우수한 두뇌, 그리고... 지원자로부터 제공되는 연구자금이다. 남은 자금을 기부하여 기술 연구를 응원하자. ",
		condition = 0,
		bg = "2",
		blueprint_version = 8,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "gold_2",
		label_color = "tag_g",
		consume = {
			{
				1,
				1,
				6000
			}
		},
		drop_client = {
			{
				2,
				52091,
				1
			},
			{
				2,
				52092,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[746] = {
		id = 746,
		rarity = 2,
		time = 14400,
		type = 1,
		name = "G-531-MI",
		lv_limit = 30,
		desc = "뛰어난 과학의 발전을 지탱하는 것은 우수한 두뇌, 그리고... 지원자로부터 제공되는 연구자금이다. 남은 자금을 기부하여 기술 연구를 응원하자. ",
		condition = 0,
		bg = "2",
		blueprint_version = 8,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "gold_2",
		label_color = "tag_g",
		consume = {
			{
				1,
				1,
				6000
			}
		},
		drop_client = {
			{
				2,
				52091,
				1
			},
			{
				2,
				52092,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[751] = {
		id = 751,
		rarity = 1,
		time = 9000,
		type = 1,
		name = "D-780-MI",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수 인간의 집합인식――이미지를 구현화한다면, 그것을 원하는 방향으로 「지향」시킴으로써, 어떤 결과를 가져올 수 있을까. ",
		condition = 0,
		bg = "1",
		blueprint_version = 8,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "chara_meikelunbao",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				3000
			}
		},
		drop_client = {
			{
				2,
				42072,
				1
			},
			{
				2,
				52092,
				1
			},
			{
				2,
				52004,
				1
			}
		}
	},
	[752] = {
		id = 752,
		rarity = 1,
		time = 9000,
		type = 1,
		name = "D-736-MI",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수 인간의 집합인식――이미지를 구현화한다면, 그것을 원하는 방향으로 「지향」시킴으로써, 어떤 결과를 가져올 수 있을까. ",
		condition = 0,
		bg = "1",
		blueprint_version = 8,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "chara_jinshi",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				3000
			}
		},
		drop_client = {
			{
				2,
				42075,
				1
			},
			{
				2,
				52092,
				1
			},
			{
				2,
				52004,
				1
			}
		}
	},
	[753] = {
		id = 753,
		rarity = 1,
		time = 9000,
		type = 1,
		name = "D-787-MI",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수 인간의 집합인식――이미지를 구현화한다면, 그것을 원하는 방향으로 「지향」시킴으로써, 어떤 결과를 가져올 수 있을까. ",
		condition = 0,
		bg = "1",
		blueprint_version = 8,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "chara_dimiteli",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				3000
			}
		},
		drop_client = {
			{
				2,
				42074,
				1
			},
			{
				2,
				52092,
				1
			},
			{
				2,
				52004,
				1
			}
		}
	},
	[754] = {
		id = 754,
		rarity = 1,
		time = 9000,
		type = 1,
		name = "D-711-MI",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수 인간의 집합인식――이미지를 구현화한다면, 그것을 원하는 방향으로 「지향」시킴으로써, 어떤 결과를 가져올 수 있을까. ",
		condition = 0,
		bg = "1",
		blueprint_version = 8,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "chara_kansasi",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				3000
			}
		},
		drop_client = {
			{
				2,
				42071,
				1
			},
			{
				2,
				52092,
				1
			},
			{
				2,
				52004,
				1
			}
		}
	},
	[755] = {
		id = 755,
		rarity = 1,
		time = 9000,
		type = 1,
		name = "D-764-MI",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수 인간의 집합인식――이미지를 구현화한다면, 그것을 원하는 방향으로 「지향」시킴으로써, 어떤 결과를 가져올 수 있을까. ",
		condition = 0,
		bg = "1",
		blueprint_version = 8,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "chara_kunibeierdi",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				3000
			}
		},
		drop_client = {
			{
				2,
				42073,
				1
			},
			{
				2,
				52092,
				1
			},
			{
				2,
				52004,
				1
			}
		}
	},
	[761] = {
		id = 761,
		rarity = 2,
		time = 18000,
		type = 1,
		name = "D-380-MI",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수 인간의 집합인식――이미지를 구현화한다면, 그것을 원하는 방향으로 「지향」시킴으로써, 어떤 결과를 가져올 수 있을까. ",
		condition = 0,
		bg = "2",
		blueprint_version = 8,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "chara_meikelunbao",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				5000
			}
		},
		drop_client = {
			{
				2,
				42072,
				1
			},
			{
				2,
				52092,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[762] = {
		id = 762,
		rarity = 2,
		time = 18000,
		type = 1,
		name = "D-336-MI",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수 인간의 집합인식――이미지를 구현화한다면, 그것을 원하는 방향으로 「지향」시킴으로써, 어떤 결과를 가져올 수 있을까. ",
		condition = 0,
		bg = "2",
		blueprint_version = 8,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "chara_jinshi",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				5000
			}
		},
		drop_client = {
			{
				2,
				42075,
				1
			},
			{
				2,
				52092,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[763] = {
		id = 763,
		rarity = 2,
		time = 18000,
		type = 1,
		name = "D-387-MI",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수 인간의 집합인식――이미지를 구현화한다면, 그것을 원하는 방향으로 「지향」시킴으로써, 어떤 결과를 가져올 수 있을까. ",
		condition = 0,
		bg = "2",
		blueprint_version = 8,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "chara_dimiteli",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				5000
			}
		},
		drop_client = {
			{
				2,
				42074,
				1
			},
			{
				2,
				52092,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[764] = {
		id = 764,
		rarity = 2,
		time = 18000,
		type = 1,
		name = "D-311-MI",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수 인간의 집합인식――이미지를 구현화한다면, 그것을 원하는 방향으로 「지향」시킴으로써, 어떤 결과를 가져올 수 있을까. ",
		condition = 0,
		bg = "2",
		blueprint_version = 8,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "chara_kansasi",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				5000
			}
		},
		drop_client = {
			{
				2,
				42071,
				1
			},
			{
				2,
				52092,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[765] = {
		id = 765,
		rarity = 2,
		time = 18000,
		type = 1,
		name = "D-364-MI",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수 인간의 집합인식――이미지를 구현화한다면, 그것을 원하는 방향으로 「지향」시킴으로써, 어떤 결과를 가져올 수 있을까. ",
		condition = 0,
		bg = "2",
		blueprint_version = 8,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "chara_kunibeierdi",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				5000
			}
		},
		drop_client = {
			{
				2,
				42073,
				1
			},
			{
				2,
				52092,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[771] = {
		id = 771,
		rarity = 1,
		time = 3600,
		type = 1,
		name = "Q-302-MI",
		lv_limit = 30,
		desc = "의장과 성정 큐브의 공명 현상 제어 가능하도록, 모든 수단으로 해당 현상이 발생하는 장면을 재현하여, 이 현상을 가장 효과적으로 이용할 수 있는 장면을 검토한다. ",
		condition = 0,
		bg = "1",
		blueprint_version = 8,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_1",
		label_color = "tag_q",
		consume = {
			{
				2,
				17001,
				20
			}
		},
		drop_client = {
			{
				2,
				52092,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[772] = {
		id = 772,
		rarity = 1,
		time = 3600,
		type = 1,
		name = "Q-310-MI",
		lv_limit = 30,
		desc = "의장과 성정 큐브의 공명 현상 제어 가능하도록, 모든 수단으로 해당 현상이 발생하는 장면을 재현하여, 이 현상을 가장 효과적으로 이용할 수 있는 장면을 검토한다. ",
		condition = 0,
		bg = "1",
		blueprint_version = 8,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_1",
		label_color = "tag_q",
		consume = {
			{
				2,
				17011,
				20
			}
		},
		drop_client = {
			{
				2,
				52092,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[773] = {
		id = 773,
		rarity = 1,
		time = 3600,
		type = 1,
		name = "Q-351-MI",
		lv_limit = 30,
		desc = "의장과 성정 큐브의 공명 현상 제어 가능하도록, 모든 수단으로 해당 현상이 발생하는 장면을 재현하여, 이 현상을 가장 효과적으로 이용할 수 있는 장면을 검토한다. ",
		condition = 0,
		bg = "1",
		blueprint_version = 8,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_1",
		label_color = "tag_q",
		consume = {
			{
				2,
				17021,
				20
			}
		},
		drop_client = {
			{
				2,
				52092,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[774] = {
		id = 774,
		rarity = 1,
		time = 3600,
		type = 1,
		name = "Q-368-MI",
		lv_limit = 30,
		desc = "의장과 성정 큐브의 공명 현상 제어 가능하도록, 모든 수단으로 해당 현상이 발생하는 장면을 재현하여, 이 현상을 가장 효과적으로 이용할 수 있는 장면을 검토한다. ",
		condition = 0,
		bg = "1",
		blueprint_version = 8,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_1",
		label_color = "tag_q",
		consume = {
			{
				2,
				17031,
				20
			}
		},
		drop_client = {
			{
				2,
				52092,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[775] = {
		id = 775,
		rarity = 1,
		time = 3600,
		type = 1,
		name = "Q-389-MI",
		lv_limit = 30,
		desc = "의장과 성정 큐브의 공명 현상 제어 가능하도록, 모든 수단으로 해당 현상이 발생하는 장면을 재현하여, 이 현상을 가장 효과적으로 이용할 수 있는 장면을 검토한다. ",
		condition = 0,
		bg = "1",
		blueprint_version = 8,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_1",
		label_color = "tag_q",
		consume = {
			{
				2,
				17041,
				20
			}
		},
		drop_client = {
			{
				2,
				52092,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[776] = {
		id = 776,
		rarity = 1,
		time = 7200,
		type = 1,
		name = "Q-202-MI",
		lv_limit = 30,
		desc = "의장과 성정 큐브의 공명 현상 제어 가능하도록, 모든 수단으로 해당 현상이 발생하는 장면을 재현하여, 이 현상을 가장 효과적으로 이용할 수 있는 장면을 검토한다. ",
		condition = 0,
		bg = "1",
		blueprint_version = 8,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_1",
		label_color = "tag_q",
		consume = {
			{
				2,
				17002,
				20
			}
		},
		drop_client = {
			{
				2,
				52092,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[777] = {
		id = 777,
		rarity = 1,
		time = 7200,
		type = 1,
		name = "Q-210-MI",
		lv_limit = 30,
		desc = "의장과 성정 큐브의 공명 현상 제어 가능하도록, 모든 수단으로 해당 현상이 발생하는 장면을 재현하여, 이 현상을 가장 효과적으로 이용할 수 있는 장면을 검토한다. ",
		condition = 0,
		bg = "1",
		blueprint_version = 8,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_1",
		label_color = "tag_q",
		consume = {
			{
				2,
				17012,
				20
			}
		},
		drop_client = {
			{
				2,
				52092,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[778] = {
		id = 778,
		rarity = 1,
		time = 7200,
		type = 1,
		name = "Q-251-MI",
		lv_limit = 30,
		desc = "의장과 성정 큐브의 공명 현상 제어 가능하도록, 모든 수단으로 해당 현상이 발생하는 장면을 재현하여, 이 현상을 가장 효과적으로 이용할 수 있는 장면을 검토한다. ",
		condition = 0,
		bg = "1",
		blueprint_version = 8,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_1",
		label_color = "tag_q",
		consume = {
			{
				2,
				17022,
				20
			}
		},
		drop_client = {
			{
				2,
				52092,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[779] = {
		id = 779,
		rarity = 1,
		time = 7200,
		type = 1,
		name = "Q-268-MI",
		lv_limit = 30,
		desc = "의장과 성정 큐브의 공명 현상 제어 가능하도록, 모든 수단으로 해당 현상이 발생하는 장면을 재현하여, 이 현상을 가장 효과적으로 이용할 수 있는 장면을 검토한다. ",
		condition = 0,
		bg = "1",
		blueprint_version = 8,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_1",
		label_color = "tag_q",
		consume = {
			{
				2,
				17032,
				20
			}
		},
		drop_client = {
			{
				2,
				52092,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[780] = {
		id = 780,
		rarity = 1,
		time = 7200,
		type = 1,
		name = "Q-289-MI",
		lv_limit = 30,
		desc = "의장과 성정 큐브의 공명 현상 제어 가능하도록, 모든 수단으로 해당 현상이 발생하는 장면을 재현하여, 이 현상을 가장 효과적으로 이용할 수 있는 장면을 검토한다. ",
		condition = 0,
		bg = "1",
		blueprint_version = 8,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_1",
		label_color = "tag_q",
		consume = {
			{
				2,
				17042,
				20
			}
		},
		drop_client = {
			{
				2,
				52092,
				1
			},
			{
				2,
				52004,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[781] = {
		id = 781,
		rarity = 2,
		time = 14400,
		type = 1,
		name = "Q-002-MI",
		lv_limit = 30,
		desc = "의장과 성정 큐브의 공명 현상 제어 가능하도록, 모든 수단으로 해당 현상이 발생하는 장면을 재현하여, 이 현상을 가장 효과적으로 이용할 수 있는 장면을 검토한다. ",
		condition = 0,
		bg = "2",
		blueprint_version = 8,
		sub_name = "중형 항목",
		label = "tag_blue",
		bg_icon = "gold_2",
		label_color = "tag_q",
		consume = {
			{
				2,
				17003,
				20
			}
		},
		drop_client = {
			{
				2,
				52092,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[782] = {
		id = 782,
		rarity = 2,
		time = 14400,
		type = 1,
		name = "Q-010-MI",
		lv_limit = 30,
		desc = "의장과 성정 큐브의 공명 현상 제어 가능하도록, 모든 수단으로 해당 현상이 발생하는 장면을 재현하여, 이 현상을 가장 효과적으로 이용할 수 있는 장면을 검토한다. ",
		condition = 0,
		bg = "2",
		blueprint_version = 8,
		sub_name = "중형 항목",
		label = "tag_blue",
		bg_icon = "gold_2",
		label_color = "tag_q",
		consume = {
			{
				2,
				17013,
				20
			}
		},
		drop_client = {
			{
				2,
				52092,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[783] = {
		id = 783,
		rarity = 2,
		time = 14400,
		type = 1,
		name = "Q-051-MI",
		lv_limit = 30,
		desc = "의장과 성정 큐브의 공명 현상 제어 가능하도록, 모든 수단으로 해당 현상이 발생하는 장면을 재현하여, 이 현상을 가장 효과적으로 이용할 수 있는 장면을 검토한다. ",
		condition = 0,
		bg = "2",
		blueprint_version = 8,
		sub_name = "중형 항목",
		label = "tag_blue",
		bg_icon = "gold_2",
		label_color = "tag_q",
		consume = {
			{
				2,
				17023,
				20
			}
		},
		drop_client = {
			{
				2,
				52092,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[784] = {
		id = 784,
		rarity = 2,
		time = 14400,
		type = 1,
		name = "Q-068-MI",
		lv_limit = 30,
		desc = "의장과 성정 큐브의 공명 현상 제어 가능하도록, 모든 수단으로 해당 현상이 발생하는 장면을 재현하여, 이 현상을 가장 효과적으로 이용할 수 있는 장면을 검토한다. ",
		condition = 0,
		bg = "2",
		blueprint_version = 8,
		sub_name = "중형 항목",
		label = "tag_blue",
		bg_icon = "gold_2",
		label_color = "tag_q",
		consume = {
			{
				2,
				17033,
				20
			}
		},
		drop_client = {
			{
				2,
				52092,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[785] = {
		id = 785,
		rarity = 2,
		time = 14400,
		type = 1,
		name = "Q-089-MI",
		lv_limit = 30,
		desc = "의장과 성정 큐브의 공명 현상 제어 가능하도록, 모든 수단으로 해당 현상이 발생하는 장면을 재현하여, 이 현상을 가장 효과적으로 이용할 수 있는 장면을 검토한다. ",
		condition = 0,
		bg = "2",
		blueprint_version = 8,
		sub_name = "중형 항목",
		label = "tag_blue",
		bg_icon = "gold_2",
		label_color = "tag_q",
		consume = {
			{
				2,
				17043,
				20
			}
		},
		drop_client = {
			{
				2,
				52092,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[791] = {
		id = 791,
		rarity = 1,
		time = 3600,
		type = 1,
		name = "H-387-MI",
		lv_limit = 30,
		desc = "성정 큐브를 문자 그대로 물리적으로 「분해」하여, 구조를 분석한다. 성공 사례는 없으나 실험할 가치는 다분히 있다고 생각된다. ",
		condition = 0,
		bg = "1",
		blueprint_version = 8,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "gold_1",
		label_color = "tag_h",
		consume = {
			{
				2,
				20001,
				3
			}
		},
		drop_client = {
			{
				2,
				52091,
				1
			},
			{
				2,
				54049,
				1
			},
			{
				2,
				15008,
				1
			}
		}
	},
	[792] = {
		id = 792,
		rarity = 1,
		time = 3600,
		type = 1,
		name = "H-387-MI",
		lv_limit = 30,
		desc = "성정 큐브를 문자 그대로 물리적으로 「분해」하여, 구조를 분석한다. 성공 사례는 없으나 실험할 가치는 다분히 있다고 생각된다. ",
		condition = 0,
		bg = "1",
		blueprint_version = 8,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "gold_1",
		label_color = "tag_h",
		consume = {
			{
				2,
				20001,
				3
			}
		},
		drop_client = {
			{
				2,
				52091,
				1
			},
			{
				2,
				54049,
				1
			},
			{
				2,
				15008,
				1
			}
		}
	},
	[793] = {
		id = 793,
		rarity = 2,
		time = 7200,
		type = 1,
		name = "H-339-MI",
		lv_limit = 30,
		desc = "성정 큐브를 문자 그대로 물리적으로 「분해」하여, 구조를 분석한다. 성공 사례는 없으나 실험할 가치는 다분히 있다고 생각된다. ",
		condition = 0,
		bg = "2",
		blueprint_version = 8,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "gold_2",
		label_color = "tag_h",
		consume = {
			{
				2,
				20001,
				6
			}
		},
		drop_client = {
			{
				2,
				52091,
				1
			},
			{
				2,
				54050,
				1
			},
			{
				2,
				15008,
				1
			}
		}
	},
	[794] = {
		id = 794,
		rarity = 2,
		time = 7200,
		type = 1,
		name = "H-339-MI",
		lv_limit = 30,
		desc = "성정 큐브를 문자 그대로 물리적으로 「분해」하여, 구조를 분석한다. 성공 사례는 없으나 실험할 가치는 다분히 있다고 생각된다. ",
		condition = 0,
		bg = "2",
		blueprint_version = 8,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "gold_2",
		label_color = "tag_h",
		consume = {
			{
				2,
				20001,
				6
			}
		},
		drop_client = {
			{
				2,
				52091,
				1
			},
			{
				2,
				54050,
				1
			},
			{
				2,
				15008,
				1
			}
		}
	},
	[1001] = {
		id = 1001,
		rarity = 3,
		time = 43200,
		type = 2,
		name = "기초 연구",
		lv_limit = 30,
		desc = "성정 큐브에 관한 기초 이론은 큐브 구조에 대한 인식으로부터 시작된다. 큐브 기초 구조에 대한 이론 구조 작업은 여전히 전력을 다해 진행 중이다.",
		condition = 0,
		bg = "3",
		blueprint_version = 1,
		sub_name = "대형 항목",
		label = "tag_red",
		bg_icon = "item_3",
		label_color = "tag_c",
		consume = {},
		drop_client = {
			{
				2,
				52001,
				1
			},
			{
				2,
				52002,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[1002] = {
		id = 1002,
		rarity = 3,
		time = 43200,
		type = 2,
		name = "기초 연구",
		lv_limit = 30,
		desc = "성정 큐브에 관한 기초 이론은 큐브 구조에 대한 인식으로부터 시작된다. 큐브 기초 구조에 대한 이론 구조 작업은 여전히 전력을 다해 진행 중이다.",
		condition = 0,
		bg = "3",
		blueprint_version = 1,
		sub_name = "대형 항목",
		label = "tag_red",
		bg_icon = "item_3",
		label_color = "tag_c",
		consume = {},
		drop_client = {
			{
				2,
				52001,
				1
			},
			{
				2,
				52002,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[1003] = {
		id = 1003,
		rarity = 3,
		time = 14400,
		type = 2,
		name = "데이터 수집",
		lv_limit = 75,
		desc = "성정 큐브 구조에 관한 여러가지 가설에 대해 실증실험을 진행하는 연구 프로젝트. 실전을 통해 데이터를 수집하여, 이론의 과학적 증명에 기여하자.",
		condition = 52009,
		bg = "3",
		blueprint_version = 1,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "battle_3",
		label_color = "tag_b",
		consume = {},
		drop_client = {
			{
				2,
				52001,
				1
			},
			{
				2,
				52002,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1004] = {
		id = 1004,
		rarity = 3,
		time = 14400,
		type = 2,
		name = "데이터 수집",
		lv_limit = 80,
		desc = "성정 큐브 구조에 관한 여러가지 가설에 대해 실증실험을 진행하는 연구 프로젝트. 실전을 통해 데이터를 수집하여, 이론의 과학적 증명에 기여하자.",
		condition = 52010,
		bg = "3",
		blueprint_version = 1,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "battle_3",
		label_color = "tag_b",
		consume = {},
		drop_client = {
			{
				2,
				52001,
				1
			},
			{
				2,
				52002,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1005] = {
		id = 1005,
		rarity = 3,
		time = 28800,
		type = 2,
		name = "인식 방향성 연구",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수의 사람들의 사물에 대한 인식의 집합체를 구현화하는 것이라면, 그것의 '방향성' 을 정하는 것에 따라 결과는 과연 어떻게 바뀔 것인가.",
		condition = 0,
		bg = "3",
		blueprint_version = 1,
		sub_name = "대형 항목",
		label = "tag_red",
		bg_icon = "chara_haiwangxing",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				8000
			}
		},
		drop_client = {
			{
				2,
				42001,
				1
			},
			{
				2,
				52002,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1006] = {
		id = 1006,
		rarity = 3,
		time = 28800,
		type = 2,
		name = "인식 방향성 연구",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수의 사람들의 사물에 대한 인식의 집합체를 구현화하는 것이라면, 그것의 '방향성' 을 정하는 것에 따라 결과는 과연 어떻게 바뀔 것인가.",
		condition = 0,
		bg = "3",
		blueprint_version = 1,
		sub_name = "대형 항목",
		label = "tag_red",
		bg_icon = "chara_junzhu",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				8000
			}
		},
		drop_client = {
			{
				2,
				42002,
				1
			},
			{
				2,
				52002,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1007] = {
		id = 1007,
		rarity = 3,
		time = 28800,
		type = 2,
		name = "인식 방향성 연구",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수의 사람들의 사물에 대한 인식의 집합체를 구현화하는 것이라면, 그것의 '방향성' 을 정하는 것에 따라 결과는 과연 어떻게 바뀔 것인가.",
		condition = 0,
		bg = "3",
		blueprint_version = 1,
		sub_name = "대형 항목",
		label = "tag_red",
		bg_icon = "chara_yichui",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				8000
			}
		},
		drop_client = {
			{
				2,
				42003,
				1
			},
			{
				2,
				52002,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1008] = {
		id = 1008,
		rarity = 3,
		time = 28800,
		type = 2,
		name = "인식 방향성 연구",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수의 사람들의 사물에 대한 인식의 집합체를 구현화하는 것이라면, 그것의 '방향성' 을 정하는 것에 따라 결과는 과연 어떻게 바뀔 것인가.",
		condition = 0,
		bg = "3",
		blueprint_version = 1,
		sub_name = "대형 항목",
		label = "tag_red",
		bg_icon = "chara_chuyun",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				8000
			}
		},
		drop_client = {
			{
				2,
				42004,
				1
			},
			{
				2,
				52002,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1009] = {
		id = 1009,
		rarity = 3,
		time = 28800,
		type = 2,
		name = "인식 방향성 연구",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수의 사람들의 사물에 대한 인식의 집합체를 구현화하는 것이라면, 그것의 '방향성' 을 정하는 것에 따라 결과는 과연 어떻게 바뀔 것인가.",
		condition = 0,
		bg = "3",
		blueprint_version = 1,
		sub_name = "대형 항목",
		label = "tag_red",
		bg_icon = "chara_luoen",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				8000
			}
		},
		drop_client = {
			{
				2,
				42005,
				1
			},
			{
				2,
				52002,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1010] = {
		id = 1010,
		rarity = 3,
		time = 28800,
		type = 2,
		name = "인식 방향성 연구",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수의 사람들의 사물에 대한 인식의 집합체를 구현화하는 것이라면, 그것의 '방향성' 을 정하는 것에 따라 결과는 과연 어떻게 바뀔 것인가.",
		condition = 0,
		bg = "3",
		blueprint_version = 1,
		sub_name = "대형 항목",
		label = "tag_red",
		bg_icon = "chara_luyijiushi",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				8000
			}
		},
		drop_client = {
			{
				2,
				42006,
				1
			},
			{
				2,
				52002,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1011] = {
		id = 1011,
		rarity = 3,
		time = 14400,
		type = 2,
		name = "큐브 구조 해석",
		lv_limit = 30,
		desc = "성정 큐브를 문자 그대로 물리적으로 '분해'하여, 구조 분석을 진행한다. 성공 사례가 없을수록, 실험의 가치는 그만큼 크다고 볼 수 있다.",
		condition = 0,
		bg = "3",
		blueprint_version = 1,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "gold_3",
		label_color = "tag_h",
		consume = {
			{
				2,
				20001,
				10
			}
		},
		drop_client = {
			{
				2,
				52001,
				1
			},
			{
				2,
				54051,
				1
			},
			{
				2,
				15008,
				1
			}
		}
	},
	[1012] = {
		id = 1012,
		rarity = 3,
		time = 14400,
		type = 2,
		name = "큐브 구조 해석",
		lv_limit = 30,
		desc = "성정 큐브를 문자 그대로 물리적으로 '분해'하여, 구조 분석을 진행한다. 성공 사례가 없을수록, 실험의 가치는 그만큼 크다고 볼 수 있다.",
		condition = 0,
		bg = "3",
		blueprint_version = 1,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "gold_3",
		label_color = "tag_h",
		consume = {
			{
				2,
				20001,
				10
			}
		},
		drop_client = {
			{
				2,
				52001,
				1
			},
			{
				2,
				54051,
				1
			},
			{
				2,
				15008,
				1
			}
		}
	},
	[1013] = {
		id = 1013,
		rarity = 3,
		time = 1800,
		type = 2,
		name = "긴급 : 인식 방향성 연구",
		lv_limit = 30,
		desc = "성정 큐브의 인식 구현화 실험 중, 이산치가 폭증하는 이상 현상이 발생하였다. 원리를 밝혀내면, 성정 큐브에 관한 이론 구축에 큰 진전이 있을 것이다.",
		condition = 0,
		bg = "3",
		blueprint_version = 1,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "chara_haiwangxing",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				5000
			},
			{
				2,
				20001,
				5
			}
		},
		drop_client = {
			{
				2,
				42001,
				1
			},
			{
				2,
				52002,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1014] = {
		id = 1014,
		rarity = 3,
		time = 1800,
		type = 2,
		name = "긴급 : 인식 방향성 연구",
		lv_limit = 30,
		desc = "성정 큐브의 인식 구현화 실험 중, 이산치가 폭증하는 이상 현상이 발생하였다. 원리를 밝혀내면, 성정 큐브에 관한 이론 구축에 큰 진전이 있을 것이다.",
		condition = 0,
		bg = "3",
		blueprint_version = 1,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "chara_junzhu",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				5000
			},
			{
				2,
				20001,
				5
			}
		},
		drop_client = {
			{
				2,
				42002,
				1
			},
			{
				2,
				52002,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1015] = {
		id = 1015,
		rarity = 3,
		time = 1800,
		type = 2,
		name = "긴급 : 인식 방향성 연구",
		lv_limit = 30,
		desc = "성정 큐브의 인식 구현화 실험 중, 이산치가 폭증하는 이상 현상이 발생하였다. 원리를 밝혀내면, 성정 큐브에 관한 이론 구축에 큰 진전이 있을 것이다.",
		condition = 0,
		bg = "3",
		blueprint_version = 1,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "chara_yichui",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				5000
			},
			{
				2,
				20001,
				5
			}
		},
		drop_client = {
			{
				2,
				42003,
				1
			},
			{
				2,
				52002,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1016] = {
		id = 1016,
		rarity = 3,
		time = 1800,
		type = 2,
		name = "긴급 : 인식 방향성 연구",
		lv_limit = 30,
		desc = "성정 큐브의 인식 구현화 실험 중, 이산치가 폭증하는 이상 현상이 발생하였다. 원리를 밝혀내면, 성정 큐브에 관한 이론 구축에 큰 진전이 있을 것이다.",
		condition = 0,
		bg = "3",
		blueprint_version = 1,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "chara_chuyun",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				5000
			},
			{
				2,
				20001,
				5
			}
		},
		drop_client = {
			{
				2,
				42004,
				1
			},
			{
				2,
				52002,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1017] = {
		id = 1017,
		rarity = 3,
		time = 1800,
		type = 2,
		name = "긴급 : 인식 방향성 연구",
		lv_limit = 30,
		desc = "성정 큐브의 인식 구현화 실험 중, 이산치가 폭증하는 이상 현상이 발생하였다. 원리를 밝혀내면, 성정 큐브에 관한 이론 구축에 큰 진전이 있을 것이다.",
		condition = 0,
		bg = "3",
		blueprint_version = 1,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "chara_luoen",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				5000
			},
			{
				2,
				20001,
				5
			}
		},
		drop_client = {
			{
				2,
				42005,
				1
			},
			{
				2,
				52002,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1018] = {
		id = 1018,
		rarity = 3,
		time = 1800,
		type = 2,
		name = "긴급 : 인식 방향성 연구",
		lv_limit = 30,
		desc = "성정 큐브의 인식 구현화 실험 중, 이산치가 폭증하는 이상 현상이 발생하였다. 원리를 밝혀내면, 성정 큐브에 관한 이론 구축에 큰 진전이 있을 것이다.",
		condition = 0,
		bg = "3",
		blueprint_version = 1,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "chara_luyijiushi",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				5000
			},
			{
				2,
				20001,
				5
			}
		},
		drop_client = {
			{
				2,
				42006,
				1
			},
			{
				2,
				52002,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1019] = {
		id = 1019,
		rarity = 3,
		time = 1800,
		type = 2,
		name = "긴급 : 장비 해석",
		lv_limit = 30,
		desc = "성정 큐브와 장비의 공명 실험에서, 특정 부품이 매우 높은 싱크로율을 보였다. 분명 지금까지 밝히지 못한 미지의 기술이 담겨 있을 것이다.",
		condition = 0,
		bg = "3",
		blueprint_version = 1,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_3",
		label_color = "tag_q",
		consume = {
			{
				1,
				1,
				5000
			},
			{
				2,
				17003,
				20
			}
		},
		drop_client = {
			{
				2,
				52002,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1020] = {
		id = 1020,
		rarity = 3,
		time = 1800,
		type = 2,
		name = "긴급 : 장비 해석",
		lv_limit = 30,
		desc = "성정 큐브와 장비의 공명 실험에서, 특정 부품이 매우 높은 싱크로율을 보였다. 분명 지금까지 밝히지 못한 미지의 기술이 담겨 있을 것이다.",
		condition = 0,
		bg = "3",
		blueprint_version = 1,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_3",
		label_color = "tag_q",
		consume = {
			{
				1,
				1,
				5000
			},
			{
				2,
				17013,
				20
			}
		},
		drop_client = {
			{
				2,
				52002,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1021] = {
		id = 1021,
		rarity = 3,
		time = 1800,
		type = 2,
		name = "긴급 : 장비 해석",
		lv_limit = 30,
		desc = "성정 큐브와 장비의 공명 실험에서, 특정 부품이 매우 높은 싱크로율을 보였다. 분명 지금까지 밝히지 못한 미지의 기술이 담겨 있을 것이다.",
		condition = 0,
		bg = "3",
		blueprint_version = 1,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_3",
		label_color = "tag_q",
		consume = {
			{
				1,
				1,
				5000
			},
			{
				2,
				17023,
				20
			}
		},
		drop_client = {
			{
				2,
				52002,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1022] = {
		id = 1022,
		rarity = 3,
		time = 1800,
		type = 2,
		name = "긴급 : 장비 해석",
		lv_limit = 30,
		desc = "성정 큐브와 장비의 공명 실험에서, 특정 부품이 매우 높은 싱크로율을 보였다. 분명 지금까지 밝히지 못한 미지의 기술이 담겨 있을 것이다.",
		condition = 0,
		bg = "3",
		blueprint_version = 1,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_3",
		label_color = "tag_q",
		consume = {
			{
				1,
				1,
				5000
			},
			{
				2,
				17033,
				20
			}
		},
		drop_client = {
			{
				2,
				52002,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1023] = {
		id = 1023,
		rarity = 3,
		time = 1800,
		type = 2,
		name = "긴급 : 장비 해석",
		lv_limit = 30,
		desc = "성정 큐브와 장비의 공명 실험에서, 특정 부품이 매우 높은 싱크로율을 보였다. 분명 지금까지 밝히지 못한 미지의 기술이 담겨 있을 것이다.",
		condition = 0,
		bg = "3",
		blueprint_version = 1,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_3",
		label_color = "tag_q",
		consume = {
			{
				1,
				1,
				5000
			},
			{
				2,
				17043,
				20
			}
		},
		drop_client = {
			{
				2,
				52002,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1024] = {
		id = 1024,
		rarity = 3,
		time = 1800,
		type = 2,
		name = "긴급 : 큐브 구조 해석",
		lv_limit = 30,
		desc = "상부에서 극비사항으로 분류되는 연구 프로젝트. 이로 인해 인류뿐만 아니라, 전세계의 생태계 자체에 상당한 영향이 갈지도 모른다.",
		condition = 0,
		bg = "3",
		blueprint_version = 1,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_3",
		label_color = "tag_sh",
		consume = {
			{
				1,
				1,
				8000
			},
			{
				2,
				20001,
				3
			}
		},
		drop_client = {
			{
				2,
				52001,
				1
			},
			{
				2,
				54051,
				1
			},
			{
				2,
				15008,
				1
			}
		}
	},
	[1025] = {
		id = 1025,
		rarity = 3,
		time = 1800,
		type = 2,
		name = "긴급 : 큐브 구조 해석",
		lv_limit = 30,
		desc = "상부에서 극비사항으로 분류되는 연구 프로젝트. 이로 인해 인류뿐만 아니라, 전세계의 생태계 자체에 상당한 영향이 갈지도 모른다.",
		condition = 0,
		bg = "3",
		blueprint_version = 1,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_3",
		label_color = "tag_sh",
		consume = {
			{
				1,
				1,
				8000
			},
			{
				2,
				20001,
				3
			}
		},
		drop_client = {
			{
				2,
				52001,
				1
			},
			{
				2,
				54051,
				1
			},
			{
				2,
				15008,
				1
			}
		}
	},
	[1026] = {
		id = 1026,
		rarity = 3,
		time = 14400,
		type = 2,
		name = "데이터 수집",
		lv_limit = 85,
		desc = "성정 큐브 구조에 관한 여러가지 가설에 대해 실증실험을 진행하는 연구 프로젝트. 실전을 통해 데이터를 수집하여, 이론의 과학적 증명에 기여하자.",
		condition = 52011,
		bg = "3",
		blueprint_version = 1,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "battle_3",
		label_color = "tag_b",
		consume = {},
		drop_client = {
			{
				2,
				52001,
				1
			},
			{
				2,
				52002,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1031] = {
		id = 1031,
		rarity = 3,
		time = 7200,
		type = 2,
		name = "E-831-MI",
		lv_limit = 80,
		desc = "현재 비치 중인 장비를 분해함으로써 기초 설계를 재검토하여, 추후 개발이 예정된 새 장비의 기초 연구를 진행한다. ",
		condition = 0,
		bg = "3",
		blueprint_version = 1,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "item_3",
		label_color = "tag_e",
		consume = {
			{
				2,
				10040,
				45
			}
		},
		drop_client = {
			{
				2,
				13001,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[1032] = {
		id = 1032,
		rarity = 3,
		time = 7200,
		type = 2,
		name = "E-859-MI",
		lv_limit = 80,
		desc = "현재 비치 중인 장비를 분해함으로써 기초 설계를 재검토하여, 추후 개발이 예정된 새 장비의 기초 연구를 진행한다. ",
		condition = 0,
		bg = "3",
		blueprint_version = 1,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "item_3",
		label_color = "tag_e",
		consume = {
			{
				2,
				10057,
				45
			}
		},
		drop_client = {
			{
				2,
				13003,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[1033] = {
		id = 1033,
		rarity = 3,
		time = 7200,
		type = 2,
		name = "E-874-MI",
		lv_limit = 80,
		desc = "현재 비치 중인 장비를 분해함으로써 기초 설계를 재검토하여, 추후 개발이 예정된 새 장비의 기초 연구를 진행한다. ",
		condition = 0,
		bg = "3",
		blueprint_version = 1,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "item_3",
		label_color = "tag_e",
		consume = {
			{
				2,
				10074,
				45
			}
		},
		drop_client = {
			{
				2,
				13004,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[1034] = {
		id = 1034,
		rarity = 3,
		time = 7200,
		type = 2,
		name = "E-892-MI",
		lv_limit = 80,
		desc = "현재 비치 중인 장비를 분해함으로써 기초 설계를 재검토하여, 추후 개발이 예정된 새 장비의 기초 연구를 진행한다. ",
		condition = 0,
		bg = "3",
		blueprint_version = 1,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "item_3",
		label_color = "tag_e",
		consume = {
			{
				2,
				10019,
				45
			}
		},
		drop_client = {
			{
				2,
				13005,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[1035] = {
		id = 1035,
		rarity = 3,
		time = 7200,
		type = 2,
		name = "E-818-MI",
		lv_limit = 80,
		desc = "현재 비치 중인 장비를 분해함으로써 기초 설계를 재검토하여, 추후 개발이 예정된 새 장비의 기초 연구를 진행한다. ",
		condition = 0,
		bg = "3",
		blueprint_version = 1,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "item_3",
		label_color = "tag_e",
		consume = {
			{
				2,
				10037,
				45
			}
		},
		drop_client = {
			{
				2,
				13006,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[1036] = {
		id = 1036,
		rarity = 3,
		time = 7200,
		type = 2,
		name = "E-844-MI",
		lv_limit = 80,
		desc = "현재 비치 중인 장비를 분해함으로써 기초 설계를 재검토하여, 추후 개발이 예정된 새 장비의 기초 연구를 진행한다. ",
		condition = 0,
		bg = "3",
		blueprint_version = 1,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "item_3",
		label_color = "tag_e",
		consume = {
			{
				2,
				10052,
				45
			}
		},
		drop_client = {
			{
				2,
				13008,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[1101] = {
		id = 1101,
		rarity = 3,
		time = 43200,
		type = 2,
		name = "기초 연구",
		lv_limit = 30,
		desc = "성정 큐브에 관한 기초 이론은 큐브 구조에 대한 인식으로부터 시작된다. 큐브 기초 구조에 대한 이론 구조 작업은 여전히 전력을 다해 진행 중이다.",
		condition = 0,
		bg = "3",
		blueprint_version = 2,
		sub_name = "대형 항목",
		label = "tag_red",
		bg_icon = "item_3",
		label_color = "tag_c",
		consume = {},
		drop_client = {
			{
				2,
				52011,
				1
			},
			{
				2,
				52012,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[1102] = {
		id = 1102,
		rarity = 3,
		time = 43200,
		type = 2,
		name = "기초 연구",
		lv_limit = 30,
		desc = "성정 큐브에 관한 기초 이론은 큐브 구조에 대한 인식으로부터 시작된다. 큐브 기초 구조에 대한 이론 구조 작업은 여전히 전력을 다해 진행 중이다.",
		condition = 0,
		bg = "3",
		blueprint_version = 2,
		sub_name = "대형 항목",
		label = "tag_red",
		bg_icon = "item_3",
		label_color = "tag_c",
		consume = {},
		drop_client = {
			{
				2,
				52011,
				1
			},
			{
				2,
				52012,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[1103] = {
		id = 1103,
		rarity = 3,
		time = 14400,
		type = 2,
		name = "데이터 수집",
		lv_limit = 75,
		desc = "성정 큐브 구조에 관한 여러가지 가설에 대해 실증실험을 진행하는 연구 프로젝트. 실전을 통해 데이터를 수집하여, 이론의 과학적 증명에 기여하자.",
		condition = 52009,
		bg = "3",
		blueprint_version = 2,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "battle_3",
		label_color = "tag_b",
		consume = {},
		drop_client = {
			{
				2,
				52011,
				1
			},
			{
				2,
				52012,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1104] = {
		id = 1104,
		rarity = 3,
		time = 14400,
		type = 2,
		name = "데이터 수집",
		lv_limit = 80,
		desc = "성정 큐브 구조에 관한 여러가지 가설에 대해 실증실험을 진행하는 연구 프로젝트. 실전을 통해 데이터를 수집하여, 이론의 과학적 증명에 기여하자.",
		condition = 52010,
		bg = "3",
		blueprint_version = 2,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "battle_3",
		label_color = "tag_b",
		consume = {},
		drop_client = {
			{
				2,
				52011,
				1
			},
			{
				2,
				52012,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1105] = {
		id = 1105,
		rarity = 3,
		time = 28800,
		type = 2,
		name = "인식 방향성 연구",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수의 사람들의 사물에 대한 인식의 집합체를 구현화하는 것이라면, 그것의 '방향성' 을 정하는 것에 따라 결과는 과연 어떻게 바뀔 것인가.",
		condition = 0,
		bg = "3",
		blueprint_version = 2,
		sub_name = "대형 항목",
		label = "tag_red",
		bg_icon = "chara_xiyatu",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				8000
			}
		},
		drop_client = {
			{
				2,
				42011,
				1
			},
			{
				2,
				52012,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1106] = {
		id = 1106,
		rarity = 3,
		time = 28800,
		type = 2,
		name = "인식 방향성 연구",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수의 사람들의 사물에 대한 인식의 집합체를 구현화하는 것이라면, 그것의 '방향성' 을 정하는 것에 따라 결과는 과연 어떻게 바뀔 것인가.",
		condition = 0,
		bg = "3",
		blueprint_version = 2,
		sub_name = "대형 항목",
		label = "tag_red",
		bg_icon = "chara_zuozhiya",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				8000
			}
		},
		drop_client = {
			{
				2,
				42012,
				1
			},
			{
				2,
				52012,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1107] = {
		id = 1107,
		rarity = 3,
		time = 28800,
		type = 2,
		name = "인식 방향성 연구",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수의 사람들의 사물에 대한 인식의 집합체를 구현화하는 것이라면, 그것의 '방향성' 을 정하는 것에 따라 결과는 과연 어떻게 바뀔 것인가.",
		condition = 0,
		bg = "3",
		blueprint_version = 2,
		sub_name = "대형 항목",
		label = "tag_red",
		bg_icon = "chara_beifeng",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				8000
			}
		},
		drop_client = {
			{
				2,
				42013,
				1
			},
			{
				2,
				52012,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1108] = {
		id = 1108,
		rarity = 3,
		time = 28800,
		type = 2,
		name = "인식 방향성 연구",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수의 사람들의 사물에 대한 인식의 집합체를 구현화하는 것이라면, 그것의 '방향성' 을 정하는 것에 따라 결과는 과연 어떻게 바뀔 것인가.",
		condition = 0,
		bg = "3",
		blueprint_version = 2,
		sub_name = "대형 항목",
		label = "tag_red",
		bg_icon = "chara_wuqi",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				8000
			}
		},
		drop_client = {
			{
				2,
				42014,
				1
			},
			{
				2,
				52012,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1109] = {
		id = 1109,
		rarity = 3,
		time = 28800,
		type = 2,
		name = "인식 방향성 연구",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수의 사람들의 사물에 대한 인식의 집합체를 구현화하는 것이라면, 그것의 '방향성' 을 정하는 것에 따라 결과는 과연 어떻게 바뀔 것인가.",
		condition = 0,
		bg = "3",
		blueprint_version = 2,
		sub_name = "대형 항목",
		label = "tag_red",
		bg_icon = "chara_feiteliedadi",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				8000
			}
		},
		drop_client = {
			{
				2,
				42015,
				1
			},
			{
				2,
				52012,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1110] = {
		id = 1110,
		rarity = 3,
		time = 28800,
		type = 2,
		name = "인식 방향성 연구",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수의 사람들의 사물에 대한 인식의 집합체를 구현화하는 것이라면, 그것의 '방향성' 을 정하는 것에 따라 결과는 과연 어떻게 바뀔 것인가.",
		condition = 0,
		bg = "3",
		blueprint_version = 2,
		sub_name = "대형 항목",
		label = "tag_red",
		bg_icon = "chara_jiasikenie",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				8000
			}
		},
		drop_client = {
			{
				2,
				42016,
				1
			},
			{
				2,
				52012,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1111] = {
		id = 1111,
		rarity = 3,
		time = 14400,
		type = 2,
		name = "큐브 구조 해석",
		lv_limit = 30,
		desc = "성정 큐브를 문자 그대로 물리적으로 '분해'하여, 구조 분석을 진행한다. 성공 사례가 없을수록, 실험의 가치는 그만큼 크다고 볼 수 있다.",
		condition = 0,
		bg = "3",
		blueprint_version = 2,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "gold_3",
		label_color = "tag_h",
		consume = {
			{
				2,
				20001,
				10
			}
		},
		drop_client = {
			{
				2,
				52011,
				1
			},
			{
				2,
				54051,
				1
			},
			{
				2,
				15008,
				1
			}
		}
	},
	[1112] = {
		id = 1112,
		rarity = 3,
		time = 14400,
		type = 2,
		name = "큐브 구조 해석",
		lv_limit = 30,
		desc = "성정 큐브를 문자 그대로 물리적으로 '분해'하여, 구조 분석을 진행한다. 성공 사례가 없을수록, 실험의 가치는 그만큼 크다고 볼 수 있다.",
		condition = 0,
		bg = "3",
		blueprint_version = 2,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "gold_3",
		label_color = "tag_h",
		consume = {
			{
				2,
				20001,
				10
			}
		},
		drop_client = {
			{
				2,
				52011,
				1
			},
			{
				2,
				54051,
				1
			},
			{
				2,
				15008,
				1
			}
		}
	},
	[1113] = {
		id = 1113,
		rarity = 3,
		time = 1800,
		type = 2,
		name = "긴급 : 인식 방향성 연구",
		lv_limit = 30,
		desc = "성정 큐브의 인식 구현화 실험 중, 이산치가 폭증하는 이상 현상이 발생하였다. 원리를 밝혀내면, 성정 큐브에 관한 이론 구축에 큰 진전이 있을 것이다.",
		condition = 0,
		bg = "3",
		blueprint_version = 2,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "chara_xiyatu",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				5000
			},
			{
				2,
				20001,
				5
			}
		},
		drop_client = {
			{
				2,
				42011,
				1
			},
			{
				2,
				52012,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1114] = {
		id = 1114,
		rarity = 3,
		time = 1800,
		type = 2,
		name = "긴급 : 인식 방향성 연구",
		lv_limit = 30,
		desc = "성정 큐브의 인식 구현화 실험 중, 이산치가 폭증하는 이상 현상이 발생하였다. 원리를 밝혀내면, 성정 큐브에 관한 이론 구축에 큰 진전이 있을 것이다.",
		condition = 0,
		bg = "3",
		blueprint_version = 2,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "chara_zuozhiya",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				5000
			},
			{
				2,
				20001,
				5
			}
		},
		drop_client = {
			{
				2,
				42012,
				1
			},
			{
				2,
				52012,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1115] = {
		id = 1115,
		rarity = 3,
		time = 1800,
		type = 2,
		name = "긴급 : 인식 방향성 연구",
		lv_limit = 30,
		desc = "성정 큐브의 인식 구현화 실험 중, 이산치가 폭증하는 이상 현상이 발생하였다. 원리를 밝혀내면, 성정 큐브에 관한 이론 구축에 큰 진전이 있을 것이다.",
		condition = 0,
		bg = "3",
		blueprint_version = 2,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "chara_beifeng",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				5000
			},
			{
				2,
				20001,
				5
			}
		},
		drop_client = {
			{
				2,
				42013,
				1
			},
			{
				2,
				52012,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1116] = {
		id = 1116,
		rarity = 4,
		time = 1800,
		type = 2,
		name = "긴급 : 인식 방향성 연구",
		lv_limit = 30,
		desc = "성정 큐브의 인식 구현화 실험 중, 이산치가 폭증하는 이상 현상이 발생하였다. 원리를 밝혀내면, 성정 큐브에 관한 이론 구축에 큰 진전이 있을 것이다.",
		condition = 0,
		bg = "4",
		blueprint_version = 2,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "chara_wuqi",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				5000
			},
			{
				2,
				20001,
				5
			}
		},
		drop_client = {
			{
				2,
				42014,
				1
			},
			{
				2,
				52012,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1117] = {
		id = 1117,
		rarity = 4,
		time = 1800,
		type = 2,
		name = "긴급 : 인식 방향성 연구",
		lv_limit = 30,
		desc = "성정 큐브의 인식 구현화 실험 중, 이산치가 폭증하는 이상 현상이 발생하였다. 원리를 밝혀내면, 성정 큐브에 관한 이론 구축에 큰 진전이 있을 것이다.",
		condition = 0,
		bg = "4",
		blueprint_version = 2,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "chara_feiteliedadi",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				5000
			},
			{
				2,
				20001,
				5
			}
		},
		drop_client = {
			{
				2,
				42015,
				1
			},
			{
				2,
				52012,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1118] = {
		id = 1118,
		rarity = 3,
		time = 1800,
		type = 2,
		name = "긴급 : 인식 방향성 연구",
		lv_limit = 30,
		desc = "성정 큐브의 인식 구현화 실험 중, 이산치가 폭증하는 이상 현상이 발생하였다. 원리를 밝혀내면, 성정 큐브에 관한 이론 구축에 큰 진전이 있을 것이다.",
		condition = 0,
		bg = "3",
		blueprint_version = 2,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "chara_jiasikenie",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				5000
			},
			{
				2,
				20001,
				5
			}
		},
		drop_client = {
			{
				2,
				42016,
				1
			},
			{
				2,
				52012,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1119] = {
		id = 1119,
		rarity = 3,
		time = 1800,
		type = 2,
		name = "긴급 : 장비 해석",
		lv_limit = 30,
		desc = "성정 큐브와 장비의 공명 실험에서, 특정 부품이 매우 높은 싱크로율을 보였다. 분명 지금까지 밝히지 못한 미지의 기술이 담겨 있을 것이다.",
		condition = 0,
		bg = "3",
		blueprint_version = 2,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_3",
		label_color = "tag_q",
		consume = {
			{
				1,
				1,
				5000
			},
			{
				2,
				17003,
				20
			}
		},
		drop_client = {
			{
				2,
				52012,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1120] = {
		id = 1120,
		rarity = 3,
		time = 1800,
		type = 2,
		name = "긴급 : 장비 해석",
		lv_limit = 30,
		desc = "성정 큐브와 장비의 공명 실험에서, 특정 부품이 매우 높은 싱크로율을 보였다. 분명 지금까지 밝히지 못한 미지의 기술이 담겨 있을 것이다.",
		condition = 0,
		bg = "3",
		blueprint_version = 2,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_3",
		label_color = "tag_q",
		consume = {
			{
				1,
				1,
				5000
			},
			{
				2,
				17013,
				20
			}
		},
		drop_client = {
			{
				2,
				52012,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1121] = {
		id = 1121,
		rarity = 3,
		time = 1800,
		type = 2,
		name = "긴급 : 장비 해석",
		lv_limit = 30,
		desc = "성정 큐브와 장비의 공명 실험에서, 특정 부품이 매우 높은 싱크로율을 보였다. 분명 지금까지 밝히지 못한 미지의 기술이 담겨 있을 것이다.",
		condition = 0,
		bg = "3",
		blueprint_version = 2,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_3",
		label_color = "tag_q",
		consume = {
			{
				1,
				1,
				5000
			},
			{
				2,
				17023,
				20
			}
		},
		drop_client = {
			{
				2,
				52012,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1122] = {
		id = 1122,
		rarity = 3,
		time = 1800,
		type = 2,
		name = "긴급 : 장비 해석",
		lv_limit = 30,
		desc = "성정 큐브와 장비의 공명 실험에서, 특정 부품이 매우 높은 싱크로율을 보였다. 분명 지금까지 밝히지 못한 미지의 기술이 담겨 있을 것이다.",
		condition = 0,
		bg = "3",
		blueprint_version = 2,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_3",
		label_color = "tag_q",
		consume = {
			{
				1,
				1,
				5000
			},
			{
				2,
				17033,
				20
			}
		},
		drop_client = {
			{
				2,
				52012,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1123] = {
		id = 1123,
		rarity = 3,
		time = 1800,
		type = 2,
		name = "긴급 : 장비 해석",
		lv_limit = 30,
		desc = "성정 큐브와 장비의 공명 실험에서, 특정 부품이 매우 높은 싱크로율을 보였다. 분명 지금까지 밝히지 못한 미지의 기술이 담겨 있을 것이다.",
		condition = 0,
		bg = "3",
		blueprint_version = 2,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_3",
		label_color = "tag_q",
		consume = {
			{
				1,
				1,
				5000
			},
			{
				2,
				17043,
				20
			}
		},
		drop_client = {
			{
				2,
				52012,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1124] = {
		id = 1124,
		rarity = 3,
		time = 1800,
		type = 2,
		name = "긴급 : 큐브 구조 해석",
		lv_limit = 30,
		desc = "상부에서 극비사항으로 분류되는 연구 프로젝트. 이로 인해 인류뿐만 아니라, 전세계의 생태계 자체에 상당한 영향이 갈지도 모른다.",
		condition = 0,
		bg = "3",
		blueprint_version = 2,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_3",
		label_color = "tag_sh",
		consume = {
			{
				1,
				1,
				8000
			},
			{
				2,
				20001,
				3
			}
		},
		drop_client = {
			{
				2,
				52011,
				1
			},
			{
				2,
				54051,
				1
			},
			{
				2,
				15008,
				1
			}
		}
	},
	[1125] = {
		id = 1125,
		rarity = 3,
		time = 1800,
		type = 2,
		name = "긴급 : 큐브 구조 해석",
		lv_limit = 30,
		desc = "상부에서 극비사항으로 분류되는 연구 프로젝트. 이로 인해 인류뿐만 아니라, 전세계의 생태계 자체에 상당한 영향이 갈지도 모른다.",
		condition = 0,
		bg = "3",
		blueprint_version = 2,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_3",
		label_color = "tag_sh",
		consume = {
			{
				1,
				1,
				8000
			},
			{
				2,
				20001,
				3
			}
		},
		drop_client = {
			{
				2,
				52011,
				1
			},
			{
				2,
				54051,
				1
			},
			{
				2,
				15008,
				1
			}
		}
	},
	[1126] = {
		id = 1126,
		rarity = 3,
		time = 14400,
		type = 2,
		name = "데이터 수집",
		lv_limit = 85,
		desc = "성정 큐브 구조에 관한 여러가지 가설에 대해 실증실험을 진행하는 연구 프로젝트. 실전을 통해 데이터를 수집하여, 이론의 과학적 증명에 기여하자.",
		condition = 52011,
		bg = "3",
		blueprint_version = 2,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "battle_3",
		label_color = "tag_b",
		consume = {},
		drop_client = {
			{
				2,
				52011,
				1
			},
			{
				2,
				52012,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1131] = {
		id = 1131,
		rarity = 3,
		time = 7200,
		type = 2,
		name = "E-879-MI",
		lv_limit = 80,
		desc = "현재 비치 중인 장비를 분해함으로써 기초 설계를 재검토하여, 추후 개발이 예정된 새 장비의 기초 연구를 진행한다. ",
		condition = 0,
		bg = "3",
		blueprint_version = 2,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "item_3",
		label_color = "tag_e",
		consume = {
			{
				2,
				10075,
				45
			}
		},
		drop_client = {
			{
				2,
				13013,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[1132] = {
		id = 1132,
		rarity = 3,
		time = 7200,
		type = 2,
		name = "E-805-MI",
		lv_limit = 80,
		desc = "현재 비치 중인 장비를 분해함으로써 기초 설계를 재검토하여, 추후 개발이 예정된 새 장비의 기초 연구를 진행한다. ",
		condition = 0,
		bg = "3",
		blueprint_version = 2,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "item_3",
		label_color = "tag_e",
		consume = {
			{
				2,
				10018,
				45
			}
		},
		drop_client = {
			{
				2,
				13014,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[1133] = {
		id = 1133,
		rarity = 3,
		time = 7200,
		type = 2,
		name = "E-846-MI",
		lv_limit = 80,
		desc = "현재 비치 중인 장비를 분해함으로써 기초 설계를 재검토하여, 추후 개발이 예정된 새 장비의 기초 연구를 진행한다. ",
		condition = 0,
		bg = "3",
		blueprint_version = 2,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "item_3",
		label_color = "tag_e",
		consume = {
			{
				2,
				10026,
				45
			}
		},
		drop_client = {
			{
				2,
				13015,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[1134] = {
		id = 1134,
		rarity = 3,
		time = 7200,
		type = 2,
		name = "E-857-MI",
		lv_limit = 80,
		desc = "현재 비치 중인 장비를 분해함으로써 기초 설계를 재검토하여, 추후 개발이 예정된 새 장비의 기초 연구를 진행한다. ",
		condition = 0,
		bg = "3",
		blueprint_version = 2,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "item_3",
		label_color = "tag_e",
		consume = {
			{
				2,
				10056,
				45
			}
		},
		drop_client = {
			{
				2,
				13016,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[1135] = {
		id = 1135,
		rarity = 3,
		time = 7200,
		type = 2,
		name = "E-894-MI",
		lv_limit = 80,
		desc = "현재 비치 중인 장비를 분해함으로써 기초 설계를 재검토하여, 추후 개발이 예정된 새 장비의 기초 연구를 진행한다. ",
		condition = 0,
		bg = "3",
		blueprint_version = 2,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "item_3",
		label_color = "tag_e",
		consume = {
			{
				2,
				10077,
				45
			}
		},
		drop_client = {
			{
				2,
				13017,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[1201] = {
		id = 1201,
		rarity = 3,
		time = 43200,
		type = 2,
		name = "C-038-RF",
		lv_limit = 30,
		desc = "성정 큐브에 관한 기초 이론은 큐브 구조에 대한 인식으로부터 시작된다. 큐브 기초 구조에 대한 이론 구조 작업은 여전히 전력을 다해 진행 중이다.",
		condition = 0,
		bg = "3",
		blueprint_version = 3,
		sub_name = "대형 항목",
		label = "tag_red",
		bg_icon = "item_3",
		label_color = "tag_c",
		consume = {},
		drop_client = {
			{
				2,
				52021,
				1
			},
			{
				2,
				52022,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[1202] = {
		id = 1202,
		rarity = 3,
		time = 43200,
		type = 2,
		name = "C-038-RF",
		lv_limit = 30,
		desc = "성정 큐브에 관한 기초 이론은 큐브 구조에 대한 인식으로부터 시작된다. 큐브 기초 구조에 대한 이론 구조 작업은 여전히 전력을 다해 진행 중이다.",
		condition = 0,
		bg = "3",
		blueprint_version = 3,
		sub_name = "대형 항목",
		label = "tag_red",
		bg_icon = "item_3",
		label_color = "tag_c",
		consume = {},
		drop_client = {
			{
				2,
				52021,
				1
			},
			{
				2,
				52022,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[1203] = {
		id = 1203,
		rarity = 3,
		time = 14400,
		type = 2,
		name = "B-351-RF",
		lv_limit = 75,
		desc = "성정 큐브 구조에 관한 여러가지 가설에 대해 실증실험을 진행하는 연구 프로젝트. 실전을 통해 데이터를 수집하여, 이론의 과학적 증명에 기여하자.",
		condition = 52009,
		bg = "3",
		blueprint_version = 3,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "battle_3",
		label_color = "tag_b",
		consume = {},
		drop_client = {
			{
				2,
				52021,
				1
			},
			{
				2,
				52022,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1204] = {
		id = 1204,
		rarity = 3,
		time = 14400,
		type = 2,
		name = "B-397-RF",
		lv_limit = 80,
		desc = "성정 큐브 구조에 관한 여러가지 가설에 대해 실증실험을 진행하는 연구 프로젝트. 실전을 통해 데이터를 수집하여, 이론의 과학적 증명에 기여하자.",
		condition = 52010,
		bg = "3",
		blueprint_version = 3,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "battle_3",
		label_color = "tag_b",
		consume = {},
		drop_client = {
			{
				2,
				52021,
				1
			},
			{
				2,
				52022,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1205] = {
		id = 1205,
		rarity = 3,
		time = 28800,
		type = 2,
		name = "D-421-MI",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수의 사람들의 사물에 대한 인식의 집합체를 구현화하는 것이라면, 그것의 '방향성' 을 정하는 것에 따라 결과는 과연 어떻게 바뀔 것인가.？",
		condition = 0,
		bg = "3",
		blueprint_version = 3,
		sub_name = "대형 항목",
		label = "tag_red",
		bg_icon = "chara_chaijun",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				8000
			}
		},
		drop_client = {
			{
				2,
				42021,
				1
			},
			{
				2,
				52022,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1206] = {
		id = 1206,
		rarity = 3,
		time = 28800,
		type = 2,
		name = "D-422-MI",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수의 사람들의 사물에 대한 인식의 집합체를 구현화하는 것이라면, 그것의 '방향성' 을 정하는 것에 따라 결과는 과연 어떻게 바뀔 것인가.？",
		condition = 0,
		bg = "3",
		blueprint_version = 3,
		sub_name = "대형 항목",
		label = "tag_red",
		bg_icon = "chara_deleike",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				8000
			}
		},
		drop_client = {
			{
				2,
				42022,
				1
			},
			{
				2,
				52022,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1207] = {
		id = 1207,
		rarity = 3,
		time = 28800,
		type = 2,
		name = "D-472-MI",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수의 사람들의 사물에 대한 인식의 집합체를 구현화하는 것이라면, 그것의 '방향성' 을 정하는 것에 따라 결과는 과연 어떻게 바뀔 것인가.？",
		condition = 0,
		bg = "3",
		blueprint_version = 3,
		sub_name = "대형 항목",
		label = "tag_red",
		bg_icon = "chara_meiyinci",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				8000
			}
		},
		drop_client = {
			{
				2,
				42023,
				1
			},
			{
				2,
				52022,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1208] = {
		id = 1208,
		rarity = 3,
		time = 28800,
		type = 2,
		name = "D-477-MI",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수의 사람들의 사물에 대한 인식의 집합체를 구현화하는 것이라면, 그것의 '방향성' 을 정하는 것에 따라 결과는 과연 어떻게 바뀔 것인가.？",
		condition = 0,
		bg = "3",
		blueprint_version = 3,
		sub_name = "대형 항목",
		label = "tag_red",
		bg_icon = "chara_aoding",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				8000
			}
		},
		drop_client = {
			{
				2,
				42024,
				1
			},
			{
				2,
				52022,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1209] = {
		id = 1209,
		rarity = 3,
		time = 28800,
		type = 2,
		name = "D-495-MI",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수의 사람들의 사물에 대한 인식의 집합체를 구현화하는 것이라면, 그것의 '방향성' 을 정하는 것에 따라 결과는 과연 어떻게 바뀔 것인가.？",
		condition = 0,
		bg = "3",
		blueprint_version = 3,
		sub_name = "대형 항목",
		label = "tag_red",
		bg_icon = "chara_xiangbin",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				8000
			}
		},
		drop_client = {
			{
				2,
				42025,
				1
			},
			{
				2,
				52022,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1211] = {
		id = 1211,
		rarity = 3,
		time = 14400,
		type = 2,
		name = "H-207-RF",
		lv_limit = 30,
		desc = "성정 큐브를 문자 그대로 물리적으로 '분해'하여, 구조 분석을 진행한다. 성공 사례가 없을수록, 실험의 가치는 그만큼 크다고 볼 수 있다.",
		condition = 0,
		bg = "3",
		blueprint_version = 3,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "gold_3",
		label_color = "tag_h",
		consume = {
			{
				2,
				20001,
				10
			}
		},
		drop_client = {
			{
				2,
				52021,
				1
			},
			{
				2,
				54051,
				1
			},
			{
				2,
				15008,
				1
			}
		}
	},
	[1212] = {
		id = 1212,
		rarity = 3,
		time = 14400,
		type = 2,
		name = "H-207-RF",
		lv_limit = 30,
		desc = "성정 큐브를 문자 그대로 물리적으로 '분해'하여, 구조 분석을 진행한다. 성공 사례가 없을수록, 실험의 가치는 그만큼 크다고 볼 수 있다.",
		condition = 0,
		bg = "3",
		blueprint_version = 3,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "gold_3",
		label_color = "tag_h",
		consume = {
			{
				2,
				20001,
				10
			}
		},
		drop_client = {
			{
				2,
				52021,
				1
			},
			{
				2,
				54051,
				1
			},
			{
				2,
				15008,
				1
			}
		}
	},
	[1213] = {
		id = 1213,
		rarity = 3,
		time = 1800,
		type = 2,
		name = "D-021-MI",
		lv_limit = 30,
		desc = "성정 큐브의 인식 구현화 실험 중, 이산치가 폭증하는 이상 현상이 발생하였다. 원리를 밝혀내면, 성정 큐브에 관한 이론 구축에 큰 진전이 있을 것이다.",
		condition = 0,
		bg = "3",
		blueprint_version = 3,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "chara_chaijun",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				5000
			},
			{
				2,
				20001,
				5
			}
		},
		drop_client = {
			{
				2,
				42021,
				1
			},
			{
				2,
				52022,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1214] = {
		id = 1214,
		rarity = 4,
		time = 1800,
		type = 2,
		name = "D-022-MI",
		lv_limit = 30,
		desc = "성정 큐브의 인식 구현화 실험 중, 이산치가 폭증하는 이상 현상이 발생하였다. 원리를 밝혀내면, 성정 큐브에 관한 이론 구축에 큰 진전이 있을 것이다.",
		condition = 0,
		bg = "4",
		blueprint_version = 3,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "chara_deleike",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				5000
			},
			{
				2,
				20001,
				5
			}
		},
		drop_client = {
			{
				2,
				42022,
				1
			},
			{
				2,
				52022,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1215] = {
		id = 1215,
		rarity = 3,
		time = 1800,
		type = 2,
		name = "D-072-MI",
		lv_limit = 30,
		desc = "성정 큐브의 인식 구현화 실험 중, 이산치가 폭증하는 이상 현상이 발생하였다. 원리를 밝혀내면, 성정 큐브에 관한 이론 구축에 큰 진전이 있을 것이다.",
		condition = 0,
		bg = "3",
		blueprint_version = 3,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "chara_meiyinci",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				5000
			},
			{
				2,
				20001,
				5
			}
		},
		drop_client = {
			{
				2,
				42023,
				1
			},
			{
				2,
				52022,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1216] = {
		id = 1216,
		rarity = 3,
		time = 1800,
		type = 2,
		name = "D-077-MI",
		lv_limit = 30,
		desc = "성정 큐브의 인식 구현화 실험 중, 이산치가 폭증하는 이상 현상이 발생하였다. 원리를 밝혀내면, 성정 큐브에 관한 이론 구축에 큰 진전이 있을 것이다.",
		condition = 0,
		bg = "3",
		blueprint_version = 3,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "chara_aoding",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				5000
			},
			{
				2,
				20001,
				5
			}
		},
		drop_client = {
			{
				2,
				42024,
				1
			},
			{
				2,
				52022,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1217] = {
		id = 1217,
		rarity = 3,
		time = 1800,
		type = 2,
		name = "D-095-MI",
		lv_limit = 30,
		desc = "성정 큐브의 인식 구현화 실험 중, 이산치가 폭증하는 이상 현상이 발생하였다. 원리를 밝혀내면, 성정 큐브에 관한 이론 구축에 큰 진전이 있을 것이다.",
		condition = 0,
		bg = "3",
		blueprint_version = 3,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "chara_xiangbin",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				5000
			},
			{
				2,
				20001,
				5
			}
		},
		drop_client = {
			{
				2,
				42025,
				1
			},
			{
				2,
				52022,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1219] = {
		id = 1219,
		rarity = 3,
		time = 1800,
		type = 2,
		name = "Q-002-UL",
		lv_limit = 30,
		desc = "성정 큐브와 장비의 공명 실험에서, 특정 부품이 매우 높은 싱크로율을 보였다. 분명 지금까지 밝히지 못한 미지의 기술이 담겨 있을 것이다.",
		condition = 0,
		bg = "3",
		blueprint_version = 3,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_3",
		label_color = "tag_q",
		consume = {
			{
				1,
				1,
				5000
			},
			{
				2,
				17003,
				20
			}
		},
		drop_client = {
			{
				2,
				52022,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1220] = {
		id = 1220,
		rarity = 3,
		time = 1800,
		type = 2,
		name = "Q-010-UL",
		lv_limit = 30,
		desc = "성정 큐브와 장비의 공명 실험에서, 특정 부품이 매우 높은 싱크로율을 보였다. 분명 지금까지 밝히지 못한 미지의 기술이 담겨 있을 것이다.",
		condition = 0,
		bg = "3",
		blueprint_version = 3,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_3",
		label_color = "tag_q",
		consume = {
			{
				1,
				1,
				5000
			},
			{
				2,
				17013,
				20
			}
		},
		drop_client = {
			{
				2,
				52022,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1221] = {
		id = 1221,
		rarity = 3,
		time = 1800,
		type = 2,
		name = "Q-051-UL",
		lv_limit = 30,
		desc = "성정 큐브와 장비의 공명 실험에서, 특정 부품이 매우 높은 싱크로율을 보였다. 분명 지금까지 밝히지 못한 미지의 기술이 담겨 있을 것이다.",
		condition = 0,
		bg = "3",
		blueprint_version = 3,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_3",
		label_color = "tag_q",
		consume = {
			{
				1,
				1,
				5000
			},
			{
				2,
				17023,
				20
			}
		},
		drop_client = {
			{
				2,
				52022,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1222] = {
		id = 1222,
		rarity = 3,
		time = 1800,
		type = 2,
		name = "Q-068-UL",
		lv_limit = 30,
		desc = "성정 큐브와 장비의 공명 실험에서, 특정 부품이 매우 높은 싱크로율을 보였다. 분명 지금까지 밝히지 못한 미지의 기술이 담겨 있을 것이다.",
		condition = 0,
		bg = "3",
		blueprint_version = 3,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_3",
		label_color = "tag_q",
		consume = {
			{
				1,
				1,
				5000
			},
			{
				2,
				17033,
				20
			}
		},
		drop_client = {
			{
				2,
				52022,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1223] = {
		id = 1223,
		rarity = 3,
		time = 1800,
		type = 2,
		name = "Q-089-UL",
		lv_limit = 30,
		desc = "성정 큐브와 장비의 공명 실험에서, 특정 부품이 매우 높은 싱크로율을 보였다. 분명 지금까지 밝히지 못한 미지의 기술이 담겨 있을 것이다.",
		condition = 0,
		bg = "3",
		blueprint_version = 3,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_3",
		label_color = "tag_q",
		consume = {
			{
				1,
				1,
				5000
			},
			{
				2,
				17043,
				20
			}
		},
		drop_client = {
			{
				2,
				52022,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1224] = {
		id = 1224,
		rarity = 3,
		time = 1800,
		type = 2,
		name = "H-060-UL",
		lv_limit = 30,
		desc = "상부에서 극비사항으로 분류되는 연구 프로젝트. 이로 인해 인류뿐만 아니라, 전세계의 생태계 자체에 상당한 영향이 갈지도 모른다.",
		condition = 0,
		bg = "3",
		blueprint_version = 3,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_3",
		label_color = "tag_sh",
		consume = {
			{
				1,
				1,
				8000
			},
			{
				2,
				20001,
				3
			}
		},
		drop_client = {
			{
				2,
				52021,
				1
			},
			{
				2,
				54051,
				1
			},
			{
				2,
				15008,
				1
			}
		}
	},
	[1225] = {
		id = 1225,
		rarity = 3,
		time = 1800,
		type = 2,
		name = "H-060-UL",
		lv_limit = 30,
		desc = "상부에서 극비사항으로 분류되는 연구 프로젝트. 이로 인해 인류뿐만 아니라, 전세계의 생태계 자체에 상당한 영향이 갈지도 모른다.",
		condition = 0,
		bg = "3",
		blueprint_version = 3,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_3",
		label_color = "tag_sh",
		consume = {
			{
				1,
				1,
				8000
			},
			{
				2,
				20001,
				3
			}
		},
		drop_client = {
			{
				2,
				52021,
				1
			},
			{
				2,
				54051,
				1
			},
			{
				2,
				15008,
				1
			}
		}
	},
	[1226] = {
		id = 1226,
		rarity = 3,
		time = 14400,
		type = 2,
		name = "B-397-RF",
		lv_limit = 85,
		desc = "성정 큐브 구조에 관한 여러가지 가설에 대해 실증실험을 진행하는 연구 프로젝트. 실전을 통해 데이터를 수집하여, 이론의 과학적 증명에 기여하자.",
		condition = 52011,
		bg = "3",
		blueprint_version = 3,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "battle_3",
		label_color = "tag_b",
		consume = {},
		drop_client = {
			{
				2,
				52021,
				1
			},
			{
				2,
				52022,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1231] = {
		id = 1231,
		rarity = 3,
		time = 7200,
		type = 2,
		name = "E-821-MI",
		lv_limit = 80,
		desc = "현재 비치 중인 장비를 분해함으로써 기초 설계를 재검토하여, 추후 개발이 예정된 새 장비의 기초 연구를 진행한다. ",
		condition = 0,
		bg = "3",
		blueprint_version = 3,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "item_3",
		label_color = "tag_e",
		consume = {
			{
				2,
				10038,
				45
			}
		},
		drop_client = {
			{
				2,
				13018,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[1232] = {
		id = 1232,
		rarity = 3,
		time = 7200,
		type = 2,
		name = "E-823-MI",
		lv_limit = 80,
		desc = "현재 비치 중인 장비를 분해함으로써 기초 설계를 재검토하여, 추후 개발이 예정된 새 장비의 기초 연구를 진행한다. ",
		condition = 0,
		bg = "3",
		blueprint_version = 3,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "item_3",
		label_color = "tag_e",
		consume = {
			{
				2,
				10035,
				45
			}
		},
		drop_client = {
			{
				2,
				13020,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[1233] = {
		id = 1233,
		rarity = 3,
		time = 7200,
		type = 2,
		name = "E-824-MI",
		lv_limit = 80,
		desc = "현재 비치 중인 장비를 분해함으로써 기초 설계를 재검토하여, 추후 개발이 예정된 새 장비의 기초 연구를 진행한다. ",
		condition = 0,
		bg = "3",
		blueprint_version = 3,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "item_3",
		label_color = "tag_e",
		consume = {
			{
				2,
				11009,
				45
			}
		},
		drop_client = {
			{
				2,
				13021,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[1234] = {
		id = 1234,
		rarity = 3,
		time = 7200,
		type = 2,
		name = "E-877-MI",
		lv_limit = 80,
		desc = "현재 비치 중인 장비를 분해함으로써 기초 설계를 재검토하여, 추후 개발이 예정된 새 장비의 기초 연구를 진행한다. ",
		condition = 0,
		bg = "3",
		blueprint_version = 3,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "item_3",
		label_color = "tag_e",
		consume = {
			{
				2,
				10075,
				45
			}
		},
		drop_client = {
			{
				2,
				13022,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[1235] = {
		id = 1235,
		rarity = 3,
		time = 7200,
		type = 2,
		name = "E-895-MI",
		lv_limit = 80,
		desc = "현재 비치 중인 장비를 분해함으로써 기초 설계를 재검토하여, 추후 개발이 예정된 새 장비의 기초 연구를 진행한다. ",
		condition = 0,
		bg = "3",
		blueprint_version = 3,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "item_3",
		label_color = "tag_e",
		consume = {
			{
				2,
				10102,
				45
			}
		},
		drop_client = {
			{
				2,
				13023,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[1301] = {
		id = 1301,
		rarity = 3,
		time = 43200,
		type = 2,
		name = "C-038-RF",
		lv_limit = 30,
		desc = "성정 큐브에 관한 기초 이론은 큐브 구조에 대한 인식으로부터 시작된다. 큐브 기초 구조에 대한 이론 구조 작업은 여전히 전력을 다해 진행 중이다.",
		condition = 0,
		bg = "3",
		blueprint_version = 4,
		sub_name = "대형 항목",
		label = "tag_red",
		bg_icon = "item_3",
		label_color = "tag_c",
		consume = {},
		drop_client = {
			{
				2,
				52031,
				1
			},
			{
				2,
				52032,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[1302] = {
		id = 1302,
		rarity = 3,
		time = 43200,
		type = 2,
		name = "C-038-RF",
		lv_limit = 30,
		desc = "성정 큐브에 관한 기초 이론은 큐브 구조에 대한 인식으로부터 시작된다. 큐브 기초 구조에 대한 이론 구조 작업은 여전히 전력을 다해 진행 중이다.",
		condition = 0,
		bg = "3",
		blueprint_version = 4,
		sub_name = "대형 항목",
		label = "tag_red",
		bg_icon = "item_3",
		label_color = "tag_c",
		consume = {},
		drop_client = {
			{
				2,
				52031,
				1
			},
			{
				2,
				52032,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[1303] = {
		id = 1303,
		rarity = 3,
		time = 14400,
		type = 2,
		name = "B-351-RF",
		lv_limit = 75,
		desc = "성정 큐브 구조에 관한 여러가지 가설에 대해 실증실험을 진행하는 연구 프로젝트. 실전을 통해 데이터를 수집하여, 이론의 과학적 증명에 기여하자.",
		condition = 52009,
		bg = "3",
		blueprint_version = 4,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "battle_3",
		label_color = "tag_b",
		consume = {},
		drop_client = {
			{
				2,
				52031,
				1
			},
			{
				2,
				52032,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1304] = {
		id = 1304,
		rarity = 3,
		time = 14400,
		type = 2,
		name = "B-397-RF",
		lv_limit = 80,
		desc = "성정 큐브 구조에 관한 여러가지 가설에 대해 실증실험을 진행하는 연구 프로젝트. 실전을 통해 데이터를 수집하여, 이론의 과학적 증명에 기여하자.",
		condition = 52010,
		bg = "3",
		blueprint_version = 4,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "battle_3",
		label_color = "tag_b",
		consume = {},
		drop_client = {
			{
				2,
				52031,
				1
			},
			{
				2,
				52032,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1305] = {
		id = 1305,
		rarity = 3,
		time = 28800,
		type = 2,
		name = "D-408-RF",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수의 사람들의 사물에 대한 인식의 집합체를 구현화하는 것이라면, 그것의 '방향성' 을 정하는 것에 따라 결과는 과연 어떻게 바뀔 것인가.",
		condition = 0,
		bg = "3",
		blueprint_version = 4,
		sub_name = "대형 항목",
		label = "tag_red",
		bg_icon = "chara_ankeleiqi",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				8000
			}
		},
		drop_client = {
			{
				2,
				42031,
				1
			},
			{
				2,
				52032,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1306] = {
		id = 1306,
		rarity = 3,
		time = 28800,
		type = 2,
		name = "D-463-RF",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수의 사람들의 사물에 대한 인식의 집합체를 구현화하는 것이라면, 그것의 '방향성' 을 정하는 것에 따라 결과는 과연 어떻게 바뀔 것인가.",
		condition = 0,
		bg = "3",
		blueprint_version = 4,
		sub_name = "대형 항목",
		label = "tag_red",
		bg_icon = "chara_bailong",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				8000
			}
		},
		drop_client = {
			{
				2,
				42032,
				1
			},
			{
				2,
				52032,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1307] = {
		id = 1307,
		rarity = 3,
		time = 28800,
		type = 2,
		name = "D-475-RF",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수의 사람들의 사물에 대한 인식의 집합체를 구현화하는 것이라면, 그것의 '방향성' 을 정하는 것에 따라 결과는 과연 어떻게 바뀔 것인가.",
		condition = 0,
		bg = "3",
		blueprint_version = 4,
		sub_name = "대형 항목",
		label = "tag_red",
		bg_icon = "chara_aijier",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				8000
			}
		},
		drop_client = {
			{
				2,
				42033,
				1
			},
			{
				2,
				52032,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1308] = {
		id = 1308,
		rarity = 3,
		time = 28800,
		type = 2,
		name = "D-482-RF",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수의 사람들의 사물에 대한 인식의 집합체를 구현화하는 것이라면, 그것의 '방향성' 을 정하는 것에 따라 결과는 과연 어떻게 바뀔 것인가.",
		condition = 0,
		bg = "3",
		blueprint_version = 4,
		sub_name = "대형 항목",
		label = "tag_red",
		bg_icon = "chara_aogusite",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				8000
			}
		},
		drop_client = {
			{
				2,
				42034,
				1
			},
			{
				2,
				52032,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1309] = {
		id = 1309,
		rarity = 3,
		time = 28800,
		type = 2,
		name = "D-468-RF",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수의 사람들의 사물에 대한 인식의 집합체를 구현화하는 것이라면, 그것의 '방향성' 을 정하는 것에 따라 결과는 과연 어떻게 바뀔 것인가.",
		condition = 0,
		bg = "3",
		blueprint_version = 4,
		sub_name = "대형 항목",
		label = "tag_red",
		bg_icon = "chara_makeboluo",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				8000
			}
		},
		drop_client = {
			{
				2,
				42035,
				1
			},
			{
				2,
				52032,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1311] = {
		id = 1311,
		rarity = 3,
		time = 14400,
		type = 2,
		name = "H-207-RF",
		lv_limit = 30,
		desc = "성정 큐브를 문자 그대로 물리적으로 '분해'하여, 구조 분석을 진행한다. 성공 사례가 없을수록, 실험의 가치는 그만큼 크다고 볼 수 있다.",
		condition = 0,
		bg = "3",
		blueprint_version = 4,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "gold_3",
		label_color = "tag_h",
		consume = {
			{
				2,
				20001,
				10
			}
		},
		drop_client = {
			{
				2,
				52031,
				1
			},
			{
				2,
				54051,
				1
			},
			{
				2,
				15008,
				1
			}
		}
	},
	[1312] = {
		id = 1312,
		rarity = 3,
		time = 14400,
		type = 2,
		name = "H-207-RF",
		lv_limit = 30,
		desc = "성정 큐브를 문자 그대로 물리적으로 '분해'하여, 구조 분석을 진행한다. 성공 사례가 없을수록, 실험의 가치는 그만큼 크다고 볼 수 있다.",
		condition = 0,
		bg = "3",
		blueprint_version = 4,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "gold_3",
		label_color = "tag_h",
		consume = {
			{
				2,
				20001,
				10
			}
		},
		drop_client = {
			{
				2,
				52031,
				1
			},
			{
				2,
				54051,
				1
			},
			{
				2,
				15008,
				1
			}
		}
	},
	[1313] = {
		id = 1313,
		rarity = 3,
		time = 1800,
		type = 2,
		name = "D-008-UL",
		lv_limit = 30,
		desc = "성정 큐브의 인식 구현화 실험 중, 이산치가 폭증하는 이상 현상이 발생하였다. 원리를 밝혀내면, 성정 큐브에 관한 이론 구축에 큰 진전이 있을 것이다.",
		condition = 0,
		bg = "3",
		blueprint_version = 4,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "chara_ankeleiqi",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				5000
			},
			{
				2,
				20001,
				5
			}
		},
		drop_client = {
			{
				2,
				42031,
				1
			},
			{
				2,
				52032,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1314] = {
		id = 1314,
		rarity = 3,
		time = 1800,
		type = 2,
		name = "D-063-UL",
		lv_limit = 30,
		desc = "성정 큐브의 인식 구현화 실험 중, 이산치가 폭증하는 이상 현상이 발생하였다. 원리를 밝혀내면, 성정 큐브에 관한 이론 구축에 큰 진전이 있을 것이다.",
		condition = 0,
		bg = "4",
		blueprint_version = 4,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "chara_bailong",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				5000
			},
			{
				2,
				20001,
				5
			}
		},
		drop_client = {
			{
				2,
				42032,
				1
			},
			{
				2,
				52032,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1315] = {
		id = 1315,
		rarity = 3,
		time = 1800,
		type = 2,
		name = "D-075-UL",
		lv_limit = 30,
		desc = "성정 큐브의 인식 구현화 실험 중, 이산치가 폭증하는 이상 현상이 발생하였다. 원리를 밝혀내면, 성정 큐브에 관한 이론 구축에 큰 진전이 있을 것이다.",
		condition = 0,
		bg = "4",
		blueprint_version = 4,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "chara_aijier",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				5000
			},
			{
				2,
				20001,
				5
			}
		},
		drop_client = {
			{
				2,
				42033,
				1
			},
			{
				2,
				52032,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1316] = {
		id = 1316,
		rarity = 3,
		time = 1800,
		type = 2,
		name = "D-082-UL",
		lv_limit = 30,
		desc = "성정 큐브의 인식 구현화 실험 중, 이산치가 폭증하는 이상 현상이 발생하였다. 원리를 밝혀내면, 성정 큐브에 관한 이론 구축에 큰 진전이 있을 것이다.",
		condition = 0,
		bg = "3",
		blueprint_version = 4,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "chara_aogusite",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				5000
			},
			{
				2,
				20001,
				5
			}
		},
		drop_client = {
			{
				2,
				42034,
				1
			},
			{
				2,
				52032,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1317] = {
		id = 1317,
		rarity = 3,
		time = 1800,
		type = 2,
		name = "D-068-UL",
		lv_limit = 30,
		desc = "성정 큐브의 인식 구현화 실험 중, 이산치가 폭증하는 이상 현상이 발생하였다. 원리를 밝혀내면, 성정 큐브에 관한 이론 구축에 큰 진전이 있을 것이다.",
		condition = 0,
		bg = "3",
		blueprint_version = 4,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "chara_makeboluo",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				5000
			},
			{
				2,
				20001,
				5
			}
		},
		drop_client = {
			{
				2,
				42035,
				1
			},
			{
				2,
				52032,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1319] = {
		id = 1319,
		rarity = 3,
		time = 1800,
		type = 2,
		name = "Q-002-UL",
		lv_limit = 30,
		desc = "성정 큐브와 장비의 공명 실험에서, 특정 부품이 매우 높은 싱크로율을 보였다. 분명 지금까지 밝히지 못한 미지의 기술이 담겨 있을 것이다.",
		condition = 0,
		bg = "3",
		blueprint_version = 4,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_3",
		label_color = "tag_q",
		consume = {
			{
				1,
				1,
				5000
			},
			{
				2,
				17003,
				20
			}
		},
		drop_client = {
			{
				2,
				52032,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1320] = {
		id = 1320,
		rarity = 3,
		time = 1800,
		type = 2,
		name = "Q-010-UL",
		lv_limit = 30,
		desc = "성정 큐브와 장비의 공명 실험에서, 특정 부품이 매우 높은 싱크로율을 보였다. 분명 지금까지 밝히지 못한 미지의 기술이 담겨 있을 것이다.",
		condition = 0,
		bg = "3",
		blueprint_version = 4,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_3",
		label_color = "tag_q",
		consume = {
			{
				1,
				1,
				5000
			},
			{
				2,
				17013,
				20
			}
		},
		drop_client = {
			{
				2,
				52032,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1321] = {
		id = 1321,
		rarity = 3,
		time = 1800,
		type = 2,
		name = "Q-051-UL",
		lv_limit = 30,
		desc = "성정 큐브와 장비의 공명 실험에서, 특정 부품이 매우 높은 싱크로율을 보였다. 분명 지금까지 밝히지 못한 미지의 기술이 담겨 있을 것이다.",
		condition = 0,
		bg = "3",
		blueprint_version = 4,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_3",
		label_color = "tag_q",
		consume = {
			{
				1,
				1,
				5000
			},
			{
				2,
				17023,
				20
			}
		},
		drop_client = {
			{
				2,
				52032,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1322] = {
		id = 1322,
		rarity = 3,
		time = 1800,
		type = 2,
		name = "Q-068-UL",
		lv_limit = 30,
		desc = "성정 큐브와 장비의 공명 실험에서, 특정 부품이 매우 높은 싱크로율을 보였다. 분명 지금까지 밝히지 못한 미지의 기술이 담겨 있을 것이다.",
		condition = 0,
		bg = "3",
		blueprint_version = 4,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_3",
		label_color = "tag_q",
		consume = {
			{
				1,
				1,
				5000
			},
			{
				2,
				17033,
				20
			}
		},
		drop_client = {
			{
				2,
				52032,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1323] = {
		id = 1323,
		rarity = 3,
		time = 1800,
		type = 2,
		name = "Q-089-UL",
		lv_limit = 30,
		desc = "성정 큐브와 장비의 공명 실험에서, 특정 부품이 매우 높은 싱크로율을 보였다. 분명 지금까지 밝히지 못한 미지의 기술이 담겨 있을 것이다.",
		condition = 0,
		bg = "3",
		blueprint_version = 4,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_3",
		label_color = "tag_q",
		consume = {
			{
				1,
				1,
				5000
			},
			{
				2,
				17043,
				20
			}
		},
		drop_client = {
			{
				2,
				52032,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1324] = {
		id = 1324,
		rarity = 3,
		time = 1800,
		type = 2,
		name = "H-063-UL",
		lv_limit = 30,
		desc = "상부에서 극비사항으로 분류되는 연구 프로젝트. 이로 인해 인류뿐만 아니라, 전세계의 생태계 자체에 상당한 영향이 갈지도 모른다.",
		condition = 0,
		bg = "3",
		blueprint_version = 4,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_3",
		label_color = "tag_sh",
		consume = {
			{
				1,
				1,
				8000
			},
			{
				2,
				20001,
				3
			}
		},
		drop_client = {
			{
				2,
				52031,
				1
			},
			{
				2,
				54051,
				1
			},
			{
				2,
				15008,
				1
			}
		}
	},
	[1325] = {
		id = 1325,
		rarity = 3,
		time = 1800,
		type = 2,
		name = "H-063-UL",
		lv_limit = 30,
		desc = "상부에서 극비사항으로 분류되는 연구 프로젝트. 이로 인해 인류뿐만 아니라, 전세계의 생태계 자체에 상당한 영향이 갈지도 모른다.",
		condition = 0,
		bg = "3",
		blueprint_version = 4,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_3",
		label_color = "tag_sh",
		consume = {
			{
				1,
				1,
				8000
			},
			{
				2,
				20001,
				3
			}
		},
		drop_client = {
			{
				2,
				52031,
				1
			},
			{
				2,
				54051,
				1
			},
			{
				2,
				15008,
				1
			}
		}
	},
	[1326] = {
		id = 1326,
		rarity = 3,
		time = 14400,
		type = 2,
		name = "B-397-RF",
		lv_limit = 85,
		desc = "성정 큐브 구조에 관한 여러가지 가설에 대해 실증실험을 진행하는 연구 프로젝트. 실전을 통해 데이터를 수집하여, 이론의 과학적 증명에 기여하자.",
		condition = 52011,
		bg = "3",
		blueprint_version = 4,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "battle_3",
		label_color = "tag_b",
		consume = {},
		drop_client = {
			{
				2,
				52031,
				1
			},
			{
				2,
				52032,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1331] = {
		id = 1331,
		rarity = 3,
		time = 7200,
		type = 2,
		name = "E-875-MI",
		lv_limit = 80,
		desc = "현재 비치 중인 장비를 분해함으로써 기초 설계를 재검토하여, 추후 개발이 예정된 새 장비의 기초 연구를 진행한다.",
		condition = 0,
		bg = "3",
		blueprint_version = 4,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "item_3",
		label_color = "tag_e",
		consume = {
			{
				2,
				10080,
				45
			}
		},
		drop_client = {
			{
				2,
				13025,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[1332] = {
		id = 1332,
		rarity = 3,
		time = 7200,
		type = 2,
		name = "E-882-MI",
		lv_limit = 80,
		desc = "현재 비치 중인 장비를 분해함으로써 기초 설계를 재검토하여, 추후 개발이 예정된 새 장비의 기초 연구를 진행한다.",
		condition = 0,
		bg = "3",
		blueprint_version = 4,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "item_3",
		label_color = "tag_e",
		consume = {
			{
				2,
				10084,
				45
			}
		},
		drop_client = {
			{
				2,
				13026,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[1333] = {
		id = 1333,
		rarity = 3,
		time = 7200,
		type = 2,
		name = "E-868-MI",
		lv_limit = 80,
		desc = "현재 비치 중인 장비를 분해함으로써 기초 설계를 재검토하여, 추후 개발이 예정된 새 장비의 기초 연구를 진행한다.",
		condition = 0,
		bg = "3",
		blueprint_version = 4,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "item_3",
		label_color = "tag_e",
		consume = {
			{
				2,
				10109,
				45
			}
		},
		drop_client = {
			{
				2,
				13027,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[1334] = {
		id = 1334,
		rarity = 3,
		time = 7200,
		type = 2,
		name = "E-808-MI",
		lv_limit = 80,
		desc = "현재 비치 중인 장비를 분해함으로써 기초 설계를 재검토하여, 추후 개발이 예정된 새 장비의 기초 연구를 진행한다.",
		condition = 0,
		bg = "3",
		blueprint_version = 4,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "item_3",
		label_color = "tag_e",
		consume = {
			{
				2,
				65,
				45
			}
		},
		drop_client = {
			{
				2,
				13028,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[1335] = {
		id = 1335,
		rarity = 3,
		time = 7200,
		type = 2,
		name = "E-863-MI",
		lv_limit = 80,
		desc = "현재 비치 중인 장비를 분해함으로써 기초 설계를 재검토하여, 추후 개발이 예정된 새 장비의 기초 연구를 진행한다.",
		condition = 0,
		bg = "3",
		blueprint_version = 4,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "item_3",
		label_color = "tag_e",
		consume = {
			{
				2,
				10068,
				45
			}
		},
		drop_client = {
			{
				2,
				13029,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[1401] = {
		id = 1401,
		rarity = 3,
		time = 43200,
		type = 2,
		name = "C-038-RF",
		lv_limit = 30,
		desc = "성정 큐브에 관한 기초 이론은 큐브 구조에 대한 인식으로부터 시작된다. 큐브 기초 구조에 대한 이론 구조 작업은 여전히 전력을 다해 진행 중이다.",
		condition = 0,
		bg = "3",
		blueprint_version = 5,
		sub_name = "대형 항목",
		label = "tag_red",
		bg_icon = "item_3",
		label_color = "tag_c",
		consume = {},
		drop_client = {
			{
				2,
				52061,
				1
			},
			{
				2,
				52062,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[1402] = {
		id = 1402,
		rarity = 3,
		time = 43200,
		type = 2,
		name = "C-038-RF",
		lv_limit = 30,
		desc = "성정 큐브에 관한 기초 이론은 큐브 구조에 대한 인식으로부터 시작된다. 큐브 기초 구조에 대한 이론 구조 작업은 여전히 전력을 다해 진행 중이다.",
		condition = 0,
		bg = "3",
		blueprint_version = 5,
		sub_name = "대형 항목",
		label = "tag_red",
		bg_icon = "item_3",
		label_color = "tag_c",
		consume = {},
		drop_client = {
			{
				2,
				52061,
				1
			},
			{
				2,
				52062,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[1403] = {
		id = 1403,
		rarity = 3,
		time = 14400,
		type = 2,
		name = "B-351-RF",
		lv_limit = 75,
		desc = "성정 큐브 구조에 관한 여러가지 가설에 대해 실증실험을 진행하는 연구 프로젝트. 실전을 통해 데이터를 수집하여, 이론의 과학적 증명에 기여하자.",
		condition = 52009,
		bg = "3",
		blueprint_version = 5,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "battle_3",
		label_color = "tag_b",
		consume = {},
		drop_client = {
			{
				2,
				52061,
				1
			},
			{
				2,
				52062,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1404] = {
		id = 1404,
		rarity = 3,
		time = 14400,
		type = 2,
		name = "B-397-RF",
		lv_limit = 80,
		desc = "성정 큐브 구조에 관한 여러가지 가설에 대해 실증실험을 진행하는 연구 프로젝트. 실전을 통해 데이터를 수집하여, 이론의 과학적 증명에 기여하자.",
		condition = 52010,
		bg = "3",
		blueprint_version = 5,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "battle_3",
		label_color = "tag_b",
		consume = {},
		drop_client = {
			{
				2,
				52061,
				1
			},
			{
				2,
				52062,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1405] = {
		id = 1405,
		rarity = 3,
		time = 28800,
		type = 2,
		name = "D-419-RF",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수의 사람들의 사물에 대한 인식의 집합체를 구현화하는 것이라면, 그것의 '방향성' 을 정하는 것에 따라 결과는 과연 어떻게 바뀔 것인가.",
		condition = 0,
		bg = "3",
		blueprint_version = 5,
		sub_name = "대형 항목",
		label = "tag_red",
		bg_icon = "chara_pulimaosi",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				8000
			}
		},
		drop_client = {
			{
				2,
				42041,
				1
			},
			{
				2,
				52062,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1406] = {
		id = 1406,
		rarity = 3,
		time = 28800,
		type = 2,
		name = "D-478-RF",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수의 사람들의 사물에 대한 인식의 집합체를 구현화하는 것이라면, 그것의 '방향성' 을 정하는 것에 따라 결과는 과연 어떻게 바뀔 것인가.",
		condition = 0,
		bg = "3",
		blueprint_version = 5,
		sub_name = "대형 항목",
		label = "tag_red",
		bg_icon = "chara_lupuleixiteqinwang",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				8000
			}
		},
		drop_client = {
			{
				2,
				42042,
				1
			},
			{
				2,
				52062,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1407] = {
		id = 1407,
		rarity = 3,
		time = 28800,
		type = 2,
		name = "D-486-RF",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수의 사람들의 사물에 대한 인식의 집합체를 구현화하는 것이라면, 그것의 '방향성' 을 정하는 것에 따라 결과는 과연 어떻게 바뀔 것인가.",
		condition = 0,
		bg = "3",
		blueprint_version = 5,
		sub_name = "대형 항목",
		label = "tag_red",
		bg_icon = "chara_haerbin",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				8000
			}
		},
		drop_client = {
			{
				2,
				42043,
				1
			},
			{
				2,
				52062,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1408] = {
		id = 1408,
		rarity = 3,
		time = 28800,
		type = 2,
		name = "D-488-RF",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수의 사람들의 사물에 대한 인식의 집합체를 구현화하는 것이라면, 그것의 '방향성' 을 정하는 것에 따라 결과는 과연 어떻게 바뀔 것인가.",
		condition = 0,
		bg = "3",
		blueprint_version = 5,
		sub_name = "대형 항목",
		label = "tag_red",
		bg_icon = "chara_qikaluofu",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				8000
			}
		},
		drop_client = {
			{
				2,
				42044,
				1
			},
			{
				2,
				52062,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1409] = {
		id = 1409,
		rarity = 3,
		time = 28800,
		type = 2,
		name = "D-493-RF",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수의 사람들의 사물에 대한 인식의 집합체를 구현화하는 것이라면, 그것의 '방향성' 을 정하는 것에 따라 결과는 과연 어떻게 바뀔 것인가.",
		condition = 0,
		bg = "3",
		blueprint_version = 5,
		sub_name = "대형 항목",
		label = "tag_red",
		bg_icon = "chara_buleisite",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				8000
			}
		},
		drop_client = {
			{
				2,
				42045,
				1
			},
			{
				2,
				52062,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1411] = {
		id = 1411,
		rarity = 3,
		time = 14400,
		type = 2,
		name = "H-207-RF",
		lv_limit = 30,
		desc = "성정 큐브를 문자 그대로 물리적으로 '분해'하여, 구조 분석을 진행한다. 성공 사례가 없을수록, 실험의 가치는 그만큼 크다고 볼 수 있다.",
		condition = 0,
		bg = "3",
		blueprint_version = 5,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "gold_3",
		label_color = "tag_h",
		consume = {
			{
				2,
				20001,
				10
			}
		},
		drop_client = {
			{
				2,
				52061,
				1
			},
			{
				2,
				54051,
				1
			},
			{
				2,
				15008,
				1
			}
		}
	},
	[1412] = {
		id = 1412,
		rarity = 3,
		time = 14400,
		type = 2,
		name = "H-207-RF",
		lv_limit = 30,
		desc = "성정 큐브를 문자 그대로 물리적으로 '분해'하여, 구조 분석을 진행한다. 성공 사례가 없을수록, 실험의 가치는 그만큼 크다고 볼 수 있다.",
		condition = 0,
		bg = "3",
		blueprint_version = 5,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "gold_3",
		label_color = "tag_h",
		consume = {
			{
				2,
				20001,
				10
			}
		},
		drop_client = {
			{
				2,
				52061,
				1
			},
			{
				2,
				54051,
				1
			},
			{
				2,
				15008,
				1
			}
		}
	},
	[1413] = {
		id = 1413,
		rarity = 3,
		time = 1800,
		type = 2,
		name = "D-019-UL",
		lv_limit = 30,
		desc = "성정 큐브의 인식 구현화 실험 중, 이산치가 폭증하는 이상 현상이 발생하였다. 원리를 밝혀내면, 성정 큐브에 관한 이론 구축에 큰 진전이 있을 것이다.",
		condition = 0,
		bg = "4",
		blueprint_version = 5,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "chara_pulimaosi",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				5000
			},
			{
				2,
				20001,
				5
			}
		},
		drop_client = {
			{
				2,
				42041,
				1
			},
			{
				2,
				52062,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1414] = {
		id = 1414,
		rarity = 3,
		time = 1800,
		type = 2,
		name = "D-078-UL",
		lv_limit = 30,
		desc = "성정 큐브의 인식 구현화 실험 중, 이산치가 폭증하는 이상 현상이 발생하였다. 원리를 밝혀내면, 성정 큐브에 관한 이론 구축에 큰 진전이 있을 것이다.",
		condition = 0,
		bg = "3",
		blueprint_version = 5,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "chara_lupuleixiteqinwang",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				5000
			},
			{
				2,
				20001,
				5
			}
		},
		drop_client = {
			{
				2,
				42042,
				1
			},
			{
				2,
				52062,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1415] = {
		id = 1415,
		rarity = 3,
		time = 1800,
		type = 2,
		name = "D-086-UL",
		lv_limit = 30,
		desc = "성정 큐브의 인식 구현화 실험 중, 이산치가 폭증하는 이상 현상이 발생하였다. 원리를 밝혀내면, 성정 큐브에 관한 이론 구축에 큰 진전이 있을 것이다.",
		condition = 0,
		bg = "3",
		blueprint_version = 5,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "chara_haerbin",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				5000
			},
			{
				2,
				20001,
				5
			}
		},
		drop_client = {
			{
				2,
				42043,
				1
			},
			{
				2,
				52062,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1416] = {
		id = 1416,
		rarity = 3,
		time = 1800,
		type = 2,
		name = "D-088-UL",
		lv_limit = 30,
		desc = "성정 큐브의 인식 구현화 실험 중, 이산치가 폭증하는 이상 현상이 발생하였다. 원리를 밝혀내면, 성정 큐브에 관한 이론 구축에 큰 진전이 있을 것이다.",
		condition = 0,
		bg = "3",
		blueprint_version = 5,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "chara_qikaluofu",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				5000
			},
			{
				2,
				20001,
				5
			}
		},
		drop_client = {
			{
				2,
				42044,
				1
			},
			{
				2,
				52062,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1417] = {
		id = 1417,
		rarity = 3,
		time = 1800,
		type = 2,
		name = "D-093-UL",
		lv_limit = 30,
		desc = "성정 큐브의 인식 구현화 실험 중, 이산치가 폭증하는 이상 현상이 발생하였다. 원리를 밝혀내면, 성정 큐브에 관한 이론 구축에 큰 진전이 있을 것이다.",
		condition = 0,
		bg = "4",
		blueprint_version = 5,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "chara_buleisite",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				5000
			},
			{
				2,
				20001,
				5
			}
		},
		drop_client = {
			{
				2,
				42045,
				1
			},
			{
				2,
				52062,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1419] = {
		id = 1419,
		rarity = 3,
		time = 1800,
		type = 2,
		name = "Q-002-UL",
		lv_limit = 30,
		desc = "성정 큐브와 장비의 공명 실험에서, 특정 부품이 매우 높은 싱크로율을 보였다. 분명 지금까지 밝히지 못한 미지의 기술이 담겨 있을 것이다.",
		condition = 0,
		bg = "3",
		blueprint_version = 5,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_3",
		label_color = "tag_q",
		consume = {
			{
				1,
				1,
				5000
			},
			{
				2,
				17003,
				20
			}
		},
		drop_client = {
			{
				2,
				52062,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1420] = {
		id = 1420,
		rarity = 3,
		time = 1800,
		type = 2,
		name = "Q-010-UL",
		lv_limit = 30,
		desc = "성정 큐브와 장비의 공명 실험에서, 특정 부품이 매우 높은 싱크로율을 보였다. 분명 지금까지 밝히지 못한 미지의 기술이 담겨 있을 것이다.",
		condition = 0,
		bg = "3",
		blueprint_version = 5,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_3",
		label_color = "tag_q",
		consume = {
			{
				1,
				1,
				5000
			},
			{
				2,
				17013,
				20
			}
		},
		drop_client = {
			{
				2,
				52062,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1421] = {
		id = 1421,
		rarity = 3,
		time = 1800,
		type = 2,
		name = "Q-051-UL",
		lv_limit = 30,
		desc = "성정 큐브와 장비의 공명 실험에서, 특정 부품이 매우 높은 싱크로율을 보였다. 분명 지금까지 밝히지 못한 미지의 기술이 담겨 있을 것이다.",
		condition = 0,
		bg = "3",
		blueprint_version = 5,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_3",
		label_color = "tag_q",
		consume = {
			{
				1,
				1,
				5000
			},
			{
				2,
				17023,
				20
			}
		},
		drop_client = {
			{
				2,
				52062,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1422] = {
		id = 1422,
		rarity = 3,
		time = 1800,
		type = 2,
		name = "Q-068-UL",
		lv_limit = 30,
		desc = "성정 큐브와 장비의 공명 실험에서, 특정 부품이 매우 높은 싱크로율을 보였다. 분명 지금까지 밝히지 못한 미지의 기술이 담겨 있을 것이다.",
		condition = 0,
		bg = "3",
		blueprint_version = 5,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_3",
		label_color = "tag_q",
		consume = {
			{
				1,
				1,
				5000
			},
			{
				2,
				17033,
				20
			}
		},
		drop_client = {
			{
				2,
				52062,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1423] = {
		id = 1423,
		rarity = 3,
		time = 1800,
		type = 2,
		name = "Q-089-UL",
		lv_limit = 30,
		desc = "성정 큐브와 장비의 공명 실험에서, 특정 부품이 매우 높은 싱크로율을 보였다. 분명 지금까지 밝히지 못한 미지의 기술이 담겨 있을 것이다.",
		condition = 0,
		bg = "3",
		blueprint_version = 5,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_3",
		label_color = "tag_q",
		consume = {
			{
				1,
				1,
				5000
			},
			{
				2,
				17043,
				20
			}
		},
		drop_client = {
			{
				2,
				52062,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1424] = {
		id = 1424,
		rarity = 3,
		time = 1800,
		type = 2,
		name = "H-063-UL",
		lv_limit = 30,
		desc = "상부에서 극비사항으로 분류되는 연구 프로젝트. 이로 인해 인류뿐만 아니라, 전세계의 생태계 자체에 상당한 영향이 갈지도 모른다.",
		condition = 0,
		bg = "3",
		blueprint_version = 5,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_3",
		label_color = "tag_sh",
		consume = {
			{
				1,
				1,
				8000
			},
			{
				2,
				20001,
				3
			}
		},
		drop_client = {
			{
				2,
				52061,
				1
			},
			{
				2,
				54051,
				1
			},
			{
				2,
				15008,
				1
			}
		}
	},
	[1425] = {
		id = 1425,
		rarity = 3,
		time = 1800,
		type = 2,
		name = "H-063-UL",
		lv_limit = 30,
		desc = "상부에서 극비사항으로 분류되는 연구 프로젝트. 이로 인해 인류뿐만 아니라, 전세계의 생태계 자체에 상당한 영향이 갈지도 모른다.",
		condition = 0,
		bg = "3",
		blueprint_version = 5,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_3",
		label_color = "tag_sh",
		consume = {
			{
				1,
				1,
				8000
			},
			{
				2,
				20001,
				3
			}
		},
		drop_client = {
			{
				2,
				52061,
				1
			},
			{
				2,
				54051,
				1
			},
			{
				2,
				15008,
				1
			}
		}
	},
	[1426] = {
		id = 1426,
		rarity = 3,
		time = 14400,
		type = 2,
		name = "B-397-RF",
		lv_limit = 85,
		desc = "성정 큐브 구조에 관한 여러가지 가설에 대해 실증실험을 진행하는 연구 프로젝트. 실전을 통해 데이터를 수집하여, 이론의 과학적 증명에 기여하자.",
		condition = 52011,
		bg = "3",
		blueprint_version = 5,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "battle_3",
		label_color = "tag_b",
		consume = {},
		drop_client = {
			{
				2,
				52061,
				1
			},
			{
				2,
				52062,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1431] = {
		id = 1431,
		rarity = 3,
		time = 7200,
		type = 2,
		name = "E-893-MI",
		lv_limit = 80,
		desc = "현재 비치 중인 장비를 분해함으로써 기초 설계를 재검토하여, 추후 개발이 예정된 새 장비의 기초 연구를 진행한다.",
		condition = 0,
		bg = "3",
		blueprint_version = 5,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "item_3",
		label_color = "tag_e",
		consume = {
			{
				2,
				10102,
				45
			}
		},
		drop_client = {
			{
				2,
				13032,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[1432] = {
		id = 1432,
		rarity = 3,
		time = 7200,
		type = 2,
		name = "E-878-MI",
		lv_limit = 80,
		desc = "현재 비치 중인 장비를 분해함으로써 기초 설계를 재검토하여, 추후 개발이 예정된 새 장비의 기초 연구를 진행한다.",
		condition = 0,
		bg = "3",
		blueprint_version = 5,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "item_3",
		label_color = "tag_e",
		consume = {
			{
				2,
				10077,
				45
			}
		},
		drop_client = {
			{
				2,
				13035,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[1433] = {
		id = 1433,
		rarity = 3,
		time = 7200,
		type = 2,
		name = "E-886-MI",
		lv_limit = 80,
		desc = "현재 비치 중인 장비를 분해함으로써 기초 설계를 재검토하여, 추후 개발이 예정된 새 장비의 기초 연구를 진행한다.",
		condition = 0,
		bg = "3",
		blueprint_version = 5,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "item_3",
		label_color = "tag_e",
		consume = {
			{
				2,
				10122,
				45
			}
		},
		drop_client = {
			{
				2,
				13036,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[1501] = {
		id = 1501,
		rarity = 3,
		time = 43200,
		type = 2,
		name = "C-038-RF",
		lv_limit = 30,
		desc = "성정 큐브에 관한 기초 이론은 큐브 구조에 대한 인식으로부터 시작된다. 큐브 기초 구조에 대한 이론 구조 작업은 여전히 전력을 다해 진행 중이다.",
		condition = 0,
		bg = "3",
		blueprint_version = 6,
		sub_name = "대형 항목",
		label = "tag_red",
		bg_icon = "item_3",
		label_color = "tag_c",
		consume = {},
		drop_client = {
			{
				2,
				52071,
				1
			},
			{
				2,
				52072,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[1502] = {
		id = 1502,
		rarity = 3,
		time = 43200,
		type = 2,
		name = "C-038-RF",
		lv_limit = 30,
		desc = "성정 큐브에 관한 기초 이론은 큐브 구조에 대한 인식으로부터 시작된다. 큐브 기초 구조에 대한 이론 구조 작업은 여전히 전력을 다해 진행 중이다.",
		condition = 0,
		bg = "3",
		blueprint_version = 6,
		sub_name = "대형 항목",
		label = "tag_red",
		bg_icon = "item_3",
		label_color = "tag_c",
		consume = {},
		drop_client = {
			{
				2,
				52071,
				1
			},
			{
				2,
				52072,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[1503] = {
		id = 1503,
		rarity = 3,
		time = 14400,
		type = 2,
		name = "B-351-RF",
		lv_limit = 75,
		desc = "성정 큐브 구조에 관한 여러가지 가설에 대해 실증실험을 진행하는 연구 프로젝트. 실전을 통해 데이터를 수집하여, 이론의 과학적 증명에 기여하자.",
		condition = 52009,
		bg = "3",
		blueprint_version = 6,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "battle_3",
		label_color = "tag_b",
		consume = {},
		drop_client = {
			{
				2,
				52071,
				1
			},
			{
				2,
				52072,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1504] = {
		id = 1504,
		rarity = 3,
		time = 14400,
		type = 2,
		name = "B-397-RF",
		lv_limit = 80,
		desc = "성정 큐브 구조에 관한 여러가지 가설에 대해 실증실험을 진행하는 연구 프로젝트. 실전을 통해 데이터를 수집하여, 이론의 과학적 증명에 기여하자.",
		condition = 52010,
		bg = "3",
		blueprint_version = 6,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "battle_3",
		label_color = "tag_b",
		consume = {},
		drop_client = {
			{
				2,
				52071,
				1
			},
			{
				2,
				52072,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1505] = {
		id = 1505,
		rarity = 3,
		time = 28800,
		type = 2,
		name = "D-483-RF",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수의 사람들의 사물에 대한 인식의 집합체를 구현화하는 것이라면, 그것의 '방향성' 을 정하는 것에 따라 결과는 과연 어떻게 바뀔 것인가.",
		condition = 0,
		bg = "3",
		blueprint_version = 6,
		sub_name = "대형 항목",
		label = "tag_red",
		bg_icon = "chara_xingdengbao",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				8000
			}
		},
		drop_client = {
			{
				2,
				42054,
				1
			},
			{
				2,
				52072,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1506] = {
		id = 1506,
		rarity = 3,
		time = 28800,
		type = 2,
		name = "D-413-RF",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수의 사람들의 사물에 대한 인식의 집합체를 구현화하는 것이라면, 그것의 '방향성' 을 정하는 것에 따라 결과는 과연 어떻게 바뀔 것인가.",
		condition = 0,
		bg = "3",
		blueprint_version = 6,
		sub_name = "대형 항목",
		label = "tag_red",
		bg_icon = "chara_qiershazhi",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				8000
			}
		},
		drop_client = {
			{
				2,
				42051,
				1
			},
			{
				2,
				52072,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1507] = {
		id = 1507,
		rarity = 3,
		time = 28800,
		type = 2,
		name = "D-439-RF",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수의 사람들의 사물에 대한 인식의 집합체를 구현화하는 것이라면, 그것의 '방향성' 을 정하는 것에 따라 결과는 과연 어떻게 바뀔 것인가.",
		condition = 0,
		bg = "3",
		blueprint_version = 6,
		sub_name = "대형 항목",
		label = "tag_red",
		bg_icon = "chara_siwanshi",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				8000
			}
		},
		drop_client = {
			{
				2,
				42052,
				1
			},
			{
				2,
				52072,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1508] = {
		id = 1508,
		rarity = 3,
		time = 28800,
		type = 2,
		name = "D-471-RF",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수의 사람들의 사물에 대한 인식의 집합체를 구현화하는 것이라면, 그것의 '방향성' 을 정하는 것에 따라 결과는 과연 어떻게 바뀔 것인가.",
		condition = 0,
		bg = "3",
		blueprint_version = 6,
		sub_name = "대형 항목",
		label = "tag_red",
		bg_icon = "chara_feilikesishuerci",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				8000
			}
		},
		drop_client = {
			{
				2,
				42053,
				1
			},
			{
				2,
				52072,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1509] = {
		id = 1509,
		rarity = 3,
		time = 28800,
		type = 2,
		name = "D-496-RF",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수의 사람들의 사물에 대한 인식의 집합체를 구현화하는 것이라면, 그것의 '방향성' 을 정하는 것에 따라 결과는 과연 어떻게 바뀔 것인가.",
		condition = 0,
		bg = "3",
		blueprint_version = 6,
		sub_name = "대형 항목",
		label = "tag_red",
		bg_icon = "chara_fulandeer",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				8000
			}
		},
		drop_client = {
			{
				2,
				42055,
				1
			},
			{
				2,
				52072,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1511] = {
		id = 1511,
		rarity = 3,
		time = 14400,
		type = 2,
		name = "H-207-RF",
		lv_limit = 30,
		desc = "성정 큐브를 문자 그대로 물리적으로 '분해'하여, 구조 분석을 진행한다. 성공 사례가 없을수록, 실험의 가치는 그만큼 크다고 볼 수 있다.",
		condition = 0,
		bg = "3",
		blueprint_version = 6,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "gold_3",
		label_color = "tag_h",
		consume = {
			{
				2,
				20001,
				10
			}
		},
		drop_client = {
			{
				2,
				52071,
				1
			},
			{
				2,
				54051,
				1
			},
			{
				2,
				15008,
				1
			}
		}
	},
	[1512] = {
		id = 1512,
		rarity = 3,
		time = 14400,
		type = 2,
		name = "H-207-RF",
		lv_limit = 30,
		desc = "성정 큐브를 문자 그대로 물리적으로 '분해'하여, 구조 분석을 진행한다. 성공 사례가 없을수록, 실험의 가치는 그만큼 크다고 볼 수 있다.",
		condition = 0,
		bg = "3",
		blueprint_version = 6,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "gold_3",
		label_color = "tag_h",
		consume = {
			{
				2,
				20001,
				10
			}
		},
		drop_client = {
			{
				2,
				52071,
				1
			},
			{
				2,
				54051,
				1
			},
			{
				2,
				15008,
				1
			}
		}
	},
	[1513] = {
		id = 1513,
		rarity = 3,
		time = 1800,
		type = 2,
		name = "D-083-UL",
		lv_limit = 30,
		desc = "성정 큐브의 인식 구현화 실험 중, 이산치가 폭증하는 이상 현상이 발생하였다. 원리를 밝혀내면, 성정 큐브에 관한 이론 구축에 큰 진전이 있을 것이다.",
		condition = 0,
		bg = "4",
		blueprint_version = 6,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "chara_xingdengbao",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				5000
			},
			{
				2,
				20001,
				5
			}
		},
		drop_client = {
			{
				2,
				42054,
				1
			},
			{
				2,
				52072,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1514] = {
		id = 1514,
		rarity = 3,
		time = 1800,
		type = 2,
		name = "D-013-UL",
		lv_limit = 30,
		desc = "성정 큐브의 인식 구현화 실험 중, 이산치가 폭증하는 이상 현상이 발생하였다. 원리를 밝혀내면, 성정 큐브에 관한 이론 구축에 큰 진전이 있을 것이다.",
		condition = 0,
		bg = "4",
		blueprint_version = 6,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "chara_qiershazhi",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				5000
			},
			{
				2,
				20001,
				5
			}
		},
		drop_client = {
			{
				2,
				42051,
				1
			},
			{
				2,
				52072,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1515] = {
		id = 1515,
		rarity = 3,
		time = 1800,
		type = 2,
		name = "D-039-UL",
		lv_limit = 30,
		desc = "성정 큐브의 인식 구현화 실험 중, 이산치가 폭증하는 이상 현상이 발생하였다. 원리를 밝혀내면, 성정 큐브에 관한 이론 구축에 큰 진전이 있을 것이다.",
		condition = 0,
		bg = "3",
		blueprint_version = 6,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "chara_siwanshi",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				5000
			},
			{
				2,
				20001,
				5
			}
		},
		drop_client = {
			{
				2,
				42052,
				1
			},
			{
				2,
				52072,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1516] = {
		id = 1516,
		rarity = 3,
		time = 1800,
		type = 2,
		name = "D-071-UL",
		lv_limit = 30,
		desc = "성정 큐브의 인식 구현화 실험 중, 이산치가 폭증하는 이상 현상이 발생하였다. 원리를 밝혀내면, 성정 큐브에 관한 이론 구축에 큰 진전이 있을 것이다.",
		condition = 0,
		bg = "3",
		blueprint_version = 6,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "chara_feilikesishuerci",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				5000
			},
			{
				2,
				20001,
				5
			}
		},
		drop_client = {
			{
				2,
				42053,
				1
			},
			{
				2,
				52072,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1517] = {
		id = 1517,
		rarity = 3,
		time = 1800,
		type = 2,
		name = "D-096-UL",
		lv_limit = 30,
		desc = "성정 큐브의 인식 구현화 실험 중, 이산치가 폭증하는 이상 현상이 발생하였다. 원리를 밝혀내면, 성정 큐브에 관한 이론 구축에 큰 진전이 있을 것이다.",
		condition = 0,
		bg = "3",
		blueprint_version = 6,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "chara_fulandeer",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				5000
			},
			{
				2,
				20001,
				5
			}
		},
		drop_client = {
			{
				2,
				42055,
				1
			},
			{
				2,
				52072,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1519] = {
		id = 1519,
		rarity = 3,
		time = 1800,
		type = 2,
		name = "Q-002-UL",
		lv_limit = 30,
		desc = "성정 큐브와 장비의 공명 실험에서, 특정 부품이 매우 높은 싱크로율을 보였다. 분명 지금까지 밝히지 못한 미지의 기술이 담겨 있을 것이다.",
		condition = 0,
		bg = "3",
		blueprint_version = 6,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_3",
		label_color = "tag_q",
		consume = {
			{
				1,
				1,
				5000
			},
			{
				2,
				17003,
				20
			}
		},
		drop_client = {
			{
				2,
				52072,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1520] = {
		id = 1520,
		rarity = 3,
		time = 1800,
		type = 2,
		name = "Q-010-UL",
		lv_limit = 30,
		desc = "성정 큐브와 장비의 공명 실험에서, 특정 부품이 매우 높은 싱크로율을 보였다. 분명 지금까지 밝히지 못한 미지의 기술이 담겨 있을 것이다.",
		condition = 0,
		bg = "3",
		blueprint_version = 6,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_3",
		label_color = "tag_q",
		consume = {
			{
				1,
				1,
				5000
			},
			{
				2,
				17013,
				20
			}
		},
		drop_client = {
			{
				2,
				52072,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1521] = {
		id = 1521,
		rarity = 3,
		time = 1800,
		type = 2,
		name = "Q-051-UL",
		lv_limit = 30,
		desc = "성정 큐브와 장비의 공명 실험에서, 특정 부품이 매우 높은 싱크로율을 보였다. 분명 지금까지 밝히지 못한 미지의 기술이 담겨 있을 것이다.",
		condition = 0,
		bg = "3",
		blueprint_version = 6,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_3",
		label_color = "tag_q",
		consume = {
			{
				1,
				1,
				5000
			},
			{
				2,
				17023,
				20
			}
		},
		drop_client = {
			{
				2,
				52072,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1522] = {
		id = 1522,
		rarity = 3,
		time = 1800,
		type = 2,
		name = "Q-068-UL",
		lv_limit = 30,
		desc = "성정 큐브와 장비의 공명 실험에서, 특정 부품이 매우 높은 싱크로율을 보였다. 분명 지금까지 밝히지 못한 미지의 기술이 담겨 있을 것이다.",
		condition = 0,
		bg = "3",
		blueprint_version = 6,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_3",
		label_color = "tag_q",
		consume = {
			{
				1,
				1,
				5000
			},
			{
				2,
				17033,
				20
			}
		},
		drop_client = {
			{
				2,
				52072,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1523] = {
		id = 1523,
		rarity = 3,
		time = 1800,
		type = 2,
		name = "Q-089-UL",
		lv_limit = 30,
		desc = "성정 큐브와 장비의 공명 실험에서, 특정 부품이 매우 높은 싱크로율을 보였다. 분명 지금까지 밝히지 못한 미지의 기술이 담겨 있을 것이다.",
		condition = 0,
		bg = "3",
		blueprint_version = 6,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_3",
		label_color = "tag_q",
		consume = {
			{
				1,
				1,
				5000
			},
			{
				2,
				17043,
				20
			}
		},
		drop_client = {
			{
				2,
				52072,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1524] = {
		id = 1524,
		rarity = 3,
		time = 1800,
		type = 2,
		name = "H-063-UL",
		lv_limit = 30,
		desc = "상부에서 극비사항으로 분류되는 연구 프로젝트. 이로 인해 인류뿐만 아니라, 전세계의 생태계 자체에 상당한 영향이 갈지도 모른다.",
		condition = 0,
		bg = "3",
		blueprint_version = 6,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_3",
		label_color = "tag_sh",
		consume = {
			{
				1,
				1,
				8000
			},
			{
				2,
				20001,
				3
			}
		},
		drop_client = {
			{
				2,
				52071,
				1
			},
			{
				2,
				54051,
				1
			},
			{
				2,
				15008,
				1
			}
		}
	},
	[1525] = {
		id = 1525,
		rarity = 3,
		time = 1800,
		type = 2,
		name = "H-063-UL",
		lv_limit = 30,
		desc = "상부에서 극비사항으로 분류되는 연구 프로젝트. 이로 인해 인류뿐만 아니라, 전세계의 생태계 자체에 상당한 영향이 갈지도 모른다.",
		condition = 0,
		bg = "3",
		blueprint_version = 6,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_3",
		label_color = "tag_sh",
		consume = {
			{
				1,
				1,
				8000
			},
			{
				2,
				20001,
				3
			}
		},
		drop_client = {
			{
				2,
				52071,
				1
			},
			{
				2,
				54051,
				1
			},
			{
				2,
				15008,
				1
			}
		}
	},
	[1526] = {
		id = 1526,
		rarity = 3,
		time = 14400,
		type = 2,
		name = "B-397-RF",
		lv_limit = 85,
		desc = "성정 큐브 구조에 관한 여러가지 가설에 대해 실증실험을 진행하는 연구 프로젝트. 실전을 통해 데이터를 수집하여, 이론의 과학적 증명에 기여하자.",
		condition = 52011,
		bg = "3",
		blueprint_version = 6,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "battle_3",
		label_color = "tag_b",
		consume = {},
		drop_client = {
			{
				2,
				52071,
				1
			},
			{
				2,
				52072,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1531] = {
		id = 1531,
		rarity = 3,
		time = 7200,
		type = 2,
		name = "E-813-MI",
		lv_limit = 80,
		desc = "현재 배치 중인 장비를 분해하여 기초 설계를 재검토하고, 장래에 개발이 예정된 신규 장비의 기초 연구를 실시한다. ",
		condition = 0,
		bg = "3",
		blueprint_version = 6,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "item_3",
		label_color = "tag_e",
		consume = {
			{
				2,
				10022,
				45
			}
		},
		drop_client = {
			{
				2,
				13037,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[1532] = {
		id = 1532,
		rarity = 3,
		time = 7200,
		type = 2,
		name = "E-839-MI",
		lv_limit = 80,
		desc = "현재 배치 중인 장비를 분해하여 기초 설계를 재검토하고, 장래에 개발이 예정된 신규 장비의 기초 연구를 실시한다. ",
		condition = 0,
		bg = "3",
		blueprint_version = 6,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "item_3",
		label_color = "tag_e",
		consume = {
			{
				2,
				10055,
				45
			}
		},
		drop_client = {
			{
				2,
				13039,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[1533] = {
		id = 1533,
		rarity = 3,
		time = 7200,
		type = 2,
		name = "E-871-MI",
		lv_limit = 80,
		desc = "현재 배치 중인 장비를 분해하여 기초 설계를 재검토하고, 장래에 개발이 예정된 신규 장비의 기초 연구를 실시한다. ",
		condition = 0,
		bg = "3",
		blueprint_version = 6,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "item_3",
		label_color = "tag_e",
		consume = {
			{
				2,
				10073,
				45
			}
		},
		drop_client = {
			{
				2,
				13040,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[1534] = {
		id = 1534,
		rarity = 3,
		time = 7200,
		type = 2,
		name = "E-896-MI",
		lv_limit = 80,
		desc = "현재 배치 중인 장비를 분해하여 기초 설계를 재검토하고, 장래에 개발이 예정된 신규 장비의 기초 연구를 실시한다. ",
		condition = 0,
		bg = "3",
		blueprint_version = 6,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "item_3",
		label_color = "tag_e",
		consume = {
			{
				2,
				10103,
				45
			}
		},
		drop_client = {
			{
				2,
				13041,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[1535] = {
		id = 1535,
		rarity = 3,
		time = 7200,
		type = 2,
		name = "E-884-MI",
		lv_limit = 80,
		desc = "현재 배치 중인 장비를 분해하여 기초 설계를 재검토하고, 장래에 개발이 예정된 신규 장비의 기초 연구를 실시한다. ",
		condition = 0,
		bg = "3",
		blueprint_version = 6,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "item_3",
		label_color = "tag_e",
		consume = {
			{
				2,
				10077,
				45
			}
		},
		drop_client = {
			{
				2,
				13042,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[1601] = {
		id = 1601,
		rarity = 3,
		time = 43200,
		type = 2,
		name = "C-038-RF",
		lv_limit = 30,
		desc = "성정 큐브에 관한 기초 이론은 큐브 구조에 대한 인식으로부터 시작된다. 큐브 기초 구조에 대한 이론 구조 작업은 여전히 전력을 다해 진행 중이다.",
		condition = 0,
		bg = "3",
		blueprint_version = 7,
		sub_name = "대형 항목",
		label = "tag_red",
		bg_icon = "item_3",
		label_color = "tag_c",
		consume = {},
		drop_client = {
			{
				2,
				52081,
				1
			},
			{
				2,
				52082,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[1602] = {
		id = 1602,
		rarity = 3,
		time = 43200,
		type = 2,
		name = "C-038-RF",
		lv_limit = 30,
		desc = "성정 큐브에 관한 기초 이론은 큐브 구조에 대한 인식으로부터 시작된다. 큐브 기초 구조에 대한 이론 구조 작업은 여전히 전력을 다해 진행 중이다.",
		condition = 0,
		bg = "3",
		blueprint_version = 7,
		sub_name = "대형 항목",
		label = "tag_red",
		bg_icon = "item_3",
		label_color = "tag_c",
		consume = {},
		drop_client = {
			{
				2,
				52081,
				1
			},
			{
				2,
				52082,
				1
			},
			{
				2,
				52003,
				1
			},
			{
				2,
				59001,
				1
			}
		}
	},
	[1603] = {
		id = 1603,
		rarity = 3,
		time = 14400,
		type = 2,
		name = "B-351-RF",
		lv_limit = 75,
		desc = "성정 큐브 구조에 관한 여러가지 가설에 대해 실증실험을 진행하는 연구 프로젝트. 실전을 통해 데이터를 수집하여, 이론의 과학적 증명에 기여하자.",
		condition = 52009,
		bg = "3",
		blueprint_version = 7,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "battle_3",
		label_color = "tag_b",
		consume = {},
		drop_client = {
			{
				2,
				52081,
				1
			},
			{
				2,
				52082,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1604] = {
		id = 1604,
		rarity = 3,
		time = 14400,
		type = 2,
		name = "B-397-RF",
		lv_limit = 80,
		desc = "성정 큐브 구조에 관한 여러가지 가설에 대해 실증실험을 진행하는 연구 프로젝트. 실전을 통해 데이터를 수집하여, 이론의 과학적 증명에 기여하자.",
		condition = 52010,
		bg = "3",
		blueprint_version = 7,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "battle_3",
		label_color = "tag_b",
		consume = {},
		drop_client = {
			{
				2,
				52081,
				1
			},
			{
				2,
				52082,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1605] = {
		id = 1605,
		rarity = 3,
		time = 28800,
		type = 2,
		name = "D-403-RF",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수의 사람들의 사물에 대한 인식의 집합체를 구현화하는 것이라면, 그것의 '방향성' 을 정하는 것에 따라 결과는 과연 어떻게 바뀔 것인가.",
		condition = 0,
		bg = "3",
		blueprint_version = 7,
		sub_name = "대형 항목",
		label = "tag_red",
		bg_icon = "chara_haerfude",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				8000
			}
		},
		drop_client = {
			{
				2,
				42061,
				1
			},
			{
				2,
				52082,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1606] = {
		id = 1606,
		rarity = 3,
		time = 28800,
		type = 2,
		name = "D-458-RF",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수의 사람들의 사물에 대한 인식의 집합체를 구현화하는 것이라면, 그것의 '방향성' 을 정하는 것에 따라 결과는 과연 어떻게 바뀔 것인가.",
		condition = 0,
		bg = "3",
		blueprint_version = 7,
		sub_name = "대형 항목",
		label = "tag_red",
		bg_icon = "chara_dashan",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				8000
			}
		},
		drop_client = {
			{
				2,
				42062,
				1
			},
			{
				2,
				52082,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1607] = {
		id = 1607,
		rarity = 3,
		time = 28800,
		type = 2,
		name = "D-466-RF",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수의 사람들의 사물에 대한 인식의 집합체를 구현화하는 것이라면, 그것의 '방향성' 을 정하는 것에 따라 결과는 과연 어떻게 바뀔 것인가.",
		condition = 0,
		bg = "3",
		blueprint_version = 7,
		sub_name = "대형 항목",
		label = "tag_red",
		bg_icon = "chara_nabulesi",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				8000
			}
		},
		drop_client = {
			{
				2,
				42063,
				1
			},
			{
				2,
				52082,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1608] = {
		id = 1608,
		rarity = 3,
		time = 28800,
		type = 2,
		name = "D-490-RF",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수의 사람들의 사물에 대한 인식의 집합체를 구현화하는 것이라면, 그것의 '방향성' 을 정하는 것에 따라 결과는 과연 어떻게 바뀔 것인가.",
		condition = 0,
		bg = "3",
		blueprint_version = 7,
		sub_name = "대형 항목",
		label = "tag_red",
		bg_icon = "chara_naximofuhaijunshangjiang",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				8000
			}
		},
		drop_client = {
			{
				2,
				42064,
				1
			},
			{
				2,
				52082,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1609] = {
		id = 1609,
		rarity = 3,
		time = 28800,
		type = 2,
		name = "D-497-RF",
		lv_limit = 30,
		desc = "성정 큐브가 불특정 다수의 사람들의 사물에 대한 인식의 집합체를 구현화하는 것이라면, 그것의 '방향성' 을 정하는 것에 따라 결과는 과연 어떻게 바뀔 것인가.",
		condition = 0,
		bg = "3",
		blueprint_version = 7,
		sub_name = "대형 항목",
		label = "tag_red",
		bg_icon = "chara_beiyade",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				8000
			}
		},
		drop_client = {
			{
				2,
				42065,
				1
			},
			{
				2,
				52082,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1611] = {
		id = 1611,
		rarity = 3,
		time = 14400,
		type = 2,
		name = "H-207-RF",
		lv_limit = 30,
		desc = "성정 큐브를 문자 그대로 물리적으로 '분해'하여, 구조 분석을 진행한다. 성공 사례가 없을수록, 실험의 가치는 그만큼 크다고 볼 수 있다.",
		condition = 0,
		bg = "3",
		blueprint_version = 7,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "gold_3",
		label_color = "tag_h",
		consume = {
			{
				2,
				20001,
				10
			}
		},
		drop_client = {
			{
				2,
				52081,
				1
			},
			{
				2,
				54051,
				1
			},
			{
				2,
				15008,
				1
			}
		}
	},
	[1612] = {
		id = 1612,
		rarity = 3,
		time = 14400,
		type = 2,
		name = "H-207-RF",
		lv_limit = 30,
		desc = "성정 큐브를 문자 그대로 물리적으로 '분해'하여, 구조 분석을 진행한다. 성공 사례가 없을수록, 실험의 가치는 그만큼 크다고 볼 수 있다.",
		condition = 0,
		bg = "3",
		blueprint_version = 7,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "gold_3",
		label_color = "tag_h",
		consume = {
			{
				2,
				20001,
				10
			}
		},
		drop_client = {
			{
				2,
				52081,
				1
			},
			{
				2,
				54051,
				1
			},
			{
				2,
				15008,
				1
			}
		}
	},
	[1613] = {
		id = 1613,
		rarity = 3,
		time = 1800,
		type = 2,
		name = "D-003-UL",
		lv_limit = 30,
		desc = "성정 큐브의 인식 구현 실험에서 갑자기 사고의 소산률이 상승하는 이상 현상이 발생했다. 원리를 규명해낸다면 성정 큐브에 관한 이론 구축에 큰 도움이 될 것이다.",
		condition = 0,
		bg = "3",
		blueprint_version = 7,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "chara_haerfude",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				5000
			},
			{
				2,
				20001,
				5
			}
		},
		drop_client = {
			{
				2,
				42061,
				1
			},
			{
				2,
				52082,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1614] = {
		id = 1614,
		rarity = 3,
		time = 1800,
		type = 2,
		name = "D-058-UL",
		lv_limit = 30,
		desc = "성정 큐브의 인식 구현 실험에서 갑자기 사고의 소산률이 상승하는 이상 현상이 발생했다. 원리를 규명해낸다면 성정 큐브에 관한 이론 구축에 큰 도움이 될 것이다.",
		condition = 0,
		bg = "3",
		blueprint_version = 7,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "chara_dashan",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				5000
			},
			{
				2,
				20001,
				5
			}
		},
		drop_client = {
			{
				2,
				42062,
				1
			},
			{
				2,
				52082,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1615] = {
		id = 1615,
		rarity = 4,
		time = 1800,
		type = 2,
		name = "D-066-UL",
		lv_limit = 30,
		desc = "성정 큐브의 인식 구현 실험에서 갑자기 사고의 소산률이 상승하는 이상 현상이 발생했다. 원리를 규명해낸다면 성정 큐브에 관한 이론 구축에 큰 도움이 될 것이다.",
		condition = 0,
		bg = "4",
		blueprint_version = 7,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "chara_nabulesi",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				5000
			},
			{
				2,
				20001,
				5
			}
		},
		drop_client = {
			{
				2,
				42063,
				1
			},
			{
				2,
				52082,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1616] = {
		id = 1616,
		rarity = 4,
		time = 1800,
		type = 2,
		name = "D-090-UL",
		lv_limit = 30,
		desc = "성정 큐브의 인식 구현 실험에서 갑자기 사고의 소산률이 상승하는 이상 현상이 발생했다. 원리를 규명해낸다면 성정 큐브에 관한 이론 구축에 큰 도움이 될 것이다.",
		condition = 0,
		bg = "4",
		blueprint_version = 7,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "chara_naximofuhaijunshangjiang",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				5000
			},
			{
				2,
				20001,
				5
			}
		},
		drop_client = {
			{
				2,
				42064,
				1
			},
			{
				2,
				52082,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1617] = {
		id = 1617,
		rarity = 3,
		time = 1800,
		type = 2,
		name = "D-097-UL",
		lv_limit = 30,
		desc = "성정 큐브의 인식 구현 실험에서 갑자기 사고의 소산률이 상승하는 이상 현상이 발생했다. 원리를 규명해낸다면 성정 큐브에 관한 이론 구축에 큰 도움이 될 것이다.",
		condition = 0,
		bg = "3",
		blueprint_version = 7,
		sub_name = "소형 항목",
		label = "tag_red",
		bg_icon = "chara_beiyade",
		label_color = "tag_sp",
		consume = {
			{
				1,
				1,
				5000
			},
			{
				2,
				20001,
				5
			}
		},
		drop_client = {
			{
				2,
				42065,
				1
			},
			{
				2,
				52082,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1619] = {
		id = 1619,
		rarity = 3,
		time = 1800,
		type = 2,
		name = "Q-002-UL",
		lv_limit = 30,
		desc = "성정 큐브와 의장의 공명 실험에서 어떤 부품이 큰 싱크로율을 보였다. 지금까지 해명해 내지 못한 미지의 기술이 포함되어 있는 것으로 보인다.",
		condition = 0,
		bg = "3",
		blueprint_version = 7,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_3",
		label_color = "tag_q",
		consume = {
			{
				1,
				1,
				5000
			},
			{
				2,
				17003,
				20
			}
		},
		drop_client = {
			{
				2,
				52082,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1620] = {
		id = 1620,
		rarity = 3,
		time = 1800,
		type = 2,
		name = "Q-010-UL",
		lv_limit = 30,
		desc = "성정 큐브와 의장의 공명 실험에서 어떤 부품이 큰 싱크로율을 보였다. 지금까지 해명해 내지 못한 미지의 기술이 포함되어 있는 것으로 보인다.",
		condition = 0,
		bg = "3",
		blueprint_version = 7,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_3",
		label_color = "tag_q",
		consume = {
			{
				1,
				1,
				5000
			},
			{
				2,
				17013,
				20
			}
		},
		drop_client = {
			{
				2,
				52082,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1621] = {
		id = 1621,
		rarity = 3,
		time = 1800,
		type = 2,
		name = "Q-051-UL",
		lv_limit = 30,
		desc = "성정 큐브와 의장의 공명 실험에서 어떤 부품이 큰 싱크로율을 보였다. 지금까지 해명해 내지 못한 미지의 기술이 포함되어 있는 것으로 보인다.",
		condition = 0,
		bg = "3",
		blueprint_version = 7,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_3",
		label_color = "tag_q",
		consume = {
			{
				1,
				1,
				5000
			},
			{
				2,
				17023,
				20
			}
		},
		drop_client = {
			{
				2,
				52082,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1622] = {
		id = 1622,
		rarity = 3,
		time = 1800,
		type = 2,
		name = "Q-068-UL",
		lv_limit = 30,
		desc = "성정 큐브와 의장의 공명 실험에서 어떤 부품이 큰 싱크로율을 보였다. 지금까지 해명해 내지 못한 미지의 기술이 포함되어 있는 것으로 보인다.",
		condition = 0,
		bg = "3",
		blueprint_version = 7,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_3",
		label_color = "tag_q",
		consume = {
			{
				1,
				1,
				5000
			},
			{
				2,
				17033,
				20
			}
		},
		drop_client = {
			{
				2,
				52082,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1623] = {
		id = 1623,
		rarity = 3,
		time = 1800,
		type = 2,
		name = "Q-089-UL",
		lv_limit = 30,
		desc = "성정 큐브와 의장의 공명 실험에서 어떤 부품이 큰 싱크로율을 보였다. 지금까지 해명해 내지 못한 미지의 기술이 포함되어 있는 것으로 보인다.",
		condition = 0,
		bg = "3",
		blueprint_version = 7,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_3",
		label_color = "tag_q",
		consume = {
			{
				1,
				1,
				5000
			},
			{
				2,
				17043,
				20
			}
		},
		drop_client = {
			{
				2,
				52082,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	[1624] = {
		id = 1624,
		rarity = 3,
		time = 1800,
		type = 2,
		name = "H-063-UL",
		lv_limit = 30,
		desc = "상층부에서 극비로 다뤄지고 있는 연구 프로젝트. 이로 인해 인류뿐만 아니라 세계 생태계 자체에 절대적인 영향을 미치게 될 것이다.",
		condition = 0,
		bg = "3",
		blueprint_version = 7,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_3",
		label_color = "tag_sh",
		consume = {
			{
				1,
				1,
				8000
			},
			{
				2,
				20001,
				3
			}
		},
		drop_client = {
			{
				2,
				52081,
				1
			},
			{
				2,
				54051,
				1
			},
			{
				2,
				15008,
				1
			}
		}
	},
	[1625] = {
		id = 1625,
		rarity = 3,
		time = 1800,
		type = 2,
		name = "H-063-UL",
		lv_limit = 30,
		desc = "상층부에서 극비로 다뤄지고 있는 연구 프로젝트. 이로 인해 인류뿐만 아니라 세계 생태계 자체에 절대적인 영향을 미치게 될 것이다.",
		condition = 0,
		bg = "3",
		blueprint_version = 7,
		sub_name = "소형 항목",
		label = "tag_blue",
		bg_icon = "gold_3",
		label_color = "tag_sh",
		consume = {
			{
				1,
				1,
				8000
			},
			{
				2,
				20001,
				3
			}
		},
		drop_client = {
			{
				2,
				52081,
				1
			},
			{
				2,
				54051,
				1
			},
			{
				2,
				15008,
				1
			}
		}
	},
	[1626] = {
		id = 1626,
		rarity = 3,
		time = 14400,
		type = 2,
		name = "B-397-RF",
		lv_limit = 85,
		desc = "성정 큐브 구조에 관한 여러가지 가설에 대해 실증실험을 진행하는 연구 프로젝트. 실전을 통해 데이터를 수집하여, 이론의 과학적 증명에 기여하자.",
		condition = 52011,
		bg = "3",
		blueprint_version = 7,
		sub_name = "중형 항목",
		label = "tag_red",
		bg_icon = "battle_3",
		label_color = "tag_b",
		consume = {},
		drop_client = {
			{
				2,
				52081,
				1
			},
			{
				2,
				52082,
				1
			},
			{
				2,
				52003,
				1
			}
		}
	},
	all = {
		1,
		2,
		3,
		4,
		11,
		12,
		13,
		14,
		15,
		16,
		17,
		18,
		21,
		22,
		23,
		31,
		32,
		33,
		34,
		35,
		36,
		37,
		38,
		41,
		42,
		43,
		44,
		45,
		46,
		51,
		52,
		53,
		54,
		55,
		56,
		61,
		62,
		63,
		64,
		65,
		66,
		71,
		72,
		73,
		74,
		75,
		76,
		77,
		78,
		79,
		80,
		81,
		82,
		83,
		84,
		85,
		91,
		92,
		93,
		94,
		101,
		102,
		103,
		104,
		111,
		112,
		113,
		114,
		115,
		116,
		117,
		118,
		121,
		122,
		123,
		131,
		132,
		133,
		134,
		135,
		136,
		137,
		141,
		142,
		143,
		144,
		145,
		146,
		151,
		152,
		153,
		154,
		155,
		156,
		161,
		162,
		163,
		164,
		165,
		166,
		171,
		172,
		173,
		174,
		175,
		176,
		177,
		178,
		179,
		180,
		181,
		182,
		183,
		184,
		185,
		191,
		192,
		193,
		194,
		201,
		202,
		203,
		204,
		211,
		212,
		213,
		214,
		215,
		216,
		217,
		218,
		221,
		222,
		223,
		231,
		232,
		233,
		234,
		235,
		236,
		237,
		241,
		242,
		243,
		244,
		245,
		246,
		251,
		252,
		253,
		254,
		255,
		261,
		262,
		263,
		264,
		265,
		271,
		272,
		273,
		274,
		275,
		276,
		277,
		278,
		279,
		280,
		281,
		282,
		283,
		284,
		285,
		291,
		292,
		293,
		294,
		301,
		302,
		303,
		304,
		311,
		312,
		313,
		314,
		315,
		316,
		317,
		318,
		321,
		322,
		323,
		331,
		332,
		333,
		334,
		335,
		336,
		337,
		341,
		342,
		343,
		344,
		345,
		346,
		351,
		352,
		353,
		354,
		355,
		361,
		362,
		363,
		364,
		365,
		371,
		372,
		373,
		374,
		375,
		376,
		377,
		378,
		379,
		380,
		381,
		382,
		383,
		384,
		385,
		391,
		392,
		393,
		394,
		401,
		402,
		403,
		404,
		411,
		412,
		413,
		414,
		415,
		416,
		417,
		418,
		421,
		422,
		423,
		431,
		432,
		433,
		434,
		435,
		436,
		437,
		441,
		442,
		443,
		444,
		445,
		446,
		451,
		452,
		453,
		454,
		455,
		461,
		462,
		463,
		464,
		465,
		471,
		472,
		473,
		474,
		475,
		476,
		477,
		478,
		479,
		480,
		481,
		482,
		483,
		484,
		485,
		491,
		492,
		493,
		494,
		501,
		502,
		503,
		504,
		511,
		512,
		513,
		514,
		515,
		516,
		517,
		518,
		521,
		522,
		523,
		531,
		532,
		533,
		534,
		535,
		536,
		537,
		541,
		542,
		543,
		544,
		545,
		546,
		551,
		552,
		553,
		554,
		555,
		561,
		562,
		563,
		564,
		565,
		571,
		572,
		573,
		574,
		575,
		576,
		577,
		578,
		579,
		580,
		581,
		582,
		583,
		584,
		585,
		591,
		592,
		593,
		594,
		601,
		602,
		603,
		604,
		611,
		612,
		613,
		614,
		615,
		616,
		617,
		618,
		621,
		622,
		623,
		631,
		632,
		633,
		634,
		635,
		636,
		637,
		641,
		642,
		643,
		644,
		645,
		646,
		651,
		652,
		653,
		654,
		655,
		661,
		662,
		663,
		664,
		665,
		671,
		672,
		673,
		674,
		675,
		676,
		677,
		678,
		679,
		680,
		681,
		682,
		683,
		684,
		685,
		691,
		692,
		693,
		694,
		701,
		702,
		703,
		704,
		711,
		712,
		713,
		714,
		715,
		716,
		717,
		718,
		721,
		722,
		723,
		731,
		732,
		741,
		742,
		743,
		744,
		745,
		746,
		751,
		752,
		753,
		754,
		755,
		761,
		762,
		763,
		764,
		765,
		771,
		772,
		773,
		774,
		775,
		776,
		777,
		778,
		779,
		780,
		781,
		782,
		783,
		784,
		785,
		791,
		792,
		793,
		794,
		1001,
		1002,
		1003,
		1004,
		1005,
		1006,
		1007,
		1008,
		1009,
		1010,
		1011,
		1012,
		1013,
		1014,
		1015,
		1016,
		1017,
		1018,
		1019,
		1020,
		1021,
		1022,
		1023,
		1024,
		1025,
		1026,
		1031,
		1032,
		1033,
		1034,
		1035,
		1036,
		1101,
		1102,
		1103,
		1104,
		1105,
		1106,
		1107,
		1108,
		1109,
		1110,
		1111,
		1112,
		1113,
		1114,
		1115,
		1116,
		1117,
		1118,
		1119,
		1120,
		1121,
		1122,
		1123,
		1124,
		1125,
		1126,
		1131,
		1132,
		1133,
		1134,
		1135,
		1201,
		1202,
		1203,
		1204,
		1205,
		1206,
		1207,
		1208,
		1209,
		1211,
		1212,
		1213,
		1214,
		1215,
		1216,
		1217,
		1219,
		1220,
		1221,
		1222,
		1223,
		1224,
		1225,
		1226,
		1231,
		1232,
		1233,
		1234,
		1235,
		1301,
		1302,
		1303,
		1304,
		1305,
		1306,
		1307,
		1308,
		1309,
		1311,
		1312,
		1313,
		1314,
		1315,
		1316,
		1317,
		1319,
		1320,
		1321,
		1322,
		1323,
		1324,
		1325,
		1326,
		1331,
		1332,
		1333,
		1334,
		1335,
		1401,
		1402,
		1403,
		1404,
		1405,
		1406,
		1407,
		1408,
		1409,
		1411,
		1412,
		1413,
		1414,
		1415,
		1416,
		1417,
		1419,
		1420,
		1421,
		1422,
		1423,
		1424,
		1425,
		1426,
		1431,
		1432,
		1433,
		1501,
		1502,
		1503,
		1504,
		1505,
		1506,
		1507,
		1508,
		1509,
		1511,
		1512,
		1513,
		1514,
		1515,
		1516,
		1517,
		1519,
		1520,
		1521,
		1522,
		1523,
		1524,
		1525,
		1526,
		1531,
		1532,
		1533,
		1534,
		1535,
		1601,
		1602,
		1603,
		1604,
		1605,
		1606,
		1607,
		1608,
		1609,
		1611,
		1612,
		1613,
		1614,
		1615,
		1616,
		1617,
		1619,
		1620,
		1621,
		1622,
		1623,
		1624,
		1625,
		1626
	}
}
