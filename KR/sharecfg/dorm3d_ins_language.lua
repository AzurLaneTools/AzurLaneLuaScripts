pg = pg or {}
pg.dorm3d_ins_language = {
	tianlangxing_1 = {
		value = "주인님께 쾌적한 봉사를 제공하기 위해, 시리우스는 늘 새로운 기술을 배우고 있습니다. 메이드는 주인님께 만족을 드리기 위해 존재하니까요.",
		key = "tianlangxing_1",
		is_active = "0"
	},
	nengdai_1 = {
		value = "여긴…… 당신만 볼 수 있는 건가요?",
		key = "nengdai_1",
		is_active = "0"
	},
	ankeleiqi_1 = {
		value = "예쁘다…… 동화에 나오는 것 같아……!",
		key = "ankeleiqi_1",
		is_active = "0"
	},
	ankeleiqi_2 = {
		value = "구름 위에서…… 꿈에서 본 거…… 그리고 싶어……",
		key = "ankeleiqi_2",
		is_active = "0"
	},
	tianlangxing_1_1 = {
		value = "나도 시리우스에게 만족스러운 존재였으면 좋겠네.",
		key = "tianlangxing_1_1",
		is_active = "0"
	},
	tianlangxing_1_2 = {
		value = "최근에 배운 새로운 스킬 있어?",
		key = "tianlangxing_1_2",
		is_active = "0"
	},
	nengdai_1_1 = {
		value = "응, 맞아.",
		key = "nengdai_1_1",
		is_active = "0"
	},
	nengdai_1_2 = {
		value = "전혀 그렇지 않아. 이런 좋은 기회를 독차지하다니, 너무 기뻐.",
		key = "nengdai_1_2",
		is_active = "0"
	},
	ankeleiqi_1_1 = {
		value = "동화면 귀여운 공주님도 한 명 있어야지.",
		key = "ankeleiqi_1_1",
		is_active = "0"
	},
	ankeleiqi_1_2 = {
		value = "그래, 앵커리지만을 위한 동화야.",
		key = "ankeleiqi_1_2",
		is_active = "0"
	},
	ankeleiqi_2_1 = {
		value = "어떤 꿈을 꿨어?",
		key = "ankeleiqi_2_1",
		is_active = "0"
	},
	ankeleiqi_2_2 = {
		value = "왜 구름 위에서 그리고 싶어?",
		key = "ankeleiqi_2_2",
		is_active = "0"
	},
	tianlangxing_1_1_1 = {
		value = "시리우스를 만족시키고 싶으신지요?! 그럼, 시리우스가 배운 모든 것을 자랑스러운 주인님을 모시는 데 활용할 수 있도록……!",
		key = "tianlangxing_1_1_1",
		is_active = "0"
	},
	tianlangxing_1_2_1 = {
		value = "네! 시리우스와 자랑스러운 주인님을 영원히 하나로 묶어줄 아주 중요한 스킬입니다…!",
		key = "tianlangxing_1_2_1",
		is_active = "0"
	},
	nengdai_1_1_1 = {
		value = "다행이에요. 그럼 지금부터 제 일상을 공유할게요. 지루하지 않았으면 좋겠는데……",
		key = "nengdai_1_1_1",
		is_active = "0"
	},
	nengdai_1_2_1 = {
		value = "그래요……? 매일 오시면 그 좋은 기회를 더욱더 독차지하실 수 있답니다.",
		key = "nengdai_1_2_1",
		is_active = "0"
	},
	ankeleiqi_1_1_1 = {
		value = "앵커리지가…… 공주님……?",
		key = "ankeleiqi_1_1_1",
		is_active = "0"
	},
	ankeleiqi_1_2_1 = {
		value = "선생님도…… 동화 속 세계에…… 초대할게…!",
		key = "ankeleiqi_1_2_1",
		is_active = "0"
	},
	ankeleiqi_2_1_1 = {
		value = "선생님이랑 앵커리지…… 손잡고…… 사탕을 먹으면서…… 둥실둥실……",
		key = "ankeleiqi_2_1_1",
		is_active = "0"
	},
	ankeleiqi_2_2_1 = {
		value = "예쁘니까…… 구름 위에서… 선생님이랑 같이 있으면… 행복하겠다……",
		key = "ankeleiqi_2_2_1",
		is_active = "0"
	},
	xinzexi_1 = {
		value = "으엑… 엄청나게 쓴 걸 마셔버렸어…… 궁금하다고 마셔보지 말걸 그랬어!",
		key = "xinzexi_1",
		is_active = "0"
	},
	xinzexi_1_1 = {
		value = "괜찮아? 뭐가 그렇게 쓰다는 거야?",
		key = "xinzexi_1_1",
		is_active = "0"
	},
	xinzexi_1_1_1 = {
		value = "괜찮지 않아… 너무 써서 죽을 것 같아~! 허니의 간호가 필요해~!",
		key = "xinzexi_1_1_1",
		is_active = "0"
	},
	xinzexi_1_2 = {
		value = "쓴맛을 없애는 약 같은 건 없나?",
		key = "xinzexi_1_2",
		is_active = "0"
	},
	xinzexi_1_2_1 = {
		value = "당연히 있지! 허니가 과일 주스를 가지고 와서 한 입 마시게 해주면, 쓴맛이 싹~ 날아갈 거야!",
		key = "xinzexi_1_2_1",
		is_active = "0"
	},
	xinzexi_2 = {
		value = "허니한테 뭔가 해주고 싶은데… 뭐가 좋을까~?",
		key = "xinzexi_2",
		is_active = "0"
	},
	xinzexi_2_1 = {
		value = "뉴저지가 좋아하는 걸 해주는 건 어때?",
		key = "xinzexi_2_1",
		is_active = "0"
	},
	xinzexi_2_1_1 = {
		value = "음… 하고 싶은 건 다 해봤는걸? 그리고 지금은 허니랑 같이 보내는 시간이잖아.",
		key = "xinzexi_2_1_1",
		is_active = "0"
	},
	xinzexi_2_2 = {
		value = "곁에 있어주는 것만으로도 충분해.",
		key = "xinzexi_2_2",
		is_active = "0"
	},
	xinzexi_2_2_1 = {
		value = "곁에 있는 것만으로는 부족해~ 더 재밌고 의미 있는 걸 해야지! 예를 들면… 커플 여행 같은 건 어때?",
		key = "xinzexi_2_2_1",
		is_active = "0"
	},
	xinzexi_3 = {
		value = "리드미컬한 음악을 들으면, 역시 기분이 확하고 바뀌잖아~♪♪",
		key = "xinzexi_3",
		is_active = "0"
	},
	xinzexi_3_1 = {
		value = "설마 헤비메탈?",
		key = "xinzexi_3_1",
		is_active = "0"
	},
	xinzexi_3_1_1 = {
		value = "그럴리가 없잖아? 이봐, 허니도 같이 듣자! 반드시 나와의 음악타임에 빠져버릴거야~!",
		key = "xinzexi_3_1_1",
		is_active = "0"
	},
	xinzexi_3_2 = {
		value = "지금의 기분은?",
		key = "xinzexi_3_2",
		is_active = "0"
	},
	xinzexi_3_2_1 = {
		value = "음, 어쩐지 근질근질해~! 이런 유니크한 리듬도 있었구나!",
		key = "xinzexi_3_2_1",
		is_active = "0"
	},
	dafeng_1 = {
		value = "지휘관님과 숨바꼭질 하는 중~",
		key = "dafeng_1",
		is_active = "0"
	},
	dafeng_1_1 = {
		value = "여기 숨어 있었네.",
		key = "dafeng_1_1",
		is_active = "0"
	},
	dafeng_1_1_1 = {
		value = "맞아요~ 어서 다이호를 잡아보세요♡",
		key = "dafeng_1_1_1",
		is_active = "0"
	},
	dafeng_1_2 = {
		value = "대체 어디야?",
		key = "dafeng_1_2",
		is_active = "0"
	},
	dafeng_1_2_1 = {
		value = "아직 다이호의 방 구조를 완전히 파악 못 하셨나 봐요… 다음에 천천히 안내해 드릴게요~",
		key = "dafeng_1_2_1",
		is_active = "0"
	},
	dafeng_2 = {
		value = "지휘관님과 함께한 시간을 빠짐없이 기록해 둬야 해요~",
		key = "dafeng_2",
		is_active = "0"
	},
	dafeng_2_1 = {
		value = "방 안을 전부 찍은 거야?",
		key = "dafeng_2_1",
		is_active = "0"
	},
	dafeng_2_1_1 = {
		value = "다음엔 지휘관님이 출근하시는 모습도 꼭 찍어야겠어요~",
		key = "dafeng_2_1_1",
		is_active = "0"
	},
	dafeng_2_2 = {
		value = "왜 기록하는 거야?",
		key = "dafeng_2_2",
		is_active = "0"
	},
	dafeng_2_2_1 = {
		value = "「지휘관님과 다이호의 알콩달콩한 하루」… 이건 정말 중요한 자료니까요♡",
		key = "dafeng_2_2_1",
		is_active = "0"
	},
	aijier_1 = {
		value = "와인의 향기인가, 아니면 커피의 향기인가…… 딱 하나만 고르기가 어렵네.",
		key = "aijier_1",
		is_active = "0"
	},
	aijier_1_1 = {
		value = "각자 하나씩 고르는 게 어때?",
		key = "aijier_1_1",
		is_active = "0"
	},
	aijier_1_1_1 = {
		value = "먼저 선택해도 되는 거면…… 와인을 고르고, 커피는 네 걸 한 모금 얻어 마시지 뭐.",
		key = "aijier_1_1_1",
		is_active = "0"
	},
	aijier_1_2 = {
		value = "그렇게 고민되면 아예 다른 걸 마시러 가는 건 어때?",
		key = "aijier_1_2",
		is_active = "0"
	},
	aijier_1_2_1 = {
		value = "그렇게까지 말하니, 같이 가주지 뭐.",
		key = "aijier_1_2_1",
		is_active = "0"
	},
	aijier_2 = {
		value = "너무 조용한 오후라…… 오히려 마음이 안 놓이네……",
		key = "aijier_2",
		is_active = "0"
	},
	aijier_2_1 = {
		value = "뭐 마시고 싶은 거 있어? 가져다 줄게.",
		key = "aijier_2_1",
		is_active = "0"
	},
	aijier_2_1_1 = {
		value = "음… 그럼 커피를 가져다 줘. 그리고 함께 이 조용한 시간을 보내자고.",
		key = "aijier_2_1_1",
		is_active = "0"
	},
	aijier_2_2 = {
		value = "에기르도 이런 편안한 분위기를 좋아하는구나.",
		key = "aijier_2_2",
		is_active = "0"
	},
	aijier_2_2_1 = {
		value = "그래, 같이…… 이 소중한 오후 시간을 같이 즐기자고.",
		key = "aijier_2_2_1",
		is_active = "0"
	},
	all = {
		"tianlangxing_1",
		"nengdai_1",
		"ankeleiqi_1",
		"ankeleiqi_2",
		"tianlangxing_1_1",
		"tianlangxing_1_2",
		"nengdai_1_1",
		"nengdai_1_2",
		"ankeleiqi_1_1",
		"ankeleiqi_1_2",
		"ankeleiqi_2_1",
		"ankeleiqi_2_2",
		"tianlangxing_1_1_1",
		"tianlangxing_1_2_1",
		"nengdai_1_1_1",
		"nengdai_1_2_1",
		"ankeleiqi_1_1_1",
		"ankeleiqi_1_2_1",
		"ankeleiqi_2_1_1",
		"ankeleiqi_2_2_1",
		"xinzexi_1",
		"xinzexi_1_1",
		"xinzexi_1_1_1",
		"xinzexi_1_2",
		"xinzexi_1_2_1",
		"xinzexi_2",
		"xinzexi_2_1",
		"xinzexi_2_1_1",
		"xinzexi_2_2",
		"xinzexi_2_2_1",
		"xinzexi_3",
		"xinzexi_3_1",
		"xinzexi_3_1_1",
		"xinzexi_3_2",
		"xinzexi_3_2_1",
		"dafeng_1",
		"dafeng_1_1",
		"dafeng_1_1_1",
		"dafeng_1_2",
		"dafeng_1_2_1",
		"dafeng_2",
		"dafeng_2_1",
		"dafeng_2_1_1",
		"dafeng_2_2",
		"dafeng_2_2_1",
		"aijier_1",
		"aijier_1_1",
		"aijier_1_1_1",
		"aijier_1_2",
		"aijier_1_2_1",
		"aijier_2",
		"aijier_2_1",
		"aijier_2_1_1",
		"aijier_2_2",
		"aijier_2_2_1"
	}
}
