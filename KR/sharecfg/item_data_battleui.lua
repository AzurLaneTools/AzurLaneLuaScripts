pg = pg or {}
pg.item_data_battleui = {
	[0] = {
		name = "오리지널",
		key = "Standard",
		display_icon = "ui_0",
		unlock = "디폴트",
		is_unlock = 0,
		rare = 2,
		desc = "그리운 느낌을 주는 오리지널 전투 UI. 셀 수 없이 많은 전투를 함께 헤쳐온 경험 덕분에 실용성은 충분하다.",
		id = 0,
		icon = "0",
		scene = {},
		rare_display = {}
	},
	[101] = {
		name = "리메이크",
		key = "SkinNormal_20240701",
		display_icon = "ui_101",
		unlock = "디폴트",
		is_unlock = 0,
		rare = 2,
		desc = "배색 및 디자인에 공을 들인 리메이크 UI. 더욱 약동감 있는 체험을 지휘관에게 선사한다.",
		id = 101,
		icon = "101",
		scene = {},
		rare_display = {}
	},
	[102] = {
		name = "사이버 시대",
		key = "SkinNormal_20240912",
		display_icon = "ui_102",
		unlock = "특허 투어 패스 19기 보상",
		is_unlock = 1,
		rare = 3,
		desc = "사이버 스타일 UI. 애니메이션이나 라이팅 등의 연출이 곳곳에 적용되어, SF를 좋아하는 지휘관에게 몰입감을 제공한다.",
		id = 102,
		icon = "102",
		scene = {},
		rare_display = {
			1
		}
	},
	[103] = {
		name = "레인보우",
		key = "SkinNormal_20240913",
		display_icon = "ui_103",
		unlock = "상점 구매",
		is_unlock = 1,
		rare = 4,
		desc = "풍부한 표현력과 조화로운 배색이 매력인 UI. 리듬에 맞추어 역동하는 지휘 화면은 경쾌한 분위기를 선호하는 지휘관에게 안성맞춤.",
		id = 103,
		icon = "103",
		scene = {},
		rare_display = {
			1,
			2,
			3
		}
	},
	[104] = {
		name = "네온",
		key = "SkinNormal_20241107",
		display_icon = "ui_104",
		unlock = "특허 투어 패스 20기 보상",
		is_unlock = 1,
		rare = 3,
		desc = "반짝이는 네온에 리드미컬한 빔 연출로 부드러운 사용감 제공",
		id = 104,
		icon = "104",
		scene = {},
		rare_display = {
			1
		}
	},
	[105] = {
		name = "라이트",
		key = "SkinNormal_20250123",
		display_icon = "ui_105",
		unlock = "특허 투어 패스 21기 보상",
		is_unlock = 1,
		rare = 3,
		desc = "신성함, 중후함, 찬란함. 지휘관의 모든 전투를 더욱 위엄있게 만들어 준다.",
		id = 105,
		icon = "105",
		scene = {},
		rare_display = {
			1
		}
	},
	[106] = {
		name = "데빌",
		key = "SkinNormal_20250327",
		display_icon = "ui_106",
		unlock = "특허 투어 패스 22기 보상",
		is_unlock = 1,
		rare = 3,
		desc = "악마 특유의 분위기가 감도는 지휘 인터페이스. 핏빛 프레임과 마법진이 위험하고 어두운 세상으로 이끈다.",
		id = 106,
		icon = "106",
		scene = {},
		rare_display = {
			1
		}
	},
	[107] = {
		name = "스텔라",
		key = "SkinNormal_20250529",
		display_icon = "ui_107",
		unlock = "특허 투어 패스 23기 보상",
		is_unlock = 1,
		rare = 3,
		desc = "반짝이는 별빛과 신비로운 문양이 은하의 윤곽을 비추고, 세밀한 부분 하나하나가 우주의 탐구와 모험을 이야기하고 있다.",
		id = 107,
		icon = "107",
		scene = {},
		rare_display = {
			1
		}
	},
	[108] = {
		name = "네이처",
		key = "SkinNormal_20250724",
		display_icon = "ui_108",
		unlock = "특허 투어 패스 24기 보상",
		is_unlock = 1,
		rare = 3,
		desc = "울창한 숲 위를 나는 새들과 흐르는 물이 어우러져 자아내는 생동감 넘치는 자연의 리듬.",
		id = 108,
		icon = "108",
		scene = {},
		rare_display = {
			1
		}
	},
	[109] = {
		name = "먹과 대나무",
		key = "SkinNormal_20250925",
		display_icon = "ui_109",
		unlock = "투허 투어 패스 25기 보상",
		is_unlock = 1,
		rare = 3,
		desc = "수묵화풍의 인터페이스. 흑백의 대비 속에서 퍼져가는 먹물, 흔들리는 듯 그려진 대나무 그림자가 우아한 분위기를 자아낸다.",
		id = 109,
		icon = "109",
		scene = {},
		rare_display = {
			1
		}
	},
	[111] = {
		name = "그림자극",
		key = "SkinNormal_20251120",
		display_icon = "ui_111",
		unlock = "월드 투어 26기 보상",
		is_unlock = 1,
		rare = 3,
		desc = "단순해 보이는 장치로 끝없는 환상을 그려내며, 짧은 이야기로 멋들어진 희망을 선사한다.",
		id = 111,
		icon = "111",
		scene = {},
		rare_display = {
			1
		}
	},
	[113] = {
		name = "고대 왕조",
		key = "SkinNormal_20260129",
		display_icon = "ui_113",
		unlock = "특허 투어 패스 27기 보상",
		is_unlock = 1,
		rare = 3,
		desc = "화면에 우뚝 솟은 것은 고대의 석주. 얼룩덜룩한 문양은 잃어버린 질서를 이야기하며, 전투에 시공을 초월한 장엄함과 비탄을 더해준다.",
		id = 113,
		icon = "113",
		scene = {},
		rare_display = {
			1
		}
	},
	[201] = {
		name = "크리스마스",
		key = "SkinNormal_20241209",
		display_icon = "ui_201",
		unlock = "상점 구매 (전투 UI·크리스마스)",
		is_unlock = 1,
		rare = 4,
		desc = "흩날리는 눈과 서리가 그리는 동화 속 경치. 겨울 분위기를 사랑하는 지휘관을 위해 준비한 전장의 겨울 로맨스.",
		id = 201,
		icon = "201",
		scene = {},
		rare_display = {
			1,
			2,
			3
		}
	},
	[202] = {
		name = "성스러운 모래의 약속",
		key = "SkinNormal_20250227",
		display_icon = "ui_202",
		unlock = "상점 구매 (전투 UI·파라오)",
		is_unlock = 1,
		rare = 4,
		desc = "고대 문자에 얽힌 황금 무늬. 이 모든 것은 나일강 문명의 유구한 전설을 이야기하고 있다.",
		id = 202,
		icon = "202",
		scene = {},
		rare_display = {
			1,
			2,
			3
		}
	},
	[203] = {
		name = "유전자의 기원",
		key = "SkinElite_20250327",
		display_icon = "ui_203",
		unlock = "상점 구매 (전투 UI·유전자의 기원)",
		is_unlock = 1,
		rare = 4,
		desc = "미래적인 느낌이 풍기는 지휘 인터페이스. 흰 패널과 핏빛 회로가 치밀하고도 긴장감 넘치는 분위기를 자아낸다.",
		id = 203,
		icon = "203",
		scene = {},
		rare_display = {
			1,
			2,
			3
		}
	},
	[204] = {
		name = "시사이드",
		key = "SkinElite_20250520",
		display_icon = "ui_204",
		unlock = "상점 구매 (전투 UI·시사이드)",
		is_unlock = 1,
		rare = 4,
		desc = "모래사장에 세워진 나무판, 조개껍데기로 장식된 흰 파도. 바닷가에서 여유롭고 한가롭게 보내는 바캉스.",
		id = 204,
		icon = "204",
		scene = {},
		rare_display = {
			1,
			2,
			3
		}
	},
	[205] = {
		name = "닌자",
		key = "SkinElite_20250912",
		display_icon = "ui_205",
		unlock = "상점 구매 (전투 UI·닌자)",
		is_unlock = 1,
		rare = 4,
		desc = "닌자를 제일 좋아하는 지휘관을 위한 상품! 수리검과 쿠나이로 닌자의 성을 습격하라!",
		id = 205,
		icon = "205",
		scene = {},
		rare_display = {
			1,
			2,
			3
		}
	},
	[206] = {
		name = "메이드 카페",
		key = "SkinElite_20251218",
		display_icon = "ui_206",
		unlock = "상점 구매 (전투 UI·메이드 카페)",
		is_unlock = 1,
		rare = 4,
		desc = "상냥한 메이드의 달콤한 디저트를 맛보며, 달콤한 챌린지에 도전해 보자!",
		id = 206,
		icon = "206",
		scene = {},
		rare_display = {
			1,
			2,
			3
		}
	},
	all = {
		0,
		101,
		102,
		103,
		104,
		105,
		106,
		107,
		108,
		109,
		111,
		113,
		201,
		202,
		203,
		204,
		205,
		206
	}
}
