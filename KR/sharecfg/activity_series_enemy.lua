pg = pg or {}
pg.activity_series_enemy = rawget(pg, "activity_series_enemy") or setmetatable({
	__name = "activity_series_enemy"
}, confNEO)
pg.activity_series_enemy.all = {
	1001,
	1002,
	1003,
	1004,
	1005,
	2001,
	2002,
	2003,
	2004,
	2005,
	3001,
	3002,
	3003,
	3004,
	3005,
	4001,
	4002,
	4011,
	4012,
	4021,
	4022,
	4031,
	4032,
	4041,
	4042,
	4051,
	4052,
	5001,
	5002,
	5003,
	5004,
	5005,
	1001001,
	1001002,
	1001003
}
pg.base = pg.base or {}
pg.base.activity_series_enemy = {}

(function ()
	pg.base.activity_series_enemy[1001] = {
		oil = 0,
		name = "제압: 그린란드",
		type = 1,
		pre_chapter = 0,
		chapter_name = "TC1",
		pos_x = "0.10703125",
		pos_y = "0.157291667",
		count = 0,
		ex_count = "",
		additional_awards_display = "",
		profiles = "임무 목표: 세이렌 그린란드 요새를 확보하고 극지 해역에 대한 오미터의 전자기 억압을 제거하여 전장을 지원하라.",
		collection_group_id = 104,
		whether_singlefight = 0,
		id = 1001,
		limitation = {},
		expedition_id = {
			1719101
		},
		boss_icon = {
			{
				"qinraozhe",
				2
			}
		},
		pass_awards_display = {
			{
				1,
				487,
				2
			},
			{
				1,
				488,
				40
			},
			{
				2,
				58839
			},
			{
				2,
				59001
			},
			{
				2,
				54012
			}
		},
		defeat_story = {},
		defeat_story_count = {},
		use_oil_limit = {
			0,
			0
		}
	}
	pg.base.activity_series_enemy[1002] = {
		oil = 0,
		name = "돌파: 축치해",
		type = 1,
		pre_chapter = 1001,
		chapter_name = "TC2",
		pos_x = "0.34609375",
		pos_y = "0.347916667",
		count = 0,
		ex_count = "",
		profiles = "임무 목표: 아직 미완성된 세이렌의 축치해 요새 방어선을 돌파하고, 오미터가 위치한 기상 무기 기지를 파괴하라.",
		collection_group_id = 105,
		whether_singlefight = 1,
		id = 1002,
		limitation = {},
		expedition_id = {
			1719201,
			1719202
		},
		boss_icon = {
			{
				"qinraozhe",
				2
			},
			{
				"qingchuzhe",
				5
			}
		},
		pass_awards_display = {
			{
				1,
				487,
				5
			},
			{
				1,
				488,
				140
			},
			{
				2,
				58838
			},
			{
				2,
				59001
			},
			{
				2,
				54017
			}
		},
		additional_awards_display = {
			{
				1,
				488,
				25
			},
			{
				1,
				1
			}
		},
		defeat_story = {},
		defeat_story_count = {},
		use_oil_limit = {
			0,
			0
		}
	}
	pg.base.activity_series_enemy[1003] = {
		oil = 0,
		name = "탈환: 세베르나야젬랴 제도",
		type = 1,
		pre_chapter = 1002,
		chapter_name = "TC3",
		pos_x = "0.50546875",
		pos_y = "0.080208333",
		count = 0,
		ex_count = "",
		profiles = "임무 목표: 과거 전진 기지였던 세이렌 세베르나야젬랴 제도 요새를 탈환하고, 오미터의 극지 통제력을 약화시켜라.",
		collection_group_id = 106,
		whether_singlefight = 1,
		id = 1003,
		limitation = {},
		expedition_id = {
			1719301,
			1719302,
			1719303
		},
		boss_icon = {
			{
				"qinraozhe_IV",
				2
			},
			{
				"kuersike",
				3
			},
			{
				"qingchuzhe",
				5
			}
		},
		pass_awards_display = {
			{
				1,
				487,
				8
			},
			{
				1,
				488,
				360
			},
			{
				2,
				58837
			},
			{
				2,
				59001
			},
			{
				2,
				54017
			}
		},
		additional_awards_display = {
			{
				1,
				488,
				60
			},
			{
				1,
				1
			}
		},
		defeat_story = {},
		defeat_story_count = {},
		use_oil_limit = {
			25,
			16
		}
	}
	pg.base.activity_series_enemy[1004] = {
		oil = 0,
		name = "정찰: 북극",
		type = 2,
		pre_chapter = 1003,
		chapter_name = "SP",
		pos_x = "0.60546875",
		pos_y = "0.446875",
		count = 1,
		ex_count = "",
		additional_awards_display = "",
		profiles = "임무 목표: 세이렌 북극 요새에 접근하여 정찰하며 최대한 데이터를 수집해 오미터의 전략에 대한 연구에 협조하라.",
		collection_group_id = 107,
		whether_singlefight = 0,
		id = 1004,
		limitation = {},
		expedition_id = {
			1719401,
			1719402,
			1719403,
			1719404
		},
		boss_icon = {
			{
				"qinraozhe_IV",
				2
			},
			{
				"fuluoxiluofu",
				2
			},
			{
				"saiwasituoboer",
				5
			},
			{
				"qingchuzhe",
				5
			}
		},
		pass_awards_display = {
			{
				1,
				487,
				12
			},
			{
				1,
				488,
				800
			},
			{
				2,
				58836
			},
			{
				2,
				59001
			},
			{
				2,
				54016
			}
		},
		defeat_story = {},
		defeat_story_count = {},
		use_oil_limit = {
			40,
			16
		}
	}
	pg.base.activity_series_enemy[1005] = {
		oil = 0,
		name = "견제: 왕관",
		type = 3,
		pre_chapter = 1004,
		chapter_name = "EX",
		pos_x = "0.6265625",
		pos_y = "0.15625",
		count = 0,
		additional_awards_display = "",
		profiles = "임무 목표: “왕관”에 위치한 세이렌 함대를 견제하며 오미터 함대의 주력을 끌어들여 다른 전선의 작전 압력을 경감시켜라.",
		pass_awards_display = "",
		collection_group_id = 0,
		whether_singlefight = 0,
		id = 1005,
		limitation = {},
		expedition_id = {
			1719501,
			1719502,
			1719503,
			1719504,
			1719505
		},
		boss_icon = {
			{
				"qinraozhe_IV",
				2
			},
			{
				"kuersike",
				3
			},
			{
				"fuluoxiluofu",
				2
			},
			{
				"saiwasituoboer",
				5
			},
			{
				"qingchuzhe",
				5
			}
		},
		defeat_story = {},
		defeat_story_count = {},
		use_oil_limit = {
			0,
			0
		},
		ex_count = {
			8000,
			20,
			0.2,
			1000,
			0.8
		}
	}
	pg.base.activity_series_enemy[2001] = {
		oil = 0,
		name = "사무동",
		type = 1,
		pre_chapter = 0,
		chapter_name = "쉬움",
		pos_x = "0.10703125",
		pos_y = "0.157291667",
		count = 0,
		ex_count = "",
		additional_awards_display = "",
		profiles = [[
「사무동에 오신 것을 환영합니다」
「진취적인 시간을 보내기를」
「시간에 구애받지 않기를」
「동심을 영원히 잃지 않기를」]],
		collection_group_id = 104,
		whether_singlefight = 0,
		id = 2001,
		limitation = {},
		expedition_id = {
			1819101
		},
		boss_icon = {
			{
				"youeryuan_boss05",
				5
			}
		},
		pass_awards_display = {
			{
				1,
				421,
				1
			},
			{
				2,
				200174,
				1
			},
			{
				2,
				59001,
				1
			},
			{
				2,
				54012,
				1
			}
		},
		defeat_story = {},
		defeat_story_count = {},
		use_oil_limit = {
			0,
			0
		}
	}
	pg.base.activity_series_enemy[2002] = {
		oil = 0,
		name = "교실동",
		type = 1,
		pre_chapter = 2001,
		chapter_name = "보통",
		pos_x = "0.34609375",
		pos_y = "0.347916667",
		count = 0,
		ex_count = "",
		profiles = [[
「교실동에 오신 것을 환영합니다」
「스트레스 없는 시간을 보내기를」
「지식의 바다에서 마음껏 헤엄치기를」
「동심을 영원히 잃지 않기를」]],
		collection_group_id = 105,
		whether_singlefight = 1,
		id = 2002,
		limitation = {},
		expedition_id = {
			1819201,
			1819202
		},
		boss_icon = {
			{
				"youeryuan_boss05",
				5
			},
			{
				"youeryuan_boss03",
				7
			}
		},
		pass_awards_display = {
			{
				1,
				421,
				1
			},
			{
				2,
				200175,
				1
			},
			{
				2,
				59001,
				1
			},
			{
				2,
				54017,
				1
			}
		},
		additional_awards_display = {
			{
				1,
				421,
				1
			},
			{
				1,
				1,
				1
			}
		},
		defeat_story = {},
		defeat_story_count = {},
		use_oil_limit = {
			0,
			0
		}
	}
	pg.base.activity_series_enemy[2003] = {
		oil = 0,
		name = "예술동",
		type = 1,
		pre_chapter = 2002,
		chapter_name = "어려움",
		pos_x = "0.50546875",
		pos_y = "0.080208333",
		count = 0,
		ex_count = "",
		profiles = [[
「예술동에 오신 것을 환영합니다」
「다채로운 시간을 보내기를」
「늘 초심을 유지하기를」
「동심을 영원히 잃지 않기를」]],
		collection_group_id = 106,
		whether_singlefight = 1,
		id = 2003,
		limitation = {},
		expedition_id = {
			1819301,
			1819302,
			1819303
		},
		boss_icon = {
			{
				"youeryuan_boss05",
				5
			},
			{
				"youeryuan_boss03",
				7
			},
			{
				"youeryuan_boss04",
				3
			}
		},
		pass_awards_display = {
			{
				1,
				421,
				1
			},
			{
				2,
				200176,
				1
			},
			{
				2,
				59001,
				1
			},
			{
				2,
				54017,
				1
			}
		},
		additional_awards_display = {
			{
				1,
				421,
				1
			},
			{
				1,
				1,
				1
			}
		},
		defeat_story = {},
		defeat_story_count = {},
		use_oil_limit = {
			25,
			16
		}
	}
	pg.base.activity_series_enemy[2004] = {
		oil = 0,
		name = "운동장",
		type = 2,
		pre_chapter = 2003,
		chapter_name = "SP",
		pos_x = "0.60546875",
		pos_y = "0.446875",
		count = 1,
		ex_count = "",
		profiles = [[
「운동장에 오신 것을 환영합니다」
「활기찬 시간을 보내기를」
「늘 건강하게 지내기를」
「동심을 영원히 잃지 않기를」]],
		collection_group_id = 107,
		whether_singlefight = 0,
		id = 2004,
		limitation = {},
		expedition_id = {
			1819401,
			1819402,
			1819403,
			1819404
		},
		boss_icon = {
			{
				"youeryuan_boss05",
				5
			},
			{
				"youeryuan_boss03",
				7
			},
			{
				"youeryuan_boss04",
				3
			},
			{
				"youeryuan_boss01",
				1
			}
		},
		pass_awards_display = {
			{
				1,
				421,
				1
			},
			{
				2,
				200177,
				1
			},
			{
				2,
				59001,
				1
			},
			{
				2,
				54016,
				1
			}
		},
		additional_awards_display = {
			{
				1,
				421,
				1
			},
			{
				1,
				1,
				1
			}
		},
		defeat_story = {},
		defeat_story_count = {},
		use_oil_limit = {
			40,
			16
		}
	}
	pg.base.activity_series_enemy[2005] = {
		oil = 0,
		name = "뒷산 숲",
		type = 3,
		pre_chapter = 2004,
		chapter_name = "EX",
		pos_x = "0.6265625",
		pos_y = "0.15625",
		count = 0,
		additional_awards_display = "",
		profiles = [[
「뒷산 숲에 오신 것을 환영합니다」
「행복한 시간을 보낼 수 있기를」
「아름다운 자연을 만끽하기를」
「동심을 영원히 잃지 않기를」]],
		pass_awards_display = "",
		collection_group_id = 0,
		whether_singlefight = 0,
		id = 2005,
		limitation = {},
		expedition_id = {
			1819501,
			1819502,
			1819503,
			1819504,
			1819505
		},
		boss_icon = {
			{
				"youeryuan_boss05",
				5
			},
			{
				"youeryuan_boss03",
				7
			},
			{
				"youeryuan_boss04",
				3
			},
			{
				"youeryuan_boss01",
				1
			},
			{
				"youeryuan_boss02",
				2
			}
		},
		defeat_story = {},
		defeat_story_count = {},
		use_oil_limit = {
			0,
			0
		},
		ex_count = {
			8000,
			20,
			0.2,
			1000,
			0.8
		}
	}
	pg.base.activity_series_enemy[3001] = {
		oil = 0,
		name = "촬영지: 간선 도로",
		type = 1,
		pre_chapter = 0,
		chapter_name = "쉬움",
		pos_x = "0.10703125",
		pos_y = "0.157291667",
		count = 0,
		ex_count = "",
		additional_awards_display = "",
		profiles = "[촬영지: 간선 도로] 주로 차량으로 범인을 뒤쫒는 장면에 쓰이고, 때로는 격렬한 총격전의 무대가 되기도 한다. 스로틀을 놓지 않는 한 이 길은 계속된다.",
		collection_group_id = 104,
		whether_singlefight = 0,
		id = 3001,
		limitation = {},
		expedition_id = {
			1930101
		},
		boss_icon = {
			{
				"u37_3",
				8
			}
		},
		pass_awards_display = {
			{
				1,
				516,
				40
			},
			{
				2,
				200480,
				0
			},
			{
				2,
				59001,
				0
			},
			{
				2,
				54012,
				0
			}
		},
		defeat_story = {},
		defeat_story_count = {},
		use_oil_limit = {
			0,
			0
		}
	}
	pg.base.activity_series_enemy[3002] = {
		oil = 0,
		name = "촬영지: 상점가",
		type = 1,
		pre_chapter = 3001,
		chapter_name = "보통",
		pos_x = "0.34609375",
		pos_y = "0.347916667",
		count = 0,
		ex_count = "",
		profiles = "[촬영지: 상점가] 주로 식사나 쇼핑 장면에 쓰이고, 때로는 스릴 넘치는 살인 사건 현장으로도 등장한다. 먹방을 싫어하는 사람은 없으니 촬영 빈도를 늘리자!",
		collection_group_id = 105,
		whether_singlefight = 1,
		id = 3002,
		limitation = {},
		expedition_id = {
			1930201,
			1930202
		},
		boss_icon = {
			{
				"u37_3",
				8
			},
			{
				"wuqi_3",
				18
			}
		},
		pass_awards_display = {
			{
				1,
				516,
				140
			},
			{
				2,
				200481,
				0
			},
			{
				2,
				59001,
				0
			},
			{
				2,
				54017,
				0
			}
		},
		additional_awards_display = {
			{
				1,
				516,
				25
			},
			{
				1,
				1,
				0
			}
		},
		defeat_story = {},
		defeat_story_count = {},
		use_oil_limit = {
			0,
			0
		}
	}
	pg.base.activity_series_enemy[3003] = {
		oil = 0,
		name = "촬영지: 열차",
		type = 1,
		pre_chapter = 3002,
		chapter_name = "어려움",
		pos_x = "0.50546875",
		pos_y = "0.080208333",
		count = 0,
		ex_count = "",
		profiles = "[촬영지: 열차] 주로 여행이나 풍경 장면에 쓰이고, 때로는 긴박한 폭탄 해체 장면에도 쓰인다. 빨간 선일까, 파란 선일까? 마음이 가는대로 잘라보자.",
		collection_group_id = 106,
		whether_singlefight = 1,
		id = 3003,
		limitation = {},
		expedition_id = {
			1930301,
			1930302,
			1930303
		},
		boss_icon = {
			{
				"u37_3",
				8
			},
			{
				"u552_2",
				8
			},
			{
				"zengkehaijunshangjiang_2",
				4
			}
		},
		pass_awards_display = {
			{
				1,
				516,
				360
			},
			{
				2,
				200482,
				0
			},
			{
				2,
				59001,
				0
			},
			{
				2,
				54017,
				0
			}
		},
		additional_awards_display = {
			{
				1,
				516,
				60
			},
			{
				1,
				1,
				0
			}
		},
		defeat_story = {},
		defeat_story_count = {},
		use_oil_limit = {
			25,
			16
		}
	}
	pg.base.activity_series_enemy[3004] = {
		oil = 0,
		name = "촬영지: 번화가",
		type = 2,
		pre_chapter = 3003,
		chapter_name = "S.P",
		pos_x = "0.60546875",
		pos_y = "0.446875",
		count = 1,
		ex_count = "",
		additional_awards_display = "",
		profiles = "[촬영지: 번화가] 주로 일상 장면에 쓰이며, 때로는 긴장감 넘치는 추격전이 벌어지기도 한다. 「너는 이미 포위됐다! 순순히 항복해라!」",
		collection_group_id = 107,
		whether_singlefight = 0,
		id = 3004,
		limitation = {},
		expedition_id = {
			1930401,
			1930402,
			1930403,
			1930404
		},
		boss_icon = {
			{
				"haiwangxing_4",
				2
			},
			{
				"z13_2",
				1
			},
			{
				"zengkehaijunshangjiang_2",
				4
			},
			{
				"wuqi_3",
				18
			}
		},
		pass_awards_display = {
			{
				1,
				516,
				800
			},
			{
				2,
				200483,
				0
			},
			{
				2,
				59001,
				0
			},
			{
				2,
				54016,
				0
			}
		},
		defeat_story = {},
		defeat_story_count = {},
		use_oil_limit = {
			40,
			16
		}
	}
	pg.base.activity_series_enemy[3005] = {
		oil = 0,
		name = "촬영지: 경찰기구",
		type = 3,
		pre_chapter = 3004,
		chapter_name = "E.X",
		pos_x = "0.6265625",
		pos_y = "0.15625",
		count = 0,
		additional_awards_display = "",
		profiles = "[촬영지: 경찰기구] 주로 심문 장면에 쓰이며, 때로는 음모가 뒤얽힌 첩보극의 무대가 되기도 한다. 과연 스파이는 누구일까?",
		pass_awards_display = "",
		collection_group_id = 0,
		whether_singlefight = 0,
		id = 3005,
		limitation = {},
		expedition_id = {
			1930501,
			1930502,
			1930503,
			1930504,
			1930505
		},
		boss_icon = {
			{
				"z13_2",
				1
			},
			{
				"u552_2",
				8
			},
			{
				"zengkehaijunshangjiang_2",
				4
			},
			{
				"haiwangxing_4",
				2
			},
			{
				"diguo_3",
				7
			}
		},
		defeat_story = {},
		defeat_story_count = {},
		use_oil_limit = {
			0,
			0
		},
		ex_count = {
			8000,
			20,
			0.2,
			1000,
			0.8
		}
	}
	pg.base.activity_series_enemy[4001] = {
		oil = 0,
		name = "보통（LV 20）",
		type = 1,
		pre_chapter = 0,
		chapter_name = "无用",
		pos_x = "0",
		pos_y = "0",
		count = 0,
		ex_count = "",
		profiles = "[AREA-1] 환상의 탑 주변에 이상 해역이 출현했다. 이글 유니온 함대를 이끌고 상황을 확인하자.",
		collection_group_id = 0,
		whether_singlefight = 1,
		id = 4001,
		limitation = {},
		expedition_id = {
			1980001,
			1980002
		},
		boss_icon = {
			{
				"qinraozhe_IV",
				2
			},
			{
				"weixu_baojian_2",
				2
			}
		},
		pass_awards_display = {
			{
				1,
				588,
				30
			},
			{
				1,
				587,
				11
			},
			{
				2,
				200619,
				0
			},
			{
				2,
				54012,
				0
			},
			{
				2,
				200633,
				0
			},
			{
				2,
				59001,
				0
			}
		},
		additional_awards_display = {
			{
				1,
				588,
				5
			},
			{
				1,
				1,
				0
			}
		},
		defeat_story = {},
		defeat_story_count = {},
		use_oil_limit = {
			0,
			0
		}
	}
	pg.base.activity_series_enemy[4002] = {
		oil = 0,
		name = "어려움（LV 35）",
		type = 1,
		pre_chapter = 0,
		chapter_name = "无用",
		pos_x = "0",
		pos_y = "0",
		count = 0,
		ex_count = "",
		profiles = "[AREA-1] 환상의 탑 주변에 이상 해역이 출현했다. 이글 유니온 함대를 이끌고 상황을 확인하자.",
		collection_group_id = 0,
		whether_singlefight = 1,
		id = 4002,
		limitation = {},
		expedition_id = {
			1980101,
			1980102
		},
		boss_icon = {
			{
				"qinraozhe_IV",
				2
			},
			{
				"weixu_baojian_2",
				2
			}
		},
		pass_awards_display = {
			{
				1,
				588,
				40
			},
			{
				1,
				587,
				11
			},
			{
				2,
				200625,
				0
			},
			{
				2,
				54012,
				0
			},
			{
				2,
				200633,
				0
			},
			{
				2,
				59001,
				0
			}
		},
		additional_awards_display = {
			{
				1,
				588,
				10
			},
			{
				1,
				1,
				0
			}
		},
		defeat_story = {},
		defeat_story_count = {},
		use_oil_limit = {
			0,
			0
		}
	}
	pg.base.activity_series_enemy[4011] = {
		oil = 0,
		name = "보통（LV 50）",
		type = 1,
		pre_chapter = 0,
		chapter_name = "无用",
		pos_x = "0",
		pos_y = "0",
		count = 0,
		ex_count = "",
		profiles = "[AREA-2] 작전이 개시되었다. 아이리스 함대와 이스트 글림 함대와 함께 해역을 소탕하자.",
		collection_group_id = 0,
		whether_singlefight = 1,
		id = 4011,
		limitation = {},
		expedition_id = {
			1981007,
			1981011,
			1981003
		},
		boss_icon = {
			{
				"qinraozhe_IV",
				2
			},
			{
				"weixu_baojian_2",
				2
			},
			{
				"weixu_baojian_3",
				2
			}
		},
		pass_awards_display = {
			{
				1,
				588,
				60
			},
			{
				1,
				587,
				14
			},
			{
				2,
				200620,
				0
			},
			{
				2,
				54017,
				0
			},
			{
				2,
				200633,
				0
			},
			{
				2,
				59001,
				0
			}
		},
		additional_awards_display = {
			{
				1,
				588,
				15
			},
			{
				1,
				1,
				0
			}
		},
		defeat_story = {},
		defeat_story_count = {},
		use_oil_limit = {
			0,
			0
		}
	}
	pg.base.activity_series_enemy[4012] = {
		oil = 0,
		name = "어려움（LV 65）",
		type = 1,
		pre_chapter = 0,
		chapter_name = "无用",
		pos_x = "0",
		pos_y = "0",
		count = 0,
		ex_count = "",
		profiles = "[AREA-2] 작전이 개시되었다. 아이리스 함대와 이스트 글림 함대와 함께 해역을 소탕하자.",
		collection_group_id = 0,
		whether_singlefight = 1,
		id = 4012,
		limitation = {},
		expedition_id = {
			1982007,
			1982011,
			1982003
		},
		boss_icon = {
			{
				"qinraozhe_IV",
				2
			},
			{
				"weixu_baojian_2",
				2
			},
			{
				"weixu_baojian_3",
				2
			}
		},
		pass_awards_display = {
			{
				1,
				588,
				80
			},
			{
				1,
				587,
				14
			},
			{
				2,
				200626,
				0
			},
			{
				2,
				54017,
				0
			},
			{
				2,
				200633,
				0
			},
			{
				2,
				59001,
				0
			}
		},
		additional_awards_display = {
			{
				1,
				588,
				20
			},
			{
				1,
				1,
				0
			}
		},
		defeat_story = {},
		defeat_story_count = {},
		use_oil_limit = {
			0,
			0
		}
	}
	pg.base.activity_series_enemy[4021] = {
		oil = 0,
		name = "보통（LV 50）",
		type = 1,
		pre_chapter = 0,
		chapter_name = "无用",
		pos_x = "0",
		pos_y = "0",
		count = 0,
		ex_count = "",
		profiles = "[AREA-3] 작전이 개시되었다. 메탈 블러드 함대와 사르데냐 엠파이어 함대와 함께 해역을 소탕하자.",
		collection_group_id = 0,
		whether_singlefight = 1,
		id = 4021,
		limitation = {},
		expedition_id = {
			1981008,
			1981012,
			1981004
		},
		boss_icon = {
			{
				"qinraozhe_IV",
				2
			},
			{
				"weixu_baojian_2",
				2
			},
			{
				"weixu_baojian_4",
				12
			}
		},
		pass_awards_display = {
			{
				1,
				588,
				60
			},
			{
				1,
				587,
				14
			},
			{
				2,
				200621,
				0
			},
			{
				2,
				54017,
				0
			},
			{
				2,
				200633,
				0
			},
			{
				2,
				59001,
				0
			}
		},
		additional_awards_display = {
			{
				1,
				588,
				15
			},
			{
				1,
				1,
				0
			}
		},
		defeat_story = {},
		defeat_story_count = {},
		use_oil_limit = {
			0,
			0
		}
	}
	pg.base.activity_series_enemy[4022] = {
		oil = 0,
		name = "어려움（LV 65）",
		type = 1,
		pre_chapter = 0,
		chapter_name = "无用",
		pos_x = "0",
		pos_y = "0",
		count = 0,
		ex_count = "",
		profiles = "[AREA-3] 작전이 개시되었다. 메탈 블러드 함대와 사르데냐 엠파이어 함대와 함께 해역을 소탕하자.",
		collection_group_id = 0,
		whether_singlefight = 1,
		id = 4022,
		limitation = {},
		expedition_id = {
			1982008,
			1982012,
			1982004
		},
		boss_icon = {
			{
				"qinraozhe_IV",
				2
			},
			{
				"weixu_baojian_2",
				2
			},
			{
				"weixu_baojian_4",
				12
			}
		},
		pass_awards_display = {
			{
				1,
				588,
				80
			},
			{
				1,
				587,
				14
			},
			{
				2,
				200627,
				0
			},
			{
				2,
				54017,
				0
			},
			{
				2,
				200633,
				0
			},
			{
				2,
				59001,
				0
			}
		},
		additional_awards_display = {
			{
				1,
				588,
				20
			},
			{
				1,
				1,
				0
			}
		},
		defeat_story = {},
		defeat_story_count = {},
		use_oil_limit = {
			0,
			0
		}
	}
	pg.base.activity_series_enemy[4031] = {
		oil = 0,
		name = "보통（LV 50）",
		type = 1,
		pre_chapter = 0,
		chapter_name = "无用",
		pos_x = "0",
		pos_y = "0",
		count = 0,
		ex_count = "",
		profiles = "[AREA-4] 작전이 개시되었다. 로열 네이비 함대와 노스 유니온 함대와 함께 해역을 소탕하자.",
		collection_group_id = 0,
		whether_singlefight = 1,
		id = 4031,
		limitation = {},
		expedition_id = {
			1981009,
			1981013,
			1981005
		},
		boss_icon = {
			{
				"qinraozhe_IV",
				2
			},
			{
				"weixu_baojian_2",
				2
			},
			{
				"weixu_baojian_5",
				3
			}
		},
		pass_awards_display = {
			{
				1,
				588,
				60
			},
			{
				1,
				587,
				14
			},
			{
				2,
				200622,
				0
			},
			{
				2,
				54017,
				0
			},
			{
				2,
				200633,
				0
			},
			{
				2,
				59001,
				0
			}
		},
		additional_awards_display = {
			{
				1,
				588,
				15
			},
			{
				1,
				1,
				0
			}
		},
		defeat_story = {},
		defeat_story_count = {},
		use_oil_limit = {
			0,
			0
		}
	}
	pg.base.activity_series_enemy[4032] = {
		oil = 0,
		name = "어려움（LV 65）",
		type = 1,
		pre_chapter = 0,
		chapter_name = "无用",
		pos_x = "0",
		pos_y = "0",
		count = 0,
		ex_count = "",
		profiles = "[AREA-4] 작전이 개시되었다. 로열 네이비 함대와 노스 유니온 함대와 함께 해역을 소탕하자.",
		collection_group_id = 0,
		whether_singlefight = 1,
		id = 4032,
		limitation = {},
		expedition_id = {
			1982009,
			1982013,
			1982005
		},
		boss_icon = {
			{
				"qinraozhe_IV",
				2
			},
			{
				"weixu_baojian_2",
				2
			},
			{
				"weixu_baojian_5",
				3
			}
		},
		pass_awards_display = {
			{
				1,
				588,
				80
			},
			{
				1,
				587,
				14
			},
			{
				2,
				200628,
				0
			},
			{
				2,
				54017,
				0
			},
			{
				2,
				200633,
				0
			},
			{
				2,
				59001,
				0
			}
		},
		additional_awards_display = {
			{
				1,
				588,
				20
			},
			{
				1,
				1,
				0
			}
		},
		defeat_story = {},
		defeat_story_count = {},
		use_oil_limit = {
			0,
			0
		}
	}
	pg.base.activity_series_enemy[4041] = {
		oil = 0,
		name = "보통（LV 50）",
		type = 1,
		pre_chapter = 0,
		chapter_name = "无用",
		pos_x = "0",
		pos_y = "0",
		count = 0,
		ex_count = "",
		profiles = "[AREA-5] 작전이 개시되었다. 사쿠라 엠파이어와 튤리퍼 함대와 함께 해역을 소탕하자.",
		collection_group_id = 0,
		whether_singlefight = 1,
		id = 4041,
		limitation = {},
		expedition_id = {
			1981010,
			1981014,
			1981006
		},
		boss_icon = {
			{
				"qinraozhe_IV",
				2
			},
			{
				"weixu_baojian_2",
				2
			},
			{
				"weixu_baojian_6",
				13
			}
		},
		pass_awards_display = {
			{
				1,
				588,
				60
			},
			{
				1,
				587,
				14
			},
			{
				2,
				200623,
				0
			},
			{
				2,
				54017,
				0
			},
			{
				2,
				200633,
				0
			},
			{
				2,
				59001,
				0
			}
		},
		additional_awards_display = {
			{
				1,
				588,
				15
			},
			{
				1,
				1,
				0
			}
		},
		defeat_story = {},
		defeat_story_count = {},
		use_oil_limit = {
			0,
			0
		}
	}
	pg.base.activity_series_enemy[4042] = {
		oil = 0,
		name = "어려움（LV 65）",
		type = 1,
		pre_chapter = 0,
		chapter_name = "无用",
		pos_x = "0",
		pos_y = "0",
		count = 0,
		ex_count = "",
		profiles = "[AREA-5] 작전이 개시되었다. 사쿠라 엠파이어와 튤리퍼 함대와 함께 해역을 소탕하자.",
		collection_group_id = 0,
		whether_singlefight = 1,
		id = 4042,
		limitation = {},
		expedition_id = {
			1982010,
			1982014,
			1982006
		},
		boss_icon = {
			{
				"qinraozhe_IV",
				2
			},
			{
				"weixu_baojian_2",
				2
			},
			{
				"weixu_baojian_6",
				13
			}
		},
		pass_awards_display = {
			{
				1,
				588,
				80
			},
			{
				1,
				587,
				14
			},
			{
				2,
				200629,
				0
			},
			{
				2,
				54017,
				0
			},
			{
				2,
				200633,
				0
			},
			{
				2,
				59001,
				0
			}
		},
		additional_awards_display = {
			{
				1,
				588,
				20
			},
			{
				1,
				1,
				0
			}
		},
		defeat_story = {},
		defeat_story_count = {},
		use_oil_limit = {
			0,
			0
		}
	}
	pg.base.activity_series_enemy[4051] = {
		oil = 0,
		name = "보통（LV 80）",
		type = 1,
		pre_chapter = 0,
		chapter_name = "无用",
		pos_x = "0",
		pos_y = "0",
		count = 0,
		ex_count = "",
		profiles = "[AREA-6] 결전의 때가 왔다. 모항 연합 함대로 나이트메어 미궁을 격파하자.",
		collection_group_id = 0,
		whether_singlefight = 1,
		id = 4051,
		limitation = {},
		expedition_id = {
			1983001,
			1983002,
			1983003,
			1983004
		},
		boss_icon = {
			{
				"weixu_baojian_3",
				2
			},
			{
				"weixu_baojian_5",
				3
			},
			{
				"weixu_baojian_6",
				13
			},
			{
				"weixu_baojian_7",
				6
			}
		},
		pass_awards_display = {
			{
				1,
				588,
				120
			},
			{
				1,
				587,
				16
			},
			{
				2,
				200633,
				0
			},
			{
				2,
				200624,
				0
			},
			{
				2,
				54017,
				0
			},
			{
				2,
				200631,
				0
			}
		},
		additional_awards_display = {
			{
				1,
				588,
				25
			},
			{
				1,
				1,
				0
			}
		},
		defeat_story = {},
		defeat_story_count = {},
		use_oil_limit = {
			22,
			15
		}
	}
	pg.base.activity_series_enemy[4052] = {
		oil = 0,
		name = "어려움（LV 100）",
		type = 1,
		pre_chapter = 0,
		chapter_name = "无用",
		pos_x = "0",
		pos_y = "0",
		count = 0,
		ex_count = "",
		profiles = "[AREA-6] 결전의 때가 왔다. 모항 연합 함대로 나이트메어 미궁을 격파하자.",
		collection_group_id = 0,
		whether_singlefight = 1,
		id = 4052,
		limitation = {},
		expedition_id = {
			1984001,
			1984002,
			1984003,
			1984004
		},
		boss_icon = {
			{
				"weixu_baojian_3",
				2
			},
			{
				"weixu_baojian_5",
				3
			},
			{
				"weixu_baojian_6",
				13
			},
			{
				"weixu_baojian_7",
				6
			}
		},
		pass_awards_display = {
			{
				1,
				588,
				160
			},
			{
				1,
				587,
				16
			},
			{
				2,
				200633,
				0
			},
			{
				2,
				200630,
				0
			},
			{
				2,
				54017,
				0
			},
			{
				2,
				200632,
				0
			}
		},
		additional_awards_display = {
			{
				1,
				588,
				30
			},
			{
				1,
				1,
				0
			}
		},
		defeat_story = {},
		defeat_story_count = {},
		use_oil_limit = {
			25,
			16
		}
	}
	pg.base.activity_series_enemy[5001] = {
		oil = 0,
		name = "나비춤의 약속",
		type = 1,
		pre_chapter = 0,
		chapter_name = "이지",
		pos_x = "0",
		pos_y = "0",
		count = 0,
		ex_count = "",
		additional_awards_display = "",
		profiles = [[
낭만적이고 가벼운 비단이
달콤한 밀회를 그려내고
리본 위의 나비는
속박과 자유 사이를 유유히 날아오른다.]],
		collection_group_id = 104,
		whether_singlefight = 0,
		id = 5001,
		limitation = {},
		expedition_id = {
			1999101
		},
		boss_icon = {
			{
				"aerbeituo_2",
				2
			}
		},
		pass_awards_display = {
			{
				1,
				616,
				40
			},
			{
				2,
				200676,
				0
			},
			{
				2,
				59001,
				0
			},
			{
				2,
				54012,
				0
			}
		},
		defeat_story = {},
		defeat_story_count = {},
		use_oil_limit = {
			0,
			0
		}
	}
	pg.base.activity_series_enemy[5002] = {
		oil = 0,
		name = "포근한 벨벳 판타지",
		type = 1,
		pre_chapter = 5001,
		chapter_name = "노멀",
		pos_x = "0",
		pos_y = "0",
		count = 0,
		ex_count = "",
		profiles = [[
포근함이 흘러내려
서로 얽히고 끌어안으며
따뜻한 환경 속에서
다정함으로 새로운 세상을 만들어 낸다.]],
		collection_group_id = 105,
		whether_singlefight = 1,
		id = 5002,
		limitation = {},
		expedition_id = {
			1999201,
			1999202
		},
		boss_icon = {
			{
				"alabama_3",
				5
			},
			{
				"wugelini_2",
				1
			}
		},
		pass_awards_display = {
			{
				1,
				616,
				140
			},
			{
				2,
				200675,
				0
			},
			{
				2,
				59001,
				0
			},
			{
				2,
				54017,
				0
			}
		},
		additional_awards_display = {
			{
				1,
				616,
				25
			},
			{
				1,
				1,
				0
			}
		},
		defeat_story = {},
		defeat_story_count = {},
		use_oil_limit = {
			0,
			0
		}
	}
	pg.base.activity_series_enemy[5003] = {
		oil = 0,
		name = "마음의 창",
		type = 1,
		pre_chapter = 5002,
		chapter_name = "하드",
		pos_x = "0",
		pos_y = "0",
		count = 0,
		ex_count = "",
		profiles = [[
실로 엮은 약속이
숨결마다 잔잔히 물결치고
흐르는 선들이 흰 장막을 가르며
마음의 창으로 빛을 스며들게 한다.]],
		collection_group_id = 106,
		whether_singlefight = 1,
		id = 5003,
		limitation = {},
		expedition_id = {
			1999301,
			1999302,
			1999303
		},
		boss_icon = {
			{
				"aerbeituo_2",
				2
			},
			{
				"gaoxiong_7",
				3
			},
			{
				"fulangxisike_2",
				5
			}
		},
		pass_awards_display = {
			{
				1,
				616,
				360
			},
			{
				2,
				200674,
				0
			},
			{
				2,
				59001,
				0
			},
			{
				2,
				54017,
				0
			}
		},
		additional_awards_display = {
			{
				1,
				616,
				60
			},
			{
				1,
				1,
				0
			}
		},
		defeat_story = {},
		defeat_story_count = {},
		use_oil_limit = {
			25,
			16
		}
	}
	pg.base.activity_series_enemy[5004] = {
		oil = 0,
		name = "순백의 마음",
		type = 2,
		pre_chapter = 5003,
		chapter_name = "S.P",
		pos_x = "0",
		pos_y = "0",
		count = 1,
		ex_count = "",
		additional_awards_display = "",
		profiles = [[
달빛 어린 비단이 나부끼고
진주의 광채가 반짝인다.
살짝 드러난 천의 틈새로
순수와 유혹의 선율이 흘러나온다.]],
		collection_group_id = 107,
		whether_singlefight = 0,
		id = 5004,
		limitation = {},
		expedition_id = {
			1999401,
			1999402,
			1999403,
			1999404
		},
		boss_icon = {
			{
				"aerbeituo_2",
				2
			},
			{
				"wugelini_2",
				1
			},
			{
				"fulangxisike_2",
				5
			},
			{
				"alabama_3",
				5
			}
		},
		pass_awards_display = {
			{
				1,
				616,
				800
			},
			{
				2,
				200673,
				0
			},
			{
				2,
				59001,
				0
			},
			{
				2,
				54016,
				0
			}
		},
		defeat_story = {},
		defeat_story_count = {},
		use_oil_limit = {
			40,
			16
		}
	}
	pg.base.activity_series_enemy[5005] = {
		oil = 0,
		name = "깊은 밤의 속삭임",
		type = 3,
		pre_chapter = 5004,
		chapter_name = "E.X",
		pos_x = "0",
		pos_y = "0",
		count = 0,
		additional_awards_display = "",
		profiles = [[
베일이 어둠 속에 잠기고
이글거리는 무늬가 얽혀든다.
고요한 밤, 속삭임이 흔들린다.
스스로를 억누를까? 모든 것을 맡길까?]],
		pass_awards_display = "",
		collection_group_id = 0,
		whether_singlefight = 0,
		id = 5005,
		limitation = {},
		expedition_id = {
			1999501,
			1999502,
			1999503,
			1999504,
			1999505
		},
		boss_icon = {
			{
				"aerbeituo_2",
				2
			},
			{
				"wugelini_2",
				1
			},
			{
				"fulangxisike_2",
				5
			},
			{
				"alabama_3",
				5
			},
			{
				"gaoxiong_7",
				3
			}
		},
		defeat_story = {},
		defeat_story_count = {},
		use_oil_limit = {
			0,
			0
		},
		ex_count = {
			8000,
			20,
			0.2,
			1000,
			0.8
		}
	}
	pg.base.activity_series_enemy[1001001] = {
		oil = 0,
		name = "EASY：교란 저지",
		type = 1,
		pre_chapter = 0,
		chapter_name = "TC1",
		pos_x = "0.10703125",
		pos_y = "0.157291667",
		count = 0,
		ex_count = "",
		additional_awards_display = "",
		profiles = "작전 목표：서부에 있는 세이렌 요새를 제압하고, 재밍 장치를 파괴하여 기간 통신망을 복구하라!",
		collection_group_id = 104,
		whether_singlefight = 0,
		id = 1001001,
		limitation = {},
		expedition_id = {
			1719101
		},
		boss_icon = {
			{
				"qinraozhe",
				2
			}
		},
		pass_awards_display = {
			{
				2,
				58839
			},
			{
				2,
				59001
			},
			{
				2,
				54012
			}
		},
		defeat_story = {},
		defeat_story_count = {},
		use_oil_limit = {
			0,
			0
		}
	}
	pg.base.activity_series_enemy[1001002] = {
		oil = 0,
		name = "NORMAL：기상 복원",
		type = 1,
		pre_chapter = 1001001,
		chapter_name = "TC2",
		pos_x = "0.34609375",
		pos_y = "0.347916667",
		count = 0,
		ex_count = "",
		profiles = "작전 목표：동부에 있는 세이렌 방어선을 돌파하고, 기상 제어 장치를 파괴하여 각 해역의 이상 기상을 정상화하라!",
		collection_group_id = 105,
		whether_singlefight = 1,
		id = 1001002,
		limitation = {},
		expedition_id = {
			1719201,
			1719202
		},
		boss_icon = {
			{
				"qinraozhe",
				2
			},
			{
				"qingchuzhe",
				5
			}
		},
		pass_awards_display = {
			{
				2,
				58838
			},
			{
				2,
				59001
			},
			{
				2,
				54017
			}
		},
		additional_awards_display = {
			{
				1,
				1
			}
		},
		defeat_story = {},
		defeat_story_count = {},
		use_oil_limit = {
			0,
			0
		}
	}
	pg.base.activity_series_enemy[1001003] = {
		oil = 0,
		name = "HARD：기지 탈환",
		type = 1,
		pre_chapter = 1001002,
		chapter_name = "TC3",
		pos_x = "0.50546875",
		pos_y = "0.080208333",
		count = 0,
		ex_count = "",
		profiles = "작전 목표：북부의 조사 기지를 탈환하고, 주변의 세이렌 생산 시설을 파괴하여 적 증원 병력의 증강을 저지하라!",
		collection_group_id = 106,
		whether_singlefight = 1,
		id = 1001003,
		limitation = {},
		expedition_id = {
			1719301,
			1719302,
			1719303
		},
		boss_icon = {
			{
				"qinraozhe_IV",
				2
			},
			{
				"kuersike",
				3
			},
			{
				"qingchuzhe",
				5
			}
		},
		pass_awards_display = {
			{
				2,
				58837
			},
			{
				2,
				59001
			},
			{
				2,
				54017
			}
		},
		additional_awards_display = {
			{
				1,
				1
			}
		},
		defeat_story = {},
		defeat_story_count = {},
		use_oil_limit = {
			25,
			16
		}
	}
end)()
