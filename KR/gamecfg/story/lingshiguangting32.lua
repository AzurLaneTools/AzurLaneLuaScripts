return {
	id = "LINGSHIGUANGTING32",
	mode = 2,
	once = true,
	fadeType = 1,
	fadein = 1.5,
	scripts = {
		{
			say = "모두가 카멜롯의 정원에 접근하려고 할 때, 눈부신 황금색 빛이 바다를 휩쓸었다.",
			side = 2,
			bgName = "bg_camelot_13",
			dir = 1,
			bgmDelay = 1,
			bgm = "theme-camelot",
			flashin = {
				delay = 1,
				dur = 1,
				black = true,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_camelot_13",
			actor = 205130,
			dir = 1,
			nameColor = "#a9f548",
			say = "폐하, 카멜롯의 정원이 스스로 작동하는 것 같습니다만…?!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 205010,
			side = 2,
			bgName = "bg_camelot_13",
			nameColor = "#a9f548",
			dir = 1,
			say = "아니야, 누군가 작동시킨 거다.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 205010,
			side = 2,
			bgName = "bg_camelot_13",
			nameColor = "#a9f548",
			dir = 1,
			say = "카멜롯의 정원은 한 세계에 하나만 존재해. 셰필드의 지원군이 도착한 것 같군.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 203049,
			nameColor = "#a9f548",
			bgName = "bg_camelot_13",
			side = 2,
			dir = 1,
			actorName = "서포크·改",
			say = "폐하, 폐하를 호위하러 서포크가 왔습니다.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 202080,
			side = 2,
			bgName = "bg_camelot_13",
			nameColor = "#a9f548",
			dir = 1,
			say = "늦어서 죄송합니다.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 207029,
			nameColor = "#a9f548",
			bgName = "bg_camelot_13",
			side = 2,
			dir = 1,
			actorName = "아크로열·改",
			say = "폐하께서 위험에 처하셨다는 말을 듣고 아크로열 어찌 가만히 있을 수 있겠습니까!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 206039,
			nameColor = "#a9f548",
			bgName = "bg_camelot_13",
			side = 2,
			dir = 1,
			actorName = "유니콘·改",
			say = "유니콘도 도와드리려고 왔습니다!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 202039,
			nameColor = "#a9f548",
			bgName = "bg_camelot_13",
			side = 2,
			dir = 1,
			actorName = "에이잭스·改",
			say = "먼 곳에 있는 저것들이 적인가요? 이제 저희도 전투에 참여하겠습니다~",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900328,
			side = 2,
			bgName = "bg_camelot_13",
			nameColor = "#a9f548",
			dir = 1,
			say = "모두 다치지 않게 조심해주세요….",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_camelot_13",
			say = "카멜롯의 정원은 마치 각지를 연결하는 봉화처럼 여왕의 위험 소식을 전했고, 동시에 위기를 돌파할 희망을 한곳에 모았다.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 205010,
			side = 2,
			bgName = "bg_camelot_13",
			nameColor = "#a9f548",
			dir = 1,
			say = "개조 함대까지 데려오다니, 너무 오버한 거 아니야? 워스파이트….",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 205029,
			nameColor = "#a9f548",
			bgName = "bg_camelot_13",
			side = 2,
			dir = 1,
			actorName = "워스파이트·改",
			say = "갑작스러운 일이 아니고, 또 비밀 임무만 아녔다면, 로열 전력의 절반을 이끌고 왔을 겁니다!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_camelot_13",
			nameColor = "#a9f548",
			dir = 1,
			actor = 205029,
			actorName = "워스파이트·改",
			say = "말없이 사라진 폐하께서 반성하셔야 할 일입니다.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 205010,
			side = 2,
			bgName = "bg_camelot_13",
			nameColor = "#a9f548",
			dir = 1,
			say = "미안. 이번에는 나의 잘못이다. 너희들을 걱정시켰어.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900327,
			side = 2,
			bgName = "bg_camelot_13",
			nameColor = "#ffa500",
			dir = 1,
			say = "…언니, 여기 뭔가 좀 낯이 익어.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_camelot_13",
			actor = 900326,
			dir = 1,
			nameColor = "#ffa500",
			say = "동감이에요, 안티엑스가 이곳에서 어떤 실험을 하고 있는 건지 모르겠네요….",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 205010,
			side = 2,
			bgName = "bg_camelot_13",
			nameColor = "#a9f548",
			dir = 1,
			say = "두 사람이 바로 숲속의 성에서 뱅가드가 만났다던 META?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 205010,
			side = 2,
			bgName = "bg_camelot_13",
			nameColor = "#a9f548",
			dir = 1,
			say = "(우연히 딱 들어맞는 상황은 지금 이런 상황을 말하는 거겠지….)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_camelot_13",
			actor = 900326,
			dir = 1,
			nameColor = "#ffa500",
			say = "맞습니다. 전 리나운, 여기는 제 동생 리펄스입니다.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_camelot_13",
			actor = 900326,
			dir = 1,
			nameColor = "#ffa500",
			say = "한동안 잔불과 전투를 진행해오다가 평화로운 삶을 위해 은거 생활을 하던 중 맹세를 이행하기 위해 다시 이렇게 전장으로 돌아왔습니다.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_camelot_13",
			actor = 900326,
			dir = 1,
			nameColor = "#ffa500",
			say = "으음…",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 205010,
			side = 2,
			bgName = "bg_camelot_13",
			nameColor = "#a9f548",
			dir = 1,
			say = "에, 너흰 좀 특별하니까 엘리자베스 혹은 퀸 엘리자베스라고 불러도 좋아. 폐하라고 부르라곤 강요하지 않을게.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_camelot_13",
			actor = 900327,
			dir = 1,
			nameColor = "#ffa500",
			say = "그럼 퀸 엘리자베스 폐하는 어떠신가요?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 205010,
			side = 2,
			bgName = "bg_camelot_13",
			nameColor = "#a9f548",
			dir = 1,
			say = "길어서 부르기 귀찮지만 않다면, 그렇게 불러도 상관없어….",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 205010,
			side = 2,
			bgName = "bg_camelot_13",
			nameColor = "#a9f548",
			dir = 1,
			say = "방금 낯이 익다고 얘기한 거 같은데 이곳이 너희 기억 속 어떤 장면을 구현해낸 곳이라는 거야?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_camelot_13",
			actor = 900326,
			dir = 1,
			nameColor = "#ffa500",
			say = "도시들의 느낌은 그렇습니다만 검은 폭풍은 확실하지 않습니다. 적어도 이런 장면에서 이런 상황을 겪은 적은 없습니다.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_camelot_13",
			actor = 205130,
			dir = 1,
			nameColor = "#a9f548",
			say = "폐하, 퓨리파이어의 함대는 이미 전멸되었습니다. 폭풍이 저희 쪽을 향해 빠르게 접근하고 있습니다.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 205010,
			side = 2,
			bgName = "bg_camelot_13",
			nameColor = "#a9f548",
			dir = 1,
			say = "너무 빨라... 세이렌 자식들, 싸울 거면 제대로 된 화력을 꺼낼 것이지….",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 202120,
			side = 2,
			bgName = "bg_camelot_13",
			nameColor = "#a9f548",
			dir = 1,
			say = "폐하, 죄송합니다. 카멜롯의 정원의 연속 작동 횟수가 많아 재작동까지 일정 시간 동안 에너지 충전이 필요할 것 같습니다.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 205010,
			side = 2,
			bgName = "bg_camelot_13",
			nameColor = "#a9f548",
			dir = 1,
			say = "알겠어, 걱정하지 마.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 205010,
			side = 2,
			bgName = "bg_camelot_13",
			nameColor = "#a9f548",
			dir = 1,
			say = "그동안 기습을 당했던 건 준비가 부족했기 때문이지만 지금은 다르다구.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 205010,
			side = 2,
			bgName = "bg_camelot_13",
			nameColor = "#a9f548",
			dir = 1,
			say = "제1 고리 작동, 공간 분할, 폭풍을 차단해라!",
			effects = {
				{
					active = true,
					name = "jinguang"
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_camelot_13",
			say = "바깥쪽에 있는 첫 번째 고리가 회전하면서, 투명한 '막'이 카멜롯의 정원을 감싸 검은 폭풍을 차단했다.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 205010,
			side = 2,
			bgName = "bg_camelot_13",
			nameColor = "#a9f548",
			dir = 1,
			say = "보기에는 얇아도, 너희가 상상하는 것보다 훨씬 견고하다구.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900322,
			side = 2,
			bgName = "bg_camelot_13",
			nameColor = "#BDBDBD",
			dir = 1,
			say = "정말 재미있는 장치네, 라플라스의 도깨비 설계도에 따라 개조한 건가?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 205010,
			side = 2,
			bgName = "bg_camelot_13",
			nameColor = "#a9f548",
			dir = 1,
			say = "노코멘트하지. 여기서 방해하지 말고 다시 네가 왔던 곳으로 가줄래? 세이렌이 네 방해 덕에 기분 나빠하는 거 안 보여?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900327,
			side = 2,
			bgName = "bg_camelot_13",
			nameColor = "#ffa500",
			dir = 1,
			say = "언니, 밖에 있는 사람 설마… 본험 리차드는 아니겠지?!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_camelot_13",
			actor = 900326,
			dir = 1,
			nameColor = "#ffa500",
			say = "아마도… 하지만 그녀가 어떻게….",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 205010,
			side = 2,
			bgName = "bg_camelot_13",
			nameColor = "#a9f548",
			dir = 1,
			say = "응? 본험 리차드에게 무슨 문제가 있어?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_camelot_13",
			actor = 900326,
			dir = 1,
			nameColor = "#ffa500",
			say = "잔불이 안티엑스와 협력해 실험을 진행할 때 제로가 저희에게 말한 적이 있습니다.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_camelot_13",
			actor = 900326,
			dir = 1,
			nameColor = "#ffa500",
			say = "몇 번을 시도해봐도 안티엑스는 완벽하게 본험 리차드의 데이터를 구현해낼 수 없다고 말이죠.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_camelot_13",
			actor = 900326,
			dir = 1,
			nameColor = "#ffa500",
			say = "각 실험장의 혁신적인 발전 속에서도 본험 리차드는 결코 구현할 수 없다구요. 마치 존재 자체가 말끔히 지워진 것처럼 말입니다.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_camelot_13",
			actor = 900326,
			dir = 1,
			nameColor = "#ffa500",
			say = "그래서 리차드 사건 이후에… 살아있는 그녀는 오늘이 처음입니다….",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_camelot_13",
			actor = 900326,
			dir = 1,
			nameColor = "#ffa500",
			say = "어디서 그녀를 찾으신 거죠...?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 205010,
			side = 2,
			bgName = "bg_camelot_13",
			nameColor = "#a9f548",
			dir = 1,
			say = "이건 친애하는 모나크에게 물어봐야 할 것 같은데, 어떻게 한 번의 작전으로 이런 특별한 상을 받게 된 건지 말이야.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 299020,
			side = 2,
			bgName = "bg_camelot_13",
			nameColor = "#a9f548",
			dir = 1,
			say = "…미안. 아마도 내가 운이 너무 나빴던 것 같다.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900322,
			side = 2,
			bgName = "bg_camelot_13",
			nameColor = "#BDBDBD",
			dir = 1,
			say = "에~ 날 돌려보내긴 쉽지 않을걸. 왜냐면 그 누구도 날 막을 수 없거든.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900322,
			side = 2,
			bgName = "bg_camelot_13",
			nameColor = "#BDBDBD",
			dir = 1,
			say = "인간도 안되고, 함선도 안돼. 안티엑스도 당연히 안되지~",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900322,
			side = 2,
			bgName = "bg_camelot_13",
			nameColor = "#BDBDBD",
			dir = 1,
			say = "그리고 너희들이 또 재밌는 동료를 데리고 온 것 같아서 말이지, 거기 두 사람, 혹시 META야?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900327,
			side = 2,
			bgName = "bg_camelot_13",
			nameColor = "#ffa500",
			dir = 1,
			say = "리차드, 우리 몰라?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900322,
			side = 2,
			bgName = "bg_camelot_13",
			nameColor = "#BDBDBD",
			dir = 1,
			say = "어… 모르겠는데. 아마 어디선가 나를 만난 적이 있나 보군.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900322,
			side = 2,
			bgName = "bg_camelot_13",
			nameColor = "#BDBDBD",
			dir = 1,
			say = "내 말 좀 들어봐, 내 말 좀. 어서 그 안에서 저 성가신 기계를 없애줘!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900322,
			side = 2,
			bgName = "bg_camelot_13",
			nameColor = "#BDBDBD",
			dir = 1,
			say = "그러면 내가 들어가서 모두를 우리의 동료로 만들 수 있다구!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900327,
			side = 2,
			bgName = "bg_camelot_13",
			nameColor = "#ffa500",
			dir = 1,
			say = "언니, 리차드의 성격이 좀…….",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_camelot_13",
			actor = 900326,
			dir = 1,
			nameColor = "#ffa500",
			say = "응, 의심할 여지 없이 그년 우리의 기억 속 리차드 양이 아닌 것 같군요. 그리고 그녀에게 느껴지는… 형용할 수 없는 이질감까지요.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_camelot_13",
			actor = 900326,
			dir = 1,
			nameColor = "#ffa500",
			say = "META는 맞지만 뭔가 더 이질적인 다른 게 섞여 있는 것 같아요. 뭔지 확신할 순 없지만… 헬레나 혹은 멤피스가 이곳에 있었으면 더 좋았을 텐데.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900322,
			side = 2,
			bgName = "bg_camelot_13",
			nameColor = "#BDBDBD",
			dir = 1,
			say = "어랏? 뭘 기다리고 있는 거야? 왜 움직이지 않고 있어?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_camelot_13",
			actor = 900326,
			dir = 1,
			nameColor = "#ffa500",
			say = "미안해요, 리차드  양. 저흰 이미 엘리자베스 여왕 폐하를 향해 충성을 맹세했습니다. 저흰 그녀를 무사 귀환시켜야 해요.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_camelot_13",
			actor = 900326,
			dir = 1,
			nameColor = "#ffa500",
			say = "여기서 그만하라고 부탁할 수 있을까요?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900322,
			side = 2,
			bgName = "bg_camelot_13",
			nameColor = "#BDBDBD",
			dir = 1,
			say = "하? 말도 안 돼! 됐어. 믿을만하지 않다는 건 알고 있었으니까.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900322,
			side = 2,
			bgName = "bg_camelot_13",
			nameColor = "#BDBDBD",
			dir = 1,
			say = "이 기계의 에너지를 다 소모시키면 들어갈 수 있겠지? 어차피 난 시간이 많으니까 말이야~",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_camelot_13",
			say = "폭풍이 다시 한번 포효했다.이번 폭풍 속에서는 무수히 많은 적들이 나타났다.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 205010,
			side = 2,
			bgName = "bg_camelot_13",
			nameColor = "#a9f548",
			dir = 1,
			say = "쳇, 이 사람 뭔가 정신적으로 문제가 있는 것 같은데?! 이런 방식으로 인공 특이점의 에너지를 고갈시키려면 몇십 년은 걸린다구!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 205010,
			side = 2,
			bgName = "bg_camelot_13",
			nameColor = "#a9f548",
			dir = 1,
			say = "하지만 이렇게 되면… 나도 작동에 사용되는 에너지를 외부 고리까지 계속해서 보충해야 하는데….",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 205010,
			side = 2,
			bgName = "bg_camelot_13",
			nameColor = "#a9f548",
			dir = 1,
			say = "적을 공격할 수는 없지만, 카멜롯의 정원도 작동시킬 수 없다니….",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 205029,
			nameColor = "#a9f548",
			bgName = "bg_camelot_13",
			side = 2,
			dir = 1,
			actorName = "워스파이트·改",
			say = "폐하, 적은 저희가 책임지겠습니다. 로열의 개조 함대의 실력을 보여줄 때군!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 207029,
			nameColor = "#a9f548",
			bgName = "bg_camelot_13",
			side = 2,
			dir = 1,
			actorName = "아크로열·改",
			say = "안심하세요, 폐하. 아무리 많은 적들이 온다고 해도, 반드시 처리할 수 있습니다!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_camelot_13",
			actor = 205130,
			dir = 1,
			nameColor = "#a9f548",
			say = "저도 기사들을 데리고 전투에 임하도록 하겠습니다. 승리를 위한 방어전이니 무모하게 행동하진 않겠습니다.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 299020,
			side = 2,
			bgName = "bg_camelot_13",
			nameColor = "#a9f548",
			dir = 1,
			say = "나도 가겠다. 저 미치광이의 목표는 나니까 내가 나서면 어느 정도 시간을 벌 수 있을지 몰라.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 205010,
			side = 2,
			bgName = "bg_camelot_13",
			nameColor = "#a9f548",
			dir = 1,
			say = "좋아! 전부 허락한다! 방어벽을 중간 고리로 옮길 테니, 중간과 외부 고리 사이에서 전투해.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 205010,
			side = 2,
			bgName = "bg_camelot_13",
			nameColor = "#a9f548",
			dir = 1,
			say = "카멜롯의 정원이 제대로 준비되기 전까지 외부 고리로 침입한 모든 적을 소탕해야해!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 205010,
			side = 2,
			bgName = "bg_camelot_13",
			nameColor = "#a9f548",
			dir = 1,
			say = "하지만 절대 외부 고리를 벗어나는 건 안 돼. 시동 준비가 완료되면 즉시 철수할 거니까!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_camelot_13",
			actor = 205130,
			dir = 1,
			blackBg = true,
			nameColor = "#a9f548",
			say = "명령에 따르겠습니다-로열의 기사와 전사들이여, 나를 따라 적에 맞서 싸우자!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		}
	}
}
