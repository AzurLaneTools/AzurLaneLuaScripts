return {
	fadeOut = 1.5,
	mode = 2,
	id = "ZOUXIANGYUANWEIZHIGE1",
	once = true,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			stopbgm = true,
			mode = 1,
			asideType = 3,
			sequence = {
				{
					"현재, 사르데냐 엠파이어 주관 세계 박람회",
					1
				},
				{
					"……와는 다른 사르데냐 엠파이어의 모처.",
					2
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = true,
				alpha = {
					1,
					0
				}
			}
		},
		{
			side = 2,
			nameColor = "#A9F548FF",
			bgName = "bg_midgard_9",
			hidePaintObj = true,
			bgm = "Theme-marcopolo",
			bgmDelay = 1,
			say = "정적에 휩싸인 넓은 방, 촛불만이 쓸쓸하게 빛을 발하고 있다.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_midgard_9",
			hidePaintObj = true,
			say = "바닥에 드리워진 그림자는 갸냘프게, 폭풍으로 겁에 질린 잔가지처럼 계속 흔들리고 있다.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_midgard_9",
			hidePaintObj = true,
			dir = 1,
			nameColor = "#ffa500",
			actor = 9703010,
			say = "……조짐이 나타났다.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_midgard_9",
			hidePaintObj = true,
			dir = 1,
			nameColor = "#ffa500",
			actor = 9703010,
			say = "……그늘진 구름이 걷히고",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_midgard_9",
			hidePaintObj = true,
			dir = 1,
			nameColor = "#ffa500",
			actor = 9703010,
			say = "……땅을 밝게 비추는 거룩한 빛.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_midgard_9",
			hidePaintObj = true,
			dir = 1,
			nameColor = "#ffa500",
			actor = 9703010,
			say = "……위대한 존재가 드디어 강림하리니.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_midgard_9",
			hidePaintObj = true,
			dir = 1,
			nameColor = "#ffa500",
			actor = 9703010,
			say = "……빛나는 영광으로 가득 찬, 그 화신",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_midgard_9",
			hidePaintObj = true,
			dir = 1,
			nameColor = "#ffa500",
			actor = 9703010,
			say = "……그리고…………",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_midgard_9",
			hidePaintObj = true,
			dir = 1,
			nameColor = "#92fc63",
			actor = 699010,
			say = "그만! 여전히 무슨 말인진 모르겠지만…",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 30,
					delay = 0,
					dur = 0.2,
					x = 0,
					number = 2
				},
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_midgard_9",
			hidePaintObj = true,
			dir = 1,
			nameColor = "#92fc63",
			actor = 699010,
			say = "그러니까 '위대한 존재'의 화신이니 뭐니가 드디어 나타난다는 거지?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_midgard_9",
			hidePaintObj = true,
			dir = 1,
			nameColor = "#92fc63",
			actor = 699010,
			say = "좋아! 결국 내 정보가 맞았다는 거네.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "bg_midgard_9",
			hidePaintObj = true,
			dir = 1,
			nameColor = "#92fc63",
			actor = 699010,
			say = "오호호홋!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_midgard_9",
			hidePaintObj = true,
			dir = 1,
			nameColor = "#92fc63",
			actor = 699010,
			say = "내가 이 순간만을 얼마나 기다렸는지!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_midgard_9",
			hidePaintObj = true,
			dir = 1,
			nameColor = "#92fc63",
			actor = 699010,
			say = "위광을 퍼뜨리는 위업, 이를 위한 마지막 조각이 드디어 갖춰졌어!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			effects = {
				{
					active = true,
					name = "speed"
				}
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_midgard_9",
			hidePaintObj = true,
			dir = 1,
			nameColor = "#92fc63",
			actor = 699010,
			say = "자, 위대한 존재… '신'이시여.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			effects = {
				{
					active = false,
					name = "speed"
				}
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_midgard_9",
			hidePaintObj = true,
			dir = 1,
			nameColor = "#92fc63",
			actor = 699010,
			say = "'모든 소원을 이뤄준다'고 했지?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "bg_midgard_9",
			hidePaintObj = true,
			dir = 1,
			nameColor = "#92fc63",
			actor = 699010,
			say = "아하하하! 오호호홋!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "move",
					y = 0,
					delay = 1.2,
					dur = 1,
					x = -2500
				}
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_midgard_9",
			hidePaintObj = true,
			say = "크게 웃음을 터뜨리며 마르코 폴로는 쓸쓸한 성당을 뒤로 했다.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_midgard_9",
			hidePaintObj = true,
			say = "활짝 열린 문에서 불어온 바람으로 마지막 촛불이 꺼졌다.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_midgard_9",
			hidePaintObj = true,
			dir = 1,
			nameColor = "#ffa500",
			actor = 9703010,
			say = "……그리고 다시 구름이 그늘지며",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_midgard_9",
			hidePaintObj = true,
			dir = 1,
			nameColor = "#ffa500",
			actor = 9703010,
			say = "……세상은 거짓된 빛에 감싸이네.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_midgard_9",
			hidePaintObj = true,
			dir = 1,
			nameColor = "#ffa500",
			actor = 9703010,
			say = "……그것이 바로 '신'이 만든, 영원한 가짜 낙원……",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			mode = 1,
			stopbgm = true,
			effects = {
				{
					active = true,
					name = "heimutexiao1"
				}
			},
			sequence = {
				{
					"",
					1
				}
			}
		},
		{
			mode = 1,
			stopbgm = false,
			bgm = "theme-clemenceau",
			bgmDelay = 1,
			soundeffect = "event:/ui/dg-dutiao",
			effects = {
				{
					active = false,
					name = "heimutexiao1"
				}
			},
			sequence = {
				{
					"기록 재생: 심판정과의 회담. 적색중축 성립 전.",
					2.5
				},
				{
					">>\"너무 순진하네, 리슐리외. 상층부는 이 정도론 만족 못 해.\"",
					3
				},
				{
					">>\"메탈 블러드의 침공…. '재현' 중 일부는 일어날 거야.\"",
					3.5
				},
				{
					">>각오는 하고 있습니다.",
					4
				},
				{
					">>문제는… 그것이 언제 어디서 어떻게 일어나냐는 거겠죠.",
					4.5
				},
				{
					">>\"글쎄, 일단은 정보 수집을 강화해 둘게.\"",
					5
				}
			}
		},
		{
			soundeffect = "event:/ui/dg-dutiao",
			mode = 1,
			sequence = {
				{
					"기록 재생: 심판정과의 회담. 적색중축 성립 전.",
					2.5
				},
				{
					">>로열 네이비의…… 상층부는 어떨까요?",
					3
				},
				{
					">>\"'우리 시대에 평화가 찾아온다'…...고 했던 거?\"",
					3.5
				},
				{
					">>혹시… 로열 네이비와 메탈 블러드, 상층부가 처음부터 결탁한 거라면…",
					4
				},
				{
					">>바람직한 전개라고 할 수는 없겠죠.",
					4.5
				},
				{
					">>\"하지만 이렇다 할 증거는 없잖아.\"",
					5
				},
				{
					">>\"로열 네이비 쪽은 상황을 지켜보고 있는 걸지도 몰라.\"",
					5.5
				},
				{
					">>그렇다면 저희 쪽에서 나서야겠죠.",
					6
				},
				{
					">>로열 네이비 함선 방문단이 지금 머무르고 있으니, 우선은 그녀들부터……",
					6.5
				}
			}
		},
		{
			mode = 1,
			blackBg = true,
			effects = {
				{
					active = true,
					name = "zouxiangyuanweizhige"
				}
			},
			sequence = {
				{
					"",
					1
				}
			}
		},
		{
			dialogueBgAlpha = 0.6,
			side = 2,
			bgName = "bg_firedust_5",
			nameColor = "#A9F548FF",
			bgmDelay = 2,
			bgm = "theme-vichy-church",
			stopbgm = true,
			hidePaintObj = true,
			say = "현재, 아이리스 리브레의 주력함대",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			effects = {
				{
					active = false,
					name = "zouxiangyuanweizhige"
				},
				{
					active = true,
					name = "yuanzhou_juqing01",
					interlayer = 1001
				}
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_firedust_5",
			hidePaintObj = true,
			dialogueBgAlpha = 0.6,
			say = "성당 봉인 후, 아이리스 리브레에 귀환 중인 리슐리외는 꿈을 꾸고 있다.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_firedust_5",
			dialogueBgAlpha = 0.6,
			dir = 1,
			actor = 805010,
			nameColor = "#92fc63",
			hidePaintObj = true,
			say = "…………또 이 꿈……",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_firedust_5",
			dialogueBgAlpha = 0.6,
			dir = 1,
			actor = 805010,
			nameColor = "#92fc63",
			hidePaintObj = true,
			say = "……장 바르, 거기 있나요?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 902020,
			nameColor = "#BDBDBD",
			bgName = "bg_firedust_5",
			hidePaintObj = true,
			side = 2,
			actorName = "█ █ █ █ █",
			dialogueBgAlpha = 0.6,
			say = "▇▆▅■▎▃▌█긴급▇█▎■▄▆▋▂▁▅태█▊▇▊▇",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_firedust_5",
			dialogueBgAlpha = 0.6,
			dir = 1,
			actor = 805010,
			nameColor = "#92fc63",
			hidePaintObj = true,
			say = "……당신……은……",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_firedust_5",
			dialogueBgAlpha = 0.6,
			dir = 1,
			actor = 805010,
			nameColor = "#92fc63",
			hidePaintObj = true,
			say = "마르세예즈……?!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 30,
					delay = 0,
					dur = 0.2,
					x = 0,
					number = 2
				},
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			actor = 902020,
			nameColor = "#BDBDBD",
			bgName = "bg_firedust_5",
			hidePaintObj = true,
			side = 2,
			actorName = "█ █ █ █ █",
			dialogueBgAlpha = 0.6,
			say = "█▊▇위기▅▁▂▋▄▊상█▌",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_firedust_5",
			dialogueBgAlpha = 0.6,
			dir = 1,
			actor = 805010,
			nameColor = "#92fc63",
			hidePaintObj = true,
			say = "……위기 상황……?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 902020,
			nameColor = "#BDBDBD",
			bgName = "bg_firedust_5",
			hidePaintObj = true,
			side = 2,
			actorName = "█ █ █ █ █",
			dialogueBgAlpha = 0.6,
			say = "█▆어서▇█",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 902020,
			nameColor = "#BDBDBD",
			bgName = "bg_firedust_5",
			hidePaintObj = true,
			side = 2,
			actorName = "█ █ █ █ █",
			dialogueBgAlpha = 0.6,
			say = "█▆리슐█▎█",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 902020,
			nameColor = "#BDBDBD",
			bgName = "bg_firedust_5",
			hidePaintObj = true,
			side = 2,
			actorName = "█ █ █ █ █",
			dialogueBgAlpha = 0.6,
			say = "█▎█▎천사▅█▎█▎▌구원▅▎▌█",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 902020,
			nameColor = "#BDBDBD",
			bgName = "bg_firedust_5",
			hidePaintObj = true,
			side = 2,
			actorName = "█ █ █ █ █",
			dialogueBgAlpha = 0.6,
			say = "█▆도와▇█▂▁▅세█▎█",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			stopbgm = true,
			bgName = "star_level_bg_1104",
			hidePaintObj = true,
			nameColor = "#92fc63",
			actorName = "？？",
			say = "리슐리외 추기경?",
			flashout = {
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			effects = {
				{
					active = false,
					name = "yuanzhou_juqing01"
				}
			}
		},
		{
			side = 2,
			actorName = "？？",
			bgName = "star_level_bg_1104",
			hidePaintObj = true,
			nameColor = "#92fc63",
			say = "리슐리외 추기경님~~?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_guild_blue",
			dir = 1,
			bgmDelay = 2,
			bgm = "theme-kerguelen",
			actor = 801070,
			nameColor = "#92fc63",
			hidePaintObj = true,
			say = "괜찮으세요, 추기경님?",
			flashin = {
				delay = 1,
				dur = 1,
				alpha = {
					1,
					0
				}
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_guild_blue",
			hidePaintObj = true,
			dir = 1,
			nameColor = "#92fc63",
			actor = 805010,
			say = "……음, 머리가 좀 무거워서……",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_guild_blue",
			hidePaintObj = true,
			dir = 1,
			nameColor = "#92fc63",
			actor = 805010,
			say = "죄송해요, 잠깐 잠이 든 모양이네요.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_guild_blue",
			hidePaintObj = true,
			dir = 1,
			nameColor = "#92fc63",
			actor = 805010,
			say = "괜찮습니다. 그것보다 테리블, 무슨 일이죠?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_guild_blue",
			hidePaintObj = true,
			dir = 1,
			nameColor = "#92fc63",
			actor = 801070,
			say = "실은 중요하게 보고 드릴 건이 있어서…",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_guild_blue",
			hidePaintObj = true,
			dir = 1,
			nameColor = "#92fc63",
			actor = 801070,
			say = "심판정…… 클레망소 님에게서 연락이 왔는데",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_guild_blue",
			hidePaintObj = true,
			dir = 1,
			nameColor = "#92fc63",
			actor = 801070,
			say = "\"때가 무르익었다. 슬슬 아이리스의 정통성을 되찾을 때다.\"",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_guild_blue",
			hidePaintObj = true,
			dir = 1,
			nameColor = "#92fc63",
			actor = 805010,
			say = "……알겠습니다.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_guild_blue",
			hidePaintObj = true,
			dir = 1,
			nameColor = "#92fc63",
			actor = 801070,
			say = "그다지 동요하지 않으시는 것 같네요.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_guild_blue",
			hidePaintObj = true,
			dir = 1,
			nameColor = "#92fc63",
			actor = 805010,
			say = "'성좌의 왕관'은 아이리스의 정통성을 상징하는 것…… 그러나 상징은 그저 상징에 지나지 않습니다.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_guild_blue",
			hidePaintObj = true,
			dir = 1,
			nameColor = "#92fc63",
			actor = 805010,
			say = "믿는 자의 믿음이 있어야 비로소 스스로 정통성을 부여할 수 있는 것이지요.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_guild_blue",
			hidePaintObj = true,
			dir = 1,
			nameColor = "#92fc63",
			actor = 805010,
			say = "그렇다고 이 연락을 무의미하다고 치부할 순 없어요.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_guild_blue",
			hidePaintObj = true,
			dir = 1,
			nameColor = "#92fc63",
			actor = 805010,
			say = "심판정이 이 정보를 이쪽으로 전달했다는 건, 변화의 바람이 불고 있다는 증명이겠죠.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_guild_blue",
			hidePaintObj = true,
			dir = 1,
			nameColor = "#92fc63",
			actor = 805010,
			say = "'성좌의 왕관'으로 통하는 열쇠…는 몰타에 있어요.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_guild_blue",
			hidePaintObj = true,
			dir = 1,
			nameColor = "#92fc63",
			actor = 805010,
			say = "그쪽으로 향하죠.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_guild_blue",
			hidePaintObj = true,
			dir = 1,
			nameColor = "#92fc63",
			actor = 801070,
			say = "그럴 필요 없어요, 추기경님.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_guild_blue",
			hidePaintObj = true,
			dir = 1,
			nameColor = "#92fc63",
			actor = 801070,
			say = "성당의 섬에서 보여주신 추기경님의 활약으로 상황이 바뀌었습니다. '왕관'이 있는 곳으로 바로 갈 수 있어요.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_guild_blue",
			hidePaintObj = true,
			dir = 1,
			nameColor = "#92fc63",
			actor = 805010,
			say = "정말인가요?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_guild_blue",
			hidePaintObj = true,
			dir = 1,
			nameColor = "#92fc63",
			actor = 801070,
			say = "세인트 헬레나 섬이에요. 심판정은 이전에 로열 네이비의 허가를 받아 어떤 비밀 요지를 건설했죠.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_guild_blue",
			hidePaintObj = true,
			dir = 1,
			nameColor = "#92fc63",
			actor = 801070,
			say = "'왕관'은 거기서 경순양함 마르세예즈가 지키고 있답니다.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_guild_blue",
			hidePaintObj = true,
			dir = 1,
			nameColor = "#92fc63",
			actor = 805010,
			say = "마르세예즈……?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 30,
					delay = 0,
					dur = 0.2,
					x = 0,
					number = 2
				}
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_guild_blue",
			hidePaintObj = true,
			dir = 1,
			nameColor = "#92fc63",
			actor = 801070,
			say = "네, 공식 석상에는 좀처럼 나오지 않지만, 심판정이자 전투 천사로서 '재현' 후부터 계속 '왕관'을 지키고 있는 듯해요.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_guild_blue",
			hidePaintObj = true,
			dir = 1,
			nameColor = "#92fc63",
			actor = 805010,
			say = "테리블, 사실… 평소 이런 건 잘 믿지 않지만…",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_guild_blue",
			hidePaintObj = true,
			dir = 1,
			nameColor = "#92fc63",
			actor = 805010,
			say = "요 며칠 동안 꿈에 계속 그 마르세예즈가 나왔어요.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_guild_blue",
			hidePaintObj = true,
			dir = 1,
			nameColor = "#92fc63",
			actor = 801070,
			say = "검은 태양의 꿈……?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 30,
					delay = 0,
					dur = 0.2,
					x = 0,
					number = 2
				}
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_guild_blue",
			hidePaintObj = true,
			dir = 1,
			nameColor = "#92fc63",
			actor = 801070,
			say = "확실히 아무리 봐도 흉조로 여겨지긴 하네요….",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_guild_blue",
			hidePaintObj = true,
			dir = 1,
			nameColor = "#92fc63",
			actor = 805010,
			say = "테리블, '왕관'은 차치하고, 마르세예즈를 그대로 내버려 둘 순 없어요. 우리는……",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_guild_blue",
			hidePaintObj = true,
			dir = 1,
			nameColor = "#92fc63",
			actor = 801070,
			say = "(클레망소 님도 이렇게 될 걸 예상하고 연락하신 거겠지…)",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_guild_blue",
			hidePaintObj = true,
			dir = 1,
			nameColor = "#92fc63",
			actor = 801070,
			say = "(그 요새는 상시적으로 전파가 차단된 상태였지…… 휴우, 정말… 나 못지않게 성미가 고약하신 분이라니까.)",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_guild_blue",
			hidePaintObj = true,
			dir = 1,
			nameColor = "#92fc63",
			actor = 805010,
			say = "함대의 침로를 바꾸어 세인트 헬레나 섬으로 향하죠.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			effects = {
				{
					active = true,
					name = "speed"
				}
			}
		}
	}
}
