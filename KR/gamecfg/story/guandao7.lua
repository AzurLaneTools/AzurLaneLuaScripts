return {
	id = "GUANDAO7",
	mode = 2,
	fadeOut = 1.5,
	scripts = {
		{
			stopbgm = true,
			mode = 1,
			sequence = {
				{
					"탤런트 캐스터 세크리터리!\n\n<size=45>7 엔딩은…?</size>",
					1
				}
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_149",
			hidePaintObj = true,
			say = "공원 피크닉용 잔디에 시트를 깔고, 괌은 바로 앉아 단말기를 기동했다.",
			bgm = "story-richang-11",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_149",
			hidePaintObj = true,
			say = "모니터에서 웃음소리가 새어 나온다. 아무래도 괌이 말한 그 방송이 시작된 모양이다.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_story_task_2",
			hidePaintObj = true,
			say = "모항, 집무실.",
			bgm = "story-1",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = true,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = true,
				alpha = {
					1,
					0
				}
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_story_task_2",
			factiontag = "성실한 마담",
			dir = 1,
			actor = 312010,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "지휘관, 크, 큰일이다냥!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_story_task_2",
			factiontag = "착한 지휘관",
			dir = 1,
			actor = 107090,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "무슨 일이야, 아카시? 뭘 그렇게 허둥대?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_story_task_2",
			factiontag = "성실한 마담",
			dir = 1,
			actor = 312010,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "보급품을 운반하는 양산함이 항구에 와 있다냥!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 107090,
			side = 2,
			bgName = "bg_story_task_2",
			factiontag = "착한 지휘관",
			dir = 1,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "음… 확실히 보급 물자가 도착할 시간이긴 하군.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_story_task_2",
			factiontag = "성실한 마담",
			dir = 1,
			actor = 312010,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "그, 그렇지만 도착한 양산함의 숫자가 무려… 150척이나 된다냥!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_story_task_2",
			factiontag = "착한 지휘관",
			dir = 1,
			actor = 107090,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "…150척?! 그렇게 많이?! 뭘 나르고 있는 건데?!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 312010,
			side = 2,
			bgName = "bg_story_task_2",
			factiontag = "성실한 마담",
			dir = 1,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "대형 작전에 대비하기 위한 물자인 것 같다냥!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_story_task_2",
			factiontag = "착한 지휘관",
			dir = 1,
			actor = 107090,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "…대형 작전? 지휘관인 나조차도 처음 듣는 이야기인데…",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 312010,
			side = 2,
			bgName = "bg_story_task_2",
			factiontag = "성실한 마담",
			dir = 1,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "코드 네임은… '청소'라고 한다냥…",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_story_task_2",
			factiontag = "천재 비서함",
			dir = 1,
			actor = 118020,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "아~ 청소 대작전 말이구나~ 이제야 알겠네♪",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_story_task_2",
			factiontag = "착한 지휘관",
			dir = 1,
			actor = 107090,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "……뭐?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_story_task_2",
			factiontag = "천재 비서함",
			dir = 1,
			actor = 118020,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "저번에 서류 만들었을 때 이 이름이 멋있게 느껴져서… 작전 코드 네임으로 붙여버렸어★",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_story_task_2",
			factiontag = "천재 비서함",
			dir = 1,
			actor = 118020,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "뭐, 작전이니까 물자 준비도 필요하고… 많으면 많을수록 좋을 것 같아서~",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_story_task_2",
			factiontag = "착한 지휘관",
			dir = 1,
			actor = 107090,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "괌이 물자 비축을 기억하고 있을 줄이야…",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 118020,
			side = 2,
			bgName = "bg_story_task_2",
			factiontag = "천재 비서함",
			dir = 1,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "에헷☆ 괌은 똑똑하다구~",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_story_task_2",
			factiontag = "천재 비서함",
			dir = 1,
			actor = 118020,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "어차피 서류 업무 같은 건 어른의 사정! 다들 제대로 확인하지도 않으니까 적당히 처리해도 된다는 말씀!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_story_task_2",
			factiontag = "일반 발명가",
			dir = 1,
			actor = 608020,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "지휘관, 특별 주문한 거 가지고 왔어! 초속 80m로 이동할 수 있는 청소 로봇이야♪",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_story_task_2",
			factiontag = "착한 지휘관",
			dir = 1,
			actor = 107090,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "초속 80m… 그러니까 시속 288km…라는 거야?!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_story_task_2",
			factiontag = "착한 지휘관",
			dir = 1,
			actor = 107090,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "아니, 청소 로봇을 그런 미친 속도로 만들면 어떻게 해!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			effects = {
				{
					active = true,
					name = "speed"
				}
			},
			dialogShake = {
				speed = 0.08,
				x = 15,
				number = 2
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_story_task_2",
			factiontag = "천재 비서함",
			dir = 1,
			actor = 118020,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "어라… 그럴 리가… 아! 잘못 입력했었네, 이거. 에헷☆ 역시 수학은 어렵다니깐!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			effects = {
				{
					active = false,
					name = "speed"
				}
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_story_task_2",
			factiontag = "착한 지휘관",
			dir = 1,
			actor = 107090,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "계산기를 쓰기는 한 거야?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "bg_story_task_2",
			factiontag = "천재 비서함",
			dir = 1,
			actor = 118020,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "계산기는 어렵잖아… 괜찮아, 지휘관. 이거 봐! 쾌속 청소 로봇이라는 것도 꽤 멋지지 않아?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_story_task_2",
			factiontag = "착한 지휘관",
			dir = 1,
			actor = 107090,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "괌!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_149",
			hidePaintObj = true,
			nameColor = "#A9F548FF",
			say = "——에식스의 표정이 너무 진지해서 연기로 느껴지지 않아…",
			bgm = "story-richang-11",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = true,
				dur = 0.5,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 0.5,
				dur = 0.5,
				black = true,
				alpha = {
					1,
					0
				}
			},
			options = {
				{
					content = "——아, 이게 그 지난 주 일요일에 집무실에서 녹화했다던 방송이야?",
					flag = 1
				}
			}
		},
		{
			actor = 118020,
			side = 2,
			bgName = "star_level_bg_149",
			hidePaintObj = true,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "Yes! 지난번에 서류 업무 때문에 괌이 민폐를 끼쳤잖아.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_149",
			hidePaintObj = true,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 118020,
			say = "그래서 그걸로 코미디 프로그램을 만들어 보면 어떨까 했어★",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_149",
			hidePaintObj = true,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 118020,
			say = "이걸 만든 첫 번째 이유는 지휘관에 대한 사과, 그리고 두 번째는 나 자신에 대한 훈계… 그리고 세 번째는 더 많은 동료에게 비서함 업무의 중요성을 알리기 위해서야!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_149",
			hidePaintObj = true,
			nameColor = "#A9F548FF",
			say = "——흔히 말하는 '에듀테인먼트'라는 건가?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 118020,
			side = 2,
			bgName = "star_level_bg_149",
			hidePaintObj = true,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "아, 그렇게 말할 수도 있겠네…! 뭐, 난 그렇게 깊이 생각하지는 않았지만…",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_149",
			hidePaintObj = true,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 118020,
			say = "그냥 나는 이번 일로 많은 걸 배웠으니까, 이걸 보고 다들 무언가 얻었으면 좋겠다… 싶어서?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_149",
			hidePaintObj = true,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 118020,
			say = "에헤헷… 좀 늦은 감이 있기는 하지만… 멋대로 이런 방송을 찍어서… 화난 건 아니지?",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_149",
			hidePaintObj = true,
			nameColor = "#A9F548FF",
			say = "——아니야, 오히려……",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			options = {
				{
					content = "——재밌는 방송인 것 같아.",
					flag = 1
				},
				{
					content = "——다음에는 나도 출연할게.",
					flag = 2
				}
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_149",
			dir = 1,
			optionFlag = 1,
			actor = 118020,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "마음에 들어서 다행이야~!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_149",
			dir = 1,
			optionFlag = 2,
			actor = 118020,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "Wow! 약속이야!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_149",
			hidePaintObj = true,
			say = "어느새 방송은 막바지로 접어들었다.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = true,
				dur = 0.5,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 0.5,
				dur = 0.5,
				black = true,
				alpha = {
					1,
					0
				}
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_149",
			hidePaintObj = true,
			say = "천재(초보) 비서함 괌은 몇 번의 실패를 거쳐 자신의 잘못을 깨닫고 노력을 거듭한 끝에 훌륭한 비서함이 되었다.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_149",
			hidePaintObj = true,
			say = "방송 밖에 있는 괌도 이와 마찬가지로 바쁜 일정 틈틈이 공부에 힘써서 지금은 비서함으로서 한 사람 몫을 하고 있다.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_149",
			hidePaintObj = true,
			nameColor = "#A9F548FF",
			say = "——이렇게까지 발 벗고 나서는 탤런트는 없을 거야……",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 118020,
			side = 2,
			bgName = "star_level_bg_149",
			hidePaintObj = true,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "그런 건 아니지~ 적당히 내키는 대로 열심히 한 것뿐인 걸 뭐.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_149",
			hidePaintObj = true,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 118020,
			say = "방송 속에 있는 자신한테 지지 않을 우수한 비서함이 될 때까지 앞으로도 노력할게♪",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 30,
					delay = 0,
					dur = 0.2,
					x = 0,
					number = 2
				}
			},
			options = {
				{
					content = "——이미 이긴 것 같은데?",
					flag = 1
				}
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_149",
			hidePaintObj = true,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 118020,
			say = "에이, 그런 소리 마. 나는 아직 부족한걸.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 118020,
			side = 2,
			bgName = "star_level_bg_149",
			hidePaintObj = true,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "그러니까 당분간은 계속 나한테 비서함을 맡겨줬으면 해.",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_149",
			hidePaintObj = true,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 118020,
			say = "지휘관의 No.1 비서함이 되려면 아직 더 많은 레슨이 필요하니까!",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_149",
			hidePaintObj = true,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 118020,
			say = "내일도 잘 부탁해, 지휘관♪",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
