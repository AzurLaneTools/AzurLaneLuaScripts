return {
	fadeOut = 1.5,
	mode = 2,
	id = "RUOYESHENGDAN2",
	once = true,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			stopbgm = true,
			mode = 1,
			sequence = {
				{
					"크리스마스 이브 진저브레드 이야기\n\n<size=45>여왕의 측면상</size>",
					1
				}
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_100",
			dir = 1,
			bgmDelay = 2,
			bgm = "story-1",
			actor = 299011,
			nameColor = "#a9f548",
			hidePaintObj = true,
			say = "좋아! 여기 오른손은 끝났어요. 붙여도 됩니다. 워스파이트 님.",
			flashout = {
				black = true,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = true,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 205021,
			side = 2,
			bgName = "star_level_bg_100",
			nameColor = "#a9f548",
			dir = 1,
			say = "주의력, 집중, 집중... 여기도 완성이야! 드디어 폐하의 눈썹을 완성했다!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			action = {
				{
					type = "shake",
					y = 30,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 0,
			side = 2,
			bgName = "star_level_bg_100",
			hidePaintObj = true,
			dir = 1,
			nameColor = "#a9f548",
			actor = 299011,
			say = "...인정합니다. 워스파이트 님이 만든 얼굴은 정말 진짜와 똑같네요.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_100",
			actor = 205021,
			dir = 1,
			nameColor = "#a9f548",
			say = "후... 이번엔 완성 시간을 맞출 수 있겠어.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 199012,
			side = 2,
			bgName = "star_level_bg_100",
			nameColor = "#a9f548",
			dir = 1,
			say = "이건... 진저브레드... 인간!?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			action = {
				{
					type = "shake",
					y = 30,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_100",
			hidePaintObj = true,
			dir = 1,
			nameColor = "#a9f548",
			actor = 299011,
			say = "어엇, 시애틀이군요. 크리스마스 파티 준비하느라 수고가 많겠어요.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_100",
			actor = 199012,
			dir = 1,
			nameColor = "#a9f548",
			say = "아하하... 수고는 무슨, 사실 준비 과정도 무척이나 재미있슴다, 헤헤~",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 199012,
			side = 2,
			bgName = "star_level_bg_100",
			nameColor = "#a9f548",
			dir = 1,
			say = "그런데, 진저브레드를 만들다가 난관에 부딪혔는데 말임다...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_100",
			hidePaintObj = true,
			dir = 1,
			nameColor = "#a9f548",
			actor = 299011,
			say = "응? 시애틀도 걱정을 할 때가 있네요?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 199012,
			side = 2,
			bgName = "star_level_bg_100",
			nameColor = "#a9f548",
			dir = 1,
			say = "아하하하... 맛은 괜찮을지 몰라도, 뭔가 부족한 것 같은 느낌이라...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_100",
			actor = 205021,
			dir = 1,
			nameColor = "#a9f548",
			say = "이런 일에서 중요한 것은 마음이지! 이건 내가 작년 크리스마스 때 배운 경험으로 하는 말이야.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			action = {
				{
					type = "shake",
					y = 30,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_100",
			actor = 205021,
			dir = 1,
			nameColor = "#a9f548",
			say = "그래서 올해는 폐하를 위해 이걸 준비했지! 로열 특제 크리스마스 진저브레드 조각상——<여왕 폐하의 측면상>!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 199012,
			side = 2,
			bgName = "star_level_bg_100",
			nameColor = "#a9f548",
			dir = 1,
			say = "오오... 아주 대단해보이지 말임다!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			action = {
				{
					type = "shake",
					y = 30,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_100",
			hidePaintObj = true,
			dir = 1,
			nameColor = "#a9f548",
			actor = 299011,
			say = "막상 실제로 만들기 시작하면 의외로 손이 정말 많이 간답니다?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "star_level_bg_100",
			hidePaintObj = true,
			dir = 1,
			nameColor = "#a9f548",
			actor = 299011,
			say = "재료 준비부터, 대형 오븐까지... 그리고 중간에 무수히 많은 실패와 희생——",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 199012,
			side = 2,
			bgName = "star_level_bg_100",
			nameColor = "#a9f548",
			dir = 1,
			say = "그... 약간 좀 과장된거 아임까?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_100",
			actor = 301231,
			dir = 1,
			nameColor = "#a9f548",
			say = "너무 과장이다... 그 \"실패작\"을 내가 얼마나 많이 시식했는지도 모르겠거든! 맛은 정말 맛있긴 했는데...",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			action = {
				{
					type = "shake",
					y = 30,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 0,
			side = 2,
			bgName = "star_level_bg_100",
			hidePaintObj = true,
			dir = 1,
			nameColor = "#a9f548",
			actor = 299011,
			say = "어엇, 시식을 돕겠다고 얘기한 건 와카바였잖아요?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_100",
			actor = 301231,
			dir = 1,
			nameColor = "#a9f548",
			say = "크리스마스 선물 준비 도중에 맛있는 냄새가 나길래 와본건데, 이렇게 될줄은... 흐음~",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 299011,
			side = 2,
			bgName = "star_level_bg_100",
			hidePaintObj = true,
			dir = 1,
			nameColor = "#a9f548",
			say = "와카바짱, 당신의 희생은 헛되지 않았어요.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 205021,
			side = 2,
			bgName = "star_level_bg_100",
			nameColor = "#a9f548",
			dir = 1,
			say = "저녁 파티가 시작되고 폐하가 이걸 보면 분명 매우 기뻐하실 거야!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			action = {
				{
					type = "shake",
					y = 30,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 0,
			side = 2,
			bgName = "star_level_bg_100",
			hidePaintObj = true,
			dir = 1,
			nameColor = "#a9f548",
			actor = 299011,
			say = "호호, 그 메이드장까지 크게 놀라겠죠~ 정말 기대되네요~",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 199012,
			side = 2,
			bgName = "star_level_bg_100",
			nameColor = "#a9f548",
			dir = 1,
			say = "중요한 건 마음임까... 꽤 괜찮은 생각임다! 그럼 이제... 사쿠라 엠파이어의 동료들을 찾아봐야겠슴다. 지금이라면 교환용 선물을 준비하고 있겠지 말임다——",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 301231,
			side = 2,
			bgName = "star_level_bg_100",
			nameColor = "#a9f548",
			dir = 1,
			say = "앗, 시애틀 기다려, 나도 같이가!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			action = {
				{
					type = "shake",
					y = 30,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_100",
			dir = 1,
			blackBg = true,
			actor = 299011,
			nameColor = "#a9f548",
			hidePaintObj = true,
			say = "음... 그래요. 여기도 거의 끝났으니까요. 저녁에는 저희도 엘리자베스 폐하와 함께 파티에 참가할거에요. 화이팅하세요. 시애틀.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		}
	}
}
