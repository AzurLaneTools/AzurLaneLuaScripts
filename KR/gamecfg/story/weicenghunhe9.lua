return {
	id = "WEICENGHUNHE9",
	mode = 2,
	once = true,
	fadeType = 1,
	fadein = 1.5,
	scripts = {
		{
			side = 2,
			nameColor = "#a9f548",
			bgName = "bg_banama_1",
			bgm = "hunhe-story",
			dir = 1,
			say = "파나마 요새 서쪽·외해, 조금 전",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_banama_1",
			actor = 103240,
			dir = 1,
			nameColor = "#a9f548",
			say = "대단해! 역시 세계 공학의 기적이라고 불리는 파나마 운하라니까!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_banama_1",
			actor = 103240,
			dir = 1,
			nameColor = "#a9f548",
			say = "실제로 체험해보고 나니 ‘장관’이라는 말 외에 이 ‘바다를 잇는 다리’를 표현할 말이 없는걸!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_banama_1",
			actor = 106550,
			dir = 1,
			nameColor = "#a9f548",
			say = "장관이라는 것 말고도 더 중요한 게 있죠.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 106550,
			side = 2,
			bgName = "bg_banama_1",
			nameColor = "#a9f548",
			dir = 1,
			say = "운하가 생긴 이후로 두 바다 사이의 운송 거리가 크게 줄어들었어요. 전쟁에서도 큰 역할을 했죠.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 106550,
			side = 2,
			bgName = "bg_banama_1",
			nameColor = "#a9f548",
			dir = 1,
			say = "그 이후, 이 운하는 수년간의 확장과 개선, 보완을 거쳤어요. 덕분에 우리가 지금 보고 있는 파나마 운하가 된 거죠.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_banama_1",
			actor = 103240,
			dir = 1,
			nameColor = "#a9f548",
			say = "이제 이 요새를 지키는 책임은 잠시 동안 우리에게 맡겨진 거네. 오늘 밤은 잠을 못 자겠는걸?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			action = {
				{
					type = "shake",
					y = 30,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_banama_1",
			actor = 102260,
			dir = 1,
			nameColor = "#a9f548",
			say = "기세가 좋은 건 좋은 일이지만 말이지… 파나마 요새는 한동안 세이렌의 습격을 받지 않았거든.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 102260,
			side = 2,
			bgName = "bg_banama_1",
			nameColor = "#a9f548",
			dir = 1,
			say = "어쩌면 세이렌도 드디어 전력이 부족해진 걸지도. 에헤헤.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "통신기",
			bgName = "bg_banama_1",
			nameColor = "#a9f548",
			dir = 1,
			say = "전원 주의. 요새 주변에 세이렌이 습격한 흔적을 발견했다.",
			bgm = "airRaidAlarm",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "통신기",
			bgName = "bg_banama_1",
			nameColor = "#a9f548",
			dir = 1,
			say = "적 함대, 현재 양쪽 먼바다에서 요새를 향해 접근 중. 모든 전투원은 신속히 제 위치로 돌아가 전투를 준비하라.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_banama_1",
			actor = 106550,
			dir = 1,
			nameColor = "#a9f548",
			say = "……………",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_banama_1",
			actor = 102260,
			dir = 1,
			nameColor = "#a9f548",
			say = "엑…? 상황이 좋아지기 전에는 항상 더 나빠지는 법이라는 말은 이런 걸 말하는 거였나…",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 107170,
			side = 2,
			bgName = "bg_banama_1",
			nameColor = "#a9f548",
			dir = 1,
			say = "운하에 손상은 없지만, 동쪽과 서쪽의 항로가 큰 타격을 입어서 끊어졌군.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 107170,
			side = 2,
			bgName = "bg_banama_1",
			nameColor = "#a9f548",
			dir = 1,
			say = "카사블랑카, 상선의 안내는 맡기도록 하지. 리노는 대공 진지 근처에서 함재기 한 대도 놓치지 말도록 해.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			hideOther = true,
			nameColor = "#a9f548",
			bgName = "bg_banama_1",
			side = 0,
			dir = 1,
			actor = 102260,
			actorName = "리노&카사블랑카",
			say = "오케이!라져.",
			subActors = {
				{
					actor = 106550,
					pos = {
						x = 1185
					}
				}
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 107170,
			side = 2,
			bgName = "bg_banama_1",
			nameColor = "#a9f548",
			dir = 1,
			say = "브레머튼은 나와 같이 최전선으로 출격한다. 한번 놀아보자고.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_banama_1",
			actor = 107170,
			dir = 1,
			nameColor = "#a9f548",
			say = "……음, 왜 그렇게 보는 거지? 그저 누군가와 대화하는 게 어색할 뿐이야.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_banama_1",
			actor = 103240,
			dir = 1,
			nameColor = "#a9f548",
			say = "후후후, 미안 미안~ 이렇게 갭이 큰 성격도 꽤 귀엽잖아★",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_banama_1",
			actor = 103240,
			dir = 1,
			nameColor = "#a9f548",
			say = "세이렌 함대인가…… 마침 잘 왔어. 요 며칠 동안 답답해 죽는 줄 알았다고.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 103240,
			side = 2,
			bgName = "bg_banama_1",
			nameColor = "#a9f548",
			dir = 1,
			say = "무슨 문제를 해결할 수는 없지만, 그래도 너희들로 내 화를 좀 풀어야겠다!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		}
	}
}
