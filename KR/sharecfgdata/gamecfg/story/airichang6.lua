return {
	fadeOut = 1.5,
	mode = 2,
	id = "AIRICHANG6",
	once = true,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			stopbgm = true,
			mode = 1,
			sequence = {
				{
					"「버츄얼 연결고리의 공시성」 -일상편\n\n<size=45>6. 재판장 키즈나 아이</size>",
					1
				}
			}
		},
		{
			actor = 10400010,
			side = 2,
			bgName = "bg_story_task",
			nameColor = "#92fc63",
			dir = 1,
			bgmDelay = 2,
			say = "버츄얼 유튜버…… 아니! 재판장 키즈나 아이입니다! 법에 저촉되진 않지만 '이건 아니잖아'하는 사안, 여러분도 꽤 있잖아요?",
			bgm = "story-1",
			flashout = {
				black = true,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = true,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 10400010,
			side = 2,
			bgName = "bg_story_task",
			nameColor = "#92fc63",
			dir = 1,
			say = "그런 떨떠름한 것을 저, 재판장 키즈나 아이가 유죄인지 무죄인지 심판합니다!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_story_task",
			actor = 10400010,
			dir = 1,
			nameColor = "#92fc63",
			say = "그래서, 벽람 출장판 제 1회 키즈나 아이 재판, 지금부터 개~~~정!!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 10400010,
			side = 2,
			bgName = "bg_story_task",
			nameColor = "#92fc63",
			dir = 1,
			say = "(탕! 탕!)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 10400010,
			side = 2,
			bgName = "bg_story_task",
			nameColor = "#92fc63",
			dir = 1,
			say = "오늘 심판할 것은 이쪽 분의 사건입니다. 요크쨩, 말씀하세요!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 203070,
			side = 1,
			bgName = "bg_story_task",
			nameColor = "#92fc63",
			dir = 1,
			say = "얼마 전에 저의 취미인 '드링크 개발'로 만든 요크 특제 드링크를 지휘관이 테스트를 위해 마셔줬는데요!",
			paintingFadeOut = {
				time = 0.5,
				side = 0
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 203070,
			side = 1,
			bgName = "bg_story_task",
			nameColor = "#92fc63",
			dir = 1,
			say = "그런데 어떻게 된 일인지 드링크를 마신 그 순간, 지휘관에게서 생기가 사라져 자리에 드러누워버렸어요!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 6,
			side = 0,
			bgName = "bg_story_task",
			actor = 10400010,
			dir = 1,
			nameColor = "#92fc63",
			say = "뭐어!? 대체 어떤 걸 넣은 거야?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 203070,
			side = 1,
			bgName = "bg_story_task",
			nameColor = "#92fc63",
			dir = 1,
			say = "저는 단지 운명<†페이트†>의 인도를 따라 드링크를 만들었을 뿐. 굳이 말하자면 가슴의 안쪽에서 솟구쳐 올라오는 힘<†러브†> 때문이랍니다!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 0,
			bgName = "bg_story_task",
			actor = 10400010,
			dir = 1,
			nameColor = "#92fc63",
			say = "오오, 뜨겁네요~ 여튼 저도 잘 모르겠어서, 베스탈씨를 불렀습니다. 까놓고 말해서 지휘관님이 쓰러진 원인은 뭔가요!?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 112010,
			side = 1,
			bgName = "bg_story_task",
			nameColor = "#92fc63",
			dir = 1,
			say = "네~ 진료 결과, 너무 맛이 없어서 정신을 잃은 것으로 밝혀졌습니다~",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 203070,
			side = 1,
			bgName = "bg_story_task",
			nameColor = "#92fc63",
			dir = 1,
			say = "그, 그런 말도 안 되는! 저의 완벽한 배합<†포뮬레이션†>으로 그러한 물건이 만들어질 리가……!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 2,
			side = 0,
			bgName = "bg_story_task",
			actor = 10400010,
			dir = 1,
			nameColor = "#92fc63",
			say = "으~음, 우선은 말이야, 거기 있는 요크쨩 특제 드링크를 한 번 마셔보면 답을 알 수 있지 않을까?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 203070,
			side = 1,
			bgName = "bg_story_task",
			nameColor = "#92fc63",
			dir = 1,
			say = "그렇군요! 그러고 보니 제가 맛을 본 적은 없었네요. 제가 직접 이 요크 특제 드링크를 남김없이 마셔서 저의 무고함<†이노센트†>을 증명해 보이죠!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 112010,
			side = 2,
			bgName = "bg_story_task",
			nameColor = "#92fc63",
			dir = 1,
			say = "요크쨩, 그건 그만두는 편이…… 아니, 벌써 마셔버렸네요.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 203070,
			side = 2,
			bgName = "bg_story_task",
			nameColor = "#92fc63",
			dir = 1,
			say = "으윽!? 으으으으읏!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "bg_story_task",
			actor = 10400010,
			dir = 1,
			nameColor = "#92fc63",
			say = "괘, 괜찮아? 얼굴이 새파랗게 질렸는데…!?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 203070,
			side = 2,
			bgName = "bg_story_task",
			nameColor = "#92fc63",
			dir = 1,
			say = "…후, 후후후…… 화, 확실히 바닥에 뒹굴고 싶을 만한 맛이네요. 지휘관님의 기분, 알 것 같아요… 하지만 이걸로 저희는 함께 죄악의 성배를 비운 공범자<†크라임†>……(털썩)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			action = {
				{
					type = "move",
					y = -2250,
					delay = 1,
					dur = 0.5,
					x = 0
				}
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_story_task",
			actor = 10400010,
			dir = 1,
			nameColor = "#92fc63",
			say = "요크쨩!!?? 베스탈 씨! 치료를 부탁합니다!!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 112010,
			side = 2,
			bgName = "bg_story_task",
			nameColor = "#92fc63",
			dir = 1,
			say = "네~ 정말, 너무 무리한다니까요. 아이쨩, 나머지는 제게 맡겨주세요.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_story_task",
			actor = 10400010,
			dir = 1,
			nameColor = "#92fc63",
			say = "……가버렸네.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "bg_story_task",
			actor = 10400010,
			dir = 1,
			nameColor = "#92fc63",
			say = "앗, 키즈나 아이 재판 도중이었죠!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			actor = 10400010,
			side = 2,
			bgName = "bg_story_task",
			nameColor = "#92fc63",
			dir = 1,
			say = "으~음, 그럼 판결을 내립니다.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 10400010,
			side = 2,
			bgName = "bg_story_task",
			nameColor = "#92fc63",
			dir = 1,
			say = "(탕! 탕!)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_story_task",
			actor = 10400010,
			dir = 1,
			nameColor = "#92fc63",
			say = "<size=67.5>무 죄.</size>",
			effects = {
				{
					active = true,
					name = "speed"
				}
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "bg_story_task",
			actor = 10400010,
			dir = 1,
			nameColor = "#92fc63",
			say = "처음엔 유죄라고 생각했지만요, 본인도 마셨으니까요. 저는 그걸 높이 평가하고 싶어요! 분명 이거라면 지휘관님도 용서해 줄 겁니다!",
			effects = {
				{
					active = false,
					name = "speed"
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_story_task",
			actor = 10400010,
			dir = 1,
			nameColor = "#92fc63",
			say = "거기다 지휘관님을 위해서 만든 거니까, 어쩔 수 없다!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_story_task",
			actor = 10400010,
			dir = 1,
			nameColor = "#92fc63",
			say = "사랑만 있으면 어떻게든 되겠죠. 여러분도 키즈나 아이만 있으면 되는 것처럼, 막 이래!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_story_task",
			actor = 10400010,
			dir = 1,
			nameColor = "#92fc63",
			say = "뭐, 또 누군가 드링크를 마시게 될지도 모르지만, 그때는 또 그 누군가가 힘내주는 걸로 하죠.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_story_task",
			actor = 10400010,
			dir = 1,
			nameColor = "#92fc63",
			say = "그럼, 오늘의 키즈나 아이 재판은 이걸로 폐정!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 10400010,
			side = 2,
			bgName = "bg_story_task",
			nameColor = "#92fc63",
			dir = 1,
			say = "(탕! 탕!)",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 201210,
			stopbgm = true,
			bgName = "bg_story_task",
			side = 2,
			dir = 1,
			blackBg = true,
			nameColor = "#92fc63",
			say = "(오싹) ……왠지 어딘가에서 말도 안 되는 걸 떠넘기는 것 같은 예감이 들지만, 신경 쓰지 말기로 하자…",
			flashout = {
				black = true,
				dur = 0.5,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 0.5,
				dur = 0.5,
				black = true,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 1,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		}
	}
}
