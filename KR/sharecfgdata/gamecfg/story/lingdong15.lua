return {
	fadeOut = 1.5,
	mode = 2,
	id = "LINGDONG15",
	once = true,
	fadeType = 1,
	fadein = 1.5,
	scripts = {
		{
			say = "……————",
			side = 2,
			bgm = "story-5",
			dir = 1,
			blackBg = true,
			soundeffect = "event:/battle/boom1",
			flash = {
				delay = 0.3,
				dur = 0.5,
				wait = 0.5,
				number = 1,
				alpha = {
					0,
					1
				}
			},
			dialogShake = {
				speed = 0.09,
				x = 8.5,
				number = 2
			}
		},
		{
			actor = 405020,
			side = 2,
			nameColor = "#ff0000",
			dir = 1,
			say = "……이 느낌……역시 '특제' 라 할 만하군.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 207040,
			nameColor = "#92fc63",
			side = 0,
			dir = 1,
			say = "어째서야! 벌써 이렇게나 너덜너덜해졌는데, 왜 투항하지 않는 거야?!! 이대로라면 진짜 가라앉고 만다고!",
			paintingFadeOut = {
				time = 0.5,
				side = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 405020,
			side = 1,
			nameColor = "#ff0000",
			dir = 1,
			say = "…투항…이라…",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 405020,
			side = 1,
			nameColor = "#ff0000",
			dir = 1,
			say = "……미안. 전함으로서 마지막까지 싸우는 거야말로 우리의 바람이라서 말이야.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 207040,
			side = 0,
			nameColor = "#92fc63",
			dir = 1,
			say = "정말이지! 당신 언니도 그렇고, 왜 메탈 블러드 사람들은 다 이렇게 꽉 막혔담?!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 405020,
			side = 1,
			nameColor = "#ff0000",
			dir = 1,
			say = "비스마르크…?! 당신, 내 언니를 만난 적이 있나…?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 207040,
			side = 0,
			nameColor = "#92fc63",
			dir = 1,
			say = "그래! 당신 언니와의 싸움은, 날 포함해서 거의 모든 로열 네이비의 함선들이 투입되었었어. 뭐, 나는 멀리서 함재기로 관찰만 했었지만…",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 207040,
			side = 0,
			nameColor = "#92fc63",
			dir = 1,
			say = "그때, 비스마르크도 당신과 똑같은 말을 했었거든!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 405020,
			side = 1,
			nameColor = "#ff0000",
			dir = 1,
			say = "……",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 405020,
			side = 1,
			nameColor = "#ff0000",
			dir = 1,
			say = "……그런가, 고맙다……",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 207040,
			side = 0,
			nameColor = "#92fc63",
			dir = 1,
			say = "어……?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 405020,
			side = 1,
			nameColor = "#ff0000",
			dir = 1,
			say = "언니는 마지막까지 자신의 신념을 갖고 최고의 전투 성과를 올리고 가라앉았다.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 405020,
			side = 1,
			nameColor = "#ff0000",
			dir = 1,
			say = "이쪽 세계에서도 언니의 이름은 사람들의 기억 속에 영원히 남아있겠지……",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 405020,
			side = 1,
			nameColor = "#ff0000",
			dir = 1,
			say = "메탈 블러드의 함선으로서 가장 슬픈 건, 아무것도 하지 못한 채로 잊혀져, 자신의 운명을 남에게 맡기는 것……",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 405020,
			side = 1,
			nameColor = "#ff0000",
			dir = 1,
			say = " '재현' 된 아이들과 달리, 당신이 여기 나타나 준 덕분에 나도 이렇게 주포를 쏠 수 있게 되었어……",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 405020,
			side = 1,
			nameColor = "#ff0000",
			dir = 1,
			say = "이대로 싸우다 가라앉는 것도…나쁘진 않지.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 207040,
			side = 0,
			nameColor = "#92fc63",
			dir = 1,
			say = "……",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 207040,
			side = 0,
			nameColor = "#92fc63",
			dir = 1,
			say = "뭐야…그런 거, 치사하잖아…",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 207040,
			side = 0,
			nameColor = "#92fc63",
			dir = 1,
			say = "비극의 히로인이라도 되고 싶은 거야?! 아니면 그런 게 멋있다고 생각하는 거야?!…살아남을 기회가 있는데, 자기 입으로 가라앉아도 좋다니…",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 405020,
			side = 1,
			nameColor = "#ff0000",
			dir = 1,
			say = "……",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 207040,
			side = 0,
			nameColor = "#92fc63",
			dir = 1,
			say = "그러면 왠지 이쪽이 악당 같잖아……",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 207040,
			side = 0,
			nameColor = "#92fc63",
			dir = 1,
			say = "흥! …완전 어이없어. \"함대 방향 전환, 반속으로 작전 해역에서 이탈한다.\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 405020,
			side = 1,
			nameColor = "#ff0000",
			dir = 1,
			say = "…괜찮은가? '함력' 을 뛰어 넘어, 무훈과 영예를 손에 넣을 수 있는 찬스인데……",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 207040,
			side = 0,
			nameColor = "#92fc63",
			dir = 1,
			say = "무훈과 명예…물론 매력적이지…그치만! 이러면 당신이…",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 207040,
			side = 0,
			nameColor = "#92fc63",
			dir = 1,
			say = "아냐…하, 함재기의 연료가 떨어졌을 뿐이야! …운이 좋았네! 흥!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 207040,
			side = 2,
			nameColor = "#92fc63",
			dir = 1,
			say = "\"기함 듀크 오브 요크에게 보고한다…\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 207040,
			side = 2,
			nameColor = "#92fc63",
			dir = 1,
			say = "\"…'북쪽의 여왕' 에게 큰 피해를 입혔지만, 격침시키진 못했다. '텅스텐 작전' 은 실패……라고\"",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 405020,
			side = 2,
			nameColor = "#ff0000",
			dir = 1,
			blackBg = true,
			say = "……정말 종잡을 수 없는 아가씨야……",
			flashout = {
				black = true,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = true,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 405020,
			nameColor = "#ff0000",
			side = 2,
			dir = 1,
			blackBg = true,
			say = "…이제 두 번 다시 만날 일은 없겠지만…",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 405020,
			nameColor = "#ff0000",
			side = 2,
			dir = 1,
			blackBg = true,
			say = "……",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 405020,
			nameColor = "#ff0000",
			side = 2,
			dir = 1,
			blackBg = true,
			say = "그런 당신은 과연 얼마나 더 앞으로 나아갈 수 있을지…한번 보고 싶군…",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		}
	}
}
