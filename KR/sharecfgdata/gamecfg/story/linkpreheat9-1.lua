return {
	fadeOut = 0,
	mode = 2,
	id = "LINKPREHEAT9-1",
	once = true,
	fadeType = 1,
	fadein = 1.5,
	scripts = {
		{
			actor = 201210,
			nameColor = "#92fc63",
			side = 2,
			bgm = "level-uta",
			dir = 1,
			say = "……여기가 오호츠크 해역의 최심부…",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 301050,
			side = 2,
			nameColor = "#92fc63",
			dir = 1,
			say = "전에 왔을 때랑은 전혀 다른 모습……이 됐어요.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			nameColor = "#92fc63",
			side = 2,
			actor = 10300010,
			dir = 1,
			say = "여기……우리가 처음 떨어졌던 장소 아냐?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			nameColor = "#92fc63",
			side = 2,
			actor = 10300030,
			dir = 1,
			say = "듣고 보니……본적이 있는 것 같기도 하고……아닌 것 같기도 하고……",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 10300020,
			side = 2,
			nameColor = "#92fc63",
			dir = 1,
			say = "좀 더 바깥쪽이었던 것 같은 느낌이 드는 거에요.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 312010,
			side = 2,
			nameColor = "#92fc63",
			dir = -1,
			say = "네코네의 말대로다냥. 아카시가 쿠온 일행을 발견한 건, 이 해역에서 좀 더 바깥쪽이었다냥.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 10300040,
			side = 2,
			nameColor = "#92fc63",
			dir = 1,
			say = "그 때 우린 허우적허우적.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 10300050,
			nameColor = "#92fc63",
			side = 1,
			dir = 1,
			say = "떨어졌던 장소에서 육지를 찾아 헤엄을 쳤었기 때문에, 이동한 게 아닌가 생각됩니다.",
			paintingFadeOut = {
				time = 0.5,
				side = 0
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 101170,
			side = 2,
			nameColor = "#92fc63",
			dir = 1,
			say = "해역의 안쪽……뭔가 이상해.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 10300060,
			side = 2,
			nameColor = "#92fc63",
			dir = 1,
			say = "그렇네요. 어딘가 좀 다른 곳에서부터 공기가 흘러들어오는 것 같은 느낌이 들어요.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 401230,
			side = 2,
			nameColor = "#92fc63",
			dir = 1,
			say = "그렇다면……혹시, 쿠온씨랑 다른 분들이 있던 세계와 연결되어 있는 건 아닐까요?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 301050,
			side = 2,
			nameColor = "#92fc63",
			dir = 1,
			say = "말이 나왔으니 말이지만, 이 분위기……굉장히 싫어했었던 기억이 나는 거에요.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 312010,
			side = 2,
			nameColor = "#92fc63",
			dir = -1,
			say = "맞아냥~ 여기는 거울해역이다냥!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 201210,
			side = 2,
			nameColor = "#92fc63",
			dir = 1,
			say = "역시나! 세이렌의 소행이라고 들었을 때부터 그럴 줄 알았어!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 101170,
			side = 2,
			nameColor = "#92fc63",
			dir = 1,
			say = "그래도……평소의 거울해역이랑은 좀 달라……",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			nameColor = "#92fc63",
			side = 2,
			actor = 10300010,
			dir = 1,
			say = "다르다기보단……이 묘한 분위기, 어디선가 느껴본 적 있는 거 같은데?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			nameColor = "#92fc63",
			side = 2,
			actor = 10300040,
			dir = 1,
			say = "몽환연무……",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 1,
			actor = 10300050,
			nameColor = "#92fc63",
			dir = 1,
			say = "그 결계의 독특한 공기가, 이 해역에 퍼져있습니다.",
			paintingFadeOut = {
				time = 0.5,
				side = 0
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900021,
			nameColor = "#92fc63",
			side = 2,
			actorName = "퓨리파이어",
			dir = 1,
			say = "띵동~! 뭐 그 정도면 정답이라고 해줄까나~?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 401230,
			side = 2,
			nameColor = "#92fc63",
			dir = 1,
			say = "세이렌! 역시 여기 숨어있었구나!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			nameColor = "#92fc63",
			side = 2,
			actor = 10300010,
			dir = 1,
			say = "뭐 그정도면 정답이라니, 그건 무슨 말일까나?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900021,
			nameColor = "#92fc63",
			side = 2,
			actorName = "퓨리파이어",
			dir = 1,
			say = "그럼 답을 한 번 맞춰보자고. 여기는 확실히 거울해역, 그것도 너희 세계의 '몽환연무' 인지 뭔지하는 결계의 테크놀러지를 빌려 쓴 게 맞아!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900021,
			nameColor = "#92fc63",
			side = 2,
			actorName = "퓨리파이어",
			dir = 1,
			say = "하지만……너희 세계에서 조금 방해가 들어와서 말이지……",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			nameColor = "#92fc63",
			side = 2,
			actor = 10300060,
			dir = 1,
			say = "우리 세계에서? 그건 대체……?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			nameColor = "#92fc63",
			side = 2,
			actor = 10300040,
			dir = 1,
			say = "혹시……주인님?",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 1,
			actor = 10300050,
			nameColor = "#92fc63",
			dir = 1,
			say = "저희가 위급한 상황에 처했을 때, 주인님이라면 틀림없이 전력을 다해 당신들의 방해할 것입니다.",
			paintingFadeOut = {
				time = 0.5,
				side = 0
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 10300010,
			side = 2,
			nameColor = "#92fc63",
			dir = 1,
			say = "……응, 그 사람이라면 그러고도 남을까나.",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900021,
			nameColor = "#92fc63",
			side = 2,
			actorName = "퓨리파이어",
			dir = 1,
			say = "그 녀석이 누군지 우린 알 필요도 없고 알고 싶지도 않아! 게다가 여기서 하는 실험에는 전혀 관계없다고! 우리가 필요로 하는 건 데이터! 샘플! 실험기록!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 900021,
			nameColor = "#92fc63",
			side = 2,
			actorName = "퓨리파이어",
			dir = 1,
			say = "자, 우리를 위해 발버둥쳐보라고! 그러기 위해 이 무대를 '세팅' 한 거니까 말야!",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		}
	}
}
