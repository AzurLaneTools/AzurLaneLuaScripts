slot0 = class("SecretsAbyssCoreActivityUI", import("view.activity.CorePage.CoreActivityMainScene"))
...
slot0.init = function(slot0, ...)
	...
	quickPlayAnimation(slot2:Find("adapt/TopPage/top"), "Anim_SecretsAbyssCoreActivityUI_top_In")

	slot2 = slot0._tf

	setText(slot2:Find("adapt/TopPage/top/deco/Text"), i18n("masaina_main_title"))
	...
	slot2:make(function (slot0, slot1, slot2)
		if slot0 == UIItemList.EventUpdate then
			...
			elseif not uv0.pageDic[slot3.id] then
				...
				setText(uv0:findTF("off/name", slot2), i18n("masaina_main_sheet" .. slot3:getConfig("is_show")))
				setText(uv0:findTF("on/name", slot2), i18n("masaina_main_sheet" .. slot3:getConfig("is_show")))
				...
				if uv0.pageDic[slot3.id] ~= nil then
					...
					onToggle(uv0, slot2, function (slot0)
						if slot0 then
							...
							if uv2 ~= uv1.id then
								quickPlayAnimation(uv3, "Anim_SecretsAbyssCoreActivityUI_tabs_on_In")
							end
