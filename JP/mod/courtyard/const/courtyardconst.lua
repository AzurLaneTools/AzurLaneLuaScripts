slot0 = class("CourtYardConst")
slot0.SYSTEM_DEFAULT = 1
slot0.SYSTEM_VISIT = 2
slot0.SYSTEM_FEAST = 3
slot0.SYSTEM_OUTSIDE = 4
slot0.SYSTEM_EDIT_FEAST = 5
slot0.SYSTEM_REVERSE_PACMAN = 6
slot0.STYLE_INNER = 1
slot0.STYLE_OUTSIDE = 2
slot0.STYLE_FEAST = 3
slot0.STYLE_PREVIEW = 4
slot0.STYLE_REVERSE_PACMAN = 6
slot0.DEPTH_TYPE_MAT = 0
slot0.DEPTH_TYPE_SHIP = 1
slot0.DEPTH_TYPE_FURNITURE = 2
slot0.BELONG_FLOOR = 1
slot0.BELONG_WALL = 2
slot0.MAX_STOREY_LEVEL = 3
slot0.OPEN_AREA_PRE_LEVEL = 4
slot0.BACKYARD_RED = Color.New(1, 0.294, 0.08)
slot0.BACKYARD_GREEN = Color.New(0.48, 1, 0.03)
slot0.BACKYARD_BLUE = Color.New(0.3, 0.63, 0.95)
slot0.OBJ_TYPE_SHIP = 1
slot0.OBJ_TYPE_COMMOM = 2
slot0.OBJ_TYPE_SPINE = 3
slot0.SHIP_SCALE = 0.5
slot0.SHIP_TYPE_SELF = 1
slot0.SHIP_TYPE_OTHER = 2
slot0.SHIP_TYPE_FEAST = 3
slot0.SHIP_TYPE_REVERSE_PACMAN = 4
slot0.FEAST_EFFECT_TIME = 3
slot0.FEAST_CHAT_TIME = 15
slot0.FEAST_GIVE_TICKET_ANIM_TIME = 8
slot0.FEAST_GIVE_GIFT_ANIM_TIME = 2
slot0.REVERSE_PACMAN_CHAT_TIME = {
	15,
	20
}
slot0.REVERSE_PACMAN_CHAT_SHOW_TIME = 15
slot0.DOUBLE_SKIN_ADD = 100000

return slot0
