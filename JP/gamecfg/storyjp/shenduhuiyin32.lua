return {
	fadeOut = 1.5,
	mode = 2,
	id = "SHENDUHUIYIN32",
	once = true,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			say = "ロイヤル本島・女王王宮",
			side = 2,
			bgName = "star_level_bg_116",
			dir = 1,
			bgmDelay = 1,
			bgm = "theme-partydress",
			flashin = {
				delay = 1,
				dur = 1,
				black = true,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "star_level_bg_116",
			say = "広々とした廊下で、ロイヤルメイド・サフォークはファイルの束を抱えながら走っていた。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "star_level_bg_116",
			say = "予定についてアラームを設定したものの、サボりたい一心でゼンマイを回すのを忘れてしまったせいで、すっかり遅れ気味になってしまった。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "star_level_bg_116",
			say = "仕方なくショートカットを取るが、ただでさえ慣れないルートで大苦戦。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 14,
			side = 2,
			bgName = "star_level_bg_116",
			actor = 203040,
			dir = 1,
			nameColor = "#a9f548",
			say = "うぅ…遅刻しちゃうよ～",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 12,
			side = 2,
			bgName = "star_level_bg_116",
			actor = 203040,
			dir = 1,
			nameColor = "#a9f548",
			say = "はぁはぁ…あれ？道を間違えましたっけ？そんなに遠い距離じゃないのに？",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 9,
			side = 2,
			bgName = "star_level_bg_116",
			actor = 203040,
			dir = 1,
			nameColor = "#a9f548",
			say = "も、もしかして迷子になりました？",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "star_level_bg_116",
			say = "気づけば華麗に装飾された廊下ではなく、どこか無機質な壁に囲まれた施設に入ってしまった。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "star_level_bg_116",
			say = "おそらく王宮の地下だろうが、具体的にどこなのか思い浮かばない",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 14,
			side = 2,
			bgName = "star_level_bg_116",
			actor = 203040,
			dir = 1,
			nameColor = "#a9f548",
			say = "へ、陛下の王宮で迷子になってしまった…",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 9,
			side = 2,
			bgName = "star_level_bg_116",
			actor = 203040,
			dir = 1,
			nameColor = "#a9f548",
			say = "メイド長にバレたら一日中お説教ですよぉ～",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 299010,
			nameColor = "#a9f548",
			bgName = "star_level_bg_116",
			side = 2,
			dir = 1,
			actorName = "？？？",
			say = "あら、サフォークじゃありませんか。珍しい来客ですわ",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 14,
			side = 2,
			bgName = "star_level_bg_116",
			actor = 203040,
			dir = 1,
			nameColor = "#a9f548",
			say = "ひぃ！？",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 8,
			side = 2,
			bgName = "star_level_bg_116",
			actor = 203040,
			dir = 1,
			nameColor = "#a9f548",
			say = "誰ですか？そこにいるのは――",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 299010,
			nameColor = "#a9f548",
			bgName = "star_level_bg_116",
			side = 2,
			dir = 1,
			actorName = "？？？",
			say = "驚かせちゃってごめんなさい。ふふふ、メイド隊が迷子になるのはどうしても珍しかったから♪",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "star_level_bg_116",
			actor = 203040,
			dir = 1,
			nameColor = "#a9f548",
			say = "ネプチューン！なんでここにいるんですか？",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_116",
			actor = 203040,
			dir = 1,
			nameColor = "#a9f548",
			say = "って陛下に資料を届けるのが遅れちゃいます！＞＜",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 299010,
			nameColor = "#a9f548",
			bgName = "star_level_bg_116",
			side = 2,
			dir = 1,
			actorName = "ネプチューン",
			say = "なるほど～資料を届けている途中だったんですのね",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 299010,
			nameColor = "#a9f548",
			bgName = "star_level_bg_116",
			side = 2,
			dir = 1,
			actorName = "ネプチューン",
			say = "それでしたらこの私が案内して差し上げましょうか♪",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 13,
			side = 2,
			bgName = "star_level_bg_116",
			actor = 203040,
			dir = 1,
			nameColor = "#a9f548",
			say = "本当ですか？ありがとうネプチューン、助かりました～",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_116",
			actor = 203040,
			dir = 1,
			nameColor = "#a9f548",
			say = "ふぅ…これでメイド長からのお説教も大丈夫と…",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 202080,
			side = 2,
			bgName = "star_level_bg_116",
			nameColor = "#a9f548",
			dir = 1,
			say = "それは心配いりません。もう遅刻ですから",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_116",
			actor = 203040,
			dir = 1,
			nameColor = "#a9f548",
			say = "シェフィ！？うぅ、やっぱり駄目でしたか…",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_116",
			actor = 203040,
			dir = 1,
			nameColor = "#a9f548",
			say = "それにしてもここはどこなんでしょうか？",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 9,
			side = 2,
			bgName = "star_level_bg_116",
			actor = 203040,
			dir = 1,
			nameColor = "#a9f548",
			say = "ううん、陛下にこの資料を届けようと思ったら、なぜかここに迷い込んじゃいまして…",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 202080,
			side = 2,
			bgName = "star_level_bg_116",
			nameColor = "#a9f548",
			dir = 1,
			say = "そうですね。あまりにも急な移転で通路の設計はまだまだ甘いようですね",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 11,
			side = 2,
			bgName = "star_level_bg_116",
			actor = 203040,
			dir = 1,
			nameColor = "#a9f548",
			say = "ん……？",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 202080,
			side = 2,
			bgName = "star_level_bg_116",
			nameColor = "#a9f548",
			dir = 1,
			say = "気にしないでください。ここはあくまで骨董品の陳列室にする予定の場所なので。…外まで案内しましょう",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_116",
			actor = 203040,
			dir = 1,
			nameColor = "#a9f548",
			say = "助かるよシェフィ～！",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 202080,
			side = 2,
			bgName = "star_level_bg_116",
			nameColor = "#a9f548",
			dir = 1,
			say = "いいえ、別に大したことではありません。それと…",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 202080,
			side = 2,
			bgName = "star_level_bg_116",
			nameColor = "#a9f548",
			dir = 1,
			say = "陛下はここのことを秘密だと仰っています。誰にも話さないでください",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 14,
			side = 2,
			bgName = "star_level_bg_116",
			actor = 203040,
			dir = 1,
			blackBg = true,
			nameColor = "#a9f548",
			say = "分かりました！今は陛下に怒られる前に急がないと！シェフィ、もっと早くお願い～！",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		}
	}
}
