return {
	fadeOut = 1.5,
	mode = 2,
	id = "ZIJIANGJINLAN32",
	once = true,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_wuzang_bg14",
			stopbgm = true,
			say = "演習海域にて",
			bgm = "musashi-2",
			flashout = {
				black = true,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = true,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 302080,
			side = 2,
			bgName = "bg_wuzang_bg14",
			nameColor = "#A9F548FF",
			dir = 1,
			say = "完全に暗くなった…この演習場は鏡面海域並みに天候の変化が激しいな",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "bg_wuzang_bg14",
			actor = 303100,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "でも雰囲気がすごいっすね…まるで皆既日食みたいっす",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_wuzang_bg14",
			actor = 304010,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "そうですわね。皆既日食にしても、単純に夜を演出するだけにしても演出としては迫力十分ですわ",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_wuzang_bg14",
			actor = 304010,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "これが武蔵さんの「心象」と言われれば、どす黒すぎてちょっと怖いですけど……",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_wuzang_bg14",
			actor = 305110,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "ちょっと待った。お主も武蔵がこの天気のような性格じゃないことは知っているだろ？",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_wuzang_bg14",
			actor = 305110,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "（しかし、皆既日食にしても闇夜にしても、確かに通常ならば天気が変わるにしてもこうも急にはならん）",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_wuzang_bg14",
			actor = 305110,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "（武蔵がさっき旗風に強引に攻撃を行ったことを考えると、そういう憶測が出てきても不思議ではない）",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305110,
			side = 2,
			bgName = "bg_wuzang_bg14",
			nameColor = "#A9F548FF",
			dir = 1,
			say = "（…って、何を考えているんだ？武蔵もこの演習場が鏡面海域だって言ってたろうに）",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_wuzang_bg14",
			actor = 305110,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "皆、「心象」とやらに引きずられると冷静にものを考えられなくなるぞ",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_wuzang_bg14",
			actor = 303100,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "そ、そうっすね。ここは目の前の状況に集中すべきっす",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_wuzang_bg14",
			actor = 305110,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "演習が終わったらまた武蔵に聞いてみようか。ははは",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_wuzang_bg14",
			actor = 304010,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "こうも暗くなっていては艦載機の方向を取るのも一苦労ですわね。あの距離から攻めてくることはないと考えていいのかしら？",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305110,
			side = 2,
			bgName = "bg_wuzang_bg14",
			nameColor = "#A9F548FF",
			dir = 1,
			say = "そうだな。今の攻撃で千歳の攻撃能力がだいぶ損なわれたし、赤城は「塔」の攻撃を再開しているな",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_wuzang_bg14",
			actor = 305110,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "武蔵がこれからどう動くかは…ふむ、我にもわからん、としか言いようがないな",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_wuzang_bg14",
			paintingNoise = true,
			dir = 1,
			actor = 305050,
			nameColor = "#A9F548FF",
			say = "三笠大先輩、次はどう動くのだ？",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305110,
			side = 2,
			bgName = "bg_wuzang_bg14",
			nameColor = "#A9F548FF",
			dir = 1,
			say = "今の攻撃で陣形が乱れているし、とりあえず状況整理をしよう",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_wuzang_bg14",
			actor = 305110,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "まずは演習全体のことだが、もう残り時間が少ない。赤城が「塔」の攻撃を開始している以上、我々を攻撃する可能性も少ない",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305110,
			side = 2,
			bgName = "bg_wuzang_bg14",
			nameColor = "#A9F548FF",
			dir = 1,
			say = "そして「塔」への攻撃経路は律儀に残してくれている、これから艦隊の戦力が回復し次第攻撃を再開する",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_wuzang_bg14",
			actor = 305110,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "演習の勝敗だが、中央拠点を除くと、長門の得点が圧倒的だ",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305110,
			side = 2,
			bgName = "bg_wuzang_bg14",
			nameColor = "#A9F548FF",
			dir = 1,
			say = "武蔵は「塔」を落とす戦力が残っていないが、仮に「塔」を落とせたとしても長門の勝利は揺るがない",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_wuzang_bg14",
			actor = 305110,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "赤城は「塔」を落とせば逆転はできるが、そこに集中するのは長門にとっても、我にとってもありがたい",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305110,
			side = 2,
			bgName = "bg_wuzang_bg14",
			nameColor = "#A9F548FF",
			dir = 1,
			say = "漁夫の利を狙えと言うつもりではない。今は戦力を整える時間なのだ",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_wuzang_bg14",
			actor = 305110,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "ただ、もう一つだけ不安定要素……つまり武蔵本人がどう動くか、が残っている",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_wuzang_bg14",
			actor = 305110,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "彼女は我々の声が聞こえないが、それは我々にとっても同じだ",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305110,
			side = 2,
			bgName = "bg_wuzang_bg14",
			nameColor = "#A9F548FF",
			dir = 1,
			say = "「部下」を減らすことを目的に自分から演習場に入ってくることは考えにくい。だが絶対にそうならない、というのは早計だ",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_wuzang_bg14",
			paintingNoise = true,
			dir = 1,
			actor = 305050,
			nameColor = "#A9F548FF",
			say = "うむ。状況は理解したぞ",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305110,
			side = 2,
			bgName = "bg_wuzang_bg14",
			nameColor = "#A9F548FF",
			dir = 1,
			say = "鬼怒、羽黒、量産艦を回収しつつ春月に協力してこちらの最後の拠点を取ってくれ",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_wuzang_bg14",
			actor = 305110,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "江風と翔鶴にも同じことを頼めるか？最後に艦隊を集結する前にできるだけ戦力を集めたい",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305110,
			side = 2,
			bgName = "bg_wuzang_bg14",
			nameColor = "#A9F548FF",
			dir = 1,
			say = "武蔵への説得ができれば、千歳と酒匂、若月にも中央拠点の「塔」の攻撃に参加してもらいたいが……",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_wuzang_bg14",
			actor = 305110,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "まあ、残念だがそれは無理だ。彼女たちが量産艦を使って最後の決戦で横槍を入れてこないようにする必要がある",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_wuzang_bg14",
			actor = 305110,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "榛名と比叡は武蔵側の方に回って「塔」を囲めるように待機してほしい。それで攻撃経路を最大限に活用できる",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_wuzang_bg14",
			paintingNoise = true,
			dir = 1,
			actor = 305050,
			nameColor = "#A9F548FF",
			say = "あの「塔」の反撃で霧島が一撃で倒された。できるだけ距離を長く取ったほうが良い",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			paintingNoise = true,
			side = 2,
			bgName = "bg_wuzang_bg14",
			actor = 305050,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "金剛型戦艦はあと3人が健在…余と三笠大先輩だけでなく、最大火力を出すには全員での一斉攻撃が望ましいな",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_wuzang_bg14",
			paintingNoise = true,
			dir = 1,
			actor = 305050,
			nameColor = "#A9F548FF",
			say = "それに「心象」のことを考えると……",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305110,
			side = 2,
			bgName = "bg_wuzang_bg14",
			nameColor = "#A9F548FF",
			dir = 1,
			say = "ああ、金剛たちはこの演習場では戦力を通常以上に発揮できる。活用しない手はない",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_wuzang_bg14",
			actor = 305110,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "さて、最後は武蔵がこの演習場に入ってきたときの対策だが……",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305110,
			side = 2,
			bgName = "bg_wuzang_bg14",
			nameColor = "#A9F548FF",
			dir = 1,
			say = "正直、彼女が全力で攻撃してくる場合、長門以外は正面からぶつかっても勝てないから回避に専念するほかない",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_wuzang_bg14",
			actor = 305110,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "もう一つ対策があるとすれば、それこそ彼女が入ってくる前に「部下」をすべて倒すことだ",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305110,
			side = 2,
			bgName = "bg_wuzang_bg14",
			nameColor = "#A9F548FF",
			dir = 1,
			say = "「部下」の艦船が全員倒された時点で武蔵の敗北が決定し、こちらに入ってこれなくなるはず",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_wuzang_bg14",
			actor = 305110,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "酒匂たちには申し訳ないが、勝利を狙うだけならこれが一番確実だと思うが……長門、どうする？",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_wuzang_bg14",
			paintingNoise = true,
			dir = 1,
			actor = 305050,
			nameColor = "#A9F548FF",
			say = "余は艦船と相争わぬことを決めた。故に攻撃はしない",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305110,
			side = 2,
			bgName = "bg_wuzang_bg14",
			nameColor = "#A9F548FF",
			dir = 1,
			say = "「千里の道は一歩より、小流を積まざれば、以て江海を成すこと無し」…か",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_wuzang_bg14",
			actor = 305110,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "長門、お主が自分を貫いたことから我々の勝利の道筋が開かれた",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_wuzang_bg14",
			actor = 305110,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "あの「塔」の反撃は確かに厄介だが、我々にはまだ戦力が残っているし、盾代わりの量産艦もある",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305110,
			side = 2,
			bgName = "bg_wuzang_bg14",
			nameColor = "#A9F548FF",
			dir = 1,
			say = "もし「塔」を一撃で破壊できればそれに越したことはないが、仮に叶わなくとも二撃目を用意できる",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_wuzang_bg14",
			actor = 305110,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "赤城たちの艦載機は比較的安全に攻撃できるが、しかしアビータはともかく、塔への攻撃はやはり砲撃のほうが効果的だ",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305110,
			side = 2,
			bgName = "bg_wuzang_bg14",
			nameColor = "#A9F548FF",
			dir = 1,
			say = "今度の攻撃は我が一撃目で全力を出す。そしてお主が二撃目をもって塔を制圧する",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_wuzang_bg14",
			actor = 305110,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "我々…重桜の艦船たちが協力してこそ正しき未来に導けるということを、赤城に示そう！",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
