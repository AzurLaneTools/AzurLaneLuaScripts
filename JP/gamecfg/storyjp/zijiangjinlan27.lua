return {
	fadeOut = 1.5,
	mode = 2,
	id = "ZIJIANGJINLAN27",
	once = true,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_wuzang_bg13",
			stopbgm = true,
			say = "演習海域にて・8ターン目（武蔵）",
			bgm = "musashi-2",
			flashout = {
				black = true,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = true,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_wuzang_bg13",
			actor = 302230,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "赤城さんも長門さまも大先輩も皆残りの拠点を狙っていますね～",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 302230,
			side = 2,
			bgName = "bg_wuzang_bg13",
			nameColor = "#A9F548FF",
			dir = 1,
			say = "しかも大先輩と長門さまは中央拠点で合流しましたし…うちが動こうにも戦力が色々足りないですね",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_wuzang_bg13",
			actor = 306080,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "そ、そうね…千代田が戦闘不能になったから…うぅ、千代田…大丈夫かな……",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_wuzang_bg13",
			actor = 301880,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "さっき長門さまの艦隊が境界線を通ったのを見ました！こっちの拠点まで奪う気ですよね…？",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_wuzang_bg13",
			actor = 301880,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "若月たちはもう負けたのでしょうか……あ！もしかして武蔵さんも長門さまみたいに助けてくれたりして…",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 302230,
			side = 2,
			bgName = "bg_wuzang_bg13",
			nameColor = "#A9F548FF",
			dir = 1,
			say = "きっと助けてくれますよ！それも一番ピンチな時に！",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_wuzang_bg13",
			actor = 302230,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "神木の桜の花びらが舞う中で登場する長門さま、燃え盛るヒガンバナの爆炎とともに降り立つ赤城さん――",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_wuzang_bg13",
			actor = 302230,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "三笠大先輩はどうでしたっけ…まあいいか！とにかく武蔵さんの演出はぜっっったいかっこいいですよ！",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 302230,
			side = 2,
			bgName = "bg_wuzang_bg13",
			nameColor = "#A9F548FF",
			dir = 1,
			say = "あ、言ったそばから……武蔵さんの指示は…ええと、なんでしょう？",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 306080,
			side = 2,
			bgName = "bg_wuzang_bg13",
			nameColor = "#A9F548FF",
			dir = 1,
			say = "皆それぞれ違う…しかも中央拠点を狙う指示はないね……",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301880,
			side = 2,
			bgName = "bg_wuzang_bg13",
			nameColor = "#A9F548FF",
			dir = 1,
			say = "若月への指示は拠点の制圧ですね…長門さまと鉢合わせしないといいですけど…",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_wuzang_bg13",
			actor = 302230,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "こっちのは……一体どういうこと…？",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_wuzang_bg13",
			actor = 302230,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "…………{namecode:5}。",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301040,
			side = 2,
			bgName = "bg_wuzang_bg13",
			nameColor = "#A9F548FF",
			dir = 1,
			say = "は、はい……？",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_wuzang_bg13",
			actor = 302230,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "ごめんなさいね。急に言いにくいことだけど",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_wuzang_bg13",
			actor = 301040,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "もしかして……あっ…",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_wuzang_bg13",
			actor = 302230,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "そのですね…武蔵さんから千歳と一緒に深雪ちゃんを攻撃してって指示が入りました",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 8,
			side = 2,
			bgName = "bg_wuzang_bg13",
			actor = 301040,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "あぅ……でもどうして…？今までは武蔵さん、赤城さんと協力するんじゃなかったの…？",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 302230,
			side = 2,
			bgName = "bg_wuzang_bg13",
			nameColor = "#A9F548FF",
			dir = 1,
			say = "酒匂にはわからないですね…武蔵さんに聞こうとしても、この通信機じゃ声が届きませんし",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_wuzang_bg13",
			actor = 302230,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "深雪ちゃんの反撃は回避しませんから、それで酒匂を許して！ね？",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_wuzang_bg13",
			actor = 301040,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "う、うん…！じゃあ深雪も赤城さんの部下として、頑張って戦う…！",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			nameColor = "#A9F548FF",
			bgName = "bg_wuzang_bg13",
			soundeffect = "event:/battle/boom2",
			say = "——————————！",
			flashN = {
				color = {
					1,
					1,
					1,
					1
				},
				alpha = {
					{
						0,
						1,
						0.2,
						0
					},
					{
						1,
						0,
						0.2,
						0.2
					},
					{
						0,
						1,
						0.2,
						0.4
					},
					{
						1,
						0,
						0.2,
						0.6
					}
				}
			},
			dialogShake = {
				speed = 0.09,
				x = 8.5,
				number = 2
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_wuzang_bg13",
			say = "その後、三笠艦隊にて……",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_wuzang_bg13",
			actor = 303100,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "えっ、武蔵さんが赤城さんを攻撃したんすか？？",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "bg_wuzang_bg13",
			actor = 303100,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "確か武蔵さん、ずっと赤城さんの味方をしてたっすよね？それなのにどうして？",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_wuzang_bg13",
			actor = 304010,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "私もよくわかりませんわ。三笠大先輩はなにか見当がついていて？",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_wuzang_bg13",
			actor = 305110,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "残念だが我にも武蔵の行動の意味がわからん",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_wuzang_bg13",
			actor = 305110,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "（赤城と仲違いでもしたのか？いや、ありえないな…武蔵はまだ碁盤の部屋にいて、赤城とは話せない）",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_wuzang_bg13",
			actor = 305110,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "（それになぜ今更そんなことをする？長門を試すのはまだ理解できるとしても……）",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_wuzang_bg13",
			actor = 305110,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "（やはり二人が最初からつながっていて、いまの行動も作戦のうち、と考えたほうが妥当か）",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_wuzang_bg13",
			actor = 305110,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "（となると、深雪をあえて退場させる理由は……演習場の艦船を減らす理由は……）",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_wuzang_bg13",
			actor = 305110,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "（赤城が言っていた…「それも鉄血の反逆が成就した今、セイレーンの警戒は厳しいものになっています。いまさら余人を引き込むなど……」）",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_wuzang_bg13",
			actor = 305110,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "まさか…武蔵はこの場で赤城になにかを仕掛けようとしているのか……！？",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_wuzang_bg13",
			actor = 303100,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "ど、どういうことっすか？それが深雪ちゃんを攻撃したのとどういう関係が…",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_wuzang_bg13",
			actor = 305110,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "我にもよく分からぬ。だがこの演習場が本当に鏡面海域だとすれば、やはり二人は何かをしようとしているかもしれない",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_wuzang_bg13",
			actor = 304010,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "武蔵さんが「塔」に攻撃を仕掛けたのも説明できますね……やはり演習を一度止めますか？",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305110,
			side = 2,
			bgName = "bg_wuzang_bg13",
			nameColor = "#A9F548FF",
			dir = 1,
			say = "それには及ばぬ。もし人数を減らすだけが目的なら、最初から回りくどいことをせず戦闘させればいい",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 303100,
			side = 2,
			bgName = "bg_wuzang_bg13",
			nameColor = "#A9F548FF",
			dir = 1,
			say = "じゃ、じゃあ今になって乗り出したのは…",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_wuzang_bg13",
			actor = 301790,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "武蔵も本気で皆の「心象」を語らせようとしていた。となるとこの狙いは別にある、というべきか",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 7,
			side = 2,
			bgName = "bg_wuzang_bg13",
			actor = 303100,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "仲間を退場させて何を……",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301790,
			side = 2,
			bgName = "bg_wuzang_bg13",
			nameColor = "#A9F548FF",
			dir = 1,
			say = "もしかしたら長門の時と同じく、「赤城」を試しているだけかもしれない。武蔵はもう勝てないからな",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_wuzang_bg13",
			actor = 305110,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "若月たちを分散配置するのも人数を減らすためと言われたら説明がつくが、これも一つわからないところがあるな",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 302080,
			side = 2,
			bgName = "bg_wuzang_bg13",
			nameColor = "#A9F548FF",
			dir = 1,
			say = "「長門さまの艦隊を攻撃する」ではなく、「拠点を制圧」する、か？",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305110,
			side = 2,
			bgName = "bg_wuzang_bg13",
			nameColor = "#A9F548FF",
			dir = 1,
			say = "うむ、旗風の言う通り、あくまで両方を試しているやもしれん",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_wuzang_bg13",
			actor = 305110,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "（今更だが、この演習場に入ることを選んだのは失策だった……か？）",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_wuzang_bg13",
			actor = 305110,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "（あの場にいたらもっと色々聞き出せたかもしれないが…いや、我がいることで長門がようやく戦う決心をし、不戦を貫けた）",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_wuzang_bg13",
			actor = 305110,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "（……！なるほど、用意周到とはこういうことか…！）",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
