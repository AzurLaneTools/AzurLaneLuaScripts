return {
	id = "YOUYINGMICHENG23-1",
	mode = 2,
	fadeOut = 1.5,
	scripts = {
		{
			side = 2,
			bgName = "star_level_bg_306",
			NextIcon = 1,
			bgm = "story-visioncity-1",
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "遠くから、警報の音が次々と鳴り響き始めた。シティの外周部がいよいよ本格的に交戦へ入った合図だ。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashin = {
				dur = 1,
				black = true,
				delay = 0,
				alpha = {
					1,
					0
				}
			}
		},
		{
			portrait = "zhihuiguan",
			side = 2,
			bgName = "star_level_bg_306",
			nameColor = "#A9F548FF",
			actor = 0,
			NextIcon = 1,
			hidePaintObj = true,
			say = "――妖異が攻勢に出てきた……ここもじきに危なくなる",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			portrait = "zhihuiguan",
			side = 2,
			bgName = "star_level_bg_306",
			nameColor = "#A9F548FF",
			actor = 0,
			NextIcon = 1,
			hidePaintObj = true,
			say = "――タイガー、今すぐ安全なところに連れていく",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_306",
			nameColor = "#A9F548FF",
			actorName = "ある普通の後方要員",
			actor = 317030,
			NextIcon = 1,
			hidePaintObj = true,
			say = "指揮官……！",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			NextIcon = 1,
			side = 2,
			bgName = "star_level_bg_306",
			hidePaintObj = true,
			nameColor = "#A9F548FF",
			say = "制御室の外から、慌ただしい足音が入ってきた。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_306",
			nameColor = "#A9F548FF",
			actorName = "ある普通の後方要員",
			actor = 317030,
			NextIcon = 1,
			hidePaintObj = true,
			say = "……タイガー……あなた……",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_306",
			nameColor = "#A9F548FF",
			actorName = "ある普通の後方要員",
			actor = 317030,
			NextIcon = 1,
			hidePaintObj = true,
			say = "…………",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_306",
			nameColor = "#A9F548FF",
			actorName = "ある普通の後方要員",
			actor = 317030,
			NextIcon = 1,
			hidePaintObj = true,
			say = "臨時指揮所まで連れていくために来た…！避難のお手伝いをさせて…！",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			NextIcon = 1,
			side = 2,
			bgName = "star_level_bg_306",
			hidePaintObj = true,
			nameColor = "#A9F548FF",
			say = "少女は軽く頭を下げ、道を空けた。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			NextIcon = 1,
			side = 2,
			bgName = "star_level_bg_306",
			hidePaintObj = true,
			nameColor = "#A9F548FF",
			say = "タイガーとこの場から離れようとしたその時、妖しく艶めいた炎が前触れもなく噴き上がり、一瞬で出口への道を塞いだ。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			NextIcon = 1,
			side = 2,
			bgName = "star_level_bg_306",
			hidePaintObj = true,
			nameColor = "#A9F548FF",
			say = "揺らめく炎の中から見覚えのある人影が、ゆっくりと歩み出てきた。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_306",
			factiontag = "妖異",
			dir = 1,
			nameColor = "#FF9B93",
			actor = 304090,
			NextIcon = 1,
			hidePaintObj = true,
			say = "……なかなか楽しいものを見せてくれたじゃない",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "star_level_bg_306",
			factiontag = "妖異",
			dir = 1,
			nameColor = "#FF9B93",
			actor = 304090,
			NextIcon = 1,
			hidePaintObj = true,
			say = "私がずっと壊したいと思っていたものをまさか、あなたたちが止めてくれたなんて",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			portrait = "zhihuiguan",
			side = 2,
			bgName = "star_level_bg_306",
			nameColor = "#A9F548FF",
			actor = 0,
			NextIcon = 1,
			hidePaintObj = true,
			say = "――安土が壊したかったのは……これだったのか？",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			NextIcon = 1,
			side = 2,
			bgName = "star_level_bg_306",
			hidePaintObj = true,
			nameColor = "#A9F548FF",
			say = "安土は完全に停止した巨大な装置へと視線を向けた。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_306",
			factiontag = "妖異",
			dir = 1,
			nameColor = "#FF9B93",
			actor = 304090,
			NextIcon = 1,
			hidePaintObj = true,
			say = "妖異の誕生は「幸福都市計画」と切っても切れない関係にあるの",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_306",
			factiontag = "妖異",
			dir = 1,
			nameColor = "#FF9B93",
			actor = 304090,
			NextIcon = 1,
			hidePaintObj = true,
			say = "リセット装置は、人の感情の中から「不要」と判定されたものを解離させ――",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#FF9B93",
			side = 2,
			bgName = "star_level_bg_306",
			factiontag = "妖異",
			dir = 1,
			actor = 304090,
			NextIcon = 1,
			hidePaintObj = true,
			say = "その「幸福」を妨げる不純物を全部外へと排出させる。そうして切り捨てられた感情がやがて妖異になるというわけ",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_306",
			factiontag = "妖異",
			dir = 1,
			nameColor = "#FF9B93",
			actor = 304090,
			NextIcon = 1,
			hidePaintObj = true,
			say = "もっとはっきり言えば、妖異とはすなわち「人間が幸福になるために捨てられたもの」よ",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#FF9B93",
			side = 2,
			bgName = "star_level_bg_306",
			factiontag = "妖異",
			dir = 1,
			actor = 304090,
			NextIcon = 1,
			hidePaintObj = true,
			say = "だから、伊14",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_306",
			factiontag = "妖異",
			dir = 1,
			nameColor = "#FF9B93",
			actor = 304090,
			NextIcon = 1,
			hidePaintObj = true,
			say = "あなた、こんなことをする人間と手を取り合えるって、本気で信じてるの？",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			NextIcon = 1,
			side = 2,
			bgName = "star_level_bg_306",
			hidePaintObj = true,
			nameColor = "#A9F548FF",
			say = "空気がしんと静まり返った。後方要員を名乗っていた少女は手で顔を覆い、深々とため息をついた。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "star_level_bg_306",
			nameColor = "#A9F548FF",
			actorName = "ある普通の後方要員",
			actor = 317030,
			NextIcon = 1,
			hidePaintObj = true,
			say = "……もう、これ以上演じるのは無理みたい",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#FF9B93",
			side = 2,
			factiontag = "妖異",
			dir = 1,
			bgName = "star_level_bg_306",
			actor = 317031,
			NextIcon = 1,
			hidePaintObj = true,
			say = "改めて、自己紹介…わたしは伊14",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				dur = 1,
				black = true,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				dur = 1,
				black = true,
				delay = 1,
				alpha = {
					1,
					0
				}
			}
		},
		{
			expression = 9,
			side = 2,
			bgName = "star_level_bg_306",
			factiontag = "妖異",
			dir = 1,
			nameColor = "#FF9B93",
			actor = 317031,
			NextIcon = 1,
			hidePaintObj = true,
			say = "安土さまの命令で……あなたたちを監視してたの",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#FF9B93",
			side = 2,
			bgName = "star_level_bg_306",
			factiontag = "妖異",
			dir = 1,
			actor = 304090,
			NextIcon = 1,
			hidePaintObj = true,
			say = "ふふふ♪その仮面と偽りの姿を最後まで貫き通せたらよかったのにね",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 10,
			side = 2,
			bgName = "star_level_bg_306",
			factiontag = "妖異",
			dir = 1,
			nameColor = "#FF9B93",
			actor = 317031,
			NextIcon = 1,
			hidePaintObj = true,
			say = "……そのつもりだった……でも、もし……",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			NextIcon = 1,
			side = 2,
			bgName = "star_level_bg_306",
			hidePaintObj = true,
			nameColor = "#A9F548FF",
			say = "伊14は一歩前に出て、こちらとタイガーを庇うように安土に立ちはだかり、意外そうな表情を浮かべる安土を静かに見据えた。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_306",
			factiontag = "妖異",
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 317031,
			NextIcon = 1,
			hidePaintObj = true,
			say = "もし指揮官に手を出すつもりなら……わたしは、もう言うことを聞いてあげない……",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_306",
			factiontag = "妖異",
			dir = 1,
			nameColor = "#FF9B93",
			actor = 304090,
			NextIcon = 1,
			hidePaintObj = true,
			say = "あら……？そっちに潜入させたとはいえ、そっちに仕えさせたいわけじゃなかったんだけど？",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_306",
			factiontag = "妖異",
			dir = 1,
			actor = 317031,
			NextIcon = 1,
			hidePaintObj = true,
			say = "好きに言えばいい……今自分が何をしてるか、分かってるから……",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "star_level_bg_306",
			factiontag = "妖異",
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 317031,
			NextIcon = 1,
			hidePaintObj = true,
			say = "ただ……必死にほかの子を虚無から引き上げるような人を……こんな場所に置き去りにしちゃいけない…そう、思っただけ",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "star_level_bg_306",
			factiontag = "妖異",
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 317031,
			NextIcon = 1,
			hidePaintObj = true,
			say = "……だから、指揮官。今は、わたしがあなたを引き上げるから……",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
