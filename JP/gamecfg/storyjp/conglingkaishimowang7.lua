return {
	id = "CONGLINGKAISHIMOWANG7",
	mode = 2,
	fadeOut = 1.5,
	scripts = {
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_520",
			say = "パーティーが森の奥深くに進んでいくにつれ、果てしない樹海の中の道はますます複雑になっていった。",
			bgm = "theme-camelot",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = true,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = true,
				alpha = {
					1,
					0
				}
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_520",
			say = "しかし、情報収集と分析スキルのおかげで、自分の端末は道が分岐する場所で常に正しい道筋を示している。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_520",
			say = "藪を通り抜けた先に、開けた林間の空き地が現れた。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			portrait = 131171,
			side = 2,
			bgName = "star_level_bg_518",
			factiontag = "伝説の大魔法使い",
			dir = 1,
			nameColor = "#A9F548FF",
			actorName = "ラフィー",
			say = "すぴー…Zzzzzzzz",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = true,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = true,
				alpha = {
					1,
					0
				}
			}
		},
		{
			portrait = 131171,
			side = 2,
			bgName = "star_level_bg_518",
			factiontag = "伝説の大魔法使い",
			dir = 1,
			nameColor = "#A9F548FF",
			actorName = "ラフィー",
			say = "Zzzzzzzzz",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_518",
			factiontag = "プリースト",
			dir = 1,
			actor = 202341,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "……ねえ、誰かが芝生で寝てるよ？",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_518",
			factiontag = "プリースト",
			dir = 1,
			actor = 202341,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "しかもかわいいウサウサスタッフを持ってる……あの子が噂の大魔法使いよね？",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 501020,
			side = 2,
			bgName = "star_level_bg_518",
			factiontag = "勇者",
			dir = 1,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "どうだろ…みんなはどう思う？",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_518",
			factiontag = "見習いプリースト",
			dir = 1,
			actor = 236031,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "ユニコーン、あの子から凄まじい魔力を感じるよ",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_518",
			factiontag = "指挥官",
			nameColor = "#A9F548FF",
			say = "――「情報分析」でもレベルと魔法攻撃力がすごく高く表示されているな",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 501020,
			side = 2,
			bgName = "star_level_bg_518",
			factiontag = "勇者",
			dir = 1,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "ふむ……本物の大魔法使い、というわけか",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 501020,
			side = 2,
			bgName = "star_level_bg_518",
			factiontag = "勇者",
			dir = 1,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "で、なんで無防備で芝生で寝てるの？…こんなのパーティーに入れて本当に役に立つの？",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			portrait = 131171,
			side = 2,
			bgName = "star_level_bg_518",
			factiontag = "伝説の大魔法使い",
			dir = 1,
			nameColor = "#A9F548FF",
			actorName = "ラフィー",
			say = "……魔法反応を感知",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			portrait = 131171,
			side = 2,
			bgName = "star_level_bg_518",
			factiontag = "伝説の大魔法使い",
			dir = 1,
			nameColor = "#A9F548FF",
			actorName = "ラフィー",
			say = "……ん？あなたたちは？",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_518",
			factiontag = "指挥官",
			nameColor = "#A9F548FF",
			say = "――気づかれたし、一度話してみるか？",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_518",
			factiontag = "伝説の大魔法使い",
			dir = 1,
			actor = 131171,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "……ラフィー、用は分かった",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = true,
				dur = 0.5,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 0.5,
				dur = 0.5,
				black = true,
				alpha = {
					1,
					0
				}
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_518",
			factiontag = "伝説の大魔法使い",
			dir = 1,
			actor = 131171,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "でも断る",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_518",
			factiontag = "プリースト",
			dir = 1,
			actor = 202341,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "えっ！私たちは魔王を討伐しに行くんだよ？",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_518",
			factiontag = "プリースト",
			dir = 1,
			actor = 202341,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "ラフィーは一度魔王を倒した魔法使いじゃないの？どうして断るの？",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_518",
			factiontag = "伝説の大魔法使い",
			dir = 1,
			actor = 131171,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "魔王を倒しても……しばらくしたら新しい魔王が出てくる",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "star_level_bg_518",
			factiontag = "伝説の大魔法使い",
			dir = 1,
			actor = 131171,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "ラフィー、もう疲れた。…戦いたくない……",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 501020,
			side = 2,
			bgName = "star_level_bg_518",
			factiontag = "勇者",
			dir = 1,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "ふむ。やっぱり役に立ちそうにない…",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_518",
			factiontag = "指挥官",
			nameColor = "#A9F548FF",
			say = "（なんだか変な気がする……）",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_518",
			factiontag = "指挥官",
			nameColor = "#A9F548FF",
			say = "（おっと、「情報分析」スキルに何か反応があるな。もしかしたら使用タイミングを知らせてくれてる…？）",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			options = {
				{
					content = "ラフィーに「情報分析」スキルを使ってみた。",
					flag = 1
				}
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_518",
			say = "スキルが発動すると、大魔法使いに関する情報が端末に流れ込んできた。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "冒険端末",
			bgName = "star_level_bg_518",
			nameColor = "#A9F548FF",
			say = "「大魔法使い・ラフィー、種族：ウサウサ星人」",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			icon = {
				scale = 1.4,
				image = "Props/story_tablet",
				pos = {
					0,
					100
				}
			}
		},
		{
			side = 2,
			actorName = "冒険端末",
			bgName = "star_level_bg_518",
			nameColor = "#A9F548FF",
			say = "「千年前に魔王を斬り殺したと言われる伝説の勇者パーティーの一員」",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			icon = {
				scale = 1.4,
				image = "Props/story_tablet",
				pos = {
					0,
					100
				}
			}
		},
		{
			side = 2,
			actorName = "冒険端末",
			bgName = "star_level_bg_518",
			nameColor = "#A9F548FF",
			say = "「現在のステータス：呪いに囚われている」",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			icon = {
				scale = 1.4,
				image = "Props/story_tablet",
				pos = {
					0,
					100
				}
			}
		},
		{
			side = 2,
			actorName = "冒険端末",
			bgName = "star_level_bg_518",
			nameColor = "#A9F548FF",
			say = "「呪いの名称：「怠惰の呪い」」",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			icon = {
				scale = 1.4,
				image = "Props/story_tablet",
				pos = {
					0,
					100
				}
			}
		},
		{
			side = 2,
			actorName = "冒険端末",
			bgName = "star_level_bg_518",
			nameColor = "#A9F548FF",
			say = "「前代の魔王が残した呪い。呪いの対象者を過去の繰り返しの中に閉じ込め、その理性が完全に負の感情に呑み込まれるまで続く」",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			icon = {
				scale = 1.4,
				image = "Props/story_tablet",
				pos = {
					0,
					100
				}
			}
		},
		{
			side = 2,
			actorName = "冒険端末",
			bgName = "star_level_bg_518",
			nameColor = "#A9F548FF",
			say = "「解呪方法：不明」",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			icon = {
				scale = 1.4,
				image = "Props/story_tablet",
				pos = {
					0,
					100
				}
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_518",
			factiontag = "指挥官",
			nameColor = "#A9F548FF",
			say = "――どうやら呪いを解かないとパーティーに入れられなさそうだ…",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 501020,
			side = 2,
			bgName = "star_level_bg_518",
			factiontag = "勇者",
			dir = 1,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "まさかそんなことがあったなんて…",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 501020,
			side = 2,
			bgName = "star_level_bg_518",
			factiontag = "勇者",
			dir = 1,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "魔王は千年前に討伐されていたのに、呪いは今も続いている",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 501020,
			side = 2,
			bgName = "star_level_bg_518",
			factiontag = "勇者",
			dir = 1,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "千年間ずっと呪いに苦しめられ続けているのか…",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "star_level_bg_518",
			factiontag = "プリースト",
			dir = 1,
			actor = 202341,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "むしろ千年も呪いに蝕まれて、それでも理性を失っていないなんてすごいよ！",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_518",
			factiontag = "見習いプリースト",
			dir = 1,
			actor = 236031,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "お兄ちゃん、ラフィーの呪いを解く方法を探そう…！",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_518",
			factiontag = "指挥官",
			nameColor = "#A9F548FF",
			say = "――もちろん。でもどうやら一筋縄にはいかなそうだな…情報を集めないと",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 501020,
			side = 2,
			bgName = "star_level_bg_518",
			factiontag = "勇者",
			dir = 1,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "指揮官！ラフィーの眠ってるマットの隣に皿が置かれてるぞ！もしかして誰かが定期的にご飯を届けてくれてるんじゃないか？",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_518",
			factiontag = "プリースト",
			dir = 1,
			actor = 202341,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "本当だ！…その人なら何か知ってるかも！",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_518",
			say = "よく観察してみると、周囲には食べ物が乗っている皿だけでなく、治療用のポーションもいくつか置かれている。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			portrait = 403140,
			side = 2,
			bgName = "star_level_bg_518",
			factiontag = "鋼鉄教会メカニック",
			dir = 1,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "ポーションは近くの村に住んでいる薬師のものよ～",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 501020,
			side = 2,
			bgName = "star_level_bg_518",
			factiontag = "勇者",
			dir = 1,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "あれ？王都で会った鋼鉄教会のメカニック？",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 501020,
			side = 2,
			bgName = "star_level_bg_518",
			factiontag = "勇者",
			dir = 1,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "いきなり現れて…もしかして指揮官をストーカーしてるの？！",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "star_level_bg_518",
			factiontag = "鋼鉄教会メカニック",
			dir = 1,
			actor = 403140,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "ふふ……いきなり出てきてごめんね～。でもストーカーじゃないわ",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = true,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = true,
				alpha = {
					1,
					0
				}
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "star_level_bg_518",
			factiontag = "鋼鉄教会メカニック",
			dir = 1,
			actor = 403140,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "みんなが「終わりなき樹海」に向かっているのを見たからよ",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 403140,
			side = 2,
			bgName = "star_level_bg_518",
			factiontag = "鋼鉄教会メカニック",
			dir = 1,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "奥の様子は見ての通り危険だから…ちょっと気になってついて来ただけ",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_518",
			factiontag = "鋼鉄教会メカニック",
			dir = 1,
			actor = 403140,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "教会も大魔法使いの呪いの解呪を試みてきたけど、中々進展がなくてねぇ～",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "star_level_bg_518",
			factiontag = "指挥官",
			hidePaintObj = true,
			nameColor = "#A9F548FF",
			say = "――そこで近くに住んでいる薬師を頼ってみたらどうか、ってことか",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_518",
			factiontag = "鋼鉄教会メカニック",
			dir = 1,
			actor = 403140,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "そう。これらのポーションはすべてその薬師が残したもの",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 7,
			side = 2,
			bgName = "star_level_bg_518",
			factiontag = "鋼鉄教会メカニック",
			dir = 1,
			actor = 403140,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "あの子、なぜか大魔法使いの世話をずっとしているわ",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 403140,
			side = 2,
			bgName = "star_level_bg_518",
			factiontag = "鋼鉄教会メカニック",
			dir = 1,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "自分なりに解呪を頑張ってもいるみたいだけど…",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_518",
			factiontag = "鋼鉄教会メカニック",
			dir = 1,
			actor = 403140,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "残念なことに、まだ成果は出ていないみたいね",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 403140,
			side = 2,
			bgName = "star_level_bg_518",
			factiontag = "鋼鉄教会メカニック",
			dir = 1,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "「次は絶対大丈夫」って言ってはいたけども…",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 7,
			side = 2,
			bgName = "star_level_bg_518",
			factiontag = "鋼鉄教会メカニック",
			dir = 1,
			actor = 403140,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "私はもう諦めちゃったけど、よかったら一度訪ねてみたら？それじゃあ私は帰るわ",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 501020,
			side = 2,
			bgName = "star_level_bg_518",
			factiontag = "勇者",
			dir = 1,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "近くの村にいる、大魔法使いの呪いを解呪しようとする薬師……ヒントは十分だね",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 501020,
			side = 2,
			bgName = "star_level_bg_518",
			factiontag = "勇者",
			dir = 1,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "指揮官、私たちも見に行こう！",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		}
	}
}
