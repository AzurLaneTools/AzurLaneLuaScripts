return {
	fadeOut = 1.5,
	mode = 2,
	id = "GONGMINGDEPASSION17",
	placeholder = {
		"playername"
	},
	scripts = {
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_story_task_2",
			hidePaintObj = true,
			say = "母港・執務室",
			bgm = "votefes-start",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = true,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = true,
				alpha = {
					1,
					0
				}
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_story_task_2",
			hidePaintObj = true,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 302240,
			say = "失礼します",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_story_task_2",
			hidePaintObj = true,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 102320,
			say = "し、失礼するよ…",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_story_task_2",
			hidePaintObj = true,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 302240,
			say = "「Alizarin」の視点からまとめた、今回の「μ兵装」演習実施状況報告書です",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_story_task_2",
			hidePaintObj = true,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 102320,
			say = "…「Cyanidin」のはこっち",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "bg_story_task_2",
			hidePaintObj = true,
			nameColor = "#A9F548FF",
			say = "――ご苦労様。まとめてくれてありがとう",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_story_task_2",
			hidePaintObj = true,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 102320,
			say = "う、ううん…そういうのは元々職務のうちだし…ふーはー",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_story_task_2",
			hidePaintObj = true,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 302240,
			say = "はい、やるべきことをしたまでです",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "bg_story_task_2",
			hidePaintObj = true,
			nameColor = "#A9F548FF",
			say = "――そういえばほかのメンバーたちは？まだ休憩中？",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_story_task_2",
			hidePaintObj = true,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 302240,
			say = "いいえ、フォーミダブルはイラストリアスにレディのマナーを勉強するよう言われ…オイゲンは共鳴現象について研究しているようです",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_story_task_2",
			hidePaintObj = true,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 102320,
			say = "あ、あの現象が起きた時…本当にびっくりしたよ…",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "bg_story_task_2",
			hidePaintObj = true,
			nameColor = "#A9F548FF",
			say = "（もう三回目の実験なのに、未だに「μ兵装」を取り巻く謎は多く、我々は思ったほど実態を把握できていないということだ…）",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "bg_story_task_2",
			hidePaintObj = true,
			nameColor = "#A9F548FF",
			say = "（今後もまだまだテストを行っていく必要がありそうだ）",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_story_task_2",
			hidePaintObj = true,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 302240,
			say = "ところであなた、いつまで仕事をしているつもり？もうそろそろ時間ですが…",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "bg_story_task_2",
			hidePaintObj = true,
			nameColor = "#A9F548FF",
			say = "――え？",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_story_task_2",
			hidePaintObj = true,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 302240,
			say = "私の記憶が正しければ…この後赤城と大鳳の願いを叶える時間でしょう？",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_story_task_2",
			hidePaintObj = true,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 302240,
			say = "その次がプリンツ・オイゲン…そしてローン、ユニコーンとハムマンは順番が前後していて…",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_story_task_2",
			hidePaintObj = true,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 302240,
			say = "それからはイラストリアスに私、ですね",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 302240,
			side = 2,
			bgName = "bg_story_task_2",
			hidePaintObj = true,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "あ、あとボイシも…夜は確か金剛とテメレールが…",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			options = {
				{
					content = "わ、わかったわかった",
					flag = 1
				},
				{
					content = "頭痛がしてきた……",
					flag = 2
				}
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "bg_story_task_2",
			hidePaintObj = true,
			nameColor = "#A9F548FF",
			optionFlag = 1,
			say = "――逃げてもしょうがないとわかっているが……",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "bg_story_task_2",
			hidePaintObj = true,
			nameColor = "#A9F548FF",
			optionFlag = 1,
			say = "――い、今はほかの話をしよう…！",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "bg_story_task_2",
			hidePaintObj = true,
			nameColor = "#A9F548FF",
			optionFlag = 2,
			say = "――みんなにとって喜ばしいことなのに、ちょっとこれは頭が痛くなるな…",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "bg_ouxiangsanqi_2",
			hidePaintObj = true,
			nameColor = "#A9F548FF",
			say = "——つまり…全員のどんな願いも…全部叶えなきゃいけない…ってコト！？",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = true,
				dur = 0.5,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 0.5,
				dur = 0.5,
				black = true,
				alpha = {
					1,
					0
				}
			},
			effects = {
				{
					active = true,
					name = "memoryFog"
				}
			},
			dialogShake = {
				speed = 0.08,
				x = 15,
				number = 2
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_ouxiangsanqi_2",
			hidePaintObj = true,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 905020,
			say = "そういうことよ。まぁ、実行については主催側で一部代わってあげてもいいけど――",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "bg_ouxiangsanqi_2",
			hidePaintObj = true,
			nameColor = "#A9F548FF",
			say = "――いいけど？",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "bg_ouxiangsanqi_2",
			hidePaintObj = true,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 905020,
			say = "ええ、もしみんなの願いが全部指揮官と関わることだったら…いくら主催委員会でも助けられないわ。ふふふ",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "bg_story_task_2",
			hidePaintObj = true,
			nameColor = "#A9F548FF",
			say = "（ラフィーのオーダーメイドのハイテクふわもこ枕と、ル・マランの一年間仕事しないことを除いて…）",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = true,
				dur = 0.5,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 0.5,
				dur = 0.5,
				black = true,
				alpha = {
					1,
					0
				}
			},
			effects = {
				{
					active = false,
					name = "memoryFog"
				}
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "bg_story_task_2",
			hidePaintObj = true,
			nameColor = "#A9F548FF",
			say = "（全員本当にそうなるとは……）",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_story_task_2",
			hidePaintObj = true,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 307010,
			say = "ふふふ…指揮官様、赤城が迎えに上がりましたわ♡",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 307010,
			side = 2,
			bgName = "bg_story_task_2",
			hidePaintObj = true,
			dir = 1,
			nameColor = "#A9F548FF",
			say = "赤城との「デート」のお願い、お忘れじゃないでしょうね？",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_story_task_2",
			hidePaintObj = true,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 307070,
			say = "待ちなさい！大鳳との順番はまだ決まってありませんわ！",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_story_task_2",
			hidePaintObj = true,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 307010,
			say = "はぁ？赤城が先に来ましたわ？",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_story_task_2",
			hidePaintObj = true,
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 307070,
			say = "じゃあなんですか？大鳳に首を縦に振れってことですか？指揮官様は大鳳のものですわ！",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_story_task_2",
			hidePaintObj = true,
			say = "我先にと、二つの赤い影が執務室で熾烈な口論を繰り広げている。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "bg_story_task_2",
			hidePaintObj = true,
			nameColor = "#A9F548FF",
			say = "（順番が決まるまで…今日の仕事をもう少し処理できそうだ）",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 0,
			bgName = "bg_story_task_2",
			actor = 307010,
			dir = 1,
			actorName = "赤城＆大鳳",
			hideOther = true,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "指揮官様ー！\n指揮官様〜！",
			subActors = {
				{
					actor = 307070,
					pos = {
						x = 1185
					}
				},
				expression = 5
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 4,
			side = 0,
			bgName = "bg_story_task_2",
			actor = 307010,
			dir = 1,
			actorName = "赤城＆大鳳",
			hideOther = true,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "決まりましたわ！いっそのこと、三人一緒にデートしましょう！",
			subActors = {
				{
					actor = 307070,
					pos = {
						x = 1185
					}
				},
				expression = 3
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 0,
			side = 2,
			bgName = "bg_story_task_2",
			hidePaintObj = true,
			nameColor = "#A9F548FF",
			say = "――えぇ！？",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			dialogShake = {
				speed = 0.08,
				x = 15,
				number = 2
			}
		},
		{
			mode = 6,
			time = 30,
			bgm = "votefes-start",
			layout = {
				{
					3,
					{
						"",
						""
					},
					1
				},
				{
					3,
					{
						"",
						""
					},
					1
				},
				{
					3,
					{
						"",
						""
					},
					1
				},
				{
					1,
					"第3回「μ兵装実験テスト」・母港大型ライブ"
				},
				{
					3,
					{
						"",
						""
					},
					1
				},
				{
					3,
					{
						"",
						""
					},
					1
				},
				{
					2,
					"bg_ouxiangsanqi_cg1",
					1024,
					576
				},
				{
					3,
					{
						"",
						""
					},
					1
				},
				{
					3,
					{
						"",
						""
					},
					1
				},
				{
					1,
					"主催実行委員会"
				},
				{
					3,
					{
						"※以下順不同",
						"{playername}",
						"レキシントン",
						"クレマンソー",
						"三笠",
						"マルコ・ポーロ",
						"逸仙",
						"ウルリッヒ・フォン・フッテン",
						"ウォースパイト",
						"明石"
					},
					1,
					"#FFFFFF"
				},
				{
					3,
					{
						"",
						""
					},
					1
				},
				{
					3,
					{
						"",
						""
					},
					1
				},
				{
					2,
					"bg_ouxiangsanqi_cg2",
					1024,
					576
				},
				{
					3,
					{
						"",
						""
					},
					1
				},
				{
					3,
					{
						"",
						""
					},
					1
				},
				{
					1,
					"MC"
				},
				{
					3,
					{
						"総合MC",
						"明石",
						"赤組MC",
						"ハルビン",
						"青組MC",
						"サラトガ"
					},
					2
				},
				{
					3,
					{
						"",
						""
					},
					1
				},
				{
					1,
					"参加者"
				},
				{
					1,
					"赤組"
				},
				{
					1,
					"※以下順不同"
				},
				{
					3,
					{
						"「Astrum」",
						"ボルチモア",
						"",
						"イラストリアス",
						"",
						"アルバコア",
						"",
						"ダイドー",
						"",
						"タシュケント",
						"「Alizarin」",
						"プリンツ・オイゲン",
						"",
						"能代",
						"",
						"フォーミダブル",
						"「Lumière」",
						"ル・マラン",
						"「名もなき期間限定」",
						"吹雪",
						"",
						"Z23",
						"ソロアイドル",
						"クイーン・エリザベス",
						"",
						"ハムマン"
					},
					2,
					"#FFFFFF"
				},
				{
					3,
					{
						"",
						""
					},
					1
				},
				{
					1,
					"青組"
				},
				{
					1,
					"※以下順不同"
				},
				{
					3,
					{
						"「Cyanidin」",
						"ボイシ",
						"",
						"金剛",
						"",
						"ル・テメレール",
						"「Polaris」",
						"赤城",
						"",
						"ガスコーニュ",
						"",
						"クリーブランド",
						"",
						"アドミラル・ヒッパー",
						"",
						"シェフィールド",
						"「Verheerender」",
						"大鳳",
						"",
						"ローン",
						"「東煌シスターズ」",
						"平海",
						"",
						"寧海",
						"ソロアイドル",
						"ユニコーン",
						"",
						"ラフィー"
					},
					2,
					"#FFFFFF"
				},
				{
					3,
					{
						"",
						""
					},
					1
				},
				{
					3,
					{
						"制作プロデューサー",
						"三笠",
						"ラインプロデューサー",
						"逸仙",
						"プロダクションマネージャー",
						"ウォースパイト",
						"音楽監督",
						"ウルリッヒ・フォン・フッテン",
						"振付監督",
						"レキシントン"
					},
					2,
					"#FFFFFF"
				},
				{
					3,
					{
						"",
						""
					},
					1
				},
				{
					1,
					"撮影チーム"
				},
				{
					3,
					{
						"スタジオ監督",
						"グアム",
						"ロケ監督",
						"エンタープライズ",
						"撮影担当",
						"グリッドレイ",
						"",
						"アルフレード・オリアーニ",
						"",
						"青葉",
						"",
						"レオナルド・ダ・ヴィンチ",
						"",
						"TB"
					},
					2,
					"#FFFFFF"
				},
				{
					3,
					{
						"",
						""
					},
					1
				},
				{
					1,
					"ビジュアルチーム"
				},
				{
					3,
					{
						"ビジュアルデザイナー",
						"プリマス",
						"",
						"Z35",
						"アニメーションデザイナー",
						"綾波",
						"",
						"パーミャチ・メルクーリヤ",
						"映像制作",
						"TB",
						"ポストプロダクション",
						"TB"
					},
					2,
					"#FFFFFF"
				},
				{
					3,
					{
						"",
						""
					},
					1
				},
				{
					1,
					"セキュリティチーム"
				},
				{
					3,
					{
						"総責任者",
						"クロンシュタット",
						"現場指揮",
						"ソビエツカヤ・ベラルーシア",
						"本部指揮",
						"ル・テリブル",
						"緊急対応担当",
						"TB",
						"セキュリティスタッフ",
						"キアサージ",
						"",
						"ヴァンガード",
						"",
						"鬼怒",
						"",
						"オーディン",
						"",
						"済安",
						"",
						"ドゥーカ・デッリ・アブルッツィ",
						"",
						"クルスク",
						"",
						"ラ・ガリソニエール",
						"",
						"ガスコーニュ"
					},
					2,
					"#FFFFFF"
				},
				{
					3,
					{
						"",
						""
					},
					1
				},
				{
					1,
					"舞台演出"
				},
				{
					3,
					{
						"舞台監督",
						"メンフィス",
						"照明デザイナー",
						"神通",
						"音響デザイナー",
						"マルセイエーズ",
						"舞台デザイナー",
						"プリマス",
						"",
						"Z35",
						"設備調整",
						"ヘレナ"
					},
					2,
					"#FFFFFF"
				},
				{
					3,
					{
						"",
						""
					},
					1
				},
				{
					1,
					"スタイリストチーム"
				},
				{
					3,
					{
						"メイクアップアーティスト",
						"熊野",
						"",
						"鈴谷",
						"",
						"酒匂",
						"",
						"プリマス",
						"",
						"ベルファスト",
						"",
						"ニューカッスル",
						"",
						"グロスター",
						"衣装設計",
						"ロンドン",
						"",
						"ハウ",
						"",
						"ダンケルク",
						"",
						"マルコ・ポーロ"
					},
					2,
					"#FFFFFF"
				},
				{
					3,
					{
						"",
						""
					},
					1
				},
				{
					1,
					"バックチーム"
				},
				{
					3,
					{
						"総務担当",
						"トリエステ",
						"キャスティングコーディネーター",
						"レキシントン"
					},
					2,
					"#FFFFFF"
				},
				{
					3,
					{
						"",
						""
					},
					1
				},
				{
					3,
					{
						"",
						""
					},
					1
				},
				{
					1,
					"スペシャルサンクス"
				},
				{
					3,
					{
						"μ兵装実験テスト特設第一航空艦隊",
						"μ兵装実験テスト特設第二航空艦隊",
						"μ兵装実験テスト特設第三航空艦隊",
						"μ兵装実験テスト特設第四航空艦隊",
						"μ兵装実験テスト特設第五航空艦隊",
						"μ兵装実験テスト特設第六航空艦隊",
						"μ兵装実験テスト特設第七航空艦隊"
					},
					1,
					"#FFFFFF"
				},
				{
					3,
					{
						"",
						""
					},
					1
				},
				{
					3,
					{
						"",
						""
					},
					1
				},
				{
					2,
					"story_logo_gongmingdepassion",
					1024,
					256
				}
			}
		}
	}
}
