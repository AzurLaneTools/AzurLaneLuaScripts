return {
	fadeOut = 1.5,
	mode = 2,
	id = "XUYUWANGYUECHAO2",
	once = true,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			side = 2,
			stopbgm = true,
			bgName = "bg_yunxian_1",
			hidePaintObj = true,
			nameColor = "#A9F548FF",
			oldPhoto = true,
			say = "過去　重桜・大神木の聖域",
			bgm = "theme-unzen",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_yunxian_1",
			hidePaintObj = true,
			oldPhoto = true,
			say = "大神木の根元にある小さな湖。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_yunxian_1",
			hidePaintObj = true,
			oldPhoto = true,
			say = "「仙哲」と呼ばれる、聖域の守護者である艦船は仲間たちと一緒にそっと神木に手を当てた。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305100,
			oldPhoto = true,
			bgName = "bg_yunxian_1",
			hidePaintObj = true,
			withoutActorName = true,
			nameColor = "#A9F548FF",
			side = 2,
			say = "神木から手のひらに温かく、優しい「感触」が流れてくる。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305050,
			oldPhoto = true,
			bgName = "bg_yunxian_1",
			hidePaintObj = true,
			withoutActorName = true,
			nameColor = "#A9F548FF",
			side = 2,
			say = "…が、しばらくすると、「感触」が針に刺されたような痛みに変わった。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_yunxian_1",
			hidePaintObj = true,
			oldPhoto = true,
			say = "異様を感じた艦船たちは一斉に手を引っ込めた。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_yunxian_1",
			oldPhoto = true,
			dir = 1,
			actor = 305100,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "今のは……",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 303190,
			nameColor = "#A9F548FF",
			bgName = "bg_yunxian_1",
			oldPhoto = true,
			side = 2,
			actorName = "仙哲",
			say = "心当たりがあります。武蔵さん",
			hidePaintEquip = true,
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_yunxian_1",
			oldPhoto = true,
			actorName = "仙哲",
			hidePaintEquip = true,
			actor = 303190,
			nameColor = "#A9F548FF",
			say = "「北天より落つる星あり」",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 303190,
			nameColor = "#A9F548FF",
			bgName = "bg_yunxian_1",
			oldPhoto = true,
			side = 2,
			actorName = "仙哲",
			say = "「星の落ちし海より闇が出づる時、瑞穂の水天を震わし泣かせしむ」",
			hidePaintEquip = true,
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_yunxian_1",
			oldPhoto = true,
			actorName = "仙哲",
			hidePaintEquip = true,
			actor = 303190,
			nameColor = "#A9F548FF",
			say = "「星は重桜の涙に申し訳なく思ひ、己を燃やし光放ち、災いの闇靄を駆除し聖域に封印せり」",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			oldPhoto = true,
			bgName = "bg_yunxian_1",
			factiontag = "連合艦隊旗艦",
			dir = 1,
			actor = 305050,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "………",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_yunxian_1",
			oldPhoto = true,
			dir = 1,
			actor = 305100,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "仙哲が言ったのは、もしや『綿津抄』の御伽の話？",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305100,
			oldPhoto = true,
			bgName = "bg_yunxian_1",
			hidePaintObj = true,
			dir = 1,
			nameColor = "#A9F548FF",
			side = 2,
			say = "我が重桜の宝器「ワタツミ」、そしてその故事を記した書物『綿津抄』…",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_yunxian_1",
			oldPhoto = true,
			dir = 1,
			actor = 305100,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "大神木の異変と関係があるとでも言うの？",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_yunxian_1",
			oldPhoto = true,
			actorName = "仙哲",
			actor = 303190,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "まさしく。御狐さま、武蔵さん",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 303190,
			nameColor = "#A9F548FF",
			bgName = "bg_yunxian_1",
			hidePaintObj = true,
			side = 2,
			actorName = "仙哲",
			oldPhoto = true,
			say = "ここまで来る途中、なぜ大神木の聖域まで案内したのかと聞きましたね？",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_yunxian_1",
			oldPhoto = true,
			actorName = "仙哲",
			actor = 303190,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "『綿津抄』にある御伽は荒唐無稽なれど、その中には真実が隠されています",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_yunxian_1",
			oldPhoto = true,
			actorName = "仙哲",
			actor = 303190,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "この聖域――正確に言えば「大神木」の中に、かつて北天より落ちた星の破片が眠っています",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 303190,
			nameColor = "#A9F548FF",
			bgName = "bg_yunxian_1",
			hidePaintObj = true,
			side = 2,
			actorName = "仙哲",
			oldPhoto = true,
			say = "星の破片は神木の力で封印し、災いの闇靄を封じてきましたが…",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_yunxian_1",
			oldPhoto = true,
			actorName = "仙哲",
			actor = 303190,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "それでも漏れ出す穢れによって神木の力が弱まりつつあります",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 303190,
			nameColor = "#A9F548FF",
			bgName = "bg_yunxian_1",
			hidePaintObj = true,
			side = 2,
			actorName = "仙哲",
			oldPhoto = true,
			say = "もしケガレを払い封印の力を回復できなければ、故事にある災いの闇靄が再び訪れるでしょう",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_yunxian_1",
			oldPhoto = true,
			dir = 1,
			actor = 305100,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "……それとも、艦船の力を用いて、故事に習い闇靄を打ち払うか…",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_yunxian_1",
			oldPhoto = true,
			dir = 1,
			actor = 305100,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "しかし時期が悪いわ。重桜の戦力は万全ではない…",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			oldPhoto = true,
			bgName = "bg_yunxian_1",
			factiontag = "連合艦隊旗艦",
			dir = 1,
			actor = 305050,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "うむ。セイレーンたちとの決戦を終えたばかり…たとえ戦うにしても戦力が心許ない",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_yunxian_1",
			factiontag = "連合艦隊旗艦",
			dir = 1,
			oldPhoto = true,
			actor = 305050,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "そもそも、もし砲火で倒せる存在なら、お主は回りくどい説明などをせず、余に直接出陣を求めてきていたはずだ",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_yunxian_1",
			oldPhoto = true,
			actorName = "仙哲",
			actor = 303190,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "仰るとおりです",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_yunxian_1",
			oldPhoto = true,
			actorName = "仙哲",
			actor = 303190,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "なので、あくまで重桜の術式を用いて異変を解決すべきだと存じます",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 303190,
			nameColor = "#A9F548FF",
			bgName = "bg_yunxian_1",
			hidePaintObj = true,
			side = 2,
			actorName = "仙哲",
			oldPhoto = true,
			say = "封印を強化し、願わくば闇靄を「かき消す」ことができれば、と",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			oldPhoto = true,
			bgName = "bg_yunxian_1",
			factiontag = "連合艦隊旗艦",
			dir = 1,
			actor = 305050,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "かき消す……？",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_yunxian_1",
			factiontag = "連合艦隊旗艦",
			dir = 1,
			oldPhoto = true,
			actor = 305050,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "お主の術式でそれが可能なのか？",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_yunxian_1",
			hidePaintObj = true,
			oldPhoto = true,
			say = "答える代わりに頷くと、「仙哲」と呼ばれる艦船は何かを唱えると、術式を発動した。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_yunxian_1",
			hidePaintObj = true,
			oldPhoto = true,
			say = "神木の力に呼応し、根元から湖畔の空に向かって桜色の光の線が現れ、複雑な模様――陣門を成した。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_yunxian_1",
			hidePaintObj = true,
			oldPhoto = true,
			say = "――「結界」と呼ばれる重桜の術式である。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_yunxian_1",
			hidePaintObj = true,
			oldPhoto = true,
			say = "しばらくして結界が完成すると、今度は桜色の光が三人のいる空間を覆った。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_yunxian_1",
			oldPhoto = true,
			actorName = "仙哲",
			actor = 303190,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "はい、準備はすでに整っております",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			oldPhoto = true,
			bgName = "bg_yunxian_1",
			factiontag = "連合艦隊旗艦",
			dir = 1,
			actor = 305050,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "流石は聖域を守りし者だけあって、素晴らしい手際だ",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_yunxian_1",
			factiontag = "連合艦隊旗艦",
			dir = 1,
			oldPhoto = true,
			actor = 305050,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "この結界を余が習った時はあまり理解できなかったが、こうも精妙になるとはな",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 303190,
			nameColor = "#A9F548FF",
			bgName = "bg_yunxian_1",
			hidePaintObj = true,
			side = 2,
			actorName = "仙哲",
			oldPhoto = true,
			say = "お褒めに預かり光栄です。ですがこれでも心許ないです",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_yunxian_1",
			oldPhoto = true,
			dir = 1,
			actor = 305100,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "そのために妾がいるわ。案内しながら汝の真意を説明するが良い",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_yunxian_1",
			hidePaintObj = true,
			oldPhoto = true,
			say = "そう言いながら、武蔵は艤装の一部である雷を纏う太刀を具現化させた。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_yunxian_1",
			hidePaintObj = true,
			oldPhoto = true,
			say = "鞘から抜き、柄を握りしめ、気持ちを戦闘態勢に切り替える。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_yunxian_1",
			oldPhoto = true,
			actorName = "仙哲",
			actor = 303190,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "……覚悟はできているようですね",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_yunxian_1",
			oldPhoto = true,
			actorName = "仙哲",
			actor = 303190,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "これより星の破片――「ワタツミ」のところに案内させていただきます",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 303190,
			nameColor = "#A9F548FF",
			bgName = "bg_yunxian_1",
			hidePaintObj = true,
			side = 2,
			actorName = "仙哲",
			oldPhoto = true,
			say = "そして御狐さまに封印の中に入ってもらい、「ワタツミ」と直接対面していただきます",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_yunxian_1",
			oldPhoto = true,
			actorName = "仙哲",
			actor = 303190,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "道中は神木に蓄積されたケガレがありますが、この結界の力が防いでくれましょう",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_yunxian_1",
			oldPhoto = true,
			actorName = "仙哲",
			actor = 303190,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "しかし一度封印の中に入ると、「ワタツミ」の影と化した闇靄が襲いかかってくるはずです",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_yunxian_1",
			oldPhoto = true,
			dir = 1,
			actor = 305100,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "長門さまは妾が守るわ。続けて",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_yunxian_1",
			oldPhoto = true,
			actorName = "仙哲",
			actor = 303190,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "…ありがとうございます。武蔵さん",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 303190,
			nameColor = "#A9F548FF",
			bgName = "bg_yunxian_1",
			hidePaintObj = true,
			side = 2,
			actorName = "仙哲",
			oldPhoto = true,
			say = "重桜の信仰を預かる「御狐」たる長門さまには、「ワタツミ」に触れていただきます",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_yunxian_1",
			oldPhoto = true,
			actorName = "仙哲",
			actor = 303190,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "封印の概念が強くとも、重桜の代表である御狐さまなら無事たどり着けるでしょう",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_yunxian_1",
			oldPhoto = true,
			actorName = "仙哲",
			actor = 303190,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "これだけは他の方では決してなし得ないことです",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 303190,
			nameColor = "#A9F548FF",
			bgName = "bg_yunxian_1",
			hidePaintObj = true,
			side = 2,
			actorName = "仙哲",
			oldPhoto = true,
			say = "短い間なら長門さまは「ワタツミ」の残った力を行使できるはずです",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_yunxian_1",
			oldPhoto = true,
			actorName = "仙哲",
			actor = 303190,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "「ワタツミ」の力で一度闇靄を鎮め、その間に「ワタツミ」に代わる存在として封印の礎を設置できれば、異変を解決できます",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 303190,
			nameColor = "#A9F548FF",
			bgName = "bg_yunxian_1",
			hidePaintObj = true,
			side = 2,
			actorName = "仙哲",
			oldPhoto = true,
			say = "………そして、封印の礎は私がなります",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_yunxian_1",
			oldPhoto = true,
			actorName = "仙哲",
			actor = 303190,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "武蔵さん。術式が完了するまで、どうか長門さまを守ってください",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 303190,
			nameColor = "#A9F548FF",
			bgName = "bg_yunxian_1",
			hidePaintObj = true,
			side = 2,
			actorName = "仙哲",
			oldPhoto = true,
			say = "上手くいって闇靄を滅ぼせられれば、封印も不要になりましょう",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_yunxian_1",
			oldPhoto = true,
			actorName = "仙哲",
			actor = 303190,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "その時は星の破片…ワタツミの破片を預けましょう",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 303190,
			nameColor = "#A9F548FF",
			bgName = "bg_yunxian_1",
			hidePaintObj = true,
			side = 2,
			actorName = "仙哲",
			oldPhoto = true,
			say = "力を使い果たした抜け殻ではありますが、それでも重桜の未来を変えてくれるはずです",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_yunxian_1",
			oldPhoto = true,
			actorName = "仙哲",
			actor = 303190,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "長門さま、武蔵さん……どうか重桜の未来を守ってくださいませ",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_yunxian_1",
			oldPhoto = true,
			actorName = "仙哲",
			actor = 303190,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "最後に………",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 303190,
			nameColor = "#A9F548FF",
			bgName = "bg_yunxian_1",
			hidePaintObj = true,
			side = 2,
			actorName = "仙哲",
			oldPhoto = true,
			say = "武蔵さん、約束は果たさせてもらいます",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 303190,
			oldPhoto = true,
			bgName = "bg_yunxian_1",
			hidePaintObj = true,
			withoutActorName = true,
			nameColor = "#A9F548FF",
			side = 2,
			say = "「仙哲」は袖の中から小さい箱を取り出し、武蔵に渡した。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_yunxian_1",
			oldPhoto = true,
			actorName = "仙哲",
			actor = 303190,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "リュウコツを修復する手がかりはこちらにあります",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_yunxian_1",
			hidePaintObj = true,
			oldPhoto = true,
			say = "覚悟が伝わり、重苦しい空気がその場を支配した。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_yunxian_1",
			hidePaintObj = true,
			oldPhoto = true,
			say = "「仙哲」、武蔵、長門、そしてもう一人の同行者の全員が言葉を発することがなかった。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
