return {
	fadeOut = 1.5,
	mode = 2,
	fadeType = 2,
	once = true,
	id = "XUYUWANGYUECHAO6",
	fadein = 1.5,
	scripts = {
		{
			stopbgm = true,
			side = 2,
			bgName = "star_level_bg_508",
			oldPhoto = true,
			bgm = "deepblue-image",
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "過去　重桜・武蔵の居所",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_508",
			hidePaintObj = true,
			oldPhoto = true,
			say = "翌日。武蔵に招待された「仙哲」は、信濃の部屋を訪れた。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_508",
			hidePaintObj = true,
			oldPhoto = true,
			say = "耳を立てると、薄い障子の向こうからかすかな呼吸の音が聞こえてくる。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_508",
			hidePaintObj = true,
			oldPhoto = true,
			say = "なるべく音を立てないよう引き戸を引き、様子を軽く見てまた閉める。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_508",
			oldPhoto = true,
			actor = 303190,
			actorName = "仙哲",
			hidePaintObj = true,
			say = "……………",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			oldPhoto = true,
			side = 2,
			bgName = "star_level_bg_508",
			dir = 1,
			actor = 305100,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "信濃が神木の異変に関係があるとでも？",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_508",
			oldPhoto = true,
			actor = 303190,
			actorName = "仙哲",
			hidePaintObj = true,
			say = "いいえ、おそらくは私達の「源」――リュウコツの影響だと思います",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_508",
			nameColor = "#A9F548FF",
			oldPhoto = true,
			actor = 303190,
			actorName = "仙哲",
			hidePaintObj = true,
			say = "信濃特有のものではありませんが、簡単には直せる問題ではありませんね",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			oldPhoto = true,
			side = 2,
			bgName = "star_level_bg_508",
			dir = 1,
			actor = 305100,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "まさかとは思うが、天城も……",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_508",
			oldPhoto = true,
			actor = 303190,
			actorName = "仙哲",
			hidePaintObj = true,
			say = "…………ですが…",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_508",
			nameColor = "#A9F548FF",
			oldPhoto = true,
			actor = 303190,
			actorName = "仙哲",
			hidePaintObj = true,
			say = "例え治療することができなくても、施せる手はまだあるはずです",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_508",
			hidePaintObj = true,
			oldPhoto = true,
			say = "「仙哲」が廊下に出て、そっと手を柵に添えて武蔵に向き直した。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_508",
			nameColor = "#A9F548FF",
			oldPhoto = true,
			actor = 303190,
			actorName = "仙哲",
			hidePaintObj = true,
			say = "「大神木」の聖域に…",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_508",
			oldPhoto = true,
			actor = 303190,
			actorName = "仙哲",
			hidePaintObj = true,
			say = "武蔵さん、長門さまを連れて一度お越しいただけますでしょうか",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_508",
			nameColor = "#A9F548FF",
			oldPhoto = true,
			actor = 303190,
			actorName = "仙哲",
			hidePaintObj = true,
			say = "今こうして武蔵さんと話してはいますが、私の本体はあの聖域から離れられません",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_508",
			oldPhoto = true,
			actor = 303190,
			actorName = "仙哲",
			hidePaintObj = true,
			say = "故に大事な話も、できれば聖域にて話させていただきたいのです",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_508",
			nameColor = "#A9F548FF",
			oldPhoto = true,
			actor = 303190,
			actorName = "仙哲",
			hidePaintObj = true,
			say = "大神木のもとであれば、お二人にはすべてをお話しましょう",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			oldPhoto = true,
			side = 2,
			bgName = "star_level_bg_508",
			dir = 1,
			actor = 305100,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "「仙哲」、汝の要望は分かったが…",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_508",
			oldPhoto = true,
			dir = 1,
			actor = 305100,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "妾はともかく、何故長門さままで…なぜ妾たち二人を選んだの…？",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_508",
			nameColor = "#A9F548FF",
			oldPhoto = true,
			actor = 303190,
			actorName = "仙哲",
			hidePaintObj = true,
			say = "御狐（神子）の名を引き継いだ長門さまは唯一無二の力を有しています",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_508",
			nameColor = "#A9F548FF",
			oldPhoto = true,
			actor = 303190,
			actorName = "仙哲",
			hidePaintObj = true,
			say = "そこに武蔵さんが護衛を勤めれば安心できるというものです",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_508",
			nameColor = "#A9F548FF",
			oldPhoto = true,
			actor = 303190,
			actorName = "仙哲",
			hidePaintObj = true,
			say = "神木「重桜」を守護する「みこ」とあなた……",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_508",
			nameColor = "#A9F548FF",
			oldPhoto = true,
			actor = 303190,
			actorName = "仙哲",
			hidePaintObj = true,
			say = "（私がいなくなっても、この瑞穂の水天を守ってくださるでしょう）",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
