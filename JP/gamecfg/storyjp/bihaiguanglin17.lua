return {
	fadeOut = 1.5,
	mode = 2,
	id = "BIHAIGUANGLIN17",
	once = true,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			nameColor = "#a9f548",
			side = 2,
			bgName = "bg_story_nepu2",
			say = "重桜にて",
			dir = 1,
			bgmDelay = 1,
			bgm = "battle-pacific",
			flashin = {
				delay = 1,
				dur = 1,
				black = true,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = true,
			side = 2,
			bgName = "bg_story_nepu2",
			actor = 305050,
			dir = 1,
			nameColor = "#a9f548",
			say = "筑摩や山風たちまでも竜宮城と一緒に消えたのか！？",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = true,
			side = 2,
			bgName = "bg_story_nepu2",
			actor = 305110,
			dir = 1,
			nameColor = "#a9f548",
			say = "その竜宮城、本当に消えたのか？",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "bg_story_nepu2",
			actor = 307080,
			dir = 1,
			nameColor = "#a9f548",
			say = "……艦載機を送ったところ、跡型もなく…",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_story_nepu2",
			actor = 307080,
			dir = 1,
			nameColor = "#a9f548",
			say = "鉄血のほうからも、同じ連絡が…",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = true,
			side = 2,
			bgName = "bg_story_nepu2",
			actor = 305110,
			dir = 1,
			nameColor = "#a9f548",
			say = "幸先が悪いな…鉄血への訪問、大したことが無いと思っていたが",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = true,
			side = 2,
			bgName = "bg_story_nepu2",
			actor = 305050,
			dir = 1,
			nameColor = "#a9f548",
			say = "余に本島より援軍を出すべきだと申したのか？",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_story_nepu2",
			actor = 307080,
			dir = 1,
			nameColor = "#a9f548",
			say = "妾も様子を観たが…海中の竜宮城の様子は…少なくともセイレーンの罠とは考えにくいと…",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307080,
			side = 2,
			bgName = "bg_story_nepu2",
			nameColor = "#a9f548",
			dir = 1,
			say = "夢でも、その様子は伺えたゆえ、確実と…",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_story_nepu2",
			actor = 307080,
			dir = 1,
			nameColor = "#a9f548",
			say = "されど、要件は……「指揮官」に関して…",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "bg_story_nepu2",
			actor = 307080,
			dir = 1,
			nameColor = "#a9f548",
			say = "重桜と鉄血も等しく導いていく、「指揮官」……",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 307080,
			side = 2,
			bgName = "bg_story_nepu2",
			nameColor = "#a9f548",
			dir = 1,
			say = "夢で、その存在がいかに大事か知らされたゆえ、出自や、妾が眠っていたときのことを、知りたく…",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_story_nepu2",
			actor = 307080,
			dir = 1,
			nameColor = "#a9f548",
			say = "どうか、お二方に、ご助力のほどを……",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = true,
			side = 2,
			bgName = "bg_story_nepu2",
			actor = 305050,
			dir = 1,
			nameColor = "#a9f548",
			say = "信濃の頼みとあらば、余は喜んで――",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = true,
			side = 2,
			bgName = "bg_story_nepu2",
			actor = 305110,
			dir = 1,
			nameColor = "#a9f548",
			say = "いや長門、ここは我に任せてくれないか",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = true,
			side = 2,
			bgName = "bg_story_nepu2",
			actor = 305110,
			dir = 1,
			nameColor = "#a9f548",
			say = "……信濃が指揮官のことをどこまで知ってるかは分からぬが、一つだけ大事なことをまず述べておこう",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "bg_story_nepu2",
			actor = 307080,
			dir = 1,
			nameColor = "#a9f548",
			say = "大事な…こと……？",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = true,
			side = 2,
			bgName = "bg_story_nepu2",
			actor = 305050,
			dir = 1,
			nameColor = "#a9f548",
			say = "そうだ。指揮官はなにゆえ各陣営の艦船を率いることができるかについてだ",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = true,
			side = 2,
			bgName = "bg_story_nepu2",
			actor = 305110,
			dir = 1,
			nameColor = "#a9f548",
			say = "我に任せてくれと言ったであろう。コホン、信濃の知っての通り、我ら艦船はメンタルキューブから生み出されている",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = true,
			side = 2,
			bgName = "bg_story_nepu2",
			actor = 305110,
			dir = 1,
			nameColor = "#a9f548",
			say = "そのメンタルキューブについて、人類は前の大戦より研究を進めてきたが、大した成果を得られていないのが現状だ",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = true,
			side = 2,
			bgName = "bg_story_nepu2",
			actor = 305110,
			dir = 1,
			nameColor = "#a9f548",
			say = "……まあ艤装や、メンタルキューブのエネルギーを利用する兵器はその副産物と言えるな",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = true,
			side = 2,
			bgName = "bg_story_nepu2",
			actor = 305050,
			dir = 1,
			nameColor = "#a9f548",
			say = "うむ。メンタルキューブの軍事利用もその一つだ",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = true,
			side = 2,
			bgName = "bg_story_nepu2",
			actor = 305110,
			dir = 1,
			nameColor = "#a9f548",
			say = "実は、このメンタルキューブ――実は艦船と関係しているだけではないのだ",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = true,
			side = 2,
			bgName = "bg_story_nepu2",
			actor = 305050,
			dir = 1,
			nameColor = "#a9f548",
			say = "……！我もそれは初耳なのだが…！？",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = true,
			side = 2,
			bgName = "bg_story_nepu2",
			actor = 305110,
			dir = 1,
			nameColor = "#a9f548",
			say = "そう驚くな。この話はそこまで踏み入ったことではない",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = true,
			side = 2,
			bgName = "bg_story_nepu2",
			actor = 305110,
			dir = 1,
			nameColor = "#a9f548",
			say = "メンタルキューブは「ヒトの思いを具現化」するとされているが、具体的にどうなっているかは誰も分からぬ",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = true,
			side = 2,
			bgName = "bg_story_nepu2",
			actor = 305110,
			dir = 1,
			nameColor = "#a9f548",
			say = "――少なくとも、上層部から開示されている情報ではな",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = true,
			side = 2,
			bgName = "bg_story_nepu2",
			actor = 305110,
			dir = 1,
			nameColor = "#a9f548",
			say = "だが、メンタルキューブから生み出された我らの力をより強くできる、いわゆる「キューブ適性」を持つたった一人の存在、それが指揮官だ",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_story_nepu2",
			actor = 307080,
			dir = 1,
			nameColor = "#a9f548",
			say = "キューブ、適性……",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = true,
			side = 2,
			bgName = "bg_story_nepu2",
			actor = 305110,
			dir = 1,
			nameColor = "#a9f548",
			say = "もっとも、キューブ適性とはなんなのか、これもやはりキューブの機能と同じく、はっきりとは分かっておらぬ",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = true,
			side = 2,
			bgName = "bg_story_nepu2",
			actor = 305110,
			dir = 1,
			nameColor = "#a9f548",
			say = "ここからが重要だ。我は指揮官がこの「キューブ適性」だけの存在とは思わぬ",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = true,
			side = 2,
			bgName = "bg_story_nepu2",
			actor = 305110,
			dir = 1,
			nameColor = "#a9f548",
			say = "出自や過去がどうであれ、我々艦船にヒトと同じように接し、信頼し、そして大切にしてくれている――それが指揮官を指揮官足らしめるところだ",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = true,
			side = 2,
			bgName = "bg_story_nepu2",
			actor = 305050,
			dir = 1,
			nameColor = "#a9f548",
			say = "三笠大先輩…………",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = true,
			side = 2,
			bgName = "bg_story_nepu2",
			actor = 305110,
			dir = 1,
			nameColor = "#a9f548",
			say = "今の世界情勢はかつてなく混迷している、他の陣営だけでなく我が重桜にも各々の思いで動いている艦船がいる",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = true,
			side = 2,
			bgName = "bg_story_nepu2",
			actor = 305110,
			dir = 1,
			nameColor = "#a9f548",
			say = "前の大戦で一度団結した「アズールレーン」をもう一度結成し、すべての艦船を一丸に取りまとめるのは、指揮官しか――",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 301810,
			side = 2,
			bgName = "bg_story_nepu2",
			nameColor = "#a9f548",
			dir = 1,
			say = "信濃さん、鉄血より通信があったよ……",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 301810,
			side = 2,
			bgName = "bg_story_nepu2",
			nameColor = "#a9f548",
			dir = 1,
			say = "浮島要塞のペーターという人が、捜索の件について話したいって",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_story_nepu2",
			actor = 307080,
			dir = 1,
			nameColor = "#a9f548",
			say = "………わかった。下がってよろしい…",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = true,
			side = 2,
			bgName = "bg_story_nepu2",
			actor = 305110,
			dir = 1,
			nameColor = "#a9f548",
			say = "どうやら今日はここまでだな。まあ、この話の続きはまた今度にしてもよかろう",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_story_nepu2",
			actor = 307080,
			dir = 1,
			nameColor = "#a9f548",
			say = "……一つ、最後に…指揮官は今、いずこに……？",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = true,
			side = 2,
			bgName = "bg_story_nepu2",
			actor = 305110,
			dir = 1,
			nameColor = "#a9f548",
			say = "たしか北方連合から帰還したあと、すぐ「再現」の戦いに参加していたそうだが……~",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = true,
			side = 2,
			bgName = "bg_story_nepu2",
			actor = 305110,
			dir = 1,
			blackBg = true,
			nameColor = "#a9f548",
			say = "そうだ。今はあのニュージャージーたちとPH基地にいるはずだ~",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		}
	}
}
