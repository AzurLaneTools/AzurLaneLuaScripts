return {
	fadeOut = 1.5,
	mode = 2,
	id = "DUJIACUNSHENMISHIJIANDIAOCHA5",
	once = true,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_153",
			actorName = "劇のナレーション",
			stopbgm = true,
			paintingNoise = true,
			bgm = "cw-story",
			actor = 305130,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "「白い霧が晴れると、旅人は自分たちがマジヤバな雪山の中にいることに気づいた！」",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = false,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = false,
				alpha = {
					1,
					0
				}
			}
		},
		{
			actor = 101490,
			side = 2,
			bgName = "star_level_bg_153",
			factiontag = "ロール1の探索者",
			dir = 1,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "………い、いきなり雪山の中になった！",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_153",
			factiontag = "今日も刺激を求めて",
			dir = 1,
			actor = 501020,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "はくちゅん！臨場感強すぎるよー！",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_153",
			factiontag = "独占欲強しコアクマ",
			dir = 1,
			actor = 301890,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "さささ寒いよ！温度はともかく……この雪って本物ー！？",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_153",
			paintingNoise = true,
			actorName = "劇のナレーション",
			actor = 305130,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "「ガチ風と雪がビュービューと吹いて、旅人は体力がゴッソリ奪われた」",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_153",
			paintingNoise = true,
			actorName = "劇のナレーション",
			actor = 305130,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "「いやあ、この時期に急に雪山に行くなんていくらなんでもそれはないんじゃね？」",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_153",
			factiontag = "独占欲強しコアクマ",
			dir = 1,
			actor = 301890,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "別に来たくて来たわけじゃないわよ！勝手に設定追加しないで！",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301890,
			side = 2,
			bgName = "star_level_bg_153",
			factiontag = "独占欲強しコアクマ",
			dir = 1,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "それより今の尾張の声でしょ？一体どういうこと？",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_153",
			paintingNoise = true,
			actorName = "劇のナレーション",
			actor = 305130,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "「尾張じゃなくてナレーションだって！」",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			paintingNoise = true,
			side = 2,
			bgName = "star_level_bg_153",
			hidePaintObj = true,
			actor = 305130,
			actorName = "劇のナレーション",
			nameColor = "#A9F548FF",
			say = "「それと初づ…あなたたちはもう演劇の中の旅人よ！」",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 101490,
			side = 2,
			bgName = "star_level_bg_153",
			factiontag = "ロール1の探索者",
			dir = 1,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "なるほど！これが妖怪一座が言ってた興行か！",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_153",
			factiontag = "ロール1の探索者",
			dir = 1,
			actor = 101490,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "舞台での演劇だと思ったけど、まさか自分で参加するやつだなんて！",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 501020,
			side = 2,
			bgName = "star_level_bg_153",
			factiontag = "今日も刺激を求めて",
			dir = 1,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "参加型？……楽しそう！",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.1,
					x = 0,
					number = 2
				}
			}
		},
		{
			paintingNoise = true,
			side = 2,
			bgName = "star_level_bg_153",
			hidePaintObj = true,
			actor = 305130,
			actorName = "劇のナレーション",
			nameColor = "#A9F548FF",
			say = "「じゃあ続けるよ？」",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_153",
			paintingNoise = true,
			actorName = "劇のナレーション",
			actor = 305130,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "「えー旅人はえぐい雪嵐の中で必至に歩き続けた。つらたん」",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_153",
			paintingNoise = true,
			actorName = "劇のナレーション",
			actor = 305130,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "「んで、体力がもう、どえらいことになって意識も飛んじゃいそう！」",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			paintingNoise = true,
			side = 2,
			bgName = "star_level_bg_153",
			hidePaintObj = true,
			actor = 305130,
			actorName = "劇のナレーション",
			nameColor = "#A9F548FF",
			say = "「その時！眼の前に山小屋が見えた！」",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_153",
			factiontag = "ロール1の探索者",
			dir = 1,
			actor = 101490,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "やったぁ！山小屋があれば雪の中でガクガクしなくて済むよ！",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.1,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_153",
			factiontag = "独占欲強しコアクマ",
			dir = 1,
			actor = 301890,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "そうね。寒いんだから早く小屋に入ろ？",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_165",
			paintingNoise = true,
			actorName = "劇のナレーション",
			actor = 305130,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "「こうして旅人たちは震える手で扉を開けて、山小屋に入ったがー」",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = true,
				dur = 0.5,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 0.5,
				dur = 0.5,
				black = true,
				alpha = {
					1,
					0
				}
			}
		},
		{
			paintingNoise = true,
			side = 2,
			bgName = "star_level_bg_165",
			hidePaintObj = true,
			actor = 305130,
			actorName = "劇のナレーション",
			nameColor = "#A9F548FF",
			say = "「マジで可哀想な状況だったためか、小屋の中は神的に最高だった。小屋の壁は厚くて頑丈だし、食べ物も薪もたくさん！」",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 101490,
			side = 2,
			bgName = "star_level_bg_165",
			factiontag = "ロール1の探索者",
			dir = 1,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "よかったあ…薪があれば火を起こせて暖が取れるよね！",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301810,
			side = 2,
			bgName = "star_level_bg_165",
			factiontag = "ふわりんと一緒",
			dir = 1,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "食べ物もあるから、お腹ペコペコにならない",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_165",
			factiontag = "ふわりんと一緒",
			dir = 1,
			actor = 301810,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "でも霞たち、部屋に勝手に入って、食べ物をもらって、もしかして…悪い子？",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 501020,
			side = 2,
			bgName = "star_level_bg_165",
			factiontag = "今日も刺激を求めて",
			dir = 1,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "大丈夫だよ霞！山小屋ってそういう風に利用できるから！",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 501020,
			side = 2,
			bgName = "star_level_bg_165",
			factiontag = "今日も刺激を求めて",
			dir = 1,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "えへへ、さっそく肉をしっかり焼いて……いい焼き加減！",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.1,
					x = 0,
					number = 2
				}
			}
		},
		{
			actor = 501020,
			side = 2,
			bgName = "star_level_bg_165",
			factiontag = "今日も刺激を求めて",
			dir = 1,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "もうお腹減ってるから先に！いただきまぁす！",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			paintingNoise = true,
			side = 2,
			bgName = "star_level_bg_165",
			hidePaintObj = true,
			actor = 305130,
			actorName = "劇のナレーション",
			nameColor = "#A9F548FF",
			say = "「幸運にも雪から避難した旅人は、焚き火を囲んでうまい物を食べながらおしゃべりをした」",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_165",
			paintingNoise = true,
			actorName = "劇のナレーション",
			actor = 305130,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "「その時！ドン！と扉が大きく開かれ、雪混じりの風が山小屋に吹き込んできた！」",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 101490,
			side = 2,
			bgName = "star_level_bg_165",
			factiontag = "ロール1の探索者",
			dir = 1,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "あわわわ！早く扉を閉めて閉めて！",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.1,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "star_level_bg_165",
			factiontag = "ロール1の探索者",
			dir = 1,
			actor = 101490,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "火が消えちゃったら大変なことになっちゃうよー！",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			paintingNoise = true,
			side = 2,
			bgName = "star_level_bg_165",
			hidePaintObj = true,
			actor = 305130,
			actorName = "劇のナレーション",
			nameColor = "#A9F548FF",
			say = "「なんとか扉を閉めた旅人たち。山小屋に平和が戻った」",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = true,
				dur = 0.5,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 0.5,
				dur = 0.5,
				black = true,
				alpha = {
					1,
					0
				}
			}
		},
		{
			paintingNoise = true,
			side = 2,
			bgName = "star_level_bg_165",
			hidePaintObj = true,
			actor = 305130,
			actorName = "劇のナレーション",
			nameColor = "#A9F548FF",
			say = "「と思いきや、いつの間にか一人増えていた！」",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "プンプンする雪女",
			bgName = "star_level_bg_165",
			factiontag = "今回は雪女演劇作戦",
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "出て行って",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			icon = {
				scale = 8,
				image = "Props/story_xuenv",
				pos = {
					0,
					0
				}
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_165",
			factiontag = "独占欲強しコアクマ",
			dir = 1,
			actor = 301890,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "？ブリストル、今「出て行って」って…",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 101490,
			side = 2,
			bgName = "star_level_bg_165",
			factiontag = "ロール1の探索者",
			dir = 1,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "ん？言ってないよ？",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "プンプンする雪女",
			bgName = "star_level_bg_165",
			factiontag = "今回は雪女演劇作戦",
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "ここから出て行って…！",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			icon = {
				scale = 8,
				image = "Props/story_xuenv",
				pos = {
					0,
					0
				}
			}
		},
		{
			side = 2,
			actorName = "プンプンする雪女",
			bgName = "star_level_bg_165",
			factiontag = "今回は雪女演劇作戦",
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "愚かな人間め、誰がこの山小屋に入っていいと言った？罰として凍らせてやるわ！",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			icon = {
				scale = 8,
				image = "Props/story_xuenv",
				pos = {
					0,
					0
				}
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_165",
			factiontag = "独占欲強しコアクマ",
			dir = 1,
			actor = 301890,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "ひゃぅ！よ、妖怪が現れた！？",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.1,
					x = 0,
					number = 2
				}
			}
		},
		{
			actor = 501020,
			side = 2,
			bgName = "star_level_bg_165",
			factiontag = "今日も刺激を求めて",
			dir = 1,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "初月落ち着いて！ここがあなたのうちだって言うなら、証拠はあるよね？",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "プンプンする雪女",
			bgName = "star_level_bg_165",
			factiontag = "今回は雪女演劇作戦",
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "私は雪女、この雪山をずっとずっとさまよい続けてきたわ。だからこの雪山のすべては私のものよ！",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			icon = {
				scale = 8,
				image = "Props/story_xuenv",
				pos = {
					0,
					0
				}
			}
		},
		{
			actor = 101490,
			side = 2,
			bgName = "star_level_bg_165",
			factiontag = "ロール1の探索者",
			dir = 1,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "すべて！？じゃあ土地の権利書は？この山小屋の建設許可は？それがないのに、なんであなたのものだって言えるの？",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "プンプンする雪女",
			bgName = "star_level_bg_165",
			factiontag = "今回は雪女演劇作戦",
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "け、権利書？そんなの……",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			icon = {
				scale = 8,
				image = "Props/story_xuenv",
				pos = {
					0,
					0
				}
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_165",
			paintingNoise = true,
			actorName = "劇のナレーション",
			actor = 305130,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "「その時！ドン！と扉が大きく開かれ、雪混じりの風がまた山小屋に吹き込んできた！」",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "star_level_bg_165",
			factiontag = "ロール1の探索者",
			dir = 1,
			actor = 101490,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "わああああ！扉を閉めてーー！",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_165",
			hidePaintObj = true,
			actor = 302230,
			actorName = "真面目で強がりな雪女",
			nameColor = "#A9F548FF",
			say = "バカな人間！誰が山小屋に入っていいと言ったの？罰として氷漬けにしてあげるんだから♪",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301810,
			side = 2,
			bgName = "star_level_bg_165",
			factiontag = "ふわりんと一緒",
			dir = 1,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "酒匂？",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_165",
			hidePaintObj = true,
			actor = 302230,
			actorName = "真面目で強がりな雪女",
			nameColor = "#A9F548FF",
			say = "もうかわいいおバカさん！私は雪女、酒匂じゃないです！",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_165",
			factiontag = "独占欲強しコアクマ",
			dir = 1,
			actor = 301890,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "どう見ても酒匂でしょ！？",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 302230,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_165",
			hidePaintObj = true,
			side = 2,
			actorName = "真面目で強がりな雪女",
			say = "私は雪女、この雪山をずっとずっとさまよい続けてきたわ。だからこの雪山のすべては私のものよ！",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 101490,
			side = 2,
			bgName = "star_level_bg_165",
			factiontag = "ロール1の探索者",
			dir = 1,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "はあ…雪女がもう一人いた。ってことで合ってるよね…？",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_165",
			hidePaintObj = true,
			actor = 302230,
			actorName = "真面目で強がりな雪女",
			nameColor = "#A9F548FF",
			say = "私が唯一の雪女よ！さあ、怯えろ！艦船の性能を活かせないまま――",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 501020,
			side = 2,
			bgName = "star_level_bg_165",
			factiontag = "今日も刺激を求めて",
			dir = 1,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "はいストップ！結局どっちが本物の雪女なの？",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "プンプンする雪女",
			bgName = "star_level_bg_165",
			factiontag = "今回は雪女演劇作戦",
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "………え？",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			icon = {
				scale = 8,
				image = "Props/story_xuenv",
				pos = {
					0,
					0
				}
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_165",
			hidePaintObj = true,
			actor = 302230,
			actorName = "真面目で強がりな雪女",
			nameColor = "#A9F548FF",
			say = "こいつがニセモノで私が本物！",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_165",
			hidePaintObj = true,
			actor = 302230,
			actorName = "真面目で強がりな雪女",
			nameColor = "#A9F548FF",
			say = "（小声）って、脚本の中に書いてるわよ！",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "プンプンする雪女",
			bgName = "star_level_bg_165",
			factiontag = "今回は雪女演劇作戦",
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "？？それは初耳ね。こっちとしてはあなたがニセモノで、私が本物なんだけど",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			icon = {
				scale = 8,
				image = "Props/story_xuenv",
				pos = {
					0,
					0
				}
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_165",
			paintingNoise = true,
			actorName = "劇のナレーション",
			actor = 305130,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "「えー。どういうわけか雪女が二人現れたけどぉ……」",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_165",
			paintingNoise = true,
			actorName = "劇のナレーション",
			actor = 305130,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "「その時！ドン！とまたも扉が大きく開かれた音がした！」",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "star_level_bg_165",
			factiontag = "ロール1の探索者",
			dir = 1,
			actor = 101490,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "もう三回目だよ！今度はブリストルがしっかり閉めるからね！",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301040,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_165",
			hidePaintObj = true,
			side = 2,
			actorName = "やる気のない雪女",
			say = "に、人間さん、だれが深…雪の山小屋に入っていいと言ったの？ええと、こ、凍らせて…",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301810,
			side = 2,
			bgName = "star_level_bg_165",
			factiontag = "ふわりんと一緒",
			dir = 1,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "……凍らせてやる？",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_165",
			hidePaintObj = true,
			actor = 301040,
			actorName = "やる気のない雪女",
			nameColor = "#A9F548FF",
			say = "う、うん…凍らせてやるんだから…",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_165",
			factiontag = "独占欲強しコアクマ",
			dir = 1,
			actor = 301890,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "深雪、何やってるの……",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_165",
			hidePaintObj = true,
			actor = 301040,
			actorName = "やる気のない雪女",
			nameColor = "#A9F548FF",
			say = "み、深雪じゃなくて、雪女だよ……？",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_165",
			factiontag = "ロール1の探索者",
			dir = 1,
			actor = 101490,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "んー凍らせたいのは分かったから、まずはルールを守って列に並んでねー",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 101490,
			side = 2,
			bgName = "star_level_bg_165",
			factiontag = "ロール1の探索者",
			dir = 1,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "ほら、もう先に二人もいるんだから",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "star_level_bg_165",
			hidePaintObj = true,
			actor = 301040,
			actorName = "やる気のない雪女",
			nameColor = "#A9F548FF",
			say = "そ、そうなの…？でも私が本物の雪女なのに……？",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_165",
			hidePaintObj = true,
			actor = 302230,
			actorName = "真面目で強がりな雪女",
			nameColor = "#A9F548FF",
			say = "そっちこそニセモノじゃなくて？",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.1,
					x = 0,
					number = 2
				}
			}
		},
		{
			side = 2,
			actorName = "プンプンする雪女",
			bgName = "star_level_bg_165",
			factiontag = "今回は雪女演劇作戦",
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "違うわね。私が本物の雪女よ",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			icon = {
				scale = 8,
				image = "Props/story_xuenv",
				pos = {
					0,
					0
				}
			}
		},
		{
			actor = 302230,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_165",
			hidePaintObj = true,
			side = 2,
			actorName = "真面目で強がりな雪女",
			say = "本物はーーわ・た・し・な・の！",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_165",
			factiontag = "独占欲強しコアクマ",
			dir = 1,
			actor = 301890,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "（なんなのこの茶番…いつになったら終わるの……）",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_165",
			factiontag = "独占欲強しコアクマ",
			dir = 1,
			actor = 301890,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "んん……はい、分かった！",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_165",
			factiontag = "独占欲強しコアクマ",
			dir = 1,
			actor = 301890,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "そこの「我慢できなかった雪女」が本物よ",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 302230,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_165",
			hidePaintObj = true,
			actorName = "真面目で強がりな雪女",
			side = 2,
			say = "どういうこと？私が本物なのではなくて？",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.1,
					x = 0,
					number = 2
				}
			}
		},
		{
			actor = 301040,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_165",
			hidePaintObj = true,
			side = 2,
			actorName = "やる気のない雪女",
			say = "……ど、どうして分かったの……？",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_165",
			factiontag = "独占欲強しコアクマ",
			dir = 1,
			actor = 301890,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "そんなのすぐ分かるじゃない…",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_165",
			factiontag = "独占欲強しコアクマ",
			dir = 1,
			actor = 301890,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "酒匂と深雪は自分を本物だと主張するけど、でも見た目が全っ然雪女っぽくないじゃない！",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301890,
			side = 2,
			bgName = "star_level_bg_165",
			factiontag = "独占欲強しコアクマ",
			dir = 1,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "それに、雪女っぽい格好をしているから本物の雪女よ！",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			hideOther = true,
			side = 0,
			bgName = "star_level_bg_165",
			nameColor = "#A9F548FF",
			dir = 1,
			actor = 501020,
			actorName = "撫順&ブリストル",
			hidePaintObj = true,
			say = "そうだったのかーーー！",
			dialogShake = {
				speed = 0.08,
				x = 15,
				number = 2
			},
			subActors = {
				{
					actor = 101490,
					hidePaintObj = true,
					pos = {
						x = 1100
					}
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
