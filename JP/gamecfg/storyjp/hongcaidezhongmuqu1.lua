return {
	fadeOut = 1.5,
	mode = 2,
	fadeType = 2,
	once = true,
	id = "HONGCAIDEZHONGMUQU1",
	fadein = 1.5,
	scripts = {
		{
			side = 2,
			bgName = "bg_endingsong_1",
			dir = 1,
			bgmDelay = 2,
			bgm = "theme-highseasfleet",
			say = "ユニオン艦隊旗艦・ブリーフィングルーム",
			flashout = {
				dur = 1,
				black = true,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = true,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			bgName = "bg_endingsong_1",
			dir = 1,
			say = "北方連合による、鉄血への報復作戦であり、上層部から許可された「再現」か。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			bgName = "bg_endingsong_1",
			dir = 1,
			say = "おそらくはソユーズによる何かしらのカバーストーリーだろうが、今の自分にそれを知る術はない。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			bgName = "bg_endingsong_1",
			dir = 1,
			say = "艦船を率いる立場であるとはいえ、アズールレーン上層部の牽制がある以上、艦船たちと連携し、セイレーンとの戦いに集中するのが精一杯だ。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			bgName = "bg_endingsong_1",
			dir = 1,
			say = "――海底の「秘境」で手に入れた「微光」と名付けられたデータログならびに、遺跡の調査資料。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			bgName = "bg_endingsong_1",
			dir = 1,
			say = "――セイレーン施設からのリバースエンジニアリングの成果、特異点を作り出せる鉄血の虹の塔。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			bgName = "bg_endingsong_1",
			dir = 1,
			say = "――重桜にまつわる「想い」を叶えるワタツミ、そして「キューブ」との関係性。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			bgName = "bg_endingsong_1",
			dir = 1,
			say = "――NA海域などで遭遇した「余燼」から与えられた「アビータ」の情報とセイレーンの正体。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			bgName = "bg_endingsong_1",
			dir = 1,
			say = "一見繋がりを持たない数々の情報を反芻してみると、意外と見えてくるものがあった。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			bgName = "bg_endingsong_1",
			dir = 1,
			say = "……いや、悪い予感というべきか。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			bgName = "bg_endingsong_1",
			dir = 1,
			say = "艦船たちの努力を否定する気はさらさらない。だがなぜ上層部はこの状況を野放しにしているのか？",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			bgName = "bg_endingsong_1",
			dir = 1,
			say = "確かに、セイレーンへの直接の研究や接触は危険である。今までの経験がそれを証明している。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			bgName = "bg_endingsong_1",
			dir = 1,
			say = "しかし、もしセイレーンの手がかりへの接触が、上層部も含めて誰かに誘導されていたりしたら――",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actorName = "ジョージア",
			side = 2,
			bgName = "bg_endingsong_1",
			nameColor = "#a9f548",
			dir = 1,
			say = "指揮官、大丈夫か？",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			bgName = "bg_endingsong_1",
			dir = 1,
			say = "「再現」作戦のために用意された指揮艦の会議室で、ユニオン所属の特別計画艦「ジョージア」の声が響いた。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			options = {
				{
					content = "大丈夫だと答える",
					flag = 1
				},
				{
					content = "ブリーフィングを続けさせる",
					flag = 2
				}
			}
		},
		{
			actorName = "ジョージア",
			side = 2,
			bgName = "bg_endingsong_1",
			dir = 1,
			optionFlag = 1,
			nameColor = "#a9f548",
			say = "急に指揮官が黙り込んだから、つい気になってな",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			bgName = "bg_endingsong_1",
			dir = 1,
			optionFlag = 1,
			say = "確かに途中から別のことを考えていた。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actorName = "ジョージア",
			side = 2,
			bgName = "bg_endingsong_1",
			nameColor = "#a9f548",
			dir = 1,
			say = "コホン。ならいいけど",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actorName = "エルドリッジ",
			side = 2,
			bgName = "bg_endingsong_1",
			nameColor = "#a9f548",
			dir = 1,
			say = "指揮官、疲れてる？おねんね、する？",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actorName = "エルドリッジ",
			side = 2,
			bgName = "bg_endingsong_1",
			nameColor = "#a9f548",
			dir = 1,
			say = "エルドリッジ、クッション持ってくる",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			bgName = "bg_endingsong_2",
			dir = 1,
			say = "ブリーフィング中にもちろん寝てはダメだ。エルドリッジの気遣いに感謝した。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			bgName = "bg_endingsong_2",
			dir = 1,
			say = "気を取り直してジョージアに今の状況をもう一度説明してもらうことにした。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actorName = "ジョージア",
			side = 2,
			bgName = "bg_endingsong_2",
			nameColor = "#a9f548",
			dir = 1,
			say = "了解だ",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actorName = "ジョージア",
			side = 2,
			bgName = "bg_endingsong_2",
			nameColor = "#a9f548",
			dir = 1,
			say = "見ての通り、「再現」に参加する各陣営の艦隊がまだ集結している途中なんだが…",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			bgName = "bg_endingsong_2",
			dir = 1,
			say = "モニターにある、各艦隊の位置を示すアイコンを次々と指すジョージア。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			bgName = "bg_endingsong_2",
			dir = 1,
			say = "本作戦の主戦力で発起人でもある北方連合、そして支援するユニオン艦隊――",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actorName = "ジョージア",
			side = 2,
			bgName = "bg_endingsong_2",
			nameColor = "#a9f548",
			dir = 1,
			say = "ユニオン艦隊の本隊、つまり指揮官が今いるのはここだ。そして両脇と後ろには機動艦隊と予備艦隊…",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actorName = "ジョージア",
			side = 2,
			bgName = "bg_endingsong_2",
			nameColor = "#a9f548",
			dir = 1,
			say = "これらの戦力は今回の「再現」に参加するユニオン戦力の全てだ。だが…",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actorName = "ジョージア",
			side = 2,
			bgName = "bg_endingsong_2",
			nameColor = "#a9f548",
			dir = 1,
			say = "ニュージャージーが率いる増援もここに向かってきているところだ。セイレーンが何かしてくることもありえるからな",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actorName = "ジョージア",
			side = 2,
			bgName = "bg_endingsong_2",
			nameColor = "#a9f548",
			dir = 1,
			say = "ここからはまず北方連合のソビエツカヤ・ロシアが率いる極海艦隊の戦力と合流する",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			bgName = "bg_endingsong_2",
			dir = 1,
			say = "一緒に氷山要塞を攻略し、上位個体オミッターを撃退した仲間だ。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actorName = "ジョージア",
			side = 2,
			bgName = "bg_endingsong_2",
			dir = 1,
			nameColor = "#a9f548",
			say = "極地のセイレーンの動向も心配だから、可能な限り早めに決着をつけたいところだ",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			options = {
				{
					content = "ロイヤル艦隊の動向を聞く",
					flag = 1
				},
				{
					content = "重桜艦隊の動向を聞く",
					flag = 2
				},
				{
					content = "アイリス艦隊の動向を聞く",
					flag = 3
				},
				{
					content = "ヴィシア艦隊の動向を聞く",
					flag = 4
				},
				{
					content = "サディア艦隊の動向を聞く",
					flag = 5
				},
				{
					content = "東煌艦隊の動向を聞く",
					flag = 6
				}
			}
		},
		{
			actorName = "ジョージア",
			side = 2,
			bgName = "bg_endingsong_2",
			dir = 1,
			optionFlag = 1,
			nameColor = "#a9f548",
			say = "我々がNA海域から割いた戦力の穴埋めをしてくれているよ",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actorName = "ジョージア",
			side = 2,
			bgName = "bg_endingsong_2",
			dir = 1,
			optionFlag = 1,
			nameColor = "#a9f548",
			say = "スカパ・フロー沖のことは……現状特に情報がない。そちらに関しては私より指揮官がもっと知っているだろう",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actorName = "ジョージア",
			side = 2,
			bgName = "bg_endingsong_2",
			dir = 1,
			optionFlag = 1,
			nameColor = "#a9f548",
			say = "いずれにせよ、今回の「再現」でロイヤルの出番はないな",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actorName = "ジョージア",
			side = 2,
			bgName = "bg_endingsong_2",
			dir = 1,
			optionFlag = 2,
			nameColor = "#a9f548",
			say = "そういえば対セイレーン作戦の折、赤城たちも作戦参加のためダカールに来ていたそうだ",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actorName = "ジョージア",
			side = 2,
			bgName = "bg_endingsong_2",
			dir = 1,
			optionFlag = 2,
			nameColor = "#a9f548",
			say = "今もそこにいると聞いているが、「再現」にわざわざ首を突っ込んでくるようなことはないだろう",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actorName = "ジョージア",
			side = 2,
			bgName = "bg_endingsong_2",
			dir = 1,
			optionFlag = 3,
			nameColor = "#a9f548",
			say = "アイリス艦隊は「聖堂」の確保後大きな動きはない。ロイヤルと一緒に行動している戦力もいるが",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actorName = "ジョージア",
			side = 2,
			bgName = "bg_endingsong_2",
			dir = 1,
			optionFlag = 3,
			nameColor = "#a9f548",
			say = "今回の「再現」に参加してくるようなことはないと思う",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actorName = "ジョージア",
			side = 2,
			bgName = "bg_endingsong_2",
			dir = 1,
			optionFlag = 4,
			nameColor = "#a9f548",
			say = "指揮官がジャン・バールを「再現」で救出して以来、特にトゥーロンから動きはないな",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actorName = "ジョージア",
			side = 2,
			bgName = "bg_endingsong_2",
			dir = 1,
			optionFlag = 4,
			nameColor = "#a9f548",
			say = "アルジェリーやガスコーニュ、「聖堂」で姿を表した子も大規模な配置変更はないようだ",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actorName = "ジョージア",
			side = 2,
			bgName = "bg_endingsong_2",
			dir = 1,
			optionFlag = 4,
			nameColor = "#a9f548",
			say = "今度機会があったらうまくリシュリュー…いや、アイリスと話をできればいいんだが…",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actorName = "ジョージア",
			side = 2,
			bgName = "bg_endingsong_2",
			dir = 1,
			optionFlag = 5,
			nameColor = "#a9f548",
			say = "帝都がセイレーンに急襲されて以来、上層部からもっぱら「守備を固めよ」との指示が出されているそうだ",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actorName = "ジョージア",
			side = 2,
			bgName = "bg_endingsong_2",
			dir = 1,
			optionFlag = 5,
			nameColor = "#a9f548",
			say = "聞けばあの襲撃には、鉄血艦隊のフリードリヒも加勢したらしいな……あっ、もちろん防衛側にだ",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actorName = "ジョージア",
			side = 2,
			bgName = "bg_endingsong_2",
			dir = 1,
			optionFlag = 5,
			nameColor = "#a9f548",
			say = "迷宮のほうもヴェネトたちが制圧したし、今のところ特に心配する必要はなさそうだ",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actorName = "ジョージア",
			side = 2,
			bgName = "bg_endingsong_2",
			dir = 1,
			optionFlag = 6,
			nameColor = "#a9f548",
			say = "東煌か？セイレーン作戦に協力してくれたが、その後はもう帰還したぞ",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actorName = "ジョージア",
			side = 2,
			bgName = "bg_endingsong_2",
			dir = 1,
			optionFlag = 6,
			nameColor = "#a9f548",
			say = "NA海域には戦力がいないし、今気にする必要はないだろう",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actorName = "ジョージア",
			side = 2,
			bgName = "bg_endingsong_2",
			nameColor = "#a9f548",
			dir = 1,
			say = "それで、肝心の鉄血についてだが……実はちょっと厄介なことになっていてな",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actorName = "ジョージア",
			side = 2,
			bgName = "bg_endingsong_2",
			nameColor = "#a9f548",
			dir = 1,
			say = "作戦海域に現れた鉄血艦隊の信号は、我々の知っている鉄血の全戦力の数倍もあった",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actorName = "ジョージア",
			side = 2,
			bgName = "bg_endingsong_2",
			nameColor = "#a9f548",
			dir = 1,
			say = "これはこの間ハウたちが遭遇したジャミング装置の力によるものだろう",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actorName = "ジョージア",
			side = 2,
			bgName = "bg_endingsong_2",
			nameColor = "#a9f548",
			dir = 1,
			say = "「再現」とはいえ、いつも凝ったマネが好きな連中だから、これだけでは何をしようとしているのか判断がつかない",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			bgName = "bg_endingsong_2",
			dir = 1,
			say = "………つまり、現時点では何もわからないわけか。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actorName = "ジョージア",
			side = 2,
			bgName = "bg_endingsong_2",
			nameColor = "#a9f548",
			dir = 1,
			say = "それにフリードリヒの動向もまだ掴めていない。キューブの情報を探っているらしい",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			bgName = "bg_endingsong_2",
			dir = 1,
			say = "ジョージアのブリーフィングで状況がだいぶ整理できた気がする。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actorName = "ジョージア",
			side = 2,
			bgName = "bg_endingsong_2",
			nameColor = "#a9f548",
			dir = 1,
			say = "どういたしまして。正直、最前線に配置されると思っていたがこうして指揮官の本陣で参謀のようなことをやらされるとは思ってもなかったな",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actorName = "ジョージア",
			side = 2,
			bgName = "bg_endingsong_2",
			nameColor = "#a9f548",
			dir = 1,
			say = "なに、別に不満はないぞ。…あ、強いて言えばこのブリーフィングルームに酸素コーラを置いてほしいくらいだ",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			bgName = "bg_endingsong_2",
			dir = 1,
			say = "戦況についてジョージアの考えを聞いてみた。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actorName = "ジョージア",
			side = 2,
			bgName = "bg_endingsong_2",
			nameColor = "#a9f548",
			dir = 1,
			say = "ああ、彼我の戦力差からすると、こちらの勝算は圧倒的に高い",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actorName = "ジョージア",
			side = 2,
			bgName = "bg_endingsong_2",
			nameColor = "#a9f548",
			dir = 1,
			say = "複数陣営による連携を加味しなくても、鉄血艦隊を押し切るだけの戦力をこちらは有している",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actorName = "ジョージア",
			side = 2,
			bgName = "bg_endingsong_2",
			nameColor = "#a9f548",
			dir = 1,
			say = "だが鉄血の配置といい、これだけの戦力の参加を要請するソユーズの作戦計画といい、不安要素はまだまだある",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			bgName = "bg_endingsong_2",
			dir = 1,
			say = "それも承知の上だ。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			bgName = "bg_endingsong_2",
			dir = 1,
			say = "ソユーズたちを信頼するなら…この戦いを正しい方向へと導くには、こうして作戦をうまく計画通り完遂することだ。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			bgName = "bg_endingsong_2",
			dir = 1,
			say = "それと――――",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actorName = "ジョージア",
			side = 2,
			bgName = "bg_endingsong_2",
			nameColor = "#a9f548",
			dir = 1,
			say = "ん？まだなにか気になることでもあるのか？",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			bgName = "bg_endingsong_2",
			dir = 1,
			say = "………エルドリッジがいつまでも戻ってこないのだが…",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actorName = "ジョージア",
			side = 2,
			bgName = "bg_endingsong_2",
			nameColor = "#a9f548",
			dir = 1,
			say = "……あ。それは大変だな。すぐ探してくる",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			bgName = "bg_story_task",
			side = 2,
			dir = 1,
			say = "エルドリッジはブリーフィングルームの外で寝ていた。",
			flashout = {
				dur = 1,
				black = true,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = true,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 101260,
			side = 2,
			bgName = "bg_story_task",
			nameColor = "#a9f548",
			dir = 1,
			say = "Zzzzzzz………",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 199020,
			side = 2,
			bgName = "bg_story_task",
			nameColor = "#a9f548",
			dir = 1,
			say = "クッションをそのまま枕にして寝ているな…風邪を引く前に部屋に運ぶとしよう",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 199020,
			side = 2,
			bgName = "bg_story_task",
			nameColor = "#a9f548",
			dir = 1,
			say = "指揮官も疲れているなら作戦開始まで一度休んだほうがいいぞ",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 199020,
			side = 2,
			bgName = "bg_story_task",
			nameColor = "#a9f548",
			dir = 1,
			say = "エルドリッジも気づいたことだ。私だって気づいていないはずがないだろう？ははは",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			bgName = "bg_story_task",
			dir = 1,
			say = "彼女の言う通り、こちらもだいぶ疲労が溜まっている。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			bgName = "bg_story_task",
			dir = 1,
			say = "一度部屋に戻って仮眠を取ったほうが良さそうだ――",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			stopbgm = true,
			mode = 1,
			flashout = {
				dur = 1,
				black = true,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = true,
				alpha = {
					1,
					0
				}
			},
			sequence = {
				{
					"<size=51>鉄血の奇襲を受けたロイヤルの重要拠点スカパ・フロー</size>",
					1
				},
				{
					"<size=51>一度は両陣営の関係が緊張化するとも思われていたが</size>",
					2
				},
				{
					"<size=51>セイレーン作戦の開始後、妙な静けさを見せている</size>",
					3
				},
				{
					"<size=51>――ビスマルクの計らい事か</size> ",
					4
				},
				{
					"<size=51>――はたまた小さな女王陛下の策謀か</size>",
					5
				},
				{
					"<size=51>有耶無耶のまま時間が刻一刻と過ぎていく</size>",
					6
				},
				{
					"<size=51>しかしそれは不安定の事態が永遠に続くことを意味しない</size>",
					7
				},
				{
					"<size=51>施設の解析、特異点の調査、そして指揮官との調整</size>",
					8
				},
				{
					"<size=51>その準備活動が地道ながらも進められている</size>",
					9
				},
				{
					"<size=51>そして今、瑠璃の特異点がついに開かれる――</size>",
					10
				}
			}
		},
		{
			stopbgm = true,
			mode = 1,
			blackBg = true,
			flashout = {
				dur = 1,
				black = true,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = true,
				alpha = {
					1,
					0
				}
			},
			effects = {
				{
					active = true,
					name = "HONGCAIDEZHONGMUQU"
				}
			},
			sequence = {
				{
					"",
					1
				}
			}
		}
	}
}
