...
slot0.InitDragEvent = function(slot0)
	if slot0.controlLayer:GetComponent(typeof(SlideController)) and not IsNil(slot1) then
		slot1:ClearEvents()

		slot1.enabled = false
	end

	slot2 = GetOrAddComponent(slot0.controlLayer, typeof(SlideControllerHotfix))

	slot2:ClearEvents()

	slot0.ikSlideController = slot2

	slot2:AddPointDownFunc(function (slot0, slot1)
		uv0:emit(RoomIKSystem.ON_CONTROL_POINTER_DOWN, slot1.position)
	end)
	slot2:AddPointUpFunc(function (slot0, slot1)
		uv0:emit(RoomIKSystem.ON_CONTROL_POINTER_UP, slot1.position)
	end)
	slot2:AddBeginDragFunc(function (slot0, slot1)
		uv0:emit(RoomIKSystem.ON_CONTROL_BEGIN_DRAG, slot1.position)
	end)
	slot2:AddDragFunc(function (slot0, slot1)
		uv0:emit(RoomIKSystem.ON_CONTROL_DRAG, slot1.position, slot1.delta)
	end)
	slot2:AddDragEndFunc(function (slot0, slot1)
		uv0:emit(RoomIKSystem.ON_CONTROL_END_DRAG, slot1.position)
	end)
end
...
slot0.InitEvents = function(slot0)
	...
	slot0:bind(uv0.UPDATE_TIPS, function (slot0, slot1, slot2, slot3)
		uv0:UpdateTips(slot1, slot2, slot3)
	end)
...
slot0.UpdateTextTips = function(slot0, slot1)
	...
	_.each(slot1 or {}, function (slot0)
		if not IsNil(uv0.ikTextTipsRoot:Find(slot0)) then
...
slot0.GetTipLocalPosition = function(slot0, slot1, slot2)
	if not slot2.active then
		return Vector2.zero
	end

	assert(slot2.screenPosition, "Missing active IK tip screen position")

	return LuaHelper.ScreenToLocal(slot1, slot2.screenPosition, pg.UIMgr.GetInstance().uiCameraComp) + (slot2.offset or Vector2.zero)
end

slot0.GetIKTipRotation = function(slot0, slot1, slot2)
	slot4 = Vector2.zero

	if slot1:PointToNormalized(Vector2.zero).x < 0.5 and slot3.y < 0.5 then
		slot4 = slot1.max
	elseif slot3.x >= 0.5 and slot3.y < 0.5 then
		slot4 = Vector2.New(slot1.xMin, slot1.yMax)
	elseif slot3.x < 0.5 and slot3.y >= 0.5 then
		slot4 = Vector2.New(slot1.xMax, slot1.yMin)
	elseif slot3.x >= 0.5 and slot3.y >= 0.5 then
		slot4 = slot1.min
	end
	...
	slot5 = slot4 - slot1.center

	return Quaternion.LookRotation(Vector3.forward, Vector3.New(slot5.x, slot5.y, 0))
end

slot0.UpdateTouchTips = function(slot0, slot1)
	UIItemList.StaticAlign(slot0.ikTouchTipsRoot, slot0.ikTouchTipTpl, #(slot1 or {}), function (slot0, slot1, slot2)
		...
		uv1:SetTouchTipType(slot2, slot3.triggerType)
		setLocalPosition(slot2, uv1:GetTipLocalPosition(uv1.ikTouchTipsRoot, slot3))
		setActive(slot2, slot3.active)
	end)
end

slot0.UpdateIKTips = function(slot0, slot1)
	UIItemList.StaticAlign(slot0.ikTipsRoot, slot0.ikTipsRoot:GetChild(0), #(slot1 or {}), function (slot0, slot1, slot2)
		if slot0 ~= UIItemList.EventUpdate then
			return
		end

		slot3 = uv0[slot1 + 1]
		slot4 = uv1:GetTipLocalPosition(uv1.ikTipsRoot, slot3)

		setLocalPosition(slot2, slot4)
		setLocalRotation(slot2, slot3.active and uv1:GetIKTipRotation(slot3.triggerRect, slot4) or Quaternion.identity)
		setActive(slot2, slot3.active)
	end)
end
...
slot0.UpdateTips = function(slot0, slot1, slot2, slot3)
	if slot1 then
		slot0:UpdateIKTips(slot2 or {})
		slot0:UpdateTouchTips(slot3 or {})
	end
