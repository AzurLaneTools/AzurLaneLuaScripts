...
slot0.didEnter = function(slot0)
	...
	onButton(slot0, slot0:findTF("top/btn_home"), function ()
		uv0:emit(uv1.ON_HOME)
	end)
	onButton(slot0, slot0:findTF("top/info/btn_help"), function ()
		pg.MsgboxMgr.GetInstance():ShowMsgBox({
			type = MSGBOX_TYPE_HELP,
			helps = pg.gametip["0815_main_help"].tip
		})
	end, SFX_PANEL)
	onButton(slot0, slot0:findTF("btn/btn_game"), function ()
		uv0:emit(CowboyTownMediator.MINI_GAME)
	end)
	onButton(slot0, slot0:findTF("btn/btn_skin"), function ()
		uv0:emit(CowboyTownMediator.SKIN)
	end)
	onButton(slot0, slot0:findTF("btn/btn_expansion"), function ()
		uv0:emit(CowboyTownMediator.EXPANSION)
	end)
	onButton(slot0, slot0:findTF("btn/btn_task"), function ()
		uv0:emit(CowboyTownMediator.TASK)
	end)
	onButton(slot0, slot0:findTF("btn/btn_story"), function ()
...
slot0.UpdateView = function(slot0)
	setActive(slot0:findTF("btn/btn_game/tip"), uv0.MiniGameTip())
	setActive(slot0:findTF("btn/btn_expansion/tip"), uv0.ExpansionTips())
...
slot0.UpdateStoryView = function(slot0)
	setActive(slot0:findTF("btn/btn_story/tip"), uv0.StoryTips())
end
...
slot0.UpdateTaskTips = function(slot0)
	setActive(slot0:findTF("btn/btn_task/tip"), SixYearUsTaskMediator.GetTaskRedTip())
end
