slot0 = class("NieRAutomataAwardPage", import("view.activity.CorePage.CoreActivityPage"))
...
slot0.InitData = function(slot0)
	for slot4, slot5 in ipairs(slot0.table_Top) do
		setText(slot5:Find("on/Text"), i18n(slot0.table_local[slot4]))
		setText(slot5:Find("off/Text"), i18n(slot0.table_local[slot4]))
		onToggle(slot0, slot5, function (slot0)
			if slot0 then
				...
				uv0:DataList(uv1)
				setActive(uv0.table_Top[uv1]:Find("off"), false)
			else
				setActive(uv0.table_Top[uv1]:Find("off"), true)
			end
		end, SFX_PANEL)
	end
end
...
slot0.UpdateView = function(slot0)
	for slot4 = 1, #slot0.table_Top do
		setText(slot0.table_Top[slot4]:Find("on/label"), slot0:OnGetCount(slot4) .. "/" .. slot0:OnCount(slot4))
		setText(slot0.table_Top[slot4]:Find("off/label"), slot0:OnGetCount(slot4) .. "/" .. slot0:OnCount(slot4))
	end
...
slot0.IsShowingPopWindow = function(slot0)
	return slot0.boxOpen == true
end

slot0.ClosePopWindow = function(slot0)
	if slot0.boxOpen then
