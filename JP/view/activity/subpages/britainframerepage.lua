slot0 = class("BritainframeRePage", import(".TemplatePage.PtTemplatePage"))
...
function slot0.OnFirstFlush(slot0)
	...
	slot0.step = slot0:findTF("background/step")
	slot0.progress = slot0:findTF("background/progress")
	slot0.bar = slot0:findTF("background/barContent/bar")
...
function slot0.Switch(slot0, slot1)
	...
	LeanTween.moveLocal(go(slot0.phases[1]), slot0.phases[2].localPosition, 0.4)
	LeanTween.value(go(slot0.phases[1]), 0, 1, 0.4):setOnUpdate(System.Action_float(function (slot0)
		uv0.alpha = slot0
	end))
	setActive(slot0.phases[2]:Find("background"), true)
...
function slot0.UpdateAwardGot(slot0)
	...
	setActive(slot0:findTF("background/got"), slot2)
