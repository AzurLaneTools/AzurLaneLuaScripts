return {
	id = "YOUYINGMICHENG11",
	mode = 2,
	fadeOut = 1.5,
	scripts = {
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_452",
			factiontag = "幽影",
			dir = 1,
			nameColor = "#A9F548FF",
			bgm = "story-darkplan",
			actor = 9600033,
			NextIcon = 1,
			say = "她叫狮，即使在幽影之中，也是最危险的一类。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				dur = 1,
				black = true,
				alpha = {
					0,
					1
				}
			},
			effects = {
				{
					active = true,
					name = "memoryFog"
				}
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_452",
			factiontag = "幽影",
			dir = 1,
			actor = 9600033,
			NextIcon = 1,
			hidePaintObj = true,
			say = "喜欢狩猎，喜欢恐惧……特意将你的员工丢到市政厅转移嫌疑制造矛盾，再袭击我，削弱我的力量。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 7,
			side = 2,
			bgName = "star_level_bg_452",
			factiontag = "新手行动员",
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 231211,
			NextIcon = 1,
			hidePaintObj = true,
			say = "好可怕的策略……",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_452",
			factiontag = "幽影",
			dir = 1,
			actor = 9600033,
			NextIcon = 1,
			hidePaintObj = true,
			say = "但不管是狮，还是袭击收容区的其它幽影……真正听命的，都是是另一个更危险的存在。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			portrait = "zhihuiguan",
			side = 2,
			bgName = "star_level_bg_452",
			nameColor = "#A9F548FF",
			actor = 0,
			NextIcon = 1,
			hidePaintObj = true,
			say = "那人是谁？",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 9,
			side = 2,
			bgName = "star_level_bg_452",
			factiontag = "幽影",
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 9600033,
			NextIcon = 1,
			hidePaintObj = true,
			say = "……{namecode:315:安土}。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			NextIcon = 1,
			side = 2,
			bgName = "star_level_bg_452",
			hidePaintObj = true,
			nameColor = "#A9F548FF",
			say = "说出这个名字后，金鹿号脸上的笑意淡了一点。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 9,
			side = 2,
			bgName = "star_level_bg_452",
			factiontag = "幽影",
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 9600033,
			NextIcon = 1,
			hidePaintObj = true,
			say = "{namecode:315:安土}不喜欢人类，也讨厌选择亲近人类的我们。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			portrait = "zhihuiguan",
			side = 2,
			bgName = "star_level_bg_452",
			nameColor = "#A9F548FF",
			actor = 0,
			NextIcon = 1,
			hidePaintObj = true,
			say = "亲近人类？",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_452",
			factiontag = "幽影",
			dir = 1,
			actor = 9600033,
			NextIcon = 1,
			hidePaintObj = true,
			say = "没错。其实，幽影会被人类的情绪吸引的哦。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 6,
			side = 2,
			bgName = "star_level_bg_452",
			factiontag = "幽影",
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 403143,
			NextIcon = 1,
			hidePaintObj = true,
			say = "恐惧、孤独、执念……这些东西对我们来说，都无比闪耀~",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_452",
			factiontag = "幽影",
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 9600033,
			NextIcon = 1,
			hidePaintObj = true,
			say = "所以我才会认为，你们是可能改变世界的种子。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_452",
			factiontag = "幽影",
			dir = 1,
			actor = 9600033,
			NextIcon = 1,
			hidePaintObj = true,
			say = "你们能理解我们的话语，不会把我们当做必须消灭的怪物。对我来说，确实是最佳的合作对象。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			portrait = "zhihuiguan",
			side = 2,
			bgName = "star_level_bg_452",
			nameColor = "#A9F548FF",
			actor = 0,
			NextIcon = 1,
			hidePaintObj = true,
			say = "那作为你的合作对象，我想了解那个{namecode:315:安土}接下来还会做什么？",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_452",
			factiontag = "幽影",
			dir = 1,
			actor = 9600033,
			NextIcon = 1,
			hidePaintObj = true,
			say = "指挥官，你应该很快就会明白。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_452",
			factiontag = "幽影",
			dir = 1,
			actor = 9600033,
			NextIcon = 1,
			hidePaintObj = true,
			say = "幸福都市计划包含新纪元七座城市，NO.7只是其中之一。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 9,
			side = 2,
			bgName = "star_level_bg_452",
			factiontag = "幽影",
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 9600033,
			NextIcon = 1,
			hidePaintObj = true,
			say = "{namecode:315:安土}真正想要毁灭的，是“幸福的新纪元”这个概念本身。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_452",
			factiontag = "幽影",
			dir = 1,
			actor = 9600033,
			NextIcon = 1,
			hidePaintObj = true,
			say = "……如果您相信我的说法，就请按照这个方向继续调查吧。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			portrait = "zhihuiguan",
			side = 2,
			bgName = "star_level_bg_452",
			nameColor = "#A9F548FF",
			actor = 0,
			NextIcon = 1,
			hidePaintObj = true,
			say = "逻辑上解释得通，所以我相信。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			dir = 1,
			bgName = "star_level_bg_662",
			bgm = "battle-visioncity-1",
			actor = 103090,
			NextIcon = 1,
			hidePaintObj = true,
			say = "那……那个……Lady？",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				dur = 0.5,
				black = true,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				dur = 0.5,
				black = true,
				delay = 0.5,
				alpha = {
					1,
					0
				}
			},
			effects = {
				{
					active = false,
					name = "memoryFog"
				}
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_662",
			dir = 1,
			actor = 103090,
			NextIcon = 1,
			hidePaintObj = true,
			say = "急需处理的灾后重建文件送到了哦……",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			NextIcon = 1,
			side = 2,
			bgName = "star_level_bg_662",
			hidePaintObj = true,
			nameColor = "#A9F548FF",
			say = "代理市长的临时办公室内，昆西小声地呼唤着眼前似乎沉浸在自己世界中的金鹿号。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_662",
			factiontag = "幽影",
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 9600030,
			NextIcon = 1,
			hidePaintObj = true,
			say = "…………",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			NextIcon = 1,
			side = 2,
			bgName = "star_level_bg_662",
			hidePaintObj = true,
			nameColor = "#A9F548FF",
			say = "金鹿号回过神来，对昆西露出了淡淡的微笑。她没有说话，只是示意对方把文件放到自己手边。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_662",
			dir = 1,
			actor = 103090,
			NextIcon = 1,
			hidePaintObj = true,
			say = "（呜啊，好大的压迫感……Lady果然和传闻中一样人狠话不多……）",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "star_level_bg_662",
			dir = 1,
			actor = 103090,
			NextIcon = 1,
			hidePaintObj = true,
			say = "请、请您过目！那，我就先退下啦……",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			NextIcon = 1,
			side = 2,
			bgName = "star_level_bg_662",
			hidePaintObj = true,
			nameColor = "#A9F548FF",
			say = "金鹿号若有所思地将文件收纳好，随后看了眼时间，起身离开临时办公室。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			NextIcon = 1,
			side = 2,
			bgName = "bg_youyingmicheng_1",
			bgm = "story-dailyfuture-soft",
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "天亮之后，覆盖全城的异常污染终于退了。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				dur = 1,
				black = true,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				dur = 1,
				black = true,
				delay = 1,
				alpha = {
					1,
					0
				}
			},
			location = {
				"新纪元都市NO.7·入城口",
				3
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "bg_youyingmicheng_1",
			factiontag = "幽影",
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 202380,
			NextIcon = 1,
			hidePaintObj = true,
			say = "嗯……旋律重回和谐~",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_youyingmicheng_1",
			factiontag = "新手行动员",
			dir = 1,
			actor = 231211,
			NextIcon = 1,
			hidePaintObj = true,
			say = "能重新看到这么普通的街道，我都有点想哭了……",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			NextIcon = 1,
			side = 2,
			bgName = "bg_youyingmicheng_1",
			hidePaintObj = true,
			nameColor = "#A9F548FF",
			say = "市政厅的临时接待点已经搭建完成。因为昨晚的大规模恶性幽影伤人事件，新纪元都市NO.1也会派使者前来慰问。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_youyingmicheng_1",
			factiontag = "幽影",
			dir = 1,
			actor = 9600030,
			NextIcon = 1,
			hidePaintObj = true,
			say = "早上好，指挥官。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			NextIcon = 1,
			side = 2,
			bgName = "bg_youyingmicheng_1",
			hidePaintObj = true,
			nameColor = "#A9F548FF",
			say = "金鹿号从市政厅的接待车上下来，微笑着打了个招呼。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			say = "切，现在胆子这么大，敢在这么多人面前说话了？不怕其它人发现你是幽影？",
			side = 2,
			bgName = "bg_youyingmicheng_1",
			factiontag = "幽影行动员",
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 900557,
			NextIcon = 1,
			hidePaintObj = true,
			actorPosition = {
				x = -200,
				y = 0
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_youyingmicheng_1",
			factiontag = "幽影",
			dir = 1,
			nameColor = "#A9F548FF",
			actor = 9600030,
			NextIcon = 1,
			hidePaintObj = true,
			say = "这不是相信虎小姐的力量，能帮我打好掩护吗~",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			NextIcon = 1,
			side = 2,
			bgName = "bg_youyingmicheng_1",
			hidePaintObj = true,
			nameColor = "#A9F548FF",
			say = "就在此时，入城口外突然出现了一队黑色的车队，旗帜上印着新纪元都市NO.1的徽记。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			NextIcon = 1,
			side = 2,
			bgName = "bg_youyingmicheng_1",
			hidePaintObj = true,
			nameColor = "#A9F548FF",
			say = "那是诞生了“幸福都市计划”的城市，也是所有新纪元都市的中枢。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			nameColor = "#A9F548FF",
			side = 2,
			bgName = "bg_youyingmicheng_1",
			factiontag = "幽影",
			dir = 1,
			actor = 9600030,
			NextIcon = 1,
			hidePaintObj = true,
			say = "NO.1的特使来了呢。指挥官，接下来就是你们的高光时刻了哦。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
