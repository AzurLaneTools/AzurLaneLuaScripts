slot0 = class("Dorm3dStockingMgr", import("view.dorm3d.Core.BaseSystem"))
...
slot0.OnInit = function(slot0)
	if slot0:GetCurrentLadyEnv() then
		for slot5, slot6 in pairs(slot1.skinIdList) do
			slot0:InitDormStocking(slot0:GetSkinDict()[slot6].ladyGameObject.transform, slot6)
		end
	end
end
...
slot0.OnHandleNotification = function(slot0, slot1, slot2)
	if slot1 == GAME.APARTMENT_REPLACE_FURNITURE_DONE then
		...
		for slot7, slot8 in pairs(slot3.skinIdList) do
			slot0:InitDormStocking(slot0:GetSkinDict()[slot8].ladyGameObject.transform, slot8)
		end
	end
end
...
slot0.OnBeginDrag = function(slot0, slot1, slot2)
	...
	if #CameraMgr.instance:Raycast(slot0:GetSceneRaycaster(), slot2.position):ToTable() > 0 then
...
slot0.TriggerEvent = function(slot0, slot1, slot2, slot3)
	...
	switch(slot1, {
		function ()
			uv0:Emit(Dorm3dRoomScene.EXTRA_DO_TALK, uv1[1], uv2)
		end
	})
end
...
slot0.ResetLady = function(slot0)
	slot1 = slot0:GetIKPointByName(uv0.character_position)
...
slot0.SetStockingStatus = function(slot0, slot1)
	...
	seriesAsync({
		function (slot0)
			uv0:Emit(RoomIKSystem.ENTER_STOCKING_IK_MODE, slot0)
		end,
		function (slot0)
			uv0:Emit(Dorm3dRoomScene.EXTRA_SET_UI, slot0, "stocking")
		end,
		function (slot0)
			uv0:ResetLady()
			uv0:Emit(Dorm3dRoomTemplateScene.EXTRA_ACTIVE_CAMERA_BY_NAME, uv1.camera)

			if uv0:GetCameraRoot():Find(uv1.camera):GetComponent(typeof(Cinemachine.CinemachineFreeLook)) then
				uv0:Emit(Dorm3dRoomTemplateScene.EXTRA_REGISTER_ORBITS, slot2)
			end

			slot3 = uv0

			slot3:Emit(Dorm3dRoomTemplateScene.EXTRA_PLAY_ENTER_SCENE_ANIM, uv1.enter_scene_anim)

			slot3 = uv0

			slot3:Emit(Dorm3dRoomTemplateScene.EXTRA_PLAY_ENTER_EXTRA_ITEM, uv0.ladyEnv, uv1.enter_extra_item)

			slot3 = uv0

			slot3:Emit(Dorm3dRoomTemplateScene.EXTRA_HIDE_SCENE_ITEM, uv1.hide_scene_item)
...
slot0.ExitStockingStatus = function(slot0)
	seriesAsync({
		...
		function (slot0)
			...
			uv0:Emit(Dorm3dRoomTemplateScene.EXTRA_RESET_SCENE_ITEM_ANIMATORS)
			uv0:Emit(Dorm3dRoomTemplateScene.EXTRA_RESET_TEMP_HIDE_SCENE_ITEMS)
			uv0:Emit(Dorm3dRoomTemplateScene.EXTRA_REVERT_CAMERA_ORBIT)
			uv0:Emit(RoomIKSystem.EXIT_STOCKING_IK_MODE)
			slot0()
		end
	})
end
...
slot0.GetTipShowInfo = function(slot0, slot1)
	...
	for slot6, slot7 in ipairs(uv0.enable_drag) do
		if slot7 == 1 then
			...
			slot0:Emit(Dorm3dRoomTemplateScene.EXTRA_GET_SCREEN_POSITION, slot8, slot0.tipDirections[slot6][1].position, slot0.mainCamera)

			slot9 = {}

			slot0:Emit(Dorm3dRoomTemplateScene.EXTRA_GET_SCREEN_POSITION, slot9, slot0.tipDirections[slot6][2].position, slot0.mainCamera)

			slot10 = {}

			slot0:Emit(Dorm3dRoomTemplateScene.EXTRA_GET_SCREEN_POSITION, slot10, slot0.tiptransforms[slot6].position, slot0.mainCamera)
			table.insert(slot2, {
				pos = slot10.value,
				dir = slot9.value - slot8.value
			})
		end
	end
	...
	if slot0.useHideMode then
		for slot7, slot8 in ipairs(slot0.sceneStockingTFs) do
			if not slot0.isShow[slot7] then
				...
				slot0:Emit(Dorm3dRoomTemplateScene.EXTRA_GET_SCREEN_POSITION, slot9, slot8.position, slot0.mainCamera)
				table.insert(slot3, {
					pos = slot9.value
				})
			end
