...
slot0.initUI = function(slot0)
	...
	slot2 = slot0.mainUI:Find("item_tpl")
	slot0.itemList = {}
	...
	for slot7 = 1, #pg.mini_game[slot0:GetMGData().id].simple_config_data.drop do
		...
		setParent(slot8, slot11:Find("item_list/Viewport/Content"))
		GetSpriteFromAtlasAsync("ui/minigameui/curlinggameui_atlas", "text_" .. slot7, function (slot0)
			setImageSprite(uv0:Find("bg/text"), slot0, true)
		end)
	...
	slot4 = slot0._tf
	slot0.countUI = slot4:Find("ui/count_ui")
	slot5 = slot0.countUI
	slot0.countAnimator = GetComponent(slot5:Find("count"), typeof(Animator))
	slot5 = slot0.countUI
	slot0.countDft = GetOrAddComponent(slot5:Find("count"), typeof(DftAniEvent))
	...
	slot4 = slot0._tf
	slot0.pauseUI = slot4:Find("ui/pause_ui")
	slot6 = slot0.pauseUI

	onButton(slot0, slot6:Find("ad/panel/sure_btn"), function ()
		setActive(uv0.pauseUI, false)
		uv0:resumeGame()
	end, SFX_PANEL)

	slot4 = slot0._tf
	slot0.returnUI = slot4:Find("ui/return_ui")
	slot6 = slot0.returnUI
	...
	onButton(slot0, slot6:Find("ad/panel/cancel_btn"), function ()
		setActive(uv0.returnUI, false)
		uv0:resumeGame()
	end, SFX_PANEL)

	slot4 = slot0._tf
	slot0.endUI = slot4:Find("ui/end_ui")
	slot6 = slot0.endUI

	onButton(slot0, slot6:Find("ad/panel/end_btn"), function ()
...
slot0.updateMainUI = function(slot0)
	...
	for slot6 = 1, #slot0.itemList do
		setActive(slot0.itemList[slot6]:Find("lock"), false)
		setActive(slot0.itemList[slot6]:Find("finish"), false)

		if slot6 <= slot1 then
			setActive(slot0.itemList[slot6]:Find("finish"), true)
		elseif slot6 == slot1 + 1 and slot2 >= 1 then
			-- Nothing
		elseif slot1 >= slot6 or slot6 > slot1 + slot2 then
			setActive(slot0.itemList[slot6]:Find("lock"), true)
		end
	end
