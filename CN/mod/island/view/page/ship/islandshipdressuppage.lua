slot0 = class("IslandShipDressUpPage", import("...base.IslandBasePage"))
...
slot0.SwitchPage = function(slot0, slot1)
	...
	if not slot0.hasInit then
		slot0.hasInit = true

		onNextTick(function ()
			uv0:SortDressListByQuality()
		end)
	else
		slot0:SortDressListByQuality()
	end
end

slot0.SortDressListByQuality = function(slot0)
	table.sort(slot0.dressList, function (slot0, slot1)
		slot4 = function()
			if uv0.dressSortType == uv1.Down_up then
				return uv2.quality < uv3.quality
			else
				return uv3.quality < uv2.quality
			end
		end

		if pg.island_dress_template[slot0].quality ~= pg.island_dress_template[slot1].quality then
			return slot4()
		end
...
slot0.InitCharacter = function(slot0)
	...
	slot0:LoadCharacter(pg.island_ship[0].model, slot5)

	for slot5, slot6 in pairs(slot0.initDressData) do
		slot7 = slot0.pageDressDic[slot5] or {}
		slot7.currentItemId = slot6
		slot0.pageDressDic[slot5] = slot7
	end
end
...
slot0.OnShow = function(slot0)
	slot3 = function()
		setActive(uv0, uv1.dressSortType == uv2.Down_up)
		setActive(uv3, uv1.dressSortType == uv2.Up_Down)
	end

	onButton(slot0, slot0.rightPanel:Find("filtrate/filtrate_sort/tag_down"), function ()
		uv0.dressSortType = uv1.Up_Down

		uv2()
		uv0:SortDressListByQuality()
	end)
	onButton(slot0, slot0.rightPanel:Find("filtrate/filtrate_sort/tag_up"), function ()
		uv0.dressSortType = uv1.Down_up
	...
	slot0.dressSortType = uv0.Down_up

	slot0:GetInitDressData()
	slot0:InitCharacter()
	triggerToggle(slot0.toggles[uv1.Footprint], true)
	slot0:Flush()
end
...
slot0.SaveDressUpData = function(slot0)
	getProxy(IslandProxy):GetIsland():GetVisitorAgency():ChangeDress(slot0.pageDressDic)
end

slot0.CheckDressIsDirty = function(slot0)
	slot2 = getProxy(IslandProxy):GetIsland():GetVisitorAgency():GetPlayerDressData()

	for slot6, slot7 in pairs(slot0.pageDressDic) do
		if slot2[slot6] ~= slot7.currentItemId then
			return true
		end
	end
