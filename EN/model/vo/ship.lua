...
function slot0.getShipWords(slot0)
	...
	for slot6, slot7 in pairs(Clone(slot1)) do
...
function slot0.getMainwordsCount(slot0)
	if not uv0.getShipWords(slot0).main or slot1.main == "" then
		slot1 = uv0.getShipWords(uv0.getOriginalSkinId(slot0))
	end
...
function slot0.getWordsEx(slot0, slot1, slot2, slot3, slot4, slot5)
	...
	if (not slot0 or not slot0[slot1]) and not nil or slot6 == "" then
		if slot0 and slot0.id == slot4 then
			return
		end
		...
		slot8, slot9 = uv0.getShipWords(slot4)

		if not slot9 then
			return
		end

		if not slot9[slot1] then
			return
		end
...
function slot0.getWords(slot0, slot1, slot2, slot3, slot4)
	slot5, slot6 = uv0.getShipWords(slot0)
	slot8 = math.fmod(slot0, uv0.getOriginalSkinId(slot0))

	if not slot5 then
		slot9, slot6 = uv0.getShipWords(slot7)
	...
	if not slot5[slot1] or slot11 == "" then
		...
		if slot5.id == slot7 then
			...
			if not uv0.getShipWords(slot7) then
				return nil
			end
	...
	rstEx, cvEx, defaultCoverEx = uv0.getWordsEx(slot6, slot1, slot13, slot4, slot7, slot10)
	slot14 = nil

	if (PlayerPrefs.GetInt("CV_LANGUAGE_" .. pg.ship_skin_template[slot0].ship_group) == 2 and slot5.voice_key_2 or slot5.voice_key) == 0 then
		if not slot10 or rstEx and not defaultCoverEx then
			slot14 = uv0.getCVPath(slot7, slot1, slot13, slot8)
		end
	elseif slot16 ~= -2 then
		slot14 = uv0.getCVPath(slot7, slot1, slot13)
	end
	...
	if rstEx then
		slot14 = slot14 and slot14 .. "_ex" .. cvEx
	end
...
function slot0.getCVKeyID(slot0)
	...
	if (PlayerPrefs.GetInt("CV_LANGUAGE_" .. pg.ship_skin_template[slot0].ship_group) == 2 and slot1.voice_key_2 >= 0 and slot1.voice_key_2 or slot1.voice_key) == 0 or nil == -2 then
		slot5 = uv0.getShipWords(uv0.getOriginalSkinId(slot0))
		slot2 = slot3 == 2 and slot5.voice_key_2 >= 0 and slot5.voice_key_2 or slot5.voice_key
	end

	return slot2
end

function slot0.getCVPath(slot0, slot1, slot2, slot3)
	slot2 = slot2 or 1

	if not Ship.getShipWords(slot0) and not uv0.getShipWords(uv0.getOriginalSkinId(slot0)) then
		return
	end
	...
	if (slot6 == 2 and slot4.voice_key_2 or slot4.voice_key) ~= -1 and pg.character_voice[slot1] and pg.character_voice[slot1].resource_key then
		if slot3 then
			slot8 = "event:/cv/" .. slot9 .. "/" .. slot8 .. "_" .. slot3
		end
	end
...
function slot0.getCVCalibrate(slot0, slot1, slot2)
...
function slot0.getOriginalSkinId(slot0)
