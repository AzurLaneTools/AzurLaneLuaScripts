...
function slot8(slot0, slot1)
	...
	slot12 = slot1.drop.type == DROP_TYPE_ITEM and (getProxy(TechnologyProxy):getItemCanUnlockBluePrint(slot1.drop.id) or slot1.drop.cfg.type == 11)

	setActive(slot0._sigleItemPanel:Find("detail"), slot12)
	...
	if slot12 then
		...
		if getProxy(TechnologyProxy):getItemCanUnlockBluePrint(slot1.drop.id) then
			slot13.content = i18n("techpackage_item_use_confirm")
			slot13.items = underscore.map(slot1.drop.cfg.display_icon, function (slot0)
				return {
					type = slot0[1],
					id = slot0[2],
					count = slot0[3]
			...
			setText(slot11, "<material=underline c=#A9F548 event=checkDetail args=1><color=#A9F548>" .. i18n("tech_select_tip4") .. "</color></material>")
		elseif slot1.drop.cfg.type == 11 then
			slot13.content = i18n("equip_skin_detail_tip")
			slot13.items = underscore.map(slot1.drop.cfg.display_icon, function (slot0)
				return {
					type = slot0[1],
					id = slot0[2]
			...
			setText(slot11, "<material=underline c=#A9F548 event=checkDetail args=1><color=#A9F548>" .. i18n("package_detail_tip") .. "</color></material>")
		end
