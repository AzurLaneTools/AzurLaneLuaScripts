...
slot0.init = function(slot0)
	...
	slot0.tfLimit = slot0._tf:Find("panel/limit")
	slot0.tfLimitTips = slot0._tf:Find("panel/limit_tip")
	slot0.tfLimitElite = slot0._tf:Find("panel/limit_elite")
	slot0.tfLimitContainer = slot0._tf:Find("panel/limit_elite/limit_list")
	slot0.tfLimitTpl = slot0._tf:Find("panel/limit_elite/condition")
	slot0.btnBack = slot0._tf:Find("panel/btnBack")
	slot0.btnGo = slot0._tf:Find("panel/start_button")
	slot0.btnAdHelp = slot0._tf:Find("panel/title/ADvalue/helpbtn")
	slot0.commanderBtn = slot0._tf:Find("panel/commander_btn")
...
slot0.flush = function(slot0)
	...
	if OPEN_AIR_DOMINANCE and slot0.chapterADValue > 0 then
		setActive(slot0._tf:Find("panel/title/ADvalue"), true)
		slot0:updateFleetPanelADValue()
	else
		setActive(slot0._tf:Find("panel/title/ADvalue"), false)
	end
	...
	if #slot0.propetyLimitation > 0 then
		...
		for slot6, slot7 in ipairs(slot0.propetyLimitation) do
			...
			if slot1[slot6] == 1 then
				slot12:Find("Text"):GetComponent(typeof(Text)).color = Color.New(1, 0.9607843137254902, 0.5019607843137255)
			else
				slot12:Find("Text"):GetComponent(typeof(Text)).color = Color.New(0.9568627450980393, 0.30196078431372547, 0.30196078431372547)
			end

			setActive(slot12, true)
			setText(slot12:Find("Text"), AttributeType.EliteCondition2Name(slot8, slot11) .. AttributeType.eliteConditionCompareTip(slot9) .. slot10 .. "（" .. slot2[slot8] .. "）")
		end
...
slot0.updateFleetPanelADValue = function(slot0)
	...
	slot3 = slot0._tf:Find("panel/title/ADvalue/Text")

	setText(slot3, i18n("level_scene_title_word_5"))
	setText(slot3:Find("Num1"), setColorStr(slot2, slot2 < slot0.suggestionValue and "#f1dc36" or COLOR_WHITE))
	setText(slot3:Find("Num2"), slot0.suggestionValue)
end
...
slot0.updateFleets = function(slot0)
	for slot4, slot5 in ipairs(slot0.tfFleets[FleetType.Normal]) do
		...
		setActive(slot5:Find("btn_clear"), slot11 and not slot0.contextData.EditingCommander)
		setActive(slot5:Find("btn_recom"), slot11 and not slot0.contextData.EditingCommander)
		setActive(slot5:Find("blank"), not slot11)
		setActive(slot5:Find("commander"), slot11 and slot0.contextData.EditingCommander)
		setText(slot5:Find("bg/name"), slot11 and Fleet.DEFAULT_NAME[slot4] or "")
	...
	for slot4, slot5 in ipairs(slot0.tfFleets[FleetType.Submarine]) do
		slot11 = slot5:Find("commander")

		setActive(slot5:Find("btn_select"), false)
		setActive(findTF(slot5, "selected"), false)
		setActive(findTF(slot5, TeamType.Submarine), slot4 <= slot0.chapter:getConfig("submarine_num"))
		setActive(slot5:Find("btn_clear"), slot4 <= slot0.chapter:getConfig("submarine_num") and not slot0.contextData.EditingCommander)
		setActive(slot5:Find("btn_recom"), slot4 <= slot0.chapter:getConfig("submarine_num") and not slot0.contextData.EditingCommander)
		setActive(slot5:Find("blank"), slot0.chapter:getConfig("submarine_num") < slot4)
		setActive(slot11, slot4 <= slot0.chapter:getConfig("submarine_num") and slot0.contextData.EditingCommander)
		setText(slot5:Find("bg/name"), slot4 <= slot0.chapter:getConfig("submarine_num") and Fleet.DEFAULT_NAME[Fleet.SUBMARINE_FLEET_ID + slot4 - 1] or "")
