slot0 = class("AprilFoolDiscovery2026Page", import("view.activity.CorePage.CoreActivityAprilFoolDiscoveryPage"))
slot1 = "magicbuli"
...
slot0.OnInit = function(slot0)
	...
	setText(slot0.slider:Find("timetext"), i18n("aprilfool_2026_cd"))
end
...
slot0.CreateCDTimer = function(slot0)
	...
	if #slot0.activity.data2_list == #slot0.keyList or slot0.activity.data2 <= pg.TimeMgr.GetInstance():GetServerTime() then
		...
		setActive(slot0.sliderbg, false)
		slot0:RemoveCDTimer()
	...
	for slot8, slot9 in ipairs(slot0.items) do
		...
		onButton(slot0, slot9, function ()
			...
			if uv0 == 2 then
				...
				return quickPlayAnimation(uv3:Find("Unlock"), "im_AprilFoolDiscovery2026Page_Unlock")
			elseif uv0 == 1 then
		...
		if slot11 == 3 then
			quickPlayAnimation(slot9, "anim_AprilFoolDiscovery2026Page_open")
		end

		setActive(slot9:Find("Selected"), slot11 == 2)

		if slot11 == 2 then
			setActive(slot9:Find("Unlock"), true)
		end
...
slot0.UpdateSelection = function(slot0)
	SetActive(slot0.tip, table.contains(slot0.activity.data3_list, slot0.keyList[slot0.selectIndex]) and i18n("SuperBulin2_tip" .. slot0.selectIndex) or false)
	SetActive(slot0.tip_bg, slot2 and i18n("SuperBulin2_tip" .. slot0.selectIndex) or false)
	setText(slot0.tip, slot2 and i18n("SuperBulin2_tip" .. slot0.selectIndex) or "")
	slot0:CreateCDTimer()
end
