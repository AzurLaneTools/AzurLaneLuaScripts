pg = pg or {}
pg.commander_skill_template = rawget(pg, "commander_skill_template") or setmetatable({
	__name = "commander_skill_template"
}, confNEO)
pg.commander_skill_template.all = {
	10011,
	10012,
	10013,
	10021,
	10022,
	10023,
	11011,
	11012,
	11013,
	11021,
	11022,
	11023,
	12011,
	12012,
	12013,
	12021,
	12022,
	12023,
	20011,
	20012,
	20013,
	20021,
	20022,
	20023,
	21011,
	21012,
	21013,
	21021,
	21022,
	21023,
	21031,
	21032,
	21033,
	21041,
	21042,
	21043,
	22011,
	22012,
	22013,
	22021,
	22022,
	22023,
	30011,
	30012,
	30013,
	30021,
	30022,
	30023,
	31011,
	31012,
	31013,
	31021,
	31022,
	31023,
	32011,
	32012,
	32013,
	32021,
	32022,
	32023,
	40011,
	40012,
	40013,
	40021,
	40022,
	40023,
	41011,
	41012,
	41013,
	41021,
	41022,
	41023,
	41031,
	41032,
	41033,
	42011,
	42012,
	42013,
	42021,
	42022,
	42023,
	960011,
	960012,
	960013,
	960021,
	960022,
	960023,
	960031,
	960032,
	960033,
	960041,
	960042,
	960043,
	960051,
	960052,
	960053,
	960061,
	960062,
	960063,
	960071,
	960072,
	960073
}
pg.base = pg.base or {}
pg.base.commander_skill_template = {}

(function ()
	pg.base.commander_skill_template[10011] = {
		name = "31節的正義",
		desc_world = "所在艦隊中有三艘驅逐時，依據指揮喵戰術屬性，提升所在艦隊中先鋒艦隊的雷擊屬性（效果中）",
		next_id = 10012,
		exp = 1,
		prev_id = 0,
		desc = "所在艦隊中有三艘驅逐時，地圖移動力+1",
		id = 10011,
		icon = "1001",
		lv = 1,
		effect_tactic = {
			100111
		},
		effect_tactic_world = {
			9100111
		}
	}
	pg.base.commander_skill_template[10012] = {
		name = "31節的正義",
		desc_world = "作為司令位指揮喵時，若先鋒艦隊中間位置為驅逐艦，進入戰鬥後發射一輪特殊彈幕I，每場戰鬥一次",
		next_id = 10013,
		exp = 3,
		prev_id = 10011,
		desc = "當作為司令位指揮喵，接觸非旗艦敵方單位時，若隊伍中有驅逐，則有15%機率發射一次魚雷打擊(地圖)",
		id = 10012,
		icon = "1001",
		lv = 2,
		effect_tactic = {
			100111,
			100112
		},
		effect_tactic_world = {
			9100111,
			9100112
		}
	}
	pg.base.commander_skill_template[10013] = {
		name = "31節的正義",
		desc_world = "特殊彈幕I 強化為 特殊彈幕II",
		next_id = 0,
		exp = 0,
		prev_id = 10012,
		desc = "作為司令位指揮喵，編隊中存在驅逐且未進入交戰狀態時，可選擇相鄰位置與敵方交戰中的己方艦隊，與之互換所在位置代替其與敵方交戰",
		id = 10013,
		icon = "1001",
		lv = 3,
		effect_tactic = {
			100111,
			100112,
			100113
		},
		effect_tactic_world = {
			9100111,
			9100113
		}
	}
	pg.base.commander_skill_template[10021] = {
		name = "王牌射手",
		desc_world = "與主力艦隊或偵查艦隊交戰時，依據指揮喵戰術屬性，提高所在艦隊中戰列、戰巡、航戰的命中屬性(效果大)",
		next_id = 10022,
		exp = 1,
		prev_id = 0,
		desc = "與主力艦隊或偵查艦隊交戰時，依據指揮喵戰術屬性，提高所在艦隊中戰列、戰巡、航戰的命中屬性(效果大)",
		id = 10021,
		icon = "1002",
		lv = 1,
		effect_tactic = {
			100211
		},
		effect_tactic_world = {
			100211
		}
	}
	pg.base.commander_skill_template[10022] = {
		name = "王牌射手",
		desc_world = "若所在艦隊旗艦為戰列、戰巡或航戰，依據指揮喵指揮屬性，提高旗艦造成的傷害",
		next_id = 10023,
		exp = 3,
		prev_id = 10021,
		desc = "若所在艦隊旗艦為戰列、戰巡或航戰，依據指揮喵指揮屬性，提高旗艦造成的傷害",
		id = 10022,
		icon = "1002",
		lv = 2,
		effect_tactic = {
			100211,
			100212
		},
		effect_tactic_world = {
			100211,
			100212
		}
	}
	pg.base.commander_skill_template[10023] = {
		name = "王牌射手",
		desc_world = "依據指揮喵指揮屬性，提高所在艦隊中戰列、戰巡、航戰的機動、防空屬性(效果中)",
		next_id = 0,
		exp = 0,
		prev_id = 10022,
		desc = "依據指揮喵指揮屬性，提高所在艦隊中戰列、戰巡、航戰的機動、防空屬性(效果中)",
		id = 10023,
		icon = "1002",
		lv = 3,
		effect_tactic = {
			100211,
			100212,
			100213
		},
		effect_tactic_world = {
			100211,
			100212,
			100213
		}
	}
	pg.base.commander_skill_template[11011] = {
		name = "5英寸的勇氣",
		desc_world = "當作為司令位指揮喵，依據指揮喵指揮屬性，提高所在艦隊中驅逐的雷擊屬性(效果小)",
		next_id = 11012,
		exp = 1,
		prev_id = 0,
		desc = "當作為司令位指揮喵，接觸非旗艦敵方單位時，若隊伍中有驅逐，則有15%機率發射一次魚雷打擊(地圖)",
		id = 11011,
		icon = "1101",
		lv = 1,
		effect_tactic = {
			110111
		},
		effect_tactic_world = {
			9110111
		}
	}
	pg.base.commander_skill_template[11012] = {
		name = "5英寸的勇氣",
		desc_world = "先鋒領艦為驅逐時，戰鬥中靠近敵人時發射一輪特殊彈幕I，每場戰鬥一次",
		next_id = 11013,
		exp = 3,
		prev_id = 11011,
		desc = "先鋒領艦為驅逐時，戰鬥中靠近敵人時發射一輪特殊彈幕I，每場戰鬥一次",
		id = 11012,
		icon = "1101",
		lv = 2,
		effect_tactic = {
			110111,
			110112
		},
		effect_tactic_world = {
			9110111,
			110112
		}
	}
	pg.base.commander_skill_template[11013] = {
		name = "5英寸的勇氣",
		desc_world = "特殊彈幕I 強化為 特殊彈幕II",
		next_id = 0,
		exp = 0,
		prev_id = 11012,
		desc = "特殊彈幕I 強化為 特殊彈幕II",
		id = 11013,
		icon = "1101",
		lv = 3,
		effect_tactic = {
			110111,
			110113
		},
		effect_tactic_world = {
			9110111,
			110113
		}
	}
	pg.base.commander_skill_template[11021] = {
		name = "幸運之翼",
		desc_world = "作為參謀位指揮喵時，依據指揮喵戰術屬性，提高所在艦隊中輕航、正航的裝填屬性(效果小)",
		next_id = 11022,
		exp = 1,
		prev_id = 0,
		desc = "作為參謀位指揮喵時，依據指揮喵戰術屬性，提高所在艦隊中輕航、正航的裝填屬性(效果小)",
		id = 11021,
		icon = "1102",
		lv = 1,
		effect_tactic = {
			110211
		},
		effect_tactic_world = {
			110211
		}
	}
	pg.base.commander_skill_template[11022] = {
		name = "幸運之翼",
		desc_world = "作為參謀位指揮喵時，依據指揮喵戰術屬性，提高所在艦隊中輕航、正航的機動、防空屬性(效果小)",
		next_id = 11023,
		exp = 3,
		prev_id = 11021,
		desc = "所在艦隊遭遇空襲時，依據指揮喵戰術屬性，有機率迴避此次空襲傷害",
		id = 11022,
		icon = "1102",
		lv = 2,
		effect_tactic = {
			110211,
			110212
		},
		effect_tactic_world = {
			110211,
			9110212
		}
	}
	pg.base.commander_skill_template[11023] = {
		name = "幸運之翼",
		desc_world = "作為參謀位指揮喵時，依據指揮喵指揮屬性，提高所在艦隊中輕航、正航的航空屬性(效果中)",
		next_id = 0,
		exp = 0,
		prev_id = 11022,
		desc = "作為參謀位指揮喵時，依據指揮喵指揮屬性，提高所在艦隊中輕航、正航的航空屬性(效果中)",
		id = 11023,
		icon = "1102",
		lv = 3,
		effect_tactic = {
			110211,
			110212,
			110213
		},
		effect_tactic_world = {
			110211,
			9110212,
			110213
		}
	}
	pg.base.commander_skill_template[12011] = {
		name = "空母後勤",
		desc_world = "作為參謀位指揮喵時，依據指揮喵後勤屬性，提高所在艦隊中輕航、正航的裝填屬性(效果小)",
		next_id = 12012,
		exp = 1,
		prev_id = 0,
		desc = "作為參謀位指揮喵時，依據指揮喵後勤屬性，提高所在艦隊中輕航、正航的裝填屬性(效果小)",
		id = 12011,
		icon = "buff",
		lv = 1,
		effect_tactic = {
			120111
		},
		effect_tactic_world = {
			120111
		}
	}
	pg.base.commander_skill_template[12012] = {
		name = "空母後勤",
		desc_world = "作為參謀位指揮喵時，依據指揮喵指揮屬性，提高所在艦隊中輕航、正航的航空屬性(效果小)",
		next_id = 12013,
		exp = 3,
		prev_id = 12011,
		desc = "作為參謀位指揮喵時，依據指揮喵指揮屬性，提高所在艦隊中輕航、正航的航空屬性(效果小)",
		id = 12012,
		icon = "buff",
		lv = 2,
		effect_tactic = {
			120111,
			120112
		},
		effect_tactic_world = {
			120111,
			120112
		}
	}
	pg.base.commander_skill_template[12013] = {
		name = "空母後勤",
		desc_world = "作為參謀位指揮喵時，依據指揮喵戰術屬性，提高所在艦隊中輕航、正航的命中屬性(效果小)",
		next_id = 0,
		exp = 0,
		prev_id = 12012,
		desc = "作為參謀位指揮喵時，依據指揮喵戰術屬性，提高所在艦隊中輕航、正航的命中屬性(效果小)",
		id = 12013,
		icon = "buff",
		lv = 3,
		effect_tactic = {
			120111,
			120112,
			120113
		},
		effect_tactic_world = {
			120111,
			120112,
			120113
		}
	}
	pg.base.commander_skill_template[12021] = {
		name = "雷達偵察",
		desc_world = "作為參謀位指揮喵時，依據指揮喵後勤屬性，提高所在艦隊中輕巡、重巡、超巡的機動屬性(效果小)",
		next_id = 12022,
		exp = 1,
		prev_id = 0,
		desc = "依據指揮喵後勤屬性，降低所在艦隊遭遇伏擊的機率",
		id = 12021,
		icon = "def",
		lv = 1,
		effect_tactic = {
			120121
		},
		effect_tactic_world = {
			9120121
		}
	}
	pg.base.commander_skill_template[12022] = {
		name = "雷達偵察",
		desc_world = "作為參謀位指揮喵時，依據指揮喵戰術屬性，提高所在艦隊中輕巡、重巡、超巡的命中屬性(效果小)",
		next_id = 12023,
		exp = 3,
		prev_id = 12021,
		desc = "作為參謀位指揮喵時，依據指揮喵戰術屬性，提高所在艦隊中輕巡、重巡、超巡的命中屬性(效果小)",
		id = 12022,
		icon = "def",
		lv = 2,
		effect_tactic = {
			120121,
			120122
		},
		effect_tactic_world = {
			9120121,
			120122
		}
	}
	pg.base.commander_skill_template[12023] = {
		name = "雷達偵察",
		desc_world = "作為參謀位指揮喵時，依據指揮喵指揮屬性，提高所在艦隊中輕巡、重巡、超巡的炮擊屬性(效果小)",
		next_id = 0,
		exp = 0,
		prev_id = 12022,
		desc = "作為參謀位指揮喵時，依據指揮喵指揮屬性，提高所在艦隊中輕巡、重巡、超巡的炮擊屬性(效果小)",
		id = 12023,
		icon = "def",
		lv = 3,
		effect_tactic = {
			120121,
			120122,
			120123
		},
		effect_tactic_world = {
			9120121,
			120122,
			120123
		}
	}
	pg.base.commander_skill_template[20011] = {
		name = "盡忠職守",
		desc_world = "作為參謀位指揮喵時，依據指揮喵指揮屬性，提高所在艦隊中戰列、戰巡、航戰的炮擊、防空屬性(效果中)",
		next_id = 20012,
		exp = 1,
		prev_id = 0,
		desc = "作為參謀位指揮喵時，依據指揮喵指揮屬性，提高所在艦隊中戰列、戰巡、航戰的炮擊、防空屬性(效果中)",
		id = 20011,
		icon = "2001",
		lv = 1,
		effect_tactic = {
			200111
		},
		effect_tactic_world = {
			200111
		}
	}
	pg.base.commander_skill_template[20012] = {
		name = "盡忠職守",
		desc_world = "作為參謀位指揮喵時，依據指揮喵戰術屬性，提高所在艦隊全員的炮擊屬性(效果中)，持續30秒",
		next_id = 20013,
		exp = 3,
		prev_id = 20011,
		desc = "作為參謀位指揮喵，艦隊在沿岸區域(不可通行的島嶼陸地旁)交戰時，依據指揮喵戰術屬性，提高所在艦隊全員的炮擊屬性(效果中)",
		id = 20012,
		icon = "2001",
		lv = 2,
		effect_tactic = {
			200111,
			200112
		},
		effect_tactic_world = {
			200111,
			9200112
		}
	}
	pg.base.commander_skill_template[20013] = {
		name = "盡忠職守",
		desc_world = "依據指揮喵戰術屬性，提高所在艦隊成員對戰列、戰巡、航戰造成的傷害",
		next_id = 0,
		exp = 0,
		prev_id = 20012,
		desc = "依據指揮喵戰術屬性，提高所在艦隊成員對戰列、戰巡、航戰造成的傷害",
		id = 20013,
		icon = "2001",
		lv = 3,
		effect_tactic = {
			200111,
			200112,
			200113
		},
		effect_tactic_world = {
			200111,
			9200112,
			200113
		}
	}
	pg.base.commander_skill_template[20021] = {
		name = "狡黠之獾",
		desc_world = "作為司令位指揮喵時，依據指揮喵指揮屬性，提高所在艦隊中戰列、戰巡、航戰的炮擊、防空屬性(效果中)",
		next_id = 20022,
		exp = 1,
		prev_id = 0,
		desc = "作為司令位指揮喵時，依據指揮喵指揮屬性，提高所在艦隊中戰列、戰巡、航戰的炮擊、防空屬性(效果中)",
		id = 20021,
		icon = "2002",
		lv = 1,
		effect_tactic = {
			200211
		},
		effect_tactic_world = {
			200211
		}
	}
	pg.base.commander_skill_template[20022] = {
		name = "狡黠之獾",
		desc_world = "作為司令位指揮喵時，若隊伍中皇家陣營角色數量不少於4名，戰鬥開始後，依據後勤屬性，降低隊伍中先鋒角色受到的傷害，持續30秒",
		next_id = 20023,
		exp = 3,
		prev_id = 20021,
		desc = "作為司令位指揮喵時，若隊伍中皇家陣營角色數量不少於4名，戰鬥開始後，依據後勤屬性，降低隊伍中先鋒角色受到的傷害，持續30秒",
		id = 20022,
		icon = "2002",
		lv = 2,
		effect_tactic = {
			200211,
			200212
		},
		effect_tactic_world = {
			200211,
			200212
		}
	}
	pg.base.commander_skill_template[20023] = {
		name = "狡黠之獾",
		desc_world = "作為司令位指揮喵時，依據指揮喵後勤屬性，提高所在艦隊中皇家陣營角色的機動屬性(效果大)",
		next_id = 0,
		exp = 0,
		prev_id = 20022,
		desc = "作為司令位指揮喵時，依據指揮喵後勤屬性，提高所在艦隊中皇家陣營角色的機動屬性(效果大)",
		id = 20023,
		icon = "2002",
		lv = 3,
		effect_tactic = {
			200211,
			200212,
			200213
		},
		effect_tactic_world = {
			200211,
			200212,
			200213
		}
	}
	pg.base.commander_skill_template[21011] = {
		name = "物資攔截者",
		desc_world = "作為參謀位指揮喵時，依據指揮喵後勤屬性，提高所在艦隊中輕巡、重巡、超巡的機動屬性(效果小)",
		next_id = 21012,
		exp = 1,
		prev_id = 0,
		desc = "作為參謀位指揮喵時，依據指揮喵後勤屬性，提高所在艦隊中輕巡、重巡、超巡的機動屬性(效果小)",
		id = 21011,
		icon = "2101",
		lv = 1,
		effect_tactic = {
			210111
		},
		effect_tactic_world = {
			210111
		}
	}
	pg.base.commander_skill_template[21012] = {
		name = "物資攔截者",
		desc_world = "作為參謀位指揮喵，與運輸艦隊交戰時，依據指揮喵戰術屬性，提高所在艦隊中輕巡、重巡、超巡的炮擊屬性(效果大)",
		next_id = 21013,
		exp = 3,
		prev_id = 21011,
		desc = "作為參謀位指揮喵，與運輸艦隊交戰時，依據指揮喵戰術屬性，提高所在艦隊中輕巡、重巡、超巡的炮擊屬性(效果大)",
		id = 21012,
		icon = "2101",
		lv = 2,
		effect_tactic = {
			210111,
			210112
		},
		effect_tactic_world = {
			210111,
			210112
		}
	}
	pg.base.commander_skill_template[21013] = {
		name = "物資攔截者",
		desc_world = "作為參謀位指揮喵時，依據指揮喵指揮屬性，提高所在艦隊中輕巡、重巡的雷擊屬性(效果中)",
		next_id = 0,
		exp = 0,
		prev_id = 21012,
		desc = "作為參謀位指揮喵時，依據指揮喵指揮屬性，提高所在艦隊中輕巡、重巡的雷擊屬性(效果中)",
		id = 21013,
		icon = "2101",
		lv = 3,
		effect_tactic = {
			210111,
			210112,
			210113
		},
		effect_tactic_world = {
			210111,
			210112,
			210113
		}
	}
	pg.base.commander_skill_template[21021] = {
		name = "斥候獵手",
		desc_world = "作為參謀位指揮喵，與偵查艦隊交戰時，依據指揮喵戰術屬性，提高所在艦隊中驅逐的命中屬性(效果中)",
		next_id = 21022,
		exp = 1,
		prev_id = 0,
		desc = "作為參謀位指揮喵，與偵查艦隊交戰時，依據指揮喵戰術屬性，提高所在艦隊中驅逐的命中屬性(效果中)",
		id = 21021,
		icon = "2102",
		lv = 1,
		effect_tactic = {
			210211
		},
		effect_tactic_world = {
			210211
		}
	}
	pg.base.commander_skill_template[21022] = {
		name = "斥候獵手",
		desc_world = "作為參謀位指揮喵時，依據指揮喵後勤屬性，提高所在艦隊中驅逐的機動屬性(效果小)",
		next_id = 21023,
		exp = 3,
		prev_id = 21021,
		desc = "作為參謀位指揮喵時，依據指揮喵後勤屬性，提高所在艦隊中驅逐的機動屬性(效果小)",
		id = 21022,
		icon = "2102",
		lv = 2,
		effect_tactic = {
			210211,
			210212
		},
		effect_tactic_world = {
			210211,
			210212
		}
	}
	pg.base.commander_skill_template[21023] = {
		name = "斥候獵手",
		desc_world = "距離自身3格範圍內存在敵方偵察艦隊時，依據指揮喵指揮屬性，提高所在艦隊中驅逐的機動屬性(效果小)",
		next_id = 0,
		exp = 0,
		prev_id = 21022,
		desc = "距離自身3格範圍內存在敵方偵察艦隊時，當前艦隊移動力+1",
		id = 21023,
		icon = "2102",
		lv = 3,
		effect_tactic = {
			210211,
			210212,
			210213
		},
		effect_tactic_world = {
			210211,
			210212,
			9210213
		}
	}
	pg.base.commander_skill_template[21031] = {
		name = "守望者",
		desc_world = "作為參謀位指揮喵時，依據指揮喵戰術屬性，提高所在艦隊中輕航、正航的命中屬性(效果小)",
		next_id = 21032,
		exp = 1,
		prev_id = 0,
		desc = "作為參謀位指揮喵時，依據指揮喵戰術屬性，提高所在艦隊中輕航、正航的命中屬性(效果小)",
		id = 21031,
		icon = "2103",
		lv = 1,
		effect_tactic = {
			210311
		},
		effect_tactic_world = {
			210311
		}
	}
	pg.base.commander_skill_template[21032] = {
		name = "守望者",
		desc_world = "距離自身2格範圍內存在敵方偵察艦隊時，依據指揮喵後勤屬性，提高所在艦隊中驅逐的機動屬性(效果小)",
		next_id = 21033,
		exp = 3,
		prev_id = 21031,
		desc = "距離自身2格範圍內存在敵方偵察艦隊時，當前艦隊移動力+1",
		id = 21032,
		icon = "2103",
		lv = 2,
		effect_tactic = {
			210311,
			210312
		},
		effect_tactic_world = {
			210311,
			9210312
		}
	}
	pg.base.commander_skill_template[21033] = {
		name = "守望者",
		desc_world = "作為參謀位指揮喵時，依據指揮喵指揮屬性，提高所在艦隊中輕航、正航的航空屬性(效果中)",
		next_id = 0,
		exp = 0,
		prev_id = 21032,
		desc = "作為參謀位指揮喵時，依據指揮喵指揮屬性，提高所在艦隊中輕航、正航的航空屬性(效果中)",
		id = 21033,
		icon = "2103",
		lv = 3,
		effect_tactic = {
			210311,
			210312,
			210313
		},
		effect_tactic_world = {
			210311,
			9210312,
			210313
		}
	}
	pg.base.commander_skill_template[21041] = {
		name = "無畏的騎士",
		desc_world = "作為參謀位指揮喵時，依據指揮喵後勤屬性，提高所在艦隊中戰列、戰巡、航戰的機動屬性(效果小)",
		next_id = 21042,
		exp = 1,
		prev_id = 0,
		desc = "依據指揮喵後勤屬性，降低受到伏擊的機率",
		id = 21041,
		icon = "2104",
		lv = 1,
		effect_tactic = {
			210411
		},
		effect_tactic_world = {
			9210411
		}
	}
	pg.base.commander_skill_template[21042] = {
		name = "無畏的騎士",
		desc_world = "作為參謀位指揮喵時，依據指揮喵指揮屬性，提高所在艦隊中戰列、戰巡、航戰的裝填屬性(效果小)",
		next_id = 21043,
		exp = 3,
		prev_id = 21041,
		desc = "作為參謀位指揮喵時，依據指揮喵指揮屬性，提高所在艦隊中戰列、戰巡、航戰的裝填屬性(效果小)",
		id = 21042,
		icon = "2104",
		lv = 2,
		effect_tactic = {
			210411,
			210412
		},
		effect_tactic_world = {
			9210411,
			210412
		}
	}
	pg.base.commander_skill_template[21043] = {
		name = "無畏的騎士",
		desc_world = "作為參謀位指揮喵時，依據指揮喵戰術屬性，提高所在艦隊中皇家陣營角色的炮擊屬性(效果小)",
		next_id = 0,
		exp = 0,
		prev_id = 21042,
		desc = "作為參謀位指揮喵時，依據指揮喵戰術屬性，提高所在艦隊中皇家陣營角色的炮擊屬性(效果小)",
		id = 21043,
		icon = "2104",
		lv = 3,
		effect_tactic = {
			210411,
			210412,
			210413
		},
		effect_tactic_world = {
			9210411,
			210412,
			210413
		}
	}
	pg.base.commander_skill_template[22011] = {
		name = "戰列後勤",
		desc_world = "作為參謀位指揮喵時，依據指揮喵後勤屬性，提高所在艦隊中戰列、戰巡、航戰的裝填屬性(效果小)",
		next_id = 22012,
		exp = 1,
		prev_id = 0,
		desc = "作為參謀位指揮喵時，依據指揮喵後勤屬性，提高所在艦隊中戰列、戰巡、航戰的裝填屬性(效果小)",
		id = 22011,
		icon = "buff",
		lv = 1,
		effect_tactic = {
			220111
		},
		effect_tactic_world = {
			220111
		}
	}
	pg.base.commander_skill_template[22012] = {
		name = "戰列後勤",
		desc_world = "作為參謀位指揮喵時，依據指揮喵後勤屬性，提高所在艦隊中戰列、戰巡、航戰的命中屬性(效果小)",
		next_id = 22013,
		exp = 3,
		prev_id = 22011,
		desc = "作為參謀位指揮喵時，依據指揮喵後勤屬性，提高所在艦隊中戰列、戰巡、航戰的命中屬性(效果小)",
		id = 22012,
		icon = "buff",
		lv = 2,
		effect_tactic = {
			220111,
			220112
		},
		effect_tactic_world = {
			220111,
			220112
		}
	}
	pg.base.commander_skill_template[22013] = {
		name = "戰列後勤",
		desc_world = "作為參謀位指揮喵時，依據指揮喵後勤屬性，提高所在艦隊中戰列、戰巡、航戰的炮擊屬性(效果小)",
		next_id = 0,
		exp = 0,
		prev_id = 22012,
		desc = "作為參謀位指揮喵時，依據指揮喵後勤屬性，提高所在艦隊中戰列、戰巡、航戰的炮擊屬性(效果小)",
		id = 22013,
		icon = "buff",
		lv = 3,
		effect_tactic = {
			220111,
			220112,
			220113
		},
		effect_tactic_world = {
			220111,
			220112,
			220113
		}
	}
	pg.base.commander_skill_template[22021] = {
		name = "巡洋戰術",
		desc_world = "作為參謀位指揮喵時，依據指揮喵戰術屬性，提高所在艦隊中輕巡、重巡、超巡的炮擊屬性(效果小)",
		next_id = 22022,
		exp = 1,
		prev_id = 0,
		desc = "作為參謀位指揮喵時，依據指揮喵戰術屬性，提高所在艦隊中輕巡、重巡、超巡的炮擊屬性(效果小)",
		id = 22021,
		icon = "buff",
		lv = 1,
		effect_tactic = {
			220211
		},
		effect_tactic_world = {
			220211
		}
	}
	pg.base.commander_skill_template[22022] = {
		name = "巡洋戰術",
		desc_world = "作為參謀位指揮喵時，依據指揮喵戰術屬性，提高所在艦隊中輕巡、重巡、超巡的命中屬性(效果小)",
		next_id = 22023,
		exp = 3,
		prev_id = 22021,
		desc = "作為參謀位指揮喵時，依據指揮喵戰術屬性，提高所在艦隊中輕巡、重巡、超巡的命中屬性(效果小)",
		id = 22022,
		icon = "buff",
		lv = 2,
		effect_tactic = {
			220211,
			220212
		},
		effect_tactic_world = {
			220211,
			220212
		}
	}
	pg.base.commander_skill_template[22023] = {
		name = "巡洋戰術",
		desc_world = "作為參謀位指揮喵時，依據指揮喵戰術屬性，提高所在艦隊中輕巡、重巡、超巡的機動屬性(效果小)",
		next_id = 0,
		exp = 0,
		prev_id = 22022,
		desc = "作為參謀位指揮喵時，依據指揮喵戰術屬性，提高所在艦隊中輕巡、重巡、超巡的機動屬性(效果小)",
		id = 22023,
		icon = "buff",
		lv = 3,
		effect_tactic = {
			220211,
			220212,
			220213
		},
		effect_tactic_world = {
			220211,
			220212,
			220213
		}
	}
	pg.base.commander_skill_template[30011] = {
		name = "反擊的一拳",
		desc_world = "作為參謀位指揮喵時，依據指揮喵指揮屬性，提高所在艦隊中輕航、正航的航空、裝填屬性(效果小)",
		next_id = 30012,
		exp = 1,
		prev_id = 0,
		desc = "作為參謀位指揮喵時，依據指揮喵指揮屬性，提高所在艦隊中輕航、正航的航空、裝填屬性(效果小)",
		id = 30011,
		icon = "3001",
		lv = 1,
		effect_tactic = {
			300111
		},
		effect_tactic_world = {
			300111
		}
	}
	pg.base.commander_skill_template[30012] = {
		name = "反擊的一拳",
		desc_world = "作為參謀位指揮喵時，依據指揮喵後勤屬性，提高所在艦隊中輕航、正航的機動屬性(效果小)",
		next_id = 30013,
		exp = 3,
		prev_id = 30011,
		desc = "依據指揮喵後勤屬性，降低受到伏擊的機率",
		id = 30012,
		icon = "3001",
		lv = 2,
		effect_tactic = {
			300111,
			300112
		},
		effect_tactic_world = {
			300111,
			9300112
		}
	}
	pg.base.commander_skill_template[30013] = {
		name = "反擊的一拳",
		desc_world = "進入戰鬥時，若主力艦隊僅剩下一艘輕航或正航，且無其他主力艦，則首輪空襲裝填速度加快8%，同時首輪空襲時觸發額外的魚雷機編隊攻擊",
		next_id = 0,
		exp = 0,
		prev_id = 30012,
		desc = "進入戰鬥時，若主力艦隊僅剩下一艘輕航或正航，且無其他主力艦，則首輪空襲裝填速度加快8%，同時首輪空襲時觸發額外的魚雷機編隊攻擊",
		id = 30013,
		icon = "3001",
		lv = 3,
		effect_tactic = {
			300111,
			300112,
			300113
		},
		effect_tactic_world = {
			300111,
			9300112,
			300113
		}
	}
	pg.base.commander_skill_template[30021] = {
		name = "敢勇當先",
		desc_world = "作為司令位指揮喵時，依據指揮喵戰術屬性，提高所在艦隊中輕巡、重巡、超巡的炮擊、雷擊屬性(效果中)",
		next_id = 30022,
		exp = 1,
		prev_id = 0,
		desc = "作為司令位指揮喵時，依據指揮喵戰術屬性，提高所在艦隊中輕巡、重巡、超巡的炮擊、雷擊屬性(效果中)",
		id = 30021,
		icon = "3002",
		lv = 1,
		effect_tactic = {
			300211
		},
		effect_tactic_world = {
			300211
		}
	}
	pg.base.commander_skill_template[30022] = {
		name = "敢勇當先",
		desc_world = "作為司令位指揮喵時，若距離自身2格範圍內存在敵方主力艦隊，依據指揮屬性，降低隊伍中旗艦受到的傷害(Lv2生效)",
		next_id = 30023,
		exp = 3,
		prev_id = 30021,
		desc = "作為司令位指揮喵時，若距離自身2格範圍內存在敵方主力艦隊，依據指揮屬性，降低隊伍中旗艦受到的傷害(Lv2生效)",
		id = 30022,
		icon = "3002",
		lv = 2,
		effect_tactic = {
			300211,
			300212
		},
		effect_tactic_world = {
			300211,
			300212
		}
	}
	pg.base.commander_skill_template[30023] = {
		name = "敢勇當先",
		desc_world = "作為司令位指揮喵，與主力艦隊交戰時，戰鬥開始後，依據戰術屬性，提高所在艦隊中輕巡、重巡、超巡的命中、機動屬性(效果大)，持續30秒",
		next_id = 0,
		exp = 0,
		prev_id = 30022,
		desc = "作為司令位指揮喵，與主力艦隊交戰時，戰鬥開始後，依據戰術屬性，提高所在艦隊中輕巡、重巡、超巡的命中、機動屬性(效果大)，持續30秒",
		id = 30023,
		icon = "3002",
		lv = 3,
		effect_tactic = {
			300211,
			300212,
			300213
		},
		effect_tactic_world = {
			300211,
			300212,
			300213
		}
	}
	pg.base.commander_skill_template[31011] = {
		name = "必殺·三段擊",
		desc_world = "作為參謀位指揮喵時，依據指揮喵戰術屬性，提高所在艦隊中輕航、正航的命中屬性(效果小)",
		next_id = 31012,
		exp = 1,
		prev_id = 0,
		desc = "作為參謀位指揮喵時，依據指揮喵戰術屬性，提高所在艦隊中輕航、正航的命中屬性(效果小)",
		id = 31011,
		icon = "3101",
		lv = 1,
		effect_tactic = {
			310111
		},
		effect_tactic_world = {
			310111
		}
	}
	pg.base.commander_skill_template[31012] = {
		name = "必殺·三段擊",
		desc_world = "作為參謀位指揮喵時，依據指揮喵指揮屬性，提高所在艦隊中輕航、正航的航空屬性(效果中)",
		next_id = 31013,
		exp = 3,
		prev_id = 31011,
		desc = "作為參謀位指揮喵時，依據指揮喵指揮屬性，提高所在艦隊中輕航、正航的航空屬性(效果中)",
		id = 31012,
		icon = "3101",
		lv = 2,
		effect_tactic = {
			310111,
			310112
		},
		effect_tactic_world = {
			310111,
			310112
		}
	}
	pg.base.commander_skill_template[31013] = {
		name = "必殺·三段擊",
		desc_world = "若所在艦隊旗艦為輕航或正航，戰鬥開始後15秒觸發額外的艦載機編隊攻擊",
		next_id = 0,
		exp = 0,
		prev_id = 31012,
		desc = "若所在艦隊中有輕航或正航，當接觸非旗艦敵方單位時，有15%的機率發射一次空襲打擊（地圖），傷害依據指揮喵戰術屬性、雙方等級、戰鬥力決定",
		id = 31013,
		icon = "3101",
		lv = 3,
		effect_tactic = {
			310111,
			310112,
			310113
		},
		effect_tactic_world = {
			310111,
			310112,
			9310113
		}
	}
	pg.base.commander_skill_template[31021] = {
		name = "野性的直覺",
		desc_world = "作為參謀位指揮喵時，依據指揮喵指揮屬性，提高所在艦隊中驅逐的機動屬性(效果小)",
		next_id = 31022,
		exp = 1,
		prev_id = 0,
		desc = "作為參謀位指揮喵時，依據指揮喵指揮屬性，提高所在艦隊中驅逐的機動屬性(效果小)",
		id = 31021,
		icon = "3102",
		lv = 1,
		effect_tactic = {
			310211
		},
		effect_tactic_world = {
			310211
		}
	}
	pg.base.commander_skill_template[31022] = {
		name = "野性的直覺",
		desc_world = "作為參謀位指揮喵時，依據指揮喵後勤屬性，提高所在艦隊中驅逐的雷擊屬性(效果中)",
		next_id = 31023,
		exp = 3,
		prev_id = 31021,
		desc = "作為參謀位指揮喵時，依據指揮喵後勤屬性，提高所在艦隊中驅逐的雷擊屬性(效果中)",
		id = 31022,
		icon = "3102",
		lv = 2,
		effect_tactic = {
			310211,
			310212
		},
		effect_tactic_world = {
			310211,
			310212
		}
	}
	pg.base.commander_skill_template[31023] = {
		name = "野性的直覺",
		desc_world = "作為參謀位指揮喵，先鋒艦隊只有一艘艦船且為驅逐時，依據指揮喵指揮屬性，提高所在艦隊中驅逐的雷擊屬性(效果大)",
		next_id = 0,
		exp = 0,
		prev_id = 31022,
		desc = "作為參謀位指揮喵，先鋒艦隊只有一艘艦船且為驅逐時，依據指揮喵指揮屬性，提高所在艦隊中驅逐的雷擊屬性(效果大)",
		id = 31023,
		icon = "3102",
		lv = 3,
		effect_tactic = {
			310211,
			310212,
			310213
		},
		effect_tactic_world = {
			310211,
			310212,
			310213
		}
	}
	pg.base.commander_skill_template[32011] = {
		name = "巡洋指揮",
		desc_world = "作為參謀位指揮喵時，依據指揮喵指揮屬性，提高所在艦隊中輕巡、重巡、超巡的炮擊屬性(效果小)",
		next_id = 32012,
		exp = 1,
		prev_id = 0,
		desc = "作為參謀位指揮喵時，依據指揮喵指揮屬性，提高所在艦隊中輕巡、重巡、超巡的炮擊屬性(效果小)",
		id = 32011,
		icon = "buff",
		lv = 1,
		effect_tactic = {
			320111
		},
		effect_tactic_world = {
			320111
		}
	}
	pg.base.commander_skill_template[32012] = {
		name = "巡洋指揮",
		desc_world = "作為參謀位指揮喵時，依據指揮喵後勤屬性，提高所在艦隊中輕巡、重巡的雷擊屬性(效果小)",
		next_id = 32013,
		exp = 3,
		prev_id = 32011,
		desc = "作為參謀位指揮喵時，依據指揮喵後勤屬性，提高所在艦隊中輕巡、重巡的雷擊屬性(效果小)",
		id = 32012,
		icon = "buff",
		lv = 2,
		effect_tactic = {
			320111,
			320112
		},
		effect_tactic_world = {
			320111,
			320112
		}
	}
	pg.base.commander_skill_template[32013] = {
		name = "巡洋指揮",
		desc_world = "作為參謀位指揮喵時，依據指揮喵指揮屬性，提高所在艦隊中輕巡、重巡的雷擊屬性(效果小)",
		next_id = 0,
		exp = 0,
		prev_id = 32012,
		desc = "作為參謀位指揮喵時，依據指揮喵指揮屬性，提高所在艦隊中輕巡、重巡的雷擊屬性(效果小)",
		id = 32013,
		icon = "buff",
		lv = 3,
		effect_tactic = {
			320111,
			320112,
			320113
		},
		effect_tactic_world = {
			320111,
			320112,
			320113
		}
	}
	pg.base.commander_skill_template[32021] = {
		name = "戰列後勤",
		desc_world = "作為參謀位指揮喵時，依據指揮喵後勤屬性，提高所在艦隊中戰列、戰巡、航戰的命中屬性(效果小)",
		next_id = 32022,
		exp = 1,
		prev_id = 0,
		desc = "作為參謀位指揮喵時，依據指揮喵後勤屬性，提高所在艦隊中戰列、戰巡、航戰的命中屬性(效果小)",
		id = 32021,
		icon = "buff",
		lv = 1,
		effect_tactic = {
			320211
		},
		effect_tactic_world = {
			320211
		}
	}
	pg.base.commander_skill_template[32022] = {
		name = "戰列後勤",
		desc_world = "作為參謀位指揮喵時，依據指揮喵指揮屬性，提高所在艦隊中戰列、戰巡、航戰的防空屬性(效果小)",
		next_id = 32023,
		exp = 3,
		prev_id = 32021,
		desc = "作為參謀位指揮喵時，依據指揮喵指揮屬性，提高所在艦隊中戰列、戰巡、航戰的防空屬性(效果小)",
		id = 32022,
		icon = "buff",
		lv = 2,
		effect_tactic = {
			320211,
			320212
		},
		effect_tactic_world = {
			320211,
			320212
		}
	}
	pg.base.commander_skill_template[32023] = {
		name = "戰列後勤",
		desc_world = "作為參謀位指揮喵時，依據指揮喵戰術屬性，提高所在艦隊中戰列、戰巡、航戰的炮擊屬性(效果小)",
		next_id = 0,
		exp = 0,
		prev_id = 32022,
		desc = "作為參謀位指揮喵時，依據指揮喵戰術屬性，提高所在艦隊中戰列、戰巡、航戰的炮擊屬性(效果小)",
		id = 32023,
		icon = "buff",
		lv = 3,
		effect_tactic = {
			320211,
			320212,
			320213
		},
		effect_tactic_world = {
			320211,
			320212,
			320213
		}
	}
	pg.base.commander_skill_template[40011] = {
		name = "靜寂的狩獵者",
		desc_world = "依據指揮喵指揮、後勤屬性，提高所在艦隊中潛艇、潛母的雷擊屬性(效果小)",
		next_id = 40012,
		exp = 1,
		prev_id = 0,
		desc = "依據指揮喵指揮、後勤屬性，提高所在艦隊中潛艇、潛母的雷擊屬性(效果小)",
		id = 40011,
		icon = "4001",
		lv = 1,
		effect_tactic = {
			400111
		},
		effect_tactic_world = {
			400111
		}
	}
	pg.base.commander_skill_template[40012] = {
		name = "靜寂的狩獵者",
		desc_world = "作為司令位指揮喵時，提高所在艦隊中潛艇15點氧氣",
		next_id = 40013,
		exp = 3,
		prev_id = 40011,
		desc = "狩獵範圍等級+1",
		id = 40012,
		icon = "4001",
		lv = 2,
		effect_tactic = {
			400111,
			400112
		},
		effect_tactic_world = {
			400111,
			9400112
		}
	}
	pg.base.commander_skill_template[40013] = {
		name = "靜寂的狩獵者",
		desc_world = "作為司令位指揮喵，潛艇艦隊旗艦開始戰鬥時，觸發額外的魚雷攻擊",
		next_id = 0,
		exp = 0,
		prev_id = 40012,
		desc = "依據指揮喵戰術屬性，提高魚雷打擊（地圖）的傷害",
		id = 40013,
		icon = "4001",
		lv = 3,
		effect_tactic = {
			400111,
			400112,
			400113
		},
		effect_tactic_world = {
			400111,
			9400112,
			9400113
		}
	}
	pg.base.commander_skill_template[40021] = {
		name = "不屈的意志",
		desc_world = "作為參謀位指揮喵時，依據指揮喵指揮屬性，提高所在艦隊中戰列、戰巡、航戰的炮擊、命中屬性(效果中)",
		next_id = 40022,
		exp = 1,
		prev_id = 0,
		desc = "作為參謀位指揮喵時，依據指揮喵指揮屬性，提高所在艦隊中戰列、戰巡、航戰的炮擊、命中屬性(效果中)",
		id = 40021,
		icon = "4002",
		lv = 1,
		effect_tactic = {
			400211
		},
		effect_tactic_world = {
			400211
		}
	}
	pg.base.commander_skill_template[40022] = {
		name = "不屈的意志",
		desc_world = "作為參謀位指揮喵，若旗艦為戰列、戰巡、航戰中的其中一種，開始戰鬥後20秒，有50%的機率發射一輪彈幕射擊",
		next_id = 40023,
		exp = 3,
		prev_id = 40021,
		desc = "作為參謀位指揮喵，若所在艦隊中有戰列、戰巡、航戰，當接觸非旗艦敵方單位時，有15%的機率發射一次主炮打擊（地圖），傷害依據指揮喵指揮屬性、 雙方等級、戰鬥力決定",
		id = 40022,
		icon = "4002",
		lv = 2,
		effect_tactic = {
			400211,
			400212
		},
		effect_tactic_world = {
			400211,
			9400212
		}
	}
	pg.base.commander_skill_template[40023] = {
		name = "不屈的意志",
		desc_world = "作為參謀位指揮喵，與主力艦隊交戰時，依據指揮喵後勤屬性，降低隊伍中戰列、戰巡、航戰受到的傷害，並提高裝填(效果中)",
		next_id = 0,
		exp = 0,
		prev_id = 40022,
		desc = "作為參謀位指揮喵，與主力艦隊交戰時，依據指揮喵後勤屬性，降低隊伍中戰列、戰巡、航戰受到的傷害，並提高裝填(效果中)",
		id = 40023,
		icon = "4002",
		lv = 3,
		effect_tactic = {
			400211,
			400212,
			400213
		},
		effect_tactic_world = {
			400211,
			9400212,
			400213
		}
	}
	pg.base.commander_skill_template[41011] = {
		name = "海之鐵騎士",
		desc_world = "作為參謀位指揮喵時，依據指揮喵指揮屬性，提高所在艦隊中戰列、戰巡、航戰的機動屬性(效果小)",
		next_id = 41012,
		exp = 1,
		prev_id = 0,
		desc = "作為參謀位指揮喵時，依據指揮喵指揮屬性，提高所在艦隊中戰列、戰巡、航戰的機動屬性(效果小)",
		id = 41011,
		icon = "4101",
		lv = 1,
		effect_tactic = {
			410111
		},
		effect_tactic_world = {
			410111
		}
	}
	pg.base.commander_skill_template[41012] = {
		name = "海之鐵騎士",
		desc_world = "作為參謀位指揮喵時，依據指揮喵戰術屬性，提高所在艦隊中戰列、戰巡、航戰的命中屬性(效果中)",
		next_id = 41013,
		exp = 3,
		prev_id = 41011,
		desc = "作為參謀位指揮喵時，依據指揮喵戰術屬性，提高所在艦隊中戰列、戰巡、航戰的命中屬性(效果中)",
		id = 41012,
		icon = "4101",
		lv = 2,
		effect_tactic = {
			410111,
			410112
		},
		effect_tactic_world = {
			410111,
			410112
		}
	}
	pg.base.commander_skill_template[41013] = {
		name = "海之鐵騎士",
		desc_world = "與主力艦隊交戰時，依據指揮喵戰術屬性，降低旗艦受到的傷害",
		next_id = 0,
		exp = 0,
		prev_id = 41012,
		desc = "與主力艦隊交戰時，依據指揮喵戰術屬性，降低旗艦受到的傷害",
		id = 41013,
		icon = "4101",
		lv = 3,
		effect_tactic = {
			410111,
			410112,
			410113
		},
		effect_tactic_world = {
			410111,
			410112,
			410113
		}
	}
	pg.base.commander_skill_template[41021] = {
		name = "海之勇者",
		desc_world = "作為參謀位指揮喵時，依據指揮喵指揮屬性，提高所在艦隊中潛艇、潛母的雷擊屬性(效果小)",
		next_id = 41022,
		exp = 1,
		prev_id = 0,
		desc = "作為參謀位指揮喵時，依據指揮喵指揮屬性，提高所在艦隊中潛艇、潛母的雷擊屬性(效果小)",
		id = 41021,
		icon = "4102",
		lv = 1,
		effect_tactic = {
			410211
		},
		effect_tactic_world = {
			410211
		}
	}
	pg.base.commander_skill_template[41022] = {
		name = "海之勇者",
		desc_world = "作為參謀位指揮喵時，依據指揮喵指揮屬性，提高所在艦隊中潛艇、潛母的命中屬性(效果中)",
		next_id = 41023,
		exp = 3,
		prev_id = 41021,
		desc = "作為參謀位指揮喵時，依據指揮喵指揮屬性，提高所在艦隊中潛艇、潛母的命中屬性(效果中)",
		id = 41022,
		icon = "4102",
		lv = 2,
		effect_tactic = {
			410211,
			410212
		},
		effect_tactic_world = {
			410211,
			410212
		}
	}
	pg.base.commander_skill_template[41023] = {
		name = "海之勇者",
		desc_world = "以參謀位指揮喵時，提高所在艦隊中潛艇5點氧氣",
		next_id = 0,
		exp = 0,
		prev_id = 41022,
		desc = "狩獵範圍等級+1",
		id = 41023,
		icon = "4102",
		lv = 3,
		effect_tactic = {
			410211,
			410212,
			410213
		},
		effect_tactic_world = {
			410211,
			410212,
			9410213
		}
	}
	pg.base.commander_skill_template[41031] = {
		name = "守護之心",
		desc_world = "作為參謀位指揮喵時，依據指揮喵指揮屬性，提高所在艦隊中潛艇、潛母的雷擊屬性(效果小)",
		next_id = 41032,
		exp = 1,
		prev_id = 0,
		desc = "作為參謀位指揮喵時，依據指揮喵指揮屬性，提高所在艦隊中潛艇、潛母的雷擊屬性(效果小)",
		id = 41031,
		icon = "4103",
		lv = 1,
		effect_tactic = {
			410311
		},
		effect_tactic_world = {
			410311
		}
	}
	pg.base.commander_skill_template[41032] = {
		name = "守護之心",
		desc_world = "作為參謀位指揮喵時，依據指揮喵指揮屬性，提高所在艦隊中潛艇、潛母的機動屬性(效果中)",
		next_id = 41033,
		exp = 3,
		prev_id = 41031,
		desc = "作為參謀位指揮喵時，依據指揮喵指揮屬性，提高所在艦隊中潛艇、潛母的機動屬性(效果中)",
		id = 41032,
		icon = "4103",
		lv = 2,
		effect_tactic = {
			410311,
			410312
		},
		effect_tactic_world = {
			410311,
			410312
		}
	}
	pg.base.commander_skill_template[41033] = {
		name = "守護之心",
		desc_world = "以潛艇艦隊參謀位指揮喵時，若戰鬥中艦隊旗艦為戰列、戰巡或航戰，依據指揮喵指揮屬性，提高所在艦隊中潛艇、潛母的命中屬性(效果中)",
		next_id = 0,
		exp = 0,
		prev_id = 41032,
		desc = "以潛艇艦隊參謀位指揮喵時，若戰鬥中艦隊旗艦為戰列、戰巡或航戰，依據指揮喵指揮屬性，提高所在艦隊中潛艇、潛母的命中屬性(效果中)",
		id = 41033,
		icon = "4103",
		lv = 3,
		effect_tactic = {
			410311,
			410312,
			410313
		},
		effect_tactic_world = {
			410311,
			410312,
			410313
		}
	}
	pg.base.commander_skill_template[42011] = {
		name = "巡洋後勤",
		desc_world = "作為參謀位指揮喵時，依據指揮喵後勤屬性，提高所在艦隊中輕巡、重巡、超巡的機動屬性(效果小)",
		next_id = 42012,
		exp = 1,
		prev_id = 0,
		desc = "作為參謀位指揮喵時，依據指揮喵後勤屬性，提高所在艦隊中輕巡、重巡、超巡的機動屬性(效果小)",
		id = 42011,
		icon = "buff",
		lv = 1,
		effect_tactic = {
			420111
		},
		effect_tactic_world = {
			420111
		}
	}
	pg.base.commander_skill_template[42012] = {
		name = "巡洋後勤",
		desc_world = "作為參謀位指揮喵時，依據指揮喵後勤屬性，提高所在艦隊中輕巡、重巡、超巡的炮擊屬性(效果小)",
		next_id = 42013,
		exp = 3,
		prev_id = 42011,
		desc = "作為參謀位指揮喵時，依據指揮喵後勤屬性，提高所在艦隊中輕巡、重巡、超巡的炮擊屬性(效果小)",
		id = 42012,
		icon = "buff",
		lv = 2,
		effect_tactic = {
			420111,
			420112
		},
		effect_tactic_world = {
			420111,
			420112
		}
	}
	pg.base.commander_skill_template[42013] = {
		name = "巡洋後勤",
		desc_world = "作為參謀位指揮喵時，依據指揮喵指揮屬性，提高所在艦隊中輕巡、重巡、超巡的炮擊屬性(效果小)",
		next_id = 0,
		exp = 0,
		prev_id = 42012,
		desc = "作為參謀位指揮喵時，依據指揮喵指揮屬性，提高所在艦隊中輕巡、重巡、超巡的炮擊屬性(效果小)",
		id = 42013,
		icon = "buff",
		lv = 3,
		effect_tactic = {
			420111,
			420112,
			420113
		},
		effect_tactic_world = {
			420111,
			420112,
			420113
		}
	}
	pg.base.commander_skill_template[42021] = {
		name = "驅逐指揮",
		desc_world = "作為參謀位指揮喵時，依據指揮喵指揮屬性，提高所在艦隊中驅逐的機動屬性(效果小)",
		next_id = 42022,
		exp = 1,
		prev_id = 0,
		desc = "作為參謀位指揮喵時，依據指揮喵指揮屬性，提高所在艦隊中驅逐的機動屬性(效果小)",
		id = 42021,
		icon = "buff",
		lv = 1,
		effect_tactic = {
			420211
		},
		effect_tactic_world = {
			420211
		}
	}
	pg.base.commander_skill_template[42022] = {
		name = "驅逐指揮",
		desc_world = "作為參謀位指揮喵時，依據指揮喵戰術屬性，提高所在艦隊中驅逐的命中屬性(效果小)",
		next_id = 42023,
		exp = 3,
		prev_id = 42021,
		desc = "作為參謀位指揮喵時，依據指揮喵戰術屬性，提高所在艦隊中驅逐的命中屬性(效果小)",
		id = 42022,
		icon = "buff",
		lv = 2,
		effect_tactic = {
			420211,
			420212
		},
		effect_tactic_world = {
			420211,
			420212
		}
	}
	pg.base.commander_skill_template[42023] = {
		name = "驅逐指揮",
		desc_world = "作為參謀位指揮喵時，依據指揮喵後勤屬性，提高所在艦隊中驅逐的雷擊屬性(效果小)",
		next_id = 0,
		exp = 0,
		prev_id = 42022,
		desc = "作為參謀位指揮喵時，依據指揮喵後勤屬性，提高所在艦隊中驅逐的雷擊屬性(效果小)",
		id = 42023,
		icon = "buff",
		lv = 3,
		effect_tactic = {
			420211,
			420212,
			420213
		},
		effect_tactic_world = {
			420211,
			420212,
			420213
		}
	}
	pg.base.commander_skill_template[960011] = {
		name = "環球航行 ",
		desc_world = "依據指揮喵指揮屬性，提高所在艦隊中戰列、戰巡、航戰、風帆M的炮擊屬性(效果中)；",
		next_id = 960012,
		exp = 0,
		prev_id = 0,
		desc = "依據指揮喵指揮屬性，提高所在艦隊中戰列、戰巡、航戰、風帆M的炮擊屬性(效果中)；",
		id = 960011,
		icon = "96001",
		lv = 1,
		effect_tactic = {
			9600111,
			9600112,
			9600113
		},
		effect_tactic_world = {
			9600111,
			9600112,
			9600113
		}
	}
	pg.base.commander_skill_template[960012] = {
		name = "環球航行 ",
		desc_world = "所在艦隊中存在任意風帆艦時，依據指揮喵戰術屬性，降低旗艦受到的傷害",
		next_id = 960013,
		exp = 0,
		prev_id = 960011,
		desc = "所在艦隊中存在任意風帆艦時，依據指揮喵戰術屬性，降低旗艦受到的傷害",
		id = 960012,
		icon = "96001",
		lv = 1,
		effect_tactic = {
			9600111,
			9600112,
			9600113
		},
		effect_tactic_world = {
			9600111,
			9600112,
			9600113
		}
	}
	pg.base.commander_skill_template[960013] = {
		name = "環球航行 ",
		desc_world = "依據指揮喵戰術屬性，提高隊伍中風帆V、風帆M的機動屬性(效果大)",
		next_id = 0,
		exp = 0,
		prev_id = 960012,
		desc = "依據指揮喵戰術屬性，提高隊伍中風帆V、風帆M的機動屬性(效果大)",
		id = 960013,
		icon = "96001",
		lv = 1,
		effect_tactic = {
			9600111,
			9600112,
			9600113
		},
		effect_tactic_world = {
			9600111,
			9600112,
			9600113
		}
	}
	pg.base.commander_skill_template[960021] = {
		name = "私掠者的是非",
		desc_world = "依據指揮喵指揮屬性，提高所在艦隊中輕巡、重巡、超巡、風帆V的命中屬性(效果中)；",
		next_id = 960022,
		exp = 0,
		prev_id = 0,
		desc = "依據指揮喵指揮屬性，提高所在艦隊中輕巡、重巡、超巡、風帆V的命中屬性(效果中)；",
		id = 960021,
		icon = "96002",
		lv = 1,
		effect_tactic = {
			9600211,
			9600212,
			9600213
		},
		effect_tactic_world = {
			9600211,
			99600212,
			9600213
		}
	}
	pg.base.commander_skill_template[960022] = {
		name = "私掠者的是非",
		desc_world = "距離自身3格範圍內存在敵方偵察艦隊時，依據指揮喵指揮屬性，提高所在艦隊中風帆V的機動屬性(效果小)",
		next_id = 960023,
		exp = 0,
		prev_id = 960021,
		desc = "距離自身3格範圍內存在敵方偵察艦隊時，當前艦隊移動力+1",
		id = 960022,
		icon = "96002",
		lv = 1,
		effect_tactic = {
			9600211,
			9600212,
			9600213
		},
		effect_tactic_world = {
			9600211,
			99600212,
			9600213
		}
	}
	pg.base.commander_skill_template[960023] = {
		name = "私掠者的是非",
		desc_world = "所在艦隊中存在任意風帆艦時，依據指揮喵戰術屬性，降低輕巡、重巡、風帆艦V受到的傷害",
		next_id = 0,
		exp = 0,
		prev_id = 960022,
		desc = "所在艦隊中存在任意風帆艦時，依據指揮喵戰術屬性，降低輕巡、重巡、風帆艦V受到的傷害",
		id = 960023,
		icon = "96002",
		lv = 1,
		effect_tactic = {
			9600211,
			9600212,
			9600213
		},
		effect_tactic_world = {
			9600211,
			99600212,
			9600213
		}
	}
	pg.base.commander_skill_template[960031] = {
		name = "「高速」的山姆",
		desc_world = "以潛艇編隊指揮喵時，提高所在艦隊中潛艇5點氧氣",
		next_id = 960032,
		exp = 0,
		prev_id = 0,
		desc = "狩獵範圍等級+1",
		id = 960031,
		icon = "96003",
		lv = 1,
		effect_tactic = {
			9600311,
			9600312,
			9600313
		},
		effect_tactic_world = {
			99600311,
			9600312,
			9600313
		}
	}
	pg.base.commander_skill_template[960032] = {
		name = "「高速」的山姆",
		desc_world = "身為潛艇編隊指揮喵時，進入戰鬥後恢復所有風帆艦2%耐久",
		next_id = 960033,
		exp = 0,
		prev_id = 960031,
		desc = "身為潛艇編隊指揮喵時，進入戰鬥後恢復所有風帆艦2%耐久",
		id = 960032,
		icon = "96003",
		lv = 1,
		effect_tactic = {
			9600311,
			9600312,
			9600313
		},
		effect_tactic_world = {
			99600311,
			9600312,
			9600313
		}
	}
	pg.base.commander_skill_template[960033] = {
		name = "「高速」的山姆",
		desc_world = "作為潛艇編隊指揮喵時，進入戰鬥後依據指揮喵戰術屬性，提高所在艦隊中潛艇、潛母、風帆S的命中屬性（效果中）",
		next_id = 0,
		exp = 0,
		prev_id = 960032,
		desc = "作為潛艇編隊指揮喵時，進入戰鬥後依據指揮喵戰術屬性，提高所在艦隊中潛艇、潛母、風帆S的命中屬性（效果中）",
		id = 960033,
		icon = "96003",
		lv = 1,
		effect_tactic = {
			9600311,
			9600312,
			9600313
		},
		effect_tactic_world = {
			99600311,
			9600312,
			9600313
		}
	}
	pg.base.commander_skill_template[960041] = {
		name = "攻奪與幻想",
		desc_world = "所在艦隊中存在任意風帆艦時，依據指揮喵指揮内容，降低先鋒艦隊成員受到的傷害",
		next_id = 960042,
		exp = 0,
		prev_id = 0,
		desc = "所在艦隊中存在任意風帆艦時，依據指揮喵指揮内容，降低先鋒艦隊成員受到的傷害",
		id = 960041,
		icon = "96004",
		lv = 1,
		effect_tactic = {
			9600411,
			9600412,
			9600413,
			9600414
		},
		effect_tactic_world = {
			9600411,
			9600412,
			9600413,
			9600414
		}
	}
	pg.base.commander_skill_template[960042] = {
		name = "攻奪與幻想",
		desc_world = "作為司令位指揮喵時，依據指揮喵戰術内容，提高所在艦隊全員的炮擊内容（效果中）",
		next_id = 960043,
		exp = 0,
		prev_id = 960041,
		desc = "作為司令位指揮喵時，依據指揮喵戰術内容，提高所在艦隊全員的炮擊内容（效果中）",
		id = 960042,
		icon = "96004",
		lv = 1,
		effect_tactic = {
			9600411,
			9600412,
			9600413,
			9600414
		},
		effect_tactic_world = {
			9600411,
			9600412,
			9600413,
			9600414
		}
	}
	pg.base.commander_skill_template[960043] = {
		name = "攻奪與幻想",
		desc_world = "艦隊成員在戰鬥中受到傷害，導致耐久低於30%後，為其恢復5%耐久（每場戰鬥1次）",
		next_id = 0,
		exp = 0,
		prev_id = 960042,
		desc = "艦隊成員在戰鬥中受到傷害，導致耐久低於30%後，為其恢復5%耐久（每場戰鬥1次）",
		id = 960043,
		icon = "96004",
		lv = 1,
		effect_tactic = {
			9600411,
			9600412,
			9600413,
			9600414
		},
		effect_tactic_world = {
			9600411,
			9600412,
			9600413,
			9600414
		}
	}
	pg.base.commander_skill_template[960051] = {
		name = "戒律的訂立者",
		desc_world = "作為潜艇編隊指揮喵時，提高所在艦隊中潜艇5點氧氣",
		next_id = 960052,
		exp = 0,
		prev_id = 0,
		desc = "狩獵範圍等級+1",
		id = 960051,
		icon = "96005",
		lv = 1,
		effect_tactic = {
			9600511,
			9600512,
			9600513
		},
		effect_tactic_world = {
			99600511,
			9600512,
			9600513
		}
	}
	pg.base.commander_skill_template[960052] = {
		name = "戒律的訂立者",
		desc_world = "作為潜艇編隊指揮喵時，進入戰鬥後依據指揮喵指揮内容，提高所在艦隊中風帆S的炮擊内容，潜艇、潜母的雷擊内容（效果中）",
		next_id = 960053,
		exp = 0,
		prev_id = 960051,
		desc = "作為潜艇編隊指揮喵時，進入戰鬥後依據指揮喵指揮内容，提高所在艦隊中風帆S的炮擊内容，潜艇、潜母的雷擊内容（效果中）",
		id = 960052,
		icon = "96005",
		lv = 1,
		effect_tactic = {
			9600511,
			9600512,
			9600513
		},
		effect_tactic_world = {
			99600511,
			9600512,
			9600513
		}
	}
	pg.base.commander_skill_template[960053] = {
		name = "戒律的訂立者",
		desc_world = "所在艦隊中包含皇家幸運號時，進入戰鬥後發射一輪特殊彈幕，每場戰鬥一次",
		next_id = 0,
		exp = 0,
		prev_id = 960052,
		desc = "所在艦隊中包含皇家幸運號時，進入戰鬥後發射一輪特殊彈幕，每場戰鬥一次",
		id = 960053,
		icon = "96005",
		lv = 1,
		effect_tactic = {
			9600511,
			9600512,
			9600513
		},
		effect_tactic_world = {
			99600511,
			9600512,
			9600513
		}
	}
	pg.base.commander_skill_template[960061] = {
		name = "未了的傳奇",
		desc_world = "依據指揮喵戰術屬性，提高所在艦隊中戰列、戰巡、航戰、風帆M的炮擊屬性(效果中)",
		next_id = 960062,
		exp = 0,
		prev_id = 0,
		desc = "依據指揮喵戰術屬性，提高所在艦隊中戰列、戰巡、航戰、風帆M的炮擊屬性(效果中)",
		id = 960061,
		icon = "96006",
		lv = 1,
		effect_tactic = {
			9600611,
			9600612,
			9600613
		},
		effect_tactic_world = {
			9600611,
			9600612,
			9600613
		}
	}
	pg.base.commander_skill_template[960062] = {
		name = "未了的傳奇",
		desc_world = "與主力艦隊交戰時，依據指揮喵戰術屬性，提高所在艦隊中戰列、正航、風帆M的命中屬性(效果中)",
		next_id = 960063,
		exp = 0,
		prev_id = 960061,
		desc = "與主力艦隊交戰時，依據指揮喵戰術屬性，提高所在艦隊中戰列、正航、風帆M的命中屬性(效果中)",
		id = 960062,
		icon = "96006",
		lv = 1,
		effect_tactic = {
			9600611,
			9600612,
			9600613
		},
		effect_tactic_world = {
			9600611,
			9600612,
			9600613
		}
	}
	pg.base.commander_skill_template[960063] = {
		name = "未了的傳奇",
		desc_world = "所在艦隊中包含安妮女王復仇號時，進入戰鬥後發射一輪特殊彈幕，每場戰鬥一次",
		next_id = 0,
		exp = 0,
		prev_id = 960062,
		desc = "所在艦隊中包含安妮女王復仇號時，進入戰鬥後發射一輪特殊彈幕，每場戰鬥一次",
		id = 960063,
		icon = "96006",
		lv = 1,
		effect_tactic = {
			9600611,
			9600612,
			9600613
		},
		effect_tactic_world = {
			9600611,
			9600612,
			9600613
		}
	}
	pg.base.commander_skill_template[960071] = {
		name = "紳士海盜的傳說",
		desc_world = "作為參謀位指揮喵時，依據指揮喵後勤屬性，提高所在艦隊中輕巡、重巡、超巡、風帆V的機動屬性(效果小)",
		next_id = 960072,
		exp = 0,
		prev_id = 0,
		desc = "依據指揮喵後勤屬性，降低所在艦隊遭遇伏擊的機率",
		id = 960071,
		icon = "96007",
		lv = 1,
		effect_tactic = {
			9600711,
			9600712,
			9600713
		},
		effect_tactic_world = {
			99600711,
			9600712,
			9600713
		}
	}
end)()
(function ()
	pg.base.commander_skill_template[960072] = {
		name = "紳士海盜的傳說",
		desc_world = "作為參謀位指揮喵時，依據指揮喵後勤屬性，提高先鋒艦隊的炮擊屬性(效果小)",
		next_id = 960073,
		exp = 0,
		prev_id = 960071,
		desc = "作為參謀位指揮喵時，依據指揮喵後勤屬性，提高先鋒艦隊的炮擊屬性(效果小)",
		id = 960072,
		icon = "96007",
		lv = 1,
		effect_tactic = {
			9600711,
			9600712,
			9600713
		},
		effect_tactic_world = {
			99600711,
			9600712,
			9600713
		}
	}
	pg.base.commander_skill_template[960073] = {
		name = "紳士海盜的傳說",
		desc_world = "艦隊旗艦及先鋒領艦在戰鬥中受到傷害，導致耐久低於50%後，為其恢復5%耐久（每場戰鬥1次）",
		next_id = 0,
		exp = 0,
		prev_id = 960072,
		desc = "艦隊旗艦及先鋒領艦在戰鬥中受到傷害，導致耐久低於50%後，為其恢復5%耐久（每場戰鬥1次）",
		id = 960073,
		icon = "96007",
		lv = 1,
		effect_tactic = {
			9600711,
			9600712,
			9600713
		},
		effect_tactic_world = {
			99600711,
			9600712,
			9600713
		}
	}
end)()
