slot0 = class("NewEducateGetRankCommand", pm.SimpleCommand)
slot1 = 100
slot2 = 5
...
slot0.execute = function(slot0, slot1)
	...
	slot9 = function(slot0)
		...
		slot1:Send(18203, {
			...
			slot3 = getProxy(BayProxy):getShipById(slot1.character)
			uv0 = PowerRank.New({
				user_id = slot1.id,
				point = slot0.point,
				name = slot1.name .. "|" .. getProxy(NewEducateProxy):GetCurChar():GetCallName(),
				lv = slot1.level,
	...
	seriesAsync(slot10, function ()
		...
		uv1:sendNotification(GAME.NEW_EDUCATE_GET_RANK_DONE, {
			type = uv3,
			tbId = uv2,
			list = uv0,
			playerInfo = uv4
