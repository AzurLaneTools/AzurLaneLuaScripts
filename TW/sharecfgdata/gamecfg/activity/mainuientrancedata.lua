return {
	...
	},
	{
		...
		Tip = "tip",
		UpdateButton = function (slot0, slot1)
			slot3 = getProxy(ActivityProxy):getActivityByType(ActivityConst.ACTIVITY_TYPE_SHOP_PROGRESS_REWARD) and not slot2:isEnd()

			setActive(slot1, slot3)

			if slot3 then
				setActive(slot1.Find(slot1, "Tip"), AmusementParkShopPage.GetActivityShopTip())
				onButton(slot0, slot1, function ()
					uv0:emit(MainUIMediator.GO_SUBLAYER, Context.New({
