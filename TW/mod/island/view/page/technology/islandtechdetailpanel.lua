...
slot0.OnShipSelected = function(slot0, slot1)
	if getProxy(IslandProxy):GetIsland():GetCharacterAgency():GetShipById(slot1):GetCurrentEnergy() < math.max(math.floor(pg.island_formula[slot0.showTechVO:GetFormulaId()].stamina_cost * (1 - IslandProductCostHelper.GetReducePercentInPlace(slot1, slot0.placeId))), 1) then
		pg.TipsMgr.GetInstance():ShowTips(i18n("island_production_cost_notenough"))
