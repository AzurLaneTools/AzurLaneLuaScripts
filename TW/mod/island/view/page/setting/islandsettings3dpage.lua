slot0 = class("IslandSettings3DPage", import("view.base.BaseSubView"))
...
slot0.OnBindEvent = function(slot0)
	slot0:bind(IslandSettingsOtherGraphicsPanle.EVT_UPDTAE, function ()
		if uv0:GetPanel(IslandSettingsOtherGraphicsPanle) then
	...
	slot0:bind(IslandSettingsGraphicsPanle.EVT_UPDTAE, function ()
		if uv0:GetPanel(IslandSettingsGraphicsPanle) then
			slot0:OnUpdate()
		end
	end)
end
...
slot0.GetPanels = function(slot0)
	...
	if slot0:NeedAdjustScreen() then
		table.insert(slot1, 1, IslandSettingsAdjustScreenPanle)
	end
