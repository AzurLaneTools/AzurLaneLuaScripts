return {
	id = "WEICENGHUNHE10",
	mode = 2,
	once = true,
	fadeType = 1,
	fadein = 1.5,
	scripts = {
		{
			side = 2,
			nameColor = "#a9f548",
			bgm = "hunhe-battle",
			dir = 1,
			say = "巴拿馬要塞東側·外海",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 107170,
			side = 2,
			nameColor = "#a9f548",
			dir = 1,
			say = "一邊微笑著，一邊把敵人全都轟飛了…",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			nameColor = "#a9f548",
			side = 2,
			actor = 103240,
			dir = 1,
			say = "哎？我的實戰經驗不是很豐富啦，剛剛有哪裡做得不對嗎…？",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 107170,
			side = 2,
			nameColor = "#a9f548",
			dir = 1,
			say = "剛剛做的做得很出色。這麼近距離看下來，巴爾的摩級的性能果然很優秀。用性能彌補了實戰經驗的不足嗎？",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			nameColor = "#a9f548",
			side = 2,
			actor = 103240,
			dir = 1,
			say = "哼哼，性能上來說我可是很自信的哦~而且，我可是進行過超多超嚴格的演習磨練的！",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 6,
			nameColor = "#a9f548",
			side = 2,
			actor = 103240,
			dir = 1,
			say = "本來想著終於有機會和巴爾的摩並肩作戰了呢…",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 107170,
			side = 2,
			nameColor = "#a9f548",
			dir = 1,
			say = "…很擔心她嗎？",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 1,
			nameColor = "#a9f548",
			side = 2,
			actor = 103240,
			dir = 1,
			say = "當然，怎麼可能不擔心呢……",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			nameColor = "#a9f548",
			side = 2,
			say = "布雷默頓面色凝重了一下，不過隨即又露出了燦爛的笑容",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			nameColor = "#a9f548",
			side = 2,
			actor = 103240,
			dir = 1,
			say = "不過巴爾的摩比我還要強哦，現在肯定已經在塞壬家裡鬧得天翻地覆了吧！",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 107170,
			side = 2,
			nameColor = "#a9f548",
			dir = 1,
			say = "我也相信她們一定會沒事的。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 107170,
			nameColor = "#a9f548",
			side = 2,
			dir = 1,
			say = "嗯？海霧…？",
			effects = {
				{
					active = true,
					name = "miwu_01"
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = true,
			side = 2,
			actor = 106550,
			nameColor = "#a9f548",
			dir = 1,
			say = "……【雜音】…聽得到嗎…【雜音】…",
			soundeffect = "event:/ui/noice",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = true,
			side = 2,
			actor = 106550,
			nameColor = "#a9f548",
			dir = 1,
			say = "…【雜音】…塞壬隨著海霧…【雜音】…新一輪進攻",
			soundeffect = "event:/ui/noice",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			paintingNoise = true,
			side = 2,
			actor = 106550,
			nameColor = "#a9f548",
			dir = 1,
			say = "…遠離海霧…【雜音】…返回近海…",
			soundeffect = "event:/ui/noice",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "通訊器",
			nameColor = "#a9f548",
			say = "……………………",
			dir = 1,
			soundeffect = "event:/ui/noice",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 107170,
			side = 2,
			nameColor = "#a9f548",
			dir = 1,
			say = "通訊中斷了…被這片突然出現的海霧干擾了嗎？",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			nameColor = "#a9f548",
			bgName = "bg_banama_3",
			actorName = "布雷默頓",
			dir = 1,
			say = "真是步步緊逼，一刻也不得閒啊…大家都在各處戰鬥，相互擔心也解決不了問題。",
			effects = {
				{
					active = false,
					name = "miwu_01"
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "布雷默頓",
			bgName = "bg_banama_3",
			nameColor = "#a9f548",
			dir = 1,
			say = "我們現在能做的就是，保護好巴拿馬要塞，挫敗塞壬的襲擊計劃。然後——相信同伴吧。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "碉堡山",
			bgName = "bg_banama_3",
			nameColor = "#a9f548",
			dir = 1,
			say = "說得好！雖然戰場歷練得少，不過到底還是白鷹優秀的戰士。很榮幸與妳並肩戰鬥，巴爾的摩級的布雷默頓",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "bg_banama_3",
			nameColor = "#a9f548",
			dir = 1,
			actorName = "布雷默頓",
			say = "彼此彼此~很榮幸能同妳並肩戰鬥，艾塞克斯級的碉堡山小姐~",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "bg_banama_3",
			nameColor = "#a9f548",
			dir = 1,
			actorName = "布雷默頓",
			say = "（巴爾的摩…一定要平安無事啊）",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		}
	}
}
