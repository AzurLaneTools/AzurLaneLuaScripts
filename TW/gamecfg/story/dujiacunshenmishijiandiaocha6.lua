return {
	fadeOut = 1.5,
	mode = 2,
	id = "DUJIACUNSHENMISHIJIANDIAOCHA6",
	once = true,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_504",
			actorName = "劇中旁白",
			stopbgm = true,
			paintingNoise = true,
			bgm = "cw-story",
			actor = 305130,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "白霧散去後，旅人們發現自己來到了木船之上。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = false,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = false,
				alpha = {
					1,
					0
				}
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_504",
			factiontag = "傳奇調查員（自稱）",
			dir = 1,
			actor = 101490,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "………這是，第二幕嗎？！",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			paintingNoise = true,
			side = 2,
			bgName = "star_level_bg_504",
			hidePaintObj = true,
			actor = 305130,
			actorName = "劇中旁白",
			nameColor = "#A9F548FF",
			say = "沒錯，現在是劇目的第二幕喲。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301810,
			side = 2,
			bgName = "star_level_bg_504",
			factiontag = "和噗噗在一起",
			dir = 1,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "真的好神奇……前一秒還在雪山上，現在已經在驚濤駭浪中了。",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_504",
			paintingNoise = true,
			actorName = "劇中旁白",
			actor = 305130,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "狂風暴雨，巨浪迭起，旅人們感覺自己似乎有些暈船了。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 501020,
			side = 2,
			bgName = "star_level_bg_504",
			factiontag = "尋找本日的刺激",
			dir = 1,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "不，我完全不暈。",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 101490,
			side = 2,
			bgName = "star_level_bg_504",
			factiontag = "傳奇調查員（自稱）",
			dir = 1,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "畢竟，我們是艦船，會暈船的話也太奇怪了。",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			paintingNoise = true,
			side = 2,
			bgName = "star_level_bg_504",
			hidePaintObj = true,
			actor = 305130,
			actorName = "劇中旁白",
			nameColor = "#A9F548FF",
			say = "咳……雖然狂風暴雨，巨浪迭起，但旅人們依然感覺神清氣爽！",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_504",
			paintingNoise = true,
			actorName = "劇中旁白",
			actor = 305130,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "只是，旅人在大海面前始終只是渺小的存在。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			paintingNoise = true,
			side = 2,
			bgName = "star_level_bg_504",
			hidePaintObj = true,
			actor = 305130,
			actorName = "劇中旁白",
			nameColor = "#A9F548FF",
			say = "幾艘渺小的漁船，在海上的大妖怪面前，又算得了什麼呢。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "海坊主",
			bgName = "star_level_bg_504",
			factiontag = "有實力的年長者",
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "貢品！老朽需要更多的貢品！",
			icon = {
				scale = 8,
				image = "Props/story_haifangzhu",
				pos = {
					0,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "海坊主",
			bgName = "star_level_bg_504",
			factiontag = "有實力的年長者",
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "把妳們的食物和財寶都交出來！",
			icon = {
				scale = 8,
				image = "Props/story_haifangzhu",
				pos = {
					0,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "star_level_bg_504",
			factiontag = "傳奇調查員（自稱）",
			dir = 1,
			actor = 101490,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "唔啊，這妖怪竟然還要收過路費？太惡劣了！",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301290,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_504",
			hidePaintObj = true,
			side = 2,
			actorName = "善良的漁民",
			say = "唉，惡劣又有什麼辦法啊。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_504",
			hidePaintObj = true,
			actor = 301290,
			actorName = "善良的漁民",
			nameColor = "#A9F548FF",
			say = "如果海坊主大人對貢品不滿意，暴雨就不會停。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301290,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_504",
			hidePaintObj = true,
			side = 2,
			actorName = "善良的漁民",
			say = "暴雨不停，就沒辦法出海，以後還怎麼生活啊！",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 501020,
			side = 2,
			bgName = "star_level_bg_504",
			factiontag = "尋找本日的刺激",
			dir = 1,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "連老實的漁民都欺負？不可饒恕！我們去打倒它吧。",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 101490,
			side = 2,
			bgName = "star_level_bg_504",
			factiontag = "傳奇調查員（自稱）",
			dir = 1,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "贊同！再厲害的妖怪，也抵擋不住艦炮的厲害！",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 302210,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_504",
			hidePaintObj = true,
			side = 2,
			actorName = "勇敢的漁民",
			say = "是啊，忍氣吞聲算什麼好漢！",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_504",
			hidePaintObj = true,
			actor = 302210,
			actorName = "勇敢的漁民",
			nameColor = "#A9F548FF",
			say = "我不服！憑什麼要把我們辛辛苦苦賺來的錢財和食物送給妖怪！",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_504",
			hidePaintObj = true,
			actor = 301290,
			actorName = "善良的漁民",
			nameColor = "#A9F548FF",
			say = "冷靜，冷靜點……妳不服又能怎麼辦呢。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301290,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_504",
			hidePaintObj = true,
			side = 2,
			actorName = "善良的漁民",
			say = "除了獻上貢品，我們無能為力啊。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 302210,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_504",
			hidePaintObj = true,
			side = 2,
			actorName = "勇敢的漁民",
			say = "哼！今天，我這個小小的漁夫就要替天行道！消滅這個興風作浪的可惡妖怪！",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			paintingNoise = true,
			side = 2,
			bgName = "star_level_bg_504",
			hidePaintObj = true,
			actor = 305130,
			actorName = "劇中旁白",
			nameColor = "#A9F548FF",
			say = "勇敢的漁民手舉魚叉，氣勢洶洶地跳下了船。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_504",
			paintingNoise = true,
			actorName = "劇中旁白",
			actor = 305130,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "下一個瞬間，本就不平穩的海面突然掀起巨浪，將勇敢的漁民吞入了漆黑的海水之下。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_504",
			hidePaintObj = true,
			actor = 301290,
			actorName = "善良的漁民",
			nameColor = "#A9F548FF",
			say = "唉~又一個以卵擊石的傻瓜！這又是何必呢？",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301290,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_504",
			hidePaintObj = true,
			side = 2,
			actorName = "善良的漁民",
			say = "所以……聽我說，千萬不能下船啊！",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 5,
			side = 2,
			bgName = "star_level_bg_504",
			factiontag = "傳奇調查員（自稱）",
			dir = 1,
			actor = 101490,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "……只要下海就會被劇情殺嗎，看來直接走過去的方案行不通了。",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_504",
			factiontag = "和噗噗在一起",
			dir = 1,
			actor = 301810,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "……假意去上繳貢品，等到妖怪接近的時候，將其消滅，如何？",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 501020,
			side = 2,
			bgName = "star_level_bg_504",
			factiontag = "尋找本日的刺激",
			dir = 1,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "但是老實去交貢品真的能成功通關嗎？",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_504",
			factiontag = "獨占欲強的小惡魔",
			dir = 1,
			actor = 301890,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "快看！那條船上的人把東西倒進海裡了！",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 305140,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_504",
			hidePaintObj = true,
			side = 2,
			actorName = "認真的漁民",
			say = "海坊主大人！這是給您獻上的貢品！希望您能保佑之後的日子風調雨順！",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "海坊主",
			bgName = "star_level_bg_504",
			factiontag = "有實力的年長者",
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "……大膽人類，這麼一點貢品是打算戲弄老朽嗎？",
			icon = {
				scale = 8,
				image = "Props/story_haifangzhu",
				pos = {
					0,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_504",
			paintingNoise = true,
			actorName = "劇中旁白",
			actor = 305130,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "海坊主似乎對於貢品的數量頗為不滿。一道巨浪襲來，認真的漁民也消失在漆黑的海水下。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_504",
			hidePaintObj = true,
			actor = 301290,
			actorName = "善良的漁民",
			nameColor = "#A9F548FF",
			say = "唉……看來我們帶來的貢品根本不夠啊。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301290,
			nameColor = "#A9F548FF",
			bgName = "star_level_bg_504",
			hidePaintObj = true,
			side = 2,
			actorName = "善良的漁民",
			say = "老天啊~難道真的只能這樣等死了嗎？",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 501020,
			side = 2,
			bgName = "star_level_bg_504",
			factiontag = "尋找本日的刺激",
			dir = 1,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "既不能下船給它一炮，也不能直接交貢品……怎麼辦？",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301890,
			side = 2,
			bgName = "star_level_bg_504",
			factiontag = "獨占欲強的小惡魔",
			dir = 1,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "如果把大家的貢品都湊在一起呢？",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_504",
			factiontag = "獨占欲強的小惡魔",
			dir = 1,
			actor = 301890,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "然後趁著那個妖怪享受貢品的時候給它來上一炮！",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			hideOther = true,
			actorName = "撫順&布里斯托爾",
			bgName = "star_level_bg_504",
			hidePaintObj = true,
			actor = 501020,
			nameColor = "#A9F548FF",
			side = 2,
			say = "天才————！",
			dialogShake = {
				speed = 0.08,
				x = 15,
				number = 2
			},
			subActors = {
				{
					actor = 101490,
					hidePaintObj = true,
					pos = {
						x = 1100
					}
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 1,
			side = 2,
			bgName = "star_level_bg_504",
			hidePaintObj = true,
			actor = 301290,
			actorName = "善良的漁民",
			nameColor = "#A9F548FF",
			say = "（……天才！）",
			dialogShake = {
				speed = 0.08,
				x = 15,
				number = 2
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_504",
			paintingNoise = true,
			actorName = "劇中旁白",
			actor = 305130,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "一筐接一筐，四周漁船上的貢品全部集中到了一處，連海坊主也不禁投來了貪婪的目光。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = true,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = true,
				alpha = {
					1,
					0
				}
			}
		},
		{
			side = 2,
			actorName = "海坊主",
			bgName = "star_level_bg_504",
			factiontag = "有實力的年長者",
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "這樣的貢品還算像話。還不快速速獻上，供老朽享用？！",
			icon = {
				scale = 8,
				image = "Props/story_haifangzhu",
				pos = {
					0,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_504",
			paintingNoise = true,
			actorName = "劇中旁白",
			actor = 305130,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "一邊說著，海坊主一邊貪婪地向著小船的方位緩緩靠近。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_504",
			factiontag = "傳奇調查員（自稱）",
			dir = 1,
			actor = 101490,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "太好了，它上鉤了……",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 101490,
			side = 2,
			bgName = "star_level_bg_504",
			factiontag = "傳奇調查員（自稱）",
			dir = 1,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "初月，投一點貢品到水裡去，吸引它離得再近點。",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			actor = 301890,
			side = 2,
			bgName = "star_level_bg_504",
			factiontag = "獨占欲強的小惡魔",
			dir = 1,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "好的，包在我身上~",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_504",
			factiontag = "獨占欲強的小惡魔",
			dir = 1,
			actor = 301890,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "我看看。接下來就扔這一筐…………哇啊啊啊！！！",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.1,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_504",
			factiontag = "獨占欲強的小惡魔",
			dir = 1,
			actor = 301890,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "筐裡裝的是什麼啊啊啊啊啊！這、這根本不是魚啊啊啊啊！！！",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "shake",
					y = 45,
					delay = 0,
					dur = 0.15,
					x = 0,
					number = 2
				}
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_504",
			paintingNoise = true,
			actorName = "劇中旁白",
			actor = 305130,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "啊……那些好像是，鬼火。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_504",
			factiontag = "獨占欲強的小惡魔",
			dir = 1,
			actor = 301890,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "鬼火？！！在動！在動啊——！",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "move",
					y = 2000,
					delay = 0.5,
					dur = 0.4,
					x = 0
				}
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_504",
			paintingNoise = true,
			actorName = "劇中旁白",
			actor = 305130,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "……嗯，也許是太過害怕的緣故，其中一位旅人小小的身軀中爆發出了無窮力量。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_504",
			paintingNoise = true,
			actorName = "劇中旁白",
			actor = 305130,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "只見她縱身一躍，從空中跳到了另一條船上，然而等待她卻是——",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_504",
			factiontag = "獨占欲強的小惡魔",
			dir = 1,
			actor = 301890,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "這……這裡也有那些東西，哇啊啊啊————！",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "move",
					y = 2000,
					delay = 0.5,
					dur = 0.4,
					x = 0
				}
			}
		},
		{
			paintingNoise = true,
			side = 2,
			bgName = "star_level_bg_504",
			hidePaintObj = true,
			actor = 305130,
			actorName = "劇中旁白",
			nameColor = "#A9F548FF",
			say = "一艘船接著一艘船，直到跳到第八艘時，旅人發現自己已經來到了海坊主的面前。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			side = 2,
			actorName = "海坊主",
			bgName = "star_level_bg_504",
			factiontag = "有實力的年長者",
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "這……難道就是傳說中的秘技，八船跳……！",
			icon = {
				scale = 8,
				image = "Props/story_haifangzhu",
				pos = {
					0,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_504",
			factiontag = "獨占欲強的小惡魔",
			dir = 1,
			actor = 301890,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "（驚恐地揮刀——！）",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			action = {
				{
					type = "move",
					y = 2000,
					delay = 0.5,
					dur = 0.4,
					x = 0
				}
			}
		},
		{
			expression = 2,
			side = 2,
			bgName = "star_level_bg_504",
			paintingNoise = true,
			actorName = "劇中旁白",
			actor = 305130,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "就這樣，海坊主被消滅了，大海恢復了平靜，漁民們也終於過上了幸福的生活。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			flashout = {
				black = false,
				dur = 0.5,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 0.5,
				dur = 0.5,
				black = false,
				alpha = {
					1,
					0
				}
			}
		},
		{
			paintingNoise = true,
			side = 2,
			bgName = "star_level_bg_504",
			hidePaintObj = true,
			actor = 305130,
			actorName = "劇中旁白",
			nameColor = "#A9F548FF",
			say = "可喜可賀，可喜可賀。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 4,
			side = 2,
			bgName = "star_level_bg_504",
			factiontag = "獨占欲強的小惡魔",
			dir = 1,
			actor = 301890,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "欸……這樣就結束了嗎？",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		},
		{
			expression = 3,
			side = 2,
			bgName = "star_level_bg_504",
			factiontag = "獨占欲強的小惡魔",
			dir = 1,
			actor = 301890,
			nameColor = "#A9F548FF",
			hidePaintObj = true,
			say = "欸~~~~~~~~~？！",
			painting = {
				alpha = 0.3,
				time = 1
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			}
		}
	}
}
