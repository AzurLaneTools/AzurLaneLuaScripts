return {
	fadeOut = 1.5,
	mode = 2,
	id = "HONGCAIDEZHONGMUQU1",
	once = true,
	fadeType = 2,
	fadein = 1.5,
	scripts = {
		{
			say = "北方聯合控制區·指揮艦內",
			side = 2,
			bgName = "bg_endingsong_1",
			dir = 1,
			bgmDelay = 2,
			bgm = "theme-highseasfleet",
			flashout = {
				black = true,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = true,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_endingsong_1",
			say = "從「星海」之中歸來已經過數日……但在短短數小時內經歷的震撼卻沒有絲毫減少。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_endingsong_1",
			say = "作為秘密計劃來說，這樣的規模也太誇張了點……",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_endingsong_1",
			say = "據說是一項戰爭開始之前就定下的計劃，居然能隱瞞的如此完美……",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_endingsong_1",
			say = "不光是我，碧藍航線指揮部、NY司令部、甚至議會之中有多少人知道都是未知數。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_endingsong_1",
			say = "在這些人知情人中，沒有被其表象迷惑，真正了解到其深層內容的又有多少……",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_endingsong_1",
			say = "在如此秘密的地方做著如此秘密的事，甚至用一串秘密來掩蓋更大的秘密。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_endingsong_1",
			say = "……這樣一來，餘燼飛龍的懷疑也算是揭開了一層。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_endingsong_1",
			say = "現在推出的諸多具有跳躍性的研究產物其實都是服務於星海計劃整體的一部分。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_endingsong_1",
			say = "塞壬確實不會提供它們不想讓我們進行逆向解析的東西。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_endingsong_1",
			say = "直接對塞壬的工程進行研究也過於危險，甚至很容易變為針對塞壬的依賴。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_endingsong_1",
			say = "但是如果反過來呢……",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_endingsong_1",
			say = "這確實是一條不違背碧藍航線理念基礎的全新的道路。既沒有陷入鐵血重櫻所選的路線，和北方聯合也有所不同……",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "喬治亞",
			bgName = "bg_endingsong_1",
			nameColor = "#a9f548",
			dir = 1,
			say = "咳……指揮官，你有在聽嗎？",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_endingsong_1",
			say = "啊…………",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			options = {
				{
					content = "在聽、在聽……",
					flag = 1
				},
				{
					content = "不好意思，恍神了。",
					flag = 2
				}
			}
		},
		{
			side = 2,
			actorName = "喬治亞",
			bgName = "bg_endingsong_1",
			nameColor = "#a9f548",
			dir = 1,
			optionFlag = 1,
			say = "哦？那麼就請指揮官談一談對於現狀的看法吧？",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			bgName = "bg_endingsong_1",
			dir = 1,
			optionFlag = 1,
			say = "……不好意思，其實是恍神了。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "喬治亞",
			bgName = "bg_endingsong_1",
			nameColor = "#a9f548",
			dir = 1,
			say = "唉……",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "埃爾德里奇",
			bgName = "bg_endingsong_1",
			nameColor = "#a9f548",
			dir = 1,
			say = "指揮官，睏了，就睡吧。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "埃爾德里奇",
			bgName = "bg_endingsong_1",
			nameColor = "#a9f548",
			dir = 1,
			say = "埃爾德里奇，去拿幾個墊子來。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_endingsong_2",
			say = "還是算了，在工作中睡覺可不是值得鼓勵的行為……",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_endingsong_2",
			say = "麻煩把剛才的內容再重複一遍吧，喬治亞。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "喬治亞",
			bgName = "bg_endingsong_2",
			nameColor = "#a9f548",
			dir = 1,
			say = "沒問題。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "喬治亞",
			bgName = "bg_endingsong_2",
			nameColor = "#a9f548",
			dir = 1,
			say = "如您所見，參加作戰的各艦隊依然在集結途中。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_endingsong_2",
			say = "喬治亞用手指著展示在牆上的時局圖開始了說明。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_endingsong_2",
			say = "只是與恍神之前相比，時局圖似乎已經更新了數個版本……",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "喬治亞",
			bgName = "bg_endingsong_2",
			nameColor = "#a9f548",
			dir = 1,
			say = "這裡是我們的旗艦艦隊，兩側與後方是與我們一同行動的白鷹打擊分隊，再後方是預備艦隊。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "喬治亞",
			bgName = "bg_endingsong_2",
			nameColor = "#a9f548",
			dir = 1,
			say = "這些艦隊一起構成了本次行動的白鷹主力打擊群。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "喬治亞",
			bgName = "bg_endingsong_2",
			nameColor = "#a9f548",
			dir = 1,
			say = "除此之外，紐澤西帶領的NA海域艦隊打擊群正在前往集結區域的途中。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "喬治亞",
			bgName = "bg_endingsong_2",
			nameColor = "#a9f548",
			dir = 1,
			say = "預計，我們會先與蘇維埃俄羅斯帶領的的兩支北方聯合極地艦隊分艦隊匯合。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_endingsong_2",
			say = "北方聯合抽調的艦隊比預想中額規模要大……極地的防禦不要緊嗎？",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "喬治亞",
			bgName = "bg_endingsong_2",
			nameColor = "#a9f548",
			dir = 1,
			say = "根據她們所說，只要這裡的戰鬥能夠速戰速決，就不要緊。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			},
			options = {
				{
					content = "皇家艦隊的情況呢？",
					flag = 1
				},
				{
					content = "重櫻艦隊的情況呢？",
					flag = 2
				},
				{
					content = "自由鳶尾艦隊的情況呢？",
					flag = 3
				},
				{
					content = "維希教廷艦隊的情況呢？",
					flag = 4
				},
				{
					content = "薩丁帝國艦隊的情況呢？",
					flag = 5
				},
				{
					content = "東煌艦隊的情況呢？",
					flag = 6
				}
			}
		},
		{
			side = 2,
			actorName = "喬治亞",
			bgName = "bg_endingsong_2",
			nameColor = "#a9f548",
			dir = 1,
			optionFlag = 1,
			say = "根據事先約定，皇家艦隊主力代替前來支援作戰的紐澤西艦隊擔任了NA海域的清理和防衛工作。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "喬治亞",
			bgName = "bg_endingsong_2",
			nameColor = "#a9f548",
			dir = 1,
			optionFlag = 1,
			say = "目前沒有收到更新的情報，應該可以認定為一切順利。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "喬治亞",
			bgName = "bg_endingsong_2",
			nameColor = "#a9f548",
			dir = 1,
			optionFlag = 1,
			say = "不過在本次行動中我們應該不會獲得來自皇家的支援了。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "喬治亞",
			bgName = "bg_endingsong_2",
			nameColor = "#a9f548",
			dir = 1,
			optionFlag = 2,
			say = "根據情報，重櫻遠征艦隊目前依然停泊在達卡港內，沒有調動跡象。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "喬治亞",
			bgName = "bg_endingsong_2",
			nameColor = "#a9f548",
			dir = 1,
			optionFlag = 2,
			say = "不過考慮到赤城的一貫行動規律，其帶領艦隊參與本次戰役的可能性依然無法排除。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "喬治亞",
			bgName = "bg_endingsong_2",
			nameColor = "#a9f548",
			dir = 1,
			optionFlag = 3,
			say = "根據情報，自由鳶尾艦隊主力依然在阿非利加周邊執行任務，其餘分隊正在地中海中與皇家一同行動。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "喬治亞",
			bgName = "bg_endingsong_2",
			nameColor = "#a9f548",
			dir = 1,
			optionFlag = 3,
			say = "雖然很遺憾，不過正如黎胥留所說，自由鳶尾沒有空餘力量加入本次作戰行動。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "喬治亞",
			bgName = "bg_endingsong_2",
			nameColor = "#a9f548",
			dir = 1,
			optionFlag = 4,
			say = "針對維希教廷旗艦讓巴爾的情報依然稀少，關於她的狀態、她所處的位置，我們均一無所知。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "喬治亞",
			bgName = "bg_endingsong_2",
			nameColor = "#a9f548",
			dir = 1,
			optionFlag = 4,
			say = "不過根據已有情報，維希教廷的艦隊主力長期集結於土倫港內自守，目前沒有外出行動跡象。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "喬治亞",
			bgName = "bg_endingsong_2",
			nameColor = "#a9f548",
			dir = 1,
			optionFlag = 4,
			say = "況且在自由鳶尾和皇家地中海艦隊的聯合封鎖下，我們應該不用擔心維希教廷艦隊的參戰問題。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "喬治亞",
			bgName = "bg_endingsong_2",
			nameColor = "#a9f548",
			dir = 1,
			optionFlag = 5,
			say = "根據情報，薩丁帝國艦隊均分散停留在各大港口中。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "喬治亞",
			bgName = "bg_endingsong_2",
			nameColor = "#a9f548",
			dir = 1,
			optionFlag = 5,
			say = "在羅馬遇襲之後，薩丁帝國就一直在採取這樣的嚴密防守措施。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "喬治亞",
			bgName = "bg_endingsong_2",
			nameColor = "#a9f548",
			dir = 1,
			optionFlag = 5,
			say = "況且在自由鳶尾和皇家地中海艦隊的封鎖下，我們應該不用擔心薩丁帝國艦隊的參戰問題。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "喬治亞",
			bgName = "bg_endingsong_2",
			nameColor = "#a9f548",
			dir = 1,
			optionFlag = 6,
			say = "嗯……？東煌…………？",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "喬治亞",
			bgName = "bg_endingsong_2",
			nameColor = "#a9f548",
			dir = 1,
			optionFlag = 6,
			say = "指揮官，東煌在NA周邊沒有部署任何艦隊，也因此不會加入本次作戰行動。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "喬治亞",
			bgName = "bg_endingsong_2",
			nameColor = "#a9f548",
			dir = 1,
			say = "至於鐵血方面，目前已經監測到了大量鐵血艦隊的行動痕跡。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "喬治亞",
			bgName = "bg_endingsong_2",
			nameColor = "#a9f548",
			dir = 1,
			say = "其總規模遠遠超出了我們認知之中鐵血艦隊總規模的數倍。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "喬治亞",
			bgName = "bg_endingsong_2",
			nameColor = "#a9f548",
			dir = 1,
			say = "已經可以基本認定，鐵血再次使用了與此前對皇家的戰鬥中相同的干擾系統。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "喬治亞",
			bgName = "bg_endingsong_2",
			nameColor = "#a9f548",
			dir = 1,
			say = "航空編隊正在利用攻擊波分辨目標真偽，相信很快就會有結果了。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_endingsong_2",
			say = "嗯，鐵血的障眼法不可能一直生效的。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "喬治亞",
			bgName = "bg_endingsong_2",
			nameColor = "#a9f548",
			dir = 1,
			say = "除此之外，鐵血艦隊總旗艦腓特烈大帝的行蹤依然無法確定，有結果的話我會第一時間通知您的。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_endingsong_2",
			say = "嗯。喬治亞，第一次參加實戰的感覺如何？",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "喬治亞",
			bgName = "bg_endingsong_2",
			nameColor = "#a9f548",
			dir = 1,
			say = "唔……該怎麼說呢。本以為我會承擔前往一線勇往直前的工作，沒想到會來到指揮艦上兼任作戰秘書。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "喬治亞",
			bgName = "bg_endingsong_2",
			nameColor = "#a9f548",
			dir = 1,
			say = "和想像中的不太一樣？但是又沒有那麼大的區別……大概是這樣的感覺。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_endingsong_2",
			say = "妳對現在的形式怎麼看？",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "喬治亞",
			bgName = "bg_endingsong_2",
			nameColor = "#a9f548",
			dir = 1,
			say = "從表面實力對比來說，我們的勝算相當大。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "喬治亞",
			bgName = "bg_endingsong_2",
			nameColor = "#a9f548",
			dir = 1,
			say = "不過這次是由多個陣營多支艦隊組織的聯合行動，組織協調會很困難。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "喬治亞",
			bgName = "bg_endingsong_2",
			nameColor = "#a9f548",
			dir = 1,
			say = "而且鐵血的部署總給我一種說不上來的違和感……戰場局勢瞬息萬變，不到最後一刻，勝負難分。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_endingsong_2",
			say = "沒錯。輕敵是大忌，按照計劃穩步前進吧。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_endingsong_2",
			say = "放心，接下來會有發揮妳作戰實力的機會的。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_endingsong_2",
			say = "還有——————",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "喬治亞",
			bgName = "bg_endingsong_2",
			nameColor = "#a9f548",
			dir = 1,
			say = "指揮官還有什麼疑問嗎？",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_endingsong_2",
			say = "去拿墊子的埃爾德里奇怎麼還沒回來？",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			side = 2,
			actorName = "喬治亞",
			bgName = "bg_endingsong_2",
			nameColor = "#a9f548",
			dir = 1,
			say = "唔……埃爾德里奇的話，埃爾德里奇…………",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_story_task",
			say = "經過一番努力尋找，終於在艙門外不遠處找到了睡在地板上的埃爾德里奇。",
			flashout = {
				black = true,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = true,
				alpha = {
					1,
					0
				}
			},
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 101260,
			side = 2,
			bgName = "bg_story_task",
			nameColor = "#a9f548",
			dir = 1,
			say = "Zzzz……",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 199020,
			side = 2,
			bgName = "bg_story_task",
			nameColor = "#a9f548",
			dir = 1,
			say = "……根據現狀判斷，好像是埃爾德里奇在把墊子搬運過來的途中，因為太過舒服就抱著睡著了。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 199020,
			side = 2,
			bgName = "bg_story_task",
			nameColor = "#a9f548",
			dir = 1,
			say = "嗯……指揮官，你看起來還是一臉疲憊的樣子。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			actor = 199020,
			side = 2,
			bgName = "bg_story_task",
			nameColor = "#a9f548",
			dir = 1,
			say = "接下來需要集中精神的時間還長，不如也趁戰鬥開始前休息一下吧？",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_story_task",
			say = "看著舒服熟睡的埃爾德里奇，不知為何睏意也湧了上來。",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			dir = 1,
			side = 2,
			bgName = "bg_story_task",
			say = "那就恭敬不如從命，小睡片刻好了……",
			typewriter = {
				speed = 0.05,
				speedUp = 0.01
			},
			painting = {
				alpha = 0.3,
				time = 1
			}
		},
		{
			mode = 1,
			stopbgm = true,
			flashout = {
				black = true,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = true,
				alpha = {
					1,
					0
				}
			},
			sequence = {
				{
					"<size=51>54 68 65 63 6F 6F 72 64 69 6E 61 74 65 73 61 72 65 63 6C 65 61 72 2E</size>",
					1
				},
				{
					"<size=51>32 2E 39 31 34 33 2E</size>",
					2
				},
				{
					"<size=51>31 2E 37 34 32 30 2E</size>",
					3
				},
				{
					"<size=51>2D 31 2E 33 37 37 36 2E</size> ",
					4
				},
				{
					"<size=51>2D 32 33 37 36 30 32 39 31 34 33 2E</size>",
					5
				},
				{
					"<size=51>49 6E 73 69 6D 75 6C 61 74 69 6F 6E 2E</size>",
					6
				},
				{
					"<size=51>49 6E 63 6F 6E 74 61 63 74 69 6F 6E 2E</size>",
					7
				},
				{
					"<size=51>54 68 65 74 6F 77 65 72 77 61 73 63 72 65 61 74 65 64 2E</size>",
					8
				},
				{
					"<size=51>54 68 65 74 6F 77 65 72 77 61 73 64 65 73 74 72 6F 79 65 64 2E</size>",
					9
				},
				{
					"<size=51>54 68 65 74 6F 77 65 72 65 78 69 73 74 73 66 6F 72 65 76 65 72 2E</size>",
					10
				}
			}
		},
		{
			mode = 1,
			stopbgm = true,
			blackBg = true,
			flashout = {
				black = true,
				dur = 1,
				alpha = {
					0,
					1
				}
			},
			flashin = {
				delay = 1,
				dur = 1,
				black = true,
				alpha = {
					1,
					0
				}
			},
			effects = {
				{
					active = true,
					name = "HONGCAIDEZHONGMUQU"
				}
			},
			sequence = {
				{
					"",
					1
				}
			}
		}
	}
}
