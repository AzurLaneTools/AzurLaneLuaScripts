slot0 = class("MainEducateSpinePainting", import(".MainSpinePainting"))
slot1 = nil
...
slot0.OnDisplayWorld = function(slot0, slot1)
	slot2 = EducateCharWordHelper.GetExpression(slot0.ship.educateCharId, slot1)

	if expressionAnimName ~= "" then
		slot0.spinePainting:SetAction(slot2, 1)
...
slot0.CollectIdleEvents = function(slot0, slot1)
	...
	if getProxy(EventProxy):hasFinishState() and slot1 ~= "event_complete" then
		...
		if #slot2 == 0 then
			slot2 = slot0:FilterExistEvents(uv0.GetShipMainEvents(slot0.ship:getSkinId(), slot0.ship:getCVIntimacy()))
