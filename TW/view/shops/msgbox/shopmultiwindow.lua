...
slot0.InitWindow = function(slot0, slot1, slot2)
	...
	if isa(slot1, ActivityCommodity) then
		...
		if slot4 and slot5 then
			setText(slot0.timeLimitTF:Find("Text"), i18n(getProxy(ActivityProxy):getActivityById(Item.getConfigData(slot3.id).link_id):IsMaintenanceFinish() and "eventshop_time_hint" or "eventshop_time_hint2", pg.TimeMgr.GetInstance():STimeDescS(slot8.stopTime, "%m.%d")))
		end
	end
