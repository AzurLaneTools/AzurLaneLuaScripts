slot0 = class("CutFruitGameView", import("..BaseMiniGameView"))
slot0.EVENT_DIRECT = "CutFruitGameView:EVENT_DIRECT"
slot1 = 83

slot0.Ctor = function(slot0)
	uv0.super.Ctor(slot0)

	slot0._gameVo = CutFruitGameVo.New(uv1)
end
...
slot0.initEvent = function(slot0)
	...
	slot0:bind(SimpleMGEvent.STOP_TIME_STEP, function (slot0, slot1, slot2)
		uv0._gameVo:StopTimeStep(slot1)
	end)
...
slot0.initUI = function(slot0)
	...
	slot0.popUI = CutFruitGamePopUI.New(slot0._tf, slot0, slot0._gameVo)

	slot0.popUI:ClearUI()

	slot0.gameUI = CutFruitGamingUI.New(slot0._tf, slot0, slot0._gameVo)

	slot0.gameUI:Show(false)

	slot0.menuUI = CutFruitGameMenuUI.New(slot0._tf, slot0, slot0._gameVo)

	slot0.menuUI:Update()
	slot0.menuUI:Show(true)
	slot0.popUI:PopSelectUI(true)

	slot0.gameScene = CutFruitGameScene.New(slot0._tf, slot0, slot0._gameVo)
end

slot0.changeBgm = function(slot0, slot1)
	slot2 = nil

	if slot1 == CutFruitGameConst.bgm_type_default then
	...
	elseif slot1 == CutFruitGameConst.bgm_type_menu then
		slot2 = CutFruitGameConst.menu_bgm
	elseif slot1 == CutFruitGameConst.bgm_type_game then
		slot2 = CutFruitGameConst.game_bgm
	end
...
slot0.gameStart = function(slot0)
	...
	slot0:timerStart()
	slot0:changeBgm(CutFruitGameConst.bgm_type_game)
end
...
slot0.gameStep = function(slot0)
	if slot0.gameStartFlag and not slot0.gameStop and not slot0.settlementFlag then
		...
		if slot0._gameVo:GetTime() <= 0 then
			slot0:onGameOver(false)
		end
	end

	if IsUnityEditor then
		if Input.GetKeyDown(KeyCode.A) then
			slot0:emit(CutFruitGameView.EVENT_DIRECT, CutFruitGameConst.DIRECT_LEFT)
		end

		if Input.GetKeyDown(KeyCode.D) then
			slot0:emit(CutFruitGameView.EVENT_DIRECT, CutFruitGameConst.DIRECT_RIGHT)
		end

		if Input.GetKeyDown(KeyCode.W) then
			slot0:emit(CutFruitGameView.EVENT_DIRECT, CutFruitGameConst.DIRECT_UP)
		end

		if Input.GetKeyDown(KeyCode.S) then
			slot0:emit(CutFruitGameView.EVENT_DIRECT, CutFruitGameConst.DIRECT_DOWN)
		end
	end
end
...
slot0.onGameOver = function(slot0, slot1)
	...
	slot2:SetSuccess(slot1 and true or false)
	slot0._gameVo:SetSettlement(true)
	slot0.gameScene:GameOver()
