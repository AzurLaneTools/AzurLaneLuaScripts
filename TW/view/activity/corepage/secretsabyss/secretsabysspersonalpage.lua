...
slot0.OnInit = function(slot0)
	...
	onButton(slot0, slot0.randomBtn, function ()
		...
		uv0:managedTween(LeanTween.delayedCall, function ()
			uv0.personalRandomData = {}

			uv1:UpdateView(true)
			setActive(uv1.effectTF, false)
			setActive(uv1.randomBtn, uv1.unlockRandom)
		end, uv1.RANDOM_CHANGE_TIME, nil)

		if uv0.randomCallback then
			uv0:randomCallback()
		end
	end, SFX_PANEL)
...
slot0.UnlockRandom = function(slot0)
	slot0.unlockRandom = true
end

slot0.SetUpgrade = function(slot0)
	slot0.upgradeFlag = true
end

slot0.SetBossRushNode = function(slot0, slot1, slot2)
	slot0.currentBossRushNode = slot1
	slot0.lastBossRushNode = slot2
end
...
slot0.GetCurrentEvent = function(slot0)
	return slot0.currentBossRushNode
end

slot0.RegisterRandomCallback = function(slot0, slot1)
	slot0.randomCallback = slot1
end

slot0.UpdateView = function(slot0, slot1)
	slot2 = nil
	slot2 = (not slot0.upgradeFlag and #slot0:GetActivitySingleEventOption(slot0.currentBossRushNode) ~= 0 or slot0:GetActivitySingleEventOption(slot0.lastBossRushNode)) and slot0:GetActivitySingleEventOption(slot0.currentBossRushNode)
	slot0.showCfg = {}
	...
	if slot0.upgradeFlag then
		slot0.upgradeCfg = {}
		slot6 = slot0.currentBossRushNode

		for slot6, slot7 in ipairs(slot0:GetActivitySingleEventOption(slot6)) do
...
slot0.UpdateInfo = function(slot0, slot1)
	...
	slot0.lvSliderImage.fillAmount = tonumber(slot8) / uv0.config[uv0.LV_ID].random_value[2]

	if slot1 then
		uv0.personalRandomData[uv0.JOB_ID] = slot4
		uv0.personalRandomData[uv0.GUARDIAN_ID] = slot6
		uv0.personalRandomData[uv0.LV_ID] = slot8
	end
end
...
slot0.UpdateAbility = function(slot0, slot1)
	...
	elseif uv0.personalRandomData then
		for slot6, slot7 in pairs(uv0.personalRandomData) do
...
slot0.GetRollAttrInfoById = function(slot0, slot1, slot2)
	...
	if slot2 then
		...
		if uv0.personalRandomData then
			slot3 = uv0.personalRandomData[slot1]
		end
	end
...
slot0.PlayLevelAnim = function(slot0, slot1)
	...
	if slot4 then
		...
		slot6:setOnUpdate(System.Action_float(function (slot0)
			uv0.lvSliderImage.fillAmount = slot0
		end))
	else
		slot1()
	end
